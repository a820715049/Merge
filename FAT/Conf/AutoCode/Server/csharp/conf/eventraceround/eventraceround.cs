...
using EventRaceRoundConf = fat.rawdata.EventRaceRoundConf;
using rawEventRaceRound = fat.rawdata.EventRaceRound;
using rawEventRaceRoundMapABValue = fat.rawdata.EventRaceRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceRoundVisitor
    {
		...
		public const string FileName = "EventRaceRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceRoundVisitor.SetABDefault单独设定EventRaceRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceRoundVisitor.SetABFallback单独设定EventRaceRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceRound>>(()=> new PBC::MapField<int, rawEventRaceRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceRoundMap;
			var abMap = v.EventRaceRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceRoundMap;
			if (v.EventRaceRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceRoundConf cc) {
			foreach(KeyValuePair <string, rawEventRaceRoundMapABValue> tagEntry in cc.EventRaceRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceRoundMapABValue();
					cc.EventRaceRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceRound> entry in cc.EventRaceRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceRound> entry in tagEntry.Value.EventRaceRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceRoundMapAB != null)
			{
				foreach (var entry in cc.EventRaceRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceRoundMap.MergeFrom(cis);
			Patch(updating.EventRaceRoundMap);
			updating.EventRaceRoundSortedKeys = genSortedKeys(updating.EventRaceRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceRoundConf();
            tmp.MergeFrom(cis);
			updating.EventRaceRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceRound> filter,string tag=""){
		...
		public static rawEventRaceRound MustGet(int key,string tag="") {
		...
		public static rawEventRaceRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceRound> GetByFilter(Func<rawEventRaceRound, bool> filterFunc,string tag="") {
			List<rawEventRaceRound> ret = new List<rawEventRaceRound>();
			foreach(KeyValuePair <int, rawEventRaceRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceRound? GetOneByFilter(Func<rawEventRaceRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceRound data) 
		{
			return data.Id;			
		}
    }
