...
using EventMiniBoardMultiConf = fat.rawdata.EventMiniBoardMultiConf;
using rawEventMiniBoardMulti = fat.rawdata.EventMiniBoardMulti;
using rawEventMiniBoardMultiMapABValue = fat.rawdata.EventMiniBoardMultiMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardMultiVisitor
    {
		...
		public const string FileName = "EventMiniBoardMultiConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardMultiVisitor.SetABDefault单独设定EventMiniBoardMulti的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardMultiVisitor.SetABFallback单独设定EventMiniBoardMulti的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoardMulti>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoardMulti>>(()=> new PBC::MapField<int, rawEventMiniBoardMulti>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoardMulti>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardMultiMap;
			var abMap = v.EventMiniBoardMultiMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardMultiMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardMultiMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoardMulti> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardMultiMap;
			if (v.EventMiniBoardMultiMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardMultiMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardMultiMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardMultiConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardMultiMapABValue> tagEntry in cc.EventMiniBoardMultiMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardMultiMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardMultiMapABValue();
					cc.EventMiniBoardMultiMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardMultiMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoardMulti> entry in cc.EventMiniBoardMultiMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoardMulti> entry in tagEntry.Value.EventMiniBoardMultiMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoardMulti> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardMultiConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardMultiMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardMultiMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardMultiMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardMultiMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardMultiMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardMultiMap);
			updating.EventMiniBoardMultiSortedKeys = genSortedKeys(updating.EventMiniBoardMultiMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardMultiConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardMultiMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardMultiSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoardMulti> filter,string tag=""){
		...
		public static rawEventMiniBoardMulti MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoardMulti? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoardMulti> GetByFilter(Func<rawEventMiniBoardMulti, bool> filterFunc,string tag="") {
			List<rawEventMiniBoardMulti> ret = new List<rawEventMiniBoardMulti>();
			foreach(KeyValuePair <int, rawEventMiniBoardMulti> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoardMulti? GetOneByFilter(Func<rawEventMiniBoardMulti, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoardMulti> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoardMulti data) 
		{
			return data.Id;			
		}
    }
