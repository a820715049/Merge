...
using EventDecorateLevelConf = fat.rawdata.EventDecorateLevelConf;
using rawEventDecorateLevel = fat.rawdata.EventDecorateLevel;
using rawEventDecorateLevelMapABValue = fat.rawdata.EventDecorateLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDecorateLevelVisitor
    {
		...
		public const string FileName = "EventDecorateLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDecorateLevelVisitor.SetABDefault单独设定EventDecorateLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDecorateLevelVisitor.SetABFallback单独设定EventDecorateLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDecorateLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDecorateLevel>>(()=> new PBC::MapField<int, rawEventDecorateLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDecorateLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDecorateLevelMap;
			var abMap = v.EventDecorateLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDecorateLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDecorateLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDecorateLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDecorateLevelMap;
			if (v.EventDecorateLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDecorateLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDecorateLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDecorateLevelConf cc) {
			foreach(KeyValuePair <string, rawEventDecorateLevelMapABValue> tagEntry in cc.EventDecorateLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDecorateLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDecorateLevelMapABValue();
					cc.EventDecorateLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDecorateLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDecorateLevel> entry in cc.EventDecorateLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDecorateLevel> entry in tagEntry.Value.EventDecorateLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDecorateLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDecorateLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDecorateLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDecorateLevelMapAB != null)
			{
				foreach (var entry in cc.EventDecorateLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDecorateLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDecorateLevelMap.MergeFrom(cis);
			Patch(updating.EventDecorateLevelMap);
			updating.EventDecorateLevelSortedKeys = genSortedKeys(updating.EventDecorateLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDecorateLevelConf();
            tmp.MergeFrom(cis);
			updating.EventDecorateLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDecorateLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDecorateLevel> filter,string tag=""){
		...
		public static rawEventDecorateLevel MustGet(int key,string tag="") {
		...
		public static rawEventDecorateLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDecorateLevel> GetByFilter(Func<rawEventDecorateLevel, bool> filterFunc,string tag="") {
			List<rawEventDecorateLevel> ret = new List<rawEventDecorateLevel>();
			foreach(KeyValuePair <int, rawEventDecorateLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDecorateLevel? GetOneByFilter(Func<rawEventDecorateLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDecorateLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDecorateLevel data) 
		{
			return data.Id;			
		}
    }
