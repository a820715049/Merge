...
using EventOrderBonusGroupConf = fat.rawdata.EventOrderBonusGroupConf;
using rawEventOrderBonusGroup = fat.rawdata.EventOrderBonusGroup;
using rawEventOrderBonusGroupMapABValue = fat.rawdata.EventOrderBonusGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderBonusGroupVisitor
    {
		...
		public const string FileName = "EventOrderBonusGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderBonusGroupVisitor.SetABDefault单独设定EventOrderBonusGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderBonusGroupVisitor.SetABFallback单独设定EventOrderBonusGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderBonusGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderBonusGroup>>(()=> new PBC::MapField<int, rawEventOrderBonusGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderBonusGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderBonusGroupMap;
			var abMap = v.EventOrderBonusGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderBonusGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderBonusGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderBonusGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderBonusGroupMap;
			if (v.EventOrderBonusGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderBonusGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderBonusGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderBonusGroupConf cc) {
			foreach(KeyValuePair <string, rawEventOrderBonusGroupMapABValue> tagEntry in cc.EventOrderBonusGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderBonusGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderBonusGroupMapABValue();
					cc.EventOrderBonusGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderBonusGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderBonusGroup> entry in cc.EventOrderBonusGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderBonusGroup> entry in tagEntry.Value.EventOrderBonusGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderBonusGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderBonusGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderBonusGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderBonusGroupMapAB != null)
			{
				foreach (var entry in cc.EventOrderBonusGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderBonusGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderBonusGroupMap.MergeFrom(cis);
			Patch(updating.EventOrderBonusGroupMap);
			updating.EventOrderBonusGroupSortedKeys = genSortedKeys(updating.EventOrderBonusGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderBonusGroupConf();
            tmp.MergeFrom(cis);
			updating.EventOrderBonusGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderBonusGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderBonusGroup> filter,string tag=""){
		...
		public static rawEventOrderBonusGroup MustGet(int key,string tag="") {
		...
		public static rawEventOrderBonusGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderBonusGroup> GetByFilter(Func<rawEventOrderBonusGroup, bool> filterFunc,string tag="") {
			List<rawEventOrderBonusGroup> ret = new List<rawEventOrderBonusGroup>();
			foreach(KeyValuePair <int, rawEventOrderBonusGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderBonusGroup? GetOneByFilter(Func<rawEventOrderBonusGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderBonusGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderBonusGroup data) 
		{
			return data.Id;			
		}
    }
