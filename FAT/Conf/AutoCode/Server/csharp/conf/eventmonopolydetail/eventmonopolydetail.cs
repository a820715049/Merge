...
using EventMonopolyDetailConf = fat.rawdata.EventMonopolyDetailConf;
using rawEventMonopolyDetail = fat.rawdata.EventMonopolyDetail;
using rawEventMonopolyDetailMapABValue = fat.rawdata.EventMonopolyDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMonopolyDetailVisitor
    {
		...
		public const string FileName = "EventMonopolyDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMonopolyDetailVisitor.SetABDefault单独设定EventMonopolyDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMonopolyDetailVisitor.SetABFallback单独设定EventMonopolyDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMonopolyDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMonopolyDetail>>(()=> new PBC::MapField<int, rawEventMonopolyDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMonopolyDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMonopolyDetailMap;
			var abMap = v.EventMonopolyDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMonopolyDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMonopolyDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMonopolyDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMonopolyDetailMap;
			if (v.EventMonopolyDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMonopolyDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMonopolyDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMonopolyDetailConf cc) {
			foreach(KeyValuePair <string, rawEventMonopolyDetailMapABValue> tagEntry in cc.EventMonopolyDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMonopolyDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMonopolyDetailMapABValue();
					cc.EventMonopolyDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMonopolyDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMonopolyDetail> entry in cc.EventMonopolyDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMonopolyDetail> entry in tagEntry.Value.EventMonopolyDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMonopolyDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMonopolyDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMonopolyDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMonopolyDetailMapAB != null)
			{
				foreach (var entry in cc.EventMonopolyDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMonopolyDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMonopolyDetailMap.MergeFrom(cis);
			Patch(updating.EventMonopolyDetailMap);
			updating.EventMonopolyDetailSortedKeys = genSortedKeys(updating.EventMonopolyDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMonopolyDetailConf();
            tmp.MergeFrom(cis);
			updating.EventMonopolyDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMonopolyDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMonopolyDetail> filter,string tag=""){
		...
		public static rawEventMonopolyDetail MustGet(int key,string tag="") {
		...
		public static rawEventMonopolyDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMonopolyDetail> GetByFilter(Func<rawEventMonopolyDetail, bool> filterFunc,string tag="") {
			List<rawEventMonopolyDetail> ret = new List<rawEventMonopolyDetail>();
			foreach(KeyValuePair <int, rawEventMonopolyDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMonopolyDetail? GetOneByFilter(Func<rawEventMonopolyDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMonopolyDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMonopolyDetail data) 
		{
			return data.Id;			
		}
    }
