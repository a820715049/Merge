...
using EventBingoTaskDetailConf = fat.rawdata.EventBingoTaskDetailConf;
using rawEventBingoTaskDetail = fat.rawdata.EventBingoTaskDetail;
using rawEventBingoTaskDetailMapABValue = fat.rawdata.EventBingoTaskDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventBingoTaskDetailVisitor
    {
		...
		public const string FileName = "EventBingoTaskDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventBingoTaskDetailVisitor.SetABDefault单独设定EventBingoTaskDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventBingoTaskDetailVisitor.SetABFallback单独设定EventBingoTaskDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventBingoTaskDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventBingoTaskDetail>>(()=> new PBC::MapField<int, rawEventBingoTaskDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventBingoTaskDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventBingoTaskDetailMap;
			var abMap = v.EventBingoTaskDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventBingoTaskDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventBingoTaskDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventBingoTaskDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventBingoTaskDetailMap;
			if (v.EventBingoTaskDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventBingoTaskDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventBingoTaskDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventBingoTaskDetailConf cc) {
			foreach(KeyValuePair <string, rawEventBingoTaskDetailMapABValue> tagEntry in cc.EventBingoTaskDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventBingoTaskDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventBingoTaskDetailMapABValue();
					cc.EventBingoTaskDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventBingoTaskDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventBingoTaskDetail> entry in cc.EventBingoTaskDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventBingoTaskDetail> entry in tagEntry.Value.EventBingoTaskDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventBingoTaskDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventBingoTaskDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventBingoTaskDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventBingoTaskDetailMapAB != null)
			{
				foreach (var entry in cc.EventBingoTaskDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventBingoTaskDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventBingoTaskDetailMap.MergeFrom(cis);
			Patch(updating.EventBingoTaskDetailMap);
			updating.EventBingoTaskDetailSortedKeys = genSortedKeys(updating.EventBingoTaskDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventBingoTaskDetailConf();
            tmp.MergeFrom(cis);
			updating.EventBingoTaskDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventBingoTaskDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventBingoTaskDetail> filter,string tag=""){
		...
		public static rawEventBingoTaskDetail MustGet(int key,string tag="") {
		...
		public static rawEventBingoTaskDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventBingoTaskDetail> GetByFilter(Func<rawEventBingoTaskDetail, bool> filterFunc,string tag="") {
			List<rawEventBingoTaskDetail> ret = new List<rawEventBingoTaskDetail>();
			foreach(KeyValuePair <int, rawEventBingoTaskDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventBingoTaskDetail? GetOneByFilter(Func<rawEventBingoTaskDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventBingoTaskDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventBingoTaskDetail data) 
		{
			return data.Id;			
		}
    }
