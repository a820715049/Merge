...
using MergeBoardConf = fat.rawdata.MergeBoardConf;
using rawMergeBoard = fat.rawdata.MergeBoard;
using rawMergeBoardMapABValue = fat.rawdata.MergeBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeBoardVisitor
    {
		...
		public const string FileName = "MergeBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeBoardVisitor.SetABDefault单独设定MergeBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeBoardVisitor.SetABFallback单独设定MergeBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeBoard>>(()=> new PBC::MapField<int, rawMergeBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeBoardMap;
			var abMap = v.MergeBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeBoardMap;
			if (v.MergeBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeBoardConf cc) {
			foreach(KeyValuePair <string, rawMergeBoardMapABValue> tagEntry in cc.MergeBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeBoardMapABValue();
					cc.MergeBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeBoard> entry in cc.MergeBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeBoard> entry in tagEntry.Value.MergeBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeBoardMapAB != null)
			{
				foreach (var entry in cc.MergeBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeBoardMap.MergeFrom(cis);
			Patch(updating.MergeBoardMap);
			updating.MergeBoardSortedKeys = genSortedKeys(updating.MergeBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeBoardConf();
            tmp.MergeFrom(cis);
			updating.MergeBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeBoard> filter,string tag=""){
		...
		public static rawMergeBoard MustGet(int key,string tag="") {
		...
		public static rawMergeBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeBoard> GetByFilter(Func<rawMergeBoard, bool> filterFunc,string tag="") {
			List<rawMergeBoard> ret = new List<rawMergeBoard>();
			foreach(KeyValuePair <int, rawMergeBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeBoard? GetOneByFilter(Func<rawMergeBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeBoard data) 
		{
			return data.Id;			
		}
    }
