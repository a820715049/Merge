...
using EventBingoTaskConf = fat.rawdata.EventBingoTaskConf;
using rawEventBingoTask = fat.rawdata.EventBingoTask;
using rawEventBingoTaskMapABValue = fat.rawdata.EventBingoTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventBingoTaskVisitor
    {
		...
		public const string FileName = "EventBingoTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventBingoTaskVisitor.SetABDefault单独设定EventBingoTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventBingoTaskVisitor.SetABFallback单独设定EventBingoTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventBingoTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventBingoTask>>(()=> new PBC::MapField<int, rawEventBingoTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventBingoTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventBingoTaskMap;
			var abMap = v.EventBingoTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventBingoTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventBingoTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventBingoTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventBingoTaskMap;
			if (v.EventBingoTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventBingoTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventBingoTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventBingoTaskConf cc) {
			foreach(KeyValuePair <string, rawEventBingoTaskMapABValue> tagEntry in cc.EventBingoTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventBingoTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventBingoTaskMapABValue();
					cc.EventBingoTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.EventBingoTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventBingoTask> entry in cc.EventBingoTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventBingoTask> entry in tagEntry.Value.EventBingoTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventBingoTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventBingoTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventBingoTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventBingoTaskMapAB != null)
			{
				foreach (var entry in cc.EventBingoTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventBingoTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventBingoTaskMap.MergeFrom(cis);
			Patch(updating.EventBingoTaskMap);
			updating.EventBingoTaskSortedKeys = genSortedKeys(updating.EventBingoTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventBingoTaskConf();
            tmp.MergeFrom(cis);
			updating.EventBingoTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventBingoTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventBingoTask> filter,string tag=""){
		...
		public static rawEventBingoTask MustGet(int key,string tag="") {
		...
		public static rawEventBingoTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventBingoTask> GetByFilter(Func<rawEventBingoTask, bool> filterFunc,string tag="") {
			List<rawEventBingoTask> ret = new List<rawEventBingoTask>();
			foreach(KeyValuePair <int, rawEventBingoTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventBingoTask? GetOneByFilter(Func<rawEventBingoTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventBingoTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventBingoTask data) 
		{
			return data.Id;			
		}
    }
