...
using EventPachinkoRoundConf = fat.rawdata.EventPachinkoRoundConf;
using rawEventPachinkoRound = fat.rawdata.EventPachinkoRound;
using rawEventPachinkoRoundMapABValue = fat.rawdata.EventPachinkoRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPachinkoRoundVisitor
    {
		...
		public const string FileName = "EventPachinkoRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPachinkoRoundVisitor.SetABDefault单独设定EventPachinkoRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPachinkoRoundVisitor.SetABFallback单独设定EventPachinkoRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPachinkoRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPachinkoRound>>(()=> new PBC::MapField<int, rawEventPachinkoRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPachinkoRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPachinkoRoundMap;
			var abMap = v.EventPachinkoRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPachinkoRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPachinkoRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPachinkoRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPachinkoRoundMap;
			if (v.EventPachinkoRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPachinkoRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPachinkoRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPachinkoRoundConf cc) {
			foreach(KeyValuePair <string, rawEventPachinkoRoundMapABValue> tagEntry in cc.EventPachinkoRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPachinkoRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPachinkoRoundMapABValue();
					cc.EventPachinkoRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPachinkoRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPachinkoRound> entry in cc.EventPachinkoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPachinkoRound> entry in tagEntry.Value.EventPachinkoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPachinkoRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPachinkoRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPachinkoRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPachinkoRoundMapAB != null)
			{
				foreach (var entry in cc.EventPachinkoRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPachinkoRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPachinkoRoundMap.MergeFrom(cis);
			Patch(updating.EventPachinkoRoundMap);
			updating.EventPachinkoRoundSortedKeys = genSortedKeys(updating.EventPachinkoRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPachinkoRoundConf();
            tmp.MergeFrom(cis);
			updating.EventPachinkoRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPachinkoRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPachinkoRound> filter,string tag=""){
		...
		public static rawEventPachinkoRound MustGet(int key,string tag="") {
		...
		public static rawEventPachinkoRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPachinkoRound> GetByFilter(Func<rawEventPachinkoRound, bool> filterFunc,string tag="") {
			List<rawEventPachinkoRound> ret = new List<rawEventPachinkoRound>();
			foreach(KeyValuePair <int, rawEventPachinkoRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPachinkoRound? GetOneByFilter(Func<rawEventPachinkoRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPachinkoRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPachinkoRound data) 
		{
			return data.Id;			
		}
    }
