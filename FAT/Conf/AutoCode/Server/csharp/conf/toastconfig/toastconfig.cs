...
using ToastConfigConf = fat.rawdata.ToastConfigConf;
using rawToastConfig = fat.rawdata.ToastConfig;
using rawToastConfigSliceABValue = fat.rawdata.ToastConfigSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ToastConfigVisitor
    {
		...
		public const string FileName = "ToastConfigConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ToastConfigVisitor.SetABDefault单独设定ToastConfig的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ToastConfigVisitor.SetABFallback单独设定ToastConfig的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawToastConfig>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawToastConfig>>(()=>new PBC::RepeatedField<rawToastConfig>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawToastConfig>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ToastConfigSlice;
			var abMap = v.ToastConfigSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ToastConfigSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ToastConfigSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawToastConfig> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ToastConfigSlice;
			if (v.ToastConfigSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ToastConfigSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawToastConfigSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ToastConfigConf cc) {
			foreach(KeyValuePair <string, rawToastConfigSliceABValue> tagEntry in cc.ToastConfigSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ToastConfigSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawToastConfigSliceABValue();
					cc.ToastConfigSliceAB[tag] = ps;
				}
                var tagTmp = ps.ToastConfigSlice;
				// 基础数据
				for(int i=0; i<cc.ToastConfigSlice.Count; ++i) 
				{
					tagTmp.Add(cc.ToastConfigSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.ToastConfigSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.ToastConfig> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ToastConfigSlice.MergeFrom(cis);
			Patch(updating.ToastConfigSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ToastConfigConf();
            tmp.MergeFrom(cis);
			updating.ToastConfigSlice = tmp;
		...
		public static void Range(Action<int, rawToastConfig> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawToastConfig MustGetByIndex(int index,string tag="") {
		...
		public static rawToastConfig? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawToastConfig> GetByFilter(Func<rawToastConfig,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawToastConfig>();
		...
		static public rawToastConfig? GetOneByFilter(Func<rawToastConfig,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
