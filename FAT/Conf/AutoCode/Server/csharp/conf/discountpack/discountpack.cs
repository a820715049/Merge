...
using DiscountPackConf = fat.rawdata.DiscountPackConf;
using rawDiscountPack = fat.rawdata.DiscountPack;
using rawDiscountPackMapABValue = fat.rawdata.DiscountPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DiscountPackVisitor
    {
		...
		public const string FileName = "DiscountPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DiscountPackVisitor.SetABDefault单独设定DiscountPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DiscountPackVisitor.SetABFallback单独设定DiscountPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDiscountPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDiscountPack>>(()=> new PBC::MapField<int, rawDiscountPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDiscountPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DiscountPackMap;
			var abMap = v.DiscountPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DiscountPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DiscountPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDiscountPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DiscountPackMap;
			if (v.DiscountPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DiscountPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDiscountPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DiscountPackConf cc) {
			foreach(KeyValuePair <string, rawDiscountPackMapABValue> tagEntry in cc.DiscountPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DiscountPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDiscountPackMapABValue();
					cc.DiscountPackMapAB[tag] = ps;
				}
                var tagTmp = ps.DiscountPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDiscountPack> entry in cc.DiscountPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDiscountPack> entry in tagEntry.Value.DiscountPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DiscountPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DiscountPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DiscountPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DiscountPackMapAB != null)
			{
				foreach (var entry in cc.DiscountPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DiscountPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DiscountPackMap.MergeFrom(cis);
			Patch(updating.DiscountPackMap);
			updating.DiscountPackSortedKeys = genSortedKeys(updating.DiscountPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DiscountPackConf();
            tmp.MergeFrom(cis);
			updating.DiscountPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DiscountPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDiscountPack> filter,string tag=""){
		...
		public static rawDiscountPack MustGet(int key,string tag="") {
		...
		public static rawDiscountPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDiscountPack> GetByFilter(Func<rawDiscountPack, bool> filterFunc,string tag="") {
			List<rawDiscountPack> ret = new List<rawDiscountPack>();
			foreach(KeyValuePair <int, rawDiscountPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDiscountPack? GetOneByFilter(Func<rawDiscountPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDiscountPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDiscountPack data) 
		{
			return data.Id;			
		}
    }
