...
using EventDecorateGroupConf = fat.rawdata.EventDecorateGroupConf;
using rawEventDecorateGroup = fat.rawdata.EventDecorateGroup;
using rawEventDecorateGroupMapABValue = fat.rawdata.EventDecorateGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDecorateGroupVisitor
    {
		...
		public const string FileName = "EventDecorateGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDecorateGroupVisitor.SetABDefault单独设定EventDecorateGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDecorateGroupVisitor.SetABFallback单独设定EventDecorateGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDecorateGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDecorateGroup>>(()=> new PBC::MapField<int, rawEventDecorateGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDecorateGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDecorateGroupMap;
			var abMap = v.EventDecorateGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDecorateGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDecorateGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDecorateGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDecorateGroupMap;
			if (v.EventDecorateGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDecorateGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDecorateGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDecorateGroupConf cc) {
			foreach(KeyValuePair <string, rawEventDecorateGroupMapABValue> tagEntry in cc.EventDecorateGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDecorateGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDecorateGroupMapABValue();
					cc.EventDecorateGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDecorateGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDecorateGroup> entry in cc.EventDecorateGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDecorateGroup> entry in tagEntry.Value.EventDecorateGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDecorateGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDecorateGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDecorateGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDecorateGroupMapAB != null)
			{
				foreach (var entry in cc.EventDecorateGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDecorateGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDecorateGroupMap.MergeFrom(cis);
			Patch(updating.EventDecorateGroupMap);
			updating.EventDecorateGroupSortedKeys = genSortedKeys(updating.EventDecorateGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDecorateGroupConf();
            tmp.MergeFrom(cis);
			updating.EventDecorateGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDecorateGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDecorateGroup> filter,string tag=""){
		...
		public static rawEventDecorateGroup MustGet(int key,string tag="") {
		...
		public static rawEventDecorateGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDecorateGroup> GetByFilter(Func<rawEventDecorateGroup, bool> filterFunc,string tag="") {
			List<rawEventDecorateGroup> ret = new List<rawEventDecorateGroup>();
			foreach(KeyValuePair <int, rawEventDecorateGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDecorateGroup? GetOneByFilter(Func<rawEventDecorateGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDecorateGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDecorateGroup data) 
		{
			return data.Id;			
		}
    }
