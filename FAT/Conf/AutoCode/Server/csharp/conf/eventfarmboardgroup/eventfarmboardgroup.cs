...
using EventFarmBoardGroupConf = fat.rawdata.EventFarmBoardGroupConf;
using rawEventFarmBoardGroup = fat.rawdata.EventFarmBoardGroup;
using rawEventFarmBoardGroupMapABValue = fat.rawdata.EventFarmBoardGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmBoardGroupVisitor
    {
		...
		public const string FileName = "EventFarmBoardGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmBoardGroupVisitor.SetABDefault单独设定EventFarmBoardGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmBoardGroupVisitor.SetABFallback单独设定EventFarmBoardGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmBoardGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmBoardGroup>>(()=> new PBC::MapField<int, rawEventFarmBoardGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmBoardGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmBoardGroupMap;
			var abMap = v.EventFarmBoardGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmBoardGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmBoardGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmBoardGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmBoardGroupMap;
			if (v.EventFarmBoardGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmBoardGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmBoardGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmBoardGroupConf cc) {
			foreach(KeyValuePair <string, rawEventFarmBoardGroupMapABValue> tagEntry in cc.EventFarmBoardGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmBoardGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmBoardGroupMapABValue();
					cc.EventFarmBoardGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmBoardGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmBoardGroup> entry in cc.EventFarmBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmBoardGroup> entry in tagEntry.Value.EventFarmBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmBoardGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmBoardGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmBoardGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmBoardGroupMapAB != null)
			{
				foreach (var entry in cc.EventFarmBoardGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmBoardGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmBoardGroupMap.MergeFrom(cis);
			Patch(updating.EventFarmBoardGroupMap);
			updating.EventFarmBoardGroupSortedKeys = genSortedKeys(updating.EventFarmBoardGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmBoardGroupConf();
            tmp.MergeFrom(cis);
			updating.EventFarmBoardGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmBoardGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmBoardGroup> filter,string tag=""){
		...
		public static rawEventFarmBoardGroup MustGet(int key,string tag="") {
		...
		public static rawEventFarmBoardGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmBoardGroup> GetByFilter(Func<rawEventFarmBoardGroup, bool> filterFunc,string tag="") {
			List<rawEventFarmBoardGroup> ret = new List<rawEventFarmBoardGroup>();
			foreach(KeyValuePair <int, rawEventFarmBoardGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmBoardGroup? GetOneByFilter(Func<rawEventFarmBoardGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmBoardGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmBoardGroup data) 
		{
			return data.Id;			
		}
    }
