...
using TrainMilestoneConf = fat.rawdata.TrainMilestoneConf;
using rawTrainMilestone = fat.rawdata.TrainMilestone;
using rawTrainMilestoneMapABValue = fat.rawdata.TrainMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class TrainMilestoneVisitor
    {
		...
		public const string FileName = "TrainMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过TrainMilestoneVisitor.SetABDefault单独设定TrainMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过TrainMilestoneVisitor.SetABFallback单独设定TrainMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawTrainMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawTrainMilestone>>(()=> new PBC::MapField<int, rawTrainMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawTrainMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().TrainMilestoneMap;
			var abMap = v.TrainMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.TrainMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].TrainMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawTrainMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().TrainMilestoneMap;
			if (v.TrainMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.TrainMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawTrainMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(TrainMilestoneConf cc) {
			foreach(KeyValuePair <string, rawTrainMilestoneMapABValue> tagEntry in cc.TrainMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.TrainMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawTrainMilestoneMapABValue();
					cc.TrainMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.TrainMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawTrainMilestone> entry in cc.TrainMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawTrainMilestone> entry in tagEntry.Value.TrainMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.TrainMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(TrainMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.TrainMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.TrainMilestoneMapAB != null)
			{
				foreach (var entry in cc.TrainMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.TrainMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.TrainMilestoneMap.MergeFrom(cis);
			Patch(updating.TrainMilestoneMap);
			updating.TrainMilestoneSortedKeys = genSortedKeys(updating.TrainMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new TrainMilestoneConf();
            tmp.MergeFrom(cis);
			updating.TrainMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().TrainMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawTrainMilestone> filter,string tag=""){
		...
		public static rawTrainMilestone MustGet(int key,string tag="") {
		...
		public static rawTrainMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawTrainMilestone> GetByFilter(Func<rawTrainMilestone, bool> filterFunc,string tag="") {
			List<rawTrainMilestone> ret = new List<rawTrainMilestone>();
			foreach(KeyValuePair <int, rawTrainMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawTrainMilestone? GetOneByFilter(Func<rawTrainMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawTrainMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawTrainMilestone data) 
		{
			return data.Id;			
		}
    }
