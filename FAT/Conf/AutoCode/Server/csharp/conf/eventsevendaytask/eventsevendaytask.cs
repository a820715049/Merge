...
using EventSevenDayTaskConf = fat.rawdata.EventSevenDayTaskConf;
using rawEventSevenDayTask = fat.rawdata.EventSevenDayTask;
using rawEventSevenDayTaskMapABValue = fat.rawdata.EventSevenDayTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSevenDayTaskVisitor
    {
		...
		public const string FileName = "EventSevenDayTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSevenDayTaskVisitor.SetABDefault单独设定EventSevenDayTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSevenDayTaskVisitor.SetABFallback单独设定EventSevenDayTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSevenDayTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSevenDayTask>>(()=> new PBC::MapField<int, rawEventSevenDayTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSevenDayTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSevenDayTaskMap;
			var abMap = v.EventSevenDayTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSevenDayTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSevenDayTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSevenDayTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSevenDayTaskMap;
			if (v.EventSevenDayTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSevenDayTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSevenDayTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSevenDayTaskConf cc) {
			foreach(KeyValuePair <string, rawEventSevenDayTaskMapABValue> tagEntry in cc.EventSevenDayTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSevenDayTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSevenDayTaskMapABValue();
					cc.EventSevenDayTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSevenDayTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSevenDayTask> entry in cc.EventSevenDayTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSevenDayTask> entry in tagEntry.Value.EventSevenDayTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSevenDayTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSevenDayTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSevenDayTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSevenDayTaskMapAB != null)
			{
				foreach (var entry in cc.EventSevenDayTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSevenDayTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSevenDayTaskMap.MergeFrom(cis);
			Patch(updating.EventSevenDayTaskMap);
			updating.EventSevenDayTaskSortedKeys = genSortedKeys(updating.EventSevenDayTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSevenDayTaskConf();
            tmp.MergeFrom(cis);
			updating.EventSevenDayTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSevenDayTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSevenDayTask> filter,string tag=""){
		...
		public static rawEventSevenDayTask MustGet(int key,string tag="") {
		...
		public static rawEventSevenDayTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSevenDayTask> GetByFilter(Func<rawEventSevenDayTask, bool> filterFunc,string tag="") {
			List<rawEventSevenDayTask> ret = new List<rawEventSevenDayTask>();
			foreach(KeyValuePair <int, rawEventSevenDayTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSevenDayTask? GetOneByFilter(Func<rawEventSevenDayTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSevenDayTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSevenDayTask data) 
		{
			return data.Id;			
		}
    }
