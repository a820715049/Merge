...
using EventAreaBoardConf = fat.rawdata.EventAreaBoardConf;
using rawEventAreaBoard = fat.rawdata.EventAreaBoard;
using rawEventAreaBoardMapABValue = fat.rawdata.EventAreaBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaBoardVisitor
    {
		...
		public const string FileName = "EventAreaBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaBoardVisitor.SetABDefault单独设定EventAreaBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaBoardVisitor.SetABFallback单独设定EventAreaBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaBoard>>(()=> new PBC::MapField<int, rawEventAreaBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaBoardMap;
			var abMap = v.EventAreaBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaBoardMap;
			if (v.EventAreaBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaBoardConf cc) {
			foreach(KeyValuePair <string, rawEventAreaBoardMapABValue> tagEntry in cc.EventAreaBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaBoardMapABValue();
					cc.EventAreaBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaBoard> entry in cc.EventAreaBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaBoard> entry in tagEntry.Value.EventAreaBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaBoardMapAB != null)
			{
				foreach (var entry in cc.EventAreaBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaBoardMap.MergeFrom(cis);
			Patch(updating.EventAreaBoardMap);
			updating.EventAreaBoardSortedKeys = genSortedKeys(updating.EventAreaBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaBoardConf();
            tmp.MergeFrom(cis);
			updating.EventAreaBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaBoard> filter,string tag=""){
		...
		public static rawEventAreaBoard MustGet(int key,string tag="") {
		...
		public static rawEventAreaBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaBoard> GetByFilter(Func<rawEventAreaBoard, bool> filterFunc,string tag="") {
			List<rawEventAreaBoard> ret = new List<rawEventAreaBoard>();
			foreach(KeyValuePair <int, rawEventAreaBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaBoard? GetOneByFilter(Func<rawEventAreaBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaBoard data) 
		{
			return data.Id;			
		}
    }
