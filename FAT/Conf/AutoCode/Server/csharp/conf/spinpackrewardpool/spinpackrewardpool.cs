...
using SpinPackRewardPoolConf = fat.rawdata.SpinPackRewardPoolConf;
using rawSpinPackRewardPool = fat.rawdata.SpinPackRewardPool;
using rawSpinPackRewardPoolMapABValue = fat.rawdata.SpinPackRewardPoolMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SpinPackRewardPoolVisitor
    {
		...
		public const string FileName = "SpinPackRewardPoolConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SpinPackRewardPoolVisitor.SetABDefault单独设定SpinPackRewardPool的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SpinPackRewardPoolVisitor.SetABFallback单独设定SpinPackRewardPool的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSpinPackRewardPool>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSpinPackRewardPool>>(()=> new PBC::MapField<int, rawSpinPackRewardPool>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSpinPackRewardPool>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SpinPackRewardPoolMap;
			var abMap = v.SpinPackRewardPoolMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SpinPackRewardPoolMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SpinPackRewardPoolMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSpinPackRewardPool> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SpinPackRewardPoolMap;
			if (v.SpinPackRewardPoolMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SpinPackRewardPoolMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSpinPackRewardPoolMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SpinPackRewardPoolConf cc) {
			foreach(KeyValuePair <string, rawSpinPackRewardPoolMapABValue> tagEntry in cc.SpinPackRewardPoolMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SpinPackRewardPoolMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSpinPackRewardPoolMapABValue();
					cc.SpinPackRewardPoolMapAB[tag] = ps;
				}
                var tagTmp = ps.SpinPackRewardPoolMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSpinPackRewardPool> entry in cc.SpinPackRewardPoolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSpinPackRewardPool> entry in tagEntry.Value.SpinPackRewardPoolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SpinPackRewardPool> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SpinPackRewardPoolConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SpinPackRewardPoolMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SpinPackRewardPoolMapAB != null)
			{
				foreach (var entry in cc.SpinPackRewardPoolMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SpinPackRewardPoolMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SpinPackRewardPoolMap.MergeFrom(cis);
			Patch(updating.SpinPackRewardPoolMap);
			updating.SpinPackRewardPoolSortedKeys = genSortedKeys(updating.SpinPackRewardPoolMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SpinPackRewardPoolConf();
            tmp.MergeFrom(cis);
			updating.SpinPackRewardPoolMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SpinPackRewardPoolSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSpinPackRewardPool> filter,string tag=""){
		...
		public static rawSpinPackRewardPool MustGet(int key,string tag="") {
		...
		public static rawSpinPackRewardPool? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSpinPackRewardPool> GetByFilter(Func<rawSpinPackRewardPool, bool> filterFunc,string tag="") {
			List<rawSpinPackRewardPool> ret = new List<rawSpinPackRewardPool>();
			foreach(KeyValuePair <int, rawSpinPackRewardPool> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSpinPackRewardPool? GetOneByFilter(Func<rawSpinPackRewardPool, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSpinPackRewardPool> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSpinPackRewardPool data) 
		{
			return data.Id;			
		}
    }
