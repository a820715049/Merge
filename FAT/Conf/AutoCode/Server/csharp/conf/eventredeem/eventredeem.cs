...
using EventRedeemConf = fat.rawdata.EventRedeemConf;
using rawEventRedeem = fat.rawdata.EventRedeem;
using rawEventRedeemMapABValue = fat.rawdata.EventRedeemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRedeemVisitor
    {
		...
		public const string FileName = "EventRedeemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRedeemVisitor.SetABDefault单独设定EventRedeem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRedeemVisitor.SetABFallback单独设定EventRedeem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRedeem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRedeem>>(()=> new PBC::MapField<int, rawEventRedeem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRedeem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRedeemMap;
			var abMap = v.EventRedeemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRedeemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRedeemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRedeem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRedeemMap;
			if (v.EventRedeemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRedeemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRedeemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRedeemConf cc) {
			foreach(KeyValuePair <string, rawEventRedeemMapABValue> tagEntry in cc.EventRedeemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRedeemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRedeemMapABValue();
					cc.EventRedeemMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRedeemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRedeem> entry in cc.EventRedeemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRedeem> entry in tagEntry.Value.EventRedeemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRedeem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRedeemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRedeemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRedeemMapAB != null)
			{
				foreach (var entry in cc.EventRedeemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRedeemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRedeemMap.MergeFrom(cis);
			Patch(updating.EventRedeemMap);
			updating.EventRedeemSortedKeys = genSortedKeys(updating.EventRedeemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRedeemConf();
            tmp.MergeFrom(cis);
			updating.EventRedeemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRedeemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRedeem> filter,string tag=""){
		...
		public static rawEventRedeem MustGet(int key,string tag="") {
		...
		public static rawEventRedeem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRedeem> GetByFilter(Func<rawEventRedeem, bool> filterFunc,string tag="") {
			List<rawEventRedeem> ret = new List<rawEventRedeem>();
			foreach(KeyValuePair <int, rawEventRedeem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRedeem? GetOneByFilter(Func<rawEventRedeem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRedeem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRedeem data) 
		{
			return data.Id;			
		}
    }
