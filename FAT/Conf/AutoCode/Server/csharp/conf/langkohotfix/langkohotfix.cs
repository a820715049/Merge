...
using LangKoHotfixConf = fat.rawdata.LangKoHotfixConf;
using rawLangKoHotfix = fat.rawdata.LangKoHotfix;
using rawLangKoHotfixSliceABValue = fat.rawdata.LangKoHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangKoHotfixVisitor
    {
		...
		public const string FileName = "LangKoHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangKoHotfixVisitor.SetABDefault单独设定LangKoHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangKoHotfixVisitor.SetABFallback单独设定LangKoHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangKoHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangKoHotfix>>(()=>new PBC::RepeatedField<rawLangKoHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangKoHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangKoHotfixSlice;
			var abMap = v.LangKoHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangKoHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangKoHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangKoHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangKoHotfixSlice;
			if (v.LangKoHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangKoHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangKoHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangKoHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangKoHotfixSliceABValue> tagEntry in cc.LangKoHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangKoHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangKoHotfixSliceABValue();
					cc.LangKoHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangKoHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangKoHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangKoHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangKoHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangKoHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangKoHotfixSlice.MergeFrom(cis);
			Patch(updating.LangKoHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangKoHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangKoHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangKoHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangKoHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangKoHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangKoHotfix> GetByFilter(Func<rawLangKoHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangKoHotfix>();
		...
		static public rawLangKoHotfix? GetOneByFilter(Func<rawLangKoHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
