...
using EventStepTaskConf = fat.rawdata.EventStepTaskConf;
using rawEventStepTask = fat.rawdata.EventStepTask;
using rawEventStepTaskMapABValue = fat.rawdata.EventStepTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventStepTaskVisitor
    {
		...
		public const string FileName = "EventStepTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventStepTaskVisitor.SetABDefault单独设定EventStepTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventStepTaskVisitor.SetABFallback单独设定EventStepTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventStepTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventStepTask>>(()=> new PBC::MapField<int, rawEventStepTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventStepTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventStepTaskMap;
			var abMap = v.EventStepTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventStepTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventStepTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventStepTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventStepTaskMap;
			if (v.EventStepTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventStepTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventStepTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventStepTaskConf cc) {
			foreach(KeyValuePair <string, rawEventStepTaskMapABValue> tagEntry in cc.EventStepTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventStepTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventStepTaskMapABValue();
					cc.EventStepTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.EventStepTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventStepTask> entry in cc.EventStepTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventStepTask> entry in tagEntry.Value.EventStepTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventStepTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventStepTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventStepTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventStepTaskMapAB != null)
			{
				foreach (var entry in cc.EventStepTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventStepTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventStepTaskMap.MergeFrom(cis);
			Patch(updating.EventStepTaskMap);
			updating.EventStepTaskSortedKeys = genSortedKeys(updating.EventStepTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventStepTaskConf();
            tmp.MergeFrom(cis);
			updating.EventStepTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventStepTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventStepTask> filter,string tag=""){
		...
		public static rawEventStepTask MustGet(int key,string tag="") {
		...
		public static rawEventStepTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventStepTask> GetByFilter(Func<rawEventStepTask, bool> filterFunc,string tag="") {
			List<rawEventStepTask> ret = new List<rawEventStepTask>();
			foreach(KeyValuePair <int, rawEventStepTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventStepTask? GetOneByFilter(Func<rawEventStepTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventStepTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventStepTask data) 
		{
			return data.Id;			
		}
    }
