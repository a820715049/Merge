...
using EventCardRoundConf = fat.rawdata.EventCardRoundConf;
using rawEventCardRound = fat.rawdata.EventCardRound;
using rawEventCardRoundMapABValue = fat.rawdata.EventCardRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventCardRoundVisitor
    {
		...
		public const string FileName = "EventCardRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventCardRoundVisitor.SetABDefault单独设定EventCardRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventCardRoundVisitor.SetABFallback单独设定EventCardRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventCardRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventCardRound>>(()=> new PBC::MapField<int, rawEventCardRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventCardRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventCardRoundMap;
			var abMap = v.EventCardRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventCardRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventCardRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventCardRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventCardRoundMap;
			if (v.EventCardRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventCardRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventCardRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventCardRoundConf cc) {
			foreach(KeyValuePair <string, rawEventCardRoundMapABValue> tagEntry in cc.EventCardRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventCardRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventCardRoundMapABValue();
					cc.EventCardRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventCardRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventCardRound> entry in cc.EventCardRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventCardRound> entry in tagEntry.Value.EventCardRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventCardRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventCardRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventCardRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventCardRoundMapAB != null)
			{
				foreach (var entry in cc.EventCardRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventCardRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventCardRoundMap.MergeFrom(cis);
			Patch(updating.EventCardRoundMap);
			updating.EventCardRoundSortedKeys = genSortedKeys(updating.EventCardRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventCardRoundConf();
            tmp.MergeFrom(cis);
			updating.EventCardRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventCardRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventCardRound> filter,string tag=""){
		...
		public static rawEventCardRound MustGet(int key,string tag="") {
		...
		public static rawEventCardRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventCardRound> GetByFilter(Func<rawEventCardRound, bool> filterFunc,string tag="") {
			List<rawEventCardRound> ret = new List<rawEventCardRound>();
			foreach(KeyValuePair <int, rawEventCardRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventCardRound? GetOneByFilter(Func<rawEventCardRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventCardRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventCardRound data) 
		{
			return data.Id;			
		}
    }
