...
using DropLimitItemConf = fat.rawdata.DropLimitItemConf;
using rawDropLimitItem = fat.rawdata.DropLimitItem;
using rawDropLimitItemMapABValue = fat.rawdata.DropLimitItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DropLimitItemVisitor
    {
		...
		public const string FileName = "DropLimitItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DropLimitItemVisitor.SetABDefault单独设定DropLimitItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DropLimitItemVisitor.SetABFallback单独设定DropLimitItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDropLimitItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDropLimitItem>>(()=> new PBC::MapField<int, rawDropLimitItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDropLimitItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DropLimitItemMap;
			var abMap = v.DropLimitItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DropLimitItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DropLimitItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDropLimitItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DropLimitItemMap;
			if (v.DropLimitItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DropLimitItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDropLimitItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DropLimitItemConf cc) {
			foreach(KeyValuePair <string, rawDropLimitItemMapABValue> tagEntry in cc.DropLimitItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DropLimitItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDropLimitItemMapABValue();
					cc.DropLimitItemMapAB[tag] = ps;
				}
                var tagTmp = ps.DropLimitItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDropLimitItem> entry in cc.DropLimitItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDropLimitItem> entry in tagEntry.Value.DropLimitItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DropLimitItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DropLimitItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DropLimitItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DropLimitItemMapAB != null)
			{
				foreach (var entry in cc.DropLimitItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DropLimitItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DropLimitItemMap.MergeFrom(cis);
			Patch(updating.DropLimitItemMap);
			updating.DropLimitItemSortedKeys = genSortedKeys(updating.DropLimitItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DropLimitItemConf();
            tmp.MergeFrom(cis);
			updating.DropLimitItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DropLimitItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDropLimitItem> filter,string tag=""){
		...
		public static rawDropLimitItem MustGet(int key,string tag="") {
		...
		public static rawDropLimitItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDropLimitItem> GetByFilter(Func<rawDropLimitItem, bool> filterFunc,string tag="") {
			List<rawDropLimitItem> ret = new List<rawDropLimitItem>();
			foreach(KeyValuePair <int, rawDropLimitItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDropLimitItem? GetOneByFilter(Func<rawDropLimitItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDropLimitItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDropLimitItem data) 
		{
			return data.Id;			
		}
    }
