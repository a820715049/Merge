...
using ComMergeActiveSourceConf = fat.rawdata.ComMergeActiveSourceConf;
using rawComMergeActiveSource = fat.rawdata.ComMergeActiveSource;
using rawComMergeActiveSourceMapABValue = fat.rawdata.ComMergeActiveSourceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeActiveSourceVisitor
    {
		...
		public const string FileName = "ComMergeActiveSourceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeActiveSourceVisitor.SetABDefault单独设定ComMergeActiveSource的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeActiveSourceVisitor.SetABFallback单独设定ComMergeActiveSource的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeActiveSource>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeActiveSource>>(()=> new PBC::MapField<int, rawComMergeActiveSource>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeActiveSource>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeActiveSourceMap;
			var abMap = v.ComMergeActiveSourceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeActiveSourceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeActiveSourceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeActiveSource> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeActiveSourceMap;
			if (v.ComMergeActiveSourceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeActiveSourceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeActiveSourceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeActiveSourceConf cc) {
			foreach(KeyValuePair <string, rawComMergeActiveSourceMapABValue> tagEntry in cc.ComMergeActiveSourceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeActiveSourceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeActiveSourceMapABValue();
					cc.ComMergeActiveSourceMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeActiveSourceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeActiveSource> entry in cc.ComMergeActiveSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeActiveSource> entry in tagEntry.Value.ComMergeActiveSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeActiveSource> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeActiveSourceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeActiveSourceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeActiveSourceMapAB != null)
			{
				foreach (var entry in cc.ComMergeActiveSourceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeActiveSourceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeActiveSourceMap.MergeFrom(cis);
			Patch(updating.ComMergeActiveSourceMap);
			updating.ComMergeActiveSourceSortedKeys = genSortedKeys(updating.ComMergeActiveSourceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeActiveSourceConf();
            tmp.MergeFrom(cis);
			updating.ComMergeActiveSourceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeActiveSourceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeActiveSource> filter,string tag=""){
		...
		public static rawComMergeActiveSource MustGet(int key,string tag="") {
		...
		public static rawComMergeActiveSource? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeActiveSource> GetByFilter(Func<rawComMergeActiveSource, bool> filterFunc,string tag="") {
			List<rawComMergeActiveSource> ret = new List<rawComMergeActiveSource>();
			foreach(KeyValuePair <int, rawComMergeActiveSource> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeActiveSource? GetOneByFilter(Func<rawComMergeActiveSource, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeActiveSource> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeActiveSource data) 
		{
			return data.Id;			
		}
    }
