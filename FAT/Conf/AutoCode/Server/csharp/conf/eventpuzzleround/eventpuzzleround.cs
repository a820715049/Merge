...
using EventPuzzleRoundConf = fat.rawdata.EventPuzzleRoundConf;
using rawEventPuzzleRound = fat.rawdata.EventPuzzleRound;
using rawEventPuzzleRoundMapABValue = fat.rawdata.EventPuzzleRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPuzzleRoundVisitor
    {
		...
		public const string FileName = "EventPuzzleRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPuzzleRoundVisitor.SetABDefault单独设定EventPuzzleRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPuzzleRoundVisitor.SetABFallback单独设定EventPuzzleRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPuzzleRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPuzzleRound>>(()=> new PBC::MapField<int, rawEventPuzzleRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPuzzleRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPuzzleRoundMap;
			var abMap = v.EventPuzzleRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPuzzleRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPuzzleRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPuzzleRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPuzzleRoundMap;
			if (v.EventPuzzleRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPuzzleRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPuzzleRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPuzzleRoundConf cc) {
			foreach(KeyValuePair <string, rawEventPuzzleRoundMapABValue> tagEntry in cc.EventPuzzleRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPuzzleRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPuzzleRoundMapABValue();
					cc.EventPuzzleRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPuzzleRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPuzzleRound> entry in cc.EventPuzzleRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPuzzleRound> entry in tagEntry.Value.EventPuzzleRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPuzzleRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPuzzleRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPuzzleRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPuzzleRoundMapAB != null)
			{
				foreach (var entry in cc.EventPuzzleRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPuzzleRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPuzzleRoundMap.MergeFrom(cis);
			Patch(updating.EventPuzzleRoundMap);
			updating.EventPuzzleRoundSortedKeys = genSortedKeys(updating.EventPuzzleRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPuzzleRoundConf();
            tmp.MergeFrom(cis);
			updating.EventPuzzleRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPuzzleRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPuzzleRound> filter,string tag=""){
		...
		public static rawEventPuzzleRound MustGet(int key,string tag="") {
		...
		public static rawEventPuzzleRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPuzzleRound> GetByFilter(Func<rawEventPuzzleRound, bool> filterFunc,string tag="") {
			List<rawEventPuzzleRound> ret = new List<rawEventPuzzleRound>();
			foreach(KeyValuePair <int, rawEventPuzzleRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPuzzleRound? GetOneByFilter(Func<rawEventPuzzleRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPuzzleRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPuzzleRound data) 
		{
			return data.Id;			
		}
    }
