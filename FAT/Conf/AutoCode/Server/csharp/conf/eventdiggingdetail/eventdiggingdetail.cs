...
using EventDiggingDetailConf = fat.rawdata.EventDiggingDetailConf;
using rawEventDiggingDetail = fat.rawdata.EventDiggingDetail;
using rawEventDiggingDetailMapABValue = fat.rawdata.EventDiggingDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingDetailVisitor
    {
		...
		public const string FileName = "EventDiggingDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingDetailVisitor.SetABDefault单独设定EventDiggingDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingDetailVisitor.SetABFallback单独设定EventDiggingDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDiggingDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDiggingDetail>>(()=> new PBC::MapField<int, rawEventDiggingDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDiggingDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingDetailMap;
			var abMap = v.EventDiggingDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDiggingDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingDetailMap;
			if (v.EventDiggingDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingDetailConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingDetailMapABValue> tagEntry in cc.EventDiggingDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingDetailMapABValue();
					cc.EventDiggingDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDiggingDetail> entry in cc.EventDiggingDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDiggingDetail> entry in tagEntry.Value.EventDiggingDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDiggingDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingDetailMapAB != null)
			{
				foreach (var entry in cc.EventDiggingDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingDetailMap.MergeFrom(cis);
			Patch(updating.EventDiggingDetailMap);
			updating.EventDiggingDetailSortedKeys = genSortedKeys(updating.EventDiggingDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingDetailConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDiggingDetail> filter,string tag=""){
		...
		public static rawEventDiggingDetail MustGet(int key,string tag="") {
		...
		public static rawEventDiggingDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDiggingDetail> GetByFilter(Func<rawEventDiggingDetail, bool> filterFunc,string tag="") {
			List<rawEventDiggingDetail> ret = new List<rawEventDiggingDetail>();
			foreach(KeyValuePair <int, rawEventDiggingDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDiggingDetail? GetOneByFilter(Func<rawEventDiggingDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDiggingDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDiggingDetail data) 
		{
			return data.Id;			
		}
    }
