...
using DailyPopPackConf = fat.rawdata.DailyPopPackConf;
using rawDailyPopPack = fat.rawdata.DailyPopPack;
using rawDailyPopPackMapABValue = fat.rawdata.DailyPopPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DailyPopPackVisitor
    {
		...
		public const string FileName = "DailyPopPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DailyPopPackVisitor.SetABDefault单独设定DailyPopPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DailyPopPackVisitor.SetABFallback单独设定DailyPopPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDailyPopPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDailyPopPack>>(()=> new PBC::MapField<int, rawDailyPopPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDailyPopPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DailyPopPackMap;
			var abMap = v.DailyPopPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DailyPopPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DailyPopPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDailyPopPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DailyPopPackMap;
			if (v.DailyPopPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DailyPopPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDailyPopPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DailyPopPackConf cc) {
			foreach(KeyValuePair <string, rawDailyPopPackMapABValue> tagEntry in cc.DailyPopPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DailyPopPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDailyPopPackMapABValue();
					cc.DailyPopPackMapAB[tag] = ps;
				}
                var tagTmp = ps.DailyPopPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDailyPopPack> entry in cc.DailyPopPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDailyPopPack> entry in tagEntry.Value.DailyPopPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DailyPopPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DailyPopPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DailyPopPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DailyPopPackMapAB != null)
			{
				foreach (var entry in cc.DailyPopPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DailyPopPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DailyPopPackMap.MergeFrom(cis);
			Patch(updating.DailyPopPackMap);
			updating.DailyPopPackSortedKeys = genSortedKeys(updating.DailyPopPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DailyPopPackConf();
            tmp.MergeFrom(cis);
			updating.DailyPopPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DailyPopPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDailyPopPack> filter,string tag=""){
		...
		public static rawDailyPopPack MustGet(int key,string tag="") {
		...
		public static rawDailyPopPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDailyPopPack> GetByFilter(Func<rawDailyPopPack, bool> filterFunc,string tag="") {
			List<rawDailyPopPack> ret = new List<rawDailyPopPack>();
			foreach(KeyValuePair <int, rawDailyPopPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDailyPopPack? GetOneByFilter(Func<rawDailyPopPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDailyPopPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDailyPopPack data) 
		{
			return data.Id;			
		}
    }
