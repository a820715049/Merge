...
using MergeBoardEnergyConf = fat.rawdata.MergeBoardEnergyConf;
using rawMergeBoardEnergy = fat.rawdata.MergeBoardEnergy;
using rawMergeBoardEnergyMapABValue = fat.rawdata.MergeBoardEnergyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeBoardEnergyVisitor
    {
		...
		public const string FileName = "MergeBoardEnergyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeBoardEnergyVisitor.SetABDefault单独设定MergeBoardEnergy的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeBoardEnergyVisitor.SetABFallback单独设定MergeBoardEnergy的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeBoardEnergy>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeBoardEnergy>>(()=> new PBC::MapField<int, rawMergeBoardEnergy>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeBoardEnergy>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeBoardEnergyMap;
			var abMap = v.MergeBoardEnergyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeBoardEnergyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeBoardEnergyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeBoardEnergy> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeBoardEnergyMap;
			if (v.MergeBoardEnergyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeBoardEnergyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeBoardEnergyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeBoardEnergyConf cc) {
			foreach(KeyValuePair <string, rawMergeBoardEnergyMapABValue> tagEntry in cc.MergeBoardEnergyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeBoardEnergyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeBoardEnergyMapABValue();
					cc.MergeBoardEnergyMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeBoardEnergyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeBoardEnergy> entry in cc.MergeBoardEnergyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeBoardEnergy> entry in tagEntry.Value.MergeBoardEnergyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeBoardEnergy> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeBoardEnergyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeBoardEnergyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeBoardEnergyMapAB != null)
			{
				foreach (var entry in cc.MergeBoardEnergyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeBoardEnergyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeBoardEnergyMap.MergeFrom(cis);
			Patch(updating.MergeBoardEnergyMap);
			updating.MergeBoardEnergySortedKeys = genSortedKeys(updating.MergeBoardEnergyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeBoardEnergyConf();
            tmp.MergeFrom(cis);
			updating.MergeBoardEnergyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeBoardEnergySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeBoardEnergy> filter,string tag=""){
		...
		public static rawMergeBoardEnergy MustGet(int key,string tag="") {
		...
		public static rawMergeBoardEnergy? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeBoardEnergy> GetByFilter(Func<rawMergeBoardEnergy, bool> filterFunc,string tag="") {
			List<rawMergeBoardEnergy> ret = new List<rawMergeBoardEnergy>();
			foreach(KeyValuePair <int, rawMergeBoardEnergy> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeBoardEnergy? GetOneByFilter(Func<rawMergeBoardEnergy, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeBoardEnergy> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeBoardEnergy data) 
		{
			return data.Id;			
		}
    }
