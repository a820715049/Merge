...
using EventTriggerConf = fat.rawdata.EventTriggerConf;
using rawEventTrigger = fat.rawdata.EventTrigger;
using rawEventTriggerMapABValue = fat.rawdata.EventTriggerMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTriggerVisitor
    {
		...
		public const string FileName = "EventTriggerConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTriggerVisitor.SetABDefault单独设定EventTrigger的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTriggerVisitor.SetABFallback单独设定EventTrigger的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTrigger>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTrigger>>(()=> new PBC::MapField<int, rawEventTrigger>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTrigger>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTriggerMap;
			var abMap = v.EventTriggerMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTriggerMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTriggerMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTrigger> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTriggerMap;
			if (v.EventTriggerMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTriggerMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTriggerMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTriggerConf cc) {
			foreach(KeyValuePair <string, rawEventTriggerMapABValue> tagEntry in cc.EventTriggerMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTriggerMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTriggerMapABValue();
					cc.EventTriggerMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTriggerMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTrigger> entry in cc.EventTriggerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTrigger> entry in tagEntry.Value.EventTriggerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTrigger> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTriggerConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTriggerMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTriggerMapAB != null)
			{
				foreach (var entry in cc.EventTriggerMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTriggerMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTriggerMap.MergeFrom(cis);
			Patch(updating.EventTriggerMap);
			updating.EventTriggerSortedKeys = genSortedKeys(updating.EventTriggerMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTriggerConf();
            tmp.MergeFrom(cis);
			updating.EventTriggerMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTriggerSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTrigger> filter,string tag=""){
		...
		public static rawEventTrigger MustGet(int key,string tag="") {
		...
		public static rawEventTrigger? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTrigger> GetByFilter(Func<rawEventTrigger, bool> filterFunc,string tag="") {
			List<rawEventTrigger> ret = new List<rawEventTrigger>();
			foreach(KeyValuePair <int, rawEventTrigger> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTrigger? GetOneByFilter(Func<rawEventTrigger, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTrigger> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTrigger data) 
		{
			return data.Id;			
		}
    }
