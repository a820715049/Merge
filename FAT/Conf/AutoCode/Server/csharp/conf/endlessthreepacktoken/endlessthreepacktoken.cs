...
using EndlessThreePackTokenConf = fat.rawdata.EndlessThreePackTokenConf;
using rawEndlessThreePackToken = fat.rawdata.EndlessThreePackToken;
using rawEndlessThreePackTokenMapABValue = fat.rawdata.EndlessThreePackTokenMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessThreePackTokenVisitor
    {
		...
		public const string FileName = "EndlessThreePackTokenConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessThreePackTokenVisitor.SetABDefault单独设定EndlessThreePackToken的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessThreePackTokenVisitor.SetABFallback单独设定EndlessThreePackToken的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessThreePackToken>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessThreePackToken>>(()=> new PBC::MapField<int, rawEndlessThreePackToken>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessThreePackToken>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessThreePackTokenMap;
			var abMap = v.EndlessThreePackTokenMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessThreePackTokenMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessThreePackTokenMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessThreePackToken> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessThreePackTokenMap;
			if (v.EndlessThreePackTokenMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessThreePackTokenMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessThreePackTokenMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessThreePackTokenConf cc) {
			foreach(KeyValuePair <string, rawEndlessThreePackTokenMapABValue> tagEntry in cc.EndlessThreePackTokenMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessThreePackTokenMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessThreePackTokenMapABValue();
					cc.EndlessThreePackTokenMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessThreePackTokenMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessThreePackToken> entry in cc.EndlessThreePackTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessThreePackToken> entry in tagEntry.Value.EndlessThreePackTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessThreePackToken> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessThreePackTokenConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessThreePackTokenMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessThreePackTokenMapAB != null)
			{
				foreach (var entry in cc.EndlessThreePackTokenMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessThreePackTokenMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessThreePackTokenMap.MergeFrom(cis);
			Patch(updating.EndlessThreePackTokenMap);
			updating.EndlessThreePackTokenSortedKeys = genSortedKeys(updating.EndlessThreePackTokenMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessThreePackTokenConf();
            tmp.MergeFrom(cis);
			updating.EndlessThreePackTokenMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessThreePackTokenSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessThreePackToken> filter,string tag=""){
		...
		public static rawEndlessThreePackToken MustGet(int key,string tag="") {
		...
		public static rawEndlessThreePackToken? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessThreePackToken> GetByFilter(Func<rawEndlessThreePackToken, bool> filterFunc,string tag="") {
			List<rawEndlessThreePackToken> ret = new List<rawEndlessThreePackToken>();
			foreach(KeyValuePair <int, rawEndlessThreePackToken> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessThreePackToken? GetOneByFilter(Func<rawEndlessThreePackToken, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessThreePackToken> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessThreePackToken data) 
		{
			return data.Id;			
		}
    }
