...
using EventWishMilestoneConf = fat.rawdata.EventWishMilestoneConf;
using rawEventWishMilestone = fat.rawdata.EventWishMilestone;
using rawEventWishMilestoneMapABValue = fat.rawdata.EventWishMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishMilestoneVisitor
    {
		...
		public const string FileName = "EventWishMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishMilestoneVisitor.SetABDefault单独设定EventWishMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishMilestoneVisitor.SetABFallback单独设定EventWishMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishMilestone>>(()=> new PBC::MapField<int, rawEventWishMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishMilestoneMap;
			var abMap = v.EventWishMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishMilestoneMap;
			if (v.EventWishMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishMilestoneConf cc) {
			foreach(KeyValuePair <string, rawEventWishMilestoneMapABValue> tagEntry in cc.EventWishMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishMilestoneMapABValue();
					cc.EventWishMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishMilestone> entry in cc.EventWishMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishMilestone> entry in tagEntry.Value.EventWishMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishMilestoneMapAB != null)
			{
				foreach (var entry in cc.EventWishMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishMilestoneMap.MergeFrom(cis);
			Patch(updating.EventWishMilestoneMap);
			updating.EventWishMilestoneSortedKeys = genSortedKeys(updating.EventWishMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishMilestoneConf();
            tmp.MergeFrom(cis);
			updating.EventWishMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishMilestone> filter,string tag=""){
		...
		public static rawEventWishMilestone MustGet(int key,string tag="") {
		...
		public static rawEventWishMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishMilestone> GetByFilter(Func<rawEventWishMilestone, bool> filterFunc,string tag="") {
			List<rawEventWishMilestone> ret = new List<rawEventWishMilestone>();
			foreach(KeyValuePair <int, rawEventWishMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishMilestone? GetOneByFilter(Func<rawEventWishMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishMilestone data) 
		{
			return data.Id;			
		}
    }
