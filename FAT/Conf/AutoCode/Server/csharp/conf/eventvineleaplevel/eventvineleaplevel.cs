...
using EventVineLeapLevelConf = fat.rawdata.EventVineLeapLevelConf;
using rawEventVineLeapLevel = fat.rawdata.EventVineLeapLevel;
using rawEventVineLeapLevelMapABValue = fat.rawdata.EventVineLeapLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventVineLeapLevelVisitor
    {
		...
		public const string FileName = "EventVineLeapLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventVineLeapLevelVisitor.SetABDefault单独设定EventVineLeapLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventVineLeapLevelVisitor.SetABFallback单独设定EventVineLeapLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventVineLeapLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventVineLeapLevel>>(()=> new PBC::MapField<int, rawEventVineLeapLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventVineLeapLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventVineLeapLevelMap;
			var abMap = v.EventVineLeapLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventVineLeapLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventVineLeapLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventVineLeapLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventVineLeapLevelMap;
			if (v.EventVineLeapLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventVineLeapLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventVineLeapLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventVineLeapLevelConf cc) {
			foreach(KeyValuePair <string, rawEventVineLeapLevelMapABValue> tagEntry in cc.EventVineLeapLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventVineLeapLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventVineLeapLevelMapABValue();
					cc.EventVineLeapLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventVineLeapLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventVineLeapLevel> entry in cc.EventVineLeapLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventVineLeapLevel> entry in tagEntry.Value.EventVineLeapLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventVineLeapLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventVineLeapLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventVineLeapLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventVineLeapLevelMapAB != null)
			{
				foreach (var entry in cc.EventVineLeapLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventVineLeapLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventVineLeapLevelMap.MergeFrom(cis);
			Patch(updating.EventVineLeapLevelMap);
			updating.EventVineLeapLevelSortedKeys = genSortedKeys(updating.EventVineLeapLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventVineLeapLevelConf();
            tmp.MergeFrom(cis);
			updating.EventVineLeapLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventVineLeapLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventVineLeapLevel> filter,string tag=""){
		...
		public static rawEventVineLeapLevel MustGet(int key,string tag="") {
		...
		public static rawEventVineLeapLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventVineLeapLevel> GetByFilter(Func<rawEventVineLeapLevel, bool> filterFunc,string tag="") {
			List<rawEventVineLeapLevel> ret = new List<rawEventVineLeapLevel>();
			foreach(KeyValuePair <int, rawEventVineLeapLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventVineLeapLevel? GetOneByFilter(Func<rawEventVineLeapLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventVineLeapLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventVineLeapLevel data) 
		{
			return data.Id;			
		}
    }
