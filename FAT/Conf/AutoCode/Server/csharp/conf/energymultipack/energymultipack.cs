...
using EnergyMultiPackConf = fat.rawdata.EnergyMultiPackConf;
using rawEnergyMultiPack = fat.rawdata.EnergyMultiPack;
using rawEnergyMultiPackMapABValue = fat.rawdata.EnergyMultiPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EnergyMultiPackVisitor
    {
		...
		public const string FileName = "EnergyMultiPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EnergyMultiPackVisitor.SetABDefault单独设定EnergyMultiPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EnergyMultiPackVisitor.SetABFallback单独设定EnergyMultiPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEnergyMultiPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEnergyMultiPack>>(()=> new PBC::MapField<int, rawEnergyMultiPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEnergyMultiPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EnergyMultiPackMap;
			var abMap = v.EnergyMultiPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EnergyMultiPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EnergyMultiPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEnergyMultiPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EnergyMultiPackMap;
			if (v.EnergyMultiPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EnergyMultiPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEnergyMultiPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EnergyMultiPackConf cc) {
			foreach(KeyValuePair <string, rawEnergyMultiPackMapABValue> tagEntry in cc.EnergyMultiPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EnergyMultiPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEnergyMultiPackMapABValue();
					cc.EnergyMultiPackMapAB[tag] = ps;
				}
                var tagTmp = ps.EnergyMultiPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEnergyMultiPack> entry in cc.EnergyMultiPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEnergyMultiPack> entry in tagEntry.Value.EnergyMultiPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EnergyMultiPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EnergyMultiPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EnergyMultiPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EnergyMultiPackMapAB != null)
			{
				foreach (var entry in cc.EnergyMultiPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EnergyMultiPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EnergyMultiPackMap.MergeFrom(cis);
			Patch(updating.EnergyMultiPackMap);
			updating.EnergyMultiPackSortedKeys = genSortedKeys(updating.EnergyMultiPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EnergyMultiPackConf();
            tmp.MergeFrom(cis);
			updating.EnergyMultiPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EnergyMultiPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEnergyMultiPack> filter,string tag=""){
		...
		public static rawEnergyMultiPack MustGet(int key,string tag="") {
		...
		public static rawEnergyMultiPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEnergyMultiPack> GetByFilter(Func<rawEnergyMultiPack, bool> filterFunc,string tag="") {
			List<rawEnergyMultiPack> ret = new List<rawEnergyMultiPack>();
			foreach(KeyValuePair <int, rawEnergyMultiPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEnergyMultiPack? GetOneByFilter(Func<rawEnergyMultiPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEnergyMultiPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEnergyMultiPack data) 
		{
			return data.Id;			
		}
    }
