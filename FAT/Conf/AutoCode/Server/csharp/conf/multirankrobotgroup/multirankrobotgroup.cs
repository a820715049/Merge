...
using MultiRankRobotGroupConf = fat.rawdata.MultiRankRobotGroupConf;
using rawMultiRankRobotGroup = fat.rawdata.MultiRankRobotGroup;
using rawMultiRankRobotGroupMapABValue = fat.rawdata.MultiRankRobotGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankRobotGroupVisitor
    {
		...
		public const string FileName = "MultiRankRobotGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankRobotGroupVisitor.SetABDefault单独设定MultiRankRobotGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankRobotGroupVisitor.SetABFallback单独设定MultiRankRobotGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRankRobotGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRankRobotGroup>>(()=> new PBC::MapField<int, rawMultiRankRobotGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRankRobotGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankRobotGroupMap;
			var abMap = v.MultiRankRobotGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankRobotGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankRobotGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRankRobotGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankRobotGroupMap;
			if (v.MultiRankRobotGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankRobotGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankRobotGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankRobotGroupConf cc) {
			foreach(KeyValuePair <string, rawMultiRankRobotGroupMapABValue> tagEntry in cc.MultiRankRobotGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankRobotGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankRobotGroupMapABValue();
					cc.MultiRankRobotGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankRobotGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRankRobotGroup> entry in cc.MultiRankRobotGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRankRobotGroup> entry in tagEntry.Value.MultiRankRobotGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRankRobotGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankRobotGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankRobotGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankRobotGroupMapAB != null)
			{
				foreach (var entry in cc.MultiRankRobotGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankRobotGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankRobotGroupMap.MergeFrom(cis);
			Patch(updating.MultiRankRobotGroupMap);
			updating.MultiRankRobotGroupSortedKeys = genSortedKeys(updating.MultiRankRobotGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankRobotGroupConf();
            tmp.MergeFrom(cis);
			updating.MultiRankRobotGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankRobotGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRankRobotGroup> filter,string tag=""){
		...
		public static rawMultiRankRobotGroup MustGet(int key,string tag="") {
		...
		public static rawMultiRankRobotGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRankRobotGroup> GetByFilter(Func<rawMultiRankRobotGroup, bool> filterFunc,string tag="") {
			List<rawMultiRankRobotGroup> ret = new List<rawMultiRankRobotGroup>();
			foreach(KeyValuePair <int, rawMultiRankRobotGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRankRobotGroup? GetOneByFilter(Func<rawMultiRankRobotGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRankRobotGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRankRobotGroup data) 
		{
			return data.Id;			
		}
    }
