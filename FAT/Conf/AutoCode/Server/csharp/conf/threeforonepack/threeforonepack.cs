...
using ThreeForOnePackConf = fat.rawdata.ThreeForOnePackConf;
using rawThreeForOnePack = fat.rawdata.ThreeForOnePack;
using rawThreeForOnePackMapABValue = fat.rawdata.ThreeForOnePackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ThreeForOnePackVisitor
    {
		...
		public const string FileName = "ThreeForOnePackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ThreeForOnePackVisitor.SetABDefault单独设定ThreeForOnePack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ThreeForOnePackVisitor.SetABFallback单独设定ThreeForOnePack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawThreeForOnePack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawThreeForOnePack>>(()=> new PBC::MapField<int, rawThreeForOnePack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawThreeForOnePack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ThreeForOnePackMap;
			var abMap = v.ThreeForOnePackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ThreeForOnePackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ThreeForOnePackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawThreeForOnePack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ThreeForOnePackMap;
			if (v.ThreeForOnePackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ThreeForOnePackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawThreeForOnePackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ThreeForOnePackConf cc) {
			foreach(KeyValuePair <string, rawThreeForOnePackMapABValue> tagEntry in cc.ThreeForOnePackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ThreeForOnePackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawThreeForOnePackMapABValue();
					cc.ThreeForOnePackMapAB[tag] = ps;
				}
                var tagTmp = ps.ThreeForOnePackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawThreeForOnePack> entry in cc.ThreeForOnePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawThreeForOnePack> entry in tagEntry.Value.ThreeForOnePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ThreeForOnePack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ThreeForOnePackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ThreeForOnePackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ThreeForOnePackMapAB != null)
			{
				foreach (var entry in cc.ThreeForOnePackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ThreeForOnePackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ThreeForOnePackMap.MergeFrom(cis);
			Patch(updating.ThreeForOnePackMap);
			updating.ThreeForOnePackSortedKeys = genSortedKeys(updating.ThreeForOnePackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ThreeForOnePackConf();
            tmp.MergeFrom(cis);
			updating.ThreeForOnePackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ThreeForOnePackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawThreeForOnePack> filter,string tag=""){
		...
		public static rawThreeForOnePack MustGet(int key,string tag="") {
		...
		public static rawThreeForOnePack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawThreeForOnePack> GetByFilter(Func<rawThreeForOnePack, bool> filterFunc,string tag="") {
			List<rawThreeForOnePack> ret = new List<rawThreeForOnePack>();
			foreach(KeyValuePair <int, rawThreeForOnePack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawThreeForOnePack? GetOneByFilter(Func<rawThreeForOnePack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawThreeForOnePack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawThreeForOnePack data) 
		{
			return data.Id;			
		}
    }
