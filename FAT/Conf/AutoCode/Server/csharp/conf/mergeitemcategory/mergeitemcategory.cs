...
using MergeItemCategoryConf = fat.rawdata.MergeItemCategoryConf;
using rawMergeItemCategory = fat.rawdata.MergeItemCategory;
using rawMergeItemCategoryMapABValue = fat.rawdata.MergeItemCategoryMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeItemCategoryVisitor
    {
		...
		public const string FileName = "MergeItemCategoryConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeItemCategoryVisitor.SetABDefault单独设定MergeItemCategory的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeItemCategoryVisitor.SetABFallback单独设定MergeItemCategory的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawMergeItemCategory>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawMergeItemCategory>>(()=> new PBC::MapField<string, rawMergeItemCategory>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawMergeItemCategory>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeItemCategoryMap;
			var abMap = v.MergeItemCategoryMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeItemCategoryMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeItemCategoryMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawMergeItemCategory> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeItemCategoryMap;
			if (v.MergeItemCategoryMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeItemCategoryMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeItemCategoryMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeItemCategoryConf cc) {
			foreach(KeyValuePair <string, rawMergeItemCategoryMapABValue> tagEntry in cc.MergeItemCategoryMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeItemCategoryMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeItemCategoryMapABValue();
					cc.MergeItemCategoryMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeItemCategoryMap;
				// 基础数据
				foreach(KeyValuePair <string, rawMergeItemCategory> entry in cc.MergeItemCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawMergeItemCategory> entry in tagEntry.Value.MergeItemCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.MergeItemCategory> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(MergeItemCategoryConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.MergeItemCategoryMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeItemCategoryMapAB != null)
			{
				foreach (var entry in cc.MergeItemCategoryMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.MergeItemCategoryMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeItemCategoryMap.MergeFrom(cis);
			Patch(updating.MergeItemCategoryMap);
			updating.MergeItemCategorySortedKeys = genSortedKeys(updating.MergeItemCategoryMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeItemCategoryConf();
            tmp.MergeFrom(cis);
			updating.MergeItemCategoryMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeItemCategorySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawMergeItemCategory> filter,string tag=""){
		...
		public static rawMergeItemCategory MustGet(string key,string tag="") {
		...
		public static rawMergeItemCategory? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeItemCategory> GetByFilter(Func<rawMergeItemCategory, bool> filterFunc,string tag="") {
			List<rawMergeItemCategory> ret = new List<rawMergeItemCategory>();
			foreach(KeyValuePair <string, rawMergeItemCategory> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeItemCategory? GetOneByFilter(Func<rawMergeItemCategory, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawMergeItemCategory> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawMergeItemCategory data) 
		{
        ...
        public class CK {
            ...
            public static void MergeItemCategoryCK() {}
        ...
        public static rawMergeItemCategory?  GetByCombinedKeys(CK ck,string tag="") {
