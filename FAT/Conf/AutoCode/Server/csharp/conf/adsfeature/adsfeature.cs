...
using AdsFeatureConf = fat.rawdata.AdsFeatureConf;
using rawAdsFeature = fat.rawdata.AdsFeature;
using rawAdsFeatureSliceABValue = fat.rawdata.AdsFeatureSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class AdsFeatureVisitor
    {
		...
		public const string FileName = "AdsFeatureConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过AdsFeatureVisitor.SetABDefault单独设定AdsFeature的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过AdsFeatureVisitor.SetABFallback单独设定AdsFeature的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawAdsFeature>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawAdsFeature>>(()=>new PBC::RepeatedField<rawAdsFeature>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawAdsFeature>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().AdsFeatureSlice;
			var abMap = v.AdsFeatureSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.AdsFeatureSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].AdsFeatureSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawAdsFeature> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().AdsFeatureSlice;
			if (v.AdsFeatureSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.AdsFeatureSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawAdsFeatureSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(AdsFeatureConf cc) {
			foreach(KeyValuePair <string, rawAdsFeatureSliceABValue> tagEntry in cc.AdsFeatureSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.AdsFeatureSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawAdsFeatureSliceABValue();
					cc.AdsFeatureSliceAB[tag] = ps;
				}
                var tagTmp = ps.AdsFeatureSlice;
				// 基础数据
				for(int i=0; i<cc.AdsFeatureSlice.Count; ++i) 
				{
					tagTmp.Add(cc.AdsFeatureSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.AdsFeatureSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.AdsFeature> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.AdsFeatureSlice.MergeFrom(cis);
			Patch(updating.AdsFeatureSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new AdsFeatureConf();
            tmp.MergeFrom(cis);
			updating.AdsFeatureSlice = tmp;
		...
		public static void Range(Action<int, rawAdsFeature> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawAdsFeature MustGetByIndex(int index,string tag="") {
		...
		public static rawAdsFeature? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawAdsFeature> GetByFilter(Func<rawAdsFeature,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawAdsFeature>();
		...
		static public rawAdsFeature? GetOneByFilter(Func<rawAdsFeature,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
