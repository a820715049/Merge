...
using EventSeaMilestoneRewardConf = fat.rawdata.EventSeaMilestoneRewardConf;
using rawEventSeaMilestoneReward = fat.rawdata.EventSeaMilestoneReward;
using rawEventSeaMilestoneRewardMapABValue = fat.rawdata.EventSeaMilestoneRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaMilestoneRewardVisitor
    {
		...
		public const string FileName = "EventSeaMilestoneRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaMilestoneRewardVisitor.SetABDefault单独设定EventSeaMilestoneReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaMilestoneRewardVisitor.SetABFallback单独设定EventSeaMilestoneReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaMilestoneReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaMilestoneReward>>(()=> new PBC::MapField<int, rawEventSeaMilestoneReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaMilestoneReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaMilestoneRewardMap;
			var abMap = v.EventSeaMilestoneRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaMilestoneRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaMilestoneRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaMilestoneReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaMilestoneRewardMap;
			if (v.EventSeaMilestoneRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaMilestoneRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaMilestoneRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaMilestoneRewardConf cc) {
			foreach(KeyValuePair <string, rawEventSeaMilestoneRewardMapABValue> tagEntry in cc.EventSeaMilestoneRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaMilestoneRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaMilestoneRewardMapABValue();
					cc.EventSeaMilestoneRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaMilestoneRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaMilestoneReward> entry in cc.EventSeaMilestoneRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaMilestoneReward> entry in tagEntry.Value.EventSeaMilestoneRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaMilestoneReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaMilestoneRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaMilestoneRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaMilestoneRewardMapAB != null)
			{
				foreach (var entry in cc.EventSeaMilestoneRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaMilestoneRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaMilestoneRewardMap.MergeFrom(cis);
			Patch(updating.EventSeaMilestoneRewardMap);
			updating.EventSeaMilestoneRewardSortedKeys = genSortedKeys(updating.EventSeaMilestoneRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaMilestoneRewardConf();
            tmp.MergeFrom(cis);
			updating.EventSeaMilestoneRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaMilestoneRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaMilestoneReward> filter,string tag=""){
		...
		public static rawEventSeaMilestoneReward MustGet(int key,string tag="") {
		...
		public static rawEventSeaMilestoneReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaMilestoneReward> GetByFilter(Func<rawEventSeaMilestoneReward, bool> filterFunc,string tag="") {
			List<rawEventSeaMilestoneReward> ret = new List<rawEventSeaMilestoneReward>();
			foreach(KeyValuePair <int, rawEventSeaMilestoneReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaMilestoneReward? GetOneByFilter(Func<rawEventSeaMilestoneReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaMilestoneReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaMilestoneReward data) 
		{
			return data.Id;			
		}
    }
