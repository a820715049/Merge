...
using EndlessThreePackConf = fat.rawdata.EndlessThreePackConf;
using rawEndlessThreePack = fat.rawdata.EndlessThreePack;
using rawEndlessThreePackMapABValue = fat.rawdata.EndlessThreePackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessThreePackVisitor
    {
		...
		public const string FileName = "EndlessThreePackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessThreePackVisitor.SetABDefault单独设定EndlessThreePack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessThreePackVisitor.SetABFallback单独设定EndlessThreePack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessThreePack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessThreePack>>(()=> new PBC::MapField<int, rawEndlessThreePack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessThreePack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessThreePackMap;
			var abMap = v.EndlessThreePackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessThreePackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessThreePackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessThreePack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessThreePackMap;
			if (v.EndlessThreePackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessThreePackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessThreePackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessThreePackConf cc) {
			foreach(KeyValuePair <string, rawEndlessThreePackMapABValue> tagEntry in cc.EndlessThreePackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessThreePackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessThreePackMapABValue();
					cc.EndlessThreePackMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessThreePackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessThreePack> entry in cc.EndlessThreePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessThreePack> entry in tagEntry.Value.EndlessThreePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessThreePack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessThreePackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessThreePackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessThreePackMapAB != null)
			{
				foreach (var entry in cc.EndlessThreePackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessThreePackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessThreePackMap.MergeFrom(cis);
			Patch(updating.EndlessThreePackMap);
			updating.EndlessThreePackSortedKeys = genSortedKeys(updating.EndlessThreePackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessThreePackConf();
            tmp.MergeFrom(cis);
			updating.EndlessThreePackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessThreePackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessThreePack> filter,string tag=""){
		...
		public static rawEndlessThreePack MustGet(int key,string tag="") {
		...
		public static rawEndlessThreePack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessThreePack> GetByFilter(Func<rawEndlessThreePack, bool> filterFunc,string tag="") {
			List<rawEndlessThreePack> ret = new List<rawEndlessThreePack>();
			foreach(KeyValuePair <int, rawEndlessThreePack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessThreePack? GetOneByFilter(Func<rawEndlessThreePack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessThreePack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessThreePack data) 
		{
			return data.Id;			
		}
    }
