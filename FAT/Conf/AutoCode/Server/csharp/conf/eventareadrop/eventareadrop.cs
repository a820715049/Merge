...
using EventAreaDropConf = fat.rawdata.EventAreaDropConf;
using rawEventAreaDrop = fat.rawdata.EventAreaDrop;
using rawEventAreaDropMapABValue = fat.rawdata.EventAreaDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaDropVisitor
    {
		...
		public const string FileName = "EventAreaDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaDropVisitor.SetABDefault单独设定EventAreaDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaDropVisitor.SetABFallback单独设定EventAreaDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaDrop>>(()=> new PBC::MapField<int, rawEventAreaDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaDropMap;
			var abMap = v.EventAreaDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaDropMap;
			if (v.EventAreaDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaDropConf cc) {
			foreach(KeyValuePair <string, rawEventAreaDropMapABValue> tagEntry in cc.EventAreaDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaDropMapABValue();
					cc.EventAreaDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaDrop> entry in cc.EventAreaDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaDrop> entry in tagEntry.Value.EventAreaDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaDropMapAB != null)
			{
				foreach (var entry in cc.EventAreaDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaDropMap.MergeFrom(cis);
			Patch(updating.EventAreaDropMap);
			updating.EventAreaDropSortedKeys = genSortedKeys(updating.EventAreaDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaDropConf();
            tmp.MergeFrom(cis);
			updating.EventAreaDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaDrop> filter,string tag=""){
		...
		public static rawEventAreaDrop MustGet(int key,string tag="") {
		...
		public static rawEventAreaDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaDrop> GetByFilter(Func<rawEventAreaDrop, bool> filterFunc,string tag="") {
			List<rawEventAreaDrop> ret = new List<rawEventAreaDrop>();
			foreach(KeyValuePair <int, rawEventAreaDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaDrop? GetOneByFilter(Func<rawEventAreaDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaDrop data) 
		{
			return data.Id;			
		}
    }
