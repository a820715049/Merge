...
using EventMiniBoardConf = fat.rawdata.EventMiniBoardConf;
using rawEventMiniBoard = fat.rawdata.EventMiniBoard;
using rawEventMiniBoardMapABValue = fat.rawdata.EventMiniBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardVisitor
    {
		...
		public const string FileName = "EventMiniBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardVisitor.SetABDefault单独设定EventMiniBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardVisitor.SetABFallback单独设定EventMiniBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoard>>(()=> new PBC::MapField<int, rawEventMiniBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardMap;
			var abMap = v.EventMiniBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardMap;
			if (v.EventMiniBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardMapABValue> tagEntry in cc.EventMiniBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardMapABValue();
					cc.EventMiniBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoard> entry in cc.EventMiniBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoard> entry in tagEntry.Value.EventMiniBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardMap);
			updating.EventMiniBoardSortedKeys = genSortedKeys(updating.EventMiniBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoard> filter,string tag=""){
		...
		public static rawEventMiniBoard MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoard> GetByFilter(Func<rawEventMiniBoard, bool> filterFunc,string tag="") {
			List<rawEventMiniBoard> ret = new List<rawEventMiniBoard>();
			foreach(KeyValuePair <int, rawEventMiniBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoard? GetOneByFilter(Func<rawEventMiniBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoard data) 
		{
			return data.Id;			
		}
    }
