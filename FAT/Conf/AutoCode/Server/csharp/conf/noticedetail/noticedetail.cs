...
using NoticeDetailConf = fat.rawdata.NoticeDetailConf;
using rawNoticeDetail = fat.rawdata.NoticeDetail;
using rawNoticeDetailMapABValue = fat.rawdata.NoticeDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class NoticeDetailVisitor
    {
		...
		public const string FileName = "NoticeDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过NoticeDetailVisitor.SetABDefault单独设定NoticeDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过NoticeDetailVisitor.SetABFallback单独设定NoticeDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawNoticeDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawNoticeDetail>>(()=> new PBC::MapField<int, rawNoticeDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawNoticeDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().NoticeDetailMap;
			var abMap = v.NoticeDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.NoticeDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].NoticeDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawNoticeDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().NoticeDetailMap;
			if (v.NoticeDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.NoticeDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawNoticeDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(NoticeDetailConf cc) {
			foreach(KeyValuePair <string, rawNoticeDetailMapABValue> tagEntry in cc.NoticeDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.NoticeDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawNoticeDetailMapABValue();
					cc.NoticeDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.NoticeDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawNoticeDetail> entry in cc.NoticeDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawNoticeDetail> entry in tagEntry.Value.NoticeDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.NoticeDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(NoticeDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.NoticeDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.NoticeDetailMapAB != null)
			{
				foreach (var entry in cc.NoticeDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.NoticeDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.NoticeDetailMap.MergeFrom(cis);
			Patch(updating.NoticeDetailMap);
			updating.NoticeDetailSortedKeys = genSortedKeys(updating.NoticeDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new NoticeDetailConf();
            tmp.MergeFrom(cis);
			updating.NoticeDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().NoticeDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawNoticeDetail> filter,string tag=""){
		...
		public static rawNoticeDetail MustGet(int key,string tag="") {
		...
		public static rawNoticeDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawNoticeDetail> GetByFilter(Func<rawNoticeDetail, bool> filterFunc,string tag="") {
			List<rawNoticeDetail> ret = new List<rawNoticeDetail>();
			foreach(KeyValuePair <int, rawNoticeDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawNoticeDetail? GetOneByFilter(Func<rawNoticeDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawNoticeDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawNoticeDetail data) 
		{
			return data.Id;			
		}
    }
