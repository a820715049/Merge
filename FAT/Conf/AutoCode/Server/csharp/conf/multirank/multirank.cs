...
using MultiRankConf = fat.rawdata.MultiRankConf;
using rawMultiRank = fat.rawdata.MultiRank;
using rawMultiRankMapABValue = fat.rawdata.MultiRankMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankVisitor
    {
		...
		public const string FileName = "MultiRankConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankVisitor.SetABDefault单独设定MultiRank的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankVisitor.SetABFallback单独设定MultiRank的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRank>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRank>>(()=> new PBC::MapField<int, rawMultiRank>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRank>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankMap;
			var abMap = v.MultiRankMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRank> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankMap;
			if (v.MultiRankMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankConf cc) {
			foreach(KeyValuePair <string, rawMultiRankMapABValue> tagEntry in cc.MultiRankMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankMapABValue();
					cc.MultiRankMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRank> entry in cc.MultiRankMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRank> entry in tagEntry.Value.MultiRankMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRank> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankMapAB != null)
			{
				foreach (var entry in cc.MultiRankMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankMap.MergeFrom(cis);
			Patch(updating.MultiRankMap);
			updating.MultiRankSortedKeys = genSortedKeys(updating.MultiRankMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankConf();
            tmp.MergeFrom(cis);
			updating.MultiRankMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRank> filter,string tag=""){
		...
		public static rawMultiRank MustGet(int key,string tag="") {
		...
		public static rawMultiRank? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRank> GetByFilter(Func<rawMultiRank, bool> filterFunc,string tag="") {
			List<rawMultiRank> ret = new List<rawMultiRank>();
			foreach(KeyValuePair <int, rawMultiRank> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRank? GetOneByFilter(Func<rawMultiRank, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRank> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRank data) 
		{
			return data.Id;			
		}
    }
