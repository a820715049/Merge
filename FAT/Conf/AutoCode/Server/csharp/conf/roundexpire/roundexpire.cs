...
using RoundExpireConf = fat.rawdata.RoundExpireConf;
using rawRoundExpire = fat.rawdata.RoundExpire;
using rawRoundExpireSliceABValue = fat.rawdata.RoundExpireSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RoundExpireVisitor
    {
		...
		public const string FileName = "RoundExpireConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RoundExpireVisitor.SetABDefault单独设定RoundExpire的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RoundExpireVisitor.SetABFallback单独设定RoundExpire的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawRoundExpire>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawRoundExpire>>(()=>new PBC::RepeatedField<rawRoundExpire>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawRoundExpire>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RoundExpireSlice;
			var abMap = v.RoundExpireSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RoundExpireSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RoundExpireSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawRoundExpire> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RoundExpireSlice;
			if (v.RoundExpireSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RoundExpireSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRoundExpireSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RoundExpireConf cc) {
			foreach(KeyValuePair <string, rawRoundExpireSliceABValue> tagEntry in cc.RoundExpireSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RoundExpireSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRoundExpireSliceABValue();
					cc.RoundExpireSliceAB[tag] = ps;
				}
                var tagTmp = ps.RoundExpireSlice;
				// 基础数据
				for(int i=0; i<cc.RoundExpireSlice.Count; ++i) 
				{
					tagTmp.Add(cc.RoundExpireSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.RoundExpireSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.RoundExpire> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RoundExpireSlice.MergeFrom(cis);
			Patch(updating.RoundExpireSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RoundExpireConf();
            tmp.MergeFrom(cis);
			updating.RoundExpireSlice = tmp;
		...
		public static void Range(Action<int, rawRoundExpire> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawRoundExpire MustGetByIndex(int index,string tag="") {
		...
		public static rawRoundExpire? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawRoundExpire> GetByFilter(Func<rawRoundExpire,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawRoundExpire>();
		...
		static public rawRoundExpire? GetOneByFilter(Func<rawRoundExpire,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
