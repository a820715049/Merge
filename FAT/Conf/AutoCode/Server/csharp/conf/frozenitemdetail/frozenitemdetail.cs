...
using FrozenItemDetailConf = fat.rawdata.FrozenItemDetailConf;
using rawFrozenItemDetail = fat.rawdata.FrozenItemDetail;
using rawFrozenItemDetailMapABValue = fat.rawdata.FrozenItemDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class FrozenItemDetailVisitor
    {
		...
		public const string FileName = "FrozenItemDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过FrozenItemDetailVisitor.SetABDefault单独设定FrozenItemDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过FrozenItemDetailVisitor.SetABFallback单独设定FrozenItemDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawFrozenItemDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawFrozenItemDetail>>(()=> new PBC::MapField<int, rawFrozenItemDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawFrozenItemDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().FrozenItemDetailMap;
			var abMap = v.FrozenItemDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.FrozenItemDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].FrozenItemDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawFrozenItemDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().FrozenItemDetailMap;
			if (v.FrozenItemDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.FrozenItemDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawFrozenItemDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(FrozenItemDetailConf cc) {
			foreach(KeyValuePair <string, rawFrozenItemDetailMapABValue> tagEntry in cc.FrozenItemDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.FrozenItemDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawFrozenItemDetailMapABValue();
					cc.FrozenItemDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.FrozenItemDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawFrozenItemDetail> entry in cc.FrozenItemDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawFrozenItemDetail> entry in tagEntry.Value.FrozenItemDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.FrozenItemDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(FrozenItemDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.FrozenItemDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.FrozenItemDetailMapAB != null)
			{
				foreach (var entry in cc.FrozenItemDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.FrozenItemDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.FrozenItemDetailMap.MergeFrom(cis);
			Patch(updating.FrozenItemDetailMap);
			updating.FrozenItemDetailSortedKeys = genSortedKeys(updating.FrozenItemDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new FrozenItemDetailConf();
            tmp.MergeFrom(cis);
			updating.FrozenItemDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().FrozenItemDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawFrozenItemDetail> filter,string tag=""){
		...
		public static rawFrozenItemDetail MustGet(int key,string tag="") {
		...
		public static rawFrozenItemDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawFrozenItemDetail> GetByFilter(Func<rawFrozenItemDetail, bool> filterFunc,string tag="") {
			List<rawFrozenItemDetail> ret = new List<rawFrozenItemDetail>();
			foreach(KeyValuePair <int, rawFrozenItemDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawFrozenItemDetail? GetOneByFilter(Func<rawFrozenItemDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawFrozenItemDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawFrozenItemDetail data) 
		{
			return data.Id;			
		}
    }
