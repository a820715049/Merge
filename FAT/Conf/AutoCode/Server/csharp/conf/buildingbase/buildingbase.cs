...
using BuildingBaseConf = fat.rawdata.BuildingBaseConf;
using rawBuildingBase = fat.rawdata.BuildingBase;
using rawBuildingBaseMapABValue = fat.rawdata.BuildingBaseMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BuildingBaseVisitor
    {
		...
		public const string FileName = "BuildingBaseConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BuildingBaseVisitor.SetABDefault单独设定BuildingBase的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BuildingBaseVisitor.SetABFallback单独设定BuildingBase的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBuildingBase>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBuildingBase>>(()=> new PBC::MapField<int, rawBuildingBase>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBuildingBase>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BuildingBaseMap;
			var abMap = v.BuildingBaseMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BuildingBaseMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BuildingBaseMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBuildingBase> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BuildingBaseMap;
			if (v.BuildingBaseMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BuildingBaseMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBuildingBaseMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BuildingBaseConf cc) {
			foreach(KeyValuePair <string, rawBuildingBaseMapABValue> tagEntry in cc.BuildingBaseMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BuildingBaseMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBuildingBaseMapABValue();
					cc.BuildingBaseMapAB[tag] = ps;
				}
                var tagTmp = ps.BuildingBaseMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBuildingBase> entry in cc.BuildingBaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBuildingBase> entry in tagEntry.Value.BuildingBaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BuildingBase> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BuildingBaseConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BuildingBaseMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BuildingBaseMapAB != null)
			{
				foreach (var entry in cc.BuildingBaseMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BuildingBaseMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BuildingBaseMap.MergeFrom(cis);
			Patch(updating.BuildingBaseMap);
			updating.BuildingBaseSortedKeys = genSortedKeys(updating.BuildingBaseMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BuildingBaseConf();
            tmp.MergeFrom(cis);
			updating.BuildingBaseMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BuildingBaseSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBuildingBase> filter,string tag=""){
		...
		public static rawBuildingBase MustGet(int key,string tag="") {
		...
		public static rawBuildingBase? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBuildingBase> GetByFilter(Func<rawBuildingBase, bool> filterFunc,string tag="") {
			List<rawBuildingBase> ret = new List<rawBuildingBase>();
			foreach(KeyValuePair <int, rawBuildingBase> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBuildingBase? GetOneByFilter(Func<rawBuildingBase, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBuildingBase> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBuildingBase data) 
		{
			return data.Id;			
		}
    }
