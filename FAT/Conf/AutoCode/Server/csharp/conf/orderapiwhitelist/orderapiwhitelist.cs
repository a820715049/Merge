...
using OrderApiWhitelistConf = fat.rawdata.OrderApiWhitelistConf;
using rawOrderApiWhitelist = fat.rawdata.OrderApiWhitelist;
using rawOrderApiWhitelistMapABValue = fat.rawdata.OrderApiWhitelistMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderApiWhitelistVisitor
    {
		...
		public const string FileName = "OrderApiWhitelistConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderApiWhitelistVisitor.SetABDefault单独设定OrderApiWhitelist的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderApiWhitelistVisitor.SetABFallback单独设定OrderApiWhitelist的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawOrderApiWhitelist>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawOrderApiWhitelist>>(()=> new PBC::MapField<string, rawOrderApiWhitelist>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawOrderApiWhitelist>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderApiWhitelistMap;
			var abMap = v.OrderApiWhitelistMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderApiWhitelistMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderApiWhitelistMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawOrderApiWhitelist> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderApiWhitelistMap;
			if (v.OrderApiWhitelistMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderApiWhitelistMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderApiWhitelistMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderApiWhitelistConf cc) {
			foreach(KeyValuePair <string, rawOrderApiWhitelistMapABValue> tagEntry in cc.OrderApiWhitelistMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderApiWhitelistMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderApiWhitelistMapABValue();
					cc.OrderApiWhitelistMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderApiWhitelistMap;
				// 基础数据
				foreach(KeyValuePair <string, rawOrderApiWhitelist> entry in cc.OrderApiWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawOrderApiWhitelist> entry in tagEntry.Value.OrderApiWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.OrderApiWhitelist> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(OrderApiWhitelistConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.OrderApiWhitelistMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderApiWhitelistMapAB != null)
			{
				foreach (var entry in cc.OrderApiWhitelistMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.OrderApiWhitelistMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderApiWhitelistMap.MergeFrom(cis);
			Patch(updating.OrderApiWhitelistMap);
			updating.OrderApiWhitelistSortedKeys = genSortedKeys(updating.OrderApiWhitelistMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderApiWhitelistConf();
            tmp.MergeFrom(cis);
			updating.OrderApiWhitelistMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderApiWhitelistSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawOrderApiWhitelist> filter,string tag=""){
		...
		public static rawOrderApiWhitelist MustGet(string key,string tag="") {
		...
		public static rawOrderApiWhitelist? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderApiWhitelist> GetByFilter(Func<rawOrderApiWhitelist, bool> filterFunc,string tag="") {
			List<rawOrderApiWhitelist> ret = new List<rawOrderApiWhitelist>();
			foreach(KeyValuePair <string, rawOrderApiWhitelist> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderApiWhitelist? GetOneByFilter(Func<rawOrderApiWhitelist, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawOrderApiWhitelist> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawOrderApiWhitelist data) 
		{
			return data.Id;			
		}
    }
