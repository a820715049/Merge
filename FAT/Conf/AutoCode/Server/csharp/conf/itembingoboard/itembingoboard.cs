...
using ItemBingoBoardConf = fat.rawdata.ItemBingoBoardConf;
using rawItemBingoBoard = fat.rawdata.ItemBingoBoard;
using rawItemBingoBoardMapABValue = fat.rawdata.ItemBingoBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ItemBingoBoardVisitor
    {
		...
		public const string FileName = "ItemBingoBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ItemBingoBoardVisitor.SetABDefault单独设定ItemBingoBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ItemBingoBoardVisitor.SetABFallback单独设定ItemBingoBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawItemBingoBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawItemBingoBoard>>(()=> new PBC::MapField<int, rawItemBingoBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawItemBingoBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ItemBingoBoardMap;
			var abMap = v.ItemBingoBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ItemBingoBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ItemBingoBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawItemBingoBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ItemBingoBoardMap;
			if (v.ItemBingoBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ItemBingoBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawItemBingoBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ItemBingoBoardConf cc) {
			foreach(KeyValuePair <string, rawItemBingoBoardMapABValue> tagEntry in cc.ItemBingoBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ItemBingoBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawItemBingoBoardMapABValue();
					cc.ItemBingoBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.ItemBingoBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawItemBingoBoard> entry in cc.ItemBingoBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawItemBingoBoard> entry in tagEntry.Value.ItemBingoBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ItemBingoBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ItemBingoBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ItemBingoBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ItemBingoBoardMapAB != null)
			{
				foreach (var entry in cc.ItemBingoBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ItemBingoBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ItemBingoBoardMap.MergeFrom(cis);
			Patch(updating.ItemBingoBoardMap);
			updating.ItemBingoBoardSortedKeys = genSortedKeys(updating.ItemBingoBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ItemBingoBoardConf();
            tmp.MergeFrom(cis);
			updating.ItemBingoBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ItemBingoBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawItemBingoBoard> filter,string tag=""){
		...
		public static rawItemBingoBoard MustGet(int key,string tag="") {
		...
		public static rawItemBingoBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawItemBingoBoard> GetByFilter(Func<rawItemBingoBoard, bool> filterFunc,string tag="") {
			List<rawItemBingoBoard> ret = new List<rawItemBingoBoard>();
			foreach(KeyValuePair <int, rawItemBingoBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawItemBingoBoard? GetOneByFilter(Func<rawItemBingoBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawItemBingoBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawItemBingoBoard data) 
		{
			return data.Id;			
		}
    }
