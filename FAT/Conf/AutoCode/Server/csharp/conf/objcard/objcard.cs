...
using ObjCardConf = fat.rawdata.ObjCardConf;
using rawObjCard = fat.rawdata.ObjCard;
using rawObjCardMapABValue = fat.rawdata.ObjCardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjCardVisitor
    {
		...
		public const string FileName = "ObjCardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjCardVisitor.SetABDefault单独设定ObjCard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjCardVisitor.SetABFallback单独设定ObjCard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjCard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjCard>>(()=> new PBC::MapField<int, rawObjCard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjCard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjCardMap;
			var abMap = v.ObjCardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjCardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjCardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjCard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjCardMap;
			if (v.ObjCardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjCardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjCardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjCardConf cc) {
			foreach(KeyValuePair <string, rawObjCardMapABValue> tagEntry in cc.ObjCardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjCardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjCardMapABValue();
					cc.ObjCardMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjCardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjCard> entry in cc.ObjCardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjCard> entry in tagEntry.Value.ObjCardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjCard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjCardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjCardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjCardMapAB != null)
			{
				foreach (var entry in cc.ObjCardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjCardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjCardMap.MergeFrom(cis);
			Patch(updating.ObjCardMap);
			updating.ObjCardSortedKeys = genSortedKeys(updating.ObjCardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjCardConf();
            tmp.MergeFrom(cis);
			updating.ObjCardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjCardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjCard> filter,string tag=""){
		...
		public static rawObjCard MustGet(int key,string tag="") {
		...
		public static rawObjCard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjCard> GetByFilter(Func<rawObjCard, bool> filterFunc,string tag="") {
			List<rawObjCard> ret = new List<rawObjCard>();
			foreach(KeyValuePair <int, rawObjCard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjCard? GetOneByFilter(Func<rawObjCard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjCard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjCard data) 
		{
			return data.Id;			
		}
    }
