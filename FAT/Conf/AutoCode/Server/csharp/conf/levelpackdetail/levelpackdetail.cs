...
using LevelPackDetailConf = fat.rawdata.LevelPackDetailConf;
using rawLevelPackDetail = fat.rawdata.LevelPackDetail;
using rawLevelPackDetailMapABValue = fat.rawdata.LevelPackDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LevelPackDetailVisitor
    {
		...
		public const string FileName = "LevelPackDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LevelPackDetailVisitor.SetABDefault单独设定LevelPackDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LevelPackDetailVisitor.SetABFallback单独设定LevelPackDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLevelPackDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLevelPackDetail>>(()=> new PBC::MapField<int, rawLevelPackDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLevelPackDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LevelPackDetailMap;
			var abMap = v.LevelPackDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LevelPackDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LevelPackDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLevelPackDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LevelPackDetailMap;
			if (v.LevelPackDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LevelPackDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLevelPackDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LevelPackDetailConf cc) {
			foreach(KeyValuePair <string, rawLevelPackDetailMapABValue> tagEntry in cc.LevelPackDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LevelPackDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLevelPackDetailMapABValue();
					cc.LevelPackDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.LevelPackDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLevelPackDetail> entry in cc.LevelPackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLevelPackDetail> entry in tagEntry.Value.LevelPackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LevelPackDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LevelPackDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LevelPackDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LevelPackDetailMapAB != null)
			{
				foreach (var entry in cc.LevelPackDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LevelPackDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LevelPackDetailMap.MergeFrom(cis);
			Patch(updating.LevelPackDetailMap);
			updating.LevelPackDetailSortedKeys = genSortedKeys(updating.LevelPackDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LevelPackDetailConf();
            tmp.MergeFrom(cis);
			updating.LevelPackDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LevelPackDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLevelPackDetail> filter,string tag=""){
		...
		public static rawLevelPackDetail MustGet(int key,string tag="") {
		...
		public static rawLevelPackDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLevelPackDetail> GetByFilter(Func<rawLevelPackDetail, bool> filterFunc,string tag="") {
			List<rawLevelPackDetail> ret = new List<rawLevelPackDetail>();
			foreach(KeyValuePair <int, rawLevelPackDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLevelPackDetail? GetOneByFilter(Func<rawLevelPackDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLevelPackDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLevelPackDetail data) 
		{
			return data.Id;			
		}
    }
