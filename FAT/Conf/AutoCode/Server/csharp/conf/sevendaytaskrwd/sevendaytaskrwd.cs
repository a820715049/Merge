...
using SevenDayTaskRwdConf = fat.rawdata.SevenDayTaskRwdConf;
using rawSevenDayTaskRwd = fat.rawdata.SevenDayTaskRwd;
using rawSevenDayTaskRwdMapABValue = fat.rawdata.SevenDayTaskRwdMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SevenDayTaskRwdVisitor
    {
		...
		public const string FileName = "SevenDayTaskRwdConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SevenDayTaskRwdVisitor.SetABDefault单独设定SevenDayTaskRwd的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SevenDayTaskRwdVisitor.SetABFallback单独设定SevenDayTaskRwd的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSevenDayTaskRwd>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSevenDayTaskRwd>>(()=> new PBC::MapField<int, rawSevenDayTaskRwd>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSevenDayTaskRwd>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SevenDayTaskRwdMap;
			var abMap = v.SevenDayTaskRwdMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SevenDayTaskRwdMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SevenDayTaskRwdMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSevenDayTaskRwd> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SevenDayTaskRwdMap;
			if (v.SevenDayTaskRwdMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SevenDayTaskRwdMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSevenDayTaskRwdMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SevenDayTaskRwdConf cc) {
			foreach(KeyValuePair <string, rawSevenDayTaskRwdMapABValue> tagEntry in cc.SevenDayTaskRwdMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SevenDayTaskRwdMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSevenDayTaskRwdMapABValue();
					cc.SevenDayTaskRwdMapAB[tag] = ps;
				}
                var tagTmp = ps.SevenDayTaskRwdMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSevenDayTaskRwd> entry in cc.SevenDayTaskRwdMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSevenDayTaskRwd> entry in tagEntry.Value.SevenDayTaskRwdMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SevenDayTaskRwd> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SevenDayTaskRwdConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SevenDayTaskRwdMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SevenDayTaskRwdMapAB != null)
			{
				foreach (var entry in cc.SevenDayTaskRwdMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SevenDayTaskRwdMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SevenDayTaskRwdMap.MergeFrom(cis);
			Patch(updating.SevenDayTaskRwdMap);
			updating.SevenDayTaskRwdSortedKeys = genSortedKeys(updating.SevenDayTaskRwdMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SevenDayTaskRwdConf();
            tmp.MergeFrom(cis);
			updating.SevenDayTaskRwdMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SevenDayTaskRwdSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSevenDayTaskRwd> filter,string tag=""){
		...
		public static rawSevenDayTaskRwd MustGet(int key,string tag="") {
		...
		public static rawSevenDayTaskRwd? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSevenDayTaskRwd> GetByFilter(Func<rawSevenDayTaskRwd, bool> filterFunc,string tag="") {
			List<rawSevenDayTaskRwd> ret = new List<rawSevenDayTaskRwd>();
			foreach(KeyValuePair <int, rawSevenDayTaskRwd> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSevenDayTaskRwd? GetOneByFilter(Func<rawSevenDayTaskRwd, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSevenDayTaskRwd> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSevenDayTaskRwd data) 
		{
			return data.Id;			
		}
    }
