...
using EventWeeklyRaffleRewardConf = fat.rawdata.EventWeeklyRaffleRewardConf;
using rawEventWeeklyRaffleReward = fat.rawdata.EventWeeklyRaffleReward;
using rawEventWeeklyRaffleRewardMapABValue = fat.rawdata.EventWeeklyRaffleRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyRaffleRewardVisitor
    {
		...
		public const string FileName = "EventWeeklyRaffleRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyRaffleRewardVisitor.SetABDefault单独设定EventWeeklyRaffleReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyRaffleRewardVisitor.SetABFallback单独设定EventWeeklyRaffleReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyRaffleReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyRaffleReward>>(()=> new PBC::MapField<int, rawEventWeeklyRaffleReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyRaffleReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyRaffleRewardMap;
			var abMap = v.EventWeeklyRaffleRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyRaffleRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyRaffleRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyRaffleReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyRaffleRewardMap;
			if (v.EventWeeklyRaffleRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyRaffleRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyRaffleRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyRaffleRewardConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyRaffleRewardMapABValue> tagEntry in cc.EventWeeklyRaffleRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyRaffleRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyRaffleRewardMapABValue();
					cc.EventWeeklyRaffleRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyRaffleRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffleReward> entry in cc.EventWeeklyRaffleRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffleReward> entry in tagEntry.Value.EventWeeklyRaffleRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyRaffleReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyRaffleRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyRaffleRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyRaffleRewardMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyRaffleRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyRaffleRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyRaffleRewardMap.MergeFrom(cis);
			Patch(updating.EventWeeklyRaffleRewardMap);
			updating.EventWeeklyRaffleRewardSortedKeys = genSortedKeys(updating.EventWeeklyRaffleRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyRaffleRewardConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyRaffleRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyRaffleRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyRaffleReward> filter,string tag=""){
		...
		public static rawEventWeeklyRaffleReward MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyRaffleReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyRaffleReward> GetByFilter(Func<rawEventWeeklyRaffleReward, bool> filterFunc,string tag="") {
			List<rawEventWeeklyRaffleReward> ret = new List<rawEventWeeklyRaffleReward>();
			foreach(KeyValuePair <int, rawEventWeeklyRaffleReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyRaffleReward? GetOneByFilter(Func<rawEventWeeklyRaffleReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyRaffleReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyRaffleReward data) 
		{
			return data.Id;			
		}
    }
