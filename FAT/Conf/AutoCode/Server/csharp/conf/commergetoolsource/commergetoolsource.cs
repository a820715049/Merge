...
using ComMergeToolSourceConf = fat.rawdata.ComMergeToolSourceConf;
using rawComMergeToolSource = fat.rawdata.ComMergeToolSource;
using rawComMergeToolSourceMapABValue = fat.rawdata.ComMergeToolSourceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeToolSourceVisitor
    {
		...
		public const string FileName = "ComMergeToolSourceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeToolSourceVisitor.SetABDefault单独设定ComMergeToolSource的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeToolSourceVisitor.SetABFallback单独设定ComMergeToolSource的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeToolSource>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeToolSource>>(()=> new PBC::MapField<int, rawComMergeToolSource>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeToolSource>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeToolSourceMap;
			var abMap = v.ComMergeToolSourceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeToolSourceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeToolSourceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeToolSource> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeToolSourceMap;
			if (v.ComMergeToolSourceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeToolSourceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeToolSourceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeToolSourceConf cc) {
			foreach(KeyValuePair <string, rawComMergeToolSourceMapABValue> tagEntry in cc.ComMergeToolSourceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeToolSourceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeToolSourceMapABValue();
					cc.ComMergeToolSourceMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeToolSourceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeToolSource> entry in cc.ComMergeToolSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeToolSource> entry in tagEntry.Value.ComMergeToolSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeToolSource> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeToolSourceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeToolSourceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeToolSourceMapAB != null)
			{
				foreach (var entry in cc.ComMergeToolSourceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeToolSourceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeToolSourceMap.MergeFrom(cis);
			Patch(updating.ComMergeToolSourceMap);
			updating.ComMergeToolSourceSortedKeys = genSortedKeys(updating.ComMergeToolSourceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeToolSourceConf();
            tmp.MergeFrom(cis);
			updating.ComMergeToolSourceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeToolSourceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeToolSource> filter,string tag=""){
		...
		public static rawComMergeToolSource MustGet(int key,string tag="") {
		...
		public static rawComMergeToolSource? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeToolSource> GetByFilter(Func<rawComMergeToolSource, bool> filterFunc,string tag="") {
			List<rawComMergeToolSource> ret = new List<rawComMergeToolSource>();
			foreach(KeyValuePair <int, rawComMergeToolSource> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeToolSource? GetOneByFilter(Func<rawComMergeToolSource, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeToolSource> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeToolSource data) 
		{
			return data.Id;			
		}
    }
