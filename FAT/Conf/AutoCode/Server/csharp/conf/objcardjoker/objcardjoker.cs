...
using ObjCardJokerConf = fat.rawdata.ObjCardJokerConf;
using rawObjCardJoker = fat.rawdata.ObjCardJoker;
using rawObjCardJokerMapABValue = fat.rawdata.ObjCardJokerMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjCardJokerVisitor
    {
		...
		public const string FileName = "ObjCardJokerConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjCardJokerVisitor.SetABDefault单独设定ObjCardJoker的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjCardJokerVisitor.SetABFallback单独设定ObjCardJoker的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjCardJoker>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjCardJoker>>(()=> new PBC::MapField<int, rawObjCardJoker>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjCardJoker>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjCardJokerMap;
			var abMap = v.ObjCardJokerMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjCardJokerMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjCardJokerMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjCardJoker> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjCardJokerMap;
			if (v.ObjCardJokerMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjCardJokerMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjCardJokerMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjCardJokerConf cc) {
			foreach(KeyValuePair <string, rawObjCardJokerMapABValue> tagEntry in cc.ObjCardJokerMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjCardJokerMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjCardJokerMapABValue();
					cc.ObjCardJokerMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjCardJokerMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjCardJoker> entry in cc.ObjCardJokerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjCardJoker> entry in tagEntry.Value.ObjCardJokerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjCardJoker> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjCardJokerConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjCardJokerMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjCardJokerMapAB != null)
			{
				foreach (var entry in cc.ObjCardJokerMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjCardJokerMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjCardJokerMap.MergeFrom(cis);
			Patch(updating.ObjCardJokerMap);
			updating.ObjCardJokerSortedKeys = genSortedKeys(updating.ObjCardJokerMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjCardJokerConf();
            tmp.MergeFrom(cis);
			updating.ObjCardJokerMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjCardJokerSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjCardJoker> filter,string tag=""){
		...
		public static rawObjCardJoker MustGet(int key,string tag="") {
		...
		public static rawObjCardJoker? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjCardJoker> GetByFilter(Func<rawObjCardJoker, bool> filterFunc,string tag="") {
			List<rawObjCardJoker> ret = new List<rawObjCardJoker>();
			foreach(KeyValuePair <int, rawObjCardJoker> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjCardJoker? GetOneByFilter(Func<rawObjCardJoker, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjCardJoker> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjCardJoker data) 
		{
			return data.Id;			
		}
    }
