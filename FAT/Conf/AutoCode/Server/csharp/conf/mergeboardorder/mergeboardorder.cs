...
using MergeBoardOrderConf = fat.rawdata.MergeBoardOrderConf;
using rawMergeBoardOrder = fat.rawdata.MergeBoardOrder;
using rawMergeBoardOrderMapABValue = fat.rawdata.MergeBoardOrderMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeBoardOrderVisitor
    {
		...
		public const string FileName = "MergeBoardOrderConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeBoardOrderVisitor.SetABDefault单独设定MergeBoardOrder的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeBoardOrderVisitor.SetABFallback单独设定MergeBoardOrder的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeBoardOrder>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeBoardOrder>>(()=> new PBC::MapField<int, rawMergeBoardOrder>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeBoardOrder>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeBoardOrderMap;
			var abMap = v.MergeBoardOrderMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeBoardOrderMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeBoardOrderMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeBoardOrder> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeBoardOrderMap;
			if (v.MergeBoardOrderMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeBoardOrderMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeBoardOrderMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeBoardOrderConf cc) {
			foreach(KeyValuePair <string, rawMergeBoardOrderMapABValue> tagEntry in cc.MergeBoardOrderMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeBoardOrderMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeBoardOrderMapABValue();
					cc.MergeBoardOrderMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeBoardOrderMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeBoardOrder> entry in cc.MergeBoardOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeBoardOrder> entry in tagEntry.Value.MergeBoardOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeBoardOrder> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeBoardOrderConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeBoardOrderMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeBoardOrderMapAB != null)
			{
				foreach (var entry in cc.MergeBoardOrderMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeBoardOrderMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeBoardOrderMap.MergeFrom(cis);
			Patch(updating.MergeBoardOrderMap);
			updating.MergeBoardOrderSortedKeys = genSortedKeys(updating.MergeBoardOrderMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeBoardOrderConf();
            tmp.MergeFrom(cis);
			updating.MergeBoardOrderMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeBoardOrderSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeBoardOrder> filter,string tag=""){
		...
		public static rawMergeBoardOrder MustGet(int key,string tag="") {
		...
		public static rawMergeBoardOrder? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeBoardOrder> GetByFilter(Func<rawMergeBoardOrder, bool> filterFunc,string tag="") {
			List<rawMergeBoardOrder> ret = new List<rawMergeBoardOrder>();
			foreach(KeyValuePair <int, rawMergeBoardOrder> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeBoardOrder? GetOneByFilter(Func<rawMergeBoardOrder, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeBoardOrder> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeBoardOrder data) 
		{
			return data.Id;			
		}
    }
