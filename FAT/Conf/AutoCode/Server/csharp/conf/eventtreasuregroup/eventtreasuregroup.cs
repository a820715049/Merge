...
using EventTreasureGroupConf = fat.rawdata.EventTreasureGroupConf;
using rawEventTreasureGroup = fat.rawdata.EventTreasureGroup;
using rawEventTreasureGroupMapABValue = fat.rawdata.EventTreasureGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTreasureGroupVisitor
    {
		...
		public const string FileName = "EventTreasureGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTreasureGroupVisitor.SetABDefault单独设定EventTreasureGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTreasureGroupVisitor.SetABFallback单独设定EventTreasureGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTreasureGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTreasureGroup>>(()=> new PBC::MapField<int, rawEventTreasureGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTreasureGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTreasureGroupMap;
			var abMap = v.EventTreasureGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTreasureGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTreasureGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTreasureGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTreasureGroupMap;
			if (v.EventTreasureGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTreasureGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTreasureGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTreasureGroupConf cc) {
			foreach(KeyValuePair <string, rawEventTreasureGroupMapABValue> tagEntry in cc.EventTreasureGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTreasureGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTreasureGroupMapABValue();
					cc.EventTreasureGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTreasureGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTreasureGroup> entry in cc.EventTreasureGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTreasureGroup> entry in tagEntry.Value.EventTreasureGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTreasureGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTreasureGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTreasureGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTreasureGroupMapAB != null)
			{
				foreach (var entry in cc.EventTreasureGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTreasureGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTreasureGroupMap.MergeFrom(cis);
			Patch(updating.EventTreasureGroupMap);
			updating.EventTreasureGroupSortedKeys = genSortedKeys(updating.EventTreasureGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTreasureGroupConf();
            tmp.MergeFrom(cis);
			updating.EventTreasureGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTreasureGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTreasureGroup> filter,string tag=""){
		...
		public static rawEventTreasureGroup MustGet(int key,string tag="") {
		...
		public static rawEventTreasureGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTreasureGroup> GetByFilter(Func<rawEventTreasureGroup, bool> filterFunc,string tag="") {
			List<rawEventTreasureGroup> ret = new List<rawEventTreasureGroup>();
			foreach(KeyValuePair <int, rawEventTreasureGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTreasureGroup? GetOneByFilter(Func<rawEventTreasureGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTreasureGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTreasureGroup data) 
		{
			return data.Id;			
		}
    }
