...
using EventRaceRobotConf = fat.rawdata.EventRaceRobotConf;
using rawEventRaceRobot = fat.rawdata.EventRaceRobot;
using rawEventRaceRobotMapABValue = fat.rawdata.EventRaceRobotMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceRobotVisitor
    {
		...
		public const string FileName = "EventRaceRobotConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceRobotVisitor.SetABDefault单独设定EventRaceRobot的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceRobotVisitor.SetABFallback单独设定EventRaceRobot的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceRobot>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceRobot>>(()=> new PBC::MapField<int, rawEventRaceRobot>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceRobot>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceRobotMap;
			var abMap = v.EventRaceRobotMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceRobotMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceRobotMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceRobot> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceRobotMap;
			if (v.EventRaceRobotMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceRobotMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceRobotMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceRobotConf cc) {
			foreach(KeyValuePair <string, rawEventRaceRobotMapABValue> tagEntry in cc.EventRaceRobotMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceRobotMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceRobotMapABValue();
					cc.EventRaceRobotMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceRobotMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceRobot> entry in cc.EventRaceRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceRobot> entry in tagEntry.Value.EventRaceRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceRobot> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceRobotConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceRobotMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceRobotMapAB != null)
			{
				foreach (var entry in cc.EventRaceRobotMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceRobotMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceRobotMap.MergeFrom(cis);
			Patch(updating.EventRaceRobotMap);
			updating.EventRaceRobotSortedKeys = genSortedKeys(updating.EventRaceRobotMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceRobotConf();
            tmp.MergeFrom(cis);
			updating.EventRaceRobotMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceRobotSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceRobot> filter,string tag=""){
		...
		public static rawEventRaceRobot MustGet(int key,string tag="") {
		...
		public static rawEventRaceRobot? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceRobot> GetByFilter(Func<rawEventRaceRobot, bool> filterFunc,string tag="") {
			List<rawEventRaceRobot> ret = new List<rawEventRaceRobot>();
			foreach(KeyValuePair <int, rawEventRaceRobot> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceRobot? GetOneByFilter(Func<rawEventRaceRobot, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceRobot> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceRobot data) 
		{
			return data.Id;			
		}
    }
