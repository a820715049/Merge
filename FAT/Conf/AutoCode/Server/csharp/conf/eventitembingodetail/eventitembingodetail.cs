...
using EventItemBingoDetailConf = fat.rawdata.EventItemBingoDetailConf;
using rawEventItemBingoDetail = fat.rawdata.EventItemBingoDetail;
using rawEventItemBingoDetailMapABValue = fat.rawdata.EventItemBingoDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventItemBingoDetailVisitor
    {
		...
		public const string FileName = "EventItemBingoDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventItemBingoDetailVisitor.SetABDefault单独设定EventItemBingoDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventItemBingoDetailVisitor.SetABFallback单独设定EventItemBingoDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventItemBingoDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventItemBingoDetail>>(()=> new PBC::MapField<int, rawEventItemBingoDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventItemBingoDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventItemBingoDetailMap;
			var abMap = v.EventItemBingoDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventItemBingoDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventItemBingoDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventItemBingoDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventItemBingoDetailMap;
			if (v.EventItemBingoDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventItemBingoDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventItemBingoDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventItemBingoDetailConf cc) {
			foreach(KeyValuePair <string, rawEventItemBingoDetailMapABValue> tagEntry in cc.EventItemBingoDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventItemBingoDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventItemBingoDetailMapABValue();
					cc.EventItemBingoDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventItemBingoDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventItemBingoDetail> entry in cc.EventItemBingoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventItemBingoDetail> entry in tagEntry.Value.EventItemBingoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventItemBingoDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventItemBingoDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventItemBingoDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventItemBingoDetailMapAB != null)
			{
				foreach (var entry in cc.EventItemBingoDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventItemBingoDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventItemBingoDetailMap.MergeFrom(cis);
			Patch(updating.EventItemBingoDetailMap);
			updating.EventItemBingoDetailSortedKeys = genSortedKeys(updating.EventItemBingoDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventItemBingoDetailConf();
            tmp.MergeFrom(cis);
			updating.EventItemBingoDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventItemBingoDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventItemBingoDetail> filter,string tag=""){
		...
		public static rawEventItemBingoDetail MustGet(int key,string tag="") {
		...
		public static rawEventItemBingoDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventItemBingoDetail> GetByFilter(Func<rawEventItemBingoDetail, bool> filterFunc,string tag="") {
			List<rawEventItemBingoDetail> ret = new List<rawEventItemBingoDetail>();
			foreach(KeyValuePair <int, rawEventItemBingoDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventItemBingoDetail? GetOneByFilter(Func<rawEventItemBingoDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventItemBingoDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventItemBingoDetail data) 
		{
			return data.Id;			
		}
    }
