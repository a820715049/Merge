...
using MarketCommondityConf = fat.rawdata.MarketCommondityConf;
using rawMarketCommondity = fat.rawdata.MarketCommondity;
using rawMarketCommondityMapABValue = fat.rawdata.MarketCommondityMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketCommondityVisitor
    {
		...
		public const string FileName = "MarketCommondityConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketCommondityVisitor.SetABDefault单独设定MarketCommondity的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketCommondityVisitor.SetABFallback单独设定MarketCommondity的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketCommondity>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketCommondity>>(()=> new PBC::MapField<int, rawMarketCommondity>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketCommondity>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketCommondityMap;
			var abMap = v.MarketCommondityMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketCommondityMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketCommondityMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketCommondity> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketCommondityMap;
			if (v.MarketCommondityMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketCommondityMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketCommondityMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketCommondityConf cc) {
			foreach(KeyValuePair <string, rawMarketCommondityMapABValue> tagEntry in cc.MarketCommondityMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketCommondityMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketCommondityMapABValue();
					cc.MarketCommondityMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketCommondityMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketCommondity> entry in cc.MarketCommondityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketCommondity> entry in tagEntry.Value.MarketCommondityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketCommondity> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketCommondityConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketCommondityMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketCommondityMapAB != null)
			{
				foreach (var entry in cc.MarketCommondityMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketCommondityMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketCommondityMap.MergeFrom(cis);
			Patch(updating.MarketCommondityMap);
			updating.MarketCommonditySortedKeys = genSortedKeys(updating.MarketCommondityMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketCommondityConf();
            tmp.MergeFrom(cis);
			updating.MarketCommondityMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketCommonditySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketCommondity> filter,string tag=""){
		...
		public static rawMarketCommondity MustGet(int key,string tag="") {
		...
		public static rawMarketCommondity? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketCommondity> GetByFilter(Func<rawMarketCommondity, bool> filterFunc,string tag="") {
			List<rawMarketCommondity> ret = new List<rawMarketCommondity>();
			foreach(KeyValuePair <int, rawMarketCommondity> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketCommondity? GetOneByFilter(Func<rawMarketCommondity, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketCommondity> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketCommondity data) 
		{
			return data.Id;			
		}
    }
