...
using MergeFixedOutputConf = fat.rawdata.MergeFixedOutputConf;
using rawMergeFixedOutput = fat.rawdata.MergeFixedOutput;
using rawMergeFixedOutputSliceABValue = fat.rawdata.MergeFixedOutputSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeFixedOutputVisitor
    {
		...
		public const string FileName = "MergeFixedOutputConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeFixedOutputVisitor.SetABDefault单独设定MergeFixedOutput的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeFixedOutputVisitor.SetABFallback单独设定MergeFixedOutput的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawMergeFixedOutput>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawMergeFixedOutput>>(()=>new PBC::RepeatedField<rawMergeFixedOutput>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawMergeFixedOutput>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeFixedOutputSlice;
			var abMap = v.MergeFixedOutputSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeFixedOutputSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeFixedOutputSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawMergeFixedOutput> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeFixedOutputSlice;
			if (v.MergeFixedOutputSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeFixedOutputSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeFixedOutputSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeFixedOutputConf cc) {
			foreach(KeyValuePair <string, rawMergeFixedOutputSliceABValue> tagEntry in cc.MergeFixedOutputSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeFixedOutputSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeFixedOutputSliceABValue();
					cc.MergeFixedOutputSliceAB[tag] = ps;
				}
                var tagTmp = ps.MergeFixedOutputSlice;
				// 基础数据
				for(int i=0; i<cc.MergeFixedOutputSlice.Count; ++i) 
				{
					tagTmp.Add(cc.MergeFixedOutputSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.MergeFixedOutputSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.MergeFixedOutput> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeFixedOutputSlice.MergeFrom(cis);
			Patch(updating.MergeFixedOutputSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeFixedOutputConf();
            tmp.MergeFrom(cis);
			updating.MergeFixedOutputSlice = tmp;
		...
		public static void Range(Action<int, rawMergeFixedOutput> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawMergeFixedOutput MustGetByIndex(int index,string tag="") {
		...
		public static rawMergeFixedOutput? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawMergeFixedOutput> GetByFilter(Func<rawMergeFixedOutput,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawMergeFixedOutput>();
		...
		static public rawMergeFixedOutput? GetOneByFilter(Func<rawMergeFixedOutput,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
