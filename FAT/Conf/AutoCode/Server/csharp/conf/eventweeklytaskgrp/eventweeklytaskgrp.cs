...
using EventWeeklyTaskGrpConf = fat.rawdata.EventWeeklyTaskGrpConf;
using rawEventWeeklyTaskGrp = fat.rawdata.EventWeeklyTaskGrp;
using rawEventWeeklyTaskGrpMapABValue = fat.rawdata.EventWeeklyTaskGrpMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyTaskGrpVisitor
    {
		...
		public const string FileName = "EventWeeklyTaskGrpConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyTaskGrpVisitor.SetABDefault单独设定EventWeeklyTaskGrp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyTaskGrpVisitor.SetABFallback单独设定EventWeeklyTaskGrp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyTaskGrp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyTaskGrp>>(()=> new PBC::MapField<int, rawEventWeeklyTaskGrp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyTaskGrp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyTaskGrpMap;
			var abMap = v.EventWeeklyTaskGrpMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyTaskGrpMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyTaskGrpMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyTaskGrp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyTaskGrpMap;
			if (v.EventWeeklyTaskGrpMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyTaskGrpMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyTaskGrpMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyTaskGrpConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyTaskGrpMapABValue> tagEntry in cc.EventWeeklyTaskGrpMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyTaskGrpMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyTaskGrpMapABValue();
					cc.EventWeeklyTaskGrpMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyTaskGrpMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskGrp> entry in cc.EventWeeklyTaskGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskGrp> entry in tagEntry.Value.EventWeeklyTaskGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyTaskGrp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyTaskGrpConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyTaskGrpMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyTaskGrpMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyTaskGrpMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyTaskGrpMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyTaskGrpMap.MergeFrom(cis);
			Patch(updating.EventWeeklyTaskGrpMap);
			updating.EventWeeklyTaskGrpSortedKeys = genSortedKeys(updating.EventWeeklyTaskGrpMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyTaskGrpConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyTaskGrpMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyTaskGrpSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyTaskGrp> filter,string tag=""){
		...
		public static rawEventWeeklyTaskGrp MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyTaskGrp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyTaskGrp> GetByFilter(Func<rawEventWeeklyTaskGrp, bool> filterFunc,string tag="") {
			List<rawEventWeeklyTaskGrp> ret = new List<rawEventWeeklyTaskGrp>();
			foreach(KeyValuePair <int, rawEventWeeklyTaskGrp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyTaskGrp? GetOneByFilter(Func<rawEventWeeklyTaskGrp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyTaskGrp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyTaskGrp data) 
		{
			return data.Id;			
		}
    }
