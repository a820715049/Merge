...
using EventFarmDropConf = fat.rawdata.EventFarmDropConf;
using rawEventFarmDrop = fat.rawdata.EventFarmDrop;
using rawEventFarmDropMapABValue = fat.rawdata.EventFarmDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmDropVisitor
    {
		...
		public const string FileName = "EventFarmDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmDropVisitor.SetABDefault单独设定EventFarmDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmDropVisitor.SetABFallback单独设定EventFarmDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmDrop>>(()=> new PBC::MapField<int, rawEventFarmDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmDropMap;
			var abMap = v.EventFarmDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmDropMap;
			if (v.EventFarmDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmDropConf cc) {
			foreach(KeyValuePair <string, rawEventFarmDropMapABValue> tagEntry in cc.EventFarmDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmDropMapABValue();
					cc.EventFarmDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmDrop> entry in cc.EventFarmDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmDrop> entry in tagEntry.Value.EventFarmDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmDropMapAB != null)
			{
				foreach (var entry in cc.EventFarmDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmDropMap.MergeFrom(cis);
			Patch(updating.EventFarmDropMap);
			updating.EventFarmDropSortedKeys = genSortedKeys(updating.EventFarmDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmDropConf();
            tmp.MergeFrom(cis);
			updating.EventFarmDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmDrop> filter,string tag=""){
		...
		public static rawEventFarmDrop MustGet(int key,string tag="") {
		...
		public static rawEventFarmDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmDrop> GetByFilter(Func<rawEventFarmDrop, bool> filterFunc,string tag="") {
			List<rawEventFarmDrop> ret = new List<rawEventFarmDrop>();
			foreach(KeyValuePair <int, rawEventFarmDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmDrop? GetOneByFilter(Func<rawEventFarmDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmDrop data) 
		{
			return data.Id;			
		}
    }
