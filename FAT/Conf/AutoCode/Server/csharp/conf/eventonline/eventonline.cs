...
using EventOnlineConf = fat.rawdata.EventOnlineConf;
using rawEventOnline = fat.rawdata.EventOnline;
using rawEventOnlineMapABValue = fat.rawdata.EventOnlineMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOnlineVisitor
    {
		...
		public const string FileName = "EventOnlineConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOnlineVisitor.SetABDefault单独设定EventOnline的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOnlineVisitor.SetABFallback单独设定EventOnline的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOnline>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOnline>>(()=> new PBC::MapField<int, rawEventOnline>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOnline>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOnlineMap;
			var abMap = v.EventOnlineMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOnlineMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOnlineMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOnline> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOnlineMap;
			if (v.EventOnlineMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOnlineMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOnlineMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOnlineConf cc) {
			foreach(KeyValuePair <string, rawEventOnlineMapABValue> tagEntry in cc.EventOnlineMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOnlineMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOnlineMapABValue();
					cc.EventOnlineMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOnlineMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOnline> entry in cc.EventOnlineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOnline> entry in tagEntry.Value.EventOnlineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOnline> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOnlineConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOnlineMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOnlineMapAB != null)
			{
				foreach (var entry in cc.EventOnlineMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOnlineMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOnlineMap.MergeFrom(cis);
			Patch(updating.EventOnlineMap);
			updating.EventOnlineSortedKeys = genSortedKeys(updating.EventOnlineMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOnlineConf();
            tmp.MergeFrom(cis);
			updating.EventOnlineMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOnlineSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOnline> filter,string tag=""){
		...
		public static rawEventOnline MustGet(int key,string tag="") {
		...
		public static rawEventOnline? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOnline> GetByFilter(Func<rawEventOnline, bool> filterFunc,string tag="") {
			List<rawEventOnline> ret = new List<rawEventOnline>();
			foreach(KeyValuePair <int, rawEventOnline> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOnline? GetOneByFilter(Func<rawEventOnline, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOnline> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOnline data) 
		{
			return data.Id;			
		}
    }
