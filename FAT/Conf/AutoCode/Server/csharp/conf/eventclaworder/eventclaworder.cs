...
using EventClawOrderConf = fat.rawdata.EventClawOrderConf;
using rawEventClawOrder = fat.rawdata.EventClawOrder;
using rawEventClawOrderMapABValue = fat.rawdata.EventClawOrderMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderVisitor
    {
		...
		public const string FileName = "EventClawOrderConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderVisitor.SetABDefault单独设定EventClawOrder的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderVisitor.SetABFallback单独设定EventClawOrder的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrder>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrder>>(()=> new PBC::MapField<int, rawEventClawOrder>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrder>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderMap;
			var abMap = v.EventClawOrderMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrder> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderMap;
			if (v.EventClawOrderMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderMapABValue> tagEntry in cc.EventClawOrderMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderMapABValue();
					cc.EventClawOrderMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrder> entry in cc.EventClawOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrder> entry in tagEntry.Value.EventClawOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrder> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderMap.MergeFrom(cis);
			Patch(updating.EventClawOrderMap);
			updating.EventClawOrderSortedKeys = genSortedKeys(updating.EventClawOrderMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrder> filter,string tag=""){
		...
		public static rawEventClawOrder MustGet(int key,string tag="") {
		...
		public static rawEventClawOrder? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrder> GetByFilter(Func<rawEventClawOrder, bool> filterFunc,string tag="") {
			List<rawEventClawOrder> ret = new List<rawEventClawOrder>();
			foreach(KeyValuePair <int, rawEventClawOrder> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrder? GetOneByFilter(Func<rawEventClawOrder, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrder> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrder data) 
		{
			return data.Id;			
		}
    }
