...
using EventFightConf = fat.rawdata.EventFightConf;
using rawEventFight = fat.rawdata.EventFight;
using rawEventFightMapABValue = fat.rawdata.EventFightMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFightVisitor
    {
		...
		public const string FileName = "EventFightConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFightVisitor.SetABDefault单独设定EventFight的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFightVisitor.SetABFallback单独设定EventFight的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFight>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFight>>(()=> new PBC::MapField<int, rawEventFight>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFight>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFightMap;
			var abMap = v.EventFightMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFightMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFightMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFight> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFightMap;
			if (v.EventFightMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFightMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFightMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFightConf cc) {
			foreach(KeyValuePair <string, rawEventFightMapABValue> tagEntry in cc.EventFightMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFightMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFightMapABValue();
					cc.EventFightMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFightMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFight> entry in cc.EventFightMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFight> entry in tagEntry.Value.EventFightMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFight> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFightConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFightMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFightMapAB != null)
			{
				foreach (var entry in cc.EventFightMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFightMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFightMap.MergeFrom(cis);
			Patch(updating.EventFightMap);
			updating.EventFightSortedKeys = genSortedKeys(updating.EventFightMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFightConf();
            tmp.MergeFrom(cis);
			updating.EventFightMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFightSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFight> filter,string tag=""){
		...
		public static rawEventFight MustGet(int key,string tag="") {
		...
		public static rawEventFight? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFight> GetByFilter(Func<rawEventFight, bool> filterFunc,string tag="") {
			List<rawEventFight> ret = new List<rawEventFight>();
			foreach(KeyValuePair <int, rawEventFight> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFight? GetOneByFilter(Func<rawEventFight, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFight> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFight data) 
		{
			return data.Id;			
		}
    }
