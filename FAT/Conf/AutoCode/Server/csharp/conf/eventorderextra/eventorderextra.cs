...
using EventOrderExtraConf = fat.rawdata.EventOrderExtraConf;
using rawEventOrderExtra = fat.rawdata.EventOrderExtra;
using rawEventOrderExtraMapABValue = fat.rawdata.EventOrderExtraMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderExtraVisitor
    {
		...
		public const string FileName = "EventOrderExtraConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderExtraVisitor.SetABDefault单独设定EventOrderExtra的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderExtraVisitor.SetABFallback单独设定EventOrderExtra的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderExtra>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderExtra>>(()=> new PBC::MapField<int, rawEventOrderExtra>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderExtra>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderExtraMap;
			var abMap = v.EventOrderExtraMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderExtraMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderExtraMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderExtra> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderExtraMap;
			if (v.EventOrderExtraMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderExtraMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderExtraMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderExtraConf cc) {
			foreach(KeyValuePair <string, rawEventOrderExtraMapABValue> tagEntry in cc.EventOrderExtraMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderExtraMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderExtraMapABValue();
					cc.EventOrderExtraMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderExtraMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderExtra> entry in cc.EventOrderExtraMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderExtra> entry in tagEntry.Value.EventOrderExtraMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderExtra> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderExtraConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderExtraMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderExtraMapAB != null)
			{
				foreach (var entry in cc.EventOrderExtraMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderExtraMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderExtraMap.MergeFrom(cis);
			Patch(updating.EventOrderExtraMap);
			updating.EventOrderExtraSortedKeys = genSortedKeys(updating.EventOrderExtraMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderExtraConf();
            tmp.MergeFrom(cis);
			updating.EventOrderExtraMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderExtraSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderExtra> filter,string tag=""){
		...
		public static rawEventOrderExtra MustGet(int key,string tag="") {
		...
		public static rawEventOrderExtra? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderExtra> GetByFilter(Func<rawEventOrderExtra, bool> filterFunc,string tag="") {
			List<rawEventOrderExtra> ret = new List<rawEventOrderExtra>();
			foreach(KeyValuePair <int, rawEventOrderExtra> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderExtra? GetOneByFilter(Func<rawEventOrderExtra, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderExtra> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderExtra data) 
		{
			return data.Id;			
		}
    }
