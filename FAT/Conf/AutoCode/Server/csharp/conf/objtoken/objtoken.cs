...
using ObjTokenConf = fat.rawdata.ObjTokenConf;
using rawObjToken = fat.rawdata.ObjToken;
using rawObjTokenMapABValue = fat.rawdata.ObjTokenMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjTokenVisitor
    {
		...
		public const string FileName = "ObjTokenConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjTokenVisitor.SetABDefault单独设定ObjToken的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjTokenVisitor.SetABFallback单独设定ObjToken的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjToken>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjToken>>(()=> new PBC::MapField<int, rawObjToken>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjToken>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjTokenMap;
			var abMap = v.ObjTokenMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjTokenMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjTokenMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjToken> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjTokenMap;
			if (v.ObjTokenMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjTokenMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjTokenMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjTokenConf cc) {
			foreach(KeyValuePair <string, rawObjTokenMapABValue> tagEntry in cc.ObjTokenMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjTokenMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjTokenMapABValue();
					cc.ObjTokenMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjTokenMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjToken> entry in cc.ObjTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjToken> entry in tagEntry.Value.ObjTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjToken> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjTokenConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjTokenMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjTokenMapAB != null)
			{
				foreach (var entry in cc.ObjTokenMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjTokenMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjTokenMap.MergeFrom(cis);
			Patch(updating.ObjTokenMap);
			updating.ObjTokenSortedKeys = genSortedKeys(updating.ObjTokenMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjTokenConf();
            tmp.MergeFrom(cis);
			updating.ObjTokenMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjTokenSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjToken> filter,string tag=""){
		...
		public static rawObjToken MustGet(int key,string tag="") {
		...
		public static rawObjToken? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjToken> GetByFilter(Func<rawObjToken, bool> filterFunc,string tag="") {
			List<rawObjToken> ret = new List<rawObjToken>();
			foreach(KeyValuePair <int, rawObjToken> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjToken? GetOneByFilter(Func<rawObjToken, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjToken> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjToken data) 
		{
			return data.Id;			
		}
    }
