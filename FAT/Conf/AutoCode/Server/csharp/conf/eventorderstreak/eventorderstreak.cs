...
using EventOrderStreakConf = fat.rawdata.EventOrderStreakConf;
using rawEventOrderStreak = fat.rawdata.EventOrderStreak;
using rawEventOrderStreakMapABValue = fat.rawdata.EventOrderStreakMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderStreakVisitor
    {
		...
		public const string FileName = "EventOrderStreakConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderStreakVisitor.SetABDefault单独设定EventOrderStreak的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderStreakVisitor.SetABFallback单独设定EventOrderStreak的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderStreak>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderStreak>>(()=> new PBC::MapField<int, rawEventOrderStreak>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderStreak>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderStreakMap;
			var abMap = v.EventOrderStreakMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderStreakMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderStreakMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderStreak> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderStreakMap;
			if (v.EventOrderStreakMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderStreakMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderStreakMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderStreakConf cc) {
			foreach(KeyValuePair <string, rawEventOrderStreakMapABValue> tagEntry in cc.EventOrderStreakMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderStreakMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderStreakMapABValue();
					cc.EventOrderStreakMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderStreakMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderStreak> entry in cc.EventOrderStreakMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderStreak> entry in tagEntry.Value.EventOrderStreakMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderStreak> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderStreakConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderStreakMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderStreakMapAB != null)
			{
				foreach (var entry in cc.EventOrderStreakMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderStreakMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderStreakMap.MergeFrom(cis);
			Patch(updating.EventOrderStreakMap);
			updating.EventOrderStreakSortedKeys = genSortedKeys(updating.EventOrderStreakMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderStreakConf();
            tmp.MergeFrom(cis);
			updating.EventOrderStreakMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderStreakSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderStreak> filter,string tag=""){
		...
		public static rawEventOrderStreak MustGet(int key,string tag="") {
		...
		public static rawEventOrderStreak? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderStreak> GetByFilter(Func<rawEventOrderStreak, bool> filterFunc,string tag="") {
			List<rawEventOrderStreak> ret = new List<rawEventOrderStreak>();
			foreach(KeyValuePair <int, rawEventOrderStreak> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderStreak? GetOneByFilter(Func<rawEventOrderStreak, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderStreak> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderStreak data) 
		{
			return data.Id;			
		}
    }
