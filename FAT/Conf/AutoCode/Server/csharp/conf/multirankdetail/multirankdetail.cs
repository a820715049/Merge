...
using MultiRankDetailConf = fat.rawdata.MultiRankDetailConf;
using rawMultiRankDetail = fat.rawdata.MultiRankDetail;
using rawMultiRankDetailMapABValue = fat.rawdata.MultiRankDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankDetailVisitor
    {
		...
		public const string FileName = "MultiRankDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankDetailVisitor.SetABDefault单独设定MultiRankDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankDetailVisitor.SetABFallback单独设定MultiRankDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRankDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRankDetail>>(()=> new PBC::MapField<int, rawMultiRankDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRankDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankDetailMap;
			var abMap = v.MultiRankDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRankDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankDetailMap;
			if (v.MultiRankDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankDetailConf cc) {
			foreach(KeyValuePair <string, rawMultiRankDetailMapABValue> tagEntry in cc.MultiRankDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankDetailMapABValue();
					cc.MultiRankDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRankDetail> entry in cc.MultiRankDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRankDetail> entry in tagEntry.Value.MultiRankDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRankDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankDetailMapAB != null)
			{
				foreach (var entry in cc.MultiRankDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankDetailMap.MergeFrom(cis);
			Patch(updating.MultiRankDetailMap);
			updating.MultiRankDetailSortedKeys = genSortedKeys(updating.MultiRankDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankDetailConf();
            tmp.MergeFrom(cis);
			updating.MultiRankDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRankDetail> filter,string tag=""){
		...
		public static rawMultiRankDetail MustGet(int key,string tag="") {
		...
		public static rawMultiRankDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRankDetail> GetByFilter(Func<rawMultiRankDetail, bool> filterFunc,string tag="") {
			List<rawMultiRankDetail> ret = new List<rawMultiRankDetail>();
			foreach(KeyValuePair <int, rawMultiRankDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRankDetail? GetOneByFilter(Func<rawMultiRankDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRankDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRankDetail data) 
		{
			return data.Id;			
		}
    }
