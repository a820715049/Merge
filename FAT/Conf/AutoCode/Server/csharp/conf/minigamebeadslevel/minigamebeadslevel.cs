...
using MiniGameBeadsLevelConf = fat.rawdata.MiniGameBeadsLevelConf;
using rawMiniGameBeadsLevel = fat.rawdata.MiniGameBeadsLevel;
using rawMiniGameBeadsLevelMapABValue = fat.rawdata.MiniGameBeadsLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameBeadsLevelVisitor
    {
		...
		public const string FileName = "MiniGameBeadsLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameBeadsLevelVisitor.SetABDefault单独设定MiniGameBeadsLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameBeadsLevelVisitor.SetABFallback单独设定MiniGameBeadsLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameBeadsLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameBeadsLevel>>(()=> new PBC::MapField<int, rawMiniGameBeadsLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameBeadsLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameBeadsLevelMap;
			var abMap = v.MiniGameBeadsLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameBeadsLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameBeadsLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameBeadsLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameBeadsLevelMap;
			if (v.MiniGameBeadsLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameBeadsLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameBeadsLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameBeadsLevelConf cc) {
			foreach(KeyValuePair <string, rawMiniGameBeadsLevelMapABValue> tagEntry in cc.MiniGameBeadsLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameBeadsLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameBeadsLevelMapABValue();
					cc.MiniGameBeadsLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameBeadsLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameBeadsLevel> entry in cc.MiniGameBeadsLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameBeadsLevel> entry in tagEntry.Value.MiniGameBeadsLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameBeadsLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameBeadsLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameBeadsLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameBeadsLevelMapAB != null)
			{
				foreach (var entry in cc.MiniGameBeadsLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameBeadsLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameBeadsLevelMap.MergeFrom(cis);
			Patch(updating.MiniGameBeadsLevelMap);
			updating.MiniGameBeadsLevelSortedKeys = genSortedKeys(updating.MiniGameBeadsLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameBeadsLevelConf();
            tmp.MergeFrom(cis);
			updating.MiniGameBeadsLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameBeadsLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameBeadsLevel> filter,string tag=""){
		...
		public static rawMiniGameBeadsLevel MustGet(int key,string tag="") {
		...
		public static rawMiniGameBeadsLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameBeadsLevel> GetByFilter(Func<rawMiniGameBeadsLevel, bool> filterFunc,string tag="") {
			List<rawMiniGameBeadsLevel> ret = new List<rawMiniGameBeadsLevel>();
			foreach(KeyValuePair <int, rawMiniGameBeadsLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameBeadsLevel? GetOneByFilter(Func<rawMiniGameBeadsLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameBeadsLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameBeadsLevel data) 
		{
			return data.Id;			
		}
    }
