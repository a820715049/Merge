...
using TrainGroupDetailConf = fat.rawdata.TrainGroupDetailConf;
using rawTrainGroupDetail = fat.rawdata.TrainGroupDetail;
using rawTrainGroupDetailMapABValue = fat.rawdata.TrainGroupDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class TrainGroupDetailVisitor
    {
		...
		public const string FileName = "TrainGroupDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过TrainGroupDetailVisitor.SetABDefault单独设定TrainGroupDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过TrainGroupDetailVisitor.SetABFallback单独设定TrainGroupDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawTrainGroupDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawTrainGroupDetail>>(()=> new PBC::MapField<int, rawTrainGroupDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawTrainGroupDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().TrainGroupDetailMap;
			var abMap = v.TrainGroupDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.TrainGroupDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].TrainGroupDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawTrainGroupDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().TrainGroupDetailMap;
			if (v.TrainGroupDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.TrainGroupDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawTrainGroupDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(TrainGroupDetailConf cc) {
			foreach(KeyValuePair <string, rawTrainGroupDetailMapABValue> tagEntry in cc.TrainGroupDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.TrainGroupDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawTrainGroupDetailMapABValue();
					cc.TrainGroupDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.TrainGroupDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawTrainGroupDetail> entry in cc.TrainGroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawTrainGroupDetail> entry in tagEntry.Value.TrainGroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.TrainGroupDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(TrainGroupDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.TrainGroupDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.TrainGroupDetailMapAB != null)
			{
				foreach (var entry in cc.TrainGroupDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.TrainGroupDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.TrainGroupDetailMap.MergeFrom(cis);
			Patch(updating.TrainGroupDetailMap);
			updating.TrainGroupDetailSortedKeys = genSortedKeys(updating.TrainGroupDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new TrainGroupDetailConf();
            tmp.MergeFrom(cis);
			updating.TrainGroupDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().TrainGroupDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawTrainGroupDetail> filter,string tag=""){
		...
		public static rawTrainGroupDetail MustGet(int key,string tag="") {
		...
		public static rawTrainGroupDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawTrainGroupDetail> GetByFilter(Func<rawTrainGroupDetail, bool> filterFunc,string tag="") {
			List<rawTrainGroupDetail> ret = new List<rawTrainGroupDetail>();
			foreach(KeyValuePair <int, rawTrainGroupDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawTrainGroupDetail? GetOneByFilter(Func<rawTrainGroupDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawTrainGroupDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawTrainGroupDetail data) 
		{
			return data.Id;			
		}
    }
