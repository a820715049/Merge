...
using TrainLevelGroupConf = fat.rawdata.TrainLevelGroupConf;
using rawTrainLevelGroup = fat.rawdata.TrainLevelGroup;
using rawTrainLevelGroupMapABValue = fat.rawdata.TrainLevelGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class TrainLevelGroupVisitor
    {
		...
		public const string FileName = "TrainLevelGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过TrainLevelGroupVisitor.SetABDefault单独设定TrainLevelGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过TrainLevelGroupVisitor.SetABFallback单独设定TrainLevelGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawTrainLevelGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawTrainLevelGroup>>(()=> new PBC::MapField<int, rawTrainLevelGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawTrainLevelGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().TrainLevelGroupMap;
			var abMap = v.TrainLevelGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.TrainLevelGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].TrainLevelGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawTrainLevelGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().TrainLevelGroupMap;
			if (v.TrainLevelGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.TrainLevelGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawTrainLevelGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(TrainLevelGroupConf cc) {
			foreach(KeyValuePair <string, rawTrainLevelGroupMapABValue> tagEntry in cc.TrainLevelGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.TrainLevelGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawTrainLevelGroupMapABValue();
					cc.TrainLevelGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.TrainLevelGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawTrainLevelGroup> entry in cc.TrainLevelGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawTrainLevelGroup> entry in tagEntry.Value.TrainLevelGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.TrainLevelGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(TrainLevelGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.TrainLevelGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.TrainLevelGroupMapAB != null)
			{
				foreach (var entry in cc.TrainLevelGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.TrainLevelGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.TrainLevelGroupMap.MergeFrom(cis);
			Patch(updating.TrainLevelGroupMap);
			updating.TrainLevelGroupSortedKeys = genSortedKeys(updating.TrainLevelGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new TrainLevelGroupConf();
            tmp.MergeFrom(cis);
			updating.TrainLevelGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().TrainLevelGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawTrainLevelGroup> filter,string tag=""){
		...
		public static rawTrainLevelGroup MustGet(int key,string tag="") {
		...
		public static rawTrainLevelGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawTrainLevelGroup> GetByFilter(Func<rawTrainLevelGroup, bool> filterFunc,string tag="") {
			List<rawTrainLevelGroup> ret = new List<rawTrainLevelGroup>();
			foreach(KeyValuePair <int, rawTrainLevelGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawTrainLevelGroup? GetOneByFilter(Func<rawTrainLevelGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawTrainLevelGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawTrainLevelGroup data) 
		{
			return data.Id;			
		}
    }
