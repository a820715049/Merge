...
using EventZeroQuestConf = fat.rawdata.EventZeroQuestConf;
using rawEventZeroQuest = fat.rawdata.EventZeroQuest;
using rawEventZeroQuestMapABValue = fat.rawdata.EventZeroQuestMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventZeroQuestVisitor
    {
		...
		public const string FileName = "EventZeroQuestConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventZeroQuestVisitor.SetABDefault单独设定EventZeroQuest的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventZeroQuestVisitor.SetABFallback单独设定EventZeroQuest的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventZeroQuest>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventZeroQuest>>(()=> new PBC::MapField<int, rawEventZeroQuest>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventZeroQuest>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventZeroQuestMap;
			var abMap = v.EventZeroQuestMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventZeroQuestMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventZeroQuestMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventZeroQuest> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventZeroQuestMap;
			if (v.EventZeroQuestMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventZeroQuestMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventZeroQuestMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventZeroQuestConf cc) {
			foreach(KeyValuePair <string, rawEventZeroQuestMapABValue> tagEntry in cc.EventZeroQuestMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventZeroQuestMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventZeroQuestMapABValue();
					cc.EventZeroQuestMapAB[tag] = ps;
				}
                var tagTmp = ps.EventZeroQuestMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventZeroQuest> entry in cc.EventZeroQuestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventZeroQuest> entry in tagEntry.Value.EventZeroQuestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventZeroQuest> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventZeroQuestConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventZeroQuestMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventZeroQuestMapAB != null)
			{
				foreach (var entry in cc.EventZeroQuestMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventZeroQuestMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventZeroQuestMap.MergeFrom(cis);
			Patch(updating.EventZeroQuestMap);
			updating.EventZeroQuestSortedKeys = genSortedKeys(updating.EventZeroQuestMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventZeroQuestConf();
            tmp.MergeFrom(cis);
			updating.EventZeroQuestMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventZeroQuestSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventZeroQuest> filter,string tag=""){
		...
		public static rawEventZeroQuest MustGet(int key,string tag="") {
		...
		public static rawEventZeroQuest? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventZeroQuest> GetByFilter(Func<rawEventZeroQuest, bool> filterFunc,string tag="") {
			List<rawEventZeroQuest> ret = new List<rawEventZeroQuest>();
			foreach(KeyValuePair <int, rawEventZeroQuest> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventZeroQuest? GetOneByFilter(Func<rawEventZeroQuest, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventZeroQuest> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventZeroQuest data) 
		{
			return data.Id;			
		}
    }
