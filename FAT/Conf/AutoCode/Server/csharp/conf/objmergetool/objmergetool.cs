...
using ObjMergeToolConf = fat.rawdata.ObjMergeToolConf;
using rawObjMergeTool = fat.rawdata.ObjMergeTool;
using rawObjMergeToolMapABValue = fat.rawdata.ObjMergeToolMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjMergeToolVisitor
    {
		...
		public const string FileName = "ObjMergeToolConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjMergeToolVisitor.SetABDefault单独设定ObjMergeTool的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjMergeToolVisitor.SetABFallback单独设定ObjMergeTool的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjMergeTool>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjMergeTool>>(()=> new PBC::MapField<int, rawObjMergeTool>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjMergeTool>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjMergeToolMap;
			var abMap = v.ObjMergeToolMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjMergeToolMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjMergeToolMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjMergeTool> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjMergeToolMap;
			if (v.ObjMergeToolMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjMergeToolMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjMergeToolMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjMergeToolConf cc) {
			foreach(KeyValuePair <string, rawObjMergeToolMapABValue> tagEntry in cc.ObjMergeToolMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjMergeToolMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjMergeToolMapABValue();
					cc.ObjMergeToolMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjMergeToolMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjMergeTool> entry in cc.ObjMergeToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjMergeTool> entry in tagEntry.Value.ObjMergeToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjMergeTool> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjMergeToolConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjMergeToolMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjMergeToolMapAB != null)
			{
				foreach (var entry in cc.ObjMergeToolMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjMergeToolMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjMergeToolMap.MergeFrom(cis);
			Patch(updating.ObjMergeToolMap);
			updating.ObjMergeToolSortedKeys = genSortedKeys(updating.ObjMergeToolMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjMergeToolConf();
            tmp.MergeFrom(cis);
			updating.ObjMergeToolMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjMergeToolSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjMergeTool> filter,string tag=""){
		...
		public static rawObjMergeTool MustGet(int key,string tag="") {
		...
		public static rawObjMergeTool? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjMergeTool> GetByFilter(Func<rawObjMergeTool, bool> filterFunc,string tag="") {
			List<rawObjMergeTool> ret = new List<rawObjMergeTool>();
			foreach(KeyValuePair <int, rawObjMergeTool> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjMergeTool? GetOneByFilter(Func<rawObjMergeTool, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjMergeTool> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjMergeTool data) 
		{
			return data.Id;			
		}
    }
