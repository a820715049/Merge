...
using EventLandmarkConf = fat.rawdata.EventLandmarkConf;
using rawEventLandmark = fat.rawdata.EventLandmark;
using rawEventLandmarkMapABValue = fat.rawdata.EventLandmarkMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventLandmarkVisitor
    {
		...
		public const string FileName = "EventLandmarkConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventLandmarkVisitor.SetABDefault单独设定EventLandmark的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventLandmarkVisitor.SetABFallback单独设定EventLandmark的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventLandmark>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventLandmark>>(()=> new PBC::MapField<int, rawEventLandmark>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventLandmark>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventLandmarkMap;
			var abMap = v.EventLandmarkMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventLandmarkMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventLandmarkMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventLandmark> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventLandmarkMap;
			if (v.EventLandmarkMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventLandmarkMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventLandmarkMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventLandmarkConf cc) {
			foreach(KeyValuePair <string, rawEventLandmarkMapABValue> tagEntry in cc.EventLandmarkMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventLandmarkMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventLandmarkMapABValue();
					cc.EventLandmarkMapAB[tag] = ps;
				}
                var tagTmp = ps.EventLandmarkMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventLandmark> entry in cc.EventLandmarkMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventLandmark> entry in tagEntry.Value.EventLandmarkMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventLandmark> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventLandmarkConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventLandmarkMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventLandmarkMapAB != null)
			{
				foreach (var entry in cc.EventLandmarkMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventLandmarkMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventLandmarkMap.MergeFrom(cis);
			Patch(updating.EventLandmarkMap);
			updating.EventLandmarkSortedKeys = genSortedKeys(updating.EventLandmarkMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventLandmarkConf();
            tmp.MergeFrom(cis);
			updating.EventLandmarkMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventLandmarkSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventLandmark> filter,string tag=""){
		...
		public static rawEventLandmark MustGet(int key,string tag="") {
		...
		public static rawEventLandmark? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventLandmark> GetByFilter(Func<rawEventLandmark, bool> filterFunc,string tag="") {
			List<rawEventLandmark> ret = new List<rawEventLandmark>();
			foreach(KeyValuePair <int, rawEventLandmark> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventLandmark? GetOneByFilter(Func<rawEventLandmark, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventLandmark> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventLandmark data) 
		{
			return data.Id;			
		}
    }
