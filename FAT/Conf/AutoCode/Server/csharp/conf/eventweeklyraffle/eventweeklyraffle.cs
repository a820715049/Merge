...
using EventWeeklyRaffleConf = fat.rawdata.EventWeeklyRaffleConf;
using rawEventWeeklyRaffle = fat.rawdata.EventWeeklyRaffle;
using rawEventWeeklyRaffleMapABValue = fat.rawdata.EventWeeklyRaffleMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyRaffleVisitor
    {
		...
		public const string FileName = "EventWeeklyRaffleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyRaffleVisitor.SetABDefault单独设定EventWeeklyRaffle的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyRaffleVisitor.SetABFallback单独设定EventWeeklyRaffle的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyRaffle>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyRaffle>>(()=> new PBC::MapField<int, rawEventWeeklyRaffle>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyRaffle>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyRaffleMap;
			var abMap = v.EventWeeklyRaffleMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyRaffleMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyRaffleMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyRaffle> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyRaffleMap;
			if (v.EventWeeklyRaffleMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyRaffleMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyRaffleMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyRaffleConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyRaffleMapABValue> tagEntry in cc.EventWeeklyRaffleMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyRaffleMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyRaffleMapABValue();
					cc.EventWeeklyRaffleMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyRaffleMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffle> entry in cc.EventWeeklyRaffleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffle> entry in tagEntry.Value.EventWeeklyRaffleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyRaffle> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyRaffleConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyRaffleMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyRaffleMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyRaffleMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyRaffleMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyRaffleMap.MergeFrom(cis);
			Patch(updating.EventWeeklyRaffleMap);
			updating.EventWeeklyRaffleSortedKeys = genSortedKeys(updating.EventWeeklyRaffleMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyRaffleConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyRaffleMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyRaffleSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyRaffle> filter,string tag=""){
		...
		public static rawEventWeeklyRaffle MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyRaffle? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyRaffle> GetByFilter(Func<rawEventWeeklyRaffle, bool> filterFunc,string tag="") {
			List<rawEventWeeklyRaffle> ret = new List<rawEventWeeklyRaffle>();
			foreach(KeyValuePair <int, rawEventWeeklyRaffle> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyRaffle? GetOneByFilter(Func<rawEventWeeklyRaffle, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyRaffle> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyRaffle data) 
		{
			return data.Id;			
		}
    }
