...
using EventWishEndlessInfoConf = fat.rawdata.EventWishEndlessInfoConf;
using rawEventWishEndlessInfo = fat.rawdata.EventWishEndlessInfo;
using rawEventWishEndlessInfoMapABValue = fat.rawdata.EventWishEndlessInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishEndlessInfoVisitor
    {
		...
		public const string FileName = "EventWishEndlessInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishEndlessInfoVisitor.SetABDefault单独设定EventWishEndlessInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishEndlessInfoVisitor.SetABFallback单独设定EventWishEndlessInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishEndlessInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishEndlessInfo>>(()=> new PBC::MapField<int, rawEventWishEndlessInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishEndlessInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishEndlessInfoMap;
			var abMap = v.EventWishEndlessInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishEndlessInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishEndlessInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishEndlessInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishEndlessInfoMap;
			if (v.EventWishEndlessInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishEndlessInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishEndlessInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishEndlessInfoConf cc) {
			foreach(KeyValuePair <string, rawEventWishEndlessInfoMapABValue> tagEntry in cc.EventWishEndlessInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishEndlessInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishEndlessInfoMapABValue();
					cc.EventWishEndlessInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishEndlessInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishEndlessInfo> entry in cc.EventWishEndlessInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishEndlessInfo> entry in tagEntry.Value.EventWishEndlessInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishEndlessInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishEndlessInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishEndlessInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishEndlessInfoMapAB != null)
			{
				foreach (var entry in cc.EventWishEndlessInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishEndlessInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishEndlessInfoMap.MergeFrom(cis);
			Patch(updating.EventWishEndlessInfoMap);
			updating.EventWishEndlessInfoSortedKeys = genSortedKeys(updating.EventWishEndlessInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishEndlessInfoConf();
            tmp.MergeFrom(cis);
			updating.EventWishEndlessInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishEndlessInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishEndlessInfo> filter,string tag=""){
		...
		public static rawEventWishEndlessInfo MustGet(int key,string tag="") {
		...
		public static rawEventWishEndlessInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishEndlessInfo> GetByFilter(Func<rawEventWishEndlessInfo, bool> filterFunc,string tag="") {
			List<rawEventWishEndlessInfo> ret = new List<rawEventWishEndlessInfo>();
			foreach(KeyValuePair <int, rawEventWishEndlessInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishEndlessInfo? GetOneByFilter(Func<rawEventWishEndlessInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishEndlessInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishEndlessInfo data) 
		{
			return data.Id;			
		}
    }
