...
using PlotDramaConf = fat.rawdata.PlotDramaConf;
using rawPlotDrama = fat.rawdata.PlotDrama;
using rawPlotDramaMapABValue = fat.rawdata.PlotDramaMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PlotDramaVisitor
    {
		...
		public const string FileName = "PlotDramaConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PlotDramaVisitor.SetABDefault单独设定PlotDrama的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PlotDramaVisitor.SetABFallback单独设定PlotDrama的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPlotDrama>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPlotDrama>>(()=> new PBC::MapField<int, rawPlotDrama>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPlotDrama>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PlotDramaMap;
			var abMap = v.PlotDramaMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PlotDramaMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PlotDramaMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPlotDrama> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PlotDramaMap;
			if (v.PlotDramaMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PlotDramaMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPlotDramaMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PlotDramaConf cc) {
			foreach(KeyValuePair <string, rawPlotDramaMapABValue> tagEntry in cc.PlotDramaMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PlotDramaMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPlotDramaMapABValue();
					cc.PlotDramaMapAB[tag] = ps;
				}
                var tagTmp = ps.PlotDramaMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPlotDrama> entry in cc.PlotDramaMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPlotDrama> entry in tagEntry.Value.PlotDramaMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.PlotDrama> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PlotDramaConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PlotDramaMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PlotDramaMapAB != null)
			{
				foreach (var entry in cc.PlotDramaMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PlotDramaMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PlotDramaMap.MergeFrom(cis);
			Patch(updating.PlotDramaMap);
			updating.PlotDramaSortedKeys = genSortedKeys(updating.PlotDramaMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PlotDramaConf();
            tmp.MergeFrom(cis);
			updating.PlotDramaMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PlotDramaSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPlotDrama> filter,string tag=""){
		...
		public static rawPlotDrama MustGet(int key,string tag="") {
		...
		public static rawPlotDrama? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPlotDrama> GetByFilter(Func<rawPlotDrama, bool> filterFunc,string tag="") {
			List<rawPlotDrama> ret = new List<rawPlotDrama>();
			foreach(KeyValuePair <int, rawPlotDrama> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPlotDrama? GetOneByFilter(Func<rawPlotDrama, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPlotDrama> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPlotDrama data) 
		{
			return data.Id;			
		}
    }
