...
using IAPPackConf = fat.rawdata.IAPPackConf;
using rawIAPPack = fat.rawdata.IAPPack;
using rawIAPPackMapABValue = fat.rawdata.IAPPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IAPPackVisitor
    {
		...
		public const string FileName = "IAPPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IAPPackVisitor.SetABDefault单独设定IAPPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IAPPackVisitor.SetABFallback单独设定IAPPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIAPPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIAPPack>>(()=> new PBC::MapField<int, rawIAPPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIAPPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IAPPackMap;
			var abMap = v.IAPPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IAPPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IAPPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIAPPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IAPPackMap;
			if (v.IAPPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IAPPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIAPPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IAPPackConf cc) {
			foreach(KeyValuePair <string, rawIAPPackMapABValue> tagEntry in cc.IAPPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IAPPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIAPPackMapABValue();
					cc.IAPPackMapAB[tag] = ps;
				}
                var tagTmp = ps.IAPPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIAPPack> entry in cc.IAPPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIAPPack> entry in tagEntry.Value.IAPPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IAPPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IAPPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IAPPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IAPPackMapAB != null)
			{
				foreach (var entry in cc.IAPPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IAPPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IAPPackMap.MergeFrom(cis);
			Patch(updating.IAPPackMap);
			updating.IAPPackSortedKeys = genSortedKeys(updating.IAPPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IAPPackConf();
            tmp.MergeFrom(cis);
			updating.IAPPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IAPPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIAPPack> filter,string tag=""){
		...
		public static rawIAPPack MustGet(int key,string tag="") {
		...
		public static rawIAPPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIAPPack> GetByFilter(Func<rawIAPPack, bool> filterFunc,string tag="") {
			List<rawIAPPack> ret = new List<rawIAPPack>();
			foreach(KeyValuePair <int, rawIAPPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIAPPack? GetOneByFilter(Func<rawIAPPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIAPPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIAPPack data) 
		{
			return data.Id;			
		}
    }
