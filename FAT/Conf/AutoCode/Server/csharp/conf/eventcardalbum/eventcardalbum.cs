...
using EventCardAlbumConf = fat.rawdata.EventCardAlbumConf;
using rawEventCardAlbum = fat.rawdata.EventCardAlbum;
using rawEventCardAlbumMapABValue = fat.rawdata.EventCardAlbumMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventCardAlbumVisitor
    {
		...
		public const string FileName = "EventCardAlbumConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventCardAlbumVisitor.SetABDefault单独设定EventCardAlbum的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventCardAlbumVisitor.SetABFallback单独设定EventCardAlbum的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventCardAlbum>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventCardAlbum>>(()=> new PBC::MapField<int, rawEventCardAlbum>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventCardAlbum>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventCardAlbumMap;
			var abMap = v.EventCardAlbumMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventCardAlbumMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventCardAlbumMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventCardAlbum> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventCardAlbumMap;
			if (v.EventCardAlbumMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventCardAlbumMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventCardAlbumMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventCardAlbumConf cc) {
			foreach(KeyValuePair <string, rawEventCardAlbumMapABValue> tagEntry in cc.EventCardAlbumMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventCardAlbumMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventCardAlbumMapABValue();
					cc.EventCardAlbumMapAB[tag] = ps;
				}
                var tagTmp = ps.EventCardAlbumMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventCardAlbum> entry in cc.EventCardAlbumMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventCardAlbum> entry in tagEntry.Value.EventCardAlbumMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventCardAlbum> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventCardAlbumConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventCardAlbumMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventCardAlbumMapAB != null)
			{
				foreach (var entry in cc.EventCardAlbumMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventCardAlbumMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventCardAlbumMap.MergeFrom(cis);
			Patch(updating.EventCardAlbumMap);
			updating.EventCardAlbumSortedKeys = genSortedKeys(updating.EventCardAlbumMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventCardAlbumConf();
            tmp.MergeFrom(cis);
			updating.EventCardAlbumMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventCardAlbumSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventCardAlbum> filter,string tag=""){
		...
		public static rawEventCardAlbum MustGet(int key,string tag="") {
		...
		public static rawEventCardAlbum? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventCardAlbum> GetByFilter(Func<rawEventCardAlbum, bool> filterFunc,string tag="") {
			List<rawEventCardAlbum> ret = new List<rawEventCardAlbum>();
			foreach(KeyValuePair <int, rawEventCardAlbum> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventCardAlbum? GetOneByFilter(Func<rawEventCardAlbum, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventCardAlbum> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventCardAlbum data) 
		{
			return data.Id;			
		}
    }
