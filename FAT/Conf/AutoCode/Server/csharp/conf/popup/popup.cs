...
using PopupConf = fat.rawdata.PopupConf;
using rawPopup = fat.rawdata.Popup;
using rawPopupMapABValue = fat.rawdata.PopupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PopupVisitor
    {
		...
		public const string FileName = "PopupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PopupVisitor.SetABDefault单独设定Popup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PopupVisitor.SetABFallback单独设定Popup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPopup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPopup>>(()=> new PBC::MapField<int, rawPopup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPopup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PopupMap;
			var abMap = v.PopupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PopupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PopupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPopup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PopupMap;
			if (v.PopupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PopupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPopupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PopupConf cc) {
			foreach(KeyValuePair <string, rawPopupMapABValue> tagEntry in cc.PopupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PopupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPopupMapABValue();
					cc.PopupMapAB[tag] = ps;
				}
                var tagTmp = ps.PopupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPopup> entry in cc.PopupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPopup> entry in tagEntry.Value.PopupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Popup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PopupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PopupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PopupMapAB != null)
			{
				foreach (var entry in cc.PopupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PopupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PopupMap.MergeFrom(cis);
			Patch(updating.PopupMap);
			updating.PopupSortedKeys = genSortedKeys(updating.PopupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PopupConf();
            tmp.MergeFrom(cis);
			updating.PopupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PopupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPopup> filter,string tag=""){
		...
		public static rawPopup MustGet(int key,string tag="") {
		...
		public static rawPopup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPopup> GetByFilter(Func<rawPopup, bool> filterFunc,string tag="") {
			List<rawPopup> ret = new List<rawPopup>();
			foreach(KeyValuePair <int, rawPopup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPopup? GetOneByFilter(Func<rawPopup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPopup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPopup data) 
		{
			return data.Id;			
		}
    }
