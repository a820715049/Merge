...
using LangJaHotfixConf = fat.rawdata.LangJaHotfixConf;
using rawLangJaHotfix = fat.rawdata.LangJaHotfix;
using rawLangJaHotfixSliceABValue = fat.rawdata.LangJaHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangJaHotfixVisitor
    {
		...
		public const string FileName = "LangJaHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangJaHotfixVisitor.SetABDefault单独设定LangJaHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangJaHotfixVisitor.SetABFallback单独设定LangJaHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangJaHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangJaHotfix>>(()=>new PBC::RepeatedField<rawLangJaHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangJaHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangJaHotfixSlice;
			var abMap = v.LangJaHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangJaHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangJaHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangJaHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangJaHotfixSlice;
			if (v.LangJaHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangJaHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangJaHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangJaHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangJaHotfixSliceABValue> tagEntry in cc.LangJaHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangJaHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangJaHotfixSliceABValue();
					cc.LangJaHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangJaHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangJaHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangJaHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangJaHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangJaHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangJaHotfixSlice.MergeFrom(cis);
			Patch(updating.LangJaHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangJaHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangJaHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangJaHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangJaHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangJaHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangJaHotfix> GetByFilter(Func<rawLangJaHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangJaHotfix>();
		...
		static public rawLangJaHotfix? GetOneByFilter(Func<rawLangJaHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
