...
using ComMergeMixSourceConf = fat.rawdata.ComMergeMixSourceConf;
using rawComMergeMixSource = fat.rawdata.ComMergeMixSource;
using rawComMergeMixSourceMapABValue = fat.rawdata.ComMergeMixSourceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeMixSourceVisitor
    {
		...
		public const string FileName = "ComMergeMixSourceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeMixSourceVisitor.SetABDefault单独设定ComMergeMixSource的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeMixSourceVisitor.SetABFallback单独设定ComMergeMixSource的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeMixSource>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeMixSource>>(()=> new PBC::MapField<int, rawComMergeMixSource>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeMixSource>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeMixSourceMap;
			var abMap = v.ComMergeMixSourceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeMixSourceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeMixSourceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeMixSource> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeMixSourceMap;
			if (v.ComMergeMixSourceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeMixSourceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeMixSourceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeMixSourceConf cc) {
			foreach(KeyValuePair <string, rawComMergeMixSourceMapABValue> tagEntry in cc.ComMergeMixSourceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeMixSourceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeMixSourceMapABValue();
					cc.ComMergeMixSourceMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeMixSourceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeMixSource> entry in cc.ComMergeMixSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeMixSource> entry in tagEntry.Value.ComMergeMixSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeMixSource> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeMixSourceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeMixSourceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeMixSourceMapAB != null)
			{
				foreach (var entry in cc.ComMergeMixSourceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeMixSourceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeMixSourceMap.MergeFrom(cis);
			Patch(updating.ComMergeMixSourceMap);
			updating.ComMergeMixSourceSortedKeys = genSortedKeys(updating.ComMergeMixSourceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeMixSourceConf();
            tmp.MergeFrom(cis);
			updating.ComMergeMixSourceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeMixSourceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeMixSource> filter,string tag=""){
		...
		public static rawComMergeMixSource MustGet(int key,string tag="") {
		...
		public static rawComMergeMixSource? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeMixSource> GetByFilter(Func<rawComMergeMixSource, bool> filterFunc,string tag="") {
			List<rawComMergeMixSource> ret = new List<rawComMergeMixSource>();
			foreach(KeyValuePair <int, rawComMergeMixSource> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeMixSource? GetOneByFilter(Func<rawComMergeMixSource, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeMixSource> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeMixSource data) 
		{
			return data.Id;			
		}
    }
