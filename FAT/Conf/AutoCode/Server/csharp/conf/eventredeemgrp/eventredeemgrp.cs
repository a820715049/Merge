...
using EventRedeemGrpConf = fat.rawdata.EventRedeemGrpConf;
using rawEventRedeemGrp = fat.rawdata.EventRedeemGrp;
using rawEventRedeemGrpMapABValue = fat.rawdata.EventRedeemGrpMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRedeemGrpVisitor
    {
		...
		public const string FileName = "EventRedeemGrpConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRedeemGrpVisitor.SetABDefault单独设定EventRedeemGrp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRedeemGrpVisitor.SetABFallback单独设定EventRedeemGrp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRedeemGrp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRedeemGrp>>(()=> new PBC::MapField<int, rawEventRedeemGrp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRedeemGrp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRedeemGrpMap;
			var abMap = v.EventRedeemGrpMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRedeemGrpMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRedeemGrpMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRedeemGrp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRedeemGrpMap;
			if (v.EventRedeemGrpMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRedeemGrpMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRedeemGrpMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRedeemGrpConf cc) {
			foreach(KeyValuePair <string, rawEventRedeemGrpMapABValue> tagEntry in cc.EventRedeemGrpMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRedeemGrpMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRedeemGrpMapABValue();
					cc.EventRedeemGrpMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRedeemGrpMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRedeemGrp> entry in cc.EventRedeemGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRedeemGrp> entry in tagEntry.Value.EventRedeemGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRedeemGrp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRedeemGrpConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRedeemGrpMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRedeemGrpMapAB != null)
			{
				foreach (var entry in cc.EventRedeemGrpMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRedeemGrpMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRedeemGrpMap.MergeFrom(cis);
			Patch(updating.EventRedeemGrpMap);
			updating.EventRedeemGrpSortedKeys = genSortedKeys(updating.EventRedeemGrpMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRedeemGrpConf();
            tmp.MergeFrom(cis);
			updating.EventRedeemGrpMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRedeemGrpSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRedeemGrp> filter,string tag=""){
		...
		public static rawEventRedeemGrp MustGet(int key,string tag="") {
		...
		public static rawEventRedeemGrp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRedeemGrp> GetByFilter(Func<rawEventRedeemGrp, bool> filterFunc,string tag="") {
			List<rawEventRedeemGrp> ret = new List<rawEventRedeemGrp>();
			foreach(KeyValuePair <int, rawEventRedeemGrp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRedeemGrp? GetOneByFilter(Func<rawEventRedeemGrp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRedeemGrp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRedeemGrp data) 
		{
			return data.Id;			
		}
    }
