...
using EventBingoTaskInfoConf = fat.rawdata.EventBingoTaskInfoConf;
using rawEventBingoTaskInfo = fat.rawdata.EventBingoTaskInfo;
using rawEventBingoTaskInfoMapABValue = fat.rawdata.EventBingoTaskInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventBingoTaskInfoVisitor
    {
		...
		public const string FileName = "EventBingoTaskInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventBingoTaskInfoVisitor.SetABDefault单独设定EventBingoTaskInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventBingoTaskInfoVisitor.SetABFallback单独设定EventBingoTaskInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventBingoTaskInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventBingoTaskInfo>>(()=> new PBC::MapField<int, rawEventBingoTaskInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventBingoTaskInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventBingoTaskInfoMap;
			var abMap = v.EventBingoTaskInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventBingoTaskInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventBingoTaskInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventBingoTaskInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventBingoTaskInfoMap;
			if (v.EventBingoTaskInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventBingoTaskInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventBingoTaskInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventBingoTaskInfoConf cc) {
			foreach(KeyValuePair <string, rawEventBingoTaskInfoMapABValue> tagEntry in cc.EventBingoTaskInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventBingoTaskInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventBingoTaskInfoMapABValue();
					cc.EventBingoTaskInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventBingoTaskInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventBingoTaskInfo> entry in cc.EventBingoTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventBingoTaskInfo> entry in tagEntry.Value.EventBingoTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventBingoTaskInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventBingoTaskInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventBingoTaskInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventBingoTaskInfoMapAB != null)
			{
				foreach (var entry in cc.EventBingoTaskInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventBingoTaskInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventBingoTaskInfoMap.MergeFrom(cis);
			Patch(updating.EventBingoTaskInfoMap);
			updating.EventBingoTaskInfoSortedKeys = genSortedKeys(updating.EventBingoTaskInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventBingoTaskInfoConf();
            tmp.MergeFrom(cis);
			updating.EventBingoTaskInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventBingoTaskInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventBingoTaskInfo> filter,string tag=""){
		...
		public static rawEventBingoTaskInfo MustGet(int key,string tag="") {
		...
		public static rawEventBingoTaskInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventBingoTaskInfo> GetByFilter(Func<rawEventBingoTaskInfo, bool> filterFunc,string tag="") {
			List<rawEventBingoTaskInfo> ret = new List<rawEventBingoTaskInfo>();
			foreach(KeyValuePair <int, rawEventBingoTaskInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventBingoTaskInfo? GetOneByFilter(Func<rawEventBingoTaskInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventBingoTaskInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventBingoTaskInfo data) 
		{
			return data.Id;			
		}
    }
