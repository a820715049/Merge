...
using ShopCommunityConf = fat.rawdata.ShopCommunityConf;
using rawShopCommunity = fat.rawdata.ShopCommunity;
using rawShopCommunityMapABValue = fat.rawdata.ShopCommunityMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ShopCommunityVisitor
    {
		...
		public const string FileName = "ShopCommunityConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ShopCommunityVisitor.SetABDefault单独设定ShopCommunity的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ShopCommunityVisitor.SetABFallback单独设定ShopCommunity的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawShopCommunity>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawShopCommunity>>(()=> new PBC::MapField<int, rawShopCommunity>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawShopCommunity>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ShopCommunityMap;
			var abMap = v.ShopCommunityMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ShopCommunityMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ShopCommunityMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawShopCommunity> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ShopCommunityMap;
			if (v.ShopCommunityMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ShopCommunityMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawShopCommunityMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ShopCommunityConf cc) {
			foreach(KeyValuePair <string, rawShopCommunityMapABValue> tagEntry in cc.ShopCommunityMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ShopCommunityMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawShopCommunityMapABValue();
					cc.ShopCommunityMapAB[tag] = ps;
				}
                var tagTmp = ps.ShopCommunityMap;
				// 基础数据
				foreach(KeyValuePair <int, rawShopCommunity> entry in cc.ShopCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawShopCommunity> entry in tagEntry.Value.ShopCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ShopCommunity> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ShopCommunityConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ShopCommunityMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ShopCommunityMapAB != null)
			{
				foreach (var entry in cc.ShopCommunityMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ShopCommunityMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ShopCommunityMap.MergeFrom(cis);
			Patch(updating.ShopCommunityMap);
			updating.ShopCommunitySortedKeys = genSortedKeys(updating.ShopCommunityMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ShopCommunityConf();
            tmp.MergeFrom(cis);
			updating.ShopCommunityMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ShopCommunitySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawShopCommunity> filter,string tag=""){
		...
		public static rawShopCommunity MustGet(int key,string tag="") {
		...
		public static rawShopCommunity? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawShopCommunity> GetByFilter(Func<rawShopCommunity, bool> filterFunc,string tag="") {
			List<rawShopCommunity> ret = new List<rawShopCommunity>();
			foreach(KeyValuePair <int, rawShopCommunity> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawShopCommunity? GetOneByFilter(Func<rawShopCommunity, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawShopCommunity> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawShopCommunity data) 
		{
			return data.Id;			
		}
    }
