...
using DailyEventListConf = fat.rawdata.DailyEventListConf;
using rawDailyEventList = fat.rawdata.DailyEventList;
using rawDailyEventListMapABValue = fat.rawdata.DailyEventListMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DailyEventListVisitor
    {
		...
		public const string FileName = "DailyEventListConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DailyEventListVisitor.SetABDefault单独设定DailyEventList的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DailyEventListVisitor.SetABFallback单独设定DailyEventList的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDailyEventList>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDailyEventList>>(()=> new PBC::MapField<int, rawDailyEventList>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDailyEventList>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DailyEventListMap;
			var abMap = v.DailyEventListMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DailyEventListMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DailyEventListMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDailyEventList> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DailyEventListMap;
			if (v.DailyEventListMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DailyEventListMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDailyEventListMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DailyEventListConf cc) {
			foreach(KeyValuePair <string, rawDailyEventListMapABValue> tagEntry in cc.DailyEventListMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DailyEventListMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDailyEventListMapABValue();
					cc.DailyEventListMapAB[tag] = ps;
				}
                var tagTmp = ps.DailyEventListMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDailyEventList> entry in cc.DailyEventListMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDailyEventList> entry in tagEntry.Value.DailyEventListMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DailyEventList> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DailyEventListConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DailyEventListMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DailyEventListMapAB != null)
			{
				foreach (var entry in cc.DailyEventListMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DailyEventListMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DailyEventListMap.MergeFrom(cis);
			Patch(updating.DailyEventListMap);
			updating.DailyEventListSortedKeys = genSortedKeys(updating.DailyEventListMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DailyEventListConf();
            tmp.MergeFrom(cis);
			updating.DailyEventListMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DailyEventListSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDailyEventList> filter,string tag=""){
		...
		public static rawDailyEventList MustGet(int key,string tag="") {
		...
		public static rawDailyEventList? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDailyEventList> GetByFilter(Func<rawDailyEventList, bool> filterFunc,string tag="") {
			List<rawDailyEventList> ret = new List<rawDailyEventList>();
			foreach(KeyValuePair <int, rawDailyEventList> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDailyEventList? GetOneByFilter(Func<rawDailyEventList, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDailyEventList> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDailyEventList data) 
		{
			return data.Id;			
		}
    }
