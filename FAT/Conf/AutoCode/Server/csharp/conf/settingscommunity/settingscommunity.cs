...
using SettingsCommunityConf = fat.rawdata.SettingsCommunityConf;
using rawSettingsCommunity = fat.rawdata.SettingsCommunity;
using rawSettingsCommunityMapABValue = fat.rawdata.SettingsCommunityMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SettingsCommunityVisitor
    {
		...
		public const string FileName = "SettingsCommunityConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SettingsCommunityVisitor.SetABDefault单独设定SettingsCommunity的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SettingsCommunityVisitor.SetABFallback单独设定SettingsCommunity的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSettingsCommunity>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSettingsCommunity>>(()=> new PBC::MapField<int, rawSettingsCommunity>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSettingsCommunity>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SettingsCommunityMap;
			var abMap = v.SettingsCommunityMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SettingsCommunityMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SettingsCommunityMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSettingsCommunity> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SettingsCommunityMap;
			if (v.SettingsCommunityMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SettingsCommunityMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSettingsCommunityMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SettingsCommunityConf cc) {
			foreach(KeyValuePair <string, rawSettingsCommunityMapABValue> tagEntry in cc.SettingsCommunityMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SettingsCommunityMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSettingsCommunityMapABValue();
					cc.SettingsCommunityMapAB[tag] = ps;
				}
                var tagTmp = ps.SettingsCommunityMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSettingsCommunity> entry in cc.SettingsCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSettingsCommunity> entry in tagEntry.Value.SettingsCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SettingsCommunity> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SettingsCommunityConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SettingsCommunityMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SettingsCommunityMapAB != null)
			{
				foreach (var entry in cc.SettingsCommunityMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SettingsCommunityMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SettingsCommunityMap.MergeFrom(cis);
			Patch(updating.SettingsCommunityMap);
			updating.SettingsCommunitySortedKeys = genSortedKeys(updating.SettingsCommunityMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SettingsCommunityConf();
            tmp.MergeFrom(cis);
			updating.SettingsCommunityMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SettingsCommunitySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSettingsCommunity> filter,string tag=""){
		...
		public static rawSettingsCommunity MustGet(int key,string tag="") {
		...
		public static rawSettingsCommunity? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSettingsCommunity> GetByFilter(Func<rawSettingsCommunity, bool> filterFunc,string tag="") {
			List<rawSettingsCommunity> ret = new List<rawSettingsCommunity>();
			foreach(KeyValuePair <int, rawSettingsCommunity> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSettingsCommunity? GetOneByFilter(Func<rawSettingsCommunity, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSettingsCommunity> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSettingsCommunity data) 
		{
			return data.Id;			
		}
    }
