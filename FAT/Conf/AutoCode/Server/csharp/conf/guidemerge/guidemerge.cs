...
using GuideMergeConf = fat.rawdata.GuideMergeConf;
using rawGuideMerge = fat.rawdata.GuideMerge;
using rawGuideMergeMapABValue = fat.rawdata.GuideMergeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GuideMergeVisitor
    {
		...
		public const string FileName = "GuideMergeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GuideMergeVisitor.SetABDefault单独设定GuideMerge的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GuideMergeVisitor.SetABFallback单独设定GuideMerge的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGuideMerge>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGuideMerge>>(()=> new PBC::MapField<int, rawGuideMerge>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGuideMerge>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GuideMergeMap;
			var abMap = v.GuideMergeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GuideMergeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GuideMergeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGuideMerge> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GuideMergeMap;
			if (v.GuideMergeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GuideMergeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGuideMergeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GuideMergeConf cc) {
			foreach(KeyValuePair <string, rawGuideMergeMapABValue> tagEntry in cc.GuideMergeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GuideMergeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGuideMergeMapABValue();
					cc.GuideMergeMapAB[tag] = ps;
				}
                var tagTmp = ps.GuideMergeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGuideMerge> entry in cc.GuideMergeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGuideMerge> entry in tagEntry.Value.GuideMergeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GuideMerge> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GuideMergeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GuideMergeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GuideMergeMapAB != null)
			{
				foreach (var entry in cc.GuideMergeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GuideMergeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GuideMergeMap.MergeFrom(cis);
			Patch(updating.GuideMergeMap);
			updating.GuideMergeSortedKeys = genSortedKeys(updating.GuideMergeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GuideMergeConf();
            tmp.MergeFrom(cis);
			updating.GuideMergeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GuideMergeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGuideMerge> filter,string tag=""){
		...
		public static rawGuideMerge MustGet(int key,string tag="") {
		...
		public static rawGuideMerge? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGuideMerge> GetByFilter(Func<rawGuideMerge, bool> filterFunc,string tag="") {
			List<rawGuideMerge> ret = new List<rawGuideMerge>();
			foreach(KeyValuePair <int, rawGuideMerge> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGuideMerge? GetOneByFilter(Func<rawGuideMerge, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGuideMerge> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGuideMerge data) 
		{
			return data.Id;			
		}
    }
