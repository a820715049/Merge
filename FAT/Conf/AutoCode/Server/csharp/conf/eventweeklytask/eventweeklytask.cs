...
using EventWeeklyTaskConf = fat.rawdata.EventWeeklyTaskConf;
using rawEventWeeklyTask = fat.rawdata.EventWeeklyTask;
using rawEventWeeklyTaskMapABValue = fat.rawdata.EventWeeklyTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyTaskVisitor
    {
		...
		public const string FileName = "EventWeeklyTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyTaskVisitor.SetABDefault单独设定EventWeeklyTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyTaskVisitor.SetABFallback单独设定EventWeeklyTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyTask>>(()=> new PBC::MapField<int, rawEventWeeklyTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyTaskMap;
			var abMap = v.EventWeeklyTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyTaskMap;
			if (v.EventWeeklyTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyTaskConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyTaskMapABValue> tagEntry in cc.EventWeeklyTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyTaskMapABValue();
					cc.EventWeeklyTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyTask> entry in cc.EventWeeklyTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyTask> entry in tagEntry.Value.EventWeeklyTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyTaskMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyTaskMap.MergeFrom(cis);
			Patch(updating.EventWeeklyTaskMap);
			updating.EventWeeklyTaskSortedKeys = genSortedKeys(updating.EventWeeklyTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyTaskConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyTask> filter,string tag=""){
		...
		public static rawEventWeeklyTask MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyTask> GetByFilter(Func<rawEventWeeklyTask, bool> filterFunc,string tag="") {
			List<rawEventWeeklyTask> ret = new List<rawEventWeeklyTask>();
			foreach(KeyValuePair <int, rawEventWeeklyTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyTask? GetOneByFilter(Func<rawEventWeeklyTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyTask data) 
		{
			return data.Id;			
		}
    }
