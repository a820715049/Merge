...
using EventCartOnePlusOneConf = fat.rawdata.EventCartOnePlusOneConf;
using rawEventCartOnePlusOne = fat.rawdata.EventCartOnePlusOne;
using rawEventCartOnePlusOneMapABValue = fat.rawdata.EventCartOnePlusOneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventCartOnePlusOneVisitor
    {
		...
		public const string FileName = "EventCartOnePlusOneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventCartOnePlusOneVisitor.SetABDefault单独设定EventCartOnePlusOne的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventCartOnePlusOneVisitor.SetABFallback单独设定EventCartOnePlusOne的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventCartOnePlusOne>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventCartOnePlusOne>>(()=> new PBC::MapField<int, rawEventCartOnePlusOne>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventCartOnePlusOne>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventCartOnePlusOneMap;
			var abMap = v.EventCartOnePlusOneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventCartOnePlusOneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventCartOnePlusOneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventCartOnePlusOne> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventCartOnePlusOneMap;
			if (v.EventCartOnePlusOneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventCartOnePlusOneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventCartOnePlusOneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventCartOnePlusOneConf cc) {
			foreach(KeyValuePair <string, rawEventCartOnePlusOneMapABValue> tagEntry in cc.EventCartOnePlusOneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventCartOnePlusOneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventCartOnePlusOneMapABValue();
					cc.EventCartOnePlusOneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventCartOnePlusOneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventCartOnePlusOne> entry in cc.EventCartOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventCartOnePlusOne> entry in tagEntry.Value.EventCartOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventCartOnePlusOne> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventCartOnePlusOneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventCartOnePlusOneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventCartOnePlusOneMapAB != null)
			{
				foreach (var entry in cc.EventCartOnePlusOneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventCartOnePlusOneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventCartOnePlusOneMap.MergeFrom(cis);
			Patch(updating.EventCartOnePlusOneMap);
			updating.EventCartOnePlusOneSortedKeys = genSortedKeys(updating.EventCartOnePlusOneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventCartOnePlusOneConf();
            tmp.MergeFrom(cis);
			updating.EventCartOnePlusOneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventCartOnePlusOneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventCartOnePlusOne> filter,string tag=""){
		...
		public static rawEventCartOnePlusOne MustGet(int key,string tag="") {
		...
		public static rawEventCartOnePlusOne? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventCartOnePlusOne> GetByFilter(Func<rawEventCartOnePlusOne, bool> filterFunc,string tag="") {
			List<rawEventCartOnePlusOne> ret = new List<rawEventCartOnePlusOne>();
			foreach(KeyValuePair <int, rawEventCartOnePlusOne> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventCartOnePlusOne? GetOneByFilter(Func<rawEventCartOnePlusOne, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventCartOnePlusOne> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventCartOnePlusOne data) 
		{
			return data.Id;			
		}
    }
