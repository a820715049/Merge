...
using EventItemBingoConf = fat.rawdata.EventItemBingoConf;
using rawEventItemBingo = fat.rawdata.EventItemBingo;
using rawEventItemBingoMapABValue = fat.rawdata.EventItemBingoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventItemBingoVisitor
    {
		...
		public const string FileName = "EventItemBingoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventItemBingoVisitor.SetABDefault单独设定EventItemBingo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventItemBingoVisitor.SetABFallback单独设定EventItemBingo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventItemBingo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventItemBingo>>(()=> new PBC::MapField<int, rawEventItemBingo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventItemBingo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventItemBingoMap;
			var abMap = v.EventItemBingoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventItemBingoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventItemBingoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventItemBingo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventItemBingoMap;
			if (v.EventItemBingoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventItemBingoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventItemBingoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventItemBingoConf cc) {
			foreach(KeyValuePair <string, rawEventItemBingoMapABValue> tagEntry in cc.EventItemBingoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventItemBingoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventItemBingoMapABValue();
					cc.EventItemBingoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventItemBingoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventItemBingo> entry in cc.EventItemBingoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventItemBingo> entry in tagEntry.Value.EventItemBingoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventItemBingo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventItemBingoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventItemBingoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventItemBingoMapAB != null)
			{
				foreach (var entry in cc.EventItemBingoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventItemBingoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventItemBingoMap.MergeFrom(cis);
			Patch(updating.EventItemBingoMap);
			updating.EventItemBingoSortedKeys = genSortedKeys(updating.EventItemBingoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventItemBingoConf();
            tmp.MergeFrom(cis);
			updating.EventItemBingoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventItemBingoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventItemBingo> filter,string tag=""){
		...
		public static rawEventItemBingo MustGet(int key,string tag="") {
		...
		public static rawEventItemBingo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventItemBingo> GetByFilter(Func<rawEventItemBingo, bool> filterFunc,string tag="") {
			List<rawEventItemBingo> ret = new List<rawEventItemBingo>();
			foreach(KeyValuePair <int, rawEventItemBingo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventItemBingo? GetOneByFilter(Func<rawEventItemBingo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventItemBingo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventItemBingo data) 
		{
			return data.Id;			
		}
    }
