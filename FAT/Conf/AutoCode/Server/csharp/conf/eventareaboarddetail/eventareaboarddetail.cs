...
using EventAreaBoardDetailConf = fat.rawdata.EventAreaBoardDetailConf;
using rawEventAreaBoardDetail = fat.rawdata.EventAreaBoardDetail;
using rawEventAreaBoardDetailMapABValue = fat.rawdata.EventAreaBoardDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaBoardDetailVisitor
    {
		...
		public const string FileName = "EventAreaBoardDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaBoardDetailVisitor.SetABDefault单独设定EventAreaBoardDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaBoardDetailVisitor.SetABFallback单独设定EventAreaBoardDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaBoardDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaBoardDetail>>(()=> new PBC::MapField<int, rawEventAreaBoardDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaBoardDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaBoardDetailMap;
			var abMap = v.EventAreaBoardDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaBoardDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaBoardDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaBoardDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaBoardDetailMap;
			if (v.EventAreaBoardDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaBoardDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaBoardDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaBoardDetailConf cc) {
			foreach(KeyValuePair <string, rawEventAreaBoardDetailMapABValue> tagEntry in cc.EventAreaBoardDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaBoardDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaBoardDetailMapABValue();
					cc.EventAreaBoardDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaBoardDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaBoardDetail> entry in cc.EventAreaBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaBoardDetail> entry in tagEntry.Value.EventAreaBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaBoardDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaBoardDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaBoardDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaBoardDetailMapAB != null)
			{
				foreach (var entry in cc.EventAreaBoardDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaBoardDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaBoardDetailMap.MergeFrom(cis);
			Patch(updating.EventAreaBoardDetailMap);
			updating.EventAreaBoardDetailSortedKeys = genSortedKeys(updating.EventAreaBoardDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaBoardDetailConf();
            tmp.MergeFrom(cis);
			updating.EventAreaBoardDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaBoardDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaBoardDetail> filter,string tag=""){
		...
		public static rawEventAreaBoardDetail MustGet(int key,string tag="") {
		...
		public static rawEventAreaBoardDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaBoardDetail> GetByFilter(Func<rawEventAreaBoardDetail, bool> filterFunc,string tag="") {
			List<rawEventAreaBoardDetail> ret = new List<rawEventAreaBoardDetail>();
			foreach(KeyValuePair <int, rawEventAreaBoardDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaBoardDetail? GetOneByFilter(Func<rawEventAreaBoardDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaBoardDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaBoardDetail data) 
		{
			return data.Id;			
		}
    }
