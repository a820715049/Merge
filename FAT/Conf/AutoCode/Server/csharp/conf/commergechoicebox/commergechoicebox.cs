...
using ComMergeChoiceBoxConf = fat.rawdata.ComMergeChoiceBoxConf;
using rawComMergeChoiceBox = fat.rawdata.ComMergeChoiceBox;
using rawComMergeChoiceBoxMapABValue = fat.rawdata.ComMergeChoiceBoxMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeChoiceBoxVisitor
    {
		...
		public const string FileName = "ComMergeChoiceBoxConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeChoiceBoxVisitor.SetABDefault单独设定ComMergeChoiceBox的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeChoiceBoxVisitor.SetABFallback单独设定ComMergeChoiceBox的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeChoiceBox>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeChoiceBox>>(()=> new PBC::MapField<int, rawComMergeChoiceBox>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeChoiceBox>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeChoiceBoxMap;
			var abMap = v.ComMergeChoiceBoxMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeChoiceBoxMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeChoiceBoxMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeChoiceBox> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeChoiceBoxMap;
			if (v.ComMergeChoiceBoxMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeChoiceBoxMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeChoiceBoxMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeChoiceBoxConf cc) {
			foreach(KeyValuePair <string, rawComMergeChoiceBoxMapABValue> tagEntry in cc.ComMergeChoiceBoxMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeChoiceBoxMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeChoiceBoxMapABValue();
					cc.ComMergeChoiceBoxMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeChoiceBoxMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeChoiceBox> entry in cc.ComMergeChoiceBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeChoiceBox> entry in tagEntry.Value.ComMergeChoiceBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeChoiceBox> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeChoiceBoxConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeChoiceBoxMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeChoiceBoxMapAB != null)
			{
				foreach (var entry in cc.ComMergeChoiceBoxMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeChoiceBoxMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeChoiceBoxMap.MergeFrom(cis);
			Patch(updating.ComMergeChoiceBoxMap);
			updating.ComMergeChoiceBoxSortedKeys = genSortedKeys(updating.ComMergeChoiceBoxMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeChoiceBoxConf();
            tmp.MergeFrom(cis);
			updating.ComMergeChoiceBoxMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeChoiceBoxSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeChoiceBox> filter,string tag=""){
		...
		public static rawComMergeChoiceBox MustGet(int key,string tag="") {
		...
		public static rawComMergeChoiceBox? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeChoiceBox> GetByFilter(Func<rawComMergeChoiceBox, bool> filterFunc,string tag="") {
			List<rawComMergeChoiceBox> ret = new List<rawComMergeChoiceBox>();
			foreach(KeyValuePair <int, rawComMergeChoiceBox> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeChoiceBox? GetOneByFilter(Func<rawComMergeChoiceBox, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeChoiceBox> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeChoiceBox data) 
		{
			return data.Id;			
		}
    }
