...
using ErgListPackConf = fat.rawdata.ErgListPackConf;
using rawErgListPack = fat.rawdata.ErgListPack;
using rawErgListPackMapABValue = fat.rawdata.ErgListPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ErgListPackVisitor
    {
		...
		public const string FileName = "ErgListPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ErgListPackVisitor.SetABDefault单独设定ErgListPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ErgListPackVisitor.SetABFallback单独设定ErgListPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawErgListPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawErgListPack>>(()=> new PBC::MapField<int, rawErgListPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawErgListPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ErgListPackMap;
			var abMap = v.ErgListPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ErgListPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ErgListPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawErgListPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ErgListPackMap;
			if (v.ErgListPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ErgListPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawErgListPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ErgListPackConf cc) {
			foreach(KeyValuePair <string, rawErgListPackMapABValue> tagEntry in cc.ErgListPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ErgListPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawErgListPackMapABValue();
					cc.ErgListPackMapAB[tag] = ps;
				}
                var tagTmp = ps.ErgListPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawErgListPack> entry in cc.ErgListPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawErgListPack> entry in tagEntry.Value.ErgListPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ErgListPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ErgListPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ErgListPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ErgListPackMapAB != null)
			{
				foreach (var entry in cc.ErgListPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ErgListPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ErgListPackMap.MergeFrom(cis);
			Patch(updating.ErgListPackMap);
			updating.ErgListPackSortedKeys = genSortedKeys(updating.ErgListPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ErgListPackConf();
            tmp.MergeFrom(cis);
			updating.ErgListPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ErgListPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawErgListPack> filter,string tag=""){
		...
		public static rawErgListPack MustGet(int key,string tag="") {
		...
		public static rawErgListPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawErgListPack> GetByFilter(Func<rawErgListPack, bool> filterFunc,string tag="") {
			List<rawErgListPack> ret = new List<rawErgListPack>();
			foreach(KeyValuePair <int, rawErgListPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawErgListPack? GetOneByFilter(Func<rawErgListPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawErgListPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawErgListPack data) 
		{
			return data.Id;			
		}
    }
