...
using EventSeaRaceConf = fat.rawdata.EventSeaRaceConf;
using rawEventSeaRace = fat.rawdata.EventSeaRace;
using rawEventSeaRaceMapABValue = fat.rawdata.EventSeaRaceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaRaceVisitor
    {
		...
		public const string FileName = "EventSeaRaceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaRaceVisitor.SetABDefault单独设定EventSeaRace的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaRaceVisitor.SetABFallback单独设定EventSeaRace的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaRace>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaRace>>(()=> new PBC::MapField<int, rawEventSeaRace>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaRace>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaRaceMap;
			var abMap = v.EventSeaRaceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaRaceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaRaceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaRace> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaRaceMap;
			if (v.EventSeaRaceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaRaceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaRaceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaRaceConf cc) {
			foreach(KeyValuePair <string, rawEventSeaRaceMapABValue> tagEntry in cc.EventSeaRaceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaRaceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaRaceMapABValue();
					cc.EventSeaRaceMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaRaceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaRace> entry in cc.EventSeaRaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaRace> entry in tagEntry.Value.EventSeaRaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaRace> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaRaceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaRaceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaRaceMapAB != null)
			{
				foreach (var entry in cc.EventSeaRaceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaRaceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaRaceMap.MergeFrom(cis);
			Patch(updating.EventSeaRaceMap);
			updating.EventSeaRaceSortedKeys = genSortedKeys(updating.EventSeaRaceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaRaceConf();
            tmp.MergeFrom(cis);
			updating.EventSeaRaceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaRaceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaRace> filter,string tag=""){
		...
		public static rawEventSeaRace MustGet(int key,string tag="") {
		...
		public static rawEventSeaRace? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaRace> GetByFilter(Func<rawEventSeaRace, bool> filterFunc,string tag="") {
			List<rawEventSeaRace> ret = new List<rawEventSeaRace>();
			foreach(KeyValuePair <int, rawEventSeaRace> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaRace? GetOneByFilter(Func<rawEventSeaRace, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaRace> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaRace data) 
		{
			return data.Id;			
		}
    }
