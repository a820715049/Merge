...
using EventLimitMergeGroupConf = fat.rawdata.EventLimitMergeGroupConf;
using rawEventLimitMergeGroup = fat.rawdata.EventLimitMergeGroup;
using rawEventLimitMergeGroupMapABValue = fat.rawdata.EventLimitMergeGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventLimitMergeGroupVisitor
    {
		...
		public const string FileName = "EventLimitMergeGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventLimitMergeGroupVisitor.SetABDefault单独设定EventLimitMergeGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventLimitMergeGroupVisitor.SetABFallback单独设定EventLimitMergeGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventLimitMergeGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventLimitMergeGroup>>(()=> new PBC::MapField<int, rawEventLimitMergeGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventLimitMergeGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventLimitMergeGroupMap;
			var abMap = v.EventLimitMergeGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventLimitMergeGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventLimitMergeGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventLimitMergeGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventLimitMergeGroupMap;
			if (v.EventLimitMergeGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventLimitMergeGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventLimitMergeGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventLimitMergeGroupConf cc) {
			foreach(KeyValuePair <string, rawEventLimitMergeGroupMapABValue> tagEntry in cc.EventLimitMergeGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventLimitMergeGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventLimitMergeGroupMapABValue();
					cc.EventLimitMergeGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventLimitMergeGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventLimitMergeGroup> entry in cc.EventLimitMergeGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventLimitMergeGroup> entry in tagEntry.Value.EventLimitMergeGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventLimitMergeGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventLimitMergeGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventLimitMergeGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventLimitMergeGroupMapAB != null)
			{
				foreach (var entry in cc.EventLimitMergeGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventLimitMergeGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventLimitMergeGroupMap.MergeFrom(cis);
			Patch(updating.EventLimitMergeGroupMap);
			updating.EventLimitMergeGroupSortedKeys = genSortedKeys(updating.EventLimitMergeGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventLimitMergeGroupConf();
            tmp.MergeFrom(cis);
			updating.EventLimitMergeGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventLimitMergeGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventLimitMergeGroup> filter,string tag=""){
		...
		public static rawEventLimitMergeGroup MustGet(int key,string tag="") {
		...
		public static rawEventLimitMergeGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventLimitMergeGroup> GetByFilter(Func<rawEventLimitMergeGroup, bool> filterFunc,string tag="") {
			List<rawEventLimitMergeGroup> ret = new List<rawEventLimitMergeGroup>();
			foreach(KeyValuePair <int, rawEventLimitMergeGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventLimitMergeGroup? GetOneByFilter(Func<rawEventLimitMergeGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventLimitMergeGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventLimitMergeGroup data) 
		{
			return data.Id;			
		}
    }
