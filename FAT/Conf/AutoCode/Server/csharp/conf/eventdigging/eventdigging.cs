...
using EventDiggingConf = fat.rawdata.EventDiggingConf;
using rawEventDigging = fat.rawdata.EventDigging;
using rawEventDiggingMapABValue = fat.rawdata.EventDiggingMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingVisitor
    {
		...
		public const string FileName = "EventDiggingConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingVisitor.SetABDefault单独设定EventDigging的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingVisitor.SetABFallback单独设定EventDigging的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDigging>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDigging>>(()=> new PBC::MapField<int, rawEventDigging>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDigging>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingMap;
			var abMap = v.EventDiggingMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDigging> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingMap;
			if (v.EventDiggingMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingMapABValue> tagEntry in cc.EventDiggingMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingMapABValue();
					cc.EventDiggingMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDigging> entry in cc.EventDiggingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDigging> entry in tagEntry.Value.EventDiggingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDigging> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingMapAB != null)
			{
				foreach (var entry in cc.EventDiggingMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingMap.MergeFrom(cis);
			Patch(updating.EventDiggingMap);
			updating.EventDiggingSortedKeys = genSortedKeys(updating.EventDiggingMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDigging> filter,string tag=""){
		...
		public static rawEventDigging MustGet(int key,string tag="") {
		...
		public static rawEventDigging? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDigging> GetByFilter(Func<rawEventDigging, bool> filterFunc,string tag="") {
			List<rawEventDigging> ret = new List<rawEventDigging>();
			foreach(KeyValuePair <int, rawEventDigging> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDigging? GetOneByFilter(Func<rawEventDigging, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDigging> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDigging data) 
		{
			return data.Id;			
		}
    }
