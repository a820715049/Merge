...
using EventAreaBoardRowConf = fat.rawdata.EventAreaBoardRowConf;
using rawEventAreaBoardRow = fat.rawdata.EventAreaBoardRow;
using rawEventAreaBoardRowMapABValue = fat.rawdata.EventAreaBoardRowMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaBoardRowVisitor
    {
		...
		public const string FileName = "EventAreaBoardRowConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaBoardRowVisitor.SetABDefault单独设定EventAreaBoardRow的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaBoardRowVisitor.SetABFallback单独设定EventAreaBoardRow的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaBoardRow>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaBoardRow>>(()=> new PBC::MapField<int, rawEventAreaBoardRow>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaBoardRow>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaBoardRowMap;
			var abMap = v.EventAreaBoardRowMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaBoardRowMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaBoardRowMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaBoardRow> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaBoardRowMap;
			if (v.EventAreaBoardRowMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaBoardRowMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaBoardRowMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaBoardRowConf cc) {
			foreach(KeyValuePair <string, rawEventAreaBoardRowMapABValue> tagEntry in cc.EventAreaBoardRowMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaBoardRowMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaBoardRowMapABValue();
					cc.EventAreaBoardRowMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaBoardRowMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaBoardRow> entry in cc.EventAreaBoardRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaBoardRow> entry in tagEntry.Value.EventAreaBoardRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaBoardRow> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaBoardRowConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaBoardRowMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaBoardRowMapAB != null)
			{
				foreach (var entry in cc.EventAreaBoardRowMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaBoardRowMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaBoardRowMap.MergeFrom(cis);
			Patch(updating.EventAreaBoardRowMap);
			updating.EventAreaBoardRowSortedKeys = genSortedKeys(updating.EventAreaBoardRowMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaBoardRowConf();
            tmp.MergeFrom(cis);
			updating.EventAreaBoardRowMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaBoardRowSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaBoardRow> filter,string tag=""){
		...
		public static rawEventAreaBoardRow MustGet(int key,string tag="") {
		...
		public static rawEventAreaBoardRow? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaBoardRow> GetByFilter(Func<rawEventAreaBoardRow, bool> filterFunc,string tag="") {
			List<rawEventAreaBoardRow> ret = new List<rawEventAreaBoardRow>();
			foreach(KeyValuePair <int, rawEventAreaBoardRow> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaBoardRow? GetOneByFilter(Func<rawEventAreaBoardRow, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaBoardRow> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaBoardRow data) 
		{
			return data.Id;			
		}
    }
