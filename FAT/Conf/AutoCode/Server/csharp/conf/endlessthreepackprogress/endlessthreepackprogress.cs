...
using EndlessThreePackProgressConf = fat.rawdata.EndlessThreePackProgressConf;
using rawEndlessThreePackProgress = fat.rawdata.EndlessThreePackProgress;
using rawEndlessThreePackProgressMapABValue = fat.rawdata.EndlessThreePackProgressMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessThreePackProgressVisitor
    {
		...
		public const string FileName = "EndlessThreePackProgressConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessThreePackProgressVisitor.SetABDefault单独设定EndlessThreePackProgress的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessThreePackProgressVisitor.SetABFallback单独设定EndlessThreePackProgress的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessThreePackProgress>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessThreePackProgress>>(()=> new PBC::MapField<int, rawEndlessThreePackProgress>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessThreePackProgress>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessThreePackProgressMap;
			var abMap = v.EndlessThreePackProgressMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessThreePackProgressMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessThreePackProgressMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessThreePackProgress> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessThreePackProgressMap;
			if (v.EndlessThreePackProgressMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessThreePackProgressMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessThreePackProgressMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessThreePackProgressConf cc) {
			foreach(KeyValuePair <string, rawEndlessThreePackProgressMapABValue> tagEntry in cc.EndlessThreePackProgressMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessThreePackProgressMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessThreePackProgressMapABValue();
					cc.EndlessThreePackProgressMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessThreePackProgressMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessThreePackProgress> entry in cc.EndlessThreePackProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessThreePackProgress> entry in tagEntry.Value.EndlessThreePackProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessThreePackProgress> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessThreePackProgressConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessThreePackProgressMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessThreePackProgressMapAB != null)
			{
				foreach (var entry in cc.EndlessThreePackProgressMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessThreePackProgressMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessThreePackProgressMap.MergeFrom(cis);
			Patch(updating.EndlessThreePackProgressMap);
			updating.EndlessThreePackProgressSortedKeys = genSortedKeys(updating.EndlessThreePackProgressMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessThreePackProgressConf();
            tmp.MergeFrom(cis);
			updating.EndlessThreePackProgressMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessThreePackProgressSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessThreePackProgress> filter,string tag=""){
		...
		public static rawEndlessThreePackProgress MustGet(int key,string tag="") {
		...
		public static rawEndlessThreePackProgress? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessThreePackProgress> GetByFilter(Func<rawEndlessThreePackProgress, bool> filterFunc,string tag="") {
			List<rawEndlessThreePackProgress> ret = new List<rawEndlessThreePackProgress>();
			foreach(KeyValuePair <int, rawEndlessThreePackProgress> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessThreePackProgress? GetOneByFilter(Func<rawEndlessThreePackProgress, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessThreePackProgress> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessThreePackProgress data) 
		{
			return data.Id;			
		}
    }
