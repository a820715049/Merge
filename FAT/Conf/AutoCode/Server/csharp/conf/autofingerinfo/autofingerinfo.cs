...
using AutoFingerInfoConf = fat.rawdata.AutoFingerInfoConf;
using rawAutoFingerInfo = fat.rawdata.AutoFingerInfo;
using rawAutoFingerInfoSliceABValue = fat.rawdata.AutoFingerInfoSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class AutoFingerInfoVisitor
    {
		...
		public const string FileName = "AutoFingerInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过AutoFingerInfoVisitor.SetABDefault单独设定AutoFingerInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过AutoFingerInfoVisitor.SetABFallback单独设定AutoFingerInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawAutoFingerInfo>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawAutoFingerInfo>>(()=>new PBC::RepeatedField<rawAutoFingerInfo>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawAutoFingerInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().AutoFingerInfoSlice;
			var abMap = v.AutoFingerInfoSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.AutoFingerInfoSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].AutoFingerInfoSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawAutoFingerInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().AutoFingerInfoSlice;
			if (v.AutoFingerInfoSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.AutoFingerInfoSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawAutoFingerInfoSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(AutoFingerInfoConf cc) {
			foreach(KeyValuePair <string, rawAutoFingerInfoSliceABValue> tagEntry in cc.AutoFingerInfoSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.AutoFingerInfoSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawAutoFingerInfoSliceABValue();
					cc.AutoFingerInfoSliceAB[tag] = ps;
				}
                var tagTmp = ps.AutoFingerInfoSlice;
				// 基础数据
				for(int i=0; i<cc.AutoFingerInfoSlice.Count; ++i) 
				{
					tagTmp.Add(cc.AutoFingerInfoSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.AutoFingerInfoSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.AutoFingerInfo> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.AutoFingerInfoSlice.MergeFrom(cis);
			Patch(updating.AutoFingerInfoSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new AutoFingerInfoConf();
            tmp.MergeFrom(cis);
			updating.AutoFingerInfoSlice = tmp;
		...
		public static void Range(Action<int, rawAutoFingerInfo> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawAutoFingerInfo MustGetByIndex(int index,string tag="") {
		...
		public static rawAutoFingerInfo? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawAutoFingerInfo> GetByFilter(Func<rawAutoFingerInfo,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawAutoFingerInfo>();
		...
		static public rawAutoFingerInfo? GetOneByFilter(Func<rawAutoFingerInfo,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
