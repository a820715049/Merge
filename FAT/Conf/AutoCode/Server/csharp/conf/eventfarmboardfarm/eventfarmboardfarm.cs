...
using EventFarmBoardFarmConf = fat.rawdata.EventFarmBoardFarmConf;
using rawEventFarmBoardFarm = fat.rawdata.EventFarmBoardFarm;
using rawEventFarmBoardFarmMapABValue = fat.rawdata.EventFarmBoardFarmMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmBoardFarmVisitor
    {
		...
		public const string FileName = "EventFarmBoardFarmConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmBoardFarmVisitor.SetABDefault单独设定EventFarmBoardFarm的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmBoardFarmVisitor.SetABFallback单独设定EventFarmBoardFarm的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmBoardFarm>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmBoardFarm>>(()=> new PBC::MapField<int, rawEventFarmBoardFarm>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmBoardFarm>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmBoardFarmMap;
			var abMap = v.EventFarmBoardFarmMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmBoardFarmMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmBoardFarmMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmBoardFarm> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmBoardFarmMap;
			if (v.EventFarmBoardFarmMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmBoardFarmMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmBoardFarmMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmBoardFarmConf cc) {
			foreach(KeyValuePair <string, rawEventFarmBoardFarmMapABValue> tagEntry in cc.EventFarmBoardFarmMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmBoardFarmMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmBoardFarmMapABValue();
					cc.EventFarmBoardFarmMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmBoardFarmMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmBoardFarm> entry in cc.EventFarmBoardFarmMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmBoardFarm> entry in tagEntry.Value.EventFarmBoardFarmMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmBoardFarm> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmBoardFarmConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmBoardFarmMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmBoardFarmMapAB != null)
			{
				foreach (var entry in cc.EventFarmBoardFarmMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmBoardFarmMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmBoardFarmMap.MergeFrom(cis);
			Patch(updating.EventFarmBoardFarmMap);
			updating.EventFarmBoardFarmSortedKeys = genSortedKeys(updating.EventFarmBoardFarmMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmBoardFarmConf();
            tmp.MergeFrom(cis);
			updating.EventFarmBoardFarmMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmBoardFarmSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmBoardFarm> filter,string tag=""){
		...
		public static rawEventFarmBoardFarm MustGet(int key,string tag="") {
		...
		public static rawEventFarmBoardFarm? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmBoardFarm> GetByFilter(Func<rawEventFarmBoardFarm, bool> filterFunc,string tag="") {
			List<rawEventFarmBoardFarm> ret = new List<rawEventFarmBoardFarm>();
			foreach(KeyValuePair <int, rawEventFarmBoardFarm> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmBoardFarm? GetOneByFilter(Func<rawEventFarmBoardFarm, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmBoardFarm> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmBoardFarm data) 
		{
			return data.Id;			
		}
    }
