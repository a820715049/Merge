...
using ExchangeCardWhitelistConf = fat.rawdata.ExchangeCardWhitelistConf;
using rawExchangeCardWhitelist = fat.rawdata.ExchangeCardWhitelist;
using rawExchangeCardWhitelistMapABValue = fat.rawdata.ExchangeCardWhitelistMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ExchangeCardWhitelistVisitor
    {
		...
		public const string FileName = "ExchangeCardWhitelistConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ExchangeCardWhitelistVisitor.SetABDefault单独设定ExchangeCardWhitelist的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ExchangeCardWhitelistVisitor.SetABFallback单独设定ExchangeCardWhitelist的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawExchangeCardWhitelist>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawExchangeCardWhitelist>>(()=> new PBC::MapField<string, rawExchangeCardWhitelist>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawExchangeCardWhitelist>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ExchangeCardWhitelistMap;
			var abMap = v.ExchangeCardWhitelistMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ExchangeCardWhitelistMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ExchangeCardWhitelistMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawExchangeCardWhitelist> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ExchangeCardWhitelistMap;
			if (v.ExchangeCardWhitelistMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ExchangeCardWhitelistMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawExchangeCardWhitelistMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ExchangeCardWhitelistConf cc) {
			foreach(KeyValuePair <string, rawExchangeCardWhitelistMapABValue> tagEntry in cc.ExchangeCardWhitelistMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ExchangeCardWhitelistMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawExchangeCardWhitelistMapABValue();
					cc.ExchangeCardWhitelistMapAB[tag] = ps;
				}
                var tagTmp = ps.ExchangeCardWhitelistMap;
				// 基础数据
				foreach(KeyValuePair <string, rawExchangeCardWhitelist> entry in cc.ExchangeCardWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawExchangeCardWhitelist> entry in tagEntry.Value.ExchangeCardWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.ExchangeCardWhitelist> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(ExchangeCardWhitelistConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.ExchangeCardWhitelistMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ExchangeCardWhitelistMapAB != null)
			{
				foreach (var entry in cc.ExchangeCardWhitelistMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.ExchangeCardWhitelistMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ExchangeCardWhitelistMap.MergeFrom(cis);
			Patch(updating.ExchangeCardWhitelistMap);
			updating.ExchangeCardWhitelistSortedKeys = genSortedKeys(updating.ExchangeCardWhitelistMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ExchangeCardWhitelistConf();
            tmp.MergeFrom(cis);
			updating.ExchangeCardWhitelistMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ExchangeCardWhitelistSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawExchangeCardWhitelist> filter,string tag=""){
		...
		public static rawExchangeCardWhitelist MustGet(string key,string tag="") {
		...
		public static rawExchangeCardWhitelist? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawExchangeCardWhitelist> GetByFilter(Func<rawExchangeCardWhitelist, bool> filterFunc,string tag="") {
			List<rawExchangeCardWhitelist> ret = new List<rawExchangeCardWhitelist>();
			foreach(KeyValuePair <string, rawExchangeCardWhitelist> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawExchangeCardWhitelist? GetOneByFilter(Func<rawExchangeCardWhitelist, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawExchangeCardWhitelist> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawExchangeCardWhitelist data) 
		{
			return data.Id;			
		}
    }
