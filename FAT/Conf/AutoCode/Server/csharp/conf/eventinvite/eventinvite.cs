...
using EventInviteConf = fat.rawdata.EventInviteConf;
using rawEventInvite = fat.rawdata.EventInvite;
using rawEventInviteMapABValue = fat.rawdata.EventInviteMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventInviteVisitor
    {
		...
		public const string FileName = "EventInviteConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventInviteVisitor.SetABDefault单独设定EventInvite的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventInviteVisitor.SetABFallback单独设定EventInvite的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventInvite>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventInvite>>(()=> new PBC::MapField<int, rawEventInvite>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventInvite>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventInviteMap;
			var abMap = v.EventInviteMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventInviteMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventInviteMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventInvite> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventInviteMap;
			if (v.EventInviteMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventInviteMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventInviteMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventInviteConf cc) {
			foreach(KeyValuePair <string, rawEventInviteMapABValue> tagEntry in cc.EventInviteMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventInviteMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventInviteMapABValue();
					cc.EventInviteMapAB[tag] = ps;
				}
                var tagTmp = ps.EventInviteMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventInvite> entry in cc.EventInviteMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventInvite> entry in tagEntry.Value.EventInviteMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventInvite> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventInviteConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventInviteMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventInviteMapAB != null)
			{
				foreach (var entry in cc.EventInviteMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventInviteMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventInviteMap.MergeFrom(cis);
			Patch(updating.EventInviteMap);
			updating.EventInviteSortedKeys = genSortedKeys(updating.EventInviteMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventInviteConf();
            tmp.MergeFrom(cis);
			updating.EventInviteMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventInviteSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventInvite> filter,string tag=""){
		...
		public static rawEventInvite MustGet(int key,string tag="") {
		...
		public static rawEventInvite? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventInvite> GetByFilter(Func<rawEventInvite, bool> filterFunc,string tag="") {
			List<rawEventInvite> ret = new List<rawEventInvite>();
			foreach(KeyValuePair <int, rawEventInvite> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventInvite? GetOneByFilter(Func<rawEventInvite, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventInvite> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventInvite data) 
		{
			return data.Id;			
		}
    }
