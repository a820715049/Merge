...
using ReshipmentConf = fat.rawdata.ReshipmentConf;
using rawReshipment = fat.rawdata.Reshipment;
using rawReshipmentMapABValue = fat.rawdata.ReshipmentMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ReshipmentVisitor
    {
		...
		public const string FileName = "ReshipmentConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ReshipmentVisitor.SetABDefault单独设定Reshipment的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ReshipmentVisitor.SetABFallback单独设定Reshipment的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawReshipment>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawReshipment>>(()=> new PBC::MapField<int, rawReshipment>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawReshipment>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ReshipmentMap;
			var abMap = v.ReshipmentMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ReshipmentMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ReshipmentMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawReshipment> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ReshipmentMap;
			if (v.ReshipmentMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ReshipmentMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawReshipmentMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ReshipmentConf cc) {
			foreach(KeyValuePair <string, rawReshipmentMapABValue> tagEntry in cc.ReshipmentMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ReshipmentMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawReshipmentMapABValue();
					cc.ReshipmentMapAB[tag] = ps;
				}
                var tagTmp = ps.ReshipmentMap;
				// 基础数据
				foreach(KeyValuePair <int, rawReshipment> entry in cc.ReshipmentMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawReshipment> entry in tagEntry.Value.ReshipmentMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Reshipment> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ReshipmentConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ReshipmentMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ReshipmentMapAB != null)
			{
				foreach (var entry in cc.ReshipmentMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ReshipmentMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ReshipmentMap.MergeFrom(cis);
			Patch(updating.ReshipmentMap);
			updating.ReshipmentSortedKeys = genSortedKeys(updating.ReshipmentMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ReshipmentConf();
            tmp.MergeFrom(cis);
			updating.ReshipmentMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ReshipmentSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawReshipment> filter,string tag=""){
		...
		public static rawReshipment MustGet(int key,string tag="") {
		...
		public static rawReshipment? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawReshipment> GetByFilter(Func<rawReshipment, bool> filterFunc,string tag="") {
			List<rawReshipment> ret = new List<rawReshipment>();
			foreach(KeyValuePair <int, rawReshipment> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawReshipment? GetOneByFilter(Func<rawReshipment, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawReshipment> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawReshipment data) 
		{
			return data.Id;			
		}
    }
