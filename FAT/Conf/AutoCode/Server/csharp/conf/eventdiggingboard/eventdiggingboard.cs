...
using EventDiggingBoardConf = fat.rawdata.EventDiggingBoardConf;
using rawEventDiggingBoard = fat.rawdata.EventDiggingBoard;
using rawEventDiggingBoardMapABValue = fat.rawdata.EventDiggingBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingBoardVisitor
    {
		...
		public const string FileName = "EventDiggingBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingBoardVisitor.SetABDefault单独设定EventDiggingBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingBoardVisitor.SetABFallback单独设定EventDiggingBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDiggingBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDiggingBoard>>(()=> new PBC::MapField<int, rawEventDiggingBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDiggingBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingBoardMap;
			var abMap = v.EventDiggingBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDiggingBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingBoardMap;
			if (v.EventDiggingBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingBoardConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingBoardMapABValue> tagEntry in cc.EventDiggingBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingBoardMapABValue();
					cc.EventDiggingBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDiggingBoard> entry in cc.EventDiggingBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDiggingBoard> entry in tagEntry.Value.EventDiggingBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDiggingBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingBoardMapAB != null)
			{
				foreach (var entry in cc.EventDiggingBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingBoardMap.MergeFrom(cis);
			Patch(updating.EventDiggingBoardMap);
			updating.EventDiggingBoardSortedKeys = genSortedKeys(updating.EventDiggingBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingBoardConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDiggingBoard> filter,string tag=""){
		...
		public static rawEventDiggingBoard MustGet(int key,string tag="") {
		...
		public static rawEventDiggingBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDiggingBoard> GetByFilter(Func<rawEventDiggingBoard, bool> filterFunc,string tag="") {
			List<rawEventDiggingBoard> ret = new List<rawEventDiggingBoard>();
			foreach(KeyValuePair <int, rawEventDiggingBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDiggingBoard? GetOneByFilter(Func<rawEventDiggingBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDiggingBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDiggingBoard data) 
		{
			return data.Id;			
		}
    }
