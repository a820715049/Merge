...
using LanguageConf = fat.rawdata.LanguageConf;
using rawLanguage = fat.rawdata.Language;
using rawLanguageSliceABValue = fat.rawdata.LanguageSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LanguageVisitor
    {
		...
		public const string FileName = "LanguageConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LanguageVisitor.SetABDefault单独设定Language的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LanguageVisitor.SetABFallback单独设定Language的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLanguage>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLanguage>>(()=>new PBC::RepeatedField<rawLanguage>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLanguage>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LanguageSlice;
			var abMap = v.LanguageSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LanguageSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LanguageSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLanguage> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LanguageSlice;
			if (v.LanguageSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LanguageSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLanguageSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LanguageConf cc) {
			foreach(KeyValuePair <string, rawLanguageSliceABValue> tagEntry in cc.LanguageSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LanguageSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLanguageSliceABValue();
					cc.LanguageSliceAB[tag] = ps;
				}
                var tagTmp = ps.LanguageSlice;
				// 基础数据
				for(int i=0; i<cc.LanguageSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LanguageSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LanguageSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.Language> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LanguageSlice.MergeFrom(cis);
			Patch(updating.LanguageSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LanguageConf();
            tmp.MergeFrom(cis);
			updating.LanguageSlice = tmp;
		...
		public static void Range(Action<int, rawLanguage> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLanguage MustGetByIndex(int index,string tag="") {
		...
		public static rawLanguage? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLanguage> GetByFilter(Func<rawLanguage,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLanguage>();
		...
		static public rawLanguage? GetOneByFilter(Func<rawLanguage,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
