...
using LabelConf = fat.rawdata.LabelConf;
using rawLabel = fat.rawdata.Label;
using rawLabelMapABValue = fat.rawdata.LabelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LabelVisitor
    {
		...
		public const string FileName = "LabelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LabelVisitor.SetABDefault单独设定Label的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LabelVisitor.SetABFallback单独设定Label的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLabel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLabel>>(()=> new PBC::MapField<int, rawLabel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLabel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LabelMap;
			var abMap = v.LabelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LabelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LabelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLabel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LabelMap;
			if (v.LabelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LabelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLabelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LabelConf cc) {
			foreach(KeyValuePair <string, rawLabelMapABValue> tagEntry in cc.LabelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LabelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLabelMapABValue();
					cc.LabelMapAB[tag] = ps;
				}
                var tagTmp = ps.LabelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLabel> entry in cc.LabelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLabel> entry in tagEntry.Value.LabelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Label> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LabelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LabelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LabelMapAB != null)
			{
				foreach (var entry in cc.LabelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LabelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LabelMap.MergeFrom(cis);
			Patch(updating.LabelMap);
			updating.LabelSortedKeys = genSortedKeys(updating.LabelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LabelConf();
            tmp.MergeFrom(cis);
			updating.LabelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LabelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLabel> filter,string tag=""){
		...
		public static rawLabel MustGet(int key,string tag="") {
		...
		public static rawLabel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLabel> GetByFilter(Func<rawLabel, bool> filterFunc,string tag="") {
			List<rawLabel> ret = new List<rawLabel>();
			foreach(KeyValuePair <int, rawLabel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLabel? GetOneByFilter(Func<rawLabel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLabel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLabel data) 
		{
			return data.Id;			
		}
    }
