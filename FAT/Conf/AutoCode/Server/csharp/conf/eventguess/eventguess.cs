...
using EventGuessConf = fat.rawdata.EventGuessConf;
using rawEventGuess = fat.rawdata.EventGuess;
using rawEventGuessMapABValue = fat.rawdata.EventGuessMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventGuessVisitor
    {
		...
		public const string FileName = "EventGuessConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventGuessVisitor.SetABDefault单独设定EventGuess的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventGuessVisitor.SetABFallback单独设定EventGuess的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventGuess>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventGuess>>(()=> new PBC::MapField<int, rawEventGuess>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventGuess>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventGuessMap;
			var abMap = v.EventGuessMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventGuessMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventGuessMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventGuess> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventGuessMap;
			if (v.EventGuessMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventGuessMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventGuessMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventGuessConf cc) {
			foreach(KeyValuePair <string, rawEventGuessMapABValue> tagEntry in cc.EventGuessMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventGuessMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventGuessMapABValue();
					cc.EventGuessMapAB[tag] = ps;
				}
                var tagTmp = ps.EventGuessMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventGuess> entry in cc.EventGuessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventGuess> entry in tagEntry.Value.EventGuessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventGuess> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventGuessConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventGuessMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventGuessMapAB != null)
			{
				foreach (var entry in cc.EventGuessMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventGuessMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventGuessMap.MergeFrom(cis);
			Patch(updating.EventGuessMap);
			updating.EventGuessSortedKeys = genSortedKeys(updating.EventGuessMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventGuessConf();
            tmp.MergeFrom(cis);
			updating.EventGuessMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventGuessSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventGuess> filter,string tag=""){
		...
		public static rawEventGuess MustGet(int key,string tag="") {
		...
		public static rawEventGuess? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventGuess> GetByFilter(Func<rawEventGuess, bool> filterFunc,string tag="") {
			List<rawEventGuess> ret = new List<rawEventGuess>();
			foreach(KeyValuePair <int, rawEventGuess> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventGuess? GetOneByFilter(Func<rawEventGuess, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventGuess> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventGuess data) 
		{
			return data.Id;			
		}
    }
