...
using GuideMergeActionConf = fat.rawdata.GuideMergeActionConf;
using rawGuideMergeAction = fat.rawdata.GuideMergeAction;
using rawGuideMergeActionMapABValue = fat.rawdata.GuideMergeActionMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GuideMergeActionVisitor
    {
		...
		public const string FileName = "GuideMergeActionConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GuideMergeActionVisitor.SetABDefault单独设定GuideMergeAction的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GuideMergeActionVisitor.SetABFallback单独设定GuideMergeAction的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGuideMergeAction>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGuideMergeAction>>(()=> new PBC::MapField<int, rawGuideMergeAction>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGuideMergeAction>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GuideMergeActionMap;
			var abMap = v.GuideMergeActionMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GuideMergeActionMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GuideMergeActionMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGuideMergeAction> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GuideMergeActionMap;
			if (v.GuideMergeActionMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GuideMergeActionMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGuideMergeActionMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GuideMergeActionConf cc) {
			foreach(KeyValuePair <string, rawGuideMergeActionMapABValue> tagEntry in cc.GuideMergeActionMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GuideMergeActionMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGuideMergeActionMapABValue();
					cc.GuideMergeActionMapAB[tag] = ps;
				}
                var tagTmp = ps.GuideMergeActionMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGuideMergeAction> entry in cc.GuideMergeActionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGuideMergeAction> entry in tagEntry.Value.GuideMergeActionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GuideMergeAction> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GuideMergeActionConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GuideMergeActionMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GuideMergeActionMapAB != null)
			{
				foreach (var entry in cc.GuideMergeActionMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GuideMergeActionMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GuideMergeActionMap.MergeFrom(cis);
			Patch(updating.GuideMergeActionMap);
			updating.GuideMergeActionSortedKeys = genSortedKeys(updating.GuideMergeActionMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GuideMergeActionConf();
            tmp.MergeFrom(cis);
			updating.GuideMergeActionMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GuideMergeActionSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGuideMergeAction> filter,string tag=""){
		...
		public static rawGuideMergeAction MustGet(int key,string tag="") {
		...
		public static rawGuideMergeAction? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGuideMergeAction> GetByFilter(Func<rawGuideMergeAction, bool> filterFunc,string tag="") {
			List<rawGuideMergeAction> ret = new List<rawGuideMergeAction>();
			foreach(KeyValuePair <int, rawGuideMergeAction> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGuideMergeAction? GetOneByFilter(Func<rawGuideMergeAction, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGuideMergeAction> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGuideMergeAction data) 
		{
			return data.Id;			
		}
    }
