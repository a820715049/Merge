...
using EventTrainMissionDetailConf = fat.rawdata.EventTrainMissionDetailConf;
using rawEventTrainMissionDetail = fat.rawdata.EventTrainMissionDetail;
using rawEventTrainMissionDetailMapABValue = fat.rawdata.EventTrainMissionDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTrainMissionDetailVisitor
    {
		...
		public const string FileName = "EventTrainMissionDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTrainMissionDetailVisitor.SetABDefault单独设定EventTrainMissionDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTrainMissionDetailVisitor.SetABFallback单独设定EventTrainMissionDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTrainMissionDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTrainMissionDetail>>(()=> new PBC::MapField<int, rawEventTrainMissionDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTrainMissionDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTrainMissionDetailMap;
			var abMap = v.EventTrainMissionDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTrainMissionDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTrainMissionDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTrainMissionDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTrainMissionDetailMap;
			if (v.EventTrainMissionDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTrainMissionDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTrainMissionDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTrainMissionDetailConf cc) {
			foreach(KeyValuePair <string, rawEventTrainMissionDetailMapABValue> tagEntry in cc.EventTrainMissionDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTrainMissionDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTrainMissionDetailMapABValue();
					cc.EventTrainMissionDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTrainMissionDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTrainMissionDetail> entry in cc.EventTrainMissionDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTrainMissionDetail> entry in tagEntry.Value.EventTrainMissionDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTrainMissionDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTrainMissionDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTrainMissionDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTrainMissionDetailMapAB != null)
			{
				foreach (var entry in cc.EventTrainMissionDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTrainMissionDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTrainMissionDetailMap.MergeFrom(cis);
			Patch(updating.EventTrainMissionDetailMap);
			updating.EventTrainMissionDetailSortedKeys = genSortedKeys(updating.EventTrainMissionDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTrainMissionDetailConf();
            tmp.MergeFrom(cis);
			updating.EventTrainMissionDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTrainMissionDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTrainMissionDetail> filter,string tag=""){
		...
		public static rawEventTrainMissionDetail MustGet(int key,string tag="") {
		...
		public static rawEventTrainMissionDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTrainMissionDetail> GetByFilter(Func<rawEventTrainMissionDetail, bool> filterFunc,string tag="") {
			List<rawEventTrainMissionDetail> ret = new List<rawEventTrainMissionDetail>();
			foreach(KeyValuePair <int, rawEventTrainMissionDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTrainMissionDetail? GetOneByFilter(Func<rawEventTrainMissionDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTrainMissionDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTrainMissionDetail data) 
		{
			return data.Id;			
		}
    }
