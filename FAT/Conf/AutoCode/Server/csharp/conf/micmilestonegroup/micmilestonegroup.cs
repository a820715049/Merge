...
using MicMilestoneGroupConf = fat.rawdata.MicMilestoneGroupConf;
using rawMicMilestoneGroup = fat.rawdata.MicMilestoneGroup;
using rawMicMilestoneGroupMapABValue = fat.rawdata.MicMilestoneGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MicMilestoneGroupVisitor
    {
		...
		public const string FileName = "MicMilestoneGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MicMilestoneGroupVisitor.SetABDefault单独设定MicMilestoneGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MicMilestoneGroupVisitor.SetABFallback单独设定MicMilestoneGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMicMilestoneGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMicMilestoneGroup>>(()=> new PBC::MapField<int, rawMicMilestoneGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMicMilestoneGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MicMilestoneGroupMap;
			var abMap = v.MicMilestoneGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MicMilestoneGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MicMilestoneGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMicMilestoneGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MicMilestoneGroupMap;
			if (v.MicMilestoneGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MicMilestoneGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMicMilestoneGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MicMilestoneGroupConf cc) {
			foreach(KeyValuePair <string, rawMicMilestoneGroupMapABValue> tagEntry in cc.MicMilestoneGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MicMilestoneGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMicMilestoneGroupMapABValue();
					cc.MicMilestoneGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.MicMilestoneGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMicMilestoneGroup> entry in cc.MicMilestoneGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMicMilestoneGroup> entry in tagEntry.Value.MicMilestoneGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MicMilestoneGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MicMilestoneGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MicMilestoneGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MicMilestoneGroupMapAB != null)
			{
				foreach (var entry in cc.MicMilestoneGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MicMilestoneGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MicMilestoneGroupMap.MergeFrom(cis);
			Patch(updating.MicMilestoneGroupMap);
			updating.MicMilestoneGroupSortedKeys = genSortedKeys(updating.MicMilestoneGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MicMilestoneGroupConf();
            tmp.MergeFrom(cis);
			updating.MicMilestoneGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MicMilestoneGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMicMilestoneGroup> filter,string tag=""){
		...
		public static rawMicMilestoneGroup MustGet(int key,string tag="") {
		...
		public static rawMicMilestoneGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMicMilestoneGroup> GetByFilter(Func<rawMicMilestoneGroup, bool> filterFunc,string tag="") {
			List<rawMicMilestoneGroup> ret = new List<rawMicMilestoneGroup>();
			foreach(KeyValuePair <int, rawMicMilestoneGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMicMilestoneGroup? GetOneByFilter(Func<rawMicMilestoneGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMicMilestoneGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMicMilestoneGroup data) 
		{
			return data.Id;			
		}
    }
