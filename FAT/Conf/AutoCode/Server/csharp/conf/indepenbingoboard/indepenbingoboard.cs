...
using IndepenBingoBoardConf = fat.rawdata.IndepenBingoBoardConf;
using rawIndepenBingoBoard = fat.rawdata.IndepenBingoBoard;
using rawIndepenBingoBoardMapABValue = fat.rawdata.IndepenBingoBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IndepenBingoBoardVisitor
    {
		...
		public const string FileName = "IndepenBingoBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IndepenBingoBoardVisitor.SetABDefault单独设定IndepenBingoBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IndepenBingoBoardVisitor.SetABFallback单独设定IndepenBingoBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIndepenBingoBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIndepenBingoBoard>>(()=> new PBC::MapField<int, rawIndepenBingoBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIndepenBingoBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IndepenBingoBoardMap;
			var abMap = v.IndepenBingoBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IndepenBingoBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IndepenBingoBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIndepenBingoBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IndepenBingoBoardMap;
			if (v.IndepenBingoBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IndepenBingoBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIndepenBingoBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IndepenBingoBoardConf cc) {
			foreach(KeyValuePair <string, rawIndepenBingoBoardMapABValue> tagEntry in cc.IndepenBingoBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IndepenBingoBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIndepenBingoBoardMapABValue();
					cc.IndepenBingoBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.IndepenBingoBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIndepenBingoBoard> entry in cc.IndepenBingoBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIndepenBingoBoard> entry in tagEntry.Value.IndepenBingoBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IndepenBingoBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IndepenBingoBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IndepenBingoBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IndepenBingoBoardMapAB != null)
			{
				foreach (var entry in cc.IndepenBingoBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IndepenBingoBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IndepenBingoBoardMap.MergeFrom(cis);
			Patch(updating.IndepenBingoBoardMap);
			updating.IndepenBingoBoardSortedKeys = genSortedKeys(updating.IndepenBingoBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IndepenBingoBoardConf();
            tmp.MergeFrom(cis);
			updating.IndepenBingoBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IndepenBingoBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIndepenBingoBoard> filter,string tag=""){
		...
		public static rawIndepenBingoBoard MustGet(int key,string tag="") {
		...
		public static rawIndepenBingoBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIndepenBingoBoard> GetByFilter(Func<rawIndepenBingoBoard, bool> filterFunc,string tag="") {
			List<rawIndepenBingoBoard> ret = new List<rawIndepenBingoBoard>();
			foreach(KeyValuePair <int, rawIndepenBingoBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIndepenBingoBoard? GetOneByFilter(Func<rawIndepenBingoBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIndepenBingoBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIndepenBingoBoard data) 
		{
			return data.Id;			
		}
    }
