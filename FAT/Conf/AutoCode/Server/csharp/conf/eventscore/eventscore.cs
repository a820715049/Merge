...
using EventScoreConf = fat.rawdata.EventScoreConf;
using rawEventScore = fat.rawdata.EventScore;
using rawEventScoreMapABValue = fat.rawdata.EventScoreMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventScoreVisitor
    {
		...
		public const string FileName = "EventScoreConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventScoreVisitor.SetABDefault单独设定EventScore的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventScoreVisitor.SetABFallback单独设定EventScore的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventScore>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventScore>>(()=> new PBC::MapField<int, rawEventScore>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventScore>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventScoreMap;
			var abMap = v.EventScoreMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventScoreMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventScoreMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventScore> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventScoreMap;
			if (v.EventScoreMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventScoreMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventScoreMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventScoreConf cc) {
			foreach(KeyValuePair <string, rawEventScoreMapABValue> tagEntry in cc.EventScoreMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventScoreMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventScoreMapABValue();
					cc.EventScoreMapAB[tag] = ps;
				}
                var tagTmp = ps.EventScoreMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventScore> entry in cc.EventScoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventScore> entry in tagEntry.Value.EventScoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventScore> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventScoreConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventScoreMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventScoreMapAB != null)
			{
				foreach (var entry in cc.EventScoreMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventScoreMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventScoreMap.MergeFrom(cis);
			Patch(updating.EventScoreMap);
			updating.EventScoreSortedKeys = genSortedKeys(updating.EventScoreMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventScoreConf();
            tmp.MergeFrom(cis);
			updating.EventScoreMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventScoreSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventScore> filter,string tag=""){
		...
		public static rawEventScore MustGet(int key,string tag="") {
		...
		public static rawEventScore? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventScore> GetByFilter(Func<rawEventScore, bool> filterFunc,string tag="") {
			List<rawEventScore> ret = new List<rawEventScore>();
			foreach(KeyValuePair <int, rawEventScore> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventScore? GetOneByFilter(Func<rawEventScore, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventScore> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventScore data) 
		{
			return data.Id;			
		}
    }
