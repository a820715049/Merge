...
using MergeRuleConf = fat.rawdata.MergeRuleConf;
using rawMergeRule = fat.rawdata.MergeRule;
using rawMergeRuleMapABValue = fat.rawdata.MergeRuleMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeRuleVisitor
    {
		...
		public const string FileName = "MergeRuleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeRuleVisitor.SetABDefault单独设定MergeRule的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeRuleVisitor.SetABFallback单独设定MergeRule的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeRule>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeRule>>(()=> new PBC::MapField<int, rawMergeRule>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeRule>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeRuleMap;
			var abMap = v.MergeRuleMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeRuleMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeRuleMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeRule> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeRuleMap;
			if (v.MergeRuleMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeRuleMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeRuleMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeRuleConf cc) {
			foreach(KeyValuePair <string, rawMergeRuleMapABValue> tagEntry in cc.MergeRuleMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeRuleMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeRuleMapABValue();
					cc.MergeRuleMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeRuleMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeRule> entry in cc.MergeRuleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeRule> entry in tagEntry.Value.MergeRuleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeRule> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeRuleConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeRuleMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeRuleMapAB != null)
			{
				foreach (var entry in cc.MergeRuleMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeRuleMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeRuleMap.MergeFrom(cis);
			Patch(updating.MergeRuleMap);
			updating.MergeRuleSortedKeys = genSortedKeys(updating.MergeRuleMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeRuleConf();
            tmp.MergeFrom(cis);
			updating.MergeRuleMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeRuleSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeRule> filter,string tag=""){
		...
		public static rawMergeRule MustGet(int key,string tag="") {
		...
		public static rawMergeRule? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeRule> GetByFilter(Func<rawMergeRule, bool> filterFunc,string tag="") {
			List<rawMergeRule> ret = new List<rawMergeRule>();
			foreach(KeyValuePair <int, rawMergeRule> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeRule? GetOneByFilter(Func<rawMergeRule, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeRule> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeRule data) 
		{
			return data.Id;			
		}
    }
