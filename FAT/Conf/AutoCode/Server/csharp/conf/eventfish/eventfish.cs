...
using EventFishConf = fat.rawdata.EventFishConf;
using rawEventFish = fat.rawdata.EventFish;
using rawEventFishMapABValue = fat.rawdata.EventFishMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFishVisitor
    {
		...
		public const string FileName = "EventFishConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFishVisitor.SetABDefault单独设定EventFish的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFishVisitor.SetABFallback单独设定EventFish的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFish>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFish>>(()=> new PBC::MapField<int, rawEventFish>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFish>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFishMap;
			var abMap = v.EventFishMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFishMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFishMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFish> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFishMap;
			if (v.EventFishMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFishMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFishMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFishConf cc) {
			foreach(KeyValuePair <string, rawEventFishMapABValue> tagEntry in cc.EventFishMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFishMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFishMapABValue();
					cc.EventFishMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFishMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFish> entry in cc.EventFishMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFish> entry in tagEntry.Value.EventFishMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFish> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFishConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFishMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFishMapAB != null)
			{
				foreach (var entry in cc.EventFishMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFishMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFishMap.MergeFrom(cis);
			Patch(updating.EventFishMap);
			updating.EventFishSortedKeys = genSortedKeys(updating.EventFishMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFishConf();
            tmp.MergeFrom(cis);
			updating.EventFishMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFishSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFish> filter,string tag=""){
		...
		public static rawEventFish MustGet(int key,string tag="") {
		...
		public static rawEventFish? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFish> GetByFilter(Func<rawEventFish, bool> filterFunc,string tag="") {
			List<rawEventFish> ret = new List<rawEventFish>();
			foreach(KeyValuePair <int, rawEventFish> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFish? GetOneByFilter(Func<rawEventFish, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFish> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFish data) 
		{
			return data.Id;			
		}
    }
