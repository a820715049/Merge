...
using EventMysteriousConf = fat.rawdata.EventMysteriousConf;
using rawEventMysterious = fat.rawdata.EventMysterious;
using rawEventMysteriousMapABValue = fat.rawdata.EventMysteriousMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMysteriousVisitor
    {
		...
		public const string FileName = "EventMysteriousConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMysteriousVisitor.SetABDefault单独设定EventMysterious的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMysteriousVisitor.SetABFallback单独设定EventMysterious的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMysterious>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMysterious>>(()=> new PBC::MapField<int, rawEventMysterious>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMysterious>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMysteriousMap;
			var abMap = v.EventMysteriousMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMysteriousMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMysteriousMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMysterious> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMysteriousMap;
			if (v.EventMysteriousMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMysteriousMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMysteriousMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMysteriousConf cc) {
			foreach(KeyValuePair <string, rawEventMysteriousMapABValue> tagEntry in cc.EventMysteriousMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMysteriousMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMysteriousMapABValue();
					cc.EventMysteriousMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMysteriousMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMysterious> entry in cc.EventMysteriousMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMysterious> entry in tagEntry.Value.EventMysteriousMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMysterious> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMysteriousConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMysteriousMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMysteriousMapAB != null)
			{
				foreach (var entry in cc.EventMysteriousMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMysteriousMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMysteriousMap.MergeFrom(cis);
			Patch(updating.EventMysteriousMap);
			updating.EventMysteriousSortedKeys = genSortedKeys(updating.EventMysteriousMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMysteriousConf();
            tmp.MergeFrom(cis);
			updating.EventMysteriousMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMysteriousSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMysterious> filter,string tag=""){
		...
		public static rawEventMysterious MustGet(int key,string tag="") {
		...
		public static rawEventMysterious? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMysterious> GetByFilter(Func<rawEventMysterious, bool> filterFunc,string tag="") {
			List<rawEventMysterious> ret = new List<rawEventMysterious>();
			foreach(KeyValuePair <int, rawEventMysterious> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMysterious? GetOneByFilter(Func<rawEventMysterious, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMysterious> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMysterious data) 
		{
			return data.Id;			
		}
    }
