...
using RetentionPackConf = fat.rawdata.RetentionPackConf;
using rawRetentionPack = fat.rawdata.RetentionPack;
using rawRetentionPackMapABValue = fat.rawdata.RetentionPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RetentionPackVisitor
    {
		...
		public const string FileName = "RetentionPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RetentionPackVisitor.SetABDefault单独设定RetentionPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RetentionPackVisitor.SetABFallback单独设定RetentionPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRetentionPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRetentionPack>>(()=> new PBC::MapField<int, rawRetentionPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRetentionPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RetentionPackMap;
			var abMap = v.RetentionPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RetentionPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RetentionPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRetentionPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RetentionPackMap;
			if (v.RetentionPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RetentionPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRetentionPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RetentionPackConf cc) {
			foreach(KeyValuePair <string, rawRetentionPackMapABValue> tagEntry in cc.RetentionPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RetentionPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRetentionPackMapABValue();
					cc.RetentionPackMapAB[tag] = ps;
				}
                var tagTmp = ps.RetentionPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRetentionPack> entry in cc.RetentionPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRetentionPack> entry in tagEntry.Value.RetentionPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RetentionPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RetentionPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RetentionPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RetentionPackMapAB != null)
			{
				foreach (var entry in cc.RetentionPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RetentionPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RetentionPackMap.MergeFrom(cis);
			Patch(updating.RetentionPackMap);
			updating.RetentionPackSortedKeys = genSortedKeys(updating.RetentionPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RetentionPackConf();
            tmp.MergeFrom(cis);
			updating.RetentionPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RetentionPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRetentionPack> filter,string tag=""){
		...
		public static rawRetentionPack MustGet(int key,string tag="") {
		...
		public static rawRetentionPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRetentionPack> GetByFilter(Func<rawRetentionPack, bool> filterFunc,string tag="") {
			List<rawRetentionPack> ret = new List<rawRetentionPack>();
			foreach(KeyValuePair <int, rawRetentionPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRetentionPack? GetOneByFilter(Func<rawRetentionPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRetentionPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRetentionPack data) 
		{
			return data.Id;			
		}
    }
