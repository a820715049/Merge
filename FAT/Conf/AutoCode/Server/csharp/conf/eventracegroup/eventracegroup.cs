...
using EventRaceGroupConf = fat.rawdata.EventRaceGroupConf;
using rawEventRaceGroup = fat.rawdata.EventRaceGroup;
using rawEventRaceGroupMapABValue = fat.rawdata.EventRaceGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceGroupVisitor
    {
		...
		public const string FileName = "EventRaceGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceGroupVisitor.SetABDefault单独设定EventRaceGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceGroupVisitor.SetABFallback单独设定EventRaceGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceGroup>>(()=> new PBC::MapField<int, rawEventRaceGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceGroupMap;
			var abMap = v.EventRaceGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceGroupMap;
			if (v.EventRaceGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceGroupConf cc) {
			foreach(KeyValuePair <string, rawEventRaceGroupMapABValue> tagEntry in cc.EventRaceGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceGroupMapABValue();
					cc.EventRaceGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceGroup> entry in cc.EventRaceGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceGroup> entry in tagEntry.Value.EventRaceGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceGroupMapAB != null)
			{
				foreach (var entry in cc.EventRaceGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceGroupMap.MergeFrom(cis);
			Patch(updating.EventRaceGroupMap);
			updating.EventRaceGroupSortedKeys = genSortedKeys(updating.EventRaceGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceGroupConf();
            tmp.MergeFrom(cis);
			updating.EventRaceGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceGroup> filter,string tag=""){
		...
		public static rawEventRaceGroup MustGet(int key,string tag="") {
		...
		public static rawEventRaceGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceGroup> GetByFilter(Func<rawEventRaceGroup, bool> filterFunc,string tag="") {
			List<rawEventRaceGroup> ret = new List<rawEventRaceGroup>();
			foreach(KeyValuePair <int, rawEventRaceGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceGroup? GetOneByFilter(Func<rawEventRaceGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceGroup data) 
		{
			return data.Id;			
		}
    }
