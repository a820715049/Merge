...
using LangFrConf = fat.rawdata.LangFrConf;
using rawLangFr = fat.rawdata.LangFr;
using rawLangFrSliceABValue = fat.rawdata.LangFrSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangFrVisitor
    {
		...
		public const string FileName = "LangFrConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangFrVisitor.SetABDefault单独设定LangFr的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangFrVisitor.SetABFallback单独设定LangFr的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangFr>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangFr>>(()=>new PBC::RepeatedField<rawLangFr>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangFr>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangFrSlice;
			var abMap = v.LangFrSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangFrSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangFrSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangFr> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangFrSlice;
			if (v.LangFrSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangFrSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangFrSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangFrConf cc) {
			foreach(KeyValuePair <string, rawLangFrSliceABValue> tagEntry in cc.LangFrSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangFrSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangFrSliceABValue();
					cc.LangFrSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangFrSlice;
				// 基础数据
				for(int i=0; i<cc.LangFrSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangFrSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangFrSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangFr> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangFrSlice.MergeFrom(cis);
			Patch(updating.LangFrSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangFrConf();
            tmp.MergeFrom(cis);
			updating.LangFrSlice = tmp;
		...
		public static void Range(Action<int, rawLangFr> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangFr MustGetByIndex(int index,string tag="") {
		...
		public static rawLangFr? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangFr> GetByFilter(Func<rawLangFr,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangFr>();
		...
		static public rawLangFr? GetOneByFilter(Func<rawLangFr,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
