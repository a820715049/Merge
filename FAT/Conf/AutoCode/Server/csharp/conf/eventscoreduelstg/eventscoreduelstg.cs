...
using EventScoreDuelSTGConf = fat.rawdata.EventScoreDuelSTGConf;
using rawEventScoreDuelSTG = fat.rawdata.EventScoreDuelSTG;
using rawEventScoreDuelSTGMapABValue = fat.rawdata.EventScoreDuelSTGMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventScoreDuelSTGVisitor
    {
		...
		public const string FileName = "EventScoreDuelSTGConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventScoreDuelSTGVisitor.SetABDefault单独设定EventScoreDuelSTG的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventScoreDuelSTGVisitor.SetABFallback单独设定EventScoreDuelSTG的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventScoreDuelSTG>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventScoreDuelSTG>>(()=> new PBC::MapField<int, rawEventScoreDuelSTG>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventScoreDuelSTG>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventScoreDuelSTGMap;
			var abMap = v.EventScoreDuelSTGMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventScoreDuelSTGMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventScoreDuelSTGMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventScoreDuelSTG> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventScoreDuelSTGMap;
			if (v.EventScoreDuelSTGMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventScoreDuelSTGMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventScoreDuelSTGMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventScoreDuelSTGConf cc) {
			foreach(KeyValuePair <string, rawEventScoreDuelSTGMapABValue> tagEntry in cc.EventScoreDuelSTGMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventScoreDuelSTGMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventScoreDuelSTGMapABValue();
					cc.EventScoreDuelSTGMapAB[tag] = ps;
				}
                var tagTmp = ps.EventScoreDuelSTGMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventScoreDuelSTG> entry in cc.EventScoreDuelSTGMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventScoreDuelSTG> entry in tagEntry.Value.EventScoreDuelSTGMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventScoreDuelSTG> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventScoreDuelSTGConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventScoreDuelSTGMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventScoreDuelSTGMapAB != null)
			{
				foreach (var entry in cc.EventScoreDuelSTGMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventScoreDuelSTGMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventScoreDuelSTGMap.MergeFrom(cis);
			Patch(updating.EventScoreDuelSTGMap);
			updating.EventScoreDuelSTGSortedKeys = genSortedKeys(updating.EventScoreDuelSTGMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventScoreDuelSTGConf();
            tmp.MergeFrom(cis);
			updating.EventScoreDuelSTGMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventScoreDuelSTGSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventScoreDuelSTG> filter,string tag=""){
		...
		public static rawEventScoreDuelSTG MustGet(int key,string tag="") {
		...
		public static rawEventScoreDuelSTG? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventScoreDuelSTG> GetByFilter(Func<rawEventScoreDuelSTG, bool> filterFunc,string tag="") {
			List<rawEventScoreDuelSTG> ret = new List<rawEventScoreDuelSTG>();
			foreach(KeyValuePair <int, rawEventScoreDuelSTG> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventScoreDuelSTG? GetOneByFilter(Func<rawEventScoreDuelSTG, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventScoreDuelSTG> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventScoreDuelSTG data) 
		{
			return data.Id;			
		}
    }
