...
using OnePlusTwoConf = fat.rawdata.OnePlusTwoConf;
using rawOnePlusTwo = fat.rawdata.OnePlusTwo;
using rawOnePlusTwoMapABValue = fat.rawdata.OnePlusTwoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OnePlusTwoVisitor
    {
		...
		public const string FileName = "OnePlusTwoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OnePlusTwoVisitor.SetABDefault单独设定OnePlusTwo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OnePlusTwoVisitor.SetABFallback单独设定OnePlusTwo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOnePlusTwo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOnePlusTwo>>(()=> new PBC::MapField<int, rawOnePlusTwo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOnePlusTwo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OnePlusTwoMap;
			var abMap = v.OnePlusTwoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OnePlusTwoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OnePlusTwoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOnePlusTwo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OnePlusTwoMap;
			if (v.OnePlusTwoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OnePlusTwoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOnePlusTwoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OnePlusTwoConf cc) {
			foreach(KeyValuePair <string, rawOnePlusTwoMapABValue> tagEntry in cc.OnePlusTwoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OnePlusTwoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOnePlusTwoMapABValue();
					cc.OnePlusTwoMapAB[tag] = ps;
				}
                var tagTmp = ps.OnePlusTwoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOnePlusTwo> entry in cc.OnePlusTwoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOnePlusTwo> entry in tagEntry.Value.OnePlusTwoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OnePlusTwo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OnePlusTwoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OnePlusTwoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OnePlusTwoMapAB != null)
			{
				foreach (var entry in cc.OnePlusTwoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OnePlusTwoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OnePlusTwoMap.MergeFrom(cis);
			Patch(updating.OnePlusTwoMap);
			updating.OnePlusTwoSortedKeys = genSortedKeys(updating.OnePlusTwoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OnePlusTwoConf();
            tmp.MergeFrom(cis);
			updating.OnePlusTwoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OnePlusTwoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOnePlusTwo> filter,string tag=""){
		...
		public static rawOnePlusTwo MustGet(int key,string tag="") {
		...
		public static rawOnePlusTwo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOnePlusTwo> GetByFilter(Func<rawOnePlusTwo, bool> filterFunc,string tag="") {
			List<rawOnePlusTwo> ret = new List<rawOnePlusTwo>();
			foreach(KeyValuePair <int, rawOnePlusTwo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOnePlusTwo? GetOneByFilter(Func<rawOnePlusTwo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOnePlusTwo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOnePlusTwo data) 
		{
			return data.Id;			
		}
    }
