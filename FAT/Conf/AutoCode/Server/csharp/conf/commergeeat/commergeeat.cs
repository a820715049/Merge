...
using ComMergeEatConf = fat.rawdata.ComMergeEatConf;
using rawComMergeEat = fat.rawdata.ComMergeEat;
using rawComMergeEatMapABValue = fat.rawdata.ComMergeEatMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeEatVisitor
    {
		...
		public const string FileName = "ComMergeEatConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeEatVisitor.SetABDefault单独设定ComMergeEat的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeEatVisitor.SetABFallback单独设定ComMergeEat的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeEat>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeEat>>(()=> new PBC::MapField<int, rawComMergeEat>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeEat>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeEatMap;
			var abMap = v.ComMergeEatMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeEatMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeEatMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeEat> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeEatMap;
			if (v.ComMergeEatMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeEatMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeEatMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeEatConf cc) {
			foreach(KeyValuePair <string, rawComMergeEatMapABValue> tagEntry in cc.ComMergeEatMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeEatMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeEatMapABValue();
					cc.ComMergeEatMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeEatMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeEat> entry in cc.ComMergeEatMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeEat> entry in tagEntry.Value.ComMergeEatMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeEat> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeEatConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeEatMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeEatMapAB != null)
			{
				foreach (var entry in cc.ComMergeEatMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeEatMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeEatMap.MergeFrom(cis);
			Patch(updating.ComMergeEatMap);
			updating.ComMergeEatSortedKeys = genSortedKeys(updating.ComMergeEatMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeEatConf();
            tmp.MergeFrom(cis);
			updating.ComMergeEatMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeEatSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeEat> filter,string tag=""){
		...
		public static rawComMergeEat MustGet(int key,string tag="") {
		...
		public static rawComMergeEat? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeEat> GetByFilter(Func<rawComMergeEat, bool> filterFunc,string tag="") {
			List<rawComMergeEat> ret = new List<rawComMergeEat>();
			foreach(KeyValuePair <int, rawComMergeEat> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeEat? GetOneByFilter(Func<rawComMergeEat, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeEat> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeEat data) 
		{
			return data.Id;			
		}
    }
