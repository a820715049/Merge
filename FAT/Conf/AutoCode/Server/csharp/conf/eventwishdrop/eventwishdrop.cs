...
using EventWishDropConf = fat.rawdata.EventWishDropConf;
using rawEventWishDrop = fat.rawdata.EventWishDrop;
using rawEventWishDropMapABValue = fat.rawdata.EventWishDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishDropVisitor
    {
		...
		public const string FileName = "EventWishDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishDropVisitor.SetABDefault单独设定EventWishDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishDropVisitor.SetABFallback单独设定EventWishDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishDrop>>(()=> new PBC::MapField<int, rawEventWishDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishDropMap;
			var abMap = v.EventWishDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishDropMap;
			if (v.EventWishDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishDropConf cc) {
			foreach(KeyValuePair <string, rawEventWishDropMapABValue> tagEntry in cc.EventWishDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishDropMapABValue();
					cc.EventWishDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishDrop> entry in cc.EventWishDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishDrop> entry in tagEntry.Value.EventWishDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishDropMapAB != null)
			{
				foreach (var entry in cc.EventWishDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishDropMap.MergeFrom(cis);
			Patch(updating.EventWishDropMap);
			updating.EventWishDropSortedKeys = genSortedKeys(updating.EventWishDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishDropConf();
            tmp.MergeFrom(cis);
			updating.EventWishDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishDrop> filter,string tag=""){
		...
		public static rawEventWishDrop MustGet(int key,string tag="") {
		...
		public static rawEventWishDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishDrop> GetByFilter(Func<rawEventWishDrop, bool> filterFunc,string tag="") {
			List<rawEventWishDrop> ret = new List<rawEventWishDrop>();
			foreach(KeyValuePair <int, rawEventWishDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishDrop? GetOneByFilter(Func<rawEventWishDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishDrop data) 
		{
			return data.Id;			
		}
    }
