...
using MergeItemSeriesConf = fat.rawdata.MergeItemSeriesConf;
using rawMergeItemSeries = fat.rawdata.MergeItemSeries;
using rawMergeItemSeriesMapABValue = fat.rawdata.MergeItemSeriesMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeItemSeriesVisitor
    {
		...
		public const string FileName = "MergeItemSeriesConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeItemSeriesVisitor.SetABDefault单独设定MergeItemSeries的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeItemSeriesVisitor.SetABFallback单独设定MergeItemSeries的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeItemSeries>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeItemSeries>>(()=> new PBC::MapField<int, rawMergeItemSeries>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeItemSeries>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeItemSeriesMap;
			var abMap = v.MergeItemSeriesMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeItemSeriesMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeItemSeriesMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeItemSeries> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeItemSeriesMap;
			if (v.MergeItemSeriesMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeItemSeriesMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeItemSeriesMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeItemSeriesConf cc) {
			foreach(KeyValuePair <string, rawMergeItemSeriesMapABValue> tagEntry in cc.MergeItemSeriesMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeItemSeriesMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeItemSeriesMapABValue();
					cc.MergeItemSeriesMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeItemSeriesMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeItemSeries> entry in cc.MergeItemSeriesMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeItemSeries> entry in tagEntry.Value.MergeItemSeriesMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeItemSeries> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeItemSeriesConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeItemSeriesMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeItemSeriesMapAB != null)
			{
				foreach (var entry in cc.MergeItemSeriesMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeItemSeriesMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeItemSeriesMap.MergeFrom(cis);
			Patch(updating.MergeItemSeriesMap);
			updating.MergeItemSeriesSortedKeys = genSortedKeys(updating.MergeItemSeriesMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeItemSeriesConf();
            tmp.MergeFrom(cis);
			updating.MergeItemSeriesMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeItemSeriesSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeItemSeries> filter,string tag=""){
		...
		public static rawMergeItemSeries MustGet(int key,string tag="") {
		...
		public static rawMergeItemSeries? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeItemSeries> GetByFilter(Func<rawMergeItemSeries, bool> filterFunc,string tag="") {
			List<rawMergeItemSeries> ret = new List<rawMergeItemSeries>();
			foreach(KeyValuePair <int, rawMergeItemSeries> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeItemSeries? GetOneByFilter(Func<rawMergeItemSeries, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeItemSeries> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeItemSeries data) 
		{
			return data.Id;			
		}
    }
