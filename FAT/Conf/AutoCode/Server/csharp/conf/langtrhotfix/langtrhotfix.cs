...
using LangTrHotfixConf = fat.rawdata.LangTrHotfixConf;
using rawLangTrHotfix = fat.rawdata.LangTrHotfix;
using rawLangTrHotfixSliceABValue = fat.rawdata.LangTrHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangTrHotfixVisitor
    {
		...
		public const string FileName = "LangTrHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangTrHotfixVisitor.SetABDefault单独设定LangTrHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangTrHotfixVisitor.SetABFallback单独设定LangTrHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangTrHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangTrHotfix>>(()=>new PBC::RepeatedField<rawLangTrHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangTrHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangTrHotfixSlice;
			var abMap = v.LangTrHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangTrHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangTrHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangTrHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangTrHotfixSlice;
			if (v.LangTrHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangTrHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangTrHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangTrHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangTrHotfixSliceABValue> tagEntry in cc.LangTrHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangTrHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangTrHotfixSliceABValue();
					cc.LangTrHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangTrHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangTrHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangTrHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangTrHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangTrHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangTrHotfixSlice.MergeFrom(cis);
			Patch(updating.LangTrHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangTrHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangTrHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangTrHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangTrHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangTrHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangTrHotfix> GetByFilter(Func<rawLangTrHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangTrHotfix>();
		...
		static public rawLangTrHotfix? GetOneByFilter(Func<rawLangTrHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
