...
using ComMergeChestConf = fat.rawdata.ComMergeChestConf;
using rawComMergeChest = fat.rawdata.ComMergeChest;
using rawComMergeChestMapABValue = fat.rawdata.ComMergeChestMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeChestVisitor
    {
		...
		public const string FileName = "ComMergeChestConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeChestVisitor.SetABDefault单独设定ComMergeChest的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeChestVisitor.SetABFallback单独设定ComMergeChest的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawComMergeChest>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawComMergeChest>>(()=> new PBC::MapField<string, rawComMergeChest>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawComMergeChest>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeChestMap;
			var abMap = v.ComMergeChestMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeChestMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeChestMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawComMergeChest> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeChestMap;
			if (v.ComMergeChestMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeChestMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeChestMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeChestConf cc) {
			foreach(KeyValuePair <string, rawComMergeChestMapABValue> tagEntry in cc.ComMergeChestMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeChestMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeChestMapABValue();
					cc.ComMergeChestMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeChestMap;
				// 基础数据
				foreach(KeyValuePair <string, rawComMergeChest> entry in cc.ComMergeChestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawComMergeChest> entry in tagEntry.Value.ComMergeChestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.ComMergeChest> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(ComMergeChestConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.ComMergeChestMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeChestMapAB != null)
			{
				foreach (var entry in cc.ComMergeChestMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.ComMergeChestMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeChestMap.MergeFrom(cis);
			Patch(updating.ComMergeChestMap);
			updating.ComMergeChestSortedKeys = genSortedKeys(updating.ComMergeChestMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeChestConf();
            tmp.MergeFrom(cis);
			updating.ComMergeChestMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeChestSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawComMergeChest> filter,string tag=""){
		...
		public static rawComMergeChest MustGet(string key,string tag="") {
		...
		public static rawComMergeChest? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeChest> GetByFilter(Func<rawComMergeChest, bool> filterFunc,string tag="") {
			List<rawComMergeChest> ret = new List<rawComMergeChest>();
			foreach(KeyValuePair <string, rawComMergeChest> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeChest? GetOneByFilter(Func<rawComMergeChest, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawComMergeChest> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawComMergeChest data) 
		{
			return string.Format("{0}_{1}",data.ConfigVersion,data.Id);			
		}
        ...
        public class CK {
            ...
            public string String() {
				return string.Format("{0}_{1}", ConfigVersion,Id);
            }
            public static void ComMergeChestCK() {}
			public CK(int configVersion,int id) 
			{
        ...
        public static rawComMergeChest?  GetByCombinedKeys(CK ck,string tag="") {
