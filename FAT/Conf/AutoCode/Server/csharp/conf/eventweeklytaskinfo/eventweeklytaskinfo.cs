...
using EventWeeklyTaskInfoConf = fat.rawdata.EventWeeklyTaskInfoConf;
using rawEventWeeklyTaskInfo = fat.rawdata.EventWeeklyTaskInfo;
using rawEventWeeklyTaskInfoMapABValue = fat.rawdata.EventWeeklyTaskInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyTaskInfoVisitor
    {
		...
		public const string FileName = "EventWeeklyTaskInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyTaskInfoVisitor.SetABDefault单独设定EventWeeklyTaskInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyTaskInfoVisitor.SetABFallback单独设定EventWeeklyTaskInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyTaskInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyTaskInfo>>(()=> new PBC::MapField<int, rawEventWeeklyTaskInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyTaskInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyTaskInfoMap;
			var abMap = v.EventWeeklyTaskInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyTaskInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyTaskInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyTaskInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyTaskInfoMap;
			if (v.EventWeeklyTaskInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyTaskInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyTaskInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyTaskInfoConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyTaskInfoMapABValue> tagEntry in cc.EventWeeklyTaskInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyTaskInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyTaskInfoMapABValue();
					cc.EventWeeklyTaskInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyTaskInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskInfo> entry in cc.EventWeeklyTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskInfo> entry in tagEntry.Value.EventWeeklyTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyTaskInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyTaskInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyTaskInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyTaskInfoMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyTaskInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyTaskInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyTaskInfoMap.MergeFrom(cis);
			Patch(updating.EventWeeklyTaskInfoMap);
			updating.EventWeeklyTaskInfoSortedKeys = genSortedKeys(updating.EventWeeklyTaskInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyTaskInfoConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyTaskInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyTaskInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyTaskInfo> filter,string tag=""){
		...
		public static rawEventWeeklyTaskInfo MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyTaskInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyTaskInfo> GetByFilter(Func<rawEventWeeklyTaskInfo, bool> filterFunc,string tag="") {
			List<rawEventWeeklyTaskInfo> ret = new List<rawEventWeeklyTaskInfo>();
			foreach(KeyValuePair <int, rawEventWeeklyTaskInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyTaskInfo? GetOneByFilter(Func<rawEventWeeklyTaskInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyTaskInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyTaskInfo data) 
		{
			return data.Id;			
		}
    }
