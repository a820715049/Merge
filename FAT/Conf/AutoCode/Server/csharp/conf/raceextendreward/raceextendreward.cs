...
using RaceExtendRewardConf = fat.rawdata.RaceExtendRewardConf;
using rawRaceExtendReward = fat.rawdata.RaceExtendReward;
using rawRaceExtendRewardMapABValue = fat.rawdata.RaceExtendRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RaceExtendRewardVisitor
    {
		...
		public const string FileName = "RaceExtendRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RaceExtendRewardVisitor.SetABDefault单独设定RaceExtendReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RaceExtendRewardVisitor.SetABFallback单独设定RaceExtendReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRaceExtendReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRaceExtendReward>>(()=> new PBC::MapField<int, rawRaceExtendReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRaceExtendReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RaceExtendRewardMap;
			var abMap = v.RaceExtendRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RaceExtendRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RaceExtendRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRaceExtendReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RaceExtendRewardMap;
			if (v.RaceExtendRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RaceExtendRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRaceExtendRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RaceExtendRewardConf cc) {
			foreach(KeyValuePair <string, rawRaceExtendRewardMapABValue> tagEntry in cc.RaceExtendRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RaceExtendRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRaceExtendRewardMapABValue();
					cc.RaceExtendRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.RaceExtendRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRaceExtendReward> entry in cc.RaceExtendRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRaceExtendReward> entry in tagEntry.Value.RaceExtendRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RaceExtendReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RaceExtendRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RaceExtendRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RaceExtendRewardMapAB != null)
			{
				foreach (var entry in cc.RaceExtendRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RaceExtendRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RaceExtendRewardMap.MergeFrom(cis);
			Patch(updating.RaceExtendRewardMap);
			updating.RaceExtendRewardSortedKeys = genSortedKeys(updating.RaceExtendRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RaceExtendRewardConf();
            tmp.MergeFrom(cis);
			updating.RaceExtendRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RaceExtendRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRaceExtendReward> filter,string tag=""){
		...
		public static rawRaceExtendReward MustGet(int key,string tag="") {
		...
		public static rawRaceExtendReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRaceExtendReward> GetByFilter(Func<rawRaceExtendReward, bool> filterFunc,string tag="") {
			List<rawRaceExtendReward> ret = new List<rawRaceExtendReward>();
			foreach(KeyValuePair <int, rawRaceExtendReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRaceExtendReward? GetOneByFilter(Func<rawRaceExtendReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRaceExtendReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRaceExtendReward data) 
		{
			return data.Id;			
		}
    }
