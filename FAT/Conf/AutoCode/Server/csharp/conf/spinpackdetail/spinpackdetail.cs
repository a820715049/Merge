...
using SpinPackDetailConf = fat.rawdata.SpinPackDetailConf;
using rawSpinPackDetail = fat.rawdata.SpinPackDetail;
using rawSpinPackDetailMapABValue = fat.rawdata.SpinPackDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SpinPackDetailVisitor
    {
		...
		public const string FileName = "SpinPackDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SpinPackDetailVisitor.SetABDefault单独设定SpinPackDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SpinPackDetailVisitor.SetABFallback单独设定SpinPackDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSpinPackDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSpinPackDetail>>(()=> new PBC::MapField<int, rawSpinPackDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSpinPackDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SpinPackDetailMap;
			var abMap = v.SpinPackDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SpinPackDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SpinPackDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSpinPackDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SpinPackDetailMap;
			if (v.SpinPackDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SpinPackDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSpinPackDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SpinPackDetailConf cc) {
			foreach(KeyValuePair <string, rawSpinPackDetailMapABValue> tagEntry in cc.SpinPackDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SpinPackDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSpinPackDetailMapABValue();
					cc.SpinPackDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.SpinPackDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSpinPackDetail> entry in cc.SpinPackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSpinPackDetail> entry in tagEntry.Value.SpinPackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SpinPackDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SpinPackDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SpinPackDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SpinPackDetailMapAB != null)
			{
				foreach (var entry in cc.SpinPackDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SpinPackDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SpinPackDetailMap.MergeFrom(cis);
			Patch(updating.SpinPackDetailMap);
			updating.SpinPackDetailSortedKeys = genSortedKeys(updating.SpinPackDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SpinPackDetailConf();
            tmp.MergeFrom(cis);
			updating.SpinPackDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SpinPackDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSpinPackDetail> filter,string tag=""){
		...
		public static rawSpinPackDetail MustGet(int key,string tag="") {
		...
		public static rawSpinPackDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSpinPackDetail> GetByFilter(Func<rawSpinPackDetail, bool> filterFunc,string tag="") {
			List<rawSpinPackDetail> ret = new List<rawSpinPackDetail>();
			foreach(KeyValuePair <int, rawSpinPackDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSpinPackDetail? GetOneByFilter(Func<rawSpinPackDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSpinPackDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSpinPackDetail data) 
		{
			return data.Id;			
		}
    }
