...
using RaceExtendRoundConf = fat.rawdata.RaceExtendRoundConf;
using rawRaceExtendRound = fat.rawdata.RaceExtendRound;
using rawRaceExtendRoundMapABValue = fat.rawdata.RaceExtendRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RaceExtendRoundVisitor
    {
		...
		public const string FileName = "RaceExtendRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RaceExtendRoundVisitor.SetABDefault单独设定RaceExtendRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RaceExtendRoundVisitor.SetABFallback单独设定RaceExtendRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRaceExtendRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRaceExtendRound>>(()=> new PBC::MapField<int, rawRaceExtendRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRaceExtendRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RaceExtendRoundMap;
			var abMap = v.RaceExtendRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RaceExtendRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RaceExtendRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRaceExtendRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RaceExtendRoundMap;
			if (v.RaceExtendRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RaceExtendRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRaceExtendRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RaceExtendRoundConf cc) {
			foreach(KeyValuePair <string, rawRaceExtendRoundMapABValue> tagEntry in cc.RaceExtendRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RaceExtendRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRaceExtendRoundMapABValue();
					cc.RaceExtendRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.RaceExtendRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRaceExtendRound> entry in cc.RaceExtendRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRaceExtendRound> entry in tagEntry.Value.RaceExtendRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RaceExtendRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RaceExtendRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RaceExtendRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RaceExtendRoundMapAB != null)
			{
				foreach (var entry in cc.RaceExtendRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RaceExtendRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RaceExtendRoundMap.MergeFrom(cis);
			Patch(updating.RaceExtendRoundMap);
			updating.RaceExtendRoundSortedKeys = genSortedKeys(updating.RaceExtendRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RaceExtendRoundConf();
            tmp.MergeFrom(cis);
			updating.RaceExtendRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RaceExtendRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRaceExtendRound> filter,string tag=""){
		...
		public static rawRaceExtendRound MustGet(int key,string tag="") {
		...
		public static rawRaceExtendRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRaceExtendRound> GetByFilter(Func<rawRaceExtendRound, bool> filterFunc,string tag="") {
			List<rawRaceExtendRound> ret = new List<rawRaceExtendRound>();
			foreach(KeyValuePair <int, rawRaceExtendRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRaceExtendRound? GetOneByFilter(Func<rawRaceExtendRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRaceExtendRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRaceExtendRound data) 
		{
			return data.Id;			
		}
    }
