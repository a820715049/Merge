...
using LoginSignTotalConf = fat.rawdata.LoginSignTotalConf;
using rawLoginSignTotal = fat.rawdata.LoginSignTotal;
using rawLoginSignTotalMapABValue = fat.rawdata.LoginSignTotalMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LoginSignTotalVisitor
    {
		...
		public const string FileName = "LoginSignTotalConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LoginSignTotalVisitor.SetABDefault单独设定LoginSignTotal的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LoginSignTotalVisitor.SetABFallback单独设定LoginSignTotal的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLoginSignTotal>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLoginSignTotal>>(()=> new PBC::MapField<int, rawLoginSignTotal>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLoginSignTotal>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LoginSignTotalMap;
			var abMap = v.LoginSignTotalMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LoginSignTotalMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LoginSignTotalMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLoginSignTotal> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LoginSignTotalMap;
			if (v.LoginSignTotalMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LoginSignTotalMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLoginSignTotalMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LoginSignTotalConf cc) {
			foreach(KeyValuePair <string, rawLoginSignTotalMapABValue> tagEntry in cc.LoginSignTotalMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LoginSignTotalMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLoginSignTotalMapABValue();
					cc.LoginSignTotalMapAB[tag] = ps;
				}
                var tagTmp = ps.LoginSignTotalMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLoginSignTotal> entry in cc.LoginSignTotalMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLoginSignTotal> entry in tagEntry.Value.LoginSignTotalMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LoginSignTotal> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LoginSignTotalConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LoginSignTotalMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LoginSignTotalMapAB != null)
			{
				foreach (var entry in cc.LoginSignTotalMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LoginSignTotalMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LoginSignTotalMap.MergeFrom(cis);
			Patch(updating.LoginSignTotalMap);
			updating.LoginSignTotalSortedKeys = genSortedKeys(updating.LoginSignTotalMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LoginSignTotalConf();
            tmp.MergeFrom(cis);
			updating.LoginSignTotalMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LoginSignTotalSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLoginSignTotal> filter,string tag=""){
		...
		public static rawLoginSignTotal MustGet(int key,string tag="") {
		...
		public static rawLoginSignTotal? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLoginSignTotal> GetByFilter(Func<rawLoginSignTotal, bool> filterFunc,string tag="") {
			List<rawLoginSignTotal> ret = new List<rawLoginSignTotal>();
			foreach(KeyValuePair <int, rawLoginSignTotal> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLoginSignTotal? GetOneByFilter(Func<rawLoginSignTotal, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLoginSignTotal> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLoginSignTotal data) 
		{
			return data.Id;			
		}
    }
