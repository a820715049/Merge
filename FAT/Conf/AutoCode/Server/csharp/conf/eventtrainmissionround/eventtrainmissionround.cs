...
using EventTrainMissionRoundConf = fat.rawdata.EventTrainMissionRoundConf;
using rawEventTrainMissionRound = fat.rawdata.EventTrainMissionRound;
using rawEventTrainMissionRoundMapABValue = fat.rawdata.EventTrainMissionRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTrainMissionRoundVisitor
    {
		...
		public const string FileName = "EventTrainMissionRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTrainMissionRoundVisitor.SetABDefault单独设定EventTrainMissionRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTrainMissionRoundVisitor.SetABFallback单独设定EventTrainMissionRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTrainMissionRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTrainMissionRound>>(()=> new PBC::MapField<int, rawEventTrainMissionRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTrainMissionRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTrainMissionRoundMap;
			var abMap = v.EventTrainMissionRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTrainMissionRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTrainMissionRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTrainMissionRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTrainMissionRoundMap;
			if (v.EventTrainMissionRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTrainMissionRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTrainMissionRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTrainMissionRoundConf cc) {
			foreach(KeyValuePair <string, rawEventTrainMissionRoundMapABValue> tagEntry in cc.EventTrainMissionRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTrainMissionRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTrainMissionRoundMapABValue();
					cc.EventTrainMissionRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTrainMissionRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTrainMissionRound> entry in cc.EventTrainMissionRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTrainMissionRound> entry in tagEntry.Value.EventTrainMissionRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTrainMissionRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTrainMissionRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTrainMissionRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTrainMissionRoundMapAB != null)
			{
				foreach (var entry in cc.EventTrainMissionRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTrainMissionRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTrainMissionRoundMap.MergeFrom(cis);
			Patch(updating.EventTrainMissionRoundMap);
			updating.EventTrainMissionRoundSortedKeys = genSortedKeys(updating.EventTrainMissionRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTrainMissionRoundConf();
            tmp.MergeFrom(cis);
			updating.EventTrainMissionRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTrainMissionRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTrainMissionRound> filter,string tag=""){
		...
		public static rawEventTrainMissionRound MustGet(int key,string tag="") {
		...
		public static rawEventTrainMissionRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTrainMissionRound> GetByFilter(Func<rawEventTrainMissionRound, bool> filterFunc,string tag="") {
			List<rawEventTrainMissionRound> ret = new List<rawEventTrainMissionRound>();
			foreach(KeyValuePair <int, rawEventTrainMissionRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTrainMissionRound? GetOneByFilter(Func<rawEventTrainMissionRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTrainMissionRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTrainMissionRound data) 
		{
			return data.Id;			
		}
    }
