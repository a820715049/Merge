...
using MultiRankRewardConf = fat.rawdata.MultiRankRewardConf;
using rawMultiRankReward = fat.rawdata.MultiRankReward;
using rawMultiRankRewardMapABValue = fat.rawdata.MultiRankRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankRewardVisitor
    {
		...
		public const string FileName = "MultiRankRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankRewardVisitor.SetABDefault单独设定MultiRankReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankRewardVisitor.SetABFallback单独设定MultiRankReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRankReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRankReward>>(()=> new PBC::MapField<int, rawMultiRankReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRankReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankRewardMap;
			var abMap = v.MultiRankRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRankReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankRewardMap;
			if (v.MultiRankRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankRewardConf cc) {
			foreach(KeyValuePair <string, rawMultiRankRewardMapABValue> tagEntry in cc.MultiRankRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankRewardMapABValue();
					cc.MultiRankRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRankReward> entry in cc.MultiRankRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRankReward> entry in tagEntry.Value.MultiRankRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRankReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankRewardMapAB != null)
			{
				foreach (var entry in cc.MultiRankRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankRewardMap.MergeFrom(cis);
			Patch(updating.MultiRankRewardMap);
			updating.MultiRankRewardSortedKeys = genSortedKeys(updating.MultiRankRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankRewardConf();
            tmp.MergeFrom(cis);
			updating.MultiRankRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRankReward> filter,string tag=""){
		...
		public static rawMultiRankReward MustGet(int key,string tag="") {
		...
		public static rawMultiRankReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRankReward> GetByFilter(Func<rawMultiRankReward, bool> filterFunc,string tag="") {
			List<rawMultiRankReward> ret = new List<rawMultiRankReward>();
			foreach(KeyValuePair <int, rawMultiRankReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRankReward? GetOneByFilter(Func<rawMultiRankReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRankReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRankReward data) 
		{
			return data.Id;			
		}
    }
