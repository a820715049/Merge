...
using MiniGameSheetConf = fat.rawdata.MiniGameSheetConf;
using rawMiniGameSheet = fat.rawdata.MiniGameSheet;
using rawMiniGameSheetMapABValue = fat.rawdata.MiniGameSheetMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameSheetVisitor
    {
		...
		public const string FileName = "MiniGameSheetConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameSheetVisitor.SetABDefault单独设定MiniGameSheet的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameSheetVisitor.SetABFallback单独设定MiniGameSheet的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameSheet>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameSheet>>(()=> new PBC::MapField<int, rawMiniGameSheet>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameSheet>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameSheetMap;
			var abMap = v.MiniGameSheetMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameSheetMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameSheetMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameSheet> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameSheetMap;
			if (v.MiniGameSheetMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameSheetMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameSheetMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameSheetConf cc) {
			foreach(KeyValuePair <string, rawMiniGameSheetMapABValue> tagEntry in cc.MiniGameSheetMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameSheetMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameSheetMapABValue();
					cc.MiniGameSheetMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameSheetMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameSheet> entry in cc.MiniGameSheetMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameSheet> entry in tagEntry.Value.MiniGameSheetMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameSheet> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameSheetConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameSheetMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameSheetMapAB != null)
			{
				foreach (var entry in cc.MiniGameSheetMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameSheetMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameSheetMap.MergeFrom(cis);
			Patch(updating.MiniGameSheetMap);
			updating.MiniGameSheetSortedKeys = genSortedKeys(updating.MiniGameSheetMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameSheetConf();
            tmp.MergeFrom(cis);
			updating.MiniGameSheetMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameSheetSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameSheet> filter,string tag=""){
		...
		public static rawMiniGameSheet MustGet(int key,string tag="") {
		...
		public static rawMiniGameSheet? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameSheet> GetByFilter(Func<rawMiniGameSheet, bool> filterFunc,string tag="") {
			List<rawMiniGameSheet> ret = new List<rawMiniGameSheet>();
			foreach(KeyValuePair <int, rawMiniGameSheet> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameSheet? GetOneByFilter(Func<rawMiniGameSheet, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameSheet> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameSheet data) 
		{
			return data.Id;			
		}
    }
