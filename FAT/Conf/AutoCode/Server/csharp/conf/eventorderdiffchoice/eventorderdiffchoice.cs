...
using EventOrderDiffChoiceConf = fat.rawdata.EventOrderDiffChoiceConf;
using rawEventOrderDiffChoice = fat.rawdata.EventOrderDiffChoice;
using rawEventOrderDiffChoiceMapABValue = fat.rawdata.EventOrderDiffChoiceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderDiffChoiceVisitor
    {
		...
		public const string FileName = "EventOrderDiffChoiceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderDiffChoiceVisitor.SetABDefault单独设定EventOrderDiffChoice的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderDiffChoiceVisitor.SetABFallback单独设定EventOrderDiffChoice的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderDiffChoice>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderDiffChoice>>(()=> new PBC::MapField<int, rawEventOrderDiffChoice>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderDiffChoice>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderDiffChoiceMap;
			var abMap = v.EventOrderDiffChoiceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderDiffChoiceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderDiffChoiceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderDiffChoice> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderDiffChoiceMap;
			if (v.EventOrderDiffChoiceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderDiffChoiceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderDiffChoiceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderDiffChoiceConf cc) {
			foreach(KeyValuePair <string, rawEventOrderDiffChoiceMapABValue> tagEntry in cc.EventOrderDiffChoiceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderDiffChoiceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderDiffChoiceMapABValue();
					cc.EventOrderDiffChoiceMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderDiffChoiceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderDiffChoice> entry in cc.EventOrderDiffChoiceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderDiffChoice> entry in tagEntry.Value.EventOrderDiffChoiceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderDiffChoice> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderDiffChoiceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderDiffChoiceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderDiffChoiceMapAB != null)
			{
				foreach (var entry in cc.EventOrderDiffChoiceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderDiffChoiceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderDiffChoiceMap.MergeFrom(cis);
			Patch(updating.EventOrderDiffChoiceMap);
			updating.EventOrderDiffChoiceSortedKeys = genSortedKeys(updating.EventOrderDiffChoiceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderDiffChoiceConf();
            tmp.MergeFrom(cis);
			updating.EventOrderDiffChoiceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderDiffChoiceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderDiffChoice> filter,string tag=""){
		...
		public static rawEventOrderDiffChoice MustGet(int key,string tag="") {
		...
		public static rawEventOrderDiffChoice? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderDiffChoice> GetByFilter(Func<rawEventOrderDiffChoice, bool> filterFunc,string tag="") {
			List<rawEventOrderDiffChoice> ret = new List<rawEventOrderDiffChoice>();
			foreach(KeyValuePair <int, rawEventOrderDiffChoice> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderDiffChoice? GetOneByFilter(Func<rawEventOrderDiffChoice, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderDiffChoice> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderDiffChoice data) 
		{
			return data.Id;			
		}
    }
