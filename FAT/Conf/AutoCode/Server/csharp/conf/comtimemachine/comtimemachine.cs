...
using ComTimeMachineConf = fat.rawdata.ComTimeMachineConf;
using rawComTimeMachine = fat.rawdata.ComTimeMachine;
using rawComTimeMachineMapABValue = fat.rawdata.ComTimeMachineMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComTimeMachineVisitor
    {
		...
		public const string FileName = "ComTimeMachineConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComTimeMachineVisitor.SetABDefault单独设定ComTimeMachine的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComTimeMachineVisitor.SetABFallback单独设定ComTimeMachine的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComTimeMachine>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComTimeMachine>>(()=> new PBC::MapField<int, rawComTimeMachine>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComTimeMachine>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComTimeMachineMap;
			var abMap = v.ComTimeMachineMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComTimeMachineMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComTimeMachineMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComTimeMachine> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComTimeMachineMap;
			if (v.ComTimeMachineMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComTimeMachineMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComTimeMachineMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComTimeMachineConf cc) {
			foreach(KeyValuePair <string, rawComTimeMachineMapABValue> tagEntry in cc.ComTimeMachineMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComTimeMachineMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComTimeMachineMapABValue();
					cc.ComTimeMachineMapAB[tag] = ps;
				}
                var tagTmp = ps.ComTimeMachineMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComTimeMachine> entry in cc.ComTimeMachineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComTimeMachine> entry in tagEntry.Value.ComTimeMachineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComTimeMachine> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComTimeMachineConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComTimeMachineMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComTimeMachineMapAB != null)
			{
				foreach (var entry in cc.ComTimeMachineMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComTimeMachineMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComTimeMachineMap.MergeFrom(cis);
			Patch(updating.ComTimeMachineMap);
			updating.ComTimeMachineSortedKeys = genSortedKeys(updating.ComTimeMachineMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComTimeMachineConf();
            tmp.MergeFrom(cis);
			updating.ComTimeMachineMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComTimeMachineSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComTimeMachine> filter,string tag=""){
		...
		public static rawComTimeMachine MustGet(int key,string tag="") {
		...
		public static rawComTimeMachine? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComTimeMachine> GetByFilter(Func<rawComTimeMachine, bool> filterFunc,string tag="") {
			List<rawComTimeMachine> ret = new List<rawComTimeMachine>();
			foreach(KeyValuePair <int, rawComTimeMachine> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComTimeMachine? GetOneByFilter(Func<rawComTimeMachine, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComTimeMachine> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComTimeMachine data) 
		{
			return data.Id;			
		}
    }
