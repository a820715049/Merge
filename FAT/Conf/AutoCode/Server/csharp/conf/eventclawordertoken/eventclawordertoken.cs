...
using EventClawOrderTokenConf = fat.rawdata.EventClawOrderTokenConf;
using rawEventClawOrderToken = fat.rawdata.EventClawOrderToken;
using rawEventClawOrderTokenMapABValue = fat.rawdata.EventClawOrderTokenMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderTokenVisitor
    {
		...
		public const string FileName = "EventClawOrderTokenConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderTokenVisitor.SetABDefault单独设定EventClawOrderToken的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderTokenVisitor.SetABFallback单独设定EventClawOrderToken的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrderToken>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrderToken>>(()=> new PBC::MapField<int, rawEventClawOrderToken>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrderToken>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderTokenMap;
			var abMap = v.EventClawOrderTokenMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderTokenMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderTokenMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrderToken> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderTokenMap;
			if (v.EventClawOrderTokenMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderTokenMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderTokenMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderTokenConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderTokenMapABValue> tagEntry in cc.EventClawOrderTokenMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderTokenMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderTokenMapABValue();
					cc.EventClawOrderTokenMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderTokenMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrderToken> entry in cc.EventClawOrderTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrderToken> entry in tagEntry.Value.EventClawOrderTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrderToken> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderTokenConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderTokenMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderTokenMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderTokenMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderTokenMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderTokenMap.MergeFrom(cis);
			Patch(updating.EventClawOrderTokenMap);
			updating.EventClawOrderTokenSortedKeys = genSortedKeys(updating.EventClawOrderTokenMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderTokenConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderTokenMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderTokenSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrderToken> filter,string tag=""){
		...
		public static rawEventClawOrderToken MustGet(int key,string tag="") {
		...
		public static rawEventClawOrderToken? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrderToken> GetByFilter(Func<rawEventClawOrderToken, bool> filterFunc,string tag="") {
			List<rawEventClawOrderToken> ret = new List<rawEventClawOrderToken>();
			foreach(KeyValuePair <int, rawEventClawOrderToken> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrderToken? GetOneByFilter(Func<rawEventClawOrderToken, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrderToken> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrderToken data) 
		{
			return data.Id;			
		}
    }
