...
using EventMineCartConf = fat.rawdata.EventMineCartConf;
using rawEventMineCart = fat.rawdata.EventMineCart;
using rawEventMineCartMapABValue = fat.rawdata.EventMineCartMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartVisitor
    {
		...
		public const string FileName = "EventMineCartConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartVisitor.SetABDefault单独设定EventMineCart的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartVisitor.SetABFallback单独设定EventMineCart的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCart>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCart>>(()=> new PBC::MapField<int, rawEventMineCart>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCart>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartMap;
			var abMap = v.EventMineCartMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCart> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartMap;
			if (v.EventMineCartMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartMapABValue> tagEntry in cc.EventMineCartMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartMapABValue();
					cc.EventMineCartMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCart> entry in cc.EventMineCartMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCart> entry in tagEntry.Value.EventMineCartMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCart> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartMapAB != null)
			{
				foreach (var entry in cc.EventMineCartMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartMap.MergeFrom(cis);
			Patch(updating.EventMineCartMap);
			updating.EventMineCartSortedKeys = genSortedKeys(updating.EventMineCartMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCart> filter,string tag=""){
		...
		public static rawEventMineCart MustGet(int key,string tag="") {
		...
		public static rawEventMineCart? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCart> GetByFilter(Func<rawEventMineCart, bool> filterFunc,string tag="") {
			List<rawEventMineCart> ret = new List<rawEventMineCart>();
			foreach(KeyValuePair <int, rawEventMineCart> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCart? GetOneByFilter(Func<rawEventMineCart, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCart> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCart data) 
		{
			return data.Id;			
		}
    }
