...
using NewUserPackConf = fat.rawdata.NewUserPackConf;
using rawNewUserPack = fat.rawdata.NewUserPack;
using rawNewUserPackMapABValue = fat.rawdata.NewUserPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class NewUserPackVisitor
    {
		...
		public const string FileName = "NewUserPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过NewUserPackVisitor.SetABDefault单独设定NewUserPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过NewUserPackVisitor.SetABFallback单独设定NewUserPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawNewUserPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawNewUserPack>>(()=> new PBC::MapField<int, rawNewUserPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawNewUserPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().NewUserPackMap;
			var abMap = v.NewUserPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.NewUserPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].NewUserPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawNewUserPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().NewUserPackMap;
			if (v.NewUserPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.NewUserPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawNewUserPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(NewUserPackConf cc) {
			foreach(KeyValuePair <string, rawNewUserPackMapABValue> tagEntry in cc.NewUserPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.NewUserPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawNewUserPackMapABValue();
					cc.NewUserPackMapAB[tag] = ps;
				}
                var tagTmp = ps.NewUserPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawNewUserPack> entry in cc.NewUserPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawNewUserPack> entry in tagEntry.Value.NewUserPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.NewUserPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(NewUserPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.NewUserPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.NewUserPackMapAB != null)
			{
				foreach (var entry in cc.NewUserPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.NewUserPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.NewUserPackMap.MergeFrom(cis);
			Patch(updating.NewUserPackMap);
			updating.NewUserPackSortedKeys = genSortedKeys(updating.NewUserPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new NewUserPackConf();
            tmp.MergeFrom(cis);
			updating.NewUserPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().NewUserPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawNewUserPack> filter,string tag=""){
		...
		public static rawNewUserPack MustGet(int key,string tag="") {
		...
		public static rawNewUserPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawNewUserPack> GetByFilter(Func<rawNewUserPack, bool> filterFunc,string tag="") {
			List<rawNewUserPack> ret = new List<rawNewUserPack>();
			foreach(KeyValuePair <int, rawNewUserPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawNewUserPack? GetOneByFilter(Func<rawNewUserPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawNewUserPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawNewUserPack data) 
		{
			return data.Id;			
		}
    }
