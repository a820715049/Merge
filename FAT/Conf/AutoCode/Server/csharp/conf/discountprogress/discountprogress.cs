...
using DiscountProgressConf = fat.rawdata.DiscountProgressConf;
using rawDiscountProgress = fat.rawdata.DiscountProgress;
using rawDiscountProgressMapABValue = fat.rawdata.DiscountProgressMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DiscountProgressVisitor
    {
		...
		public const string FileName = "DiscountProgressConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DiscountProgressVisitor.SetABDefault单独设定DiscountProgress的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DiscountProgressVisitor.SetABFallback单独设定DiscountProgress的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDiscountProgress>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDiscountProgress>>(()=> new PBC::MapField<int, rawDiscountProgress>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDiscountProgress>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DiscountProgressMap;
			var abMap = v.DiscountProgressMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DiscountProgressMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DiscountProgressMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDiscountProgress> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DiscountProgressMap;
			if (v.DiscountProgressMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DiscountProgressMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDiscountProgressMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DiscountProgressConf cc) {
			foreach(KeyValuePair <string, rawDiscountProgressMapABValue> tagEntry in cc.DiscountProgressMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DiscountProgressMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDiscountProgressMapABValue();
					cc.DiscountProgressMapAB[tag] = ps;
				}
                var tagTmp = ps.DiscountProgressMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDiscountProgress> entry in cc.DiscountProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDiscountProgress> entry in tagEntry.Value.DiscountProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DiscountProgress> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DiscountProgressConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DiscountProgressMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DiscountProgressMapAB != null)
			{
				foreach (var entry in cc.DiscountProgressMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DiscountProgressMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DiscountProgressMap.MergeFrom(cis);
			Patch(updating.DiscountProgressMap);
			updating.DiscountProgressSortedKeys = genSortedKeys(updating.DiscountProgressMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DiscountProgressConf();
            tmp.MergeFrom(cis);
			updating.DiscountProgressMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DiscountProgressSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDiscountProgress> filter,string tag=""){
		...
		public static rawDiscountProgress MustGet(int key,string tag="") {
		...
		public static rawDiscountProgress? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDiscountProgress> GetByFilter(Func<rawDiscountProgress, bool> filterFunc,string tag="") {
			List<rawDiscountProgress> ret = new List<rawDiscountProgress>();
			foreach(KeyValuePair <int, rawDiscountProgress> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDiscountProgress? GetOneByFilter(Func<rawDiscountProgress, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDiscountProgress> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDiscountProgress data) 
		{
			return data.Id;			
		}
    }
