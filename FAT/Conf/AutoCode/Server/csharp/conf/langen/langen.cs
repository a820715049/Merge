...
using LangEnConf = fat.rawdata.LangEnConf;
using rawLangEn = fat.rawdata.LangEn;
using rawLangEnSliceABValue = fat.rawdata.LangEnSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangEnVisitor
    {
		...
		public const string FileName = "LangEnConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangEnVisitor.SetABDefault单独设定LangEn的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangEnVisitor.SetABFallback单独设定LangEn的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangEn>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangEn>>(()=>new PBC::RepeatedField<rawLangEn>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangEn>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangEnSlice;
			var abMap = v.LangEnSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangEnSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangEnSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangEn> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangEnSlice;
			if (v.LangEnSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangEnSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangEnSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangEnConf cc) {
			foreach(KeyValuePair <string, rawLangEnSliceABValue> tagEntry in cc.LangEnSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangEnSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangEnSliceABValue();
					cc.LangEnSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangEnSlice;
				// 基础数据
				for(int i=0; i<cc.LangEnSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangEnSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangEnSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangEn> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangEnSlice.MergeFrom(cis);
			Patch(updating.LangEnSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangEnConf();
            tmp.MergeFrom(cis);
			updating.LangEnSlice = tmp;
		...
		public static void Range(Action<int, rawLangEn> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangEn MustGetByIndex(int index,string tag="") {
		...
		public static rawLangEn? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangEn> GetByFilter(Func<rawLangEn,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangEn>();
		...
		static public rawLangEn? GetOneByFilter(Func<rawLangEn,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
