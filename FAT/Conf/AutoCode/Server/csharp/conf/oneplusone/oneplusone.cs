...
using OnePlusOneConf = fat.rawdata.OnePlusOneConf;
using rawOnePlusOne = fat.rawdata.OnePlusOne;
using rawOnePlusOneMapABValue = fat.rawdata.OnePlusOneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OnePlusOneVisitor
    {
		...
		public const string FileName = "OnePlusOneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OnePlusOneVisitor.SetABDefault单独设定OnePlusOne的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OnePlusOneVisitor.SetABFallback单独设定OnePlusOne的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOnePlusOne>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOnePlusOne>>(()=> new PBC::MapField<int, rawOnePlusOne>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOnePlusOne>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OnePlusOneMap;
			var abMap = v.OnePlusOneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OnePlusOneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OnePlusOneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOnePlusOne> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OnePlusOneMap;
			if (v.OnePlusOneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OnePlusOneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOnePlusOneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OnePlusOneConf cc) {
			foreach(KeyValuePair <string, rawOnePlusOneMapABValue> tagEntry in cc.OnePlusOneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OnePlusOneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOnePlusOneMapABValue();
					cc.OnePlusOneMapAB[tag] = ps;
				}
                var tagTmp = ps.OnePlusOneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOnePlusOne> entry in cc.OnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOnePlusOne> entry in tagEntry.Value.OnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OnePlusOne> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OnePlusOneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OnePlusOneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OnePlusOneMapAB != null)
			{
				foreach (var entry in cc.OnePlusOneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OnePlusOneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OnePlusOneMap.MergeFrom(cis);
			Patch(updating.OnePlusOneMap);
			updating.OnePlusOneSortedKeys = genSortedKeys(updating.OnePlusOneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OnePlusOneConf();
            tmp.MergeFrom(cis);
			updating.OnePlusOneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OnePlusOneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOnePlusOne> filter,string tag=""){
		...
		public static rawOnePlusOne MustGet(int key,string tag="") {
		...
		public static rawOnePlusOne? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOnePlusOne> GetByFilter(Func<rawOnePlusOne, bool> filterFunc,string tag="") {
			List<rawOnePlusOne> ret = new List<rawOnePlusOne>();
			foreach(KeyValuePair <int, rawOnePlusOne> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOnePlusOne? GetOneByFilter(Func<rawOnePlusOne, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOnePlusOne> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOnePlusOne data) 
		{
			return data.Id;			
		}
    }
