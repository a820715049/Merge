...
using DEGroupConf = fat.rawdata.DEGroupConf;
using rawDEGroup = fat.rawdata.DEGroup;
using rawDEGroupMapABValue = fat.rawdata.DEGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DEGroupVisitor
    {
		...
		public const string FileName = "DEGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DEGroupVisitor.SetABDefault单独设定DEGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DEGroupVisitor.SetABFallback单独设定DEGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDEGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDEGroup>>(()=> new PBC::MapField<int, rawDEGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDEGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DEGroupMap;
			var abMap = v.DEGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DEGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DEGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDEGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DEGroupMap;
			if (v.DEGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DEGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDEGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DEGroupConf cc) {
			foreach(KeyValuePair <string, rawDEGroupMapABValue> tagEntry in cc.DEGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DEGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDEGroupMapABValue();
					cc.DEGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.DEGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDEGroup> entry in cc.DEGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDEGroup> entry in tagEntry.Value.DEGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DEGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DEGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DEGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DEGroupMapAB != null)
			{
				foreach (var entry in cc.DEGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DEGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DEGroupMap.MergeFrom(cis);
			Patch(updating.DEGroupMap);
			updating.DEGroupSortedKeys = genSortedKeys(updating.DEGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DEGroupConf();
            tmp.MergeFrom(cis);
			updating.DEGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DEGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDEGroup> filter,string tag=""){
		...
		public static rawDEGroup MustGet(int key,string tag="") {
		...
		public static rawDEGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDEGroup> GetByFilter(Func<rawDEGroup, bool> filterFunc,string tag="") {
			List<rawDEGroup> ret = new List<rawDEGroup>();
			foreach(KeyValuePair <int, rawDEGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDEGroup? GetOneByFilter(Func<rawDEGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDEGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDEGroup data) 
		{
			return data.Id;			
		}
    }
