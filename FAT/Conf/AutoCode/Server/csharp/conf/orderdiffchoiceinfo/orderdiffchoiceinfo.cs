...
using OrderDiffChoiceInfoConf = fat.rawdata.OrderDiffChoiceInfoConf;
using rawOrderDiffChoiceInfo = fat.rawdata.OrderDiffChoiceInfo;
using rawOrderDiffChoiceInfoMapABValue = fat.rawdata.OrderDiffChoiceInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderDiffChoiceInfoVisitor
    {
		...
		public const string FileName = "OrderDiffChoiceInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderDiffChoiceInfoVisitor.SetABDefault单独设定OrderDiffChoiceInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderDiffChoiceInfoVisitor.SetABFallback单独设定OrderDiffChoiceInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderDiffChoiceInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderDiffChoiceInfo>>(()=> new PBC::MapField<int, rawOrderDiffChoiceInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderDiffChoiceInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderDiffChoiceInfoMap;
			var abMap = v.OrderDiffChoiceInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderDiffChoiceInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderDiffChoiceInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderDiffChoiceInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderDiffChoiceInfoMap;
			if (v.OrderDiffChoiceInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderDiffChoiceInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderDiffChoiceInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderDiffChoiceInfoConf cc) {
			foreach(KeyValuePair <string, rawOrderDiffChoiceInfoMapABValue> tagEntry in cc.OrderDiffChoiceInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderDiffChoiceInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderDiffChoiceInfoMapABValue();
					cc.OrderDiffChoiceInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderDiffChoiceInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderDiffChoiceInfo> entry in cc.OrderDiffChoiceInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderDiffChoiceInfo> entry in tagEntry.Value.OrderDiffChoiceInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderDiffChoiceInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderDiffChoiceInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderDiffChoiceInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderDiffChoiceInfoMapAB != null)
			{
				foreach (var entry in cc.OrderDiffChoiceInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderDiffChoiceInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderDiffChoiceInfoMap.MergeFrom(cis);
			Patch(updating.OrderDiffChoiceInfoMap);
			updating.OrderDiffChoiceInfoSortedKeys = genSortedKeys(updating.OrderDiffChoiceInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderDiffChoiceInfoConf();
            tmp.MergeFrom(cis);
			updating.OrderDiffChoiceInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderDiffChoiceInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderDiffChoiceInfo> filter,string tag=""){
		...
		public static rawOrderDiffChoiceInfo MustGet(int key,string tag="") {
		...
		public static rawOrderDiffChoiceInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderDiffChoiceInfo> GetByFilter(Func<rawOrderDiffChoiceInfo, bool> filterFunc,string tag="") {
			List<rawOrderDiffChoiceInfo> ret = new List<rawOrderDiffChoiceInfo>();
			foreach(KeyValuePair <int, rawOrderDiffChoiceInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderDiffChoiceInfo? GetOneByFilter(Func<rawOrderDiffChoiceInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderDiffChoiceInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderDiffChoiceInfo data) 
		{
			return data.Id;			
		}
    }
