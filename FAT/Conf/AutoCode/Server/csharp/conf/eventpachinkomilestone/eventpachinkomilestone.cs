...
using EventPachinkoMilestoneConf = fat.rawdata.EventPachinkoMilestoneConf;
using rawEventPachinkoMilestone = fat.rawdata.EventPachinkoMilestone;
using rawEventPachinkoMilestoneMapABValue = fat.rawdata.EventPachinkoMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPachinkoMilestoneVisitor
    {
		...
		public const string FileName = "EventPachinkoMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPachinkoMilestoneVisitor.SetABDefault单独设定EventPachinkoMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPachinkoMilestoneVisitor.SetABFallback单独设定EventPachinkoMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPachinkoMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPachinkoMilestone>>(()=> new PBC::MapField<int, rawEventPachinkoMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPachinkoMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPachinkoMilestoneMap;
			var abMap = v.EventPachinkoMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPachinkoMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPachinkoMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPachinkoMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPachinkoMilestoneMap;
			if (v.EventPachinkoMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPachinkoMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPachinkoMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPachinkoMilestoneConf cc) {
			foreach(KeyValuePair <string, rawEventPachinkoMilestoneMapABValue> tagEntry in cc.EventPachinkoMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPachinkoMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPachinkoMilestoneMapABValue();
					cc.EventPachinkoMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPachinkoMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPachinkoMilestone> entry in cc.EventPachinkoMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPachinkoMilestone> entry in tagEntry.Value.EventPachinkoMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPachinkoMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPachinkoMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPachinkoMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPachinkoMilestoneMapAB != null)
			{
				foreach (var entry in cc.EventPachinkoMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPachinkoMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPachinkoMilestoneMap.MergeFrom(cis);
			Patch(updating.EventPachinkoMilestoneMap);
			updating.EventPachinkoMilestoneSortedKeys = genSortedKeys(updating.EventPachinkoMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPachinkoMilestoneConf();
            tmp.MergeFrom(cis);
			updating.EventPachinkoMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPachinkoMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPachinkoMilestone> filter,string tag=""){
		...
		public static rawEventPachinkoMilestone MustGet(int key,string tag="") {
		...
		public static rawEventPachinkoMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPachinkoMilestone> GetByFilter(Func<rawEventPachinkoMilestone, bool> filterFunc,string tag="") {
			List<rawEventPachinkoMilestone> ret = new List<rawEventPachinkoMilestone>();
			foreach(KeyValuePair <int, rawEventPachinkoMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPachinkoMilestone? GetOneByFilter(Func<rawEventPachinkoMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPachinkoMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPachinkoMilestone data) 
		{
			return data.Id;			
		}
    }
