...
using ErgListDetailConf = fat.rawdata.ErgListDetailConf;
using rawErgListDetail = fat.rawdata.ErgListDetail;
using rawErgListDetailMapABValue = fat.rawdata.ErgListDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ErgListDetailVisitor
    {
		...
		public const string FileName = "ErgListDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ErgListDetailVisitor.SetABDefault单独设定ErgListDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ErgListDetailVisitor.SetABFallback单独设定ErgListDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawErgListDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawErgListDetail>>(()=> new PBC::MapField<int, rawErgListDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawErgListDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ErgListDetailMap;
			var abMap = v.ErgListDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ErgListDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ErgListDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawErgListDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ErgListDetailMap;
			if (v.ErgListDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ErgListDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawErgListDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ErgListDetailConf cc) {
			foreach(KeyValuePair <string, rawErgListDetailMapABValue> tagEntry in cc.ErgListDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ErgListDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawErgListDetailMapABValue();
					cc.ErgListDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.ErgListDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawErgListDetail> entry in cc.ErgListDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawErgListDetail> entry in tagEntry.Value.ErgListDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ErgListDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ErgListDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ErgListDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ErgListDetailMapAB != null)
			{
				foreach (var entry in cc.ErgListDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ErgListDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ErgListDetailMap.MergeFrom(cis);
			Patch(updating.ErgListDetailMap);
			updating.ErgListDetailSortedKeys = genSortedKeys(updating.ErgListDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ErgListDetailConf();
            tmp.MergeFrom(cis);
			updating.ErgListDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ErgListDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawErgListDetail> filter,string tag=""){
		...
		public static rawErgListDetail MustGet(int key,string tag="") {
		...
		public static rawErgListDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawErgListDetail> GetByFilter(Func<rawErgListDetail, bool> filterFunc,string tag="") {
			List<rawErgListDetail> ret = new List<rawErgListDetail>();
			foreach(KeyValuePair <int, rawErgListDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawErgListDetail? GetOneByFilter(Func<rawErgListDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawErgListDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawErgListDetail data) 
		{
			return data.Id;			
		}
    }
