...
using MarketWeightConf = fat.rawdata.MarketWeightConf;
using rawMarketWeight = fat.rawdata.MarketWeight;
using rawMarketWeightMapABValue = fat.rawdata.MarketWeightMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketWeightVisitor
    {
		...
		public const string FileName = "MarketWeightConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketWeightVisitor.SetABDefault单独设定MarketWeight的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketWeightVisitor.SetABFallback单独设定MarketWeight的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketWeight>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketWeight>>(()=> new PBC::MapField<int, rawMarketWeight>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketWeight>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketWeightMap;
			var abMap = v.MarketWeightMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketWeightMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketWeightMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketWeight> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketWeightMap;
			if (v.MarketWeightMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketWeightMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketWeightMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketWeightConf cc) {
			foreach(KeyValuePair <string, rawMarketWeightMapABValue> tagEntry in cc.MarketWeightMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketWeightMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketWeightMapABValue();
					cc.MarketWeightMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketWeightMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketWeight> entry in cc.MarketWeightMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketWeight> entry in tagEntry.Value.MarketWeightMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketWeight> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketWeightConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketWeightMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketWeightMapAB != null)
			{
				foreach (var entry in cc.MarketWeightMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketWeightMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketWeightMap.MergeFrom(cis);
			Patch(updating.MarketWeightMap);
			updating.MarketWeightSortedKeys = genSortedKeys(updating.MarketWeightMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketWeightConf();
            tmp.MergeFrom(cis);
			updating.MarketWeightMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketWeightSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketWeight> filter,string tag=""){
		...
		public static rawMarketWeight MustGet(int key,string tag="") {
		...
		public static rawMarketWeight? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketWeight> GetByFilter(Func<rawMarketWeight, bool> filterFunc,string tag="") {
			List<rawMarketWeight> ret = new List<rawMarketWeight>();
			foreach(KeyValuePair <int, rawMarketWeight> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketWeight? GetOneByFilter(Func<rawMarketWeight, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketWeight> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketWeight data) 
		{
			return data.Id;			
		}
    }
