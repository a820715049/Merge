...
using EventScoreDuelDetailConf = fat.rawdata.EventScoreDuelDetailConf;
using rawEventScoreDuelDetail = fat.rawdata.EventScoreDuelDetail;
using rawEventScoreDuelDetailMapABValue = fat.rawdata.EventScoreDuelDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventScoreDuelDetailVisitor
    {
		...
		public const string FileName = "EventScoreDuelDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventScoreDuelDetailVisitor.SetABDefault单独设定EventScoreDuelDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventScoreDuelDetailVisitor.SetABFallback单独设定EventScoreDuelDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventScoreDuelDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventScoreDuelDetail>>(()=> new PBC::MapField<int, rawEventScoreDuelDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventScoreDuelDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventScoreDuelDetailMap;
			var abMap = v.EventScoreDuelDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventScoreDuelDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventScoreDuelDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventScoreDuelDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventScoreDuelDetailMap;
			if (v.EventScoreDuelDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventScoreDuelDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventScoreDuelDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventScoreDuelDetailConf cc) {
			foreach(KeyValuePair <string, rawEventScoreDuelDetailMapABValue> tagEntry in cc.EventScoreDuelDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventScoreDuelDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventScoreDuelDetailMapABValue();
					cc.EventScoreDuelDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventScoreDuelDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventScoreDuelDetail> entry in cc.EventScoreDuelDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventScoreDuelDetail> entry in tagEntry.Value.EventScoreDuelDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventScoreDuelDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventScoreDuelDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventScoreDuelDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventScoreDuelDetailMapAB != null)
			{
				foreach (var entry in cc.EventScoreDuelDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventScoreDuelDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventScoreDuelDetailMap.MergeFrom(cis);
			Patch(updating.EventScoreDuelDetailMap);
			updating.EventScoreDuelDetailSortedKeys = genSortedKeys(updating.EventScoreDuelDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventScoreDuelDetailConf();
            tmp.MergeFrom(cis);
			updating.EventScoreDuelDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventScoreDuelDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventScoreDuelDetail> filter,string tag=""){
		...
		public static rawEventScoreDuelDetail MustGet(int key,string tag="") {
		...
		public static rawEventScoreDuelDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventScoreDuelDetail> GetByFilter(Func<rawEventScoreDuelDetail, bool> filterFunc,string tag="") {
			List<rawEventScoreDuelDetail> ret = new List<rawEventScoreDuelDetail>();
			foreach(KeyValuePair <int, rawEventScoreDuelDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventScoreDuelDetail? GetOneByFilter(Func<rawEventScoreDuelDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventScoreDuelDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventScoreDuelDetail data) 
		{
			return data.Id;			
		}
    }
