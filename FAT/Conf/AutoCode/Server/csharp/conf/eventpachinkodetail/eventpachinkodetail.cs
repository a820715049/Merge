...
using EventPachinkoDetailConf = fat.rawdata.EventPachinkoDetailConf;
using rawEventPachinkoDetail = fat.rawdata.EventPachinkoDetail;
using rawEventPachinkoDetailMapABValue = fat.rawdata.EventPachinkoDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPachinkoDetailVisitor
    {
		...
		public const string FileName = "EventPachinkoDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPachinkoDetailVisitor.SetABDefault单独设定EventPachinkoDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPachinkoDetailVisitor.SetABFallback单独设定EventPachinkoDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPachinkoDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPachinkoDetail>>(()=> new PBC::MapField<int, rawEventPachinkoDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPachinkoDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPachinkoDetailMap;
			var abMap = v.EventPachinkoDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPachinkoDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPachinkoDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPachinkoDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPachinkoDetailMap;
			if (v.EventPachinkoDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPachinkoDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPachinkoDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPachinkoDetailConf cc) {
			foreach(KeyValuePair <string, rawEventPachinkoDetailMapABValue> tagEntry in cc.EventPachinkoDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPachinkoDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPachinkoDetailMapABValue();
					cc.EventPachinkoDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPachinkoDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPachinkoDetail> entry in cc.EventPachinkoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPachinkoDetail> entry in tagEntry.Value.EventPachinkoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPachinkoDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPachinkoDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPachinkoDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPachinkoDetailMapAB != null)
			{
				foreach (var entry in cc.EventPachinkoDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPachinkoDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPachinkoDetailMap.MergeFrom(cis);
			Patch(updating.EventPachinkoDetailMap);
			updating.EventPachinkoDetailSortedKeys = genSortedKeys(updating.EventPachinkoDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPachinkoDetailConf();
            tmp.MergeFrom(cis);
			updating.EventPachinkoDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPachinkoDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPachinkoDetail> filter,string tag=""){
		...
		public static rawEventPachinkoDetail MustGet(int key,string tag="") {
		...
		public static rawEventPachinkoDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPachinkoDetail> GetByFilter(Func<rawEventPachinkoDetail, bool> filterFunc,string tag="") {
			List<rawEventPachinkoDetail> ret = new List<rawEventPachinkoDetail>();
			foreach(KeyValuePair <int, rawEventPachinkoDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPachinkoDetail? GetOneByFilter(Func<rawEventPachinkoDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPachinkoDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPachinkoDetail data) 
		{
			return data.Id;			
		}
    }
