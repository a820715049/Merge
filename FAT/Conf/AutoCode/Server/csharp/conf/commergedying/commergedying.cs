...
using ComMergeDyingConf = fat.rawdata.ComMergeDyingConf;
using rawComMergeDying = fat.rawdata.ComMergeDying;
using rawComMergeDyingMapABValue = fat.rawdata.ComMergeDyingMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeDyingVisitor
    {
		...
		public const string FileName = "ComMergeDyingConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeDyingVisitor.SetABDefault单独设定ComMergeDying的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeDyingVisitor.SetABFallback单独设定ComMergeDying的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeDying>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeDying>>(()=> new PBC::MapField<int, rawComMergeDying>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeDying>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeDyingMap;
			var abMap = v.ComMergeDyingMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeDyingMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeDyingMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeDying> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeDyingMap;
			if (v.ComMergeDyingMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeDyingMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeDyingMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeDyingConf cc) {
			foreach(KeyValuePair <string, rawComMergeDyingMapABValue> tagEntry in cc.ComMergeDyingMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeDyingMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeDyingMapABValue();
					cc.ComMergeDyingMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeDyingMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeDying> entry in cc.ComMergeDyingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeDying> entry in tagEntry.Value.ComMergeDyingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeDying> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeDyingConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeDyingMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeDyingMapAB != null)
			{
				foreach (var entry in cc.ComMergeDyingMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeDyingMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeDyingMap.MergeFrom(cis);
			Patch(updating.ComMergeDyingMap);
			updating.ComMergeDyingSortedKeys = genSortedKeys(updating.ComMergeDyingMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeDyingConf();
            tmp.MergeFrom(cis);
			updating.ComMergeDyingMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeDyingSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeDying> filter,string tag=""){
		...
		public static rawComMergeDying MustGet(int key,string tag="") {
		...
		public static rawComMergeDying? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeDying> GetByFilter(Func<rawComMergeDying, bool> filterFunc,string tag="") {
			List<rawComMergeDying> ret = new List<rawComMergeDying>();
			foreach(KeyValuePair <int, rawComMergeDying> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeDying? GetOneByFilter(Func<rawComMergeDying, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeDying> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeDying data) 
		{
			return data.Id;			
		}
    }
