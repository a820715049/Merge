...
using EventRaceRobotIconConf = fat.rawdata.EventRaceRobotIconConf;
using rawEventRaceRobotIcon = fat.rawdata.EventRaceRobotIcon;
using rawEventRaceRobotIconMapABValue = fat.rawdata.EventRaceRobotIconMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceRobotIconVisitor
    {
		...
		public const string FileName = "EventRaceRobotIconConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceRobotIconVisitor.SetABDefault单独设定EventRaceRobotIcon的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceRobotIconVisitor.SetABFallback单独设定EventRaceRobotIcon的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceRobotIcon>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceRobotIcon>>(()=> new PBC::MapField<int, rawEventRaceRobotIcon>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceRobotIcon>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceRobotIconMap;
			var abMap = v.EventRaceRobotIconMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceRobotIconMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceRobotIconMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceRobotIcon> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceRobotIconMap;
			if (v.EventRaceRobotIconMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceRobotIconMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceRobotIconMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceRobotIconConf cc) {
			foreach(KeyValuePair <string, rawEventRaceRobotIconMapABValue> tagEntry in cc.EventRaceRobotIconMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceRobotIconMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceRobotIconMapABValue();
					cc.EventRaceRobotIconMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceRobotIconMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceRobotIcon> entry in cc.EventRaceRobotIconMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceRobotIcon> entry in tagEntry.Value.EventRaceRobotIconMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceRobotIcon> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceRobotIconConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceRobotIconMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceRobotIconMapAB != null)
			{
				foreach (var entry in cc.EventRaceRobotIconMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceRobotIconMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceRobotIconMap.MergeFrom(cis);
			Patch(updating.EventRaceRobotIconMap);
			updating.EventRaceRobotIconSortedKeys = genSortedKeys(updating.EventRaceRobotIconMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceRobotIconConf();
            tmp.MergeFrom(cis);
			updating.EventRaceRobotIconMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceRobotIconSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceRobotIcon> filter,string tag=""){
		...
		public static rawEventRaceRobotIcon MustGet(int key,string tag="") {
		...
		public static rawEventRaceRobotIcon? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceRobotIcon> GetByFilter(Func<rawEventRaceRobotIcon, bool> filterFunc,string tag="") {
			List<rawEventRaceRobotIcon> ret = new List<rawEventRaceRobotIcon>();
			foreach(KeyValuePair <int, rawEventRaceRobotIcon> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceRobotIcon? GetOneByFilter(Func<rawEventRaceRobotIcon, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceRobotIcon> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceRobotIcon data) 
		{
			return data.Id;			
		}
    }
