...
using MiniGameSlideMergeItemConf = fat.rawdata.MiniGameSlideMergeItemConf;
using rawMiniGameSlideMergeItem = fat.rawdata.MiniGameSlideMergeItem;
using rawMiniGameSlideMergeItemMapABValue = fat.rawdata.MiniGameSlideMergeItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameSlideMergeItemVisitor
    {
		...
		public const string FileName = "MiniGameSlideMergeItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameSlideMergeItemVisitor.SetABDefault单独设定MiniGameSlideMergeItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameSlideMergeItemVisitor.SetABFallback单独设定MiniGameSlideMergeItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameSlideMergeItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameSlideMergeItem>>(()=> new PBC::MapField<int, rawMiniGameSlideMergeItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameSlideMergeItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameSlideMergeItemMap;
			var abMap = v.MiniGameSlideMergeItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameSlideMergeItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameSlideMergeItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameSlideMergeItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameSlideMergeItemMap;
			if (v.MiniGameSlideMergeItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameSlideMergeItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameSlideMergeItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameSlideMergeItemConf cc) {
			foreach(KeyValuePair <string, rawMiniGameSlideMergeItemMapABValue> tagEntry in cc.MiniGameSlideMergeItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameSlideMergeItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameSlideMergeItemMapABValue();
					cc.MiniGameSlideMergeItemMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameSlideMergeItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeItem> entry in cc.MiniGameSlideMergeItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeItem> entry in tagEntry.Value.MiniGameSlideMergeItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameSlideMergeItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameSlideMergeItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameSlideMergeItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameSlideMergeItemMapAB != null)
			{
				foreach (var entry in cc.MiniGameSlideMergeItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameSlideMergeItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameSlideMergeItemMap.MergeFrom(cis);
			Patch(updating.MiniGameSlideMergeItemMap);
			updating.MiniGameSlideMergeItemSortedKeys = genSortedKeys(updating.MiniGameSlideMergeItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameSlideMergeItemConf();
            tmp.MergeFrom(cis);
			updating.MiniGameSlideMergeItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameSlideMergeItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameSlideMergeItem> filter,string tag=""){
		...
		public static rawMiniGameSlideMergeItem MustGet(int key,string tag="") {
		...
		public static rawMiniGameSlideMergeItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameSlideMergeItem> GetByFilter(Func<rawMiniGameSlideMergeItem, bool> filterFunc,string tag="") {
			List<rawMiniGameSlideMergeItem> ret = new List<rawMiniGameSlideMergeItem>();
			foreach(KeyValuePair <int, rawMiniGameSlideMergeItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameSlideMergeItem? GetOneByFilter(Func<rawMiniGameSlideMergeItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameSlideMergeItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameSlideMergeItem data) 
		{
			return data.Id;			
		}
    }
