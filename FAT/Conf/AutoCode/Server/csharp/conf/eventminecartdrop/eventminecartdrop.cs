...
using EventMineCartDropConf = fat.rawdata.EventMineCartDropConf;
using rawEventMineCartDrop = fat.rawdata.EventMineCartDrop;
using rawEventMineCartDropMapABValue = fat.rawdata.EventMineCartDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartDropVisitor
    {
		...
		public const string FileName = "EventMineCartDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartDropVisitor.SetABDefault单独设定EventMineCartDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartDropVisitor.SetABFallback单独设定EventMineCartDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartDrop>>(()=> new PBC::MapField<int, rawEventMineCartDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartDropMap;
			var abMap = v.EventMineCartDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartDropMap;
			if (v.EventMineCartDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartDropConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartDropMapABValue> tagEntry in cc.EventMineCartDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartDropMapABValue();
					cc.EventMineCartDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartDrop> entry in cc.EventMineCartDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartDrop> entry in tagEntry.Value.EventMineCartDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartDropMapAB != null)
			{
				foreach (var entry in cc.EventMineCartDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartDropMap.MergeFrom(cis);
			Patch(updating.EventMineCartDropMap);
			updating.EventMineCartDropSortedKeys = genSortedKeys(updating.EventMineCartDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartDropConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartDrop> filter,string tag=""){
		...
		public static rawEventMineCartDrop MustGet(int key,string tag="") {
		...
		public static rawEventMineCartDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartDrop> GetByFilter(Func<rawEventMineCartDrop, bool> filterFunc,string tag="") {
			List<rawEventMineCartDrop> ret = new List<rawEventMineCartDrop>();
			foreach(KeyValuePair <int, rawEventMineCartDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartDrop? GetOneByFilter(Func<rawEventMineCartDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartDrop data) 
		{
			return data.Id;			
		}
    }
