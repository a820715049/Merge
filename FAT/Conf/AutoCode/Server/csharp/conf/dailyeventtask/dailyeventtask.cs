...
using DailyEventTaskConf = fat.rawdata.DailyEventTaskConf;
using rawDailyEventTask = fat.rawdata.DailyEventTask;
using rawDailyEventTaskMapABValue = fat.rawdata.DailyEventTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DailyEventTaskVisitor
    {
		...
		public const string FileName = "DailyEventTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DailyEventTaskVisitor.SetABDefault单独设定DailyEventTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DailyEventTaskVisitor.SetABFallback单独设定DailyEventTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDailyEventTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDailyEventTask>>(()=> new PBC::MapField<int, rawDailyEventTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDailyEventTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DailyEventTaskMap;
			var abMap = v.DailyEventTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DailyEventTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DailyEventTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDailyEventTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DailyEventTaskMap;
			if (v.DailyEventTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DailyEventTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDailyEventTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DailyEventTaskConf cc) {
			foreach(KeyValuePair <string, rawDailyEventTaskMapABValue> tagEntry in cc.DailyEventTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DailyEventTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDailyEventTaskMapABValue();
					cc.DailyEventTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.DailyEventTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDailyEventTask> entry in cc.DailyEventTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDailyEventTask> entry in tagEntry.Value.DailyEventTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DailyEventTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DailyEventTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DailyEventTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DailyEventTaskMapAB != null)
			{
				foreach (var entry in cc.DailyEventTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DailyEventTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DailyEventTaskMap.MergeFrom(cis);
			Patch(updating.DailyEventTaskMap);
			updating.DailyEventTaskSortedKeys = genSortedKeys(updating.DailyEventTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DailyEventTaskConf();
            tmp.MergeFrom(cis);
			updating.DailyEventTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DailyEventTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDailyEventTask> filter,string tag=""){
		...
		public static rawDailyEventTask MustGet(int key,string tag="") {
		...
		public static rawDailyEventTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDailyEventTask> GetByFilter(Func<rawDailyEventTask, bool> filterFunc,string tag="") {
			List<rawDailyEventTask> ret = new List<rawDailyEventTask>();
			foreach(KeyValuePair <int, rawDailyEventTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDailyEventTask? GetOneByFilter(Func<rawDailyEventTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDailyEventTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDailyEventTask data) 
		{
			return data.Id;			
		}
    }
