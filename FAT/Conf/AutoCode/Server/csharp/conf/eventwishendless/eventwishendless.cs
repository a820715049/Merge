...
using EventWishEndlessConf = fat.rawdata.EventWishEndlessConf;
using rawEventWishEndless = fat.rawdata.EventWishEndless;
using rawEventWishEndlessMapABValue = fat.rawdata.EventWishEndlessMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishEndlessVisitor
    {
		...
		public const string FileName = "EventWishEndlessConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishEndlessVisitor.SetABDefault单独设定EventWishEndless的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishEndlessVisitor.SetABFallback单独设定EventWishEndless的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishEndless>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishEndless>>(()=> new PBC::MapField<int, rawEventWishEndless>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishEndless>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishEndlessMap;
			var abMap = v.EventWishEndlessMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishEndlessMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishEndlessMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishEndless> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishEndlessMap;
			if (v.EventWishEndlessMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishEndlessMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishEndlessMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishEndlessConf cc) {
			foreach(KeyValuePair <string, rawEventWishEndlessMapABValue> tagEntry in cc.EventWishEndlessMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishEndlessMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishEndlessMapABValue();
					cc.EventWishEndlessMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishEndlessMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishEndless> entry in cc.EventWishEndlessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishEndless> entry in tagEntry.Value.EventWishEndlessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishEndless> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishEndlessConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishEndlessMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishEndlessMapAB != null)
			{
				foreach (var entry in cc.EventWishEndlessMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishEndlessMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishEndlessMap.MergeFrom(cis);
			Patch(updating.EventWishEndlessMap);
			updating.EventWishEndlessSortedKeys = genSortedKeys(updating.EventWishEndlessMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishEndlessConf();
            tmp.MergeFrom(cis);
			updating.EventWishEndlessMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishEndlessSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishEndless> filter,string tag=""){
		...
		public static rawEventWishEndless MustGet(int key,string tag="") {
		...
		public static rawEventWishEndless? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishEndless> GetByFilter(Func<rawEventWishEndless, bool> filterFunc,string tag="") {
			List<rawEventWishEndless> ret = new List<rawEventWishEndless>();
			foreach(KeyValuePair <int, rawEventWishEndless> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishEndless? GetOneByFilter(Func<rawEventWishEndless, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishEndless> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishEndless data) 
		{
			return data.Id;			
		}
    }
