...
using RandomRewardConf = fat.rawdata.RandomRewardConf;
using rawRandomReward = fat.rawdata.RandomReward;
using rawRandomRewardMapABValue = fat.rawdata.RandomRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RandomRewardVisitor
    {
		...
		public const string FileName = "RandomRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RandomRewardVisitor.SetABDefault单独设定RandomReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RandomRewardVisitor.SetABFallback单独设定RandomReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRandomReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRandomReward>>(()=> new PBC::MapField<int, rawRandomReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRandomReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RandomRewardMap;
			var abMap = v.RandomRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RandomRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RandomRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRandomReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RandomRewardMap;
			if (v.RandomRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RandomRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRandomRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RandomRewardConf cc) {
			foreach(KeyValuePair <string, rawRandomRewardMapABValue> tagEntry in cc.RandomRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RandomRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRandomRewardMapABValue();
					cc.RandomRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.RandomRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRandomReward> entry in cc.RandomRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRandomReward> entry in tagEntry.Value.RandomRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RandomReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RandomRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RandomRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RandomRewardMapAB != null)
			{
				foreach (var entry in cc.RandomRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RandomRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RandomRewardMap.MergeFrom(cis);
			Patch(updating.RandomRewardMap);
			updating.RandomRewardSortedKeys = genSortedKeys(updating.RandomRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RandomRewardConf();
            tmp.MergeFrom(cis);
			updating.RandomRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RandomRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRandomReward> filter,string tag=""){
		...
		public static rawRandomReward MustGet(int key,string tag="") {
		...
		public static rawRandomReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRandomReward> GetByFilter(Func<rawRandomReward, bool> filterFunc,string tag="") {
			List<rawRandomReward> ret = new List<rawRandomReward>();
			foreach(KeyValuePair <int, rawRandomReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRandomReward? GetOneByFilter(Func<rawRandomReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRandomReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRandomReward data) 
		{
			return data.Id;			
		}
    }
