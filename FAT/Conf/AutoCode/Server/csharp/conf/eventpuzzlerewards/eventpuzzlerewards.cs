...
using EventPuzzleRewardsConf = fat.rawdata.EventPuzzleRewardsConf;
using rawEventPuzzleRewards = fat.rawdata.EventPuzzleRewards;
using rawEventPuzzleRewardsMapABValue = fat.rawdata.EventPuzzleRewardsMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPuzzleRewardsVisitor
    {
		...
		public const string FileName = "EventPuzzleRewardsConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPuzzleRewardsVisitor.SetABDefault单独设定EventPuzzleRewards的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPuzzleRewardsVisitor.SetABFallback单独设定EventPuzzleRewards的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPuzzleRewards>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPuzzleRewards>>(()=> new PBC::MapField<int, rawEventPuzzleRewards>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPuzzleRewards>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPuzzleRewardsMap;
			var abMap = v.EventPuzzleRewardsMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPuzzleRewardsMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPuzzleRewardsMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPuzzleRewards> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPuzzleRewardsMap;
			if (v.EventPuzzleRewardsMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPuzzleRewardsMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPuzzleRewardsMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPuzzleRewardsConf cc) {
			foreach(KeyValuePair <string, rawEventPuzzleRewardsMapABValue> tagEntry in cc.EventPuzzleRewardsMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPuzzleRewardsMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPuzzleRewardsMapABValue();
					cc.EventPuzzleRewardsMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPuzzleRewardsMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPuzzleRewards> entry in cc.EventPuzzleRewardsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPuzzleRewards> entry in tagEntry.Value.EventPuzzleRewardsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPuzzleRewards> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPuzzleRewardsConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPuzzleRewardsMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPuzzleRewardsMapAB != null)
			{
				foreach (var entry in cc.EventPuzzleRewardsMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPuzzleRewardsMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPuzzleRewardsMap.MergeFrom(cis);
			Patch(updating.EventPuzzleRewardsMap);
			updating.EventPuzzleRewardsSortedKeys = genSortedKeys(updating.EventPuzzleRewardsMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPuzzleRewardsConf();
            tmp.MergeFrom(cis);
			updating.EventPuzzleRewardsMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPuzzleRewardsSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPuzzleRewards> filter,string tag=""){
		...
		public static rawEventPuzzleRewards MustGet(int key,string tag="") {
		...
		public static rawEventPuzzleRewards? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPuzzleRewards> GetByFilter(Func<rawEventPuzzleRewards, bool> filterFunc,string tag="") {
			List<rawEventPuzzleRewards> ret = new List<rawEventPuzzleRewards>();
			foreach(KeyValuePair <int, rawEventPuzzleRewards> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPuzzleRewards? GetOneByFilter(Func<rawEventPuzzleRewards, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPuzzleRewards> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPuzzleRewards data) 
		{
			return data.Id;			
		}
    }
