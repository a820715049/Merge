...
using EventTrainMissionConf = fat.rawdata.EventTrainMissionConf;
using rawEventTrainMission = fat.rawdata.EventTrainMission;
using rawEventTrainMissionMapABValue = fat.rawdata.EventTrainMissionMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTrainMissionVisitor
    {
		...
		public const string FileName = "EventTrainMissionConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTrainMissionVisitor.SetABDefault单独设定EventTrainMission的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTrainMissionVisitor.SetABFallback单独设定EventTrainMission的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTrainMission>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTrainMission>>(()=> new PBC::MapField<int, rawEventTrainMission>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTrainMission>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTrainMissionMap;
			var abMap = v.EventTrainMissionMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTrainMissionMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTrainMissionMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTrainMission> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTrainMissionMap;
			if (v.EventTrainMissionMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTrainMissionMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTrainMissionMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTrainMissionConf cc) {
			foreach(KeyValuePair <string, rawEventTrainMissionMapABValue> tagEntry in cc.EventTrainMissionMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTrainMissionMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTrainMissionMapABValue();
					cc.EventTrainMissionMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTrainMissionMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTrainMission> entry in cc.EventTrainMissionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTrainMission> entry in tagEntry.Value.EventTrainMissionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTrainMission> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTrainMissionConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTrainMissionMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTrainMissionMapAB != null)
			{
				foreach (var entry in cc.EventTrainMissionMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTrainMissionMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTrainMissionMap.MergeFrom(cis);
			Patch(updating.EventTrainMissionMap);
			updating.EventTrainMissionSortedKeys = genSortedKeys(updating.EventTrainMissionMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTrainMissionConf();
            tmp.MergeFrom(cis);
			updating.EventTrainMissionMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTrainMissionSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTrainMission> filter,string tag=""){
		...
		public static rawEventTrainMission MustGet(int key,string tag="") {
		...
		public static rawEventTrainMission? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTrainMission> GetByFilter(Func<rawEventTrainMission, bool> filterFunc,string tag="") {
			List<rawEventTrainMission> ret = new List<rawEventTrainMission>();
			foreach(KeyValuePair <int, rawEventTrainMission> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTrainMission? GetOneByFilter(Func<rawEventTrainMission, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTrainMission> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTrainMission data) 
		{
			return data.Id;			
		}
    }
