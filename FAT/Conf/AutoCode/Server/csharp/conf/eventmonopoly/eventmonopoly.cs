...
using EventMonopolyConf = fat.rawdata.EventMonopolyConf;
using rawEventMonopoly = fat.rawdata.EventMonopoly;
using rawEventMonopolyMapABValue = fat.rawdata.EventMonopolyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMonopolyVisitor
    {
		...
		public const string FileName = "EventMonopolyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMonopolyVisitor.SetABDefault单独设定EventMonopoly的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMonopolyVisitor.SetABFallback单独设定EventMonopoly的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMonopoly>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMonopoly>>(()=> new PBC::MapField<int, rawEventMonopoly>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMonopoly>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMonopolyMap;
			var abMap = v.EventMonopolyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMonopolyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMonopolyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMonopoly> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMonopolyMap;
			if (v.EventMonopolyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMonopolyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMonopolyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMonopolyConf cc) {
			foreach(KeyValuePair <string, rawEventMonopolyMapABValue> tagEntry in cc.EventMonopolyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMonopolyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMonopolyMapABValue();
					cc.EventMonopolyMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMonopolyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMonopoly> entry in cc.EventMonopolyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMonopoly> entry in tagEntry.Value.EventMonopolyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMonopoly> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMonopolyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMonopolyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMonopolyMapAB != null)
			{
				foreach (var entry in cc.EventMonopolyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMonopolyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMonopolyMap.MergeFrom(cis);
			Patch(updating.EventMonopolyMap);
			updating.EventMonopolySortedKeys = genSortedKeys(updating.EventMonopolyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMonopolyConf();
            tmp.MergeFrom(cis);
			updating.EventMonopolyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMonopolySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMonopoly> filter,string tag=""){
		...
		public static rawEventMonopoly MustGet(int key,string tag="") {
		...
		public static rawEventMonopoly? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMonopoly> GetByFilter(Func<rawEventMonopoly, bool> filterFunc,string tag="") {
			List<rawEventMonopoly> ret = new List<rawEventMonopoly>();
			foreach(KeyValuePair <int, rawEventMonopoly> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMonopoly? GetOneByFilter(Func<rawEventMonopoly, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMonopoly> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMonopoly data) 
		{
			return data.Id;			
		}
    }
