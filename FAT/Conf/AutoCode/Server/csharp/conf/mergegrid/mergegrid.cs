...
using MergeGridConf = fat.rawdata.MergeGridConf;
using rawMergeGrid = fat.rawdata.MergeGrid;
using rawMergeGridMapABValue = fat.rawdata.MergeGridMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeGridVisitor
    {
		...
		public const string FileName = "MergeGridConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeGridVisitor.SetABDefault单独设定MergeGrid的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeGridVisitor.SetABFallback单独设定MergeGrid的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeGrid>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeGrid>>(()=> new PBC::MapField<int, rawMergeGrid>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeGrid>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeGridMap;
			var abMap = v.MergeGridMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeGridMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeGridMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeGrid> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeGridMap;
			if (v.MergeGridMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeGridMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeGridMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeGridConf cc) {
			foreach(KeyValuePair <string, rawMergeGridMapABValue> tagEntry in cc.MergeGridMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeGridMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeGridMapABValue();
					cc.MergeGridMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeGridMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeGrid> entry in cc.MergeGridMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeGrid> entry in tagEntry.Value.MergeGridMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeGrid> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeGridConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeGridMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeGridMapAB != null)
			{
				foreach (var entry in cc.MergeGridMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeGridMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeGridMap.MergeFrom(cis);
			Patch(updating.MergeGridMap);
			updating.MergeGridSortedKeys = genSortedKeys(updating.MergeGridMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeGridConf();
            tmp.MergeFrom(cis);
			updating.MergeGridMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeGridSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeGrid> filter,string tag=""){
		...
		public static rawMergeGrid MustGet(int key,string tag="") {
		...
		public static rawMergeGrid? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeGrid> GetByFilter(Func<rawMergeGrid, bool> filterFunc,string tag="") {
			List<rawMergeGrid> ret = new List<rawMergeGrid>();
			foreach(KeyValuePair <int, rawMergeGrid> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeGrid? GetOneByFilter(Func<rawMergeGrid, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeGrid> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeGrid data) 
		{
			return data.Id;			
		}
    }
