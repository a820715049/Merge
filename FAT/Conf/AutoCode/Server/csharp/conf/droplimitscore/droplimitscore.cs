...
using DropLimitScoreConf = fat.rawdata.DropLimitScoreConf;
using rawDropLimitScore = fat.rawdata.DropLimitScore;
using rawDropLimitScoreMapABValue = fat.rawdata.DropLimitScoreMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DropLimitScoreVisitor
    {
		...
		public const string FileName = "DropLimitScoreConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DropLimitScoreVisitor.SetABDefault单独设定DropLimitScore的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DropLimitScoreVisitor.SetABFallback单独设定DropLimitScore的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDropLimitScore>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDropLimitScore>>(()=> new PBC::MapField<int, rawDropLimitScore>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDropLimitScore>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DropLimitScoreMap;
			var abMap = v.DropLimitScoreMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DropLimitScoreMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DropLimitScoreMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDropLimitScore> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DropLimitScoreMap;
			if (v.DropLimitScoreMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DropLimitScoreMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDropLimitScoreMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DropLimitScoreConf cc) {
			foreach(KeyValuePair <string, rawDropLimitScoreMapABValue> tagEntry in cc.DropLimitScoreMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DropLimitScoreMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDropLimitScoreMapABValue();
					cc.DropLimitScoreMapAB[tag] = ps;
				}
                var tagTmp = ps.DropLimitScoreMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDropLimitScore> entry in cc.DropLimitScoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDropLimitScore> entry in tagEntry.Value.DropLimitScoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DropLimitScore> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DropLimitScoreConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DropLimitScoreMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DropLimitScoreMapAB != null)
			{
				foreach (var entry in cc.DropLimitScoreMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DropLimitScoreMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DropLimitScoreMap.MergeFrom(cis);
			Patch(updating.DropLimitScoreMap);
			updating.DropLimitScoreSortedKeys = genSortedKeys(updating.DropLimitScoreMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DropLimitScoreConf();
            tmp.MergeFrom(cis);
			updating.DropLimitScoreMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DropLimitScoreSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDropLimitScore> filter,string tag=""){
		...
		public static rawDropLimitScore MustGet(int key,string tag="") {
		...
		public static rawDropLimitScore? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDropLimitScore> GetByFilter(Func<rawDropLimitScore, bool> filterFunc,string tag="") {
			List<rawDropLimitScore> ret = new List<rawDropLimitScore>();
			foreach(KeyValuePair <int, rawDropLimitScore> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDropLimitScore? GetOneByFilter(Func<rawDropLimitScore, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDropLimitScore> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDropLimitScore data) 
		{
			return data.Id;			
		}
    }
