...
using AdSettingConf = fat.rawdata.AdSettingConf;
using rawAdSetting = fat.rawdata.AdSetting;
using rawAdSettingMapABValue = fat.rawdata.AdSettingMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class AdSettingVisitor
    {
		...
		public const string FileName = "AdSettingConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过AdSettingVisitor.SetABDefault单独设定AdSetting的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过AdSettingVisitor.SetABFallback单独设定AdSetting的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawAdSetting>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawAdSetting>>(()=> new PBC::MapField<int, rawAdSetting>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawAdSetting>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().AdSettingMap;
			var abMap = v.AdSettingMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.AdSettingMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].AdSettingMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawAdSetting> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().AdSettingMap;
			if (v.AdSettingMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.AdSettingMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawAdSettingMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(AdSettingConf cc) {
			foreach(KeyValuePair <string, rawAdSettingMapABValue> tagEntry in cc.AdSettingMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.AdSettingMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawAdSettingMapABValue();
					cc.AdSettingMapAB[tag] = ps;
				}
                var tagTmp = ps.AdSettingMap;
				// 基础数据
				foreach(KeyValuePair <int, rawAdSetting> entry in cc.AdSettingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawAdSetting> entry in tagEntry.Value.AdSettingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.AdSetting> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(AdSettingConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.AdSettingMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.AdSettingMapAB != null)
			{
				foreach (var entry in cc.AdSettingMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.AdSettingMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.AdSettingMap.MergeFrom(cis);
			Patch(updating.AdSettingMap);
			updating.AdSettingSortedKeys = genSortedKeys(updating.AdSettingMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new AdSettingConf();
            tmp.MergeFrom(cis);
			updating.AdSettingMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().AdSettingSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawAdSetting> filter,string tag=""){
		...
		public static rawAdSetting MustGet(int key,string tag="") {
		...
		public static rawAdSetting? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawAdSetting> GetByFilter(Func<rawAdSetting, bool> filterFunc,string tag="") {
			List<rawAdSetting> ret = new List<rawAdSetting>();
			foreach(KeyValuePair <int, rawAdSetting> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawAdSetting? GetOneByFilter(Func<rawAdSetting, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawAdSetting> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawAdSetting data) 
		{
			return data.Id;			
		}
    }
