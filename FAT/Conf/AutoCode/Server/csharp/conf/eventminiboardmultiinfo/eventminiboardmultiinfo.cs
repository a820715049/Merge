...
using EventMiniBoardMultiInfoConf = fat.rawdata.EventMiniBoardMultiInfoConf;
using rawEventMiniBoardMultiInfo = fat.rawdata.EventMiniBoardMultiInfo;
using rawEventMiniBoardMultiInfoMapABValue = fat.rawdata.EventMiniBoardMultiInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardMultiInfoVisitor
    {
		...
		public const string FileName = "EventMiniBoardMultiInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardMultiInfoVisitor.SetABDefault单独设定EventMiniBoardMultiInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardMultiInfoVisitor.SetABFallback单独设定EventMiniBoardMultiInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoardMultiInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoardMultiInfo>>(()=> new PBC::MapField<int, rawEventMiniBoardMultiInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoardMultiInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardMultiInfoMap;
			var abMap = v.EventMiniBoardMultiInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardMultiInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardMultiInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoardMultiInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardMultiInfoMap;
			if (v.EventMiniBoardMultiInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardMultiInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardMultiInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardMultiInfoConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardMultiInfoMapABValue> tagEntry in cc.EventMiniBoardMultiInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardMultiInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardMultiInfoMapABValue();
					cc.EventMiniBoardMultiInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardMultiInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoardMultiInfo> entry in cc.EventMiniBoardMultiInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoardMultiInfo> entry in tagEntry.Value.EventMiniBoardMultiInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoardMultiInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardMultiInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardMultiInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardMultiInfoMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardMultiInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardMultiInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardMultiInfoMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardMultiInfoMap);
			updating.EventMiniBoardMultiInfoSortedKeys = genSortedKeys(updating.EventMiniBoardMultiInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardMultiInfoConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardMultiInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardMultiInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoardMultiInfo> filter,string tag=""){
		...
		public static rawEventMiniBoardMultiInfo MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoardMultiInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoardMultiInfo> GetByFilter(Func<rawEventMiniBoardMultiInfo, bool> filterFunc,string tag="") {
			List<rawEventMiniBoardMultiInfo> ret = new List<rawEventMiniBoardMultiInfo>();
			foreach(KeyValuePair <int, rawEventMiniBoardMultiInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoardMultiInfo? GetOneByFilter(Func<rawEventMiniBoardMultiInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoardMultiInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoardMultiInfo data) 
		{
			return data.Id;			
		}
    }
