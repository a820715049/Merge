...
using EventMineConf = fat.rawdata.EventMineConf;
using rawEventMine = fat.rawdata.EventMine;
using rawEventMineMapABValue = fat.rawdata.EventMineMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineVisitor
    {
		...
		public const string FileName = "EventMineConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineVisitor.SetABDefault单独设定EventMine的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineVisitor.SetABFallback单独设定EventMine的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMine>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMine>>(()=> new PBC::MapField<int, rawEventMine>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMine>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineMap;
			var abMap = v.EventMineMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMine> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineMap;
			if (v.EventMineMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineConf cc) {
			foreach(KeyValuePair <string, rawEventMineMapABValue> tagEntry in cc.EventMineMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineMapABValue();
					cc.EventMineMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMine> entry in cc.EventMineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMine> entry in tagEntry.Value.EventMineMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMine> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineMapAB != null)
			{
				foreach (var entry in cc.EventMineMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineMap.MergeFrom(cis);
			Patch(updating.EventMineMap);
			updating.EventMineSortedKeys = genSortedKeys(updating.EventMineMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineConf();
            tmp.MergeFrom(cis);
			updating.EventMineMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMine> filter,string tag=""){
		...
		public static rawEventMine MustGet(int key,string tag="") {
		...
		public static rawEventMine? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMine> GetByFilter(Func<rawEventMine, bool> filterFunc,string tag="") {
			List<rawEventMine> ret = new List<rawEventMine>();
			foreach(KeyValuePair <int, rawEventMine> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMine? GetOneByFilter(Func<rawEventMine, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMine> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMine data) 
		{
			return data.Id;			
		}
    }
