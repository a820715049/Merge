...
using MonopolyOrderItemConf = fat.rawdata.MonopolyOrderItemConf;
using rawMonopolyOrderItem = fat.rawdata.MonopolyOrderItem;
using rawMonopolyOrderItemMapABValue = fat.rawdata.MonopolyOrderItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MonopolyOrderItemVisitor
    {
		...
		public const string FileName = "MonopolyOrderItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MonopolyOrderItemVisitor.SetABDefault单独设定MonopolyOrderItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MonopolyOrderItemVisitor.SetABFallback单独设定MonopolyOrderItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMonopolyOrderItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMonopolyOrderItem>>(()=> new PBC::MapField<int, rawMonopolyOrderItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMonopolyOrderItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MonopolyOrderItemMap;
			var abMap = v.MonopolyOrderItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MonopolyOrderItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MonopolyOrderItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMonopolyOrderItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MonopolyOrderItemMap;
			if (v.MonopolyOrderItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MonopolyOrderItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMonopolyOrderItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MonopolyOrderItemConf cc) {
			foreach(KeyValuePair <string, rawMonopolyOrderItemMapABValue> tagEntry in cc.MonopolyOrderItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MonopolyOrderItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMonopolyOrderItemMapABValue();
					cc.MonopolyOrderItemMapAB[tag] = ps;
				}
                var tagTmp = ps.MonopolyOrderItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMonopolyOrderItem> entry in cc.MonopolyOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMonopolyOrderItem> entry in tagEntry.Value.MonopolyOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MonopolyOrderItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MonopolyOrderItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MonopolyOrderItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MonopolyOrderItemMapAB != null)
			{
				foreach (var entry in cc.MonopolyOrderItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MonopolyOrderItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MonopolyOrderItemMap.MergeFrom(cis);
			Patch(updating.MonopolyOrderItemMap);
			updating.MonopolyOrderItemSortedKeys = genSortedKeys(updating.MonopolyOrderItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MonopolyOrderItemConf();
            tmp.MergeFrom(cis);
			updating.MonopolyOrderItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MonopolyOrderItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMonopolyOrderItem> filter,string tag=""){
		...
		public static rawMonopolyOrderItem MustGet(int key,string tag="") {
		...
		public static rawMonopolyOrderItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMonopolyOrderItem> GetByFilter(Func<rawMonopolyOrderItem, bool> filterFunc,string tag="") {
			List<rawMonopolyOrderItem> ret = new List<rawMonopolyOrderItem>();
			foreach(KeyValuePair <int, rawMonopolyOrderItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMonopolyOrderItem? GetOneByFilter(Func<rawMonopolyOrderItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMonopolyOrderItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMonopolyOrderItem data) 
		{
			return data.Id;			
		}
    }
