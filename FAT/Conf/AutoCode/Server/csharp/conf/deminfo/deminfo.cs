...
using DEMInfoConf = fat.rawdata.DEMInfoConf;
using rawDEMInfo = fat.rawdata.DEMInfo;
using rawDEMInfoMapABValue = fat.rawdata.DEMInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DEMInfoVisitor
    {
		...
		public const string FileName = "DEMInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DEMInfoVisitor.SetABDefault单独设定DEMInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DEMInfoVisitor.SetABFallback单独设定DEMInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDEMInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDEMInfo>>(()=> new PBC::MapField<int, rawDEMInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDEMInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DEMInfoMap;
			var abMap = v.DEMInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DEMInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DEMInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDEMInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DEMInfoMap;
			if (v.DEMInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DEMInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDEMInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DEMInfoConf cc) {
			foreach(KeyValuePair <string, rawDEMInfoMapABValue> tagEntry in cc.DEMInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DEMInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDEMInfoMapABValue();
					cc.DEMInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.DEMInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDEMInfo> entry in cc.DEMInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDEMInfo> entry in tagEntry.Value.DEMInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DEMInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DEMInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DEMInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DEMInfoMapAB != null)
			{
				foreach (var entry in cc.DEMInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DEMInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DEMInfoMap.MergeFrom(cis);
			Patch(updating.DEMInfoMap);
			updating.DEMInfoSortedKeys = genSortedKeys(updating.DEMInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DEMInfoConf();
            tmp.MergeFrom(cis);
			updating.DEMInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DEMInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDEMInfo> filter,string tag=""){
		...
		public static rawDEMInfo MustGet(int key,string tag="") {
		...
		public static rawDEMInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDEMInfo> GetByFilter(Func<rawDEMInfo, bool> filterFunc,string tag="") {
			List<rawDEMInfo> ret = new List<rawDEMInfo>();
			foreach(KeyValuePair <int, rawDEMInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDEMInfo? GetOneByFilter(Func<rawDEMInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDEMInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDEMInfo data) 
		{
			return data.Id;			
		}
    }
