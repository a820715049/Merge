...
using UserTagConf = fat.rawdata.UserTagConf;
using rawUserTag = fat.rawdata.UserTag;
using rawUserTagMapABValue = fat.rawdata.UserTagMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class UserTagVisitor
    {
		...
		public const string FileName = "UserTagConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过UserTagVisitor.SetABDefault单独设定UserTag的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过UserTagVisitor.SetABFallback单独设定UserTag的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawUserTag>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawUserTag>>(()=> new PBC::MapField<int, rawUserTag>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawUserTag>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().UserTagMap;
			var abMap = v.UserTagMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.UserTagMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].UserTagMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawUserTag> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().UserTagMap;
			if (v.UserTagMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.UserTagMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawUserTagMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(UserTagConf cc) {
			foreach(KeyValuePair <string, rawUserTagMapABValue> tagEntry in cc.UserTagMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.UserTagMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawUserTagMapABValue();
					cc.UserTagMapAB[tag] = ps;
				}
                var tagTmp = ps.UserTagMap;
				// 基础数据
				foreach(KeyValuePair <int, rawUserTag> entry in cc.UserTagMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawUserTag> entry in tagEntry.Value.UserTagMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.UserTag> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(UserTagConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.UserTagMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.UserTagMapAB != null)
			{
				foreach (var entry in cc.UserTagMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.UserTagMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.UserTagMap.MergeFrom(cis);
			Patch(updating.UserTagMap);
			updating.UserTagSortedKeys = genSortedKeys(updating.UserTagMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new UserTagConf();
            tmp.MergeFrom(cis);
			updating.UserTagMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().UserTagSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawUserTag> filter,string tag=""){
		...
		public static rawUserTag MustGet(int key,string tag="") {
		...
		public static rawUserTag? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawUserTag> GetByFilter(Func<rawUserTag, bool> filterFunc,string tag="") {
			List<rawUserTag> ret = new List<rawUserTag>();
			foreach(KeyValuePair <int, rawUserTag> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawUserTag? GetOneByFilter(Func<rawUserTag, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawUserTag> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawUserTag data) 
		{
			return data.Id;			
		}
    }
