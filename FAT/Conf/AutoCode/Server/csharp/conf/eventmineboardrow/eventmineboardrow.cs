...
using EventMineBoardRowConf = fat.rawdata.EventMineBoardRowConf;
using rawEventMineBoardRow = fat.rawdata.EventMineBoardRow;
using rawEventMineBoardRowMapABValue = fat.rawdata.EventMineBoardRowMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineBoardRowVisitor
    {
		...
		public const string FileName = "EventMineBoardRowConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineBoardRowVisitor.SetABDefault单独设定EventMineBoardRow的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineBoardRowVisitor.SetABFallback单独设定EventMineBoardRow的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineBoardRow>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineBoardRow>>(()=> new PBC::MapField<int, rawEventMineBoardRow>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineBoardRow>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineBoardRowMap;
			var abMap = v.EventMineBoardRowMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineBoardRowMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineBoardRowMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineBoardRow> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineBoardRowMap;
			if (v.EventMineBoardRowMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineBoardRowMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineBoardRowMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineBoardRowConf cc) {
			foreach(KeyValuePair <string, rawEventMineBoardRowMapABValue> tagEntry in cc.EventMineBoardRowMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineBoardRowMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineBoardRowMapABValue();
					cc.EventMineBoardRowMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineBoardRowMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineBoardRow> entry in cc.EventMineBoardRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineBoardRow> entry in tagEntry.Value.EventMineBoardRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineBoardRow> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineBoardRowConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineBoardRowMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineBoardRowMapAB != null)
			{
				foreach (var entry in cc.EventMineBoardRowMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineBoardRowMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineBoardRowMap.MergeFrom(cis);
			Patch(updating.EventMineBoardRowMap);
			updating.EventMineBoardRowSortedKeys = genSortedKeys(updating.EventMineBoardRowMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineBoardRowConf();
            tmp.MergeFrom(cis);
			updating.EventMineBoardRowMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineBoardRowSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineBoardRow> filter,string tag=""){
		...
		public static rawEventMineBoardRow MustGet(int key,string tag="") {
		...
		public static rawEventMineBoardRow? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineBoardRow> GetByFilter(Func<rawEventMineBoardRow, bool> filterFunc,string tag="") {
			List<rawEventMineBoardRow> ret = new List<rawEventMineBoardRow>();
			foreach(KeyValuePair <int, rawEventMineBoardRow> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineBoardRow? GetOneByFilter(Func<rawEventMineBoardRow, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineBoardRow> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineBoardRow data) 
		{
			return data.Id;			
		}
    }
