...
using GroupDetailConf = fat.rawdata.GroupDetailConf;
using rawGroupDetail = fat.rawdata.GroupDetail;
using rawGroupDetailMapABValue = fat.rawdata.GroupDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GroupDetailVisitor
    {
		...
		public const string FileName = "GroupDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GroupDetailVisitor.SetABDefault单独设定GroupDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GroupDetailVisitor.SetABFallback单独设定GroupDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGroupDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGroupDetail>>(()=> new PBC::MapField<int, rawGroupDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGroupDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GroupDetailMap;
			var abMap = v.GroupDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GroupDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GroupDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGroupDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GroupDetailMap;
			if (v.GroupDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GroupDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGroupDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GroupDetailConf cc) {
			foreach(KeyValuePair <string, rawGroupDetailMapABValue> tagEntry in cc.GroupDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GroupDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGroupDetailMapABValue();
					cc.GroupDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.GroupDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGroupDetail> entry in cc.GroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGroupDetail> entry in tagEntry.Value.GroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GroupDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GroupDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GroupDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GroupDetailMapAB != null)
			{
				foreach (var entry in cc.GroupDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GroupDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GroupDetailMap.MergeFrom(cis);
			Patch(updating.GroupDetailMap);
			updating.GroupDetailSortedKeys = genSortedKeys(updating.GroupDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GroupDetailConf();
            tmp.MergeFrom(cis);
			updating.GroupDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GroupDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGroupDetail> filter,string tag=""){
		...
		public static rawGroupDetail MustGet(int key,string tag="") {
		...
		public static rawGroupDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGroupDetail> GetByFilter(Func<rawGroupDetail, bool> filterFunc,string tag="") {
			List<rawGroupDetail> ret = new List<rawGroupDetail>();
			foreach(KeyValuePair <int, rawGroupDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGroupDetail? GetOneByFilter(Func<rawGroupDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGroupDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGroupDetail data) 
		{
			return data.Id;			
		}
    }
