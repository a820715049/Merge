...
using RoundScoreConf = fat.rawdata.RoundScoreConf;
using rawRoundScore = fat.rawdata.RoundScore;
using rawRoundScoreSliceABValue = fat.rawdata.RoundScoreSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RoundScoreVisitor
    {
		...
		public const string FileName = "RoundScoreConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RoundScoreVisitor.SetABDefault单独设定RoundScore的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RoundScoreVisitor.SetABFallback单独设定RoundScore的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawRoundScore>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawRoundScore>>(()=>new PBC::RepeatedField<rawRoundScore>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawRoundScore>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RoundScoreSlice;
			var abMap = v.RoundScoreSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RoundScoreSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RoundScoreSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawRoundScore> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RoundScoreSlice;
			if (v.RoundScoreSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RoundScoreSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRoundScoreSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RoundScoreConf cc) {
			foreach(KeyValuePair <string, rawRoundScoreSliceABValue> tagEntry in cc.RoundScoreSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RoundScoreSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRoundScoreSliceABValue();
					cc.RoundScoreSliceAB[tag] = ps;
				}
                var tagTmp = ps.RoundScoreSlice;
				// 基础数据
				for(int i=0; i<cc.RoundScoreSlice.Count; ++i) 
				{
					tagTmp.Add(cc.RoundScoreSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.RoundScoreSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.RoundScore> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RoundScoreSlice.MergeFrom(cis);
			Patch(updating.RoundScoreSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RoundScoreConf();
            tmp.MergeFrom(cis);
			updating.RoundScoreSlice = tmp;
		...
		public static void Range(Action<int, rawRoundScore> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawRoundScore MustGetByIndex(int index,string tag="") {
		...
		public static rawRoundScore? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawRoundScore> GetByFilter(Func<rawRoundScore,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawRoundScore>();
		...
		static public rawRoundScore? GetOneByFilter(Func<rawRoundScore,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
