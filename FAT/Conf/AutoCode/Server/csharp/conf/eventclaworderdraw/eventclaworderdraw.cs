...
using EventClawOrderDrawConf = fat.rawdata.EventClawOrderDrawConf;
using rawEventClawOrderDraw = fat.rawdata.EventClawOrderDraw;
using rawEventClawOrderDrawMapABValue = fat.rawdata.EventClawOrderDrawMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderDrawVisitor
    {
		...
		public const string FileName = "EventClawOrderDrawConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderDrawVisitor.SetABDefault单独设定EventClawOrderDraw的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderDrawVisitor.SetABFallback单独设定EventClawOrderDraw的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrderDraw>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrderDraw>>(()=> new PBC::MapField<int, rawEventClawOrderDraw>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrderDraw>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderDrawMap;
			var abMap = v.EventClawOrderDrawMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderDrawMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderDrawMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrderDraw> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderDrawMap;
			if (v.EventClawOrderDrawMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderDrawMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderDrawMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderDrawConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderDrawMapABValue> tagEntry in cc.EventClawOrderDrawMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderDrawMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderDrawMapABValue();
					cc.EventClawOrderDrawMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderDrawMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrderDraw> entry in cc.EventClawOrderDrawMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrderDraw> entry in tagEntry.Value.EventClawOrderDrawMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrderDraw> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderDrawConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderDrawMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderDrawMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderDrawMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderDrawMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderDrawMap.MergeFrom(cis);
			Patch(updating.EventClawOrderDrawMap);
			updating.EventClawOrderDrawSortedKeys = genSortedKeys(updating.EventClawOrderDrawMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderDrawConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderDrawMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderDrawSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrderDraw> filter,string tag=""){
		...
		public static rawEventClawOrderDraw MustGet(int key,string tag="") {
		...
		public static rawEventClawOrderDraw? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrderDraw> GetByFilter(Func<rawEventClawOrderDraw, bool> filterFunc,string tag="") {
			List<rawEventClawOrderDraw> ret = new List<rawEventClawOrderDraw>();
			foreach(KeyValuePair <int, rawEventClawOrderDraw> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrderDraw? GetOneByFilter(Func<rawEventClawOrderDraw, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrderDraw> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrderDraw data) 
		{
			return data.Id;			
		}
    }
