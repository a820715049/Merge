...
using ComMergeOrderBoxConf = fat.rawdata.ComMergeOrderBoxConf;
using rawComMergeOrderBox = fat.rawdata.ComMergeOrderBox;
using rawComMergeOrderBoxMapABValue = fat.rawdata.ComMergeOrderBoxMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeOrderBoxVisitor
    {
		...
		public const string FileName = "ComMergeOrderBoxConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeOrderBoxVisitor.SetABDefault单独设定ComMergeOrderBox的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeOrderBoxVisitor.SetABFallback单独设定ComMergeOrderBox的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeOrderBox>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeOrderBox>>(()=> new PBC::MapField<int, rawComMergeOrderBox>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeOrderBox>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeOrderBoxMap;
			var abMap = v.ComMergeOrderBoxMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeOrderBoxMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeOrderBoxMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeOrderBox> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeOrderBoxMap;
			if (v.ComMergeOrderBoxMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeOrderBoxMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeOrderBoxMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeOrderBoxConf cc) {
			foreach(KeyValuePair <string, rawComMergeOrderBoxMapABValue> tagEntry in cc.ComMergeOrderBoxMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeOrderBoxMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeOrderBoxMapABValue();
					cc.ComMergeOrderBoxMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeOrderBoxMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeOrderBox> entry in cc.ComMergeOrderBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeOrderBox> entry in tagEntry.Value.ComMergeOrderBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeOrderBox> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeOrderBoxConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeOrderBoxMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeOrderBoxMapAB != null)
			{
				foreach (var entry in cc.ComMergeOrderBoxMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeOrderBoxMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeOrderBoxMap.MergeFrom(cis);
			Patch(updating.ComMergeOrderBoxMap);
			updating.ComMergeOrderBoxSortedKeys = genSortedKeys(updating.ComMergeOrderBoxMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeOrderBoxConf();
            tmp.MergeFrom(cis);
			updating.ComMergeOrderBoxMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeOrderBoxSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeOrderBox> filter,string tag=""){
		...
		public static rawComMergeOrderBox MustGet(int key,string tag="") {
		...
		public static rawComMergeOrderBox? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeOrderBox> GetByFilter(Func<rawComMergeOrderBox, bool> filterFunc,string tag="") {
			List<rawComMergeOrderBox> ret = new List<rawComMergeOrderBox>();
			foreach(KeyValuePair <int, rawComMergeOrderBox> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeOrderBox? GetOneByFilter(Func<rawComMergeOrderBox, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeOrderBox> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeOrderBox data) 
		{
			return data.Id;			
		}
    }
