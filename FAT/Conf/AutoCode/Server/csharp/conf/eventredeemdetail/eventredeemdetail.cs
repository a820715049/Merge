...
using EventRedeemDetailConf = fat.rawdata.EventRedeemDetailConf;
using rawEventRedeemDetail = fat.rawdata.EventRedeemDetail;
using rawEventRedeemDetailMapABValue = fat.rawdata.EventRedeemDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRedeemDetailVisitor
    {
		...
		public const string FileName = "EventRedeemDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRedeemDetailVisitor.SetABDefault单独设定EventRedeemDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRedeemDetailVisitor.SetABFallback单独设定EventRedeemDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRedeemDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRedeemDetail>>(()=> new PBC::MapField<int, rawEventRedeemDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRedeemDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRedeemDetailMap;
			var abMap = v.EventRedeemDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRedeemDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRedeemDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRedeemDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRedeemDetailMap;
			if (v.EventRedeemDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRedeemDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRedeemDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRedeemDetailConf cc) {
			foreach(KeyValuePair <string, rawEventRedeemDetailMapABValue> tagEntry in cc.EventRedeemDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRedeemDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRedeemDetailMapABValue();
					cc.EventRedeemDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRedeemDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRedeemDetail> entry in cc.EventRedeemDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRedeemDetail> entry in tagEntry.Value.EventRedeemDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRedeemDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRedeemDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRedeemDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRedeemDetailMapAB != null)
			{
				foreach (var entry in cc.EventRedeemDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRedeemDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRedeemDetailMap.MergeFrom(cis);
			Patch(updating.EventRedeemDetailMap);
			updating.EventRedeemDetailSortedKeys = genSortedKeys(updating.EventRedeemDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRedeemDetailConf();
            tmp.MergeFrom(cis);
			updating.EventRedeemDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRedeemDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRedeemDetail> filter,string tag=""){
		...
		public static rawEventRedeemDetail MustGet(int key,string tag="") {
		...
		public static rawEventRedeemDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRedeemDetail> GetByFilter(Func<rawEventRedeemDetail, bool> filterFunc,string tag="") {
			List<rawEventRedeemDetail> ret = new List<rawEventRedeemDetail>();
			foreach(KeyValuePair <int, rawEventRedeemDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRedeemDetail? GetOneByFilter(Func<rawEventRedeemDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRedeemDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRedeemDetail data) 
		{
			return data.Id;			
		}
    }
