...
using EventWishBoardGroupConf = fat.rawdata.EventWishBoardGroupConf;
using rawEventWishBoardGroup = fat.rawdata.EventWishBoardGroup;
using rawEventWishBoardGroupMapABValue = fat.rawdata.EventWishBoardGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishBoardGroupVisitor
    {
		...
		public const string FileName = "EventWishBoardGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishBoardGroupVisitor.SetABDefault单独设定EventWishBoardGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishBoardGroupVisitor.SetABFallback单独设定EventWishBoardGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishBoardGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishBoardGroup>>(()=> new PBC::MapField<int, rawEventWishBoardGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishBoardGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishBoardGroupMap;
			var abMap = v.EventWishBoardGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishBoardGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishBoardGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishBoardGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishBoardGroupMap;
			if (v.EventWishBoardGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishBoardGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishBoardGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishBoardGroupConf cc) {
			foreach(KeyValuePair <string, rawEventWishBoardGroupMapABValue> tagEntry in cc.EventWishBoardGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishBoardGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishBoardGroupMapABValue();
					cc.EventWishBoardGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishBoardGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishBoardGroup> entry in cc.EventWishBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishBoardGroup> entry in tagEntry.Value.EventWishBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishBoardGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishBoardGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishBoardGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishBoardGroupMapAB != null)
			{
				foreach (var entry in cc.EventWishBoardGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishBoardGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishBoardGroupMap.MergeFrom(cis);
			Patch(updating.EventWishBoardGroupMap);
			updating.EventWishBoardGroupSortedKeys = genSortedKeys(updating.EventWishBoardGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishBoardGroupConf();
            tmp.MergeFrom(cis);
			updating.EventWishBoardGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishBoardGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishBoardGroup> filter,string tag=""){
		...
		public static rawEventWishBoardGroup MustGet(int key,string tag="") {
		...
		public static rawEventWishBoardGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishBoardGroup> GetByFilter(Func<rawEventWishBoardGroup, bool> filterFunc,string tag="") {
			List<rawEventWishBoardGroup> ret = new List<rawEventWishBoardGroup>();
			foreach(KeyValuePair <int, rawEventWishBoardGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishBoardGroup? GetOneByFilter(Func<rawEventWishBoardGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishBoardGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishBoardGroup data) 
		{
			return data.Id;			
		}
    }
