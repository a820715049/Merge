...
using EventMineRewardConf = fat.rawdata.EventMineRewardConf;
using rawEventMineReward = fat.rawdata.EventMineReward;
using rawEventMineRewardMapABValue = fat.rawdata.EventMineRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineRewardVisitor
    {
		...
		public const string FileName = "EventMineRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineRewardVisitor.SetABDefault单独设定EventMineReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineRewardVisitor.SetABFallback单独设定EventMineReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineReward>>(()=> new PBC::MapField<int, rawEventMineReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineRewardMap;
			var abMap = v.EventMineRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineRewardMap;
			if (v.EventMineRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineRewardConf cc) {
			foreach(KeyValuePair <string, rawEventMineRewardMapABValue> tagEntry in cc.EventMineRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineRewardMapABValue();
					cc.EventMineRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineReward> entry in cc.EventMineRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineReward> entry in tagEntry.Value.EventMineRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineRewardMapAB != null)
			{
				foreach (var entry in cc.EventMineRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineRewardMap.MergeFrom(cis);
			Patch(updating.EventMineRewardMap);
			updating.EventMineRewardSortedKeys = genSortedKeys(updating.EventMineRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineRewardConf();
            tmp.MergeFrom(cis);
			updating.EventMineRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineReward> filter,string tag=""){
		...
		public static rawEventMineReward MustGet(int key,string tag="") {
		...
		public static rawEventMineReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineReward> GetByFilter(Func<rawEventMineReward, bool> filterFunc,string tag="") {
			List<rawEventMineReward> ret = new List<rawEventMineReward>();
			foreach(KeyValuePair <int, rawEventMineReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineReward? GetOneByFilter(Func<rawEventMineReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineReward data) 
		{
			return data.Id;			
		}
    }
