...
using UserGradeGroupConf = fat.rawdata.UserGradeGroupConf;
using rawUserGradeGroup = fat.rawdata.UserGradeGroup;
using rawUserGradeGroupMapABValue = fat.rawdata.UserGradeGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class UserGradeGroupVisitor
    {
		...
		public const string FileName = "UserGradeGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过UserGradeGroupVisitor.SetABDefault单独设定UserGradeGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过UserGradeGroupVisitor.SetABFallback单独设定UserGradeGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawUserGradeGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawUserGradeGroup>>(()=> new PBC::MapField<int, rawUserGradeGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawUserGradeGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().UserGradeGroupMap;
			var abMap = v.UserGradeGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.UserGradeGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].UserGradeGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawUserGradeGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().UserGradeGroupMap;
			if (v.UserGradeGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.UserGradeGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawUserGradeGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(UserGradeGroupConf cc) {
			foreach(KeyValuePair <string, rawUserGradeGroupMapABValue> tagEntry in cc.UserGradeGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.UserGradeGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawUserGradeGroupMapABValue();
					cc.UserGradeGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.UserGradeGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawUserGradeGroup> entry in cc.UserGradeGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawUserGradeGroup> entry in tagEntry.Value.UserGradeGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.UserGradeGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(UserGradeGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.UserGradeGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.UserGradeGroupMapAB != null)
			{
				foreach (var entry in cc.UserGradeGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.UserGradeGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.UserGradeGroupMap.MergeFrom(cis);
			Patch(updating.UserGradeGroupMap);
			updating.UserGradeGroupSortedKeys = genSortedKeys(updating.UserGradeGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new UserGradeGroupConf();
            tmp.MergeFrom(cis);
			updating.UserGradeGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().UserGradeGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawUserGradeGroup> filter,string tag=""){
		...
		public static rawUserGradeGroup MustGet(int key,string tag="") {
		...
		public static rawUserGradeGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawUserGradeGroup> GetByFilter(Func<rawUserGradeGroup, bool> filterFunc,string tag="") {
			List<rawUserGradeGroup> ret = new List<rawUserGradeGroup>();
			foreach(KeyValuePair <int, rawUserGradeGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawUserGradeGroup? GetOneByFilter(Func<rawUserGradeGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawUserGradeGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawUserGradeGroup data) 
		{
			return data.Id;			
		}
    }
