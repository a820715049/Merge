...
using EventWishUponConf = fat.rawdata.EventWishUponConf;
using rawEventWishUpon = fat.rawdata.EventWishUpon;
using rawEventWishUponMapABValue = fat.rawdata.EventWishUponMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishUponVisitor
    {
		...
		public const string FileName = "EventWishUponConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishUponVisitor.SetABDefault单独设定EventWishUpon的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishUponVisitor.SetABFallback单独设定EventWishUpon的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishUpon>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishUpon>>(()=> new PBC::MapField<int, rawEventWishUpon>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishUpon>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishUponMap;
			var abMap = v.EventWishUponMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishUponMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishUponMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishUpon> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishUponMap;
			if (v.EventWishUponMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishUponMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishUponMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishUponConf cc) {
			foreach(KeyValuePair <string, rawEventWishUponMapABValue> tagEntry in cc.EventWishUponMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishUponMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishUponMapABValue();
					cc.EventWishUponMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishUponMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishUpon> entry in cc.EventWishUponMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishUpon> entry in tagEntry.Value.EventWishUponMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishUpon> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishUponConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishUponMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishUponMapAB != null)
			{
				foreach (var entry in cc.EventWishUponMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishUponMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishUponMap.MergeFrom(cis);
			Patch(updating.EventWishUponMap);
			updating.EventWishUponSortedKeys = genSortedKeys(updating.EventWishUponMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishUponConf();
            tmp.MergeFrom(cis);
			updating.EventWishUponMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishUponSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishUpon> filter,string tag=""){
		...
		public static rawEventWishUpon MustGet(int key,string tag="") {
		...
		public static rawEventWishUpon? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishUpon> GetByFilter(Func<rawEventWishUpon, bool> filterFunc,string tag="") {
			List<rawEventWishUpon> ret = new List<rawEventWishUpon>();
			foreach(KeyValuePair <int, rawEventWishUpon> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishUpon? GetOneByFilter(Func<rawEventWishUpon, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishUpon> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishUpon data) 
		{
			return data.Id;			
		}
    }
