...
using SoundConf = fat.rawdata.SoundConf;
using rawSound = fat.rawdata.Sound;
using rawSoundMapABValue = fat.rawdata.SoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SoundVisitor
    {
		...
		public const string FileName = "SoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SoundVisitor.SetABDefault单独设定Sound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SoundVisitor.SetABFallback单独设定Sound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawSound>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawSound>>(()=> new PBC::MapField<string, rawSound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawSound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SoundMap;
			var abMap = v.SoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawSound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SoundMap;
			if (v.SoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SoundConf cc) {
			foreach(KeyValuePair <string, rawSoundMapABValue> tagEntry in cc.SoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSoundMapABValue();
					cc.SoundMapAB[tag] = ps;
				}
                var tagTmp = ps.SoundMap;
				// 基础数据
				foreach(KeyValuePair <string, rawSound> entry in cc.SoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawSound> entry in tagEntry.Value.SoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.Sound> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(SoundConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.SoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SoundMapAB != null)
			{
				foreach (var entry in cc.SoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.SoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SoundMap.MergeFrom(cis);
			Patch(updating.SoundMap);
			updating.SoundSortedKeys = genSortedKeys(updating.SoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SoundConf();
            tmp.MergeFrom(cis);
			updating.SoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawSound> filter,string tag=""){
		...
		public static rawSound MustGet(string key,string tag="") {
		...
		public static rawSound? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSound> GetByFilter(Func<rawSound, bool> filterFunc,string tag="") {
			List<rawSound> ret = new List<rawSound>();
			foreach(KeyValuePair <string, rawSound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSound? GetOneByFilter(Func<rawSound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawSound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawSound data) 
		{
			return data.Id;			
		}
    }
