...
using EndlessThreePackDetailConf = fat.rawdata.EndlessThreePackDetailConf;
using rawEndlessThreePackDetail = fat.rawdata.EndlessThreePackDetail;
using rawEndlessThreePackDetailSliceABValue = fat.rawdata.EndlessThreePackDetailSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessThreePackDetailVisitor
    {
		...
		public const string FileName = "EndlessThreePackDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessThreePackDetailVisitor.SetABDefault单独设定EndlessThreePackDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessThreePackDetailVisitor.SetABFallback单独设定EndlessThreePackDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawEndlessThreePackDetail>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawEndlessThreePackDetail>>(()=>new PBC::RepeatedField<rawEndlessThreePackDetail>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawEndlessThreePackDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessThreePackDetailSlice;
			var abMap = v.EndlessThreePackDetailSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessThreePackDetailSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessThreePackDetailSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawEndlessThreePackDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessThreePackDetailSlice;
			if (v.EndlessThreePackDetailSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessThreePackDetailSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessThreePackDetailSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessThreePackDetailConf cc) {
			foreach(KeyValuePair <string, rawEndlessThreePackDetailSliceABValue> tagEntry in cc.EndlessThreePackDetailSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessThreePackDetailSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessThreePackDetailSliceABValue();
					cc.EndlessThreePackDetailSliceAB[tag] = ps;
				}
                var tagTmp = ps.EndlessThreePackDetailSlice;
				// 基础数据
				for(int i=0; i<cc.EndlessThreePackDetailSlice.Count; ++i) 
				{
					tagTmp.Add(cc.EndlessThreePackDetailSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.EndlessThreePackDetailSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.EndlessThreePackDetail> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessThreePackDetailSlice.MergeFrom(cis);
			Patch(updating.EndlessThreePackDetailSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessThreePackDetailConf();
            tmp.MergeFrom(cis);
			updating.EndlessThreePackDetailSlice = tmp;
		...
		public static void Range(Action<int, rawEndlessThreePackDetail> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawEndlessThreePackDetail MustGetByIndex(int index,string tag="") {
		...
		public static rawEndlessThreePackDetail? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawEndlessThreePackDetail> GetByFilter(Func<rawEndlessThreePackDetail,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawEndlessThreePackDetail>();
		...
		static public rawEndlessThreePackDetail? GetOneByFilter(Func<rawEndlessThreePackDetail,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
