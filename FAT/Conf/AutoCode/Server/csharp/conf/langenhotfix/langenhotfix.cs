...
using LangEnHotfixConf = fat.rawdata.LangEnHotfixConf;
using rawLangEnHotfix = fat.rawdata.LangEnHotfix;
using rawLangEnHotfixSliceABValue = fat.rawdata.LangEnHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangEnHotfixVisitor
    {
		...
		public const string FileName = "LangEnHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangEnHotfixVisitor.SetABDefault单独设定LangEnHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangEnHotfixVisitor.SetABFallback单独设定LangEnHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangEnHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangEnHotfix>>(()=>new PBC::RepeatedField<rawLangEnHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangEnHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangEnHotfixSlice;
			var abMap = v.LangEnHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangEnHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangEnHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangEnHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangEnHotfixSlice;
			if (v.LangEnHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangEnHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangEnHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangEnHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangEnHotfixSliceABValue> tagEntry in cc.LangEnHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangEnHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangEnHotfixSliceABValue();
					cc.LangEnHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangEnHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangEnHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangEnHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangEnHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangEnHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangEnHotfixSlice.MergeFrom(cis);
			Patch(updating.LangEnHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangEnHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangEnHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangEnHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangEnHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangEnHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangEnHotfix> GetByFilter(Func<rawLangEnHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangEnHotfix>();
		...
		static public rawLangEnHotfix? GetOneByFilter(Func<rawLangEnHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
