...
using EventMineCartOrderItemConf = fat.rawdata.EventMineCartOrderItemConf;
using rawEventMineCartOrderItem = fat.rawdata.EventMineCartOrderItem;
using rawEventMineCartOrderItemMapABValue = fat.rawdata.EventMineCartOrderItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartOrderItemVisitor
    {
		...
		public const string FileName = "EventMineCartOrderItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartOrderItemVisitor.SetABDefault单独设定EventMineCartOrderItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartOrderItemVisitor.SetABFallback单独设定EventMineCartOrderItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartOrderItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartOrderItem>>(()=> new PBC::MapField<int, rawEventMineCartOrderItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartOrderItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartOrderItemMap;
			var abMap = v.EventMineCartOrderItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartOrderItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartOrderItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartOrderItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartOrderItemMap;
			if (v.EventMineCartOrderItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartOrderItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartOrderItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartOrderItemConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartOrderItemMapABValue> tagEntry in cc.EventMineCartOrderItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartOrderItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartOrderItemMapABValue();
					cc.EventMineCartOrderItemMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartOrderItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartOrderItem> entry in cc.EventMineCartOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartOrderItem> entry in tagEntry.Value.EventMineCartOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartOrderItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartOrderItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartOrderItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartOrderItemMapAB != null)
			{
				foreach (var entry in cc.EventMineCartOrderItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartOrderItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartOrderItemMap.MergeFrom(cis);
			Patch(updating.EventMineCartOrderItemMap);
			updating.EventMineCartOrderItemSortedKeys = genSortedKeys(updating.EventMineCartOrderItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartOrderItemConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartOrderItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartOrderItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartOrderItem> filter,string tag=""){
		...
		public static rawEventMineCartOrderItem MustGet(int key,string tag="") {
		...
		public static rawEventMineCartOrderItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartOrderItem> GetByFilter(Func<rawEventMineCartOrderItem, bool> filterFunc,string tag="") {
			List<rawEventMineCartOrderItem> ret = new List<rawEventMineCartOrderItem>();
			foreach(KeyValuePair <int, rawEventMineCartOrderItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartOrderItem? GetOneByFilter(Func<rawEventMineCartOrderItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartOrderItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartOrderItem data) 
		{
			return data.Id;			
		}
    }
