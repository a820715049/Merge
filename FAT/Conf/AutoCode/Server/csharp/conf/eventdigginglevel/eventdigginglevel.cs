...
using EventDiggingLevelConf = fat.rawdata.EventDiggingLevelConf;
using rawEventDiggingLevel = fat.rawdata.EventDiggingLevel;
using rawEventDiggingLevelMapABValue = fat.rawdata.EventDiggingLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingLevelVisitor
    {
		...
		public const string FileName = "EventDiggingLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingLevelVisitor.SetABDefault单独设定EventDiggingLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingLevelVisitor.SetABFallback单独设定EventDiggingLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDiggingLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDiggingLevel>>(()=> new PBC::MapField<int, rawEventDiggingLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDiggingLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingLevelMap;
			var abMap = v.EventDiggingLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDiggingLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingLevelMap;
			if (v.EventDiggingLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingLevelConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingLevelMapABValue> tagEntry in cc.EventDiggingLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingLevelMapABValue();
					cc.EventDiggingLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDiggingLevel> entry in cc.EventDiggingLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDiggingLevel> entry in tagEntry.Value.EventDiggingLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDiggingLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingLevelMapAB != null)
			{
				foreach (var entry in cc.EventDiggingLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingLevelMap.MergeFrom(cis);
			Patch(updating.EventDiggingLevelMap);
			updating.EventDiggingLevelSortedKeys = genSortedKeys(updating.EventDiggingLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingLevelConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDiggingLevel> filter,string tag=""){
		...
		public static rawEventDiggingLevel MustGet(int key,string tag="") {
		...
		public static rawEventDiggingLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDiggingLevel> GetByFilter(Func<rawEventDiggingLevel, bool> filterFunc,string tag="") {
			List<rawEventDiggingLevel> ret = new List<rawEventDiggingLevel>();
			foreach(KeyValuePair <int, rawEventDiggingLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDiggingLevel? GetOneByFilter(Func<rawEventDiggingLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDiggingLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDiggingLevel data) 
		{
			return data.Id;			
		}
    }
