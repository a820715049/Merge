...
using LangFrHotfixConf = fat.rawdata.LangFrHotfixConf;
using rawLangFrHotfix = fat.rawdata.LangFrHotfix;
using rawLangFrHotfixSliceABValue = fat.rawdata.LangFrHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangFrHotfixVisitor
    {
		...
		public const string FileName = "LangFrHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangFrHotfixVisitor.SetABDefault单独设定LangFrHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangFrHotfixVisitor.SetABFallback单独设定LangFrHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangFrHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangFrHotfix>>(()=>new PBC::RepeatedField<rawLangFrHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangFrHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangFrHotfixSlice;
			var abMap = v.LangFrHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangFrHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangFrHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangFrHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangFrHotfixSlice;
			if (v.LangFrHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangFrHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangFrHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangFrHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangFrHotfixSliceABValue> tagEntry in cc.LangFrHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangFrHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangFrHotfixSliceABValue();
					cc.LangFrHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangFrHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangFrHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangFrHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangFrHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangFrHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangFrHotfixSlice.MergeFrom(cis);
			Patch(updating.LangFrHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangFrHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangFrHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangFrHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangFrHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangFrHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangFrHotfix> GetByFilter(Func<rawLangFrHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangFrHotfix>();
		...
		static public rawLangFrHotfix? GetOneByFilter(Func<rawLangFrHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
