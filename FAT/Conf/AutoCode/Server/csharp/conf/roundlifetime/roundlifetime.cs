...
using RoundLifeTimeConf = fat.rawdata.RoundLifeTimeConf;
using rawRoundLifeTime = fat.rawdata.RoundLifeTime;
using rawRoundLifeTimeSliceABValue = fat.rawdata.RoundLifeTimeSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RoundLifeTimeVisitor
    {
		...
		public const string FileName = "RoundLifeTimeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RoundLifeTimeVisitor.SetABDefault单独设定RoundLifeTime的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RoundLifeTimeVisitor.SetABFallback单独设定RoundLifeTime的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawRoundLifeTime>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawRoundLifeTime>>(()=>new PBC::RepeatedField<rawRoundLifeTime>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawRoundLifeTime>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RoundLifeTimeSlice;
			var abMap = v.RoundLifeTimeSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RoundLifeTimeSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RoundLifeTimeSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawRoundLifeTime> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RoundLifeTimeSlice;
			if (v.RoundLifeTimeSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RoundLifeTimeSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRoundLifeTimeSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RoundLifeTimeConf cc) {
			foreach(KeyValuePair <string, rawRoundLifeTimeSliceABValue> tagEntry in cc.RoundLifeTimeSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RoundLifeTimeSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRoundLifeTimeSliceABValue();
					cc.RoundLifeTimeSliceAB[tag] = ps;
				}
                var tagTmp = ps.RoundLifeTimeSlice;
				// 基础数据
				for(int i=0; i<cc.RoundLifeTimeSlice.Count; ++i) 
				{
					tagTmp.Add(cc.RoundLifeTimeSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.RoundLifeTimeSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.RoundLifeTime> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RoundLifeTimeSlice.MergeFrom(cis);
			Patch(updating.RoundLifeTimeSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RoundLifeTimeConf();
            tmp.MergeFrom(cis);
			updating.RoundLifeTimeSlice = tmp;
		...
		public static void Range(Action<int, rawRoundLifeTime> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawRoundLifeTime MustGetByIndex(int index,string tag="") {
		...
		public static rawRoundLifeTime? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawRoundLifeTime> GetByFilter(Func<rawRoundLifeTime,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawRoundLifeTime>();
		...
		static public rawRoundLifeTime? GetOneByFilter(Func<rawRoundLifeTime,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
