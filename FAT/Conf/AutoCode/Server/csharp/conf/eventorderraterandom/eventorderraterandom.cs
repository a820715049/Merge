...
using EventOrderRateRandomConf = fat.rawdata.EventOrderRateRandomConf;
using rawEventOrderRateRandom = fat.rawdata.EventOrderRateRandom;
using rawEventOrderRateRandomMapABValue = fat.rawdata.EventOrderRateRandomMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderRateRandomVisitor
    {
		...
		public const string FileName = "EventOrderRateRandomConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderRateRandomVisitor.SetABDefault单独设定EventOrderRateRandom的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderRateRandomVisitor.SetABFallback单独设定EventOrderRateRandom的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderRateRandom>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderRateRandom>>(()=> new PBC::MapField<int, rawEventOrderRateRandom>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderRateRandom>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderRateRandomMap;
			var abMap = v.EventOrderRateRandomMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderRateRandomMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderRateRandomMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderRateRandom> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderRateRandomMap;
			if (v.EventOrderRateRandomMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderRateRandomMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderRateRandomMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderRateRandomConf cc) {
			foreach(KeyValuePair <string, rawEventOrderRateRandomMapABValue> tagEntry in cc.EventOrderRateRandomMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderRateRandomMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderRateRandomMapABValue();
					cc.EventOrderRateRandomMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderRateRandomMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderRateRandom> entry in cc.EventOrderRateRandomMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderRateRandom> entry in tagEntry.Value.EventOrderRateRandomMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderRateRandom> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderRateRandomConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderRateRandomMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderRateRandomMapAB != null)
			{
				foreach (var entry in cc.EventOrderRateRandomMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderRateRandomMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderRateRandomMap.MergeFrom(cis);
			Patch(updating.EventOrderRateRandomMap);
			updating.EventOrderRateRandomSortedKeys = genSortedKeys(updating.EventOrderRateRandomMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderRateRandomConf();
            tmp.MergeFrom(cis);
			updating.EventOrderRateRandomMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderRateRandomSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderRateRandom> filter,string tag=""){
		...
		public static rawEventOrderRateRandom MustGet(int key,string tag="") {
		...
		public static rawEventOrderRateRandom? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderRateRandom> GetByFilter(Func<rawEventOrderRateRandom, bool> filterFunc,string tag="") {
			List<rawEventOrderRateRandom> ret = new List<rawEventOrderRateRandom>();
			foreach(KeyValuePair <int, rawEventOrderRateRandom> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderRateRandom? GetOneByFilter(Func<rawEventOrderRateRandom, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderRateRandom> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderRateRandom data) 
		{
			return data.Id;			
		}
    }
