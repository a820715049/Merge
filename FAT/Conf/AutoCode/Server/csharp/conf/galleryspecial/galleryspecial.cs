...
using GallerySpecialConf = fat.rawdata.GallerySpecialConf;
using rawGallerySpecial = fat.rawdata.GallerySpecial;
using rawGallerySpecialMapABValue = fat.rawdata.GallerySpecialMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GallerySpecialVisitor
    {
		...
		public const string FileName = "GallerySpecialConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GallerySpecialVisitor.SetABDefault单独设定GallerySpecial的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GallerySpecialVisitor.SetABFallback单独设定GallerySpecial的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGallerySpecial>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGallerySpecial>>(()=> new PBC::MapField<int, rawGallerySpecial>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGallerySpecial>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GallerySpecialMap;
			var abMap = v.GallerySpecialMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GallerySpecialMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GallerySpecialMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGallerySpecial> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GallerySpecialMap;
			if (v.GallerySpecialMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GallerySpecialMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGallerySpecialMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GallerySpecialConf cc) {
			foreach(KeyValuePair <string, rawGallerySpecialMapABValue> tagEntry in cc.GallerySpecialMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GallerySpecialMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGallerySpecialMapABValue();
					cc.GallerySpecialMapAB[tag] = ps;
				}
                var tagTmp = ps.GallerySpecialMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGallerySpecial> entry in cc.GallerySpecialMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGallerySpecial> entry in tagEntry.Value.GallerySpecialMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GallerySpecial> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GallerySpecialConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GallerySpecialMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GallerySpecialMapAB != null)
			{
				foreach (var entry in cc.GallerySpecialMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GallerySpecialMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GallerySpecialMap.MergeFrom(cis);
			Patch(updating.GallerySpecialMap);
			updating.GallerySpecialSortedKeys = genSortedKeys(updating.GallerySpecialMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GallerySpecialConf();
            tmp.MergeFrom(cis);
			updating.GallerySpecialMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GallerySpecialSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGallerySpecial> filter,string tag=""){
		...
		public static rawGallerySpecial MustGet(int key,string tag="") {
		...
		public static rawGallerySpecial? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGallerySpecial> GetByFilter(Func<rawGallerySpecial, bool> filterFunc,string tag="") {
			List<rawGallerySpecial> ret = new List<rawGallerySpecial>();
			foreach(KeyValuePair <int, rawGallerySpecial> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGallerySpecial? GetOneByFilter(Func<rawGallerySpecial, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGallerySpecial> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGallerySpecial data) 
		{
			return data.Id;			
		}
    }
