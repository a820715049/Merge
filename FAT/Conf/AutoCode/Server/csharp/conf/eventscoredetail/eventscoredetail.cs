...
using EventScoreDetailConf = fat.rawdata.EventScoreDetailConf;
using rawEventScoreDetail = fat.rawdata.EventScoreDetail;
using rawEventScoreDetailMapABValue = fat.rawdata.EventScoreDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventScoreDetailVisitor
    {
		...
		public const string FileName = "EventScoreDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventScoreDetailVisitor.SetABDefault单独设定EventScoreDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventScoreDetailVisitor.SetABFallback单独设定EventScoreDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventScoreDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventScoreDetail>>(()=> new PBC::MapField<int, rawEventScoreDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventScoreDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventScoreDetailMap;
			var abMap = v.EventScoreDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventScoreDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventScoreDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventScoreDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventScoreDetailMap;
			if (v.EventScoreDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventScoreDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventScoreDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventScoreDetailConf cc) {
			foreach(KeyValuePair <string, rawEventScoreDetailMapABValue> tagEntry in cc.EventScoreDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventScoreDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventScoreDetailMapABValue();
					cc.EventScoreDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventScoreDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventScoreDetail> entry in cc.EventScoreDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventScoreDetail> entry in tagEntry.Value.EventScoreDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventScoreDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventScoreDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventScoreDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventScoreDetailMapAB != null)
			{
				foreach (var entry in cc.EventScoreDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventScoreDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventScoreDetailMap.MergeFrom(cis);
			Patch(updating.EventScoreDetailMap);
			updating.EventScoreDetailSortedKeys = genSortedKeys(updating.EventScoreDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventScoreDetailConf();
            tmp.MergeFrom(cis);
			updating.EventScoreDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventScoreDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventScoreDetail> filter,string tag=""){
		...
		public static rawEventScoreDetail MustGet(int key,string tag="") {
		...
		public static rawEventScoreDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventScoreDetail> GetByFilter(Func<rawEventScoreDetail, bool> filterFunc,string tag="") {
			List<rawEventScoreDetail> ret = new List<rawEventScoreDetail>();
			foreach(KeyValuePair <int, rawEventScoreDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventScoreDetail? GetOneByFilter(Func<rawEventScoreDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventScoreDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventScoreDetail data) 
		{
			return data.Id;			
		}
    }
