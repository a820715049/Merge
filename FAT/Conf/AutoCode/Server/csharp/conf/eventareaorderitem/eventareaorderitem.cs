...
using EventAreaOrderItemConf = fat.rawdata.EventAreaOrderItemConf;
using rawEventAreaOrderItem = fat.rawdata.EventAreaOrderItem;
using rawEventAreaOrderItemMapABValue = fat.rawdata.EventAreaOrderItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaOrderItemVisitor
    {
		...
		public const string FileName = "EventAreaOrderItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaOrderItemVisitor.SetABDefault单独设定EventAreaOrderItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaOrderItemVisitor.SetABFallback单独设定EventAreaOrderItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaOrderItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaOrderItem>>(()=> new PBC::MapField<int, rawEventAreaOrderItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaOrderItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaOrderItemMap;
			var abMap = v.EventAreaOrderItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaOrderItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaOrderItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaOrderItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaOrderItemMap;
			if (v.EventAreaOrderItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaOrderItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaOrderItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaOrderItemConf cc) {
			foreach(KeyValuePair <string, rawEventAreaOrderItemMapABValue> tagEntry in cc.EventAreaOrderItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaOrderItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaOrderItemMapABValue();
					cc.EventAreaOrderItemMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaOrderItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaOrderItem> entry in cc.EventAreaOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaOrderItem> entry in tagEntry.Value.EventAreaOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaOrderItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaOrderItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaOrderItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaOrderItemMapAB != null)
			{
				foreach (var entry in cc.EventAreaOrderItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaOrderItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaOrderItemMap.MergeFrom(cis);
			Patch(updating.EventAreaOrderItemMap);
			updating.EventAreaOrderItemSortedKeys = genSortedKeys(updating.EventAreaOrderItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaOrderItemConf();
            tmp.MergeFrom(cis);
			updating.EventAreaOrderItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaOrderItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaOrderItem> filter,string tag=""){
		...
		public static rawEventAreaOrderItem MustGet(int key,string tag="") {
		...
		public static rawEventAreaOrderItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaOrderItem> GetByFilter(Func<rawEventAreaOrderItem, bool> filterFunc,string tag="") {
			List<rawEventAreaOrderItem> ret = new List<rawEventAreaOrderItem>();
			foreach(KeyValuePair <int, rawEventAreaOrderItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaOrderItem? GetOneByFilter(Func<rawEventAreaOrderItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaOrderItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaOrderItem data) 
		{
			return data.Id;			
		}
    }
