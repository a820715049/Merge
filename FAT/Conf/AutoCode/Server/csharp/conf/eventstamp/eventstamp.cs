...
using EventStampConf = fat.rawdata.EventStampConf;
using rawEventStamp = fat.rawdata.EventStamp;
using rawEventStampMapABValue = fat.rawdata.EventStampMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventStampVisitor
    {
		...
		public const string FileName = "EventStampConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventStampVisitor.SetABDefault单独设定EventStamp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventStampVisitor.SetABFallback单独设定EventStamp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventStamp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventStamp>>(()=> new PBC::MapField<int, rawEventStamp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventStamp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventStampMap;
			var abMap = v.EventStampMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventStampMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventStampMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventStamp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventStampMap;
			if (v.EventStampMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventStampMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventStampMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventStampConf cc) {
			foreach(KeyValuePair <string, rawEventStampMapABValue> tagEntry in cc.EventStampMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventStampMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventStampMapABValue();
					cc.EventStampMapAB[tag] = ps;
				}
                var tagTmp = ps.EventStampMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventStamp> entry in cc.EventStampMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventStamp> entry in tagEntry.Value.EventStampMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventStamp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventStampConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventStampMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventStampMapAB != null)
			{
				foreach (var entry in cc.EventStampMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventStampMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventStampMap.MergeFrom(cis);
			Patch(updating.EventStampMap);
			updating.EventStampSortedKeys = genSortedKeys(updating.EventStampMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventStampConf();
            tmp.MergeFrom(cis);
			updating.EventStampMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventStampSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventStamp> filter,string tag=""){
		...
		public static rawEventStamp MustGet(int key,string tag="") {
		...
		public static rawEventStamp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventStamp> GetByFilter(Func<rawEventStamp, bool> filterFunc,string tag="") {
			List<rawEventStamp> ret = new List<rawEventStamp>();
			foreach(KeyValuePair <int, rawEventStamp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventStamp? GetOneByFilter(Func<rawEventStamp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventStamp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventStamp data) 
		{
			return data.Id;			
		}
    }
