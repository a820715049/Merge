...
using LevelPackConf = fat.rawdata.LevelPackConf;
using rawLevelPack = fat.rawdata.LevelPack;
using rawLevelPackMapABValue = fat.rawdata.LevelPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LevelPackVisitor
    {
		...
		public const string FileName = "LevelPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LevelPackVisitor.SetABDefault单独设定LevelPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LevelPackVisitor.SetABFallback单独设定LevelPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLevelPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLevelPack>>(()=> new PBC::MapField<int, rawLevelPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLevelPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LevelPackMap;
			var abMap = v.LevelPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LevelPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LevelPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLevelPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LevelPackMap;
			if (v.LevelPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LevelPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLevelPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LevelPackConf cc) {
			foreach(KeyValuePair <string, rawLevelPackMapABValue> tagEntry in cc.LevelPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LevelPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLevelPackMapABValue();
					cc.LevelPackMapAB[tag] = ps;
				}
                var tagTmp = ps.LevelPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLevelPack> entry in cc.LevelPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLevelPack> entry in tagEntry.Value.LevelPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LevelPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LevelPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LevelPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LevelPackMapAB != null)
			{
				foreach (var entry in cc.LevelPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LevelPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LevelPackMap.MergeFrom(cis);
			Patch(updating.LevelPackMap);
			updating.LevelPackSortedKeys = genSortedKeys(updating.LevelPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LevelPackConf();
            tmp.MergeFrom(cis);
			updating.LevelPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LevelPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLevelPack> filter,string tag=""){
		...
		public static rawLevelPack MustGet(int key,string tag="") {
		...
		public static rawLevelPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLevelPack> GetByFilter(Func<rawLevelPack, bool> filterFunc,string tag="") {
			List<rawLevelPack> ret = new List<rawLevelPack>();
			foreach(KeyValuePair <int, rawLevelPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLevelPack? GetOneByFilter(Func<rawLevelPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLevelPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLevelPack data) 
		{
			return data.Id;			
		}
    }
