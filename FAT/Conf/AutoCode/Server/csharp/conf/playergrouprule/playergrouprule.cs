...
using PlayerGroupRuleConf = fat.rawdata.PlayerGroupRuleConf;
using rawPlayerGroupRule = fat.rawdata.PlayerGroupRule;
using rawPlayerGroupRuleSliceABValue = fat.rawdata.PlayerGroupRuleSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PlayerGroupRuleVisitor
    {
		...
		public const string FileName = "PlayerGroupRuleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PlayerGroupRuleVisitor.SetABDefault单独设定PlayerGroupRule的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PlayerGroupRuleVisitor.SetABFallback单独设定PlayerGroupRule的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawPlayerGroupRule>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawPlayerGroupRule>>(()=>new PBC::RepeatedField<rawPlayerGroupRule>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawPlayerGroupRule>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PlayerGroupRuleSlice;
			var abMap = v.PlayerGroupRuleSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PlayerGroupRuleSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PlayerGroupRuleSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawPlayerGroupRule> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PlayerGroupRuleSlice;
			if (v.PlayerGroupRuleSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PlayerGroupRuleSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPlayerGroupRuleSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PlayerGroupRuleConf cc) {
			foreach(KeyValuePair <string, rawPlayerGroupRuleSliceABValue> tagEntry in cc.PlayerGroupRuleSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PlayerGroupRuleSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPlayerGroupRuleSliceABValue();
					cc.PlayerGroupRuleSliceAB[tag] = ps;
				}
                var tagTmp = ps.PlayerGroupRuleSlice;
				// 基础数据
				for(int i=0; i<cc.PlayerGroupRuleSlice.Count; ++i) 
				{
					tagTmp.Add(cc.PlayerGroupRuleSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.PlayerGroupRuleSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.PlayerGroupRule> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PlayerGroupRuleSlice.MergeFrom(cis);
			Patch(updating.PlayerGroupRuleSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PlayerGroupRuleConf();
            tmp.MergeFrom(cis);
			updating.PlayerGroupRuleSlice = tmp;
		...
		public static void Range(Action<int, rawPlayerGroupRule> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawPlayerGroupRule MustGetByIndex(int index,string tag="") {
		...
		public static rawPlayerGroupRule? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawPlayerGroupRule> GetByFilter(Func<rawPlayerGroupRule,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawPlayerGroupRule>();
		...
		static public rawPlayerGroupRule? GetOneByFilter(Func<rawPlayerGroupRule,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
