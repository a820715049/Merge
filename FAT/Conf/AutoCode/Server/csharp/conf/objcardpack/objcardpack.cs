...
using ObjCardPackConf = fat.rawdata.ObjCardPackConf;
using rawObjCardPack = fat.rawdata.ObjCardPack;
using rawObjCardPackMapABValue = fat.rawdata.ObjCardPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjCardPackVisitor
    {
		...
		public const string FileName = "ObjCardPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjCardPackVisitor.SetABDefault单独设定ObjCardPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjCardPackVisitor.SetABFallback单独设定ObjCardPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjCardPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjCardPack>>(()=> new PBC::MapField<int, rawObjCardPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjCardPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjCardPackMap;
			var abMap = v.ObjCardPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjCardPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjCardPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjCardPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjCardPackMap;
			if (v.ObjCardPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjCardPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjCardPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjCardPackConf cc) {
			foreach(KeyValuePair <string, rawObjCardPackMapABValue> tagEntry in cc.ObjCardPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjCardPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjCardPackMapABValue();
					cc.ObjCardPackMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjCardPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjCardPack> entry in cc.ObjCardPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjCardPack> entry in tagEntry.Value.ObjCardPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjCardPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjCardPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjCardPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjCardPackMapAB != null)
			{
				foreach (var entry in cc.ObjCardPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjCardPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjCardPackMap.MergeFrom(cis);
			Patch(updating.ObjCardPackMap);
			updating.ObjCardPackSortedKeys = genSortedKeys(updating.ObjCardPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjCardPackConf();
            tmp.MergeFrom(cis);
			updating.ObjCardPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjCardPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjCardPack> filter,string tag=""){
		...
		public static rawObjCardPack MustGet(int key,string tag="") {
		...
		public static rawObjCardPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjCardPack> GetByFilter(Func<rawObjCardPack, bool> filterFunc,string tag="") {
			List<rawObjCardPack> ret = new List<rawObjCardPack>();
			foreach(KeyValuePair <int, rawObjCardPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjCardPack? GetOneByFilter(Func<rawObjCardPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjCardPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjCardPack data) 
		{
			return data.Id;			
		}
    }
