...
using EventOrderStreakDetailConf = fat.rawdata.EventOrderStreakDetailConf;
using rawEventOrderStreakDetail = fat.rawdata.EventOrderStreakDetail;
using rawEventOrderStreakDetailMapABValue = fat.rawdata.EventOrderStreakDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderStreakDetailVisitor
    {
		...
		public const string FileName = "EventOrderStreakDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderStreakDetailVisitor.SetABDefault单独设定EventOrderStreakDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderStreakDetailVisitor.SetABFallback单独设定EventOrderStreakDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderStreakDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderStreakDetail>>(()=> new PBC::MapField<int, rawEventOrderStreakDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderStreakDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderStreakDetailMap;
			var abMap = v.EventOrderStreakDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderStreakDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderStreakDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderStreakDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderStreakDetailMap;
			if (v.EventOrderStreakDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderStreakDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderStreakDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderStreakDetailConf cc) {
			foreach(KeyValuePair <string, rawEventOrderStreakDetailMapABValue> tagEntry in cc.EventOrderStreakDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderStreakDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderStreakDetailMapABValue();
					cc.EventOrderStreakDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderStreakDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderStreakDetail> entry in cc.EventOrderStreakDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderStreakDetail> entry in tagEntry.Value.EventOrderStreakDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderStreakDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderStreakDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderStreakDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderStreakDetailMapAB != null)
			{
				foreach (var entry in cc.EventOrderStreakDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderStreakDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderStreakDetailMap.MergeFrom(cis);
			Patch(updating.EventOrderStreakDetailMap);
			updating.EventOrderStreakDetailSortedKeys = genSortedKeys(updating.EventOrderStreakDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderStreakDetailConf();
            tmp.MergeFrom(cis);
			updating.EventOrderStreakDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderStreakDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderStreakDetail> filter,string tag=""){
		...
		public static rawEventOrderStreakDetail MustGet(int key,string tag="") {
		...
		public static rawEventOrderStreakDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderStreakDetail> GetByFilter(Func<rawEventOrderStreakDetail, bool> filterFunc,string tag="") {
			List<rawEventOrderStreakDetail> ret = new List<rawEventOrderStreakDetail>();
			foreach(KeyValuePair <int, rawEventOrderStreakDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderStreakDetail? GetOneByFilter(Func<rawEventOrderStreakDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderStreakDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderStreakDetail data) 
		{
			return data.Id;			
		}
    }
