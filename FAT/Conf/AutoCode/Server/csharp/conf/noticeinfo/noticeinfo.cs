...
using NoticeInfoConf = fat.rawdata.NoticeInfoConf;
using rawNoticeInfo = fat.rawdata.NoticeInfo;
using rawNoticeInfoSliceABValue = fat.rawdata.NoticeInfoSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class NoticeInfoVisitor
    {
		...
		public const string FileName = "NoticeInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过NoticeInfoVisitor.SetABDefault单独设定NoticeInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过NoticeInfoVisitor.SetABFallback单独设定NoticeInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawNoticeInfo>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawNoticeInfo>>(()=>new PBC::RepeatedField<rawNoticeInfo>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawNoticeInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().NoticeInfoSlice;
			var abMap = v.NoticeInfoSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.NoticeInfoSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].NoticeInfoSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawNoticeInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().NoticeInfoSlice;
			if (v.NoticeInfoSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.NoticeInfoSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawNoticeInfoSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(NoticeInfoConf cc) {
			foreach(KeyValuePair <string, rawNoticeInfoSliceABValue> tagEntry in cc.NoticeInfoSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.NoticeInfoSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawNoticeInfoSliceABValue();
					cc.NoticeInfoSliceAB[tag] = ps;
				}
                var tagTmp = ps.NoticeInfoSlice;
				// 基础数据
				for(int i=0; i<cc.NoticeInfoSlice.Count; ++i) 
				{
					tagTmp.Add(cc.NoticeInfoSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.NoticeInfoSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.NoticeInfo> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.NoticeInfoSlice.MergeFrom(cis);
			Patch(updating.NoticeInfoSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new NoticeInfoConf();
            tmp.MergeFrom(cis);
			updating.NoticeInfoSlice = tmp;
		...
		public static void Range(Action<int, rawNoticeInfo> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawNoticeInfo MustGetByIndex(int index,string tag="") {
		...
		public static rawNoticeInfo? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawNoticeInfo> GetByFilter(Func<rawNoticeInfo,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawNoticeInfo>();
		...
		static public rawNoticeInfo? GetOneByFilter(Func<rawNoticeInfo,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
