...
using ComMergeSkillConf = fat.rawdata.ComMergeSkillConf;
using rawComMergeSkill = fat.rawdata.ComMergeSkill;
using rawComMergeSkillMapABValue = fat.rawdata.ComMergeSkillMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeSkillVisitor
    {
		...
		public const string FileName = "ComMergeSkillConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeSkillVisitor.SetABDefault单独设定ComMergeSkill的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeSkillVisitor.SetABFallback单独设定ComMergeSkill的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeSkill>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeSkill>>(()=> new PBC::MapField<int, rawComMergeSkill>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeSkill>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeSkillMap;
			var abMap = v.ComMergeSkillMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeSkillMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeSkillMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeSkill> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeSkillMap;
			if (v.ComMergeSkillMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeSkillMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeSkillMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeSkillConf cc) {
			foreach(KeyValuePair <string, rawComMergeSkillMapABValue> tagEntry in cc.ComMergeSkillMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeSkillMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeSkillMapABValue();
					cc.ComMergeSkillMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeSkillMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeSkill> entry in cc.ComMergeSkillMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeSkill> entry in tagEntry.Value.ComMergeSkillMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeSkill> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeSkillConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeSkillMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeSkillMapAB != null)
			{
				foreach (var entry in cc.ComMergeSkillMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeSkillMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeSkillMap.MergeFrom(cis);
			Patch(updating.ComMergeSkillMap);
			updating.ComMergeSkillSortedKeys = genSortedKeys(updating.ComMergeSkillMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeSkillConf();
            tmp.MergeFrom(cis);
			updating.ComMergeSkillMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeSkillSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeSkill> filter,string tag=""){
		...
		public static rawComMergeSkill MustGet(int key,string tag="") {
		...
		public static rawComMergeSkill? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeSkill> GetByFilter(Func<rawComMergeSkill, bool> filterFunc,string tag="") {
			List<rawComMergeSkill> ret = new List<rawComMergeSkill>();
			foreach(KeyValuePair <int, rawComMergeSkill> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeSkill? GetOneByFilter(Func<rawComMergeSkill, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeSkill> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeSkill data) 
		{
			return data.Id;			
		}
    }
