...
using IAPFreeConf = fat.rawdata.IAPFreeConf;
using rawIAPFree = fat.rawdata.IAPFree;
using rawIAPFreeMapABValue = fat.rawdata.IAPFreeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IAPFreeVisitor
    {
		...
		public const string FileName = "IAPFreeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IAPFreeVisitor.SetABDefault单独设定IAPFree的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IAPFreeVisitor.SetABFallback单独设定IAPFree的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIAPFree>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIAPFree>>(()=> new PBC::MapField<int, rawIAPFree>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIAPFree>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IAPFreeMap;
			var abMap = v.IAPFreeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IAPFreeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IAPFreeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIAPFree> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IAPFreeMap;
			if (v.IAPFreeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IAPFreeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIAPFreeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IAPFreeConf cc) {
			foreach(KeyValuePair <string, rawIAPFreeMapABValue> tagEntry in cc.IAPFreeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IAPFreeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIAPFreeMapABValue();
					cc.IAPFreeMapAB[tag] = ps;
				}
                var tagTmp = ps.IAPFreeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIAPFree> entry in cc.IAPFreeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIAPFree> entry in tagEntry.Value.IAPFreeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IAPFree> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IAPFreeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IAPFreeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IAPFreeMapAB != null)
			{
				foreach (var entry in cc.IAPFreeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IAPFreeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IAPFreeMap.MergeFrom(cis);
			Patch(updating.IAPFreeMap);
			updating.IAPFreeSortedKeys = genSortedKeys(updating.IAPFreeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IAPFreeConf();
            tmp.MergeFrom(cis);
			updating.IAPFreeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IAPFreeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIAPFree> filter,string tag=""){
		...
		public static rawIAPFree MustGet(int key,string tag="") {
		...
		public static rawIAPFree? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIAPFree> GetByFilter(Func<rawIAPFree, bool> filterFunc,string tag="") {
			List<rawIAPFree> ret = new List<rawIAPFree>();
			foreach(KeyValuePair <int, rawIAPFree> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIAPFree? GetOneByFilter(Func<rawIAPFree, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIAPFree> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIAPFree data) 
		{
			return data.Id;			
		}
    }
