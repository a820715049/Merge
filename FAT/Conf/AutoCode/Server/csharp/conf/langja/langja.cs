...
using LangJaConf = fat.rawdata.LangJaConf;
using rawLangJa = fat.rawdata.LangJa;
using rawLangJaSliceABValue = fat.rawdata.LangJaSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangJaVisitor
    {
		...
		public const string FileName = "LangJaConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangJaVisitor.SetABDefault单独设定LangJa的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangJaVisitor.SetABFallback单独设定LangJa的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangJa>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangJa>>(()=>new PBC::RepeatedField<rawLangJa>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangJa>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangJaSlice;
			var abMap = v.LangJaSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangJaSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangJaSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangJa> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangJaSlice;
			if (v.LangJaSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangJaSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangJaSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangJaConf cc) {
			foreach(KeyValuePair <string, rawLangJaSliceABValue> tagEntry in cc.LangJaSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangJaSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangJaSliceABValue();
					cc.LangJaSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangJaSlice;
				// 基础数据
				for(int i=0; i<cc.LangJaSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangJaSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangJaSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangJa> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangJaSlice.MergeFrom(cis);
			Patch(updating.LangJaSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangJaConf();
            tmp.MergeFrom(cis);
			updating.LangJaSlice = tmp;
		...
		public static void Range(Action<int, rawLangJa> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangJa MustGetByIndex(int index,string tag="") {
		...
		public static rawLangJa? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangJa> GetByFilter(Func<rawLangJa,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangJa>();
		...
		static public rawLangJa? GetOneByFilter(Func<rawLangJa,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
