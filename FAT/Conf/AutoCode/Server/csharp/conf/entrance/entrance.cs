...
using EntranceConf = fat.rawdata.EntranceConf;
using rawEntrance = fat.rawdata.Entrance;
using rawEntranceMapABValue = fat.rawdata.EntranceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EntranceVisitor
    {
		...
		public const string FileName = "EntranceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EntranceVisitor.SetABDefault单独设定Entrance的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EntranceVisitor.SetABFallback单独设定Entrance的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEntrance>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEntrance>>(()=> new PBC::MapField<int, rawEntrance>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEntrance>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EntranceMap;
			var abMap = v.EntranceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EntranceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EntranceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEntrance> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EntranceMap;
			if (v.EntranceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EntranceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEntranceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EntranceConf cc) {
			foreach(KeyValuePair <string, rawEntranceMapABValue> tagEntry in cc.EntranceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EntranceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEntranceMapABValue();
					cc.EntranceMapAB[tag] = ps;
				}
                var tagTmp = ps.EntranceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEntrance> entry in cc.EntranceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEntrance> entry in tagEntry.Value.EntranceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Entrance> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EntranceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EntranceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EntranceMapAB != null)
			{
				foreach (var entry in cc.EntranceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EntranceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EntranceMap.MergeFrom(cis);
			Patch(updating.EntranceMap);
			updating.EntranceSortedKeys = genSortedKeys(updating.EntranceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EntranceConf();
            tmp.MergeFrom(cis);
			updating.EntranceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EntranceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEntrance> filter,string tag=""){
		...
		public static rawEntrance MustGet(int key,string tag="") {
		...
		public static rawEntrance? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEntrance> GetByFilter(Func<rawEntrance, bool> filterFunc,string tag="") {
			List<rawEntrance> ret = new List<rawEntrance>();
			foreach(KeyValuePair <int, rawEntrance> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEntrance? GetOneByFilter(Func<rawEntrance, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEntrance> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEntrance data) 
		{
			return data.Id;			
		}
    }
