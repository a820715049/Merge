...
using BpTaskConf = fat.rawdata.BpTaskConf;
using rawBpTask = fat.rawdata.BpTask;
using rawBpTaskMapABValue = fat.rawdata.BpTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BpTaskVisitor
    {
		...
		public const string FileName = "BpTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BpTaskVisitor.SetABDefault单独设定BpTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BpTaskVisitor.SetABFallback单独设定BpTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBpTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBpTask>>(()=> new PBC::MapField<int, rawBpTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBpTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BpTaskMap;
			var abMap = v.BpTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BpTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BpTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBpTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BpTaskMap;
			if (v.BpTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BpTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBpTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BpTaskConf cc) {
			foreach(KeyValuePair <string, rawBpTaskMapABValue> tagEntry in cc.BpTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BpTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBpTaskMapABValue();
					cc.BpTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.BpTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBpTask> entry in cc.BpTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBpTask> entry in tagEntry.Value.BpTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BpTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BpTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BpTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BpTaskMapAB != null)
			{
				foreach (var entry in cc.BpTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BpTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BpTaskMap.MergeFrom(cis);
			Patch(updating.BpTaskMap);
			updating.BpTaskSortedKeys = genSortedKeys(updating.BpTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BpTaskConf();
            tmp.MergeFrom(cis);
			updating.BpTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BpTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBpTask> filter,string tag=""){
		...
		public static rawBpTask MustGet(int key,string tag="") {
		...
		public static rawBpTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBpTask> GetByFilter(Func<rawBpTask, bool> filterFunc,string tag="") {
			List<rawBpTask> ret = new List<rawBpTask>();
			foreach(KeyValuePair <int, rawBpTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBpTask? GetOneByFilter(Func<rawBpTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBpTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBpTask data) 
		{
			return data.Id;			
		}
    }
