...
using CardLimitConf = fat.rawdata.CardLimitConf;
using rawCardLimit = fat.rawdata.CardLimit;
using rawCardLimitMapABValue = fat.rawdata.CardLimitMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CardLimitVisitor
    {
		...
		public const string FileName = "CardLimitConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CardLimitVisitor.SetABDefault单独设定CardLimit的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CardLimitVisitor.SetABFallback单独设定CardLimit的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCardLimit>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCardLimit>>(()=> new PBC::MapField<int, rawCardLimit>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCardLimit>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CardLimitMap;
			var abMap = v.CardLimitMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CardLimitMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CardLimitMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCardLimit> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CardLimitMap;
			if (v.CardLimitMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CardLimitMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCardLimitMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CardLimitConf cc) {
			foreach(KeyValuePair <string, rawCardLimitMapABValue> tagEntry in cc.CardLimitMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CardLimitMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCardLimitMapABValue();
					cc.CardLimitMapAB[tag] = ps;
				}
                var tagTmp = ps.CardLimitMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCardLimit> entry in cc.CardLimitMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCardLimit> entry in tagEntry.Value.CardLimitMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CardLimit> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CardLimitConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CardLimitMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CardLimitMapAB != null)
			{
				foreach (var entry in cc.CardLimitMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CardLimitMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CardLimitMap.MergeFrom(cis);
			Patch(updating.CardLimitMap);
			updating.CardLimitSortedKeys = genSortedKeys(updating.CardLimitMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CardLimitConf();
            tmp.MergeFrom(cis);
			updating.CardLimitMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CardLimitSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCardLimit> filter,string tag=""){
		...
		public static rawCardLimit MustGet(int key,string tag="") {
		...
		public static rawCardLimit? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCardLimit> GetByFilter(Func<rawCardLimit, bool> filterFunc,string tag="") {
			List<rawCardLimit> ret = new List<rawCardLimit>();
			foreach(KeyValuePair <int, rawCardLimit> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCardLimit? GetOneByFilter(Func<rawCardLimit, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCardLimit> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCardLimit data) 
		{
			return data.Id;			
		}
    }
