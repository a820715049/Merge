...
using IAPProductConf = fat.rawdata.IAPProductConf;
using rawIAPProduct = fat.rawdata.IAPProduct;
using rawIAPProductMapABValue = fat.rawdata.IAPProductMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IAPProductVisitor
    {
		...
		public const string FileName = "IAPProductConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IAPProductVisitor.SetABDefault单独设定IAPProduct的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IAPProductVisitor.SetABFallback单独设定IAPProduct的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIAPProduct>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIAPProduct>>(()=> new PBC::MapField<int, rawIAPProduct>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIAPProduct>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IAPProductMap;
			var abMap = v.IAPProductMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IAPProductMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IAPProductMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIAPProduct> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IAPProductMap;
			if (v.IAPProductMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IAPProductMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIAPProductMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IAPProductConf cc) {
			foreach(KeyValuePair <string, rawIAPProductMapABValue> tagEntry in cc.IAPProductMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IAPProductMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIAPProductMapABValue();
					cc.IAPProductMapAB[tag] = ps;
				}
                var tagTmp = ps.IAPProductMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIAPProduct> entry in cc.IAPProductMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIAPProduct> entry in tagEntry.Value.IAPProductMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IAPProduct> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IAPProductConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IAPProductMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IAPProductMapAB != null)
			{
				foreach (var entry in cc.IAPProductMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IAPProductMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IAPProductMap.MergeFrom(cis);
			Patch(updating.IAPProductMap);
			updating.IAPProductSortedKeys = genSortedKeys(updating.IAPProductMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IAPProductConf();
            tmp.MergeFrom(cis);
			updating.IAPProductMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IAPProductSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIAPProduct> filter,string tag=""){
		...
		public static rawIAPProduct MustGet(int key,string tag="") {
		...
		public static rawIAPProduct? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIAPProduct> GetByFilter(Func<rawIAPProduct, bool> filterFunc,string tag="") {
			List<rawIAPProduct> ret = new List<rawIAPProduct>();
			foreach(KeyValuePair <int, rawIAPProduct> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIAPProduct? GetOneByFilter(Func<rawIAPProduct, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIAPProduct> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIAPProduct data) 
		{
			return data.Id;			
		}
    }
