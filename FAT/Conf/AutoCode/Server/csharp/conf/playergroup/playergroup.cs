...
using PlayerGroupConf = fat.rawdata.PlayerGroupConf;
using rawPlayerGroup = fat.rawdata.PlayerGroup;
using rawPlayerGroupMapABValue = fat.rawdata.PlayerGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PlayerGroupVisitor
    {
		...
		public const string FileName = "PlayerGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PlayerGroupVisitor.SetABDefault单独设定PlayerGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PlayerGroupVisitor.SetABFallback单独设定PlayerGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPlayerGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPlayerGroup>>(()=> new PBC::MapField<int, rawPlayerGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPlayerGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PlayerGroupMap;
			var abMap = v.PlayerGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PlayerGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PlayerGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPlayerGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PlayerGroupMap;
			if (v.PlayerGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PlayerGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPlayerGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PlayerGroupConf cc) {
			foreach(KeyValuePair <string, rawPlayerGroupMapABValue> tagEntry in cc.PlayerGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PlayerGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPlayerGroupMapABValue();
					cc.PlayerGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.PlayerGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPlayerGroup> entry in cc.PlayerGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPlayerGroup> entry in tagEntry.Value.PlayerGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.PlayerGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PlayerGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PlayerGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PlayerGroupMapAB != null)
			{
				foreach (var entry in cc.PlayerGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PlayerGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PlayerGroupMap.MergeFrom(cis);
			Patch(updating.PlayerGroupMap);
			updating.PlayerGroupSortedKeys = genSortedKeys(updating.PlayerGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PlayerGroupConf();
            tmp.MergeFrom(cis);
			updating.PlayerGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PlayerGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPlayerGroup> filter,string tag=""){
		...
		public static rawPlayerGroup MustGet(int key,string tag="") {
		...
		public static rawPlayerGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPlayerGroup> GetByFilter(Func<rawPlayerGroup, bool> filterFunc,string tag="") {
			List<rawPlayerGroup> ret = new List<rawPlayerGroup>();
			foreach(KeyValuePair <int, rawPlayerGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPlayerGroup? GetOneByFilter(Func<rawPlayerGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPlayerGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPlayerGroup data) 
		{
			return data.Id;			
		}
    }
