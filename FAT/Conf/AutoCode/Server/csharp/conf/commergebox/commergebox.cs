...
using ComMergeBoxConf = fat.rawdata.ComMergeBoxConf;
using rawComMergeBox = fat.rawdata.ComMergeBox;
using rawComMergeBoxMapABValue = fat.rawdata.ComMergeBoxMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeBoxVisitor
    {
		...
		public const string FileName = "ComMergeBoxConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeBoxVisitor.SetABDefault单独设定ComMergeBox的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeBoxVisitor.SetABFallback单独设定ComMergeBox的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeBox>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeBox>>(()=> new PBC::MapField<int, rawComMergeBox>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeBox>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeBoxMap;
			var abMap = v.ComMergeBoxMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeBoxMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeBoxMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeBox> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeBoxMap;
			if (v.ComMergeBoxMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeBoxMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeBoxMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeBoxConf cc) {
			foreach(KeyValuePair <string, rawComMergeBoxMapABValue> tagEntry in cc.ComMergeBoxMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeBoxMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeBoxMapABValue();
					cc.ComMergeBoxMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeBoxMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeBox> entry in cc.ComMergeBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeBox> entry in tagEntry.Value.ComMergeBoxMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeBox> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeBoxConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeBoxMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeBoxMapAB != null)
			{
				foreach (var entry in cc.ComMergeBoxMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeBoxMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeBoxMap.MergeFrom(cis);
			Patch(updating.ComMergeBoxMap);
			updating.ComMergeBoxSortedKeys = genSortedKeys(updating.ComMergeBoxMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeBoxConf();
            tmp.MergeFrom(cis);
			updating.ComMergeBoxMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeBoxSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeBox> filter,string tag=""){
		...
		public static rawComMergeBox MustGet(int key,string tag="") {
		...
		public static rawComMergeBox? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeBox> GetByFilter(Func<rawComMergeBox, bool> filterFunc,string tag="") {
			List<rawComMergeBox> ret = new List<rawComMergeBox>();
			foreach(KeyValuePair <int, rawComMergeBox> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeBox? GetOneByFilter(Func<rawComMergeBox, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeBox> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeBox data) 
		{
			return data.Id;			
		}
    }
