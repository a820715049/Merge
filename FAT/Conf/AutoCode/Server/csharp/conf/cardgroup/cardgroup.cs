...
using CardGroupConf = fat.rawdata.CardGroupConf;
using rawCardGroup = fat.rawdata.CardGroup;
using rawCardGroupMapABValue = fat.rawdata.CardGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CardGroupVisitor
    {
		...
		public const string FileName = "CardGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CardGroupVisitor.SetABDefault单独设定CardGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CardGroupVisitor.SetABFallback单独设定CardGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCardGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCardGroup>>(()=> new PBC::MapField<int, rawCardGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCardGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CardGroupMap;
			var abMap = v.CardGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CardGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CardGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCardGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CardGroupMap;
			if (v.CardGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CardGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCardGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CardGroupConf cc) {
			foreach(KeyValuePair <string, rawCardGroupMapABValue> tagEntry in cc.CardGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CardGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCardGroupMapABValue();
					cc.CardGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.CardGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCardGroup> entry in cc.CardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCardGroup> entry in tagEntry.Value.CardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CardGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CardGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CardGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CardGroupMapAB != null)
			{
				foreach (var entry in cc.CardGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CardGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CardGroupMap.MergeFrom(cis);
			Patch(updating.CardGroupMap);
			updating.CardGroupSortedKeys = genSortedKeys(updating.CardGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CardGroupConf();
            tmp.MergeFrom(cis);
			updating.CardGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CardGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCardGroup> filter,string tag=""){
		...
		public static rawCardGroup MustGet(int key,string tag="") {
		...
		public static rawCardGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCardGroup> GetByFilter(Func<rawCardGroup, bool> filterFunc,string tag="") {
			List<rawCardGroup> ret = new List<rawCardGroup>();
			foreach(KeyValuePair <int, rawCardGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCardGroup? GetOneByFilter(Func<rawCardGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCardGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCardGroup data) 
		{
			return data.Id;			
		}
    }
