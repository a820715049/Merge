...
using EventMiniBoardMultiDropConf = fat.rawdata.EventMiniBoardMultiDropConf;
using rawEventMiniBoardMultiDrop = fat.rawdata.EventMiniBoardMultiDrop;
using rawEventMiniBoardMultiDropMapABValue = fat.rawdata.EventMiniBoardMultiDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardMultiDropVisitor
    {
		...
		public const string FileName = "EventMiniBoardMultiDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardMultiDropVisitor.SetABDefault单独设定EventMiniBoardMultiDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardMultiDropVisitor.SetABFallback单独设定EventMiniBoardMultiDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoardMultiDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoardMultiDrop>>(()=> new PBC::MapField<int, rawEventMiniBoardMultiDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoardMultiDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardMultiDropMap;
			var abMap = v.EventMiniBoardMultiDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardMultiDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardMultiDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoardMultiDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardMultiDropMap;
			if (v.EventMiniBoardMultiDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardMultiDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardMultiDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardMultiDropConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardMultiDropMapABValue> tagEntry in cc.EventMiniBoardMultiDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardMultiDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardMultiDropMapABValue();
					cc.EventMiniBoardMultiDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardMultiDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoardMultiDrop> entry in cc.EventMiniBoardMultiDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoardMultiDrop> entry in tagEntry.Value.EventMiniBoardMultiDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoardMultiDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardMultiDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardMultiDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardMultiDropMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardMultiDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardMultiDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardMultiDropMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardMultiDropMap);
			updating.EventMiniBoardMultiDropSortedKeys = genSortedKeys(updating.EventMiniBoardMultiDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardMultiDropConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardMultiDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardMultiDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoardMultiDrop> filter,string tag=""){
		...
		public static rawEventMiniBoardMultiDrop MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoardMultiDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoardMultiDrop> GetByFilter(Func<rawEventMiniBoardMultiDrop, bool> filterFunc,string tag="") {
			List<rawEventMiniBoardMultiDrop> ret = new List<rawEventMiniBoardMultiDrop>();
			foreach(KeyValuePair <int, rawEventMiniBoardMultiDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoardMultiDrop? GetOneByFilter(Func<rawEventMiniBoardMultiDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoardMultiDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoardMultiDrop data) 
		{
			return data.Id;			
		}
    }
