...
using EventIndepenBingoDetailConf = fat.rawdata.EventIndepenBingoDetailConf;
using rawEventIndepenBingoDetail = fat.rawdata.EventIndepenBingoDetail;
using rawEventIndepenBingoDetailMapABValue = fat.rawdata.EventIndepenBingoDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventIndepenBingoDetailVisitor
    {
		...
		public const string FileName = "EventIndepenBingoDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventIndepenBingoDetailVisitor.SetABDefault单独设定EventIndepenBingoDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventIndepenBingoDetailVisitor.SetABFallback单独设定EventIndepenBingoDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventIndepenBingoDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventIndepenBingoDetail>>(()=> new PBC::MapField<int, rawEventIndepenBingoDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventIndepenBingoDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventIndepenBingoDetailMap;
			var abMap = v.EventIndepenBingoDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventIndepenBingoDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventIndepenBingoDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventIndepenBingoDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventIndepenBingoDetailMap;
			if (v.EventIndepenBingoDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventIndepenBingoDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventIndepenBingoDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventIndepenBingoDetailConf cc) {
			foreach(KeyValuePair <string, rawEventIndepenBingoDetailMapABValue> tagEntry in cc.EventIndepenBingoDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventIndepenBingoDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventIndepenBingoDetailMapABValue();
					cc.EventIndepenBingoDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventIndepenBingoDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventIndepenBingoDetail> entry in cc.EventIndepenBingoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventIndepenBingoDetail> entry in tagEntry.Value.EventIndepenBingoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventIndepenBingoDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventIndepenBingoDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventIndepenBingoDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventIndepenBingoDetailMapAB != null)
			{
				foreach (var entry in cc.EventIndepenBingoDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventIndepenBingoDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventIndepenBingoDetailMap.MergeFrom(cis);
			Patch(updating.EventIndepenBingoDetailMap);
			updating.EventIndepenBingoDetailSortedKeys = genSortedKeys(updating.EventIndepenBingoDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventIndepenBingoDetailConf();
            tmp.MergeFrom(cis);
			updating.EventIndepenBingoDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventIndepenBingoDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventIndepenBingoDetail> filter,string tag=""){
		...
		public static rawEventIndepenBingoDetail MustGet(int key,string tag="") {
		...
		public static rawEventIndepenBingoDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventIndepenBingoDetail> GetByFilter(Func<rawEventIndepenBingoDetail, bool> filterFunc,string tag="") {
			List<rawEventIndepenBingoDetail> ret = new List<rawEventIndepenBingoDetail>();
			foreach(KeyValuePair <int, rawEventIndepenBingoDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventIndepenBingoDetail? GetOneByFilter(Func<rawEventIndepenBingoDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventIndepenBingoDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventIndepenBingoDetail data) 
		{
			return data.Id;			
		}
    }
