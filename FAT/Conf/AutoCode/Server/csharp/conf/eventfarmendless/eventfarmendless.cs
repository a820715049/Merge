...
using EventFarmEndlessConf = fat.rawdata.EventFarmEndlessConf;
using rawEventFarmEndless = fat.rawdata.EventFarmEndless;
using rawEventFarmEndlessMapABValue = fat.rawdata.EventFarmEndlessMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmEndlessVisitor
    {
		...
		public const string FileName = "EventFarmEndlessConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmEndlessVisitor.SetABDefault单独设定EventFarmEndless的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmEndlessVisitor.SetABFallback单独设定EventFarmEndless的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmEndless>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmEndless>>(()=> new PBC::MapField<int, rawEventFarmEndless>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmEndless>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmEndlessMap;
			var abMap = v.EventFarmEndlessMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmEndlessMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmEndlessMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmEndless> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmEndlessMap;
			if (v.EventFarmEndlessMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmEndlessMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmEndlessMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmEndlessConf cc) {
			foreach(KeyValuePair <string, rawEventFarmEndlessMapABValue> tagEntry in cc.EventFarmEndlessMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmEndlessMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmEndlessMapABValue();
					cc.EventFarmEndlessMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmEndlessMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmEndless> entry in cc.EventFarmEndlessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmEndless> entry in tagEntry.Value.EventFarmEndlessMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmEndless> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmEndlessConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmEndlessMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmEndlessMapAB != null)
			{
				foreach (var entry in cc.EventFarmEndlessMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmEndlessMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmEndlessMap.MergeFrom(cis);
			Patch(updating.EventFarmEndlessMap);
			updating.EventFarmEndlessSortedKeys = genSortedKeys(updating.EventFarmEndlessMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmEndlessConf();
            tmp.MergeFrom(cis);
			updating.EventFarmEndlessMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmEndlessSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmEndless> filter,string tag=""){
		...
		public static rawEventFarmEndless MustGet(int key,string tag="") {
		...
		public static rawEventFarmEndless? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmEndless> GetByFilter(Func<rawEventFarmEndless, bool> filterFunc,string tag="") {
			List<rawEventFarmEndless> ret = new List<rawEventFarmEndless>();
			foreach(KeyValuePair <int, rawEventFarmEndless> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmEndless? GetOneByFilter(Func<rawEventFarmEndless, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmEndless> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmEndless data) 
		{
			return data.Id;			
		}
    }
