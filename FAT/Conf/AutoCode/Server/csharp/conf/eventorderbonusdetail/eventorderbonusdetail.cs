...
using EventOrderBonusDetailConf = fat.rawdata.EventOrderBonusDetailConf;
using rawEventOrderBonusDetail = fat.rawdata.EventOrderBonusDetail;
using rawEventOrderBonusDetailMapABValue = fat.rawdata.EventOrderBonusDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderBonusDetailVisitor
    {
		...
		public const string FileName = "EventOrderBonusDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderBonusDetailVisitor.SetABDefault单独设定EventOrderBonusDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderBonusDetailVisitor.SetABFallback单独设定EventOrderBonusDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderBonusDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderBonusDetail>>(()=> new PBC::MapField<int, rawEventOrderBonusDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderBonusDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderBonusDetailMap;
			var abMap = v.EventOrderBonusDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderBonusDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderBonusDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderBonusDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderBonusDetailMap;
			if (v.EventOrderBonusDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderBonusDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderBonusDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderBonusDetailConf cc) {
			foreach(KeyValuePair <string, rawEventOrderBonusDetailMapABValue> tagEntry in cc.EventOrderBonusDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderBonusDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderBonusDetailMapABValue();
					cc.EventOrderBonusDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderBonusDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderBonusDetail> entry in cc.EventOrderBonusDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderBonusDetail> entry in tagEntry.Value.EventOrderBonusDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderBonusDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderBonusDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderBonusDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderBonusDetailMapAB != null)
			{
				foreach (var entry in cc.EventOrderBonusDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderBonusDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderBonusDetailMap.MergeFrom(cis);
			Patch(updating.EventOrderBonusDetailMap);
			updating.EventOrderBonusDetailSortedKeys = genSortedKeys(updating.EventOrderBonusDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderBonusDetailConf();
            tmp.MergeFrom(cis);
			updating.EventOrderBonusDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderBonusDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderBonusDetail> filter,string tag=""){
		...
		public static rawEventOrderBonusDetail MustGet(int key,string tag="") {
		...
		public static rawEventOrderBonusDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderBonusDetail> GetByFilter(Func<rawEventOrderBonusDetail, bool> filterFunc,string tag="") {
			List<rawEventOrderBonusDetail> ret = new List<rawEventOrderBonusDetail>();
			foreach(KeyValuePair <int, rawEventOrderBonusDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderBonusDetail? GetOneByFilter(Func<rawEventOrderBonusDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderBonusDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderBonusDetail data) 
		{
			return data.Id;			
		}
    }
