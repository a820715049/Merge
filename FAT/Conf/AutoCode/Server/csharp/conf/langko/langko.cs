...
using LangKoConf = fat.rawdata.LangKoConf;
using rawLangKo = fat.rawdata.LangKo;
using rawLangKoSliceABValue = fat.rawdata.LangKoSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangKoVisitor
    {
		...
		public const string FileName = "LangKoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangKoVisitor.SetABDefault单独设定LangKo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangKoVisitor.SetABFallback单独设定LangKo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangKo>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangKo>>(()=>new PBC::RepeatedField<rawLangKo>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangKo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangKoSlice;
			var abMap = v.LangKoSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangKoSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangKoSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangKo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangKoSlice;
			if (v.LangKoSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangKoSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangKoSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangKoConf cc) {
			foreach(KeyValuePair <string, rawLangKoSliceABValue> tagEntry in cc.LangKoSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangKoSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangKoSliceABValue();
					cc.LangKoSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangKoSlice;
				// 基础数据
				for(int i=0; i<cc.LangKoSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangKoSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangKoSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangKo> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangKoSlice.MergeFrom(cis);
			Patch(updating.LangKoSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangKoConf();
            tmp.MergeFrom(cis);
			updating.LangKoSlice = tmp;
		...
		public static void Range(Action<int, rawLangKo> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangKo MustGetByIndex(int index,string tag="") {
		...
		public static rawLangKo? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangKo> GetByFilter(Func<rawLangKo,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangKo>();
		...
		static public rawLangKo? GetOneByFilter(Func<rawLangKo,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
