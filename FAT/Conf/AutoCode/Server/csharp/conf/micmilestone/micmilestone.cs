...
using MicMilestoneConf = fat.rawdata.MicMilestoneConf;
using rawMicMilestone = fat.rawdata.MicMilestone;
using rawMicMilestoneMapABValue = fat.rawdata.MicMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MicMilestoneVisitor
    {
		...
		public const string FileName = "MicMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MicMilestoneVisitor.SetABDefault单独设定MicMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MicMilestoneVisitor.SetABFallback单独设定MicMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMicMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMicMilestone>>(()=> new PBC::MapField<int, rawMicMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMicMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MicMilestoneMap;
			var abMap = v.MicMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MicMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MicMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMicMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MicMilestoneMap;
			if (v.MicMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MicMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMicMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MicMilestoneConf cc) {
			foreach(KeyValuePair <string, rawMicMilestoneMapABValue> tagEntry in cc.MicMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MicMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMicMilestoneMapABValue();
					cc.MicMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.MicMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMicMilestone> entry in cc.MicMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMicMilestone> entry in tagEntry.Value.MicMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MicMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MicMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MicMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MicMilestoneMapAB != null)
			{
				foreach (var entry in cc.MicMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MicMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MicMilestoneMap.MergeFrom(cis);
			Patch(updating.MicMilestoneMap);
			updating.MicMilestoneSortedKeys = genSortedKeys(updating.MicMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MicMilestoneConf();
            tmp.MergeFrom(cis);
			updating.MicMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MicMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMicMilestone> filter,string tag=""){
		...
		public static rawMicMilestone MustGet(int key,string tag="") {
		...
		public static rawMicMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMicMilestone> GetByFilter(Func<rawMicMilestone, bool> filterFunc,string tag="") {
			List<rawMicMilestone> ret = new List<rawMicMilestone>();
			foreach(KeyValuePair <int, rawMicMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMicMilestone? GetOneByFilter(Func<rawMicMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMicMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMicMilestone data) 
		{
			return data.Id;			
		}
    }
