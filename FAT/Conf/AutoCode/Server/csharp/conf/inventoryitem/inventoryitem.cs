...
using InventoryItemConf = fat.rawdata.InventoryItemConf;
using rawInventoryItem = fat.rawdata.InventoryItem;
using rawInventoryItemMapABValue = fat.rawdata.InventoryItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class InventoryItemVisitor
    {
		...
		public const string FileName = "InventoryItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过InventoryItemVisitor.SetABDefault单独设定InventoryItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过InventoryItemVisitor.SetABFallback单独设定InventoryItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawInventoryItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawInventoryItem>>(()=> new PBC::MapField<int, rawInventoryItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawInventoryItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().InventoryItemMap;
			var abMap = v.InventoryItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.InventoryItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].InventoryItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawInventoryItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().InventoryItemMap;
			if (v.InventoryItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.InventoryItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawInventoryItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(InventoryItemConf cc) {
			foreach(KeyValuePair <string, rawInventoryItemMapABValue> tagEntry in cc.InventoryItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.InventoryItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawInventoryItemMapABValue();
					cc.InventoryItemMapAB[tag] = ps;
				}
                var tagTmp = ps.InventoryItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawInventoryItem> entry in cc.InventoryItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawInventoryItem> entry in tagEntry.Value.InventoryItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.InventoryItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(InventoryItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.InventoryItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.InventoryItemMapAB != null)
			{
				foreach (var entry in cc.InventoryItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.InventoryItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.InventoryItemMap.MergeFrom(cis);
			Patch(updating.InventoryItemMap);
			updating.InventoryItemSortedKeys = genSortedKeys(updating.InventoryItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new InventoryItemConf();
            tmp.MergeFrom(cis);
			updating.InventoryItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().InventoryItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawInventoryItem> filter,string tag=""){
		...
		public static rawInventoryItem MustGet(int key,string tag="") {
		...
		public static rawInventoryItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawInventoryItem> GetByFilter(Func<rawInventoryItem, bool> filterFunc,string tag="") {
			List<rawInventoryItem> ret = new List<rawInventoryItem>();
			foreach(KeyValuePair <int, rawInventoryItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawInventoryItem? GetOneByFilter(Func<rawInventoryItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawInventoryItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawInventoryItem data) 
		{
			return data.Id;			
		}
    }
