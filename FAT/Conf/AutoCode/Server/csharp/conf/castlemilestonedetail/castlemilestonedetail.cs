...
using CastleMilestoneDetailConf = fat.rawdata.CastleMilestoneDetailConf;
using rawCastleMilestoneDetail = fat.rawdata.CastleMilestoneDetail;
using rawCastleMilestoneDetailMapABValue = fat.rawdata.CastleMilestoneDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CastleMilestoneDetailVisitor
    {
		...
		public const string FileName = "CastleMilestoneDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CastleMilestoneDetailVisitor.SetABDefault单独设定CastleMilestoneDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CastleMilestoneDetailVisitor.SetABFallback单独设定CastleMilestoneDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCastleMilestoneDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCastleMilestoneDetail>>(()=> new PBC::MapField<int, rawCastleMilestoneDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCastleMilestoneDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CastleMilestoneDetailMap;
			var abMap = v.CastleMilestoneDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CastleMilestoneDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CastleMilestoneDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCastleMilestoneDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CastleMilestoneDetailMap;
			if (v.CastleMilestoneDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CastleMilestoneDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCastleMilestoneDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CastleMilestoneDetailConf cc) {
			foreach(KeyValuePair <string, rawCastleMilestoneDetailMapABValue> tagEntry in cc.CastleMilestoneDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CastleMilestoneDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCastleMilestoneDetailMapABValue();
					cc.CastleMilestoneDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.CastleMilestoneDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCastleMilestoneDetail> entry in cc.CastleMilestoneDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCastleMilestoneDetail> entry in tagEntry.Value.CastleMilestoneDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CastleMilestoneDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CastleMilestoneDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CastleMilestoneDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CastleMilestoneDetailMapAB != null)
			{
				foreach (var entry in cc.CastleMilestoneDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CastleMilestoneDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CastleMilestoneDetailMap.MergeFrom(cis);
			Patch(updating.CastleMilestoneDetailMap);
			updating.CastleMilestoneDetailSortedKeys = genSortedKeys(updating.CastleMilestoneDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CastleMilestoneDetailConf();
            tmp.MergeFrom(cis);
			updating.CastleMilestoneDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CastleMilestoneDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCastleMilestoneDetail> filter,string tag=""){
		...
		public static rawCastleMilestoneDetail MustGet(int key,string tag="") {
		...
		public static rawCastleMilestoneDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCastleMilestoneDetail> GetByFilter(Func<rawCastleMilestoneDetail, bool> filterFunc,string tag="") {
			List<rawCastleMilestoneDetail> ret = new List<rawCastleMilestoneDetail>();
			foreach(KeyValuePair <int, rawCastleMilestoneDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCastleMilestoneDetail? GetOneByFilter(Func<rawCastleMilestoneDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCastleMilestoneDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCastleMilestoneDetail data) 
		{
			return data.Id;			
		}
    }
