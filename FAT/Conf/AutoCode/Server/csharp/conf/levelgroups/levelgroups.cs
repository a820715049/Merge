...
using LevelGroupsConf = fat.rawdata.LevelGroupsConf;
using rawLevelGroups = fat.rawdata.LevelGroups;
using rawLevelGroupsMapABValue = fat.rawdata.LevelGroupsMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LevelGroupsVisitor
    {
		...
		public const string FileName = "LevelGroupsConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LevelGroupsVisitor.SetABDefault单独设定LevelGroups的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LevelGroupsVisitor.SetABFallback单独设定LevelGroups的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLevelGroups>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLevelGroups>>(()=> new PBC::MapField<int, rawLevelGroups>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLevelGroups>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LevelGroupsMap;
			var abMap = v.LevelGroupsMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LevelGroupsMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LevelGroupsMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLevelGroups> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LevelGroupsMap;
			if (v.LevelGroupsMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LevelGroupsMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLevelGroupsMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LevelGroupsConf cc) {
			foreach(KeyValuePair <string, rawLevelGroupsMapABValue> tagEntry in cc.LevelGroupsMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LevelGroupsMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLevelGroupsMapABValue();
					cc.LevelGroupsMapAB[tag] = ps;
				}
                var tagTmp = ps.LevelGroupsMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLevelGroups> entry in cc.LevelGroupsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLevelGroups> entry in tagEntry.Value.LevelGroupsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LevelGroups> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LevelGroupsConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LevelGroupsMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LevelGroupsMapAB != null)
			{
				foreach (var entry in cc.LevelGroupsMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LevelGroupsMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LevelGroupsMap.MergeFrom(cis);
			Patch(updating.LevelGroupsMap);
			updating.LevelGroupsSortedKeys = genSortedKeys(updating.LevelGroupsMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LevelGroupsConf();
            tmp.MergeFrom(cis);
			updating.LevelGroupsMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LevelGroupsSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLevelGroups> filter,string tag=""){
		...
		public static rawLevelGroups MustGet(int key,string tag="") {
		...
		public static rawLevelGroups? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLevelGroups> GetByFilter(Func<rawLevelGroups, bool> filterFunc,string tag="") {
			List<rawLevelGroups> ret = new List<rawLevelGroups>();
			foreach(KeyValuePair <int, rawLevelGroups> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLevelGroups? GetOneByFilter(Func<rawLevelGroups, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLevelGroups> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLevelGroups data) 
		{
			return data.Id;			
		}
    }
