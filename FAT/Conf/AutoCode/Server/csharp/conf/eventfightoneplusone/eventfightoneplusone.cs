...
using EventFightOnePlusOneConf = fat.rawdata.EventFightOnePlusOneConf;
using rawEventFightOnePlusOne = fat.rawdata.EventFightOnePlusOne;
using rawEventFightOnePlusOneMapABValue = fat.rawdata.EventFightOnePlusOneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFightOnePlusOneVisitor
    {
		...
		public const string FileName = "EventFightOnePlusOneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFightOnePlusOneVisitor.SetABDefault单独设定EventFightOnePlusOne的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFightOnePlusOneVisitor.SetABFallback单独设定EventFightOnePlusOne的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFightOnePlusOne>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFightOnePlusOne>>(()=> new PBC::MapField<int, rawEventFightOnePlusOne>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFightOnePlusOne>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFightOnePlusOneMap;
			var abMap = v.EventFightOnePlusOneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFightOnePlusOneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFightOnePlusOneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFightOnePlusOne> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFightOnePlusOneMap;
			if (v.EventFightOnePlusOneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFightOnePlusOneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFightOnePlusOneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFightOnePlusOneConf cc) {
			foreach(KeyValuePair <string, rawEventFightOnePlusOneMapABValue> tagEntry in cc.EventFightOnePlusOneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFightOnePlusOneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFightOnePlusOneMapABValue();
					cc.EventFightOnePlusOneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFightOnePlusOneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFightOnePlusOne> entry in cc.EventFightOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFightOnePlusOne> entry in tagEntry.Value.EventFightOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFightOnePlusOne> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFightOnePlusOneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFightOnePlusOneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFightOnePlusOneMapAB != null)
			{
				foreach (var entry in cc.EventFightOnePlusOneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFightOnePlusOneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFightOnePlusOneMap.MergeFrom(cis);
			Patch(updating.EventFightOnePlusOneMap);
			updating.EventFightOnePlusOneSortedKeys = genSortedKeys(updating.EventFightOnePlusOneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFightOnePlusOneConf();
            tmp.MergeFrom(cis);
			updating.EventFightOnePlusOneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFightOnePlusOneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFightOnePlusOne> filter,string tag=""){
		...
		public static rawEventFightOnePlusOne MustGet(int key,string tag="") {
		...
		public static rawEventFightOnePlusOne? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFightOnePlusOne> GetByFilter(Func<rawEventFightOnePlusOne, bool> filterFunc,string tag="") {
			List<rawEventFightOnePlusOne> ret = new List<rawEventFightOnePlusOne>();
			foreach(KeyValuePair <int, rawEventFightOnePlusOne> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFightOnePlusOne? GetOneByFilter(Func<rawEventFightOnePlusOne, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFightOnePlusOne> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFightOnePlusOne data) 
		{
			return data.Id;			
		}
    }
