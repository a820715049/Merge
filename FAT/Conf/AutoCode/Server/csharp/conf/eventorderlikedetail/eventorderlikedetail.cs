...
using EventOrderLikeDetailConf = fat.rawdata.EventOrderLikeDetailConf;
using rawEventOrderLikeDetail = fat.rawdata.EventOrderLikeDetail;
using rawEventOrderLikeDetailMapABValue = fat.rawdata.EventOrderLikeDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderLikeDetailVisitor
    {
		...
		public const string FileName = "EventOrderLikeDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderLikeDetailVisitor.SetABDefault单独设定EventOrderLikeDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderLikeDetailVisitor.SetABFallback单独设定EventOrderLikeDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderLikeDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderLikeDetail>>(()=> new PBC::MapField<int, rawEventOrderLikeDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderLikeDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderLikeDetailMap;
			var abMap = v.EventOrderLikeDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderLikeDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderLikeDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderLikeDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderLikeDetailMap;
			if (v.EventOrderLikeDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderLikeDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderLikeDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderLikeDetailConf cc) {
			foreach(KeyValuePair <string, rawEventOrderLikeDetailMapABValue> tagEntry in cc.EventOrderLikeDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderLikeDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderLikeDetailMapABValue();
					cc.EventOrderLikeDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderLikeDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderLikeDetail> entry in cc.EventOrderLikeDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderLikeDetail> entry in tagEntry.Value.EventOrderLikeDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderLikeDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderLikeDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderLikeDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderLikeDetailMapAB != null)
			{
				foreach (var entry in cc.EventOrderLikeDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderLikeDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderLikeDetailMap.MergeFrom(cis);
			Patch(updating.EventOrderLikeDetailMap);
			updating.EventOrderLikeDetailSortedKeys = genSortedKeys(updating.EventOrderLikeDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderLikeDetailConf();
            tmp.MergeFrom(cis);
			updating.EventOrderLikeDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderLikeDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderLikeDetail> filter,string tag=""){
		...
		public static rawEventOrderLikeDetail MustGet(int key,string tag="") {
		...
		public static rawEventOrderLikeDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderLikeDetail> GetByFilter(Func<rawEventOrderLikeDetail, bool> filterFunc,string tag="") {
			List<rawEventOrderLikeDetail> ret = new List<rawEventOrderLikeDetail>();
			foreach(KeyValuePair <int, rawEventOrderLikeDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderLikeDetail? GetOneByFilter(Func<rawEventOrderLikeDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderLikeDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderLikeDetail data) 
		{
			return data.Id;			
		}
    }
