...
using ObjBasicConf = fat.rawdata.ObjBasicConf;
using rawObjBasic = fat.rawdata.ObjBasic;
using rawObjBasicMapABValue = fat.rawdata.ObjBasicMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjBasicVisitor
    {
		...
		public const string FileName = "ObjBasicConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjBasicVisitor.SetABDefault单独设定ObjBasic的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjBasicVisitor.SetABFallback单独设定ObjBasic的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjBasic>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjBasic>>(()=> new PBC::MapField<int, rawObjBasic>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjBasic>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjBasicMap;
			var abMap = v.ObjBasicMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjBasicMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjBasicMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjBasic> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjBasicMap;
			if (v.ObjBasicMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjBasicMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjBasicMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjBasicConf cc) {
			foreach(KeyValuePair <string, rawObjBasicMapABValue> tagEntry in cc.ObjBasicMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjBasicMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjBasicMapABValue();
					cc.ObjBasicMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjBasicMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjBasic> entry in cc.ObjBasicMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjBasic> entry in tagEntry.Value.ObjBasicMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjBasic> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjBasicConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjBasicMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjBasicMapAB != null)
			{
				foreach (var entry in cc.ObjBasicMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjBasicMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjBasicMap.MergeFrom(cis);
			Patch(updating.ObjBasicMap);
			updating.ObjBasicSortedKeys = genSortedKeys(updating.ObjBasicMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjBasicConf();
            tmp.MergeFrom(cis);
			updating.ObjBasicMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjBasicSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjBasic> filter,string tag=""){
		...
		public static rawObjBasic MustGet(int key,string tag="") {
		...
		public static rawObjBasic? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjBasic> GetByFilter(Func<rawObjBasic, bool> filterFunc,string tag="") {
			List<rawObjBasic> ret = new List<rawObjBasic>();
			foreach(KeyValuePair <int, rawObjBasic> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjBasic? GetOneByFilter(Func<rawObjBasic, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjBasic> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjBasic data) 
		{
			return data.Id;			
		}
    }
