...
using TrainMissionConf = fat.rawdata.TrainMissionConf;
using rawTrainMission = fat.rawdata.TrainMission;
using rawTrainMissionMapABValue = fat.rawdata.TrainMissionMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class TrainMissionVisitor
    {
		...
		public const string FileName = "TrainMissionConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过TrainMissionVisitor.SetABDefault单独设定TrainMission的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过TrainMissionVisitor.SetABFallback单独设定TrainMission的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawTrainMission>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawTrainMission>>(()=> new PBC::MapField<int, rawTrainMission>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawTrainMission>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().TrainMissionMap;
			var abMap = v.TrainMissionMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.TrainMissionMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].TrainMissionMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawTrainMission> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().TrainMissionMap;
			if (v.TrainMissionMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.TrainMissionMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawTrainMissionMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(TrainMissionConf cc) {
			foreach(KeyValuePair <string, rawTrainMissionMapABValue> tagEntry in cc.TrainMissionMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.TrainMissionMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawTrainMissionMapABValue();
					cc.TrainMissionMapAB[tag] = ps;
				}
                var tagTmp = ps.TrainMissionMap;
				// 基础数据
				foreach(KeyValuePair <int, rawTrainMission> entry in cc.TrainMissionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawTrainMission> entry in tagEntry.Value.TrainMissionMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.TrainMission> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(TrainMissionConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.TrainMissionMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.TrainMissionMapAB != null)
			{
				foreach (var entry in cc.TrainMissionMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.TrainMissionMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.TrainMissionMap.MergeFrom(cis);
			Patch(updating.TrainMissionMap);
			updating.TrainMissionSortedKeys = genSortedKeys(updating.TrainMissionMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new TrainMissionConf();
            tmp.MergeFrom(cis);
			updating.TrainMissionMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().TrainMissionSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawTrainMission> filter,string tag=""){
		...
		public static rawTrainMission MustGet(int key,string tag="") {
		...
		public static rawTrainMission? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawTrainMission> GetByFilter(Func<rawTrainMission, bool> filterFunc,string tag="") {
			List<rawTrainMission> ret = new List<rawTrainMission>();
			foreach(KeyValuePair <int, rawTrainMission> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawTrainMission? GetOneByFilter(Func<rawTrainMission, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawTrainMission> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawTrainMission data) 
		{
			return data.Id;			
		}
    }
