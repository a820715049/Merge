...
using GemEndlessThreeConf = fat.rawdata.GemEndlessThreeConf;
using rawGemEndlessThree = fat.rawdata.GemEndlessThree;
using rawGemEndlessThreeMapABValue = fat.rawdata.GemEndlessThreeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GemEndlessThreeVisitor
    {
		...
		public const string FileName = "GemEndlessThreeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GemEndlessThreeVisitor.SetABDefault单独设定GemEndlessThree的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GemEndlessThreeVisitor.SetABFallback单独设定GemEndlessThree的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGemEndlessThree>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGemEndlessThree>>(()=> new PBC::MapField<int, rawGemEndlessThree>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGemEndlessThree>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GemEndlessThreeMap;
			var abMap = v.GemEndlessThreeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GemEndlessThreeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GemEndlessThreeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGemEndlessThree> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GemEndlessThreeMap;
			if (v.GemEndlessThreeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GemEndlessThreeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGemEndlessThreeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GemEndlessThreeConf cc) {
			foreach(KeyValuePair <string, rawGemEndlessThreeMapABValue> tagEntry in cc.GemEndlessThreeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GemEndlessThreeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGemEndlessThreeMapABValue();
					cc.GemEndlessThreeMapAB[tag] = ps;
				}
                var tagTmp = ps.GemEndlessThreeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGemEndlessThree> entry in cc.GemEndlessThreeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGemEndlessThree> entry in tagEntry.Value.GemEndlessThreeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GemEndlessThree> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GemEndlessThreeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GemEndlessThreeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GemEndlessThreeMapAB != null)
			{
				foreach (var entry in cc.GemEndlessThreeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GemEndlessThreeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GemEndlessThreeMap.MergeFrom(cis);
			Patch(updating.GemEndlessThreeMap);
			updating.GemEndlessThreeSortedKeys = genSortedKeys(updating.GemEndlessThreeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GemEndlessThreeConf();
            tmp.MergeFrom(cis);
			updating.GemEndlessThreeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GemEndlessThreeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGemEndlessThree> filter,string tag=""){
		...
		public static rawGemEndlessThree MustGet(int key,string tag="") {
		...
		public static rawGemEndlessThree? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGemEndlessThree> GetByFilter(Func<rawGemEndlessThree, bool> filterFunc,string tag="") {
			List<rawGemEndlessThree> ret = new List<rawGemEndlessThree>();
			foreach(KeyValuePair <int, rawGemEndlessThree> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGemEndlessThree? GetOneByFilter(Func<rawGemEndlessThree, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGemEndlessThree> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGemEndlessThree data) 
		{
			return data.Id;			
		}
    }
