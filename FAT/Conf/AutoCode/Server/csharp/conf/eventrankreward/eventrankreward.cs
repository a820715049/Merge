...
using EventRankRewardConf = fat.rawdata.EventRankRewardConf;
using rawEventRankReward = fat.rawdata.EventRankReward;
using rawEventRankRewardMapABValue = fat.rawdata.EventRankRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRankRewardVisitor
    {
		...
		public const string FileName = "EventRankRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRankRewardVisitor.SetABDefault单独设定EventRankReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRankRewardVisitor.SetABFallback单独设定EventRankReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRankReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRankReward>>(()=> new PBC::MapField<int, rawEventRankReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRankReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRankRewardMap;
			var abMap = v.EventRankRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRankRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRankRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRankReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRankRewardMap;
			if (v.EventRankRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRankRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRankRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRankRewardConf cc) {
			foreach(KeyValuePair <string, rawEventRankRewardMapABValue> tagEntry in cc.EventRankRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRankRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRankRewardMapABValue();
					cc.EventRankRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRankRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRankReward> entry in cc.EventRankRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRankReward> entry in tagEntry.Value.EventRankRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRankReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRankRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRankRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRankRewardMapAB != null)
			{
				foreach (var entry in cc.EventRankRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRankRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRankRewardMap.MergeFrom(cis);
			Patch(updating.EventRankRewardMap);
			updating.EventRankRewardSortedKeys = genSortedKeys(updating.EventRankRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRankRewardConf();
            tmp.MergeFrom(cis);
			updating.EventRankRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRankRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRankReward> filter,string tag=""){
		...
		public static rawEventRankReward MustGet(int key,string tag="") {
		...
		public static rawEventRankReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRankReward> GetByFilter(Func<rawEventRankReward, bool> filterFunc,string tag="") {
			List<rawEventRankReward> ret = new List<rawEventRankReward>();
			foreach(KeyValuePair <int, rawEventRankReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRankReward? GetOneByFilter(Func<rawEventRankReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRankReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRankReward data) 
		{
			return data.Id;			
		}
    }
