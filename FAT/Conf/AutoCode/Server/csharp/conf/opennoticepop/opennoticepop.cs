...
using OpenNoticePopConf = fat.rawdata.OpenNoticePopConf;
using rawOpenNoticePop = fat.rawdata.OpenNoticePop;
using rawOpenNoticePopSliceABValue = fat.rawdata.OpenNoticePopSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OpenNoticePopVisitor
    {
		...
		public const string FileName = "OpenNoticePopConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OpenNoticePopVisitor.SetABDefault单独设定OpenNoticePop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OpenNoticePopVisitor.SetABFallback单独设定OpenNoticePop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawOpenNoticePop>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawOpenNoticePop>>(()=>new PBC::RepeatedField<rawOpenNoticePop>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawOpenNoticePop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OpenNoticePopSlice;
			var abMap = v.OpenNoticePopSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OpenNoticePopSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OpenNoticePopSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawOpenNoticePop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OpenNoticePopSlice;
			if (v.OpenNoticePopSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OpenNoticePopSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOpenNoticePopSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OpenNoticePopConf cc) {
			foreach(KeyValuePair <string, rawOpenNoticePopSliceABValue> tagEntry in cc.OpenNoticePopSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OpenNoticePopSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOpenNoticePopSliceABValue();
					cc.OpenNoticePopSliceAB[tag] = ps;
				}
                var tagTmp = ps.OpenNoticePopSlice;
				// 基础数据
				for(int i=0; i<cc.OpenNoticePopSlice.Count; ++i) 
				{
					tagTmp.Add(cc.OpenNoticePopSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.OpenNoticePopSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.OpenNoticePop> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OpenNoticePopSlice.MergeFrom(cis);
			Patch(updating.OpenNoticePopSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OpenNoticePopConf();
            tmp.MergeFrom(cis);
			updating.OpenNoticePopSlice = tmp;
		...
		public static void Range(Action<int, rawOpenNoticePop> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawOpenNoticePop MustGetByIndex(int index,string tag="") {
		...
		public static rawOpenNoticePop? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawOpenNoticePop> GetByFilter(Func<rawOpenNoticePop,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawOpenNoticePop>();
		...
		static public rawOpenNoticePop? GetOneByFilter(Func<rawOpenNoticePop,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
