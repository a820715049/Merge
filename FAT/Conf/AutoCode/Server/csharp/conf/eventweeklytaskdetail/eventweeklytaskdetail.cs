...
using EventWeeklyTaskDetailConf = fat.rawdata.EventWeeklyTaskDetailConf;
using rawEventWeeklyTaskDetail = fat.rawdata.EventWeeklyTaskDetail;
using rawEventWeeklyTaskDetailMapABValue = fat.rawdata.EventWeeklyTaskDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyTaskDetailVisitor
    {
		...
		public const string FileName = "EventWeeklyTaskDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyTaskDetailVisitor.SetABDefault单独设定EventWeeklyTaskDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyTaskDetailVisitor.SetABFallback单独设定EventWeeklyTaskDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyTaskDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyTaskDetail>>(()=> new PBC::MapField<int, rawEventWeeklyTaskDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyTaskDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyTaskDetailMap;
			var abMap = v.EventWeeklyTaskDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyTaskDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyTaskDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyTaskDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyTaskDetailMap;
			if (v.EventWeeklyTaskDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyTaskDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyTaskDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyTaskDetailConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyTaskDetailMapABValue> tagEntry in cc.EventWeeklyTaskDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyTaskDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyTaskDetailMapABValue();
					cc.EventWeeklyTaskDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyTaskDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskDetail> entry in cc.EventWeeklyTaskDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyTaskDetail> entry in tagEntry.Value.EventWeeklyTaskDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyTaskDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyTaskDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyTaskDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyTaskDetailMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyTaskDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyTaskDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyTaskDetailMap.MergeFrom(cis);
			Patch(updating.EventWeeklyTaskDetailMap);
			updating.EventWeeklyTaskDetailSortedKeys = genSortedKeys(updating.EventWeeklyTaskDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyTaskDetailConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyTaskDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyTaskDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyTaskDetail> filter,string tag=""){
		...
		public static rawEventWeeklyTaskDetail MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyTaskDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyTaskDetail> GetByFilter(Func<rawEventWeeklyTaskDetail, bool> filterFunc,string tag="") {
			List<rawEventWeeklyTaskDetail> ret = new List<rawEventWeeklyTaskDetail>();
			foreach(KeyValuePair <int, rawEventWeeklyTaskDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyTaskDetail? GetOneByFilter(Func<rawEventWeeklyTaskDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyTaskDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyTaskDetail data) 
		{
			return data.Id;			
		}
    }
