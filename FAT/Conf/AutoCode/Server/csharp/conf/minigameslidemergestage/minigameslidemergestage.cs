...
using MiniGameSlideMergeStageConf = fat.rawdata.MiniGameSlideMergeStageConf;
using rawMiniGameSlideMergeStage = fat.rawdata.MiniGameSlideMergeStage;
using rawMiniGameSlideMergeStageMapABValue = fat.rawdata.MiniGameSlideMergeStageMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameSlideMergeStageVisitor
    {
		...
		public const string FileName = "MiniGameSlideMergeStageConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameSlideMergeStageVisitor.SetABDefault单独设定MiniGameSlideMergeStage的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameSlideMergeStageVisitor.SetABFallback单独设定MiniGameSlideMergeStage的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameSlideMergeStage>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameSlideMergeStage>>(()=> new PBC::MapField<int, rawMiniGameSlideMergeStage>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameSlideMergeStage>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameSlideMergeStageMap;
			var abMap = v.MiniGameSlideMergeStageMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameSlideMergeStageMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameSlideMergeStageMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameSlideMergeStage> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameSlideMergeStageMap;
			if (v.MiniGameSlideMergeStageMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameSlideMergeStageMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameSlideMergeStageMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameSlideMergeStageConf cc) {
			foreach(KeyValuePair <string, rawMiniGameSlideMergeStageMapABValue> tagEntry in cc.MiniGameSlideMergeStageMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameSlideMergeStageMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameSlideMergeStageMapABValue();
					cc.MiniGameSlideMergeStageMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameSlideMergeStageMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeStage> entry in cc.MiniGameSlideMergeStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeStage> entry in tagEntry.Value.MiniGameSlideMergeStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameSlideMergeStage> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameSlideMergeStageConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameSlideMergeStageMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameSlideMergeStageMapAB != null)
			{
				foreach (var entry in cc.MiniGameSlideMergeStageMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameSlideMergeStageMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameSlideMergeStageMap.MergeFrom(cis);
			Patch(updating.MiniGameSlideMergeStageMap);
			updating.MiniGameSlideMergeStageSortedKeys = genSortedKeys(updating.MiniGameSlideMergeStageMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameSlideMergeStageConf();
            tmp.MergeFrom(cis);
			updating.MiniGameSlideMergeStageMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameSlideMergeStageSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameSlideMergeStage> filter,string tag=""){
		...
		public static rawMiniGameSlideMergeStage MustGet(int key,string tag="") {
		...
		public static rawMiniGameSlideMergeStage? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameSlideMergeStage> GetByFilter(Func<rawMiniGameSlideMergeStage, bool> filterFunc,string tag="") {
			List<rawMiniGameSlideMergeStage> ret = new List<rawMiniGameSlideMergeStage>();
			foreach(KeyValuePair <int, rawMiniGameSlideMergeStage> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameSlideMergeStage? GetOneByFilter(Func<rawMiniGameSlideMergeStage, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameSlideMergeStage> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameSlideMergeStage data) 
		{
			return data.Id;			
		}
    }
