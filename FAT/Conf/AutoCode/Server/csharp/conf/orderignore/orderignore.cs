...
using OrderIgnoreConf = fat.rawdata.OrderIgnoreConf;
using rawOrderIgnore = fat.rawdata.OrderIgnore;
using rawOrderIgnoreMapABValue = fat.rawdata.OrderIgnoreMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderIgnoreVisitor
    {
		...
		public const string FileName = "OrderIgnoreConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderIgnoreVisitor.SetABDefault单独设定OrderIgnore的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderIgnoreVisitor.SetABFallback单独设定OrderIgnore的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderIgnore>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderIgnore>>(()=> new PBC::MapField<int, rawOrderIgnore>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderIgnore>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderIgnoreMap;
			var abMap = v.OrderIgnoreMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderIgnoreMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderIgnoreMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderIgnore> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderIgnoreMap;
			if (v.OrderIgnoreMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderIgnoreMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderIgnoreMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderIgnoreConf cc) {
			foreach(KeyValuePair <string, rawOrderIgnoreMapABValue> tagEntry in cc.OrderIgnoreMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderIgnoreMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderIgnoreMapABValue();
					cc.OrderIgnoreMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderIgnoreMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderIgnore> entry in cc.OrderIgnoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderIgnore> entry in tagEntry.Value.OrderIgnoreMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderIgnore> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderIgnoreConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderIgnoreMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderIgnoreMapAB != null)
			{
				foreach (var entry in cc.OrderIgnoreMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderIgnoreMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderIgnoreMap.MergeFrom(cis);
			Patch(updating.OrderIgnoreMap);
			updating.OrderIgnoreSortedKeys = genSortedKeys(updating.OrderIgnoreMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderIgnoreConf();
            tmp.MergeFrom(cis);
			updating.OrderIgnoreMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderIgnoreSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderIgnore> filter,string tag=""){
		...
		public static rawOrderIgnore MustGet(int key,string tag="") {
		...
		public static rawOrderIgnore? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderIgnore> GetByFilter(Func<rawOrderIgnore, bool> filterFunc,string tag="") {
			List<rawOrderIgnore> ret = new List<rawOrderIgnore>();
			foreach(KeyValuePair <int, rawOrderIgnore> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderIgnore? GetOneByFilter(Func<rawOrderIgnore, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderIgnore> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderIgnore data) 
		{
			return data.Id;			
		}
    }
