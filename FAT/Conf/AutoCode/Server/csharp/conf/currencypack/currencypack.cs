...
using CurrencyPackConf = fat.rawdata.CurrencyPackConf;
using rawCurrencyPack = fat.rawdata.CurrencyPack;
using rawCurrencyPackMapABValue = fat.rawdata.CurrencyPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CurrencyPackVisitor
    {
		...
		public const string FileName = "CurrencyPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CurrencyPackVisitor.SetABDefault单独设定CurrencyPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CurrencyPackVisitor.SetABFallback单独设定CurrencyPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCurrencyPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCurrencyPack>>(()=> new PBC::MapField<int, rawCurrencyPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCurrencyPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CurrencyPackMap;
			var abMap = v.CurrencyPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CurrencyPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CurrencyPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCurrencyPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CurrencyPackMap;
			if (v.CurrencyPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CurrencyPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCurrencyPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CurrencyPackConf cc) {
			foreach(KeyValuePair <string, rawCurrencyPackMapABValue> tagEntry in cc.CurrencyPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CurrencyPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCurrencyPackMapABValue();
					cc.CurrencyPackMapAB[tag] = ps;
				}
                var tagTmp = ps.CurrencyPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCurrencyPack> entry in cc.CurrencyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCurrencyPack> entry in tagEntry.Value.CurrencyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CurrencyPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CurrencyPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CurrencyPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CurrencyPackMapAB != null)
			{
				foreach (var entry in cc.CurrencyPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CurrencyPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CurrencyPackMap.MergeFrom(cis);
			Patch(updating.CurrencyPackMap);
			updating.CurrencyPackSortedKeys = genSortedKeys(updating.CurrencyPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CurrencyPackConf();
            tmp.MergeFrom(cis);
			updating.CurrencyPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CurrencyPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCurrencyPack> filter,string tag=""){
		...
		public static rawCurrencyPack MustGet(int key,string tag="") {
		...
		public static rawCurrencyPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCurrencyPack> GetByFilter(Func<rawCurrencyPack, bool> filterFunc,string tag="") {
			List<rawCurrencyPack> ret = new List<rawCurrencyPack>();
			foreach(KeyValuePair <int, rawCurrencyPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCurrencyPack? GetOneByFilter(Func<rawCurrencyPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCurrencyPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCurrencyPack data) 
		{
			return data.Id;			
		}
    }
