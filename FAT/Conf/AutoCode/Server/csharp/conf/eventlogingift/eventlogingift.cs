...
using EventLoginGiftConf = fat.rawdata.EventLoginGiftConf;
using rawEventLoginGift = fat.rawdata.EventLoginGift;
using rawEventLoginGiftMapABValue = fat.rawdata.EventLoginGiftMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventLoginGiftVisitor
    {
		...
		public const string FileName = "EventLoginGiftConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventLoginGiftVisitor.SetABDefault单独设定EventLoginGift的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventLoginGiftVisitor.SetABFallback单独设定EventLoginGift的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventLoginGift>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventLoginGift>>(()=> new PBC::MapField<int, rawEventLoginGift>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventLoginGift>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventLoginGiftMap;
			var abMap = v.EventLoginGiftMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventLoginGiftMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventLoginGiftMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventLoginGift> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventLoginGiftMap;
			if (v.EventLoginGiftMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventLoginGiftMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventLoginGiftMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventLoginGiftConf cc) {
			foreach(KeyValuePair <string, rawEventLoginGiftMapABValue> tagEntry in cc.EventLoginGiftMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventLoginGiftMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventLoginGiftMapABValue();
					cc.EventLoginGiftMapAB[tag] = ps;
				}
                var tagTmp = ps.EventLoginGiftMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventLoginGift> entry in cc.EventLoginGiftMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventLoginGift> entry in tagEntry.Value.EventLoginGiftMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventLoginGift> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventLoginGiftConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventLoginGiftMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventLoginGiftMapAB != null)
			{
				foreach (var entry in cc.EventLoginGiftMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventLoginGiftMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventLoginGiftMap.MergeFrom(cis);
			Patch(updating.EventLoginGiftMap);
			updating.EventLoginGiftSortedKeys = genSortedKeys(updating.EventLoginGiftMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventLoginGiftConf();
            tmp.MergeFrom(cis);
			updating.EventLoginGiftMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventLoginGiftSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventLoginGift> filter,string tag=""){
		...
		public static rawEventLoginGift MustGet(int key,string tag="") {
		...
		public static rawEventLoginGift? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventLoginGift> GetByFilter(Func<rawEventLoginGift, bool> filterFunc,string tag="") {
			List<rawEventLoginGift> ret = new List<rawEventLoginGift>();
			foreach(KeyValuePair <int, rawEventLoginGift> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventLoginGift? GetOneByFilter(Func<rawEventLoginGift, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventLoginGift> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventLoginGift data) 
		{
			return data.Id;			
		}
    }
