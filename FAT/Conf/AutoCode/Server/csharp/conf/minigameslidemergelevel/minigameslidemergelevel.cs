...
using MiniGameSlideMergeLevelConf = fat.rawdata.MiniGameSlideMergeLevelConf;
using rawMiniGameSlideMergeLevel = fat.rawdata.MiniGameSlideMergeLevel;
using rawMiniGameSlideMergeLevelMapABValue = fat.rawdata.MiniGameSlideMergeLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameSlideMergeLevelVisitor
    {
		...
		public const string FileName = "MiniGameSlideMergeLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameSlideMergeLevelVisitor.SetABDefault单独设定MiniGameSlideMergeLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameSlideMergeLevelVisitor.SetABFallback单独设定MiniGameSlideMergeLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameSlideMergeLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameSlideMergeLevel>>(()=> new PBC::MapField<int, rawMiniGameSlideMergeLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameSlideMergeLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameSlideMergeLevelMap;
			var abMap = v.MiniGameSlideMergeLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameSlideMergeLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameSlideMergeLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameSlideMergeLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameSlideMergeLevelMap;
			if (v.MiniGameSlideMergeLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameSlideMergeLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameSlideMergeLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameSlideMergeLevelConf cc) {
			foreach(KeyValuePair <string, rawMiniGameSlideMergeLevelMapABValue> tagEntry in cc.MiniGameSlideMergeLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameSlideMergeLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameSlideMergeLevelMapABValue();
					cc.MiniGameSlideMergeLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameSlideMergeLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeLevel> entry in cc.MiniGameSlideMergeLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameSlideMergeLevel> entry in tagEntry.Value.MiniGameSlideMergeLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameSlideMergeLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameSlideMergeLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameSlideMergeLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameSlideMergeLevelMapAB != null)
			{
				foreach (var entry in cc.MiniGameSlideMergeLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameSlideMergeLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameSlideMergeLevelMap.MergeFrom(cis);
			Patch(updating.MiniGameSlideMergeLevelMap);
			updating.MiniGameSlideMergeLevelSortedKeys = genSortedKeys(updating.MiniGameSlideMergeLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameSlideMergeLevelConf();
            tmp.MergeFrom(cis);
			updating.MiniGameSlideMergeLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameSlideMergeLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameSlideMergeLevel> filter,string tag=""){
		...
		public static rawMiniGameSlideMergeLevel MustGet(int key,string tag="") {
		...
		public static rawMiniGameSlideMergeLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameSlideMergeLevel> GetByFilter(Func<rawMiniGameSlideMergeLevel, bool> filterFunc,string tag="") {
			List<rawMiniGameSlideMergeLevel> ret = new List<rawMiniGameSlideMergeLevel>();
			foreach(KeyValuePair <int, rawMiniGameSlideMergeLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameSlideMergeLevel? GetOneByFilter(Func<rawMiniGameSlideMergeLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameSlideMergeLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameSlideMergeLevel data) 
		{
			return data.Id;			
		}
    }
