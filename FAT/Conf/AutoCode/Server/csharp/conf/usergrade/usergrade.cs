...
using UserGradeConf = fat.rawdata.UserGradeConf;
using rawUserGrade = fat.rawdata.UserGrade;
using rawUserGradeMapABValue = fat.rawdata.UserGradeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class UserGradeVisitor
    {
		...
		public const string FileName = "UserGradeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过UserGradeVisitor.SetABDefault单独设定UserGrade的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过UserGradeVisitor.SetABFallback单独设定UserGrade的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawUserGrade>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawUserGrade>>(()=> new PBC::MapField<int, rawUserGrade>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawUserGrade>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().UserGradeMap;
			var abMap = v.UserGradeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.UserGradeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].UserGradeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawUserGrade> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().UserGradeMap;
			if (v.UserGradeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.UserGradeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawUserGradeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(UserGradeConf cc) {
			foreach(KeyValuePair <string, rawUserGradeMapABValue> tagEntry in cc.UserGradeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.UserGradeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawUserGradeMapABValue();
					cc.UserGradeMapAB[tag] = ps;
				}
                var tagTmp = ps.UserGradeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawUserGrade> entry in cc.UserGradeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawUserGrade> entry in tagEntry.Value.UserGradeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.UserGrade> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(UserGradeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.UserGradeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.UserGradeMapAB != null)
			{
				foreach (var entry in cc.UserGradeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.UserGradeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.UserGradeMap.MergeFrom(cis);
			Patch(updating.UserGradeMap);
			updating.UserGradeSortedKeys = genSortedKeys(updating.UserGradeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new UserGradeConf();
            tmp.MergeFrom(cis);
			updating.UserGradeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().UserGradeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawUserGrade> filter,string tag=""){
		...
		public static rawUserGrade MustGet(int key,string tag="") {
		...
		public static rawUserGrade? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawUserGrade> GetByFilter(Func<rawUserGrade, bool> filterFunc,string tag="") {
			List<rawUserGrade> ret = new List<rawUserGrade>();
			foreach(KeyValuePair <int, rawUserGrade> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawUserGrade? GetOneByFilter(Func<rawUserGrade, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawUserGrade> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawUserGrade data) 
		{
			return data.Id;			
		}
    }
