...
using EventFarmEndlessInfoConf = fat.rawdata.EventFarmEndlessInfoConf;
using rawEventFarmEndlessInfo = fat.rawdata.EventFarmEndlessInfo;
using rawEventFarmEndlessInfoMapABValue = fat.rawdata.EventFarmEndlessInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmEndlessInfoVisitor
    {
		...
		public const string FileName = "EventFarmEndlessInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmEndlessInfoVisitor.SetABDefault单独设定EventFarmEndlessInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmEndlessInfoVisitor.SetABFallback单独设定EventFarmEndlessInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmEndlessInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmEndlessInfo>>(()=> new PBC::MapField<int, rawEventFarmEndlessInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmEndlessInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmEndlessInfoMap;
			var abMap = v.EventFarmEndlessInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmEndlessInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmEndlessInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmEndlessInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmEndlessInfoMap;
			if (v.EventFarmEndlessInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmEndlessInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmEndlessInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmEndlessInfoConf cc) {
			foreach(KeyValuePair <string, rawEventFarmEndlessInfoMapABValue> tagEntry in cc.EventFarmEndlessInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmEndlessInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmEndlessInfoMapABValue();
					cc.EventFarmEndlessInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmEndlessInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmEndlessInfo> entry in cc.EventFarmEndlessInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmEndlessInfo> entry in tagEntry.Value.EventFarmEndlessInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmEndlessInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmEndlessInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmEndlessInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmEndlessInfoMapAB != null)
			{
				foreach (var entry in cc.EventFarmEndlessInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmEndlessInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmEndlessInfoMap.MergeFrom(cis);
			Patch(updating.EventFarmEndlessInfoMap);
			updating.EventFarmEndlessInfoSortedKeys = genSortedKeys(updating.EventFarmEndlessInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmEndlessInfoConf();
            tmp.MergeFrom(cis);
			updating.EventFarmEndlessInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmEndlessInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmEndlessInfo> filter,string tag=""){
		...
		public static rawEventFarmEndlessInfo MustGet(int key,string tag="") {
		...
		public static rawEventFarmEndlessInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmEndlessInfo> GetByFilter(Func<rawEventFarmEndlessInfo, bool> filterFunc,string tag="") {
			List<rawEventFarmEndlessInfo> ret = new List<rawEventFarmEndlessInfo>();
			foreach(KeyValuePair <int, rawEventFarmEndlessInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmEndlessInfo? GetOneByFilter(Func<rawEventFarmEndlessInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmEndlessInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmEndlessInfo data) 
		{
			return data.Id;			
		}
    }
