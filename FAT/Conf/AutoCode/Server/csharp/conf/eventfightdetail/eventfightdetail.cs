...
using EventFightDetailConf = fat.rawdata.EventFightDetailConf;
using rawEventFightDetail = fat.rawdata.EventFightDetail;
using rawEventFightDetailMapABValue = fat.rawdata.EventFightDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFightDetailVisitor
    {
		...
		public const string FileName = "EventFightDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFightDetailVisitor.SetABDefault单独设定EventFightDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFightDetailVisitor.SetABFallback单独设定EventFightDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFightDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFightDetail>>(()=> new PBC::MapField<int, rawEventFightDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFightDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFightDetailMap;
			var abMap = v.EventFightDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFightDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFightDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFightDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFightDetailMap;
			if (v.EventFightDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFightDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFightDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFightDetailConf cc) {
			foreach(KeyValuePair <string, rawEventFightDetailMapABValue> tagEntry in cc.EventFightDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFightDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFightDetailMapABValue();
					cc.EventFightDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFightDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFightDetail> entry in cc.EventFightDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFightDetail> entry in tagEntry.Value.EventFightDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFightDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFightDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFightDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFightDetailMapAB != null)
			{
				foreach (var entry in cc.EventFightDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFightDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFightDetailMap.MergeFrom(cis);
			Patch(updating.EventFightDetailMap);
			updating.EventFightDetailSortedKeys = genSortedKeys(updating.EventFightDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFightDetailConf();
            tmp.MergeFrom(cis);
			updating.EventFightDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFightDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFightDetail> filter,string tag=""){
		...
		public static rawEventFightDetail MustGet(int key,string tag="") {
		...
		public static rawEventFightDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFightDetail> GetByFilter(Func<rawEventFightDetail, bool> filterFunc,string tag="") {
			List<rawEventFightDetail> ret = new List<rawEventFightDetail>();
			foreach(KeyValuePair <int, rawEventFightDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFightDetail? GetOneByFilter(Func<rawEventFightDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFightDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFightDetail data) 
		{
			return data.Id;			
		}
    }
