...
using EventDiggingRoundConf = fat.rawdata.EventDiggingRoundConf;
using rawEventDiggingRound = fat.rawdata.EventDiggingRound;
using rawEventDiggingRoundMapABValue = fat.rawdata.EventDiggingRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingRoundVisitor
    {
		...
		public const string FileName = "EventDiggingRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingRoundVisitor.SetABDefault单独设定EventDiggingRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingRoundVisitor.SetABFallback单独设定EventDiggingRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDiggingRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDiggingRound>>(()=> new PBC::MapField<int, rawEventDiggingRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDiggingRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingRoundMap;
			var abMap = v.EventDiggingRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDiggingRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingRoundMap;
			if (v.EventDiggingRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingRoundConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingRoundMapABValue> tagEntry in cc.EventDiggingRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingRoundMapABValue();
					cc.EventDiggingRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDiggingRound> entry in cc.EventDiggingRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDiggingRound> entry in tagEntry.Value.EventDiggingRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDiggingRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingRoundMapAB != null)
			{
				foreach (var entry in cc.EventDiggingRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingRoundMap.MergeFrom(cis);
			Patch(updating.EventDiggingRoundMap);
			updating.EventDiggingRoundSortedKeys = genSortedKeys(updating.EventDiggingRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingRoundConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDiggingRound> filter,string tag=""){
		...
		public static rawEventDiggingRound MustGet(int key,string tag="") {
		...
		public static rawEventDiggingRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDiggingRound> GetByFilter(Func<rawEventDiggingRound, bool> filterFunc,string tag="") {
			List<rawEventDiggingRound> ret = new List<rawEventDiggingRound>();
			foreach(KeyValuePair <int, rawEventDiggingRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDiggingRound? GetOneByFilter(Func<rawEventDiggingRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDiggingRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDiggingRound data) 
		{
			return data.Id;			
		}
    }
