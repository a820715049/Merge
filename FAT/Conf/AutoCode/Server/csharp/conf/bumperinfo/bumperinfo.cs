...
using BumperInfoConf = fat.rawdata.BumperInfoConf;
using rawBumperInfo = fat.rawdata.BumperInfo;
using rawBumperInfoMapABValue = fat.rawdata.BumperInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BumperInfoVisitor
    {
		...
		public const string FileName = "BumperInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BumperInfoVisitor.SetABDefault单独设定BumperInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BumperInfoVisitor.SetABFallback单独设定BumperInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBumperInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBumperInfo>>(()=> new PBC::MapField<int, rawBumperInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBumperInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BumperInfoMap;
			var abMap = v.BumperInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BumperInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BumperInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBumperInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BumperInfoMap;
			if (v.BumperInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BumperInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBumperInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BumperInfoConf cc) {
			foreach(KeyValuePair <string, rawBumperInfoMapABValue> tagEntry in cc.BumperInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BumperInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBumperInfoMapABValue();
					cc.BumperInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.BumperInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBumperInfo> entry in cc.BumperInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBumperInfo> entry in tagEntry.Value.BumperInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BumperInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BumperInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BumperInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BumperInfoMapAB != null)
			{
				foreach (var entry in cc.BumperInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BumperInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BumperInfoMap.MergeFrom(cis);
			Patch(updating.BumperInfoMap);
			updating.BumperInfoSortedKeys = genSortedKeys(updating.BumperInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BumperInfoConf();
            tmp.MergeFrom(cis);
			updating.BumperInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BumperInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBumperInfo> filter,string tag=""){
		...
		public static rawBumperInfo MustGet(int key,string tag="") {
		...
		public static rawBumperInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBumperInfo> GetByFilter(Func<rawBumperInfo, bool> filterFunc,string tag="") {
			List<rawBumperInfo> ret = new List<rawBumperInfo>();
			foreach(KeyValuePair <int, rawBumperInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBumperInfo? GetOneByFilter(Func<rawBumperInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBumperInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBumperInfo data) 
		{
			return data.Id;			
		}
    }
