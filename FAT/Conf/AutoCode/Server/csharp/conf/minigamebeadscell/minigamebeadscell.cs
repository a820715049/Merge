...
using MiniGameBeadsCellConf = fat.rawdata.MiniGameBeadsCellConf;
using rawMiniGameBeadsCell = fat.rawdata.MiniGameBeadsCell;
using rawMiniGameBeadsCellMapABValue = fat.rawdata.MiniGameBeadsCellMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameBeadsCellVisitor
    {
		...
		public const string FileName = "MiniGameBeadsCellConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameBeadsCellVisitor.SetABDefault单独设定MiniGameBeadsCell的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameBeadsCellVisitor.SetABFallback单独设定MiniGameBeadsCell的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameBeadsCell>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameBeadsCell>>(()=> new PBC::MapField<int, rawMiniGameBeadsCell>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameBeadsCell>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameBeadsCellMap;
			var abMap = v.MiniGameBeadsCellMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameBeadsCellMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameBeadsCellMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameBeadsCell> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameBeadsCellMap;
			if (v.MiniGameBeadsCellMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameBeadsCellMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameBeadsCellMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameBeadsCellConf cc) {
			foreach(KeyValuePair <string, rawMiniGameBeadsCellMapABValue> tagEntry in cc.MiniGameBeadsCellMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameBeadsCellMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameBeadsCellMapABValue();
					cc.MiniGameBeadsCellMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameBeadsCellMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameBeadsCell> entry in cc.MiniGameBeadsCellMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameBeadsCell> entry in tagEntry.Value.MiniGameBeadsCellMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameBeadsCell> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameBeadsCellConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameBeadsCellMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameBeadsCellMapAB != null)
			{
				foreach (var entry in cc.MiniGameBeadsCellMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameBeadsCellMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameBeadsCellMap.MergeFrom(cis);
			Patch(updating.MiniGameBeadsCellMap);
			updating.MiniGameBeadsCellSortedKeys = genSortedKeys(updating.MiniGameBeadsCellMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameBeadsCellConf();
            tmp.MergeFrom(cis);
			updating.MiniGameBeadsCellMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameBeadsCellSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameBeadsCell> filter,string tag=""){
		...
		public static rawMiniGameBeadsCell MustGet(int key,string tag="") {
		...
		public static rawMiniGameBeadsCell? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameBeadsCell> GetByFilter(Func<rawMiniGameBeadsCell, bool> filterFunc,string tag="") {
			List<rawMiniGameBeadsCell> ret = new List<rawMiniGameBeadsCell>();
			foreach(KeyValuePair <int, rawMiniGameBeadsCell> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameBeadsCell? GetOneByFilter(Func<rawMiniGameBeadsCell, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameBeadsCell> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameBeadsCell data) 
		{
			return data.Id;			
		}
    }
