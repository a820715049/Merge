...
using EventVineLeapRewardConf = fat.rawdata.EventVineLeapRewardConf;
using rawEventVineLeapReward = fat.rawdata.EventVineLeapReward;
using rawEventVineLeapRewardMapABValue = fat.rawdata.EventVineLeapRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventVineLeapRewardVisitor
    {
		...
		public const string FileName = "EventVineLeapRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventVineLeapRewardVisitor.SetABDefault单独设定EventVineLeapReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventVineLeapRewardVisitor.SetABFallback单独设定EventVineLeapReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventVineLeapReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventVineLeapReward>>(()=> new PBC::MapField<int, rawEventVineLeapReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventVineLeapReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventVineLeapRewardMap;
			var abMap = v.EventVineLeapRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventVineLeapRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventVineLeapRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventVineLeapReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventVineLeapRewardMap;
			if (v.EventVineLeapRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventVineLeapRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventVineLeapRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventVineLeapRewardConf cc) {
			foreach(KeyValuePair <string, rawEventVineLeapRewardMapABValue> tagEntry in cc.EventVineLeapRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventVineLeapRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventVineLeapRewardMapABValue();
					cc.EventVineLeapRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventVineLeapRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventVineLeapReward> entry in cc.EventVineLeapRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventVineLeapReward> entry in tagEntry.Value.EventVineLeapRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventVineLeapReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventVineLeapRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventVineLeapRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventVineLeapRewardMapAB != null)
			{
				foreach (var entry in cc.EventVineLeapRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventVineLeapRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventVineLeapRewardMap.MergeFrom(cis);
			Patch(updating.EventVineLeapRewardMap);
			updating.EventVineLeapRewardSortedKeys = genSortedKeys(updating.EventVineLeapRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventVineLeapRewardConf();
            tmp.MergeFrom(cis);
			updating.EventVineLeapRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventVineLeapRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventVineLeapReward> filter,string tag=""){
		...
		public static rawEventVineLeapReward MustGet(int key,string tag="") {
		...
		public static rawEventVineLeapReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventVineLeapReward> GetByFilter(Func<rawEventVineLeapReward, bool> filterFunc,string tag="") {
			List<rawEventVineLeapReward> ret = new List<rawEventVineLeapReward>();
			foreach(KeyValuePair <int, rawEventVineLeapReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventVineLeapReward? GetOneByFilter(Func<rawEventVineLeapReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventVineLeapReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventVineLeapReward data) 
		{
			return data.Id;			
		}
    }
