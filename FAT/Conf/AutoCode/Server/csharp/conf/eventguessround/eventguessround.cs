...
using EventGuessRoundConf = fat.rawdata.EventGuessRoundConf;
using rawEventGuessRound = fat.rawdata.EventGuessRound;
using rawEventGuessRoundMapABValue = fat.rawdata.EventGuessRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventGuessRoundVisitor
    {
		...
		public const string FileName = "EventGuessRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventGuessRoundVisitor.SetABDefault单独设定EventGuessRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventGuessRoundVisitor.SetABFallback单独设定EventGuessRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventGuessRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventGuessRound>>(()=> new PBC::MapField<int, rawEventGuessRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventGuessRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventGuessRoundMap;
			var abMap = v.EventGuessRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventGuessRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventGuessRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventGuessRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventGuessRoundMap;
			if (v.EventGuessRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventGuessRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventGuessRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventGuessRoundConf cc) {
			foreach(KeyValuePair <string, rawEventGuessRoundMapABValue> tagEntry in cc.EventGuessRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventGuessRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventGuessRoundMapABValue();
					cc.EventGuessRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventGuessRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventGuessRound> entry in cc.EventGuessRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventGuessRound> entry in tagEntry.Value.EventGuessRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventGuessRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventGuessRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventGuessRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventGuessRoundMapAB != null)
			{
				foreach (var entry in cc.EventGuessRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventGuessRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventGuessRoundMap.MergeFrom(cis);
			Patch(updating.EventGuessRoundMap);
			updating.EventGuessRoundSortedKeys = genSortedKeys(updating.EventGuessRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventGuessRoundConf();
            tmp.MergeFrom(cis);
			updating.EventGuessRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventGuessRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventGuessRound> filter,string tag=""){
		...
		public static rawEventGuessRound MustGet(int key,string tag="") {
		...
		public static rawEventGuessRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventGuessRound> GetByFilter(Func<rawEventGuessRound, bool> filterFunc,string tag="") {
			List<rawEventGuessRound> ret = new List<rawEventGuessRound>();
			foreach(KeyValuePair <int, rawEventGuessRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventGuessRound? GetOneByFilter(Func<rawEventGuessRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventGuessRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventGuessRound data) 
		{
			return data.Id;			
		}
    }
