...
using EventFightLevelConf = fat.rawdata.EventFightLevelConf;
using rawEventFightLevel = fat.rawdata.EventFightLevel;
using rawEventFightLevelMapABValue = fat.rawdata.EventFightLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFightLevelVisitor
    {
		...
		public const string FileName = "EventFightLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFightLevelVisitor.SetABDefault单独设定EventFightLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFightLevelVisitor.SetABFallback单独设定EventFightLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFightLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFightLevel>>(()=> new PBC::MapField<int, rawEventFightLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFightLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFightLevelMap;
			var abMap = v.EventFightLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFightLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFightLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFightLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFightLevelMap;
			if (v.EventFightLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFightLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFightLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFightLevelConf cc) {
			foreach(KeyValuePair <string, rawEventFightLevelMapABValue> tagEntry in cc.EventFightLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFightLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFightLevelMapABValue();
					cc.EventFightLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFightLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFightLevel> entry in cc.EventFightLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFightLevel> entry in tagEntry.Value.EventFightLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFightLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFightLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFightLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFightLevelMapAB != null)
			{
				foreach (var entry in cc.EventFightLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFightLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFightLevelMap.MergeFrom(cis);
			Patch(updating.EventFightLevelMap);
			updating.EventFightLevelSortedKeys = genSortedKeys(updating.EventFightLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFightLevelConf();
            tmp.MergeFrom(cis);
			updating.EventFightLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFightLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFightLevel> filter,string tag=""){
		...
		public static rawEventFightLevel MustGet(int key,string tag="") {
		...
		public static rawEventFightLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFightLevel> GetByFilter(Func<rawEventFightLevel, bool> filterFunc,string tag="") {
			List<rawEventFightLevel> ret = new List<rawEventFightLevel>();
			foreach(KeyValuePair <int, rawEventFightLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFightLevel? GetOneByFilter(Func<rawEventFightLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFightLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFightLevel data) 
		{
			return data.Id;			
		}
    }
