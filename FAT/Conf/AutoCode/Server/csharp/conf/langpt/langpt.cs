...
using LangPtConf = fat.rawdata.LangPtConf;
using rawLangPt = fat.rawdata.LangPt;
using rawLangPtSliceABValue = fat.rawdata.LangPtSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangPtVisitor
    {
		...
		public const string FileName = "LangPtConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangPtVisitor.SetABDefault单独设定LangPt的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangPtVisitor.SetABFallback单独设定LangPt的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangPt>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangPt>>(()=>new PBC::RepeatedField<rawLangPt>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangPt>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangPtSlice;
			var abMap = v.LangPtSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangPtSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangPtSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangPt> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangPtSlice;
			if (v.LangPtSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangPtSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangPtSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangPtConf cc) {
			foreach(KeyValuePair <string, rawLangPtSliceABValue> tagEntry in cc.LangPtSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangPtSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangPtSliceABValue();
					cc.LangPtSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangPtSlice;
				// 基础数据
				for(int i=0; i<cc.LangPtSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangPtSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangPtSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangPt> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangPtSlice.MergeFrom(cis);
			Patch(updating.LangPtSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangPtConf();
            tmp.MergeFrom(cis);
			updating.LangPtSlice = tmp;
		...
		public static void Range(Action<int, rawLangPt> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangPt MustGetByIndex(int index,string tag="") {
		...
		public static rawLangPt? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangPt> GetByFilter(Func<rawLangPt,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangPt>();
		...
		static public rawLangPt? GetOneByFilter(Func<rawLangPt,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
