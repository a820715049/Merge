...
using EventWishBoardConf = fat.rawdata.EventWishBoardConf;
using rawEventWishBoard = fat.rawdata.EventWishBoard;
using rawEventWishBoardMapABValue = fat.rawdata.EventWishBoardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishBoardVisitor
    {
		...
		public const string FileName = "EventWishBoardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishBoardVisitor.SetABDefault单独设定EventWishBoard的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishBoardVisitor.SetABFallback单独设定EventWishBoard的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishBoard>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishBoard>>(()=> new PBC::MapField<int, rawEventWishBoard>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishBoard>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishBoardMap;
			var abMap = v.EventWishBoardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishBoardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishBoardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishBoard> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishBoardMap;
			if (v.EventWishBoardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishBoardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishBoardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishBoardConf cc) {
			foreach(KeyValuePair <string, rawEventWishBoardMapABValue> tagEntry in cc.EventWishBoardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishBoardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishBoardMapABValue();
					cc.EventWishBoardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishBoardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishBoard> entry in cc.EventWishBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishBoard> entry in tagEntry.Value.EventWishBoardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishBoard> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishBoardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishBoardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishBoardMapAB != null)
			{
				foreach (var entry in cc.EventWishBoardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishBoardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishBoardMap.MergeFrom(cis);
			Patch(updating.EventWishBoardMap);
			updating.EventWishBoardSortedKeys = genSortedKeys(updating.EventWishBoardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishBoardConf();
            tmp.MergeFrom(cis);
			updating.EventWishBoardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishBoardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishBoard> filter,string tag=""){
		...
		public static rawEventWishBoard MustGet(int key,string tag="") {
		...
		public static rawEventWishBoard? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishBoard> GetByFilter(Func<rawEventWishBoard, bool> filterFunc,string tag="") {
			List<rawEventWishBoard> ret = new List<rawEventWishBoard>();
			foreach(KeyValuePair <int, rawEventWishBoard> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishBoard? GetOneByFilter(Func<rawEventWishBoard, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishBoard> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishBoard data) 
		{
			return data.Id;			
		}
    }
