...
using EventTypeInfoConf = fat.rawdata.EventTypeInfoConf;
using rawEventTypeInfo = fat.rawdata.EventTypeInfo;
using rawEventTypeInfoSliceABValue = fat.rawdata.EventTypeInfoSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTypeInfoVisitor
    {
		...
		public const string FileName = "EventTypeInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTypeInfoVisitor.SetABDefault单独设定EventTypeInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTypeInfoVisitor.SetABFallback单独设定EventTypeInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawEventTypeInfo>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawEventTypeInfo>>(()=>new PBC::RepeatedField<rawEventTypeInfo>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawEventTypeInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTypeInfoSlice;
			var abMap = v.EventTypeInfoSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTypeInfoSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTypeInfoSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawEventTypeInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTypeInfoSlice;
			if (v.EventTypeInfoSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTypeInfoSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTypeInfoSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTypeInfoConf cc) {
			foreach(KeyValuePair <string, rawEventTypeInfoSliceABValue> tagEntry in cc.EventTypeInfoSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTypeInfoSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTypeInfoSliceABValue();
					cc.EventTypeInfoSliceAB[tag] = ps;
				}
                var tagTmp = ps.EventTypeInfoSlice;
				// 基础数据
				for(int i=0; i<cc.EventTypeInfoSlice.Count; ++i) 
				{
					tagTmp.Add(cc.EventTypeInfoSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.EventTypeInfoSlice;
				for(int i=0; i<patchData.Count; ++i) 
				{
					var found = false;
					for(int j=0; j<tagTmp.Count; ++j) {
						if((patchData[i].Key != 0) && (tagTmp[j].Key == patchData[i].Key))
						{
							tagTmp[j] = patchData[i];
							found = true;
						}
					}
					if(!found) {
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.EventTypeInfo> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTypeInfoSlice.MergeFrom(cis);
			Patch(updating.EventTypeInfoSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTypeInfoConf();
            tmp.MergeFrom(cis);
			updating.EventTypeInfoSlice = tmp;
		...
		public static void Range(Action<int, rawEventTypeInfo> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawEventTypeInfo MustGetByIndex(int index,string tag="") {
		...
		public static rawEventTypeInfo? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawEventTypeInfo> GetByFilter(Func<rawEventTypeInfo,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawEventTypeInfo>();
		...
		static public rawEventTypeInfo? GetOneByFilter(Func<rawEventTypeInfo,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
