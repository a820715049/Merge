...
using EventRankConf = fat.rawdata.EventRankConf;
using rawEventRank = fat.rawdata.EventRank;
using rawEventRankMapABValue = fat.rawdata.EventRankMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRankVisitor
    {
		...
		public const string FileName = "EventRankConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRankVisitor.SetABDefault单独设定EventRank的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRankVisitor.SetABFallback单独设定EventRank的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRank>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRank>>(()=> new PBC::MapField<int, rawEventRank>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRank>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRankMap;
			var abMap = v.EventRankMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRankMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRankMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRank> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRankMap;
			if (v.EventRankMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRankMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRankMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRankConf cc) {
			foreach(KeyValuePair <string, rawEventRankMapABValue> tagEntry in cc.EventRankMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRankMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRankMapABValue();
					cc.EventRankMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRankMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRank> entry in cc.EventRankMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRank> entry in tagEntry.Value.EventRankMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRank> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRankConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRankMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRankMapAB != null)
			{
				foreach (var entry in cc.EventRankMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRankMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRankMap.MergeFrom(cis);
			Patch(updating.EventRankMap);
			updating.EventRankSortedKeys = genSortedKeys(updating.EventRankMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRankConf();
            tmp.MergeFrom(cis);
			updating.EventRankMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRankSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRank> filter,string tag=""){
		...
		public static rawEventRank MustGet(int key,string tag="") {
		...
		public static rawEventRank? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRank> GetByFilter(Func<rawEventRank, bool> filterFunc,string tag="") {
			List<rawEventRank> ret = new List<rawEventRank>();
			foreach(KeyValuePair <int, rawEventRank> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRank? GetOneByFilter(Func<rawEventRank, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRank> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRank data) 
		{
			return data.Id;			
		}
    }
