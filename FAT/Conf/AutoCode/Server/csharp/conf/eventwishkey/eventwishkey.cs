...
using EventWishKeyConf = fat.rawdata.EventWishKeyConf;
using rawEventWishKey = fat.rawdata.EventWishKey;
using rawEventWishKeyMapABValue = fat.rawdata.EventWishKeyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishKeyVisitor
    {
		...
		public const string FileName = "EventWishKeyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishKeyVisitor.SetABDefault单独设定EventWishKey的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishKeyVisitor.SetABFallback单独设定EventWishKey的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishKey>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishKey>>(()=> new PBC::MapField<int, rawEventWishKey>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishKey>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishKeyMap;
			var abMap = v.EventWishKeyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishKeyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishKeyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishKey> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishKeyMap;
			if (v.EventWishKeyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishKeyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishKeyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishKeyConf cc) {
			foreach(KeyValuePair <string, rawEventWishKeyMapABValue> tagEntry in cc.EventWishKeyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishKeyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishKeyMapABValue();
					cc.EventWishKeyMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishKeyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishKey> entry in cc.EventWishKeyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishKey> entry in tagEntry.Value.EventWishKeyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishKey> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishKeyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishKeyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishKeyMapAB != null)
			{
				foreach (var entry in cc.EventWishKeyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishKeyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishKeyMap.MergeFrom(cis);
			Patch(updating.EventWishKeyMap);
			updating.EventWishKeySortedKeys = genSortedKeys(updating.EventWishKeyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishKeyConf();
            tmp.MergeFrom(cis);
			updating.EventWishKeyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishKeySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishKey> filter,string tag=""){
		...
		public static rawEventWishKey MustGet(int key,string tag="") {
		...
		public static rawEventWishKey? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishKey> GetByFilter(Func<rawEventWishKey, bool> filterFunc,string tag="") {
			List<rawEventWishKey> ret = new List<rawEventWishKey>();
			foreach(KeyValuePair <int, rawEventWishKey> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishKey? GetOneByFilter(Func<rawEventWishKey, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishKey> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishKey data) 
		{
			return data.Id;			
		}
    }
