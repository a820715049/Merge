...
using LoginSignPoolConf = fat.rawdata.LoginSignPoolConf;
using rawLoginSignPool = fat.rawdata.LoginSignPool;
using rawLoginSignPoolMapABValue = fat.rawdata.LoginSignPoolMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LoginSignPoolVisitor
    {
		...
		public const string FileName = "LoginSignPoolConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LoginSignPoolVisitor.SetABDefault单独设定LoginSignPool的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LoginSignPoolVisitor.SetABFallback单独设定LoginSignPool的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLoginSignPool>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLoginSignPool>>(()=> new PBC::MapField<int, rawLoginSignPool>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLoginSignPool>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LoginSignPoolMap;
			var abMap = v.LoginSignPoolMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LoginSignPoolMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LoginSignPoolMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLoginSignPool> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LoginSignPoolMap;
			if (v.LoginSignPoolMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LoginSignPoolMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLoginSignPoolMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LoginSignPoolConf cc) {
			foreach(KeyValuePair <string, rawLoginSignPoolMapABValue> tagEntry in cc.LoginSignPoolMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LoginSignPoolMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLoginSignPoolMapABValue();
					cc.LoginSignPoolMapAB[tag] = ps;
				}
                var tagTmp = ps.LoginSignPoolMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLoginSignPool> entry in cc.LoginSignPoolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLoginSignPool> entry in tagEntry.Value.LoginSignPoolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LoginSignPool> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LoginSignPoolConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LoginSignPoolMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LoginSignPoolMapAB != null)
			{
				foreach (var entry in cc.LoginSignPoolMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LoginSignPoolMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LoginSignPoolMap.MergeFrom(cis);
			Patch(updating.LoginSignPoolMap);
			updating.LoginSignPoolSortedKeys = genSortedKeys(updating.LoginSignPoolMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LoginSignPoolConf();
            tmp.MergeFrom(cis);
			updating.LoginSignPoolMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LoginSignPoolSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLoginSignPool> filter,string tag=""){
		...
		public static rawLoginSignPool MustGet(int key,string tag="") {
		...
		public static rawLoginSignPool? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLoginSignPool> GetByFilter(Func<rawLoginSignPool, bool> filterFunc,string tag="") {
			List<rawLoginSignPool> ret = new List<rawLoginSignPool>();
			foreach(KeyValuePair <int, rawLoginSignPool> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLoginSignPool? GetOneByFilter(Func<rawLoginSignPool, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLoginSignPool> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLoginSignPool data) 
		{
			return data.Id;			
		}
    }
