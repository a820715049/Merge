...
using EventTimeConf = fat.rawdata.EventTimeConf;
using rawEventTime = fat.rawdata.EventTime;
using rawEventTimeMapABValue = fat.rawdata.EventTimeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTimeVisitor
    {
		...
		public const string FileName = "EventTimeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTimeVisitor.SetABDefault单独设定EventTime的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTimeVisitor.SetABFallback单独设定EventTime的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTime>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTime>>(()=> new PBC::MapField<int, rawEventTime>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTime>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTimeMap;
			var abMap = v.EventTimeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTimeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTimeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTime> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTimeMap;
			if (v.EventTimeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTimeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTimeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTimeConf cc) {
			foreach(KeyValuePair <string, rawEventTimeMapABValue> tagEntry in cc.EventTimeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTimeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTimeMapABValue();
					cc.EventTimeMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTimeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTime> entry in cc.EventTimeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTime> entry in tagEntry.Value.EventTimeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTime> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTimeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTimeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTimeMapAB != null)
			{
				foreach (var entry in cc.EventTimeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTimeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTimeMap.MergeFrom(cis);
			Patch(updating.EventTimeMap);
			updating.EventTimeSortedKeys = genSortedKeys(updating.EventTimeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTimeConf();
            tmp.MergeFrom(cis);
			updating.EventTimeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTimeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTime> filter,string tag=""){
		...
		public static rawEventTime MustGet(int key,string tag="") {
		...
		public static rawEventTime? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTime> GetByFilter(Func<rawEventTime, bool> filterFunc,string tag="") {
			List<rawEventTime> ret = new List<rawEventTime>();
			foreach(KeyValuePair <int, rawEventTime> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTime? GetOneByFilter(Func<rawEventTime, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTime> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTime data) 
		{
			return data.Id;			
		}
    }
