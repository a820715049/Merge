...
using EventBpConf = fat.rawdata.EventBpConf;
using rawEventBp = fat.rawdata.EventBp;
using rawEventBpMapABValue = fat.rawdata.EventBpMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventBpVisitor
    {
		...
		public const string FileName = "EventBpConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventBpVisitor.SetABDefault单独设定EventBp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventBpVisitor.SetABFallback单独设定EventBp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventBp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventBp>>(()=> new PBC::MapField<int, rawEventBp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventBp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventBpMap;
			var abMap = v.EventBpMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventBpMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventBpMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventBp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventBpMap;
			if (v.EventBpMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventBpMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventBpMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventBpConf cc) {
			foreach(KeyValuePair <string, rawEventBpMapABValue> tagEntry in cc.EventBpMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventBpMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventBpMapABValue();
					cc.EventBpMapAB[tag] = ps;
				}
                var tagTmp = ps.EventBpMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventBp> entry in cc.EventBpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventBp> entry in tagEntry.Value.EventBpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventBp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventBpConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventBpMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventBpMapAB != null)
			{
				foreach (var entry in cc.EventBpMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventBpMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventBpMap.MergeFrom(cis);
			Patch(updating.EventBpMap);
			updating.EventBpSortedKeys = genSortedKeys(updating.EventBpMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventBpConf();
            tmp.MergeFrom(cis);
			updating.EventBpMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventBpSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventBp> filter,string tag=""){
		...
		public static rawEventBp MustGet(int key,string tag="") {
		...
		public static rawEventBp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventBp> GetByFilter(Func<rawEventBp, bool> filterFunc,string tag="") {
			List<rawEventBp> ret = new List<rawEventBp>();
			foreach(KeyValuePair <int, rawEventBp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventBp? GetOneByFilter(Func<rawEventBp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventBp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventBp data) 
		{
			return data.Id;			
		}
    }
