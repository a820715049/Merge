...
using EventOrderLikeConf = fat.rawdata.EventOrderLikeConf;
using rawEventOrderLike = fat.rawdata.EventOrderLike;
using rawEventOrderLikeMapABValue = fat.rawdata.EventOrderLikeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderLikeVisitor
    {
		...
		public const string FileName = "EventOrderLikeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderLikeVisitor.SetABDefault单独设定EventOrderLike的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderLikeVisitor.SetABFallback单独设定EventOrderLike的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderLike>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderLike>>(()=> new PBC::MapField<int, rawEventOrderLike>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderLike>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderLikeMap;
			var abMap = v.EventOrderLikeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderLikeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderLikeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderLike> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderLikeMap;
			if (v.EventOrderLikeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderLikeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderLikeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderLikeConf cc) {
			foreach(KeyValuePair <string, rawEventOrderLikeMapABValue> tagEntry in cc.EventOrderLikeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderLikeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderLikeMapABValue();
					cc.EventOrderLikeMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderLikeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderLike> entry in cc.EventOrderLikeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderLike> entry in tagEntry.Value.EventOrderLikeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderLike> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderLikeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderLikeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderLikeMapAB != null)
			{
				foreach (var entry in cc.EventOrderLikeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderLikeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderLikeMap.MergeFrom(cis);
			Patch(updating.EventOrderLikeMap);
			updating.EventOrderLikeSortedKeys = genSortedKeys(updating.EventOrderLikeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderLikeConf();
            tmp.MergeFrom(cis);
			updating.EventOrderLikeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderLikeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderLike> filter,string tag=""){
		...
		public static rawEventOrderLike MustGet(int key,string tag="") {
		...
		public static rawEventOrderLike? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderLike> GetByFilter(Func<rawEventOrderLike, bool> filterFunc,string tag="") {
			List<rawEventOrderLike> ret = new List<rawEventOrderLike>();
			foreach(KeyValuePair <int, rawEventOrderLike> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderLike? GetOneByFilter(Func<rawEventOrderLike, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderLike> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderLike data) 
		{
			return data.Id;			
		}
    }
