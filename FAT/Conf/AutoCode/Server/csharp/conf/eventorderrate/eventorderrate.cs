...
using EventOrderRateConf = fat.rawdata.EventOrderRateConf;
using rawEventOrderRate = fat.rawdata.EventOrderRate;
using rawEventOrderRateMapABValue = fat.rawdata.EventOrderRateMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderRateVisitor
    {
		...
		public const string FileName = "EventOrderRateConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderRateVisitor.SetABDefault单独设定EventOrderRate的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderRateVisitor.SetABFallback单独设定EventOrderRate的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderRate>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderRate>>(()=> new PBC::MapField<int, rawEventOrderRate>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderRate>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderRateMap;
			var abMap = v.EventOrderRateMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderRateMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderRateMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderRate> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderRateMap;
			if (v.EventOrderRateMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderRateMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderRateMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderRateConf cc) {
			foreach(KeyValuePair <string, rawEventOrderRateMapABValue> tagEntry in cc.EventOrderRateMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderRateMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderRateMapABValue();
					cc.EventOrderRateMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderRateMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderRate> entry in cc.EventOrderRateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderRate> entry in tagEntry.Value.EventOrderRateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderRate> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderRateConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderRateMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderRateMapAB != null)
			{
				foreach (var entry in cc.EventOrderRateMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderRateMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderRateMap.MergeFrom(cis);
			Patch(updating.EventOrderRateMap);
			updating.EventOrderRateSortedKeys = genSortedKeys(updating.EventOrderRateMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderRateConf();
            tmp.MergeFrom(cis);
			updating.EventOrderRateMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderRateSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderRate> filter,string tag=""){
		...
		public static rawEventOrderRate MustGet(int key,string tag="") {
		...
		public static rawEventOrderRate? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderRate> GetByFilter(Func<rawEventOrderRate, bool> filterFunc,string tag="") {
			List<rawEventOrderRate> ret = new List<rawEventOrderRate>();
			foreach(KeyValuePair <int, rawEventOrderRate> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderRate? GetOneByFilter(Func<rawEventOrderRate, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderRate> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderRate data) 
		{
			return data.Id;			
		}
    }
