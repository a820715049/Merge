...
using EventRaceRewardConf = fat.rawdata.EventRaceRewardConf;
using rawEventRaceReward = fat.rawdata.EventRaceReward;
using rawEventRaceRewardMapABValue = fat.rawdata.EventRaceRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceRewardVisitor
    {
		...
		public const string FileName = "EventRaceRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceRewardVisitor.SetABDefault单独设定EventRaceReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceRewardVisitor.SetABFallback单独设定EventRaceReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceReward>>(()=> new PBC::MapField<int, rawEventRaceReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceRewardMap;
			var abMap = v.EventRaceRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceRewardMap;
			if (v.EventRaceRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceRewardConf cc) {
			foreach(KeyValuePair <string, rawEventRaceRewardMapABValue> tagEntry in cc.EventRaceRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceRewardMapABValue();
					cc.EventRaceRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceReward> entry in cc.EventRaceRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceReward> entry in tagEntry.Value.EventRaceRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceRewardMapAB != null)
			{
				foreach (var entry in cc.EventRaceRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceRewardMap.MergeFrom(cis);
			Patch(updating.EventRaceRewardMap);
			updating.EventRaceRewardSortedKeys = genSortedKeys(updating.EventRaceRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceRewardConf();
            tmp.MergeFrom(cis);
			updating.EventRaceRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceReward> filter,string tag=""){
		...
		public static rawEventRaceReward MustGet(int key,string tag="") {
		...
		public static rawEventRaceReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceReward> GetByFilter(Func<rawEventRaceReward, bool> filterFunc,string tag="") {
			List<rawEventRaceReward> ret = new List<rawEventRaceReward>();
			foreach(KeyValuePair <int, rawEventRaceReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceReward? GetOneByFilter(Func<rawEventRaceReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceReward data) 
		{
			return data.Id;			
		}
    }
