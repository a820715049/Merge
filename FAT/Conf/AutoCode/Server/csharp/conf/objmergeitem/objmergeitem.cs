...
using ObjMergeItemConf = fat.rawdata.ObjMergeItemConf;
using rawObjMergeItem = fat.rawdata.ObjMergeItem;
using rawObjMergeItemMapABValue = fat.rawdata.ObjMergeItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjMergeItemVisitor
    {
		...
		public const string FileName = "ObjMergeItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjMergeItemVisitor.SetABDefault单独设定ObjMergeItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjMergeItemVisitor.SetABFallback单独设定ObjMergeItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawObjMergeItem>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawObjMergeItem>>(()=> new PBC::MapField<string, rawObjMergeItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawObjMergeItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjMergeItemMap;
			var abMap = v.ObjMergeItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjMergeItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjMergeItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawObjMergeItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjMergeItemMap;
			if (v.ObjMergeItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjMergeItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjMergeItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjMergeItemConf cc) {
			foreach(KeyValuePair <string, rawObjMergeItemMapABValue> tagEntry in cc.ObjMergeItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjMergeItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjMergeItemMapABValue();
					cc.ObjMergeItemMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjMergeItemMap;
				// 基础数据
				foreach(KeyValuePair <string, rawObjMergeItem> entry in cc.ObjMergeItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawObjMergeItem> entry in tagEntry.Value.ObjMergeItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.ObjMergeItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(ObjMergeItemConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.ObjMergeItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjMergeItemMapAB != null)
			{
				foreach (var entry in cc.ObjMergeItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.ObjMergeItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjMergeItemMap.MergeFrom(cis);
			Patch(updating.ObjMergeItemMap);
			updating.ObjMergeItemSortedKeys = genSortedKeys(updating.ObjMergeItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjMergeItemConf();
            tmp.MergeFrom(cis);
			updating.ObjMergeItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjMergeItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawObjMergeItem> filter,string tag=""){
		...
		public static rawObjMergeItem MustGet(string key,string tag="") {
		...
		public static rawObjMergeItem? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjMergeItem> GetByFilter(Func<rawObjMergeItem, bool> filterFunc,string tag="") {
			List<rawObjMergeItem> ret = new List<rawObjMergeItem>();
			foreach(KeyValuePair <string, rawObjMergeItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjMergeItem? GetOneByFilter(Func<rawObjMergeItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawObjMergeItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawObjMergeItem data) 
		{
        ...
        public class CK {
            ...
            public static void ObjMergeItemCK() {}
        ...
        public static rawObjMergeItem?  GetByCombinedKeys(CK ck,string tag="") {
