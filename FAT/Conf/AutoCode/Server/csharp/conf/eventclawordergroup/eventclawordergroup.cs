...
using EventClawOrderGroupConf = fat.rawdata.EventClawOrderGroupConf;
using rawEventClawOrderGroup = fat.rawdata.EventClawOrderGroup;
using rawEventClawOrderGroupMapABValue = fat.rawdata.EventClawOrderGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderGroupVisitor
    {
		...
		public const string FileName = "EventClawOrderGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderGroupVisitor.SetABDefault单独设定EventClawOrderGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderGroupVisitor.SetABFallback单独设定EventClawOrderGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrderGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrderGroup>>(()=> new PBC::MapField<int, rawEventClawOrderGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrderGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderGroupMap;
			var abMap = v.EventClawOrderGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrderGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderGroupMap;
			if (v.EventClawOrderGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderGroupConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderGroupMapABValue> tagEntry in cc.EventClawOrderGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderGroupMapABValue();
					cc.EventClawOrderGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrderGroup> entry in cc.EventClawOrderGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrderGroup> entry in tagEntry.Value.EventClawOrderGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrderGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderGroupMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderGroupMap.MergeFrom(cis);
			Patch(updating.EventClawOrderGroupMap);
			updating.EventClawOrderGroupSortedKeys = genSortedKeys(updating.EventClawOrderGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderGroupConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrderGroup> filter,string tag=""){
		...
		public static rawEventClawOrderGroup MustGet(int key,string tag="") {
		...
		public static rawEventClawOrderGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrderGroup> GetByFilter(Func<rawEventClawOrderGroup, bool> filterFunc,string tag="") {
			List<rawEventClawOrderGroup> ret = new List<rawEventClawOrderGroup>();
			foreach(KeyValuePair <int, rawEventClawOrderGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrderGroup? GetOneByFilter(Func<rawEventClawOrderGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrderGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrderGroup data) 
		{
			return data.Id;			
		}
    }
