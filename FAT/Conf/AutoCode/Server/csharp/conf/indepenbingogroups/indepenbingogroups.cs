...
using IndepenBingoGroupsConf = fat.rawdata.IndepenBingoGroupsConf;
using rawIndepenBingoGroups = fat.rawdata.IndepenBingoGroups;
using rawIndepenBingoGroupsMapABValue = fat.rawdata.IndepenBingoGroupsMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IndepenBingoGroupsVisitor
    {
		...
		public const string FileName = "IndepenBingoGroupsConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IndepenBingoGroupsVisitor.SetABDefault单独设定IndepenBingoGroups的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IndepenBingoGroupsVisitor.SetABFallback单独设定IndepenBingoGroups的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIndepenBingoGroups>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIndepenBingoGroups>>(()=> new PBC::MapField<int, rawIndepenBingoGroups>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIndepenBingoGroups>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IndepenBingoGroupsMap;
			var abMap = v.IndepenBingoGroupsMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IndepenBingoGroupsMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IndepenBingoGroupsMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIndepenBingoGroups> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IndepenBingoGroupsMap;
			if (v.IndepenBingoGroupsMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IndepenBingoGroupsMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIndepenBingoGroupsMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IndepenBingoGroupsConf cc) {
			foreach(KeyValuePair <string, rawIndepenBingoGroupsMapABValue> tagEntry in cc.IndepenBingoGroupsMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IndepenBingoGroupsMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIndepenBingoGroupsMapABValue();
					cc.IndepenBingoGroupsMapAB[tag] = ps;
				}
                var tagTmp = ps.IndepenBingoGroupsMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIndepenBingoGroups> entry in cc.IndepenBingoGroupsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIndepenBingoGroups> entry in tagEntry.Value.IndepenBingoGroupsMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IndepenBingoGroups> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IndepenBingoGroupsConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IndepenBingoGroupsMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IndepenBingoGroupsMapAB != null)
			{
				foreach (var entry in cc.IndepenBingoGroupsMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IndepenBingoGroupsMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IndepenBingoGroupsMap.MergeFrom(cis);
			Patch(updating.IndepenBingoGroupsMap);
			updating.IndepenBingoGroupsSortedKeys = genSortedKeys(updating.IndepenBingoGroupsMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IndepenBingoGroupsConf();
            tmp.MergeFrom(cis);
			updating.IndepenBingoGroupsMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IndepenBingoGroupsSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIndepenBingoGroups> filter,string tag=""){
		...
		public static rawIndepenBingoGroups MustGet(int key,string tag="") {
		...
		public static rawIndepenBingoGroups? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIndepenBingoGroups> GetByFilter(Func<rawIndepenBingoGroups, bool> filterFunc,string tag="") {
			List<rawIndepenBingoGroups> ret = new List<rawIndepenBingoGroups>();
			foreach(KeyValuePair <int, rawIndepenBingoGroups> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIndepenBingoGroups? GetOneByFilter(Func<rawIndepenBingoGroups, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIndepenBingoGroups> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIndepenBingoGroups data) 
		{
			return data.Id;			
		}
    }
