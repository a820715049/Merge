...
using ComMergeFeatureConf = fat.rawdata.ComMergeFeatureConf;
using rawComMergeFeature = fat.rawdata.ComMergeFeature;
using rawComMergeFeatureMapABValue = fat.rawdata.ComMergeFeatureMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeFeatureVisitor
    {
		...
		public const string FileName = "ComMergeFeatureConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeFeatureVisitor.SetABDefault单独设定ComMergeFeature的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeFeatureVisitor.SetABFallback单独设定ComMergeFeature的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeFeature>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeFeature>>(()=> new PBC::MapField<int, rawComMergeFeature>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeFeature>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeFeatureMap;
			var abMap = v.ComMergeFeatureMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeFeatureMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeFeatureMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeFeature> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeFeatureMap;
			if (v.ComMergeFeatureMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeFeatureMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeFeatureMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeFeatureConf cc) {
			foreach(KeyValuePair <string, rawComMergeFeatureMapABValue> tagEntry in cc.ComMergeFeatureMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeFeatureMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeFeatureMapABValue();
					cc.ComMergeFeatureMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeFeatureMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeFeature> entry in cc.ComMergeFeatureMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeFeature> entry in tagEntry.Value.ComMergeFeatureMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeFeature> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeFeatureConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeFeatureMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeFeatureMapAB != null)
			{
				foreach (var entry in cc.ComMergeFeatureMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeFeatureMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeFeatureMap.MergeFrom(cis);
			Patch(updating.ComMergeFeatureMap);
			updating.ComMergeFeatureSortedKeys = genSortedKeys(updating.ComMergeFeatureMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeFeatureConf();
            tmp.MergeFrom(cis);
			updating.ComMergeFeatureMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeFeatureSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeFeature> filter,string tag=""){
		...
		public static rawComMergeFeature MustGet(int key,string tag="") {
		...
		public static rawComMergeFeature? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeFeature> GetByFilter(Func<rawComMergeFeature, bool> filterFunc,string tag="") {
			List<rawComMergeFeature> ret = new List<rawComMergeFeature>();
			foreach(KeyValuePair <int, rawComMergeFeature> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeFeature? GetOneByFilter(Func<rawComMergeFeature, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeFeature> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeFeature data) 
		{
			return data.Id;			
		}
    }
