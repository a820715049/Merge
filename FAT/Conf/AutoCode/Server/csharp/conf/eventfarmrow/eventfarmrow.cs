...
using EventFarmRowConf = fat.rawdata.EventFarmRowConf;
using rawEventFarmRow = fat.rawdata.EventFarmRow;
using rawEventFarmRowMapABValue = fat.rawdata.EventFarmRowMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmRowVisitor
    {
		...
		public const string FileName = "EventFarmRowConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmRowVisitor.SetABDefault单独设定EventFarmRow的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmRowVisitor.SetABFallback单独设定EventFarmRow的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmRow>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmRow>>(()=> new PBC::MapField<int, rawEventFarmRow>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmRow>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmRowMap;
			var abMap = v.EventFarmRowMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmRowMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmRowMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmRow> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmRowMap;
			if (v.EventFarmRowMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmRowMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmRowMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmRowConf cc) {
			foreach(KeyValuePair <string, rawEventFarmRowMapABValue> tagEntry in cc.EventFarmRowMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmRowMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmRowMapABValue();
					cc.EventFarmRowMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmRowMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmRow> entry in cc.EventFarmRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmRow> entry in tagEntry.Value.EventFarmRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmRow> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmRowConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmRowMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmRowMapAB != null)
			{
				foreach (var entry in cc.EventFarmRowMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmRowMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmRowMap.MergeFrom(cis);
			Patch(updating.EventFarmRowMap);
			updating.EventFarmRowSortedKeys = genSortedKeys(updating.EventFarmRowMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmRowConf();
            tmp.MergeFrom(cis);
			updating.EventFarmRowMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmRowSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmRow> filter,string tag=""){
		...
		public static rawEventFarmRow MustGet(int key,string tag="") {
		...
		public static rawEventFarmRow? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmRow> GetByFilter(Func<rawEventFarmRow, bool> filterFunc,string tag="") {
			List<rawEventFarmRow> ret = new List<rawEventFarmRow>();
			foreach(KeyValuePair <int, rawEventFarmRow> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmRow? GetOneByFilter(Func<rawEventFarmRow, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmRow> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmRow data) 
		{
			return data.Id;			
		}
    }
