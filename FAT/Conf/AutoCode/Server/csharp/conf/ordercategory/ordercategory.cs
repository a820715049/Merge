...
using OrderCategoryConf = fat.rawdata.OrderCategoryConf;
using rawOrderCategory = fat.rawdata.OrderCategory;
using rawOrderCategoryMapABValue = fat.rawdata.OrderCategoryMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderCategoryVisitor
    {
		...
		public const string FileName = "OrderCategoryConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderCategoryVisitor.SetABDefault单独设定OrderCategory的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderCategoryVisitor.SetABFallback单独设定OrderCategory的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderCategory>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderCategory>>(()=> new PBC::MapField<int, rawOrderCategory>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderCategory>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderCategoryMap;
			var abMap = v.OrderCategoryMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderCategoryMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderCategoryMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderCategory> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderCategoryMap;
			if (v.OrderCategoryMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderCategoryMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderCategoryMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderCategoryConf cc) {
			foreach(KeyValuePair <string, rawOrderCategoryMapABValue> tagEntry in cc.OrderCategoryMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderCategoryMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderCategoryMapABValue();
					cc.OrderCategoryMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderCategoryMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderCategory> entry in cc.OrderCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderCategory> entry in tagEntry.Value.OrderCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderCategory> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderCategoryConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderCategoryMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderCategoryMapAB != null)
			{
				foreach (var entry in cc.OrderCategoryMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderCategoryMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderCategoryMap.MergeFrom(cis);
			Patch(updating.OrderCategoryMap);
			updating.OrderCategorySortedKeys = genSortedKeys(updating.OrderCategoryMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderCategoryConf();
            tmp.MergeFrom(cis);
			updating.OrderCategoryMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderCategorySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderCategory> filter,string tag=""){
		...
		public static rawOrderCategory MustGet(int key,string tag="") {
		...
		public static rawOrderCategory? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderCategory> GetByFilter(Func<rawOrderCategory, bool> filterFunc,string tag="") {
			List<rawOrderCategory> ret = new List<rawOrderCategory>();
			foreach(KeyValuePair <int, rawOrderCategory> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderCategory? GetOneByFilter(Func<rawOrderCategory, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderCategory> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderCategory data) 
		{
			return data.Id;			
		}
    }
