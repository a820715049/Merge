...
using EnergyPackConf = fat.rawdata.EnergyPackConf;
using rawEnergyPack = fat.rawdata.EnergyPack;
using rawEnergyPackMapABValue = fat.rawdata.EnergyPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EnergyPackVisitor
    {
		...
		public const string FileName = "EnergyPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EnergyPackVisitor.SetABDefault单独设定EnergyPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EnergyPackVisitor.SetABFallback单独设定EnergyPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEnergyPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEnergyPack>>(()=> new PBC::MapField<int, rawEnergyPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEnergyPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EnergyPackMap;
			var abMap = v.EnergyPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EnergyPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EnergyPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEnergyPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EnergyPackMap;
			if (v.EnergyPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EnergyPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEnergyPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EnergyPackConf cc) {
			foreach(KeyValuePair <string, rawEnergyPackMapABValue> tagEntry in cc.EnergyPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EnergyPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEnergyPackMapABValue();
					cc.EnergyPackMapAB[tag] = ps;
				}
                var tagTmp = ps.EnergyPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEnergyPack> entry in cc.EnergyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEnergyPack> entry in tagEntry.Value.EnergyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EnergyPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EnergyPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EnergyPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EnergyPackMapAB != null)
			{
				foreach (var entry in cc.EnergyPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EnergyPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EnergyPackMap.MergeFrom(cis);
			Patch(updating.EnergyPackMap);
			updating.EnergyPackSortedKeys = genSortedKeys(updating.EnergyPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EnergyPackConf();
            tmp.MergeFrom(cis);
			updating.EnergyPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EnergyPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEnergyPack> filter,string tag=""){
		...
		public static rawEnergyPack MustGet(int key,string tag="") {
		...
		public static rawEnergyPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEnergyPack> GetByFilter(Func<rawEnergyPack, bool> filterFunc,string tag="") {
			List<rawEnergyPack> ret = new List<rawEnergyPack>();
			foreach(KeyValuePair <int, rawEnergyPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEnergyPack? GetOneByFilter(Func<rawEnergyPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEnergyPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEnergyPack data) 
		{
			return data.Id;			
		}
    }
