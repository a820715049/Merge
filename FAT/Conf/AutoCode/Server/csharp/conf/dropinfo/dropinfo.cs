...
using DropInfoConf = fat.rawdata.DropInfoConf;
using rawDropInfo = fat.rawdata.DropInfo;
using rawDropInfoMapABValue = fat.rawdata.DropInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DropInfoVisitor
    {
		...
		public const string FileName = "DropInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DropInfoVisitor.SetABDefault单独设定DropInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DropInfoVisitor.SetABFallback单独设定DropInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDropInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDropInfo>>(()=> new PBC::MapField<int, rawDropInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDropInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DropInfoMap;
			var abMap = v.DropInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DropInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DropInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDropInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DropInfoMap;
			if (v.DropInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DropInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDropInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DropInfoConf cc) {
			foreach(KeyValuePair <string, rawDropInfoMapABValue> tagEntry in cc.DropInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DropInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDropInfoMapABValue();
					cc.DropInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.DropInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDropInfo> entry in cc.DropInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDropInfo> entry in tagEntry.Value.DropInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DropInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DropInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DropInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DropInfoMapAB != null)
			{
				foreach (var entry in cc.DropInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DropInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DropInfoMap.MergeFrom(cis);
			Patch(updating.DropInfoMap);
			updating.DropInfoSortedKeys = genSortedKeys(updating.DropInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DropInfoConf();
            tmp.MergeFrom(cis);
			updating.DropInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DropInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDropInfo> filter,string tag=""){
		...
		public static rawDropInfo MustGet(int key,string tag="") {
		...
		public static rawDropInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDropInfo> GetByFilter(Func<rawDropInfo, bool> filterFunc,string tag="") {
			List<rawDropInfo> ret = new List<rawDropInfo>();
			foreach(KeyValuePair <int, rawDropInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDropInfo? GetOneByFilter(Func<rawDropInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDropInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDropInfo data) 
		{
			return data.Id;			
		}
    }
