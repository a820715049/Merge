...
using EventSurveyConf = fat.rawdata.EventSurveyConf;
using rawEventSurvey = fat.rawdata.EventSurvey;
using rawEventSurveyMapABValue = fat.rawdata.EventSurveyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSurveyVisitor
    {
		...
		public const string FileName = "EventSurveyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSurveyVisitor.SetABDefault单独设定EventSurvey的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSurveyVisitor.SetABFallback单独设定EventSurvey的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSurvey>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSurvey>>(()=> new PBC::MapField<int, rawEventSurvey>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSurvey>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSurveyMap;
			var abMap = v.EventSurveyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSurveyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSurveyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSurvey> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSurveyMap;
			if (v.EventSurveyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSurveyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSurveyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSurveyConf cc) {
			foreach(KeyValuePair <string, rawEventSurveyMapABValue> tagEntry in cc.EventSurveyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSurveyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSurveyMapABValue();
					cc.EventSurveyMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSurveyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSurvey> entry in cc.EventSurveyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSurvey> entry in tagEntry.Value.EventSurveyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSurvey> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSurveyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSurveyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSurveyMapAB != null)
			{
				foreach (var entry in cc.EventSurveyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSurveyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSurveyMap.MergeFrom(cis);
			Patch(updating.EventSurveyMap);
			updating.EventSurveySortedKeys = genSortedKeys(updating.EventSurveyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSurveyConf();
            tmp.MergeFrom(cis);
			updating.EventSurveyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSurveySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSurvey> filter,string tag=""){
		...
		public static rawEventSurvey MustGet(int key,string tag="") {
		...
		public static rawEventSurvey? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSurvey> GetByFilter(Func<rawEventSurvey, bool> filterFunc,string tag="") {
			List<rawEventSurvey> ret = new List<rawEventSurvey>();
			foreach(KeyValuePair <int, rawEventSurvey> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSurvey? GetOneByFilter(Func<rawEventSurvey, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSurvey> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSurvey data) 
		{
			return data.Id;			
		}
    }
