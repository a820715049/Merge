...
using LangTrConf = fat.rawdata.LangTrConf;
using rawLangTr = fat.rawdata.LangTr;
using rawLangTrSliceABValue = fat.rawdata.LangTrSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangTrVisitor
    {
		...
		public const string FileName = "LangTrConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangTrVisitor.SetABDefault单独设定LangTr的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangTrVisitor.SetABFallback单独设定LangTr的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangTr>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangTr>>(()=>new PBC::RepeatedField<rawLangTr>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangTr>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangTrSlice;
			var abMap = v.LangTrSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangTrSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangTrSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangTr> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangTrSlice;
			if (v.LangTrSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangTrSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangTrSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangTrConf cc) {
			foreach(KeyValuePair <string, rawLangTrSliceABValue> tagEntry in cc.LangTrSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangTrSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangTrSliceABValue();
					cc.LangTrSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangTrSlice;
				// 基础数据
				for(int i=0; i<cc.LangTrSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangTrSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangTrSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangTr> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangTrSlice.MergeFrom(cis);
			Patch(updating.LangTrSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangTrConf();
            tmp.MergeFrom(cis);
			updating.LangTrSlice = tmp;
		...
		public static void Range(Action<int, rawLangTr> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangTr MustGetByIndex(int index,string tag="") {
		...
		public static rawLangTr? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangTr> GetByFilter(Func<rawLangTr,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangTr>();
		...
		static public rawLangTr? GetOneByFilter(Func<rawLangTr,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
