...
using EventMineOnePlusOneConf = fat.rawdata.EventMineOnePlusOneConf;
using rawEventMineOnePlusOne = fat.rawdata.EventMineOnePlusOne;
using rawEventMineOnePlusOneMapABValue = fat.rawdata.EventMineOnePlusOneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineOnePlusOneVisitor
    {
		...
		public const string FileName = "EventMineOnePlusOneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineOnePlusOneVisitor.SetABDefault单独设定EventMineOnePlusOne的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineOnePlusOneVisitor.SetABFallback单独设定EventMineOnePlusOne的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineOnePlusOne>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineOnePlusOne>>(()=> new PBC::MapField<int, rawEventMineOnePlusOne>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineOnePlusOne>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineOnePlusOneMap;
			var abMap = v.EventMineOnePlusOneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineOnePlusOneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineOnePlusOneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineOnePlusOne> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineOnePlusOneMap;
			if (v.EventMineOnePlusOneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineOnePlusOneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineOnePlusOneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineOnePlusOneConf cc) {
			foreach(KeyValuePair <string, rawEventMineOnePlusOneMapABValue> tagEntry in cc.EventMineOnePlusOneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineOnePlusOneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineOnePlusOneMapABValue();
					cc.EventMineOnePlusOneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineOnePlusOneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineOnePlusOne> entry in cc.EventMineOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineOnePlusOne> entry in tagEntry.Value.EventMineOnePlusOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineOnePlusOne> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineOnePlusOneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineOnePlusOneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineOnePlusOneMapAB != null)
			{
				foreach (var entry in cc.EventMineOnePlusOneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineOnePlusOneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineOnePlusOneMap.MergeFrom(cis);
			Patch(updating.EventMineOnePlusOneMap);
			updating.EventMineOnePlusOneSortedKeys = genSortedKeys(updating.EventMineOnePlusOneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineOnePlusOneConf();
            tmp.MergeFrom(cis);
			updating.EventMineOnePlusOneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineOnePlusOneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineOnePlusOne> filter,string tag=""){
		...
		public static rawEventMineOnePlusOne MustGet(int key,string tag="") {
		...
		public static rawEventMineOnePlusOne? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineOnePlusOne> GetByFilter(Func<rawEventMineOnePlusOne, bool> filterFunc,string tag="") {
			List<rawEventMineOnePlusOne> ret = new List<rawEventMineOnePlusOne>();
			foreach(KeyValuePair <int, rawEventMineOnePlusOne> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineOnePlusOne? GetOneByFilter(Func<rawEventMineOnePlusOne, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineOnePlusOne> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineOnePlusOne data) 
		{
			return data.Id;			
		}
    }
