...
using BpPackInfoConf = fat.rawdata.BpPackInfoConf;
using rawBpPackInfo = fat.rawdata.BpPackInfo;
using rawBpPackInfoMapABValue = fat.rawdata.BpPackInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BpPackInfoVisitor
    {
		...
		public const string FileName = "BpPackInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BpPackInfoVisitor.SetABDefault单独设定BpPackInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BpPackInfoVisitor.SetABFallback单独设定BpPackInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBpPackInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBpPackInfo>>(()=> new PBC::MapField<int, rawBpPackInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBpPackInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BpPackInfoMap;
			var abMap = v.BpPackInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BpPackInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BpPackInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBpPackInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BpPackInfoMap;
			if (v.BpPackInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BpPackInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBpPackInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BpPackInfoConf cc) {
			foreach(KeyValuePair <string, rawBpPackInfoMapABValue> tagEntry in cc.BpPackInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BpPackInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBpPackInfoMapABValue();
					cc.BpPackInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.BpPackInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBpPackInfo> entry in cc.BpPackInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBpPackInfo> entry in tagEntry.Value.BpPackInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BpPackInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BpPackInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BpPackInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BpPackInfoMapAB != null)
			{
				foreach (var entry in cc.BpPackInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BpPackInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BpPackInfoMap.MergeFrom(cis);
			Patch(updating.BpPackInfoMap);
			updating.BpPackInfoSortedKeys = genSortedKeys(updating.BpPackInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BpPackInfoConf();
            tmp.MergeFrom(cis);
			updating.BpPackInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BpPackInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBpPackInfo> filter,string tag=""){
		...
		public static rawBpPackInfo MustGet(int key,string tag="") {
		...
		public static rawBpPackInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBpPackInfo> GetByFilter(Func<rawBpPackInfo, bool> filterFunc,string tag="") {
			List<rawBpPackInfo> ret = new List<rawBpPackInfo>();
			foreach(KeyValuePair <int, rawBpPackInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBpPackInfo? GetOneByFilter(Func<rawBpPackInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBpPackInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBpPackInfo data) 
		{
			return data.Id;			
		}
    }
