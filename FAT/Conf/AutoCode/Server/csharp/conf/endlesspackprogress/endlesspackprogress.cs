...
using EndlessPackProgressConf = fat.rawdata.EndlessPackProgressConf;
using rawEndlessPackProgress = fat.rawdata.EndlessPackProgress;
using rawEndlessPackProgressMapABValue = fat.rawdata.EndlessPackProgressMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessPackProgressVisitor
    {
		...
		public const string FileName = "EndlessPackProgressConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessPackProgressVisitor.SetABDefault单独设定EndlessPackProgress的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessPackProgressVisitor.SetABFallback单独设定EndlessPackProgress的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessPackProgress>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessPackProgress>>(()=> new PBC::MapField<int, rawEndlessPackProgress>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessPackProgress>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessPackProgressMap;
			var abMap = v.EndlessPackProgressMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessPackProgressMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessPackProgressMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessPackProgress> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessPackProgressMap;
			if (v.EndlessPackProgressMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessPackProgressMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessPackProgressMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessPackProgressConf cc) {
			foreach(KeyValuePair <string, rawEndlessPackProgressMapABValue> tagEntry in cc.EndlessPackProgressMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessPackProgressMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessPackProgressMapABValue();
					cc.EndlessPackProgressMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessPackProgressMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessPackProgress> entry in cc.EndlessPackProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessPackProgress> entry in tagEntry.Value.EndlessPackProgressMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessPackProgress> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessPackProgressConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessPackProgressMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessPackProgressMapAB != null)
			{
				foreach (var entry in cc.EndlessPackProgressMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessPackProgressMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessPackProgressMap.MergeFrom(cis);
			Patch(updating.EndlessPackProgressMap);
			updating.EndlessPackProgressSortedKeys = genSortedKeys(updating.EndlessPackProgressMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessPackProgressConf();
            tmp.MergeFrom(cis);
			updating.EndlessPackProgressMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessPackProgressSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessPackProgress> filter,string tag=""){
		...
		public static rawEndlessPackProgress MustGet(int key,string tag="") {
		...
		public static rawEndlessPackProgress? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessPackProgress> GetByFilter(Func<rawEndlessPackProgress, bool> filterFunc,string tag="") {
			List<rawEndlessPackProgress> ret = new List<rawEndlessPackProgress>();
			foreach(KeyValuePair <int, rawEndlessPackProgress> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessPackProgress? GetOneByFilter(Func<rawEndlessPackProgress, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessPackProgress> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessPackProgress data) 
		{
			return data.Id;			
		}
    }
