...
using RaceExtendRobotConf = fat.rawdata.RaceExtendRobotConf;
using rawRaceExtendRobot = fat.rawdata.RaceExtendRobot;
using rawRaceExtendRobotMapABValue = fat.rawdata.RaceExtendRobotMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RaceExtendRobotVisitor
    {
		...
		public const string FileName = "RaceExtendRobotConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RaceExtendRobotVisitor.SetABDefault单独设定RaceExtendRobot的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RaceExtendRobotVisitor.SetABFallback单独设定RaceExtendRobot的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRaceExtendRobot>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRaceExtendRobot>>(()=> new PBC::MapField<int, rawRaceExtendRobot>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRaceExtendRobot>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RaceExtendRobotMap;
			var abMap = v.RaceExtendRobotMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RaceExtendRobotMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RaceExtendRobotMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRaceExtendRobot> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RaceExtendRobotMap;
			if (v.RaceExtendRobotMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RaceExtendRobotMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRaceExtendRobotMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RaceExtendRobotConf cc) {
			foreach(KeyValuePair <string, rawRaceExtendRobotMapABValue> tagEntry in cc.RaceExtendRobotMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RaceExtendRobotMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRaceExtendRobotMapABValue();
					cc.RaceExtendRobotMapAB[tag] = ps;
				}
                var tagTmp = ps.RaceExtendRobotMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRaceExtendRobot> entry in cc.RaceExtendRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRaceExtendRobot> entry in tagEntry.Value.RaceExtendRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RaceExtendRobot> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RaceExtendRobotConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RaceExtendRobotMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RaceExtendRobotMapAB != null)
			{
				foreach (var entry in cc.RaceExtendRobotMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RaceExtendRobotMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RaceExtendRobotMap.MergeFrom(cis);
			Patch(updating.RaceExtendRobotMap);
			updating.RaceExtendRobotSortedKeys = genSortedKeys(updating.RaceExtendRobotMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RaceExtendRobotConf();
            tmp.MergeFrom(cis);
			updating.RaceExtendRobotMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RaceExtendRobotSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRaceExtendRobot> filter,string tag=""){
		...
		public static rawRaceExtendRobot MustGet(int key,string tag="") {
		...
		public static rawRaceExtendRobot? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRaceExtendRobot> GetByFilter(Func<rawRaceExtendRobot, bool> filterFunc,string tag="") {
			List<rawRaceExtendRobot> ret = new List<rawRaceExtendRobot>();
			foreach(KeyValuePair <int, rawRaceExtendRobot> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRaceExtendRobot? GetOneByFilter(Func<rawRaceExtendRobot, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRaceExtendRobot> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRaceExtendRobot data) 
		{
			return data.Id;			
		}
    }
