...
using ComMergeEatSourceConf = fat.rawdata.ComMergeEatSourceConf;
using rawComMergeEatSource = fat.rawdata.ComMergeEatSource;
using rawComMergeEatSourceMapABValue = fat.rawdata.ComMergeEatSourceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeEatSourceVisitor
    {
		...
		public const string FileName = "ComMergeEatSourceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeEatSourceVisitor.SetABDefault单独设定ComMergeEatSource的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeEatSourceVisitor.SetABFallback单独设定ComMergeEatSource的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeEatSource>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeEatSource>>(()=> new PBC::MapField<int, rawComMergeEatSource>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeEatSource>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeEatSourceMap;
			var abMap = v.ComMergeEatSourceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeEatSourceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeEatSourceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeEatSource> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeEatSourceMap;
			if (v.ComMergeEatSourceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeEatSourceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeEatSourceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeEatSourceConf cc) {
			foreach(KeyValuePair <string, rawComMergeEatSourceMapABValue> tagEntry in cc.ComMergeEatSourceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeEatSourceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeEatSourceMapABValue();
					cc.ComMergeEatSourceMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeEatSourceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeEatSource> entry in cc.ComMergeEatSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeEatSource> entry in tagEntry.Value.ComMergeEatSourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeEatSource> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeEatSourceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeEatSourceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeEatSourceMapAB != null)
			{
				foreach (var entry in cc.ComMergeEatSourceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeEatSourceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeEatSourceMap.MergeFrom(cis);
			Patch(updating.ComMergeEatSourceMap);
			updating.ComMergeEatSourceSortedKeys = genSortedKeys(updating.ComMergeEatSourceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeEatSourceConf();
            tmp.MergeFrom(cis);
			updating.ComMergeEatSourceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeEatSourceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeEatSource> filter,string tag=""){
		...
		public static rawComMergeEatSource MustGet(int key,string tag="") {
		...
		public static rawComMergeEatSource? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeEatSource> GetByFilter(Func<rawComMergeEatSource, bool> filterFunc,string tag="") {
			List<rawComMergeEatSource> ret = new List<rawComMergeEatSource>();
			foreach(KeyValuePair <int, rawComMergeEatSource> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeEatSource? GetOneByFilter(Func<rawComMergeEatSource, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeEatSource> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeEatSource data) 
		{
			return data.Id;			
		}
    }
