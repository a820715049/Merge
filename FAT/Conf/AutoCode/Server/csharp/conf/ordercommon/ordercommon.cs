...
using OrderCommonConf = fat.rawdata.OrderCommonConf;
using rawOrderCommon = fat.rawdata.OrderCommon;
using rawOrderCommonMapABValue = fat.rawdata.OrderCommonMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderCommonVisitor
    {
		...
		public const string FileName = "OrderCommonConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderCommonVisitor.SetABDefault单独设定OrderCommon的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderCommonVisitor.SetABFallback单独设定OrderCommon的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderCommon>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderCommon>>(()=> new PBC::MapField<int, rawOrderCommon>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderCommon>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderCommonMap;
			var abMap = v.OrderCommonMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderCommonMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderCommonMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderCommon> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderCommonMap;
			if (v.OrderCommonMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderCommonMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderCommonMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderCommonConf cc) {
			foreach(KeyValuePair <string, rawOrderCommonMapABValue> tagEntry in cc.OrderCommonMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderCommonMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderCommonMapABValue();
					cc.OrderCommonMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderCommonMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderCommon> entry in cc.OrderCommonMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderCommon> entry in tagEntry.Value.OrderCommonMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderCommon> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderCommonConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderCommonMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderCommonMapAB != null)
			{
				foreach (var entry in cc.OrderCommonMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderCommonMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderCommonMap.MergeFrom(cis);
			Patch(updating.OrderCommonMap);
			updating.OrderCommonSortedKeys = genSortedKeys(updating.OrderCommonMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderCommonConf();
            tmp.MergeFrom(cis);
			updating.OrderCommonMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderCommonSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderCommon> filter,string tag=""){
		...
		public static rawOrderCommon MustGet(int key,string tag="") {
		...
		public static rawOrderCommon? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderCommon> GetByFilter(Func<rawOrderCommon, bool> filterFunc,string tag="") {
			List<rawOrderCommon> ret = new List<rawOrderCommon>();
			foreach(KeyValuePair <int, rawOrderCommon> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderCommon? GetOneByFilter(Func<rawOrderCommon, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderCommon> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderCommon data) 
		{
			return data.Id;			
		}
    }
