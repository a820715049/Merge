...
using EventMarketIAPGiftConf = fat.rawdata.EventMarketIAPGiftConf;
using rawEventMarketIAPGift = fat.rawdata.EventMarketIAPGift;
using rawEventMarketIAPGiftMapABValue = fat.rawdata.EventMarketIAPGiftMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMarketIAPGiftVisitor
    {
		...
		public const string FileName = "EventMarketIAPGiftConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMarketIAPGiftVisitor.SetABDefault单独设定EventMarketIAPGift的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMarketIAPGiftVisitor.SetABFallback单独设定EventMarketIAPGift的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMarketIAPGift>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMarketIAPGift>>(()=> new PBC::MapField<int, rawEventMarketIAPGift>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMarketIAPGift>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMarketIAPGiftMap;
			var abMap = v.EventMarketIAPGiftMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMarketIAPGiftMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMarketIAPGiftMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMarketIAPGift> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMarketIAPGiftMap;
			if (v.EventMarketIAPGiftMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMarketIAPGiftMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMarketIAPGiftMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMarketIAPGiftConf cc) {
			foreach(KeyValuePair <string, rawEventMarketIAPGiftMapABValue> tagEntry in cc.EventMarketIAPGiftMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMarketIAPGiftMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMarketIAPGiftMapABValue();
					cc.EventMarketIAPGiftMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMarketIAPGiftMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMarketIAPGift> entry in cc.EventMarketIAPGiftMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMarketIAPGift> entry in tagEntry.Value.EventMarketIAPGiftMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMarketIAPGift> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMarketIAPGiftConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMarketIAPGiftMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMarketIAPGiftMapAB != null)
			{
				foreach (var entry in cc.EventMarketIAPGiftMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMarketIAPGiftMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMarketIAPGiftMap.MergeFrom(cis);
			Patch(updating.EventMarketIAPGiftMap);
			updating.EventMarketIAPGiftSortedKeys = genSortedKeys(updating.EventMarketIAPGiftMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMarketIAPGiftConf();
            tmp.MergeFrom(cis);
			updating.EventMarketIAPGiftMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMarketIAPGiftSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMarketIAPGift> filter,string tag=""){
		...
		public static rawEventMarketIAPGift MustGet(int key,string tag="") {
		...
		public static rawEventMarketIAPGift? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMarketIAPGift> GetByFilter(Func<rawEventMarketIAPGift, bool> filterFunc,string tag="") {
			List<rawEventMarketIAPGift> ret = new List<rawEventMarketIAPGift>();
			foreach(KeyValuePair <int, rawEventMarketIAPGift> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMarketIAPGift? GetOneByFilter(Func<rawEventMarketIAPGift, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMarketIAPGift> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMarketIAPGift data) 
		{
			return data.Id;			
		}
    }
