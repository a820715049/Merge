...
using FishInfoConf = fat.rawdata.FishInfoConf;
using rawFishInfo = fat.rawdata.FishInfo;
using rawFishInfoMapABValue = fat.rawdata.FishInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class FishInfoVisitor
    {
		...
		public const string FileName = "FishInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过FishInfoVisitor.SetABDefault单独设定FishInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过FishInfoVisitor.SetABFallback单独设定FishInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawFishInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawFishInfo>>(()=> new PBC::MapField<int, rawFishInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawFishInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().FishInfoMap;
			var abMap = v.FishInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.FishInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].FishInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawFishInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().FishInfoMap;
			if (v.FishInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.FishInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawFishInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(FishInfoConf cc) {
			foreach(KeyValuePair <string, rawFishInfoMapABValue> tagEntry in cc.FishInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.FishInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawFishInfoMapABValue();
					cc.FishInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.FishInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawFishInfo> entry in cc.FishInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawFishInfo> entry in tagEntry.Value.FishInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.FishInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(FishInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.FishInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.FishInfoMapAB != null)
			{
				foreach (var entry in cc.FishInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.FishInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.FishInfoMap.MergeFrom(cis);
			Patch(updating.FishInfoMap);
			updating.FishInfoSortedKeys = genSortedKeys(updating.FishInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new FishInfoConf();
            tmp.MergeFrom(cis);
			updating.FishInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().FishInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawFishInfo> filter,string tag=""){
		...
		public static rawFishInfo MustGet(int key,string tag="") {
		...
		public static rawFishInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawFishInfo> GetByFilter(Func<rawFishInfo, bool> filterFunc,string tag="") {
			List<rawFishInfo> ret = new List<rawFishInfo>();
			foreach(KeyValuePair <int, rawFishInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawFishInfo? GetOneByFilter(Func<rawFishInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawFishInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawFishInfo data) 
		{
			return data.Id;			
		}
    }
