...
using PlotStoryConf = fat.rawdata.PlotStoryConf;
using rawPlotStory = fat.rawdata.PlotStory;
using rawPlotStoryMapABValue = fat.rawdata.PlotStoryMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PlotStoryVisitor
    {
		...
		public const string FileName = "PlotStoryConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PlotStoryVisitor.SetABDefault单独设定PlotStory的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PlotStoryVisitor.SetABFallback单独设定PlotStory的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPlotStory>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPlotStory>>(()=> new PBC::MapField<int, rawPlotStory>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPlotStory>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PlotStoryMap;
			var abMap = v.PlotStoryMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PlotStoryMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PlotStoryMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPlotStory> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PlotStoryMap;
			if (v.PlotStoryMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PlotStoryMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPlotStoryMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PlotStoryConf cc) {
			foreach(KeyValuePair <string, rawPlotStoryMapABValue> tagEntry in cc.PlotStoryMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PlotStoryMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPlotStoryMapABValue();
					cc.PlotStoryMapAB[tag] = ps;
				}
                var tagTmp = ps.PlotStoryMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPlotStory> entry in cc.PlotStoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPlotStory> entry in tagEntry.Value.PlotStoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.PlotStory> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PlotStoryConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PlotStoryMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PlotStoryMapAB != null)
			{
				foreach (var entry in cc.PlotStoryMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PlotStoryMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PlotStoryMap.MergeFrom(cis);
			Patch(updating.PlotStoryMap);
			updating.PlotStorySortedKeys = genSortedKeys(updating.PlotStoryMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PlotStoryConf();
            tmp.MergeFrom(cis);
			updating.PlotStoryMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PlotStorySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPlotStory> filter,string tag=""){
		...
		public static rawPlotStory MustGet(int key,string tag="") {
		...
		public static rawPlotStory? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPlotStory> GetByFilter(Func<rawPlotStory, bool> filterFunc,string tag="") {
			List<rawPlotStory> ret = new List<rawPlotStory>();
			foreach(KeyValuePair <int, rawPlotStory> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPlotStory? GetOneByFilter(Func<rawPlotStory, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPlotStory> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPlotStory data) 
		{
			return data.Id;			
		}
    }
