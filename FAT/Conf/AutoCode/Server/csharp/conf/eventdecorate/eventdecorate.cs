...
using EventDecorateConf = fat.rawdata.EventDecorateConf;
using rawEventDecorate = fat.rawdata.EventDecorate;
using rawEventDecorateMapABValue = fat.rawdata.EventDecorateMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDecorateVisitor
    {
		...
		public const string FileName = "EventDecorateConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDecorateVisitor.SetABDefault单独设定EventDecorate的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDecorateVisitor.SetABFallback单独设定EventDecorate的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDecorate>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDecorate>>(()=> new PBC::MapField<int, rawEventDecorate>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDecorate>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDecorateMap;
			var abMap = v.EventDecorateMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDecorateMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDecorateMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDecorate> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDecorateMap;
			if (v.EventDecorateMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDecorateMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDecorateMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDecorateConf cc) {
			foreach(KeyValuePair <string, rawEventDecorateMapABValue> tagEntry in cc.EventDecorateMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDecorateMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDecorateMapABValue();
					cc.EventDecorateMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDecorateMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDecorate> entry in cc.EventDecorateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDecorate> entry in tagEntry.Value.EventDecorateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDecorate> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDecorateConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDecorateMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDecorateMapAB != null)
			{
				foreach (var entry in cc.EventDecorateMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDecorateMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDecorateMap.MergeFrom(cis);
			Patch(updating.EventDecorateMap);
			updating.EventDecorateSortedKeys = genSortedKeys(updating.EventDecorateMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDecorateConf();
            tmp.MergeFrom(cis);
			updating.EventDecorateMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDecorateSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDecorate> filter,string tag=""){
		...
		public static rawEventDecorate MustGet(int key,string tag="") {
		...
		public static rawEventDecorate? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDecorate> GetByFilter(Func<rawEventDecorate, bool> filterFunc,string tag="") {
			List<rawEventDecorate> ret = new List<rawEventDecorate>();
			foreach(KeyValuePair <int, rawEventDecorate> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDecorate? GetOneByFilter(Func<rawEventDecorate, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDecorate> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDecorate data) 
		{
			return data.Id;			
		}
    }
