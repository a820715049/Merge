...
using MergeBoardGrpConf = fat.rawdata.MergeBoardGrpConf;
using rawMergeBoardGrp = fat.rawdata.MergeBoardGrp;
using rawMergeBoardGrpMapABValue = fat.rawdata.MergeBoardGrpMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeBoardGrpVisitor
    {
		...
		public const string FileName = "MergeBoardGrpConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeBoardGrpVisitor.SetABDefault单独设定MergeBoardGrp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeBoardGrpVisitor.SetABFallback单独设定MergeBoardGrp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeBoardGrp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeBoardGrp>>(()=> new PBC::MapField<int, rawMergeBoardGrp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeBoardGrp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeBoardGrpMap;
			var abMap = v.MergeBoardGrpMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeBoardGrpMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeBoardGrpMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeBoardGrp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeBoardGrpMap;
			if (v.MergeBoardGrpMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeBoardGrpMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeBoardGrpMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeBoardGrpConf cc) {
			foreach(KeyValuePair <string, rawMergeBoardGrpMapABValue> tagEntry in cc.MergeBoardGrpMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeBoardGrpMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeBoardGrpMapABValue();
					cc.MergeBoardGrpMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeBoardGrpMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeBoardGrp> entry in cc.MergeBoardGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeBoardGrp> entry in tagEntry.Value.MergeBoardGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeBoardGrp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeBoardGrpConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeBoardGrpMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeBoardGrpMapAB != null)
			{
				foreach (var entry in cc.MergeBoardGrpMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeBoardGrpMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeBoardGrpMap.MergeFrom(cis);
			Patch(updating.MergeBoardGrpMap);
			updating.MergeBoardGrpSortedKeys = genSortedKeys(updating.MergeBoardGrpMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeBoardGrpConf();
            tmp.MergeFrom(cis);
			updating.MergeBoardGrpMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeBoardGrpSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeBoardGrp> filter,string tag=""){
		...
		public static rawMergeBoardGrp MustGet(int key,string tag="") {
		...
		public static rawMergeBoardGrp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeBoardGrp> GetByFilter(Func<rawMergeBoardGrp, bool> filterFunc,string tag="") {
			List<rawMergeBoardGrp> ret = new List<rawMergeBoardGrp>();
			foreach(KeyValuePair <int, rawMergeBoardGrp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeBoardGrp? GetOneByFilter(Func<rawMergeBoardGrp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeBoardGrp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeBoardGrp data) 
		{
			return data.Id;			
		}
    }
