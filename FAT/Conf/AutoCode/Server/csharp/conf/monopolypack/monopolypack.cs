...
using MonopolyPackConf = fat.rawdata.MonopolyPackConf;
using rawMonopolyPack = fat.rawdata.MonopolyPack;
using rawMonopolyPackMapABValue = fat.rawdata.MonopolyPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MonopolyPackVisitor
    {
		...
		public const string FileName = "MonopolyPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MonopolyPackVisitor.SetABDefault单独设定MonopolyPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MonopolyPackVisitor.SetABFallback单独设定MonopolyPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMonopolyPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMonopolyPack>>(()=> new PBC::MapField<int, rawMonopolyPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMonopolyPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MonopolyPackMap;
			var abMap = v.MonopolyPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MonopolyPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MonopolyPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMonopolyPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MonopolyPackMap;
			if (v.MonopolyPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MonopolyPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMonopolyPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MonopolyPackConf cc) {
			foreach(KeyValuePair <string, rawMonopolyPackMapABValue> tagEntry in cc.MonopolyPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MonopolyPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMonopolyPackMapABValue();
					cc.MonopolyPackMapAB[tag] = ps;
				}
                var tagTmp = ps.MonopolyPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMonopolyPack> entry in cc.MonopolyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMonopolyPack> entry in tagEntry.Value.MonopolyPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MonopolyPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MonopolyPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MonopolyPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MonopolyPackMapAB != null)
			{
				foreach (var entry in cc.MonopolyPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MonopolyPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MonopolyPackMap.MergeFrom(cis);
			Patch(updating.MonopolyPackMap);
			updating.MonopolyPackSortedKeys = genSortedKeys(updating.MonopolyPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MonopolyPackConf();
            tmp.MergeFrom(cis);
			updating.MonopolyPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MonopolyPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMonopolyPack> filter,string tag=""){
		...
		public static rawMonopolyPack MustGet(int key,string tag="") {
		...
		public static rawMonopolyPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMonopolyPack> GetByFilter(Func<rawMonopolyPack, bool> filterFunc,string tag="") {
			List<rawMonopolyPack> ret = new List<rawMonopolyPack>();
			foreach(KeyValuePair <int, rawMonopolyPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMonopolyPack? GetOneByFilter(Func<rawMonopolyPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMonopolyPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMonopolyPack data) 
		{
			return data.Id;			
		}
    }
