...
using EventFarmBoardDetailConf = fat.rawdata.EventFarmBoardDetailConf;
using rawEventFarmBoardDetail = fat.rawdata.EventFarmBoardDetail;
using rawEventFarmBoardDetailMapABValue = fat.rawdata.EventFarmBoardDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFarmBoardDetailVisitor
    {
		...
		public const string FileName = "EventFarmBoardDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFarmBoardDetailVisitor.SetABDefault单独设定EventFarmBoardDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFarmBoardDetailVisitor.SetABFallback单独设定EventFarmBoardDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFarmBoardDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFarmBoardDetail>>(()=> new PBC::MapField<int, rawEventFarmBoardDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFarmBoardDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFarmBoardDetailMap;
			var abMap = v.EventFarmBoardDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFarmBoardDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFarmBoardDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFarmBoardDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFarmBoardDetailMap;
			if (v.EventFarmBoardDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFarmBoardDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFarmBoardDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFarmBoardDetailConf cc) {
			foreach(KeyValuePair <string, rawEventFarmBoardDetailMapABValue> tagEntry in cc.EventFarmBoardDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFarmBoardDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFarmBoardDetailMapABValue();
					cc.EventFarmBoardDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFarmBoardDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFarmBoardDetail> entry in cc.EventFarmBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFarmBoardDetail> entry in tagEntry.Value.EventFarmBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFarmBoardDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFarmBoardDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFarmBoardDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFarmBoardDetailMapAB != null)
			{
				foreach (var entry in cc.EventFarmBoardDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFarmBoardDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFarmBoardDetailMap.MergeFrom(cis);
			Patch(updating.EventFarmBoardDetailMap);
			updating.EventFarmBoardDetailSortedKeys = genSortedKeys(updating.EventFarmBoardDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFarmBoardDetailConf();
            tmp.MergeFrom(cis);
			updating.EventFarmBoardDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFarmBoardDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFarmBoardDetail> filter,string tag=""){
		...
		public static rawEventFarmBoardDetail MustGet(int key,string tag="") {
		...
		public static rawEventFarmBoardDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFarmBoardDetail> GetByFilter(Func<rawEventFarmBoardDetail, bool> filterFunc,string tag="") {
			List<rawEventFarmBoardDetail> ret = new List<rawEventFarmBoardDetail>();
			foreach(KeyValuePair <int, rawEventFarmBoardDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFarmBoardDetail? GetOneByFilter(Func<rawEventFarmBoardDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFarmBoardDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFarmBoardDetail data) 
		{
			return data.Id;			
		}
    }
