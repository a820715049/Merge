...
using OrderRandomerConf = fat.rawdata.OrderRandomerConf;
using rawOrderRandomer = fat.rawdata.OrderRandomer;
using rawOrderRandomerMapABValue = fat.rawdata.OrderRandomerMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderRandomerVisitor
    {
		...
		public const string FileName = "OrderRandomerConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderRandomerVisitor.SetABDefault单独设定OrderRandomer的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderRandomerVisitor.SetABFallback单独设定OrderRandomer的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderRandomer>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderRandomer>>(()=> new PBC::MapField<int, rawOrderRandomer>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderRandomer>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderRandomerMap;
			var abMap = v.OrderRandomerMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderRandomerMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderRandomerMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderRandomer> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderRandomerMap;
			if (v.OrderRandomerMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderRandomerMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderRandomerMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderRandomerConf cc) {
			foreach(KeyValuePair <string, rawOrderRandomerMapABValue> tagEntry in cc.OrderRandomerMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderRandomerMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderRandomerMapABValue();
					cc.OrderRandomerMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderRandomerMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderRandomer> entry in cc.OrderRandomerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderRandomer> entry in tagEntry.Value.OrderRandomerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderRandomer> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderRandomerConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderRandomerMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderRandomerMapAB != null)
			{
				foreach (var entry in cc.OrderRandomerMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderRandomerMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderRandomerMap.MergeFrom(cis);
			Patch(updating.OrderRandomerMap);
			updating.OrderRandomerSortedKeys = genSortedKeys(updating.OrderRandomerMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderRandomerConf();
            tmp.MergeFrom(cis);
			updating.OrderRandomerMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderRandomerSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderRandomer> filter,string tag=""){
		...
		public static rawOrderRandomer MustGet(int key,string tag="") {
		...
		public static rawOrderRandomer? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderRandomer> GetByFilter(Func<rawOrderRandomer, bool> filterFunc,string tag="") {
			List<rawOrderRandomer> ret = new List<rawOrderRandomer>();
			foreach(KeyValuePair <int, rawOrderRandomer> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderRandomer? GetOneByFilter(Func<rawOrderRandomer, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderRandomer> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderRandomer data) 
		{
			return data.Id;			
		}
    }
