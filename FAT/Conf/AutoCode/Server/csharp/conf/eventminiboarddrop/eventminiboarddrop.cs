...
using EventMiniBoardDropConf = fat.rawdata.EventMiniBoardDropConf;
using rawEventMiniBoardDrop = fat.rawdata.EventMiniBoardDrop;
using rawEventMiniBoardDropMapABValue = fat.rawdata.EventMiniBoardDropMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardDropVisitor
    {
		...
		public const string FileName = "EventMiniBoardDropConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardDropVisitor.SetABDefault单独设定EventMiniBoardDrop的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardDropVisitor.SetABFallback单独设定EventMiniBoardDrop的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoardDrop>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoardDrop>>(()=> new PBC::MapField<int, rawEventMiniBoardDrop>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoardDrop>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardDropMap;
			var abMap = v.EventMiniBoardDropMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardDropMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardDropMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoardDrop> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardDropMap;
			if (v.EventMiniBoardDropMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardDropMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardDropMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardDropConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardDropMapABValue> tagEntry in cc.EventMiniBoardDropMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardDropMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardDropMapABValue();
					cc.EventMiniBoardDropMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardDropMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoardDrop> entry in cc.EventMiniBoardDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoardDrop> entry in tagEntry.Value.EventMiniBoardDropMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoardDrop> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardDropConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardDropMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardDropMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardDropMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardDropMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardDropMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardDropMap);
			updating.EventMiniBoardDropSortedKeys = genSortedKeys(updating.EventMiniBoardDropMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardDropConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardDropMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardDropSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoardDrop> filter,string tag=""){
		...
		public static rawEventMiniBoardDrop MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoardDrop? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoardDrop> GetByFilter(Func<rawEventMiniBoardDrop, bool> filterFunc,string tag="") {
			List<rawEventMiniBoardDrop> ret = new List<rawEventMiniBoardDrop>();
			foreach(KeyValuePair <int, rawEventMiniBoardDrop> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoardDrop? GetOneByFilter(Func<rawEventMiniBoardDrop, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoardDrop> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoardDrop data) 
		{
			return data.Id;			
		}
    }
