...
using EventSeaRaceRoundConf = fat.rawdata.EventSeaRaceRoundConf;
using rawEventSeaRaceRound = fat.rawdata.EventSeaRaceRound;
using rawEventSeaRaceRoundMapABValue = fat.rawdata.EventSeaRaceRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaRaceRoundVisitor
    {
		...
		public const string FileName = "EventSeaRaceRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaRaceRoundVisitor.SetABDefault单独设定EventSeaRaceRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaRaceRoundVisitor.SetABFallback单独设定EventSeaRaceRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaRaceRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaRaceRound>>(()=> new PBC::MapField<int, rawEventSeaRaceRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaRaceRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaRaceRoundMap;
			var abMap = v.EventSeaRaceRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaRaceRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaRaceRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaRaceRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaRaceRoundMap;
			if (v.EventSeaRaceRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaRaceRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaRaceRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaRaceRoundConf cc) {
			foreach(KeyValuePair <string, rawEventSeaRaceRoundMapABValue> tagEntry in cc.EventSeaRaceRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaRaceRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaRaceRoundMapABValue();
					cc.EventSeaRaceRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaRaceRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaRaceRound> entry in cc.EventSeaRaceRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaRaceRound> entry in tagEntry.Value.EventSeaRaceRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaRaceRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaRaceRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaRaceRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaRaceRoundMapAB != null)
			{
				foreach (var entry in cc.EventSeaRaceRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaRaceRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaRaceRoundMap.MergeFrom(cis);
			Patch(updating.EventSeaRaceRoundMap);
			updating.EventSeaRaceRoundSortedKeys = genSortedKeys(updating.EventSeaRaceRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaRaceRoundConf();
            tmp.MergeFrom(cis);
			updating.EventSeaRaceRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaRaceRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaRaceRound> filter,string tag=""){
		...
		public static rawEventSeaRaceRound MustGet(int key,string tag="") {
		...
		public static rawEventSeaRaceRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaRaceRound> GetByFilter(Func<rawEventSeaRaceRound, bool> filterFunc,string tag="") {
			List<rawEventSeaRaceRound> ret = new List<rawEventSeaRaceRound>();
			foreach(KeyValuePair <int, rawEventSeaRaceRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaRaceRound? GetOneByFilter(Func<rawEventSeaRaceRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaRaceRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaRaceRound data) 
		{
			return data.Id;			
		}
    }
