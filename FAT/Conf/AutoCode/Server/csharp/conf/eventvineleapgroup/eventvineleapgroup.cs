...
using EventVineLeapGroupConf = fat.rawdata.EventVineLeapGroupConf;
using rawEventVineLeapGroup = fat.rawdata.EventVineLeapGroup;
using rawEventVineLeapGroupMapABValue = fat.rawdata.EventVineLeapGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventVineLeapGroupVisitor
    {
		...
		public const string FileName = "EventVineLeapGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventVineLeapGroupVisitor.SetABDefault单独设定EventVineLeapGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventVineLeapGroupVisitor.SetABFallback单独设定EventVineLeapGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventVineLeapGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventVineLeapGroup>>(()=> new PBC::MapField<int, rawEventVineLeapGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventVineLeapGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventVineLeapGroupMap;
			var abMap = v.EventVineLeapGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventVineLeapGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventVineLeapGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventVineLeapGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventVineLeapGroupMap;
			if (v.EventVineLeapGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventVineLeapGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventVineLeapGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventVineLeapGroupConf cc) {
			foreach(KeyValuePair <string, rawEventVineLeapGroupMapABValue> tagEntry in cc.EventVineLeapGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventVineLeapGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventVineLeapGroupMapABValue();
					cc.EventVineLeapGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventVineLeapGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventVineLeapGroup> entry in cc.EventVineLeapGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventVineLeapGroup> entry in tagEntry.Value.EventVineLeapGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventVineLeapGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventVineLeapGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventVineLeapGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventVineLeapGroupMapAB != null)
			{
				foreach (var entry in cc.EventVineLeapGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventVineLeapGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventVineLeapGroupMap.MergeFrom(cis);
			Patch(updating.EventVineLeapGroupMap);
			updating.EventVineLeapGroupSortedKeys = genSortedKeys(updating.EventVineLeapGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventVineLeapGroupConf();
            tmp.MergeFrom(cis);
			updating.EventVineLeapGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventVineLeapGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventVineLeapGroup> filter,string tag=""){
		...
		public static rawEventVineLeapGroup MustGet(int key,string tag="") {
		...
		public static rawEventVineLeapGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventVineLeapGroup> GetByFilter(Func<rawEventVineLeapGroup, bool> filterFunc,string tag="") {
			List<rawEventVineLeapGroup> ret = new List<rawEventVineLeapGroup>();
			foreach(KeyValuePair <int, rawEventVineLeapGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventVineLeapGroup? GetOneByFilter(Func<rawEventVineLeapGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventVineLeapGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventVineLeapGroup data) 
		{
			return data.Id;			
		}
    }
