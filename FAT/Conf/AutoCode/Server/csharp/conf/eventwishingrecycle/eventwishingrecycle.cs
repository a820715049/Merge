...
using EventWishingRecycleConf = fat.rawdata.EventWishingRecycleConf;
using rawEventWishingRecycle = fat.rawdata.EventWishingRecycle;
using rawEventWishingRecycleMapABValue = fat.rawdata.EventWishingRecycleMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishingRecycleVisitor
    {
		...
		public const string FileName = "EventWishingRecycleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishingRecycleVisitor.SetABDefault单独设定EventWishingRecycle的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishingRecycleVisitor.SetABFallback单独设定EventWishingRecycle的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishingRecycle>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishingRecycle>>(()=> new PBC::MapField<int, rawEventWishingRecycle>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishingRecycle>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishingRecycleMap;
			var abMap = v.EventWishingRecycleMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishingRecycleMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishingRecycleMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishingRecycle> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishingRecycleMap;
			if (v.EventWishingRecycleMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishingRecycleMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishingRecycleMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishingRecycleConf cc) {
			foreach(KeyValuePair <string, rawEventWishingRecycleMapABValue> tagEntry in cc.EventWishingRecycleMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishingRecycleMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishingRecycleMapABValue();
					cc.EventWishingRecycleMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishingRecycleMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishingRecycle> entry in cc.EventWishingRecycleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishingRecycle> entry in tagEntry.Value.EventWishingRecycleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishingRecycle> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishingRecycleConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishingRecycleMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishingRecycleMapAB != null)
			{
				foreach (var entry in cc.EventWishingRecycleMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishingRecycleMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishingRecycleMap.MergeFrom(cis);
			Patch(updating.EventWishingRecycleMap);
			updating.EventWishingRecycleSortedKeys = genSortedKeys(updating.EventWishingRecycleMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishingRecycleConf();
            tmp.MergeFrom(cis);
			updating.EventWishingRecycleMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishingRecycleSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishingRecycle> filter,string tag=""){
		...
		public static rawEventWishingRecycle MustGet(int key,string tag="") {
		...
		public static rawEventWishingRecycle? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishingRecycle> GetByFilter(Func<rawEventWishingRecycle, bool> filterFunc,string tag="") {
			List<rawEventWishingRecycle> ret = new List<rawEventWishingRecycle>();
			foreach(KeyValuePair <int, rawEventWishingRecycle> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishingRecycle? GetOneByFilter(Func<rawEventWishingRecycle, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishingRecycle> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishingRecycle data) 
		{
			return data.Id;			
		}
    }
