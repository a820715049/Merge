...
using MailConf = fat.rawdata.MailConf;
using rawMail = fat.rawdata.Mail;
using rawMailMapABValue = fat.rawdata.MailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MailVisitor
    {
		...
		public const string FileName = "MailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MailVisitor.SetABDefault单独设定Mail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MailVisitor.SetABFallback单独设定Mail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMail>>(()=> new PBC::MapField<int, rawMail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MailMap;
			var abMap = v.MailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			return new List<string>(v.MailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MailConf cc) {
			foreach(KeyValuePair <string, rawMailMapABValue> tagEntry in cc.MailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMailMapABValue();
					cc.MailMapAB[tag] = ps;
				}
                var tagTmp = ps.MailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMail> entry in cc.MailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMail> entry in tagEntry.Value.MailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Mail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MailMapAB != null)
			{
				foreach (var entry in cc.MailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MailMap.MergeFrom(cis);
			Patch(updating.MailMap);
			updating.MailSortedKeys = genSortedKeys(updating.MailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MailConf();
            tmp.MergeFrom(cis);
			updating.MailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMail> filter,string tag=""){
		...
		public static rawMail MustGet(int key,string tag="") {
		...
		public static rawMail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMail> GetByFilter(Func<rawMail, bool> filterFunc,string tag="") {
			List<rawMail> ret = new List<rawMail>();
			foreach(KeyValuePair <int, rawMail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMail? GetOneByFilter(Func<rawMail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMail data) 
		{
			return data.Id;			
		}
    }
