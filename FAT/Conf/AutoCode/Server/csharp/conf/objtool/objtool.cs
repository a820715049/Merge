...
using ObjToolConf = fat.rawdata.ObjToolConf;
using rawObjTool = fat.rawdata.ObjTool;
using rawObjToolMapABValue = fat.rawdata.ObjToolMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjToolVisitor
    {
		...
		public const string FileName = "ObjToolConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjToolVisitor.SetABDefault单独设定ObjTool的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjToolVisitor.SetABFallback单独设定ObjTool的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjTool>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjTool>>(()=> new PBC::MapField<int, rawObjTool>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjTool>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjToolMap;
			var abMap = v.ObjToolMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjToolMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjToolMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjTool> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjToolMap;
			if (v.ObjToolMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjToolMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjToolMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjToolConf cc) {
			foreach(KeyValuePair <string, rawObjToolMapABValue> tagEntry in cc.ObjToolMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjToolMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjToolMapABValue();
					cc.ObjToolMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjToolMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjTool> entry in cc.ObjToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjTool> entry in tagEntry.Value.ObjToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjTool> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjToolConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjToolMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjToolMapAB != null)
			{
				foreach (var entry in cc.ObjToolMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjToolMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjToolMap.MergeFrom(cis);
			Patch(updating.ObjToolMap);
			updating.ObjToolSortedKeys = genSortedKeys(updating.ObjToolMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjToolConf();
            tmp.MergeFrom(cis);
			updating.ObjToolMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjToolSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjTool> filter,string tag=""){
		...
		public static rawObjTool MustGet(int key,string tag="") {
		...
		public static rawObjTool? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjTool> GetByFilter(Func<rawObjTool, bool> filterFunc,string tag="") {
			List<rawObjTool> ret = new List<rawObjTool>();
			foreach(KeyValuePair <int, rawObjTool> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjTool? GetOneByFilter(Func<rawObjTool, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjTool> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjTool data) 
		{
			return data.Id;			
		}
    }
