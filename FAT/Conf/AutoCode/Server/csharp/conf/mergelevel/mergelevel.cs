...
using MergeLevelConf = fat.rawdata.MergeLevelConf;
using rawMergeLevel = fat.rawdata.MergeLevel;
using rawMergeLevelSliceABValue = fat.rawdata.MergeLevelSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeLevelVisitor
    {
		...
		public const string FileName = "MergeLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeLevelVisitor.SetABDefault单独设定MergeLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeLevelVisitor.SetABFallback单独设定MergeLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawMergeLevel>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawMergeLevel>>(()=>new PBC::RepeatedField<rawMergeLevel>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawMergeLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeLevelSlice;
			var abMap = v.MergeLevelSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeLevelSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeLevelSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawMergeLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeLevelSlice;
			if (v.MergeLevelSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeLevelSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeLevelSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeLevelConf cc) {
			foreach(KeyValuePair <string, rawMergeLevelSliceABValue> tagEntry in cc.MergeLevelSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeLevelSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeLevelSliceABValue();
					cc.MergeLevelSliceAB[tag] = ps;
				}
                var tagTmp = ps.MergeLevelSlice;
				// 基础数据
				for(int i=0; i<cc.MergeLevelSlice.Count; ++i) 
				{
					tagTmp.Add(cc.MergeLevelSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.MergeLevelSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.MergeLevel> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeLevelSlice.MergeFrom(cis);
			Patch(updating.MergeLevelSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeLevelConf();
            tmp.MergeFrom(cis);
			updating.MergeLevelSlice = tmp;
		...
		public static void Range(Action<int, rawMergeLevel> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawMergeLevel MustGetByIndex(int index,string tag="") {
		...
		public static rawMergeLevel? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawMergeLevel> GetByFilter(Func<rawMergeLevel,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawMergeLevel>();
		...
		static public rawMergeLevel? GetOneByFilter(Func<rawMergeLevel,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
