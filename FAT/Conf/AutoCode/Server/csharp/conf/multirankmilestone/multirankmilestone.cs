...
using MultiRankMilestoneConf = fat.rawdata.MultiRankMilestoneConf;
using rawMultiRankMilestone = fat.rawdata.MultiRankMilestone;
using rawMultiRankMilestoneMapABValue = fat.rawdata.MultiRankMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankMilestoneVisitor
    {
		...
		public const string FileName = "MultiRankMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankMilestoneVisitor.SetABDefault单独设定MultiRankMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankMilestoneVisitor.SetABFallback单独设定MultiRankMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRankMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRankMilestone>>(()=> new PBC::MapField<int, rawMultiRankMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRankMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankMilestoneMap;
			var abMap = v.MultiRankMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRankMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankMilestoneMap;
			if (v.MultiRankMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankMilestoneConf cc) {
			foreach(KeyValuePair <string, rawMultiRankMilestoneMapABValue> tagEntry in cc.MultiRankMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankMilestoneMapABValue();
					cc.MultiRankMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRankMilestone> entry in cc.MultiRankMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRankMilestone> entry in tagEntry.Value.MultiRankMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRankMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankMilestoneMapAB != null)
			{
				foreach (var entry in cc.MultiRankMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankMilestoneMap.MergeFrom(cis);
			Patch(updating.MultiRankMilestoneMap);
			updating.MultiRankMilestoneSortedKeys = genSortedKeys(updating.MultiRankMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankMilestoneConf();
            tmp.MergeFrom(cis);
			updating.MultiRankMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRankMilestone> filter,string tag=""){
		...
		public static rawMultiRankMilestone MustGet(int key,string tag="") {
		...
		public static rawMultiRankMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRankMilestone> GetByFilter(Func<rawMultiRankMilestone, bool> filterFunc,string tag="") {
			List<rawMultiRankMilestone> ret = new List<rawMultiRankMilestone>();
			foreach(KeyValuePair <int, rawMultiRankMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRankMilestone? GetOneByFilter(Func<rawMultiRankMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRankMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRankMilestone data) 
		{
			return data.Id;			
		}
    }
