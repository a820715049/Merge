...
using UserTagGroupConf = fat.rawdata.UserTagGroupConf;
using rawUserTagGroup = fat.rawdata.UserTagGroup;
using rawUserTagGroupMapABValue = fat.rawdata.UserTagGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class UserTagGroupVisitor
    {
		...
		public const string FileName = "UserTagGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过UserTagGroupVisitor.SetABDefault单独设定UserTagGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过UserTagGroupVisitor.SetABFallback单独设定UserTagGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawUserTagGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawUserTagGroup>>(()=> new PBC::MapField<int, rawUserTagGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawUserTagGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().UserTagGroupMap;
			var abMap = v.UserTagGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.UserTagGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].UserTagGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawUserTagGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().UserTagGroupMap;
			if (v.UserTagGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.UserTagGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawUserTagGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(UserTagGroupConf cc) {
			foreach(KeyValuePair <string, rawUserTagGroupMapABValue> tagEntry in cc.UserTagGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.UserTagGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawUserTagGroupMapABValue();
					cc.UserTagGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.UserTagGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawUserTagGroup> entry in cc.UserTagGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawUserTagGroup> entry in tagEntry.Value.UserTagGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.UserTagGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(UserTagGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.UserTagGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.UserTagGroupMapAB != null)
			{
				foreach (var entry in cc.UserTagGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.UserTagGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.UserTagGroupMap.MergeFrom(cis);
			Patch(updating.UserTagGroupMap);
			updating.UserTagGroupSortedKeys = genSortedKeys(updating.UserTagGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new UserTagGroupConf();
            tmp.MergeFrom(cis);
			updating.UserTagGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().UserTagGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawUserTagGroup> filter,string tag=""){
		...
		public static rawUserTagGroup MustGet(int key,string tag="") {
		...
		public static rawUserTagGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawUserTagGroup> GetByFilter(Func<rawUserTagGroup, bool> filterFunc,string tag="") {
			List<rawUserTagGroup> ret = new List<rawUserTagGroup>();
			foreach(KeyValuePair <int, rawUserTagGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawUserTagGroup? GetOneByFilter(Func<rawUserTagGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawUserTagGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawUserTagGroup data) 
		{
			return data.Id;			
		}
    }
