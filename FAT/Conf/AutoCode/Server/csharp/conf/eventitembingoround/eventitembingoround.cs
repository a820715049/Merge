...
using EventItemBingoRoundConf = fat.rawdata.EventItemBingoRoundConf;
using rawEventItemBingoRound = fat.rawdata.EventItemBingoRound;
using rawEventItemBingoRoundMapABValue = fat.rawdata.EventItemBingoRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventItemBingoRoundVisitor
    {
		...
		public const string FileName = "EventItemBingoRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventItemBingoRoundVisitor.SetABDefault单独设定EventItemBingoRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventItemBingoRoundVisitor.SetABFallback单独设定EventItemBingoRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventItemBingoRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventItemBingoRound>>(()=> new PBC::MapField<int, rawEventItemBingoRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventItemBingoRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventItemBingoRoundMap;
			var abMap = v.EventItemBingoRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventItemBingoRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventItemBingoRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventItemBingoRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventItemBingoRoundMap;
			if (v.EventItemBingoRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventItemBingoRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventItemBingoRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventItemBingoRoundConf cc) {
			foreach(KeyValuePair <string, rawEventItemBingoRoundMapABValue> tagEntry in cc.EventItemBingoRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventItemBingoRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventItemBingoRoundMapABValue();
					cc.EventItemBingoRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventItemBingoRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventItemBingoRound> entry in cc.EventItemBingoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventItemBingoRound> entry in tagEntry.Value.EventItemBingoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventItemBingoRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventItemBingoRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventItemBingoRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventItemBingoRoundMapAB != null)
			{
				foreach (var entry in cc.EventItemBingoRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventItemBingoRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventItemBingoRoundMap.MergeFrom(cis);
			Patch(updating.EventItemBingoRoundMap);
			updating.EventItemBingoRoundSortedKeys = genSortedKeys(updating.EventItemBingoRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventItemBingoRoundConf();
            tmp.MergeFrom(cis);
			updating.EventItemBingoRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventItemBingoRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventItemBingoRound> filter,string tag=""){
		...
		public static rawEventItemBingoRound MustGet(int key,string tag="") {
		...
		public static rawEventItemBingoRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventItemBingoRound> GetByFilter(Func<rawEventItemBingoRound, bool> filterFunc,string tag="") {
			List<rawEventItemBingoRound> ret = new List<rawEventItemBingoRound>();
			foreach(KeyValuePair <int, rawEventItemBingoRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventItemBingoRound? GetOneByFilter(Func<rawEventItemBingoRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventItemBingoRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventItemBingoRound data) 
		{
			return data.Id;			
		}
    }
