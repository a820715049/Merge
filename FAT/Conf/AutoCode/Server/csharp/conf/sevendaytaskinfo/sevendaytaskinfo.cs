...
using SevenDayTaskInfoConf = fat.rawdata.SevenDayTaskInfoConf;
using rawSevenDayTaskInfo = fat.rawdata.SevenDayTaskInfo;
using rawSevenDayTaskInfoMapABValue = fat.rawdata.SevenDayTaskInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SevenDayTaskInfoVisitor
    {
		...
		public const string FileName = "SevenDayTaskInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SevenDayTaskInfoVisitor.SetABDefault单独设定SevenDayTaskInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SevenDayTaskInfoVisitor.SetABFallback单独设定SevenDayTaskInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSevenDayTaskInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSevenDayTaskInfo>>(()=> new PBC::MapField<int, rawSevenDayTaskInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSevenDayTaskInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SevenDayTaskInfoMap;
			var abMap = v.SevenDayTaskInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SevenDayTaskInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SevenDayTaskInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSevenDayTaskInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SevenDayTaskInfoMap;
			if (v.SevenDayTaskInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SevenDayTaskInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSevenDayTaskInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SevenDayTaskInfoConf cc) {
			foreach(KeyValuePair <string, rawSevenDayTaskInfoMapABValue> tagEntry in cc.SevenDayTaskInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SevenDayTaskInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSevenDayTaskInfoMapABValue();
					cc.SevenDayTaskInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.SevenDayTaskInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSevenDayTaskInfo> entry in cc.SevenDayTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSevenDayTaskInfo> entry in tagEntry.Value.SevenDayTaskInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SevenDayTaskInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SevenDayTaskInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SevenDayTaskInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SevenDayTaskInfoMapAB != null)
			{
				foreach (var entry in cc.SevenDayTaskInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SevenDayTaskInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SevenDayTaskInfoMap.MergeFrom(cis);
			Patch(updating.SevenDayTaskInfoMap);
			updating.SevenDayTaskInfoSortedKeys = genSortedKeys(updating.SevenDayTaskInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SevenDayTaskInfoConf();
            tmp.MergeFrom(cis);
			updating.SevenDayTaskInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SevenDayTaskInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSevenDayTaskInfo> filter,string tag=""){
		...
		public static rawSevenDayTaskInfo MustGet(int key,string tag="") {
		...
		public static rawSevenDayTaskInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSevenDayTaskInfo> GetByFilter(Func<rawSevenDayTaskInfo, bool> filterFunc,string tag="") {
			List<rawSevenDayTaskInfo> ret = new List<rawSevenDayTaskInfo>();
			foreach(KeyValuePair <int, rawSevenDayTaskInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSevenDayTaskInfo? GetOneByFilter(Func<rawSevenDayTaskInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSevenDayTaskInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSevenDayTaskInfo data) 
		{
			return data.Id;			
		}
    }
