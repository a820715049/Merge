...
using EventSeaRaceDetailConf = fat.rawdata.EventSeaRaceDetailConf;
using rawEventSeaRaceDetail = fat.rawdata.EventSeaRaceDetail;
using rawEventSeaRaceDetailMapABValue = fat.rawdata.EventSeaRaceDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaRaceDetailVisitor
    {
		...
		public const string FileName = "EventSeaRaceDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaRaceDetailVisitor.SetABDefault单独设定EventSeaRaceDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaRaceDetailVisitor.SetABFallback单独设定EventSeaRaceDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaRaceDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaRaceDetail>>(()=> new PBC::MapField<int, rawEventSeaRaceDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaRaceDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaRaceDetailMap;
			var abMap = v.EventSeaRaceDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaRaceDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaRaceDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaRaceDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaRaceDetailMap;
			if (v.EventSeaRaceDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaRaceDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaRaceDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaRaceDetailConf cc) {
			foreach(KeyValuePair <string, rawEventSeaRaceDetailMapABValue> tagEntry in cc.EventSeaRaceDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaRaceDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaRaceDetailMapABValue();
					cc.EventSeaRaceDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaRaceDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaRaceDetail> entry in cc.EventSeaRaceDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaRaceDetail> entry in tagEntry.Value.EventSeaRaceDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaRaceDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaRaceDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaRaceDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaRaceDetailMapAB != null)
			{
				foreach (var entry in cc.EventSeaRaceDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaRaceDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaRaceDetailMap.MergeFrom(cis);
			Patch(updating.EventSeaRaceDetailMap);
			updating.EventSeaRaceDetailSortedKeys = genSortedKeys(updating.EventSeaRaceDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaRaceDetailConf();
            tmp.MergeFrom(cis);
			updating.EventSeaRaceDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaRaceDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaRaceDetail> filter,string tag=""){
		...
		public static rawEventSeaRaceDetail MustGet(int key,string tag="") {
		...
		public static rawEventSeaRaceDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaRaceDetail> GetByFilter(Func<rawEventSeaRaceDetail, bool> filterFunc,string tag="") {
			List<rawEventSeaRaceDetail> ret = new List<rawEventSeaRaceDetail>();
			foreach(KeyValuePair <int, rawEventSeaRaceDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaRaceDetail? GetOneByFilter(Func<rawEventSeaRaceDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaRaceDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaRaceDetail data) 
		{
			return data.Id;			
		}
    }
