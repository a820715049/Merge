...
using ObjRandomChestConf = fat.rawdata.ObjRandomChestConf;
using rawObjRandomChest = fat.rawdata.ObjRandomChest;
using rawObjRandomChestMapABValue = fat.rawdata.ObjRandomChestMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjRandomChestVisitor
    {
		...
		public const string FileName = "ObjRandomChestConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjRandomChestVisitor.SetABDefault单独设定ObjRandomChest的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjRandomChestVisitor.SetABFallback单独设定ObjRandomChest的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjRandomChest>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjRandomChest>>(()=> new PBC::MapField<int, rawObjRandomChest>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjRandomChest>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjRandomChestMap;
			var abMap = v.ObjRandomChestMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjRandomChestMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjRandomChestMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjRandomChest> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjRandomChestMap;
			if (v.ObjRandomChestMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjRandomChestMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjRandomChestMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjRandomChestConf cc) {
			foreach(KeyValuePair <string, rawObjRandomChestMapABValue> tagEntry in cc.ObjRandomChestMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjRandomChestMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjRandomChestMapABValue();
					cc.ObjRandomChestMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjRandomChestMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjRandomChest> entry in cc.ObjRandomChestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjRandomChest> entry in tagEntry.Value.ObjRandomChestMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjRandomChest> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjRandomChestConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjRandomChestMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjRandomChestMapAB != null)
			{
				foreach (var entry in cc.ObjRandomChestMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjRandomChestMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjRandomChestMap.MergeFrom(cis);
			Patch(updating.ObjRandomChestMap);
			updating.ObjRandomChestSortedKeys = genSortedKeys(updating.ObjRandomChestMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjRandomChestConf();
            tmp.MergeFrom(cis);
			updating.ObjRandomChestMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjRandomChestSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjRandomChest> filter,string tag=""){
		...
		public static rawObjRandomChest MustGet(int key,string tag="") {
		...
		public static rawObjRandomChest? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjRandomChest> GetByFilter(Func<rawObjRandomChest, bool> filterFunc,string tag="") {
			List<rawObjRandomChest> ret = new List<rawObjRandomChest>();
			foreach(KeyValuePair <int, rawObjRandomChest> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjRandomChest? GetOneByFilter(Func<rawObjRandomChest, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjRandomChest> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjRandomChest data) 
		{
			return data.Id;			
		}
    }
