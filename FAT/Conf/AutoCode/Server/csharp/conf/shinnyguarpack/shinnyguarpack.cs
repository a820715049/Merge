...
using ShinnyGuarPackConf = fat.rawdata.ShinnyGuarPackConf;
using rawShinnyGuarPack = fat.rawdata.ShinnyGuarPack;
using rawShinnyGuarPackMapABValue = fat.rawdata.ShinnyGuarPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ShinnyGuarPackVisitor
    {
		...
		public const string FileName = "ShinnyGuarPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ShinnyGuarPackVisitor.SetABDefault单独设定ShinnyGuarPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ShinnyGuarPackVisitor.SetABFallback单独设定ShinnyGuarPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawShinnyGuarPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawShinnyGuarPack>>(()=> new PBC::MapField<int, rawShinnyGuarPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawShinnyGuarPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ShinnyGuarPackMap;
			var abMap = v.ShinnyGuarPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ShinnyGuarPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ShinnyGuarPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawShinnyGuarPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ShinnyGuarPackMap;
			if (v.ShinnyGuarPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ShinnyGuarPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawShinnyGuarPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ShinnyGuarPackConf cc) {
			foreach(KeyValuePair <string, rawShinnyGuarPackMapABValue> tagEntry in cc.ShinnyGuarPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ShinnyGuarPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawShinnyGuarPackMapABValue();
					cc.ShinnyGuarPackMapAB[tag] = ps;
				}
                var tagTmp = ps.ShinnyGuarPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawShinnyGuarPack> entry in cc.ShinnyGuarPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawShinnyGuarPack> entry in tagEntry.Value.ShinnyGuarPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ShinnyGuarPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ShinnyGuarPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ShinnyGuarPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ShinnyGuarPackMapAB != null)
			{
				foreach (var entry in cc.ShinnyGuarPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ShinnyGuarPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ShinnyGuarPackMap.MergeFrom(cis);
			Patch(updating.ShinnyGuarPackMap);
			updating.ShinnyGuarPackSortedKeys = genSortedKeys(updating.ShinnyGuarPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ShinnyGuarPackConf();
            tmp.MergeFrom(cis);
			updating.ShinnyGuarPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ShinnyGuarPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawShinnyGuarPack> filter,string tag=""){
		...
		public static rawShinnyGuarPack MustGet(int key,string tag="") {
		...
		public static rawShinnyGuarPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawShinnyGuarPack> GetByFilter(Func<rawShinnyGuarPack, bool> filterFunc,string tag="") {
			List<rawShinnyGuarPack> ret = new List<rawShinnyGuarPack>();
			foreach(KeyValuePair <int, rawShinnyGuarPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawShinnyGuarPack? GetOneByFilter(Func<rawShinnyGuarPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawShinnyGuarPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawShinnyGuarPack data) 
		{
			return data.Id;			
		}
    }
