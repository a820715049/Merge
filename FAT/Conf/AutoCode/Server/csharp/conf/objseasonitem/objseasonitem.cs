...
using ObjSeasonItemConf = fat.rawdata.ObjSeasonItemConf;
using rawObjSeasonItem = fat.rawdata.ObjSeasonItem;
using rawObjSeasonItemMapABValue = fat.rawdata.ObjSeasonItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjSeasonItemVisitor
    {
		...
		public const string FileName = "ObjSeasonItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjSeasonItemVisitor.SetABDefault单独设定ObjSeasonItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjSeasonItemVisitor.SetABFallback单独设定ObjSeasonItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjSeasonItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjSeasonItem>>(()=> new PBC::MapField<int, rawObjSeasonItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjSeasonItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjSeasonItemMap;
			var abMap = v.ObjSeasonItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjSeasonItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjSeasonItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjSeasonItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjSeasonItemMap;
			if (v.ObjSeasonItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjSeasonItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjSeasonItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjSeasonItemConf cc) {
			foreach(KeyValuePair <string, rawObjSeasonItemMapABValue> tagEntry in cc.ObjSeasonItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjSeasonItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjSeasonItemMapABValue();
					cc.ObjSeasonItemMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjSeasonItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjSeasonItem> entry in cc.ObjSeasonItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjSeasonItem> entry in tagEntry.Value.ObjSeasonItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjSeasonItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjSeasonItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjSeasonItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjSeasonItemMapAB != null)
			{
				foreach (var entry in cc.ObjSeasonItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjSeasonItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjSeasonItemMap.MergeFrom(cis);
			Patch(updating.ObjSeasonItemMap);
			updating.ObjSeasonItemSortedKeys = genSortedKeys(updating.ObjSeasonItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjSeasonItemConf();
            tmp.MergeFrom(cis);
			updating.ObjSeasonItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjSeasonItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjSeasonItem> filter,string tag=""){
		...
		public static rawObjSeasonItem MustGet(int key,string tag="") {
		...
		public static rawObjSeasonItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjSeasonItem> GetByFilter(Func<rawObjSeasonItem, bool> filterFunc,string tag="") {
			List<rawObjSeasonItem> ret = new List<rawObjSeasonItem>();
			foreach(KeyValuePair <int, rawObjSeasonItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjSeasonItem? GetOneByFilter(Func<rawObjSeasonItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjSeasonItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjSeasonItem data) 
		{
			return data.Id;			
		}
    }
