...
using MarketSlidePackDetailConf = fat.rawdata.MarketSlidePackDetailConf;
using rawMarketSlidePackDetail = fat.rawdata.MarketSlidePackDetail;
using rawMarketSlidePackDetailMapABValue = fat.rawdata.MarketSlidePackDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketSlidePackDetailVisitor
    {
		...
		public const string FileName = "MarketSlidePackDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketSlidePackDetailVisitor.SetABDefault单独设定MarketSlidePackDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketSlidePackDetailVisitor.SetABFallback单独设定MarketSlidePackDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketSlidePackDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketSlidePackDetail>>(()=> new PBC::MapField<int, rawMarketSlidePackDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketSlidePackDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketSlidePackDetailMap;
			var abMap = v.MarketSlidePackDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketSlidePackDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketSlidePackDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketSlidePackDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketSlidePackDetailMap;
			if (v.MarketSlidePackDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketSlidePackDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketSlidePackDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketSlidePackDetailConf cc) {
			foreach(KeyValuePair <string, rawMarketSlidePackDetailMapABValue> tagEntry in cc.MarketSlidePackDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketSlidePackDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketSlidePackDetailMapABValue();
					cc.MarketSlidePackDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketSlidePackDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketSlidePackDetail> entry in cc.MarketSlidePackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketSlidePackDetail> entry in tagEntry.Value.MarketSlidePackDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketSlidePackDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketSlidePackDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketSlidePackDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketSlidePackDetailMapAB != null)
			{
				foreach (var entry in cc.MarketSlidePackDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketSlidePackDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketSlidePackDetailMap.MergeFrom(cis);
			Patch(updating.MarketSlidePackDetailMap);
			updating.MarketSlidePackDetailSortedKeys = genSortedKeys(updating.MarketSlidePackDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketSlidePackDetailConf();
            tmp.MergeFrom(cis);
			updating.MarketSlidePackDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketSlidePackDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketSlidePackDetail> filter,string tag=""){
		...
		public static rawMarketSlidePackDetail MustGet(int key,string tag="") {
		...
		public static rawMarketSlidePackDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketSlidePackDetail> GetByFilter(Func<rawMarketSlidePackDetail, bool> filterFunc,string tag="") {
			List<rawMarketSlidePackDetail> ret = new List<rawMarketSlidePackDetail>();
			foreach(KeyValuePair <int, rawMarketSlidePackDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketSlidePackDetail? GetOneByFilter(Func<rawMarketSlidePackDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketSlidePackDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketSlidePackDetail data) 
		{
			return data.Id;			
		}
    }
