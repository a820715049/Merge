...
using ComMergeTimeSkipConf = fat.rawdata.ComMergeTimeSkipConf;
using rawComMergeTimeSkip = fat.rawdata.ComMergeTimeSkip;
using rawComMergeTimeSkipMapABValue = fat.rawdata.ComMergeTimeSkipMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeTimeSkipVisitor
    {
		...
		public const string FileName = "ComMergeTimeSkipConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeTimeSkipVisitor.SetABDefault单独设定ComMergeTimeSkip的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeTimeSkipVisitor.SetABFallback单独设定ComMergeTimeSkip的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeTimeSkip>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeTimeSkip>>(()=> new PBC::MapField<int, rawComMergeTimeSkip>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeTimeSkip>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeTimeSkipMap;
			var abMap = v.ComMergeTimeSkipMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeTimeSkipMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeTimeSkipMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeTimeSkip> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeTimeSkipMap;
			if (v.ComMergeTimeSkipMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeTimeSkipMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeTimeSkipMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeTimeSkipConf cc) {
			foreach(KeyValuePair <string, rawComMergeTimeSkipMapABValue> tagEntry in cc.ComMergeTimeSkipMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeTimeSkipMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeTimeSkipMapABValue();
					cc.ComMergeTimeSkipMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeTimeSkipMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeTimeSkip> entry in cc.ComMergeTimeSkipMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeTimeSkip> entry in tagEntry.Value.ComMergeTimeSkipMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeTimeSkip> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeTimeSkipConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeTimeSkipMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeTimeSkipMapAB != null)
			{
				foreach (var entry in cc.ComMergeTimeSkipMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeTimeSkipMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeTimeSkipMap.MergeFrom(cis);
			Patch(updating.ComMergeTimeSkipMap);
			updating.ComMergeTimeSkipSortedKeys = genSortedKeys(updating.ComMergeTimeSkipMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeTimeSkipConf();
            tmp.MergeFrom(cis);
			updating.ComMergeTimeSkipMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeTimeSkipSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeTimeSkip> filter,string tag=""){
		...
		public static rawComMergeTimeSkip MustGet(int key,string tag="") {
		...
		public static rawComMergeTimeSkip? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeTimeSkip> GetByFilter(Func<rawComMergeTimeSkip, bool> filterFunc,string tag="") {
			List<rawComMergeTimeSkip> ret = new List<rawComMergeTimeSkip>();
			foreach(KeyValuePair <int, rawComMergeTimeSkip> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeTimeSkip? GetOneByFilter(Func<rawComMergeTimeSkip, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeTimeSkip> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeTimeSkip data) 
		{
			return data.Id;			
		}
    }
