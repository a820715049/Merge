...
using BuildingLevelConf = fat.rawdata.BuildingLevelConf;
using rawBuildingLevel = fat.rawdata.BuildingLevel;
using rawBuildingLevelMapABValue = fat.rawdata.BuildingLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BuildingLevelVisitor
    {
		...
		public const string FileName = "BuildingLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BuildingLevelVisitor.SetABDefault单独设定BuildingLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BuildingLevelVisitor.SetABFallback单独设定BuildingLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBuildingLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBuildingLevel>>(()=> new PBC::MapField<int, rawBuildingLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBuildingLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BuildingLevelMap;
			var abMap = v.BuildingLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BuildingLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BuildingLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBuildingLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BuildingLevelMap;
			if (v.BuildingLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BuildingLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBuildingLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BuildingLevelConf cc) {
			foreach(KeyValuePair <string, rawBuildingLevelMapABValue> tagEntry in cc.BuildingLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BuildingLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBuildingLevelMapABValue();
					cc.BuildingLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.BuildingLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBuildingLevel> entry in cc.BuildingLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBuildingLevel> entry in tagEntry.Value.BuildingLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BuildingLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BuildingLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BuildingLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BuildingLevelMapAB != null)
			{
				foreach (var entry in cc.BuildingLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BuildingLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BuildingLevelMap.MergeFrom(cis);
			Patch(updating.BuildingLevelMap);
			updating.BuildingLevelSortedKeys = genSortedKeys(updating.BuildingLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BuildingLevelConf();
            tmp.MergeFrom(cis);
			updating.BuildingLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BuildingLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBuildingLevel> filter,string tag=""){
		...
		public static rawBuildingLevel MustGet(int key,string tag="") {
		...
		public static rawBuildingLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBuildingLevel> GetByFilter(Func<rawBuildingLevel, bool> filterFunc,string tag="") {
			List<rawBuildingLevel> ret = new List<rawBuildingLevel>();
			foreach(KeyValuePair <int, rawBuildingLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBuildingLevel? GetOneByFilter(Func<rawBuildingLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBuildingLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBuildingLevel data) 
		{
			return data.Id;			
		}
    }
