...
using EventMysteriousTokenConf = fat.rawdata.EventMysteriousTokenConf;
using rawEventMysteriousToken = fat.rawdata.EventMysteriousToken;
using rawEventMysteriousTokenMapABValue = fat.rawdata.EventMysteriousTokenMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMysteriousTokenVisitor
    {
		...
		public const string FileName = "EventMysteriousTokenConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMysteriousTokenVisitor.SetABDefault单独设定EventMysteriousToken的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMysteriousTokenVisitor.SetABFallback单独设定EventMysteriousToken的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMysteriousToken>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMysteriousToken>>(()=> new PBC::MapField<int, rawEventMysteriousToken>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMysteriousToken>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMysteriousTokenMap;
			var abMap = v.EventMysteriousTokenMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMysteriousTokenMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMysteriousTokenMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMysteriousToken> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMysteriousTokenMap;
			if (v.EventMysteriousTokenMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMysteriousTokenMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMysteriousTokenMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMysteriousTokenConf cc) {
			foreach(KeyValuePair <string, rawEventMysteriousTokenMapABValue> tagEntry in cc.EventMysteriousTokenMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMysteriousTokenMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMysteriousTokenMapABValue();
					cc.EventMysteriousTokenMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMysteriousTokenMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMysteriousToken> entry in cc.EventMysteriousTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMysteriousToken> entry in tagEntry.Value.EventMysteriousTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMysteriousToken> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMysteriousTokenConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMysteriousTokenMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMysteriousTokenMapAB != null)
			{
				foreach (var entry in cc.EventMysteriousTokenMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMysteriousTokenMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMysteriousTokenMap.MergeFrom(cis);
			Patch(updating.EventMysteriousTokenMap);
			updating.EventMysteriousTokenSortedKeys = genSortedKeys(updating.EventMysteriousTokenMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMysteriousTokenConf();
            tmp.MergeFrom(cis);
			updating.EventMysteriousTokenMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMysteriousTokenSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMysteriousToken> filter,string tag=""){
		...
		public static rawEventMysteriousToken MustGet(int key,string tag="") {
		...
		public static rawEventMysteriousToken? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMysteriousToken> GetByFilter(Func<rawEventMysteriousToken, bool> filterFunc,string tag="") {
			List<rawEventMysteriousToken> ret = new List<rawEventMysteriousToken>();
			foreach(KeyValuePair <int, rawEventMysteriousToken> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMysteriousToken? GetOneByFilter(Func<rawEventMysteriousToken, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMysteriousToken> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMysteriousToken data) 
		{
			return data.Id;			
		}
    }
