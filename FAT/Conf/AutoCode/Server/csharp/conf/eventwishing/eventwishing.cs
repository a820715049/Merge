...
using EventWishingConf = fat.rawdata.EventWishingConf;
using rawEventWishing = fat.rawdata.EventWishing;
using rawEventWishingMapABValue = fat.rawdata.EventWishingMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishingVisitor
    {
		...
		public const string FileName = "EventWishingConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishingVisitor.SetABDefault单独设定EventWishing的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishingVisitor.SetABFallback单独设定EventWishing的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishing>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishing>>(()=> new PBC::MapField<int, rawEventWishing>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishing>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishingMap;
			var abMap = v.EventWishingMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishingMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishingMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishing> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishingMap;
			if (v.EventWishingMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishingMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishingMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishingConf cc) {
			foreach(KeyValuePair <string, rawEventWishingMapABValue> tagEntry in cc.EventWishingMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishingMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishingMapABValue();
					cc.EventWishingMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishingMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishing> entry in cc.EventWishingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishing> entry in tagEntry.Value.EventWishingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishing> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishingConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishingMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishingMapAB != null)
			{
				foreach (var entry in cc.EventWishingMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishingMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishingMap.MergeFrom(cis);
			Patch(updating.EventWishingMap);
			updating.EventWishingSortedKeys = genSortedKeys(updating.EventWishingMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishingConf();
            tmp.MergeFrom(cis);
			updating.EventWishingMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishingSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishing> filter,string tag=""){
		...
		public static rawEventWishing MustGet(int key,string tag="") {
		...
		public static rawEventWishing? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishing> GetByFilter(Func<rawEventWishing, bool> filterFunc,string tag="") {
			List<rawEventWishing> ret = new List<rawEventWishing>();
			foreach(KeyValuePair <int, rawEventWishing> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishing? GetOneByFilter(Func<rawEventWishing, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishing> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishing data) 
		{
			return data.Id;			
		}
    }
