...
using EventMineCartRowConf = fat.rawdata.EventMineCartRowConf;
using rawEventMineCartRow = fat.rawdata.EventMineCartRow;
using rawEventMineCartRowMapABValue = fat.rawdata.EventMineCartRowMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartRowVisitor
    {
		...
		public const string FileName = "EventMineCartRowConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartRowVisitor.SetABDefault单独设定EventMineCartRow的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartRowVisitor.SetABFallback单独设定EventMineCartRow的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartRow>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartRow>>(()=> new PBC::MapField<int, rawEventMineCartRow>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartRow>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartRowMap;
			var abMap = v.EventMineCartRowMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartRowMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartRowMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartRow> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartRowMap;
			if (v.EventMineCartRowMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartRowMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartRowMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartRowConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartRowMapABValue> tagEntry in cc.EventMineCartRowMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartRowMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartRowMapABValue();
					cc.EventMineCartRowMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartRowMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartRow> entry in cc.EventMineCartRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartRow> entry in tagEntry.Value.EventMineCartRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartRow> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartRowConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartRowMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartRowMapAB != null)
			{
				foreach (var entry in cc.EventMineCartRowMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartRowMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartRowMap.MergeFrom(cis);
			Patch(updating.EventMineCartRowMap);
			updating.EventMineCartRowSortedKeys = genSortedKeys(updating.EventMineCartRowMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartRowConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartRowMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartRowSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartRow> filter,string tag=""){
		...
		public static rawEventMineCartRow MustGet(int key,string tag="") {
		...
		public static rawEventMineCartRow? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartRow> GetByFilter(Func<rawEventMineCartRow, bool> filterFunc,string tag="") {
			List<rawEventMineCartRow> ret = new List<rawEventMineCartRow>();
			foreach(KeyValuePair <int, rawEventMineCartRow> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartRow? GetOneByFilter(Func<rawEventMineCartRow, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartRow> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartRow data) 
		{
			return data.Id;			
		}
    }
