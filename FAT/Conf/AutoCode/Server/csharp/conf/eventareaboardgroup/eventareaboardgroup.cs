...
using EventAreaBoardGroupConf = fat.rawdata.EventAreaBoardGroupConf;
using rawEventAreaBoardGroup = fat.rawdata.EventAreaBoardGroup;
using rawEventAreaBoardGroupMapABValue = fat.rawdata.EventAreaBoardGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaBoardGroupVisitor
    {
		...
		public const string FileName = "EventAreaBoardGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaBoardGroupVisitor.SetABDefault单独设定EventAreaBoardGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaBoardGroupVisitor.SetABFallback单独设定EventAreaBoardGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaBoardGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaBoardGroup>>(()=> new PBC::MapField<int, rawEventAreaBoardGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaBoardGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaBoardGroupMap;
			var abMap = v.EventAreaBoardGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaBoardGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaBoardGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaBoardGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaBoardGroupMap;
			if (v.EventAreaBoardGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaBoardGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaBoardGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaBoardGroupConf cc) {
			foreach(KeyValuePair <string, rawEventAreaBoardGroupMapABValue> tagEntry in cc.EventAreaBoardGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaBoardGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaBoardGroupMapABValue();
					cc.EventAreaBoardGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaBoardGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaBoardGroup> entry in cc.EventAreaBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaBoardGroup> entry in tagEntry.Value.EventAreaBoardGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaBoardGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaBoardGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaBoardGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaBoardGroupMapAB != null)
			{
				foreach (var entry in cc.EventAreaBoardGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaBoardGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaBoardGroupMap.MergeFrom(cis);
			Patch(updating.EventAreaBoardGroupMap);
			updating.EventAreaBoardGroupSortedKeys = genSortedKeys(updating.EventAreaBoardGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaBoardGroupConf();
            tmp.MergeFrom(cis);
			updating.EventAreaBoardGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaBoardGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaBoardGroup> filter,string tag=""){
		...
		public static rawEventAreaBoardGroup MustGet(int key,string tag="") {
		...
		public static rawEventAreaBoardGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaBoardGroup> GetByFilter(Func<rawEventAreaBoardGroup, bool> filterFunc,string tag="") {
			List<rawEventAreaBoardGroup> ret = new List<rawEventAreaBoardGroup>();
			foreach(KeyValuePair <int, rawEventAreaBoardGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaBoardGroup? GetOneByFilter(Func<rawEventAreaBoardGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaBoardGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaBoardGroup data) 
		{
			return data.Id;			
		}
    }
