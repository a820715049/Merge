...
using EventClawOrderResourceConf = fat.rawdata.EventClawOrderResourceConf;
using rawEventClawOrderResource = fat.rawdata.EventClawOrderResource;
using rawEventClawOrderResourceMapABValue = fat.rawdata.EventClawOrderResourceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderResourceVisitor
    {
		...
		public const string FileName = "EventClawOrderResourceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderResourceVisitor.SetABDefault单独设定EventClawOrderResource的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderResourceVisitor.SetABFallback单独设定EventClawOrderResource的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrderResource>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrderResource>>(()=> new PBC::MapField<int, rawEventClawOrderResource>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrderResource>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderResourceMap;
			var abMap = v.EventClawOrderResourceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderResourceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderResourceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrderResource> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderResourceMap;
			if (v.EventClawOrderResourceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderResourceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderResourceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderResourceConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderResourceMapABValue> tagEntry in cc.EventClawOrderResourceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderResourceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderResourceMapABValue();
					cc.EventClawOrderResourceMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderResourceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrderResource> entry in cc.EventClawOrderResourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrderResource> entry in tagEntry.Value.EventClawOrderResourceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrderResource> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderResourceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderResourceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderResourceMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderResourceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderResourceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderResourceMap.MergeFrom(cis);
			Patch(updating.EventClawOrderResourceMap);
			updating.EventClawOrderResourceSortedKeys = genSortedKeys(updating.EventClawOrderResourceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderResourceConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderResourceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderResourceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrderResource> filter,string tag=""){
		...
		public static rawEventClawOrderResource MustGet(int key,string tag="") {
		...
		public static rawEventClawOrderResource? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrderResource> GetByFilter(Func<rawEventClawOrderResource, bool> filterFunc,string tag="") {
			List<rawEventClawOrderResource> ret = new List<rawEventClawOrderResource>();
			foreach(KeyValuePair <int, rawEventClawOrderResource> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrderResource? GetOneByFilter(Func<rawEventClawOrderResource, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrderResource> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrderResource data) 
		{
			return data.Id;			
		}
    }
