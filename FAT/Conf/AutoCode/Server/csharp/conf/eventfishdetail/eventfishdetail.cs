...
using EventFishDetailConf = fat.rawdata.EventFishDetailConf;
using rawEventFishDetail = fat.rawdata.EventFishDetail;
using rawEventFishDetailMapABValue = fat.rawdata.EventFishDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventFishDetailVisitor
    {
		...
		public const string FileName = "EventFishDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventFishDetailVisitor.SetABDefault单独设定EventFishDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventFishDetailVisitor.SetABFallback单独设定EventFishDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventFishDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventFishDetail>>(()=> new PBC::MapField<int, rawEventFishDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventFishDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventFishDetailMap;
			var abMap = v.EventFishDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventFishDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventFishDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventFishDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventFishDetailMap;
			if (v.EventFishDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventFishDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventFishDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventFishDetailConf cc) {
			foreach(KeyValuePair <string, rawEventFishDetailMapABValue> tagEntry in cc.EventFishDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventFishDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventFishDetailMapABValue();
					cc.EventFishDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventFishDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventFishDetail> entry in cc.EventFishDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventFishDetail> entry in tagEntry.Value.EventFishDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventFishDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventFishDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventFishDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventFishDetailMapAB != null)
			{
				foreach (var entry in cc.EventFishDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventFishDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventFishDetailMap.MergeFrom(cis);
			Patch(updating.EventFishDetailMap);
			updating.EventFishDetailSortedKeys = genSortedKeys(updating.EventFishDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventFishDetailConf();
            tmp.MergeFrom(cis);
			updating.EventFishDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventFishDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventFishDetail> filter,string tag=""){
		...
		public static rawEventFishDetail MustGet(int key,string tag="") {
		...
		public static rawEventFishDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventFishDetail> GetByFilter(Func<rawEventFishDetail, bool> filterFunc,string tag="") {
			List<rawEventFishDetail> ret = new List<rawEventFishDetail>();
			foreach(KeyValuePair <int, rawEventFishDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventFishDetail? GetOneByFilter(Func<rawEventFishDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventFishDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventFishDetail data) 
		{
			return data.Id;			
		}
    }
