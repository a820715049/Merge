...
using LoginSignConf = fat.rawdata.LoginSignConf;
using rawLoginSign = fat.rawdata.LoginSign;
using rawLoginSignSliceABValue = fat.rawdata.LoginSignSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LoginSignVisitor
    {
		...
		public const string FileName = "LoginSignConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LoginSignVisitor.SetABDefault单独设定LoginSign的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LoginSignVisitor.SetABFallback单独设定LoginSign的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLoginSign>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLoginSign>>(()=>new PBC::RepeatedField<rawLoginSign>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLoginSign>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LoginSignSlice;
			var abMap = v.LoginSignSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LoginSignSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LoginSignSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLoginSign> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LoginSignSlice;
			if (v.LoginSignSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LoginSignSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLoginSignSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LoginSignConf cc) {
			foreach(KeyValuePair <string, rawLoginSignSliceABValue> tagEntry in cc.LoginSignSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LoginSignSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLoginSignSliceABValue();
					cc.LoginSignSliceAB[tag] = ps;
				}
                var tagTmp = ps.LoginSignSlice;
				// 基础数据
				for(int i=0; i<cc.LoginSignSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LoginSignSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LoginSignSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LoginSign> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LoginSignSlice.MergeFrom(cis);
			Patch(updating.LoginSignSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LoginSignConf();
            tmp.MergeFrom(cis);
			updating.LoginSignSlice = tmp;
		...
		public static void Range(Action<int, rawLoginSign> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLoginSign MustGetByIndex(int index,string tag="") {
		...
		public static rawLoginSign? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLoginSign> GetByFilter(Func<rawLoginSign,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLoginSign>();
		...
		static public rawLoginSign? GetOneByFilter(Func<rawLoginSign,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
