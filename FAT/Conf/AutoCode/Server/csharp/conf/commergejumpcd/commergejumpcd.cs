...
using ComMergeJumpCDConf = fat.rawdata.ComMergeJumpCDConf;
using rawComMergeJumpCD = fat.rawdata.ComMergeJumpCD;
using rawComMergeJumpCDMapABValue = fat.rawdata.ComMergeJumpCDMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeJumpCDVisitor
    {
		...
		public const string FileName = "ComMergeJumpCDConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeJumpCDVisitor.SetABDefault单独设定ComMergeJumpCD的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeJumpCDVisitor.SetABFallback单独设定ComMergeJumpCD的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeJumpCD>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeJumpCD>>(()=> new PBC::MapField<int, rawComMergeJumpCD>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeJumpCD>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeJumpCDMap;
			var abMap = v.ComMergeJumpCDMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeJumpCDMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeJumpCDMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeJumpCD> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeJumpCDMap;
			if (v.ComMergeJumpCDMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeJumpCDMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeJumpCDMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeJumpCDConf cc) {
			foreach(KeyValuePair <string, rawComMergeJumpCDMapABValue> tagEntry in cc.ComMergeJumpCDMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeJumpCDMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeJumpCDMapABValue();
					cc.ComMergeJumpCDMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeJumpCDMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeJumpCD> entry in cc.ComMergeJumpCDMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeJumpCD> entry in tagEntry.Value.ComMergeJumpCDMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeJumpCD> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeJumpCDConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeJumpCDMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeJumpCDMapAB != null)
			{
				foreach (var entry in cc.ComMergeJumpCDMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeJumpCDMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeJumpCDMap.MergeFrom(cis);
			Patch(updating.ComMergeJumpCDMap);
			updating.ComMergeJumpCDSortedKeys = genSortedKeys(updating.ComMergeJumpCDMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeJumpCDConf();
            tmp.MergeFrom(cis);
			updating.ComMergeJumpCDMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeJumpCDSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeJumpCD> filter,string tag=""){
		...
		public static rawComMergeJumpCD MustGet(int key,string tag="") {
		...
		public static rawComMergeJumpCD? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeJumpCD> GetByFilter(Func<rawComMergeJumpCD, bool> filterFunc,string tag="") {
			List<rawComMergeJumpCD> ret = new List<rawComMergeJumpCD>();
			foreach(KeyValuePair <int, rawComMergeJumpCD> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeJumpCD? GetOneByFilter(Func<rawComMergeJumpCD, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeJumpCD> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeJumpCD data) 
		{
			return data.Id;			
		}
    }
