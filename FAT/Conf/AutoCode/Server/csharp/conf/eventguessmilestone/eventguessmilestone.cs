...
using EventGuessMilestoneConf = fat.rawdata.EventGuessMilestoneConf;
using rawEventGuessMilestone = fat.rawdata.EventGuessMilestone;
using rawEventGuessMilestoneMapABValue = fat.rawdata.EventGuessMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventGuessMilestoneVisitor
    {
		...
		public const string FileName = "EventGuessMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventGuessMilestoneVisitor.SetABDefault单独设定EventGuessMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventGuessMilestoneVisitor.SetABFallback单独设定EventGuessMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventGuessMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventGuessMilestone>>(()=> new PBC::MapField<int, rawEventGuessMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventGuessMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventGuessMilestoneMap;
			var abMap = v.EventGuessMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventGuessMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventGuessMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventGuessMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventGuessMilestoneMap;
			if (v.EventGuessMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventGuessMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventGuessMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventGuessMilestoneConf cc) {
			foreach(KeyValuePair <string, rawEventGuessMilestoneMapABValue> tagEntry in cc.EventGuessMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventGuessMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventGuessMilestoneMapABValue();
					cc.EventGuessMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventGuessMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventGuessMilestone> entry in cc.EventGuessMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventGuessMilestone> entry in tagEntry.Value.EventGuessMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventGuessMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventGuessMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventGuessMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventGuessMilestoneMapAB != null)
			{
				foreach (var entry in cc.EventGuessMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventGuessMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventGuessMilestoneMap.MergeFrom(cis);
			Patch(updating.EventGuessMilestoneMap);
			updating.EventGuessMilestoneSortedKeys = genSortedKeys(updating.EventGuessMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventGuessMilestoneConf();
            tmp.MergeFrom(cis);
			updating.EventGuessMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventGuessMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventGuessMilestone> filter,string tag=""){
		...
		public static rawEventGuessMilestone MustGet(int key,string tag="") {
		...
		public static rawEventGuessMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventGuessMilestone> GetByFilter(Func<rawEventGuessMilestone, bool> filterFunc,string tag="") {
			List<rawEventGuessMilestone> ret = new List<rawEventGuessMilestone>();
			foreach(KeyValuePair <int, rawEventGuessMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventGuessMilestone? GetOneByFilter(Func<rawEventGuessMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventGuessMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventGuessMilestone data) 
		{
			return data.Id;			
		}
    }
