...
using EventOnlineDetailConf = fat.rawdata.EventOnlineDetailConf;
using rawEventOnlineDetail = fat.rawdata.EventOnlineDetail;
using rawEventOnlineDetailMapABValue = fat.rawdata.EventOnlineDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOnlineDetailVisitor
    {
		...
		public const string FileName = "EventOnlineDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOnlineDetailVisitor.SetABDefault单独设定EventOnlineDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOnlineDetailVisitor.SetABFallback单独设定EventOnlineDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOnlineDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOnlineDetail>>(()=> new PBC::MapField<int, rawEventOnlineDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOnlineDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOnlineDetailMap;
			var abMap = v.EventOnlineDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOnlineDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOnlineDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOnlineDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOnlineDetailMap;
			if (v.EventOnlineDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOnlineDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOnlineDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOnlineDetailConf cc) {
			foreach(KeyValuePair <string, rawEventOnlineDetailMapABValue> tagEntry in cc.EventOnlineDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOnlineDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOnlineDetailMapABValue();
					cc.EventOnlineDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOnlineDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOnlineDetail> entry in cc.EventOnlineDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOnlineDetail> entry in tagEntry.Value.EventOnlineDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOnlineDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOnlineDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOnlineDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOnlineDetailMapAB != null)
			{
				foreach (var entry in cc.EventOnlineDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOnlineDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOnlineDetailMap.MergeFrom(cis);
			Patch(updating.EventOnlineDetailMap);
			updating.EventOnlineDetailSortedKeys = genSortedKeys(updating.EventOnlineDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOnlineDetailConf();
            tmp.MergeFrom(cis);
			updating.EventOnlineDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOnlineDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOnlineDetail> filter,string tag=""){
		...
		public static rawEventOnlineDetail MustGet(int key,string tag="") {
		...
		public static rawEventOnlineDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOnlineDetail> GetByFilter(Func<rawEventOnlineDetail, bool> filterFunc,string tag="") {
			List<rawEventOnlineDetail> ret = new List<rawEventOnlineDetail>();
			foreach(KeyValuePair <int, rawEventOnlineDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOnlineDetail? GetOneByFilter(Func<rawEventOnlineDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOnlineDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOnlineDetail data) 
		{
			return data.Id;			
		}
    }
