...
using AreaBoardPackConf = fat.rawdata.AreaBoardPackConf;
using rawAreaBoardPack = fat.rawdata.AreaBoardPack;
using rawAreaBoardPackMapABValue = fat.rawdata.AreaBoardPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class AreaBoardPackVisitor
    {
		...
		public const string FileName = "AreaBoardPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过AreaBoardPackVisitor.SetABDefault单独设定AreaBoardPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过AreaBoardPackVisitor.SetABFallback单独设定AreaBoardPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawAreaBoardPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawAreaBoardPack>>(()=> new PBC::MapField<int, rawAreaBoardPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawAreaBoardPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().AreaBoardPackMap;
			var abMap = v.AreaBoardPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.AreaBoardPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].AreaBoardPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawAreaBoardPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().AreaBoardPackMap;
			if (v.AreaBoardPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.AreaBoardPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawAreaBoardPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(AreaBoardPackConf cc) {
			foreach(KeyValuePair <string, rawAreaBoardPackMapABValue> tagEntry in cc.AreaBoardPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.AreaBoardPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawAreaBoardPackMapABValue();
					cc.AreaBoardPackMapAB[tag] = ps;
				}
                var tagTmp = ps.AreaBoardPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawAreaBoardPack> entry in cc.AreaBoardPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawAreaBoardPack> entry in tagEntry.Value.AreaBoardPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.AreaBoardPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(AreaBoardPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.AreaBoardPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.AreaBoardPackMapAB != null)
			{
				foreach (var entry in cc.AreaBoardPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.AreaBoardPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.AreaBoardPackMap.MergeFrom(cis);
			Patch(updating.AreaBoardPackMap);
			updating.AreaBoardPackSortedKeys = genSortedKeys(updating.AreaBoardPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new AreaBoardPackConf();
            tmp.MergeFrom(cis);
			updating.AreaBoardPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().AreaBoardPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawAreaBoardPack> filter,string tag=""){
		...
		public static rawAreaBoardPack MustGet(int key,string tag="") {
		...
		public static rawAreaBoardPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawAreaBoardPack> GetByFilter(Func<rawAreaBoardPack, bool> filterFunc,string tag="") {
			List<rawAreaBoardPack> ret = new List<rawAreaBoardPack>();
			foreach(KeyValuePair <int, rawAreaBoardPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawAreaBoardPack? GetOneByFilter(Func<rawAreaBoardPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawAreaBoardPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawAreaBoardPack data) 
		{
			return data.Id;			
		}
    }
