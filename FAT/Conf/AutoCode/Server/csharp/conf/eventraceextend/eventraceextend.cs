...
using EventRaceExtendConf = fat.rawdata.EventRaceExtendConf;
using rawEventRaceExtend = fat.rawdata.EventRaceExtend;
using rawEventRaceExtendMapABValue = fat.rawdata.EventRaceExtendMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceExtendVisitor
    {
		...
		public const string FileName = "EventRaceExtendConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceExtendVisitor.SetABDefault单独设定EventRaceExtend的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceExtendVisitor.SetABFallback单独设定EventRaceExtend的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRaceExtend>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRaceExtend>>(()=> new PBC::MapField<int, rawEventRaceExtend>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRaceExtend>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceExtendMap;
			var abMap = v.EventRaceExtendMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceExtendMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceExtendMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRaceExtend> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceExtendMap;
			if (v.EventRaceExtendMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceExtendMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceExtendMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceExtendConf cc) {
			foreach(KeyValuePair <string, rawEventRaceExtendMapABValue> tagEntry in cc.EventRaceExtendMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceExtendMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceExtendMapABValue();
					cc.EventRaceExtendMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceExtendMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRaceExtend> entry in cc.EventRaceExtendMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRaceExtend> entry in tagEntry.Value.EventRaceExtendMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRaceExtend> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceExtendConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceExtendMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceExtendMapAB != null)
			{
				foreach (var entry in cc.EventRaceExtendMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceExtendMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceExtendMap.MergeFrom(cis);
			Patch(updating.EventRaceExtendMap);
			updating.EventRaceExtendSortedKeys = genSortedKeys(updating.EventRaceExtendMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceExtendConf();
            tmp.MergeFrom(cis);
			updating.EventRaceExtendMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceExtendSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRaceExtend> filter,string tag=""){
		...
		public static rawEventRaceExtend MustGet(int key,string tag="") {
		...
		public static rawEventRaceExtend? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRaceExtend> GetByFilter(Func<rawEventRaceExtend, bool> filterFunc,string tag="") {
			List<rawEventRaceExtend> ret = new List<rawEventRaceExtend>();
			foreach(KeyValuePair <int, rawEventRaceExtend> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRaceExtend? GetOneByFilter(Func<rawEventRaceExtend, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRaceExtend> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRaceExtend data) 
		{
			return data.Id;			
		}
    }
