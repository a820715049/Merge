...
using EventThemeConf = fat.rawdata.EventThemeConf;
using rawEventTheme = fat.rawdata.EventTheme;
using rawEventThemeMapABValue = fat.rawdata.EventThemeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventThemeVisitor
    {
		...
		public const string FileName = "EventThemeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventThemeVisitor.SetABDefault单独设定EventTheme的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventThemeVisitor.SetABFallback单独设定EventTheme的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTheme>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTheme>>(()=> new PBC::MapField<int, rawEventTheme>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTheme>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventThemeMap;
			var abMap = v.EventThemeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventThemeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventThemeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTheme> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventThemeMap;
			if (v.EventThemeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventThemeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventThemeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventThemeConf cc) {
			foreach(KeyValuePair <string, rawEventThemeMapABValue> tagEntry in cc.EventThemeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventThemeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventThemeMapABValue();
					cc.EventThemeMapAB[tag] = ps;
				}
                var tagTmp = ps.EventThemeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTheme> entry in cc.EventThemeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTheme> entry in tagEntry.Value.EventThemeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTheme> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventThemeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventThemeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventThemeMapAB != null)
			{
				foreach (var entry in cc.EventThemeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventThemeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventThemeMap.MergeFrom(cis);
			Patch(updating.EventThemeMap);
			updating.EventThemeSortedKeys = genSortedKeys(updating.EventThemeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventThemeConf();
            tmp.MergeFrom(cis);
			updating.EventThemeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventThemeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTheme> filter,string tag=""){
		...
		public static rawEventTheme MustGet(int key,string tag="") {
		...
		public static rawEventTheme? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTheme> GetByFilter(Func<rawEventTheme, bool> filterFunc,string tag="") {
			List<rawEventTheme> ret = new List<rawEventTheme>();
			foreach(KeyValuePair <int, rawEventTheme> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTheme? GetOneByFilter(Func<rawEventTheme, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTheme> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTheme data) 
		{
			return data.Id;			
		}
    }
