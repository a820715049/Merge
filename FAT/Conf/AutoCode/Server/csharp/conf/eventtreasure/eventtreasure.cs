...
using EventTreasureConf = fat.rawdata.EventTreasureConf;
using rawEventTreasure = fat.rawdata.EventTreasure;
using rawEventTreasureMapABValue = fat.rawdata.EventTreasureMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTreasureVisitor
    {
		...
		public const string FileName = "EventTreasureConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTreasureVisitor.SetABDefault单独设定EventTreasure的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTreasureVisitor.SetABFallback单独设定EventTreasure的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTreasure>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTreasure>>(()=> new PBC::MapField<int, rawEventTreasure>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTreasure>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTreasureMap;
			var abMap = v.EventTreasureMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTreasureMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTreasureMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTreasure> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTreasureMap;
			if (v.EventTreasureMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTreasureMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTreasureMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTreasureConf cc) {
			foreach(KeyValuePair <string, rawEventTreasureMapABValue> tagEntry in cc.EventTreasureMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTreasureMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTreasureMapABValue();
					cc.EventTreasureMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTreasureMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTreasure> entry in cc.EventTreasureMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTreasure> entry in tagEntry.Value.EventTreasureMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTreasure> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTreasureConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTreasureMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTreasureMapAB != null)
			{
				foreach (var entry in cc.EventTreasureMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTreasureMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTreasureMap.MergeFrom(cis);
			Patch(updating.EventTreasureMap);
			updating.EventTreasureSortedKeys = genSortedKeys(updating.EventTreasureMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTreasureConf();
            tmp.MergeFrom(cis);
			updating.EventTreasureMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTreasureSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTreasure> filter,string tag=""){
		...
		public static rawEventTreasure MustGet(int key,string tag="") {
		...
		public static rawEventTreasure? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTreasure> GetByFilter(Func<rawEventTreasure, bool> filterFunc,string tag="") {
			List<rawEventTreasure> ret = new List<rawEventTreasure>();
			foreach(KeyValuePair <int, rawEventTreasure> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTreasure? GetOneByFilter(Func<rawEventTreasure, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTreasure> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTreasure data) 
		{
			return data.Id;			
		}
    }
