...
using EventZeroQuestGroupConf = fat.rawdata.EventZeroQuestGroupConf;
using rawEventZeroQuestGroup = fat.rawdata.EventZeroQuestGroup;
using rawEventZeroQuestGroupMapABValue = fat.rawdata.EventZeroQuestGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventZeroQuestGroupVisitor
    {
		...
		public const string FileName = "EventZeroQuestGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventZeroQuestGroupVisitor.SetABDefault单独设定EventZeroQuestGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventZeroQuestGroupVisitor.SetABFallback单独设定EventZeroQuestGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventZeroQuestGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventZeroQuestGroup>>(()=> new PBC::MapField<int, rawEventZeroQuestGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventZeroQuestGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventZeroQuestGroupMap;
			var abMap = v.EventZeroQuestGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventZeroQuestGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventZeroQuestGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventZeroQuestGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventZeroQuestGroupMap;
			if (v.EventZeroQuestGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventZeroQuestGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventZeroQuestGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventZeroQuestGroupConf cc) {
			foreach(KeyValuePair <string, rawEventZeroQuestGroupMapABValue> tagEntry in cc.EventZeroQuestGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventZeroQuestGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventZeroQuestGroupMapABValue();
					cc.EventZeroQuestGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventZeroQuestGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventZeroQuestGroup> entry in cc.EventZeroQuestGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventZeroQuestGroup> entry in tagEntry.Value.EventZeroQuestGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventZeroQuestGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventZeroQuestGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventZeroQuestGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventZeroQuestGroupMapAB != null)
			{
				foreach (var entry in cc.EventZeroQuestGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventZeroQuestGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventZeroQuestGroupMap.MergeFrom(cis);
			Patch(updating.EventZeroQuestGroupMap);
			updating.EventZeroQuestGroupSortedKeys = genSortedKeys(updating.EventZeroQuestGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventZeroQuestGroupConf();
            tmp.MergeFrom(cis);
			updating.EventZeroQuestGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventZeroQuestGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventZeroQuestGroup> filter,string tag=""){
		...
		public static rawEventZeroQuestGroup MustGet(int key,string tag="") {
		...
		public static rawEventZeroQuestGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventZeroQuestGroup> GetByFilter(Func<rawEventZeroQuestGroup, bool> filterFunc,string tag="") {
			List<rawEventZeroQuestGroup> ret = new List<rawEventZeroQuestGroup>();
			foreach(KeyValuePair <int, rawEventZeroQuestGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventZeroQuestGroup? GetOneByFilter(Func<rawEventZeroQuestGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventZeroQuestGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventZeroQuestGroup data) 
		{
			return data.Id;			
		}
    }
