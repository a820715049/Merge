...
using MultiRankRobotDetailConf = fat.rawdata.MultiRankRobotDetailConf;
using rawMultiRankRobotDetail = fat.rawdata.MultiRankRobotDetail;
using rawMultiRankRobotDetailMapABValue = fat.rawdata.MultiRankRobotDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MultiRankRobotDetailVisitor
    {
		...
		public const string FileName = "MultiRankRobotDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MultiRankRobotDetailVisitor.SetABDefault单独设定MultiRankRobotDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MultiRankRobotDetailVisitor.SetABFallback单独设定MultiRankRobotDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMultiRankRobotDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMultiRankRobotDetail>>(()=> new PBC::MapField<int, rawMultiRankRobotDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMultiRankRobotDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MultiRankRobotDetailMap;
			var abMap = v.MultiRankRobotDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MultiRankRobotDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MultiRankRobotDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMultiRankRobotDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MultiRankRobotDetailMap;
			if (v.MultiRankRobotDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MultiRankRobotDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMultiRankRobotDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MultiRankRobotDetailConf cc) {
			foreach(KeyValuePair <string, rawMultiRankRobotDetailMapABValue> tagEntry in cc.MultiRankRobotDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MultiRankRobotDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMultiRankRobotDetailMapABValue();
					cc.MultiRankRobotDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.MultiRankRobotDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMultiRankRobotDetail> entry in cc.MultiRankRobotDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMultiRankRobotDetail> entry in tagEntry.Value.MultiRankRobotDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MultiRankRobotDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MultiRankRobotDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MultiRankRobotDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MultiRankRobotDetailMapAB != null)
			{
				foreach (var entry in cc.MultiRankRobotDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MultiRankRobotDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MultiRankRobotDetailMap.MergeFrom(cis);
			Patch(updating.MultiRankRobotDetailMap);
			updating.MultiRankRobotDetailSortedKeys = genSortedKeys(updating.MultiRankRobotDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MultiRankRobotDetailConf();
            tmp.MergeFrom(cis);
			updating.MultiRankRobotDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MultiRankRobotDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMultiRankRobotDetail> filter,string tag=""){
		...
		public static rawMultiRankRobotDetail MustGet(int key,string tag="") {
		...
		public static rawMultiRankRobotDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMultiRankRobotDetail> GetByFilter(Func<rawMultiRankRobotDetail, bool> filterFunc,string tag="") {
			List<rawMultiRankRobotDetail> ret = new List<rawMultiRankRobotDetail>();
			foreach(KeyValuePair <int, rawMultiRankRobotDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMultiRankRobotDetail? GetOneByFilter(Func<rawMultiRankRobotDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMultiRankRobotDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMultiRankRobotDetail data) 
		{
			return data.Id;			
		}
    }
