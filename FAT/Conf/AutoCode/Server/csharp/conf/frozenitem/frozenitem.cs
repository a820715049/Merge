...
using FrozenItemConf = fat.rawdata.FrozenItemConf;
using rawFrozenItem = fat.rawdata.FrozenItem;
using rawFrozenItemMapABValue = fat.rawdata.FrozenItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class FrozenItemVisitor
    {
		...
		public const string FileName = "FrozenItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过FrozenItemVisitor.SetABDefault单独设定FrozenItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过FrozenItemVisitor.SetABFallback单独设定FrozenItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawFrozenItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawFrozenItem>>(()=> new PBC::MapField<int, rawFrozenItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawFrozenItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().FrozenItemMap;
			var abMap = v.FrozenItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.FrozenItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].FrozenItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawFrozenItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().FrozenItemMap;
			if (v.FrozenItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.FrozenItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawFrozenItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(FrozenItemConf cc) {
			foreach(KeyValuePair <string, rawFrozenItemMapABValue> tagEntry in cc.FrozenItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.FrozenItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawFrozenItemMapABValue();
					cc.FrozenItemMapAB[tag] = ps;
				}
                var tagTmp = ps.FrozenItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawFrozenItem> entry in cc.FrozenItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawFrozenItem> entry in tagEntry.Value.FrozenItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.FrozenItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(FrozenItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.FrozenItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.FrozenItemMapAB != null)
			{
				foreach (var entry in cc.FrozenItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.FrozenItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.FrozenItemMap.MergeFrom(cis);
			Patch(updating.FrozenItemMap);
			updating.FrozenItemSortedKeys = genSortedKeys(updating.FrozenItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new FrozenItemConf();
            tmp.MergeFrom(cis);
			updating.FrozenItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().FrozenItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawFrozenItem> filter,string tag=""){
		...
		public static rawFrozenItem MustGet(int key,string tag="") {
		...
		public static rawFrozenItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawFrozenItem> GetByFilter(Func<rawFrozenItem, bool> filterFunc,string tag="") {
			List<rawFrozenItem> ret = new List<rawFrozenItem>();
			foreach(KeyValuePair <int, rawFrozenItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawFrozenItem? GetOneByFilter(Func<rawFrozenItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawFrozenItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawFrozenItem data) 
		{
			return data.Id;			
		}
    }
