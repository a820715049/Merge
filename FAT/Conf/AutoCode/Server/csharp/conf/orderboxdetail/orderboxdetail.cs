...
using OrderBoxDetailConf = fat.rawdata.OrderBoxDetailConf;
using rawOrderBoxDetail = fat.rawdata.OrderBoxDetail;
using rawOrderBoxDetailMapABValue = fat.rawdata.OrderBoxDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderBoxDetailVisitor
    {
		...
		public const string FileName = "OrderBoxDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderBoxDetailVisitor.SetABDefault单独设定OrderBoxDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderBoxDetailVisitor.SetABFallback单独设定OrderBoxDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderBoxDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderBoxDetail>>(()=> new PBC::MapField<int, rawOrderBoxDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderBoxDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderBoxDetailMap;
			var abMap = v.OrderBoxDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderBoxDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderBoxDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderBoxDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderBoxDetailMap;
			if (v.OrderBoxDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderBoxDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderBoxDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderBoxDetailConf cc) {
			foreach(KeyValuePair <string, rawOrderBoxDetailMapABValue> tagEntry in cc.OrderBoxDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderBoxDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderBoxDetailMapABValue();
					cc.OrderBoxDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderBoxDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderBoxDetail> entry in cc.OrderBoxDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderBoxDetail> entry in tagEntry.Value.OrderBoxDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderBoxDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderBoxDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderBoxDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderBoxDetailMapAB != null)
			{
				foreach (var entry in cc.OrderBoxDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderBoxDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderBoxDetailMap.MergeFrom(cis);
			Patch(updating.OrderBoxDetailMap);
			updating.OrderBoxDetailSortedKeys = genSortedKeys(updating.OrderBoxDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderBoxDetailConf();
            tmp.MergeFrom(cis);
			updating.OrderBoxDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderBoxDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderBoxDetail> filter,string tag=""){
		...
		public static rawOrderBoxDetail MustGet(int key,string tag="") {
		...
		public static rawOrderBoxDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderBoxDetail> GetByFilter(Func<rawOrderBoxDetail, bool> filterFunc,string tag="") {
			List<rawOrderBoxDetail> ret = new List<rawOrderBoxDetail>();
			foreach(KeyValuePair <int, rawOrderBoxDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderBoxDetail? GetOneByFilter(Func<rawOrderBoxDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderBoxDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderBoxDetail data) 
		{
			return data.Id;			
		}
    }
