...
using EventStepConf = fat.rawdata.EventStepConf;
using rawEventStep = fat.rawdata.EventStep;
using rawEventStepMapABValue = fat.rawdata.EventStepMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventStepVisitor
    {
		...
		public const string FileName = "EventStepConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventStepVisitor.SetABDefault单独设定EventStep的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventStepVisitor.SetABFallback单独设定EventStep的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventStep>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventStep>>(()=> new PBC::MapField<int, rawEventStep>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventStep>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventStepMap;
			var abMap = v.EventStepMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventStepMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventStepMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventStep> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventStepMap;
			if (v.EventStepMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventStepMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventStepMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventStepConf cc) {
			foreach(KeyValuePair <string, rawEventStepMapABValue> tagEntry in cc.EventStepMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventStepMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventStepMapABValue();
					cc.EventStepMapAB[tag] = ps;
				}
                var tagTmp = ps.EventStepMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventStep> entry in cc.EventStepMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventStep> entry in tagEntry.Value.EventStepMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventStep> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventStepConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventStepMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventStepMapAB != null)
			{
				foreach (var entry in cc.EventStepMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventStepMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventStepMap.MergeFrom(cis);
			Patch(updating.EventStepMap);
			updating.EventStepSortedKeys = genSortedKeys(updating.EventStepMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventStepConf();
            tmp.MergeFrom(cis);
			updating.EventStepMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventStepSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventStep> filter,string tag=""){
		...
		public static rawEventStep MustGet(int key,string tag="") {
		...
		public static rawEventStep? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventStep> GetByFilter(Func<rawEventStep, bool> filterFunc,string tag="") {
			List<rawEventStep> ret = new List<rawEventStep>();
			foreach(KeyValuePair <int, rawEventStep> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventStep? GetOneByFilter(Func<rawEventStep, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventStep> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventStep data) 
		{
			return data.Id;			
		}
    }
