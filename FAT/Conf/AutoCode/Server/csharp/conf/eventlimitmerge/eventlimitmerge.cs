...
using EventLimitMergeConf = fat.rawdata.EventLimitMergeConf;
using rawEventLimitMerge = fat.rawdata.EventLimitMerge;
using rawEventLimitMergeMapABValue = fat.rawdata.EventLimitMergeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventLimitMergeVisitor
    {
		...
		public const string FileName = "EventLimitMergeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventLimitMergeVisitor.SetABDefault单独设定EventLimitMerge的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventLimitMergeVisitor.SetABFallback单独设定EventLimitMerge的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventLimitMerge>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventLimitMerge>>(()=> new PBC::MapField<int, rawEventLimitMerge>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventLimitMerge>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventLimitMergeMap;
			var abMap = v.EventLimitMergeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventLimitMergeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventLimitMergeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventLimitMerge> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventLimitMergeMap;
			if (v.EventLimitMergeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventLimitMergeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventLimitMergeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventLimitMergeConf cc) {
			foreach(KeyValuePair <string, rawEventLimitMergeMapABValue> tagEntry in cc.EventLimitMergeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventLimitMergeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventLimitMergeMapABValue();
					cc.EventLimitMergeMapAB[tag] = ps;
				}
                var tagTmp = ps.EventLimitMergeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventLimitMerge> entry in cc.EventLimitMergeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventLimitMerge> entry in tagEntry.Value.EventLimitMergeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventLimitMerge> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventLimitMergeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventLimitMergeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventLimitMergeMapAB != null)
			{
				foreach (var entry in cc.EventLimitMergeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventLimitMergeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventLimitMergeMap.MergeFrom(cis);
			Patch(updating.EventLimitMergeMap);
			updating.EventLimitMergeSortedKeys = genSortedKeys(updating.EventLimitMergeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventLimitMergeConf();
            tmp.MergeFrom(cis);
			updating.EventLimitMergeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventLimitMergeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventLimitMerge> filter,string tag=""){
		...
		public static rawEventLimitMerge MustGet(int key,string tag="") {
		...
		public static rawEventLimitMerge? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventLimitMerge> GetByFilter(Func<rawEventLimitMerge, bool> filterFunc,string tag="") {
			List<rawEventLimitMerge> ret = new List<rawEventLimitMerge>();
			foreach(KeyValuePair <int, rawEventLimitMerge> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventLimitMerge? GetOneByFilter(Func<rawEventLimitMerge, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventLimitMerge> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventLimitMerge data) 
		{
			return data.Id;			
		}
    }
