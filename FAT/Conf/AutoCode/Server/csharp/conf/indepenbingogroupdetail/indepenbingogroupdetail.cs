...
using IndepenBingoGroupDetailConf = fat.rawdata.IndepenBingoGroupDetailConf;
using rawIndepenBingoGroupDetail = fat.rawdata.IndepenBingoGroupDetail;
using rawIndepenBingoGroupDetailMapABValue = fat.rawdata.IndepenBingoGroupDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class IndepenBingoGroupDetailVisitor
    {
		...
		public const string FileName = "IndepenBingoGroupDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过IndepenBingoGroupDetailVisitor.SetABDefault单独设定IndepenBingoGroupDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过IndepenBingoGroupDetailVisitor.SetABFallback单独设定IndepenBingoGroupDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawIndepenBingoGroupDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawIndepenBingoGroupDetail>>(()=> new PBC::MapField<int, rawIndepenBingoGroupDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawIndepenBingoGroupDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().IndepenBingoGroupDetailMap;
			var abMap = v.IndepenBingoGroupDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.IndepenBingoGroupDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].IndepenBingoGroupDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawIndepenBingoGroupDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().IndepenBingoGroupDetailMap;
			if (v.IndepenBingoGroupDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.IndepenBingoGroupDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawIndepenBingoGroupDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(IndepenBingoGroupDetailConf cc) {
			foreach(KeyValuePair <string, rawIndepenBingoGroupDetailMapABValue> tagEntry in cc.IndepenBingoGroupDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.IndepenBingoGroupDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawIndepenBingoGroupDetailMapABValue();
					cc.IndepenBingoGroupDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.IndepenBingoGroupDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawIndepenBingoGroupDetail> entry in cc.IndepenBingoGroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawIndepenBingoGroupDetail> entry in tagEntry.Value.IndepenBingoGroupDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.IndepenBingoGroupDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(IndepenBingoGroupDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.IndepenBingoGroupDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.IndepenBingoGroupDetailMapAB != null)
			{
				foreach (var entry in cc.IndepenBingoGroupDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.IndepenBingoGroupDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.IndepenBingoGroupDetailMap.MergeFrom(cis);
			Patch(updating.IndepenBingoGroupDetailMap);
			updating.IndepenBingoGroupDetailSortedKeys = genSortedKeys(updating.IndepenBingoGroupDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new IndepenBingoGroupDetailConf();
            tmp.MergeFrom(cis);
			updating.IndepenBingoGroupDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().IndepenBingoGroupDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawIndepenBingoGroupDetail> filter,string tag=""){
		...
		public static rawIndepenBingoGroupDetail MustGet(int key,string tag="") {
		...
		public static rawIndepenBingoGroupDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawIndepenBingoGroupDetail> GetByFilter(Func<rawIndepenBingoGroupDetail, bool> filterFunc,string tag="") {
			List<rawIndepenBingoGroupDetail> ret = new List<rawIndepenBingoGroupDetail>();
			foreach(KeyValuePair <int, rawIndepenBingoGroupDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawIndepenBingoGroupDetail? GetOneByFilter(Func<rawIndepenBingoGroupDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawIndepenBingoGroupDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawIndepenBingoGroupDetail data) 
		{
			return data.Id;			
		}
    }
