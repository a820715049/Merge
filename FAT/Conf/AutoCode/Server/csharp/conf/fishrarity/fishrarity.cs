...
using FishRarityConf = fat.rawdata.FishRarityConf;
using rawFishRarity = fat.rawdata.FishRarity;
using rawFishRarityMapABValue = fat.rawdata.FishRarityMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class FishRarityVisitor
    {
		...
		public const string FileName = "FishRarityConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过FishRarityVisitor.SetABDefault单独设定FishRarity的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过FishRarityVisitor.SetABFallback单独设定FishRarity的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawFishRarity>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawFishRarity>>(()=> new PBC::MapField<int, rawFishRarity>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawFishRarity>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().FishRarityMap;
			var abMap = v.FishRarityMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.FishRarityMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].FishRarityMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawFishRarity> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().FishRarityMap;
			if (v.FishRarityMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.FishRarityMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawFishRarityMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(FishRarityConf cc) {
			foreach(KeyValuePair <string, rawFishRarityMapABValue> tagEntry in cc.FishRarityMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.FishRarityMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawFishRarityMapABValue();
					cc.FishRarityMapAB[tag] = ps;
				}
                var tagTmp = ps.FishRarityMap;
				// 基础数据
				foreach(KeyValuePair <int, rawFishRarity> entry in cc.FishRarityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawFishRarity> entry in tagEntry.Value.FishRarityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.FishRarity> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(FishRarityConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.FishRarityMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.FishRarityMapAB != null)
			{
				foreach (var entry in cc.FishRarityMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.FishRarityMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.FishRarityMap.MergeFrom(cis);
			Patch(updating.FishRarityMap);
			updating.FishRaritySortedKeys = genSortedKeys(updating.FishRarityMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new FishRarityConf();
            tmp.MergeFrom(cis);
			updating.FishRarityMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().FishRaritySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawFishRarity> filter,string tag=""){
		...
		public static rawFishRarity MustGet(int key,string tag="") {
		...
		public static rawFishRarity? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawFishRarity> GetByFilter(Func<rawFishRarity, bool> filterFunc,string tag="") {
			List<rawFishRarity> ret = new List<rawFishRarity>();
			foreach(KeyValuePair <int, rawFishRarity> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawFishRarity? GetOneByFilter(Func<rawFishRarity, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawFishRarity> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawFishRarity data) 
		{
			return data.Id;			
		}
    }
