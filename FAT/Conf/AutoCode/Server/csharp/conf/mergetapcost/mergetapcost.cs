...
using MergeTapCostConf = fat.rawdata.MergeTapCostConf;
using rawMergeTapCost = fat.rawdata.MergeTapCost;
using rawMergeTapCostMapABValue = fat.rawdata.MergeTapCostMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeTapCostVisitor
    {
		...
		public const string FileName = "MergeTapCostConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeTapCostVisitor.SetABDefault单独设定MergeTapCost的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeTapCostVisitor.SetABFallback单独设定MergeTapCost的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeTapCost>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeTapCost>>(()=> new PBC::MapField<int, rawMergeTapCost>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeTapCost>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeTapCostMap;
			var abMap = v.MergeTapCostMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeTapCostMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeTapCostMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeTapCost> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeTapCostMap;
			if (v.MergeTapCostMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeTapCostMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeTapCostMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeTapCostConf cc) {
			foreach(KeyValuePair <string, rawMergeTapCostMapABValue> tagEntry in cc.MergeTapCostMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeTapCostMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeTapCostMapABValue();
					cc.MergeTapCostMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeTapCostMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeTapCost> entry in cc.MergeTapCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeTapCost> entry in tagEntry.Value.MergeTapCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeTapCost> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeTapCostConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeTapCostMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeTapCostMapAB != null)
			{
				foreach (var entry in cc.MergeTapCostMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeTapCostMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeTapCostMap.MergeFrom(cis);
			Patch(updating.MergeTapCostMap);
			updating.MergeTapCostSortedKeys = genSortedKeys(updating.MergeTapCostMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeTapCostConf();
            tmp.MergeFrom(cis);
			updating.MergeTapCostMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeTapCostSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeTapCost> filter,string tag=""){
		...
		public static rawMergeTapCost MustGet(int key,string tag="") {
		...
		public static rawMergeTapCost? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeTapCost> GetByFilter(Func<rawMergeTapCost, bool> filterFunc,string tag="") {
			List<rawMergeTapCost> ret = new List<rawMergeTapCost>();
			foreach(KeyValuePair <int, rawMergeTapCost> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeTapCost? GetOneByFilter(Func<rawMergeTapCost, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeTapCost> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeTapCost data) 
		{
			return data.Id;			
		}
    }
