...
using EventPuzzleDetaileConf = fat.rawdata.EventPuzzleDetaileConf;
using rawEventPuzzleDetaile = fat.rawdata.EventPuzzleDetaile;
using rawEventPuzzleDetaileMapABValue = fat.rawdata.EventPuzzleDetaileMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPuzzleDetaileVisitor
    {
		...
		public const string FileName = "EventPuzzleDetaileConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPuzzleDetaileVisitor.SetABDefault单独设定EventPuzzleDetaile的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPuzzleDetaileVisitor.SetABFallback单独设定EventPuzzleDetaile的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPuzzleDetaile>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPuzzleDetaile>>(()=> new PBC::MapField<int, rawEventPuzzleDetaile>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPuzzleDetaile>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPuzzleDetaileMap;
			var abMap = v.EventPuzzleDetaileMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPuzzleDetaileMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPuzzleDetaileMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPuzzleDetaile> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPuzzleDetaileMap;
			if (v.EventPuzzleDetaileMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPuzzleDetaileMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPuzzleDetaileMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPuzzleDetaileConf cc) {
			foreach(KeyValuePair <string, rawEventPuzzleDetaileMapABValue> tagEntry in cc.EventPuzzleDetaileMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPuzzleDetaileMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPuzzleDetaileMapABValue();
					cc.EventPuzzleDetaileMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPuzzleDetaileMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPuzzleDetaile> entry in cc.EventPuzzleDetaileMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPuzzleDetaile> entry in tagEntry.Value.EventPuzzleDetaileMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPuzzleDetaile> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPuzzleDetaileConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPuzzleDetaileMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPuzzleDetaileMapAB != null)
			{
				foreach (var entry in cc.EventPuzzleDetaileMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPuzzleDetaileMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPuzzleDetaileMap.MergeFrom(cis);
			Patch(updating.EventPuzzleDetaileMap);
			updating.EventPuzzleDetaileSortedKeys = genSortedKeys(updating.EventPuzzleDetaileMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPuzzleDetaileConf();
            tmp.MergeFrom(cis);
			updating.EventPuzzleDetaileMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPuzzleDetaileSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPuzzleDetaile> filter,string tag=""){
		...
		public static rawEventPuzzleDetaile MustGet(int key,string tag="") {
		...
		public static rawEventPuzzleDetaile? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPuzzleDetaile> GetByFilter(Func<rawEventPuzzleDetaile, bool> filterFunc,string tag="") {
			List<rawEventPuzzleDetaile> ret = new List<rawEventPuzzleDetaile>();
			foreach(KeyValuePair <int, rawEventPuzzleDetaile> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPuzzleDetaile? GetOneByFilter(Func<rawEventPuzzleDetaile, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPuzzleDetaile> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPuzzleDetaile data) 
		{
			return data.Id;			
		}
    }
