...
using GradeIndexMappingConf = fat.rawdata.GradeIndexMappingConf;
using rawGradeIndexMapping = fat.rawdata.GradeIndexMapping;
using rawGradeIndexMappingMapABValue = fat.rawdata.GradeIndexMappingMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GradeIndexMappingVisitor
    {
		...
		public const string FileName = "GradeIndexMappingConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GradeIndexMappingVisitor.SetABDefault单独设定GradeIndexMapping的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GradeIndexMappingVisitor.SetABFallback单独设定GradeIndexMapping的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGradeIndexMapping>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGradeIndexMapping>>(()=> new PBC::MapField<int, rawGradeIndexMapping>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGradeIndexMapping>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GradeIndexMappingMap;
			var abMap = v.GradeIndexMappingMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GradeIndexMappingMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GradeIndexMappingMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGradeIndexMapping> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GradeIndexMappingMap;
			if (v.GradeIndexMappingMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GradeIndexMappingMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGradeIndexMappingMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GradeIndexMappingConf cc) {
			foreach(KeyValuePair <string, rawGradeIndexMappingMapABValue> tagEntry in cc.GradeIndexMappingMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GradeIndexMappingMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGradeIndexMappingMapABValue();
					cc.GradeIndexMappingMapAB[tag] = ps;
				}
                var tagTmp = ps.GradeIndexMappingMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGradeIndexMapping> entry in cc.GradeIndexMappingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGradeIndexMapping> entry in tagEntry.Value.GradeIndexMappingMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GradeIndexMapping> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GradeIndexMappingConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GradeIndexMappingMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GradeIndexMappingMapAB != null)
			{
				foreach (var entry in cc.GradeIndexMappingMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GradeIndexMappingMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GradeIndexMappingMap.MergeFrom(cis);
			Patch(updating.GradeIndexMappingMap);
			updating.GradeIndexMappingSortedKeys = genSortedKeys(updating.GradeIndexMappingMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GradeIndexMappingConf();
            tmp.MergeFrom(cis);
			updating.GradeIndexMappingMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GradeIndexMappingSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGradeIndexMapping> filter,string tag=""){
		...
		public static rawGradeIndexMapping MustGet(int key,string tag="") {
		...
		public static rawGradeIndexMapping? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGradeIndexMapping> GetByFilter(Func<rawGradeIndexMapping, bool> filterFunc,string tag="") {
			List<rawGradeIndexMapping> ret = new List<rawGradeIndexMapping>();
			foreach(KeyValuePair <int, rawGradeIndexMapping> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGradeIndexMapping? GetOneByFilter(Func<rawGradeIndexMapping, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGradeIndexMapping> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGradeIndexMapping data) 
		{
			return data.Id;			
		}
    }
