...
using MonsterConf = fat.rawdata.MonsterConf;
using rawMonster = fat.rawdata.Monster;
using rawMonsterMapABValue = fat.rawdata.MonsterMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MonsterVisitor
    {
		...
		public const string FileName = "MonsterConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MonsterVisitor.SetABDefault单独设定Monster的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MonsterVisitor.SetABFallback单独设定Monster的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMonster>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMonster>>(()=> new PBC::MapField<int, rawMonster>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMonster>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MonsterMap;
			var abMap = v.MonsterMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MonsterMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MonsterMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMonster> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MonsterMap;
			if (v.MonsterMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MonsterMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMonsterMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MonsterConf cc) {
			foreach(KeyValuePair <string, rawMonsterMapABValue> tagEntry in cc.MonsterMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MonsterMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMonsterMapABValue();
					cc.MonsterMapAB[tag] = ps;
				}
                var tagTmp = ps.MonsterMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMonster> entry in cc.MonsterMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMonster> entry in tagEntry.Value.MonsterMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Monster> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MonsterConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MonsterMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MonsterMapAB != null)
			{
				foreach (var entry in cc.MonsterMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MonsterMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MonsterMap.MergeFrom(cis);
			Patch(updating.MonsterMap);
			updating.MonsterSortedKeys = genSortedKeys(updating.MonsterMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MonsterConf();
            tmp.MergeFrom(cis);
			updating.MonsterMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MonsterSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMonster> filter,string tag=""){
		...
		public static rawMonster MustGet(int key,string tag="") {
		...
		public static rawMonster? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMonster> GetByFilter(Func<rawMonster, bool> filterFunc,string tag="") {
			List<rawMonster> ret = new List<rawMonster>();
			foreach(KeyValuePair <int, rawMonster> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMonster? GetOneByFilter(Func<rawMonster, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMonster> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMonster data) 
		{
			return data.Id;			
		}
    }
