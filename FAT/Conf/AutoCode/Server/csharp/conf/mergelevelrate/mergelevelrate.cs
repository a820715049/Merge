...
using MergeLevelRateConf = fat.rawdata.MergeLevelRateConf;
using rawMergeLevelRate = fat.rawdata.MergeLevelRate;
using rawMergeLevelRateMapABValue = fat.rawdata.MergeLevelRateMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeLevelRateVisitor
    {
		...
		public const string FileName = "MergeLevelRateConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeLevelRateVisitor.SetABDefault单独设定MergeLevelRate的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeLevelRateVisitor.SetABFallback单独设定MergeLevelRate的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeLevelRate>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeLevelRate>>(()=> new PBC::MapField<int, rawMergeLevelRate>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeLevelRate>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeLevelRateMap;
			var abMap = v.MergeLevelRateMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeLevelRateMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeLevelRateMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeLevelRate> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeLevelRateMap;
			if (v.MergeLevelRateMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeLevelRateMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeLevelRateMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeLevelRateConf cc) {
			foreach(KeyValuePair <string, rawMergeLevelRateMapABValue> tagEntry in cc.MergeLevelRateMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeLevelRateMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeLevelRateMapABValue();
					cc.MergeLevelRateMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeLevelRateMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeLevelRate> entry in cc.MergeLevelRateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeLevelRate> entry in tagEntry.Value.MergeLevelRateMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeLevelRate> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeLevelRateConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeLevelRateMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeLevelRateMapAB != null)
			{
				foreach (var entry in cc.MergeLevelRateMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeLevelRateMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeLevelRateMap.MergeFrom(cis);
			Patch(updating.MergeLevelRateMap);
			updating.MergeLevelRateSortedKeys = genSortedKeys(updating.MergeLevelRateMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeLevelRateConf();
            tmp.MergeFrom(cis);
			updating.MergeLevelRateMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeLevelRateSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeLevelRate> filter,string tag=""){
		...
		public static rawMergeLevelRate MustGet(int key,string tag="") {
		...
		public static rawMergeLevelRate? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeLevelRate> GetByFilter(Func<rawMergeLevelRate, bool> filterFunc,string tag="") {
			List<rawMergeLevelRate> ret = new List<rawMergeLevelRate>();
			foreach(KeyValuePair <int, rawMergeLevelRate> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeLevelRate? GetOneByFilter(Func<rawMergeLevelRate, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeLevelRate> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeLevelRate data) 
		{
			return data.Id;			
		}
    }
