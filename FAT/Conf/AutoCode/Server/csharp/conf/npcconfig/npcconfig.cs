...
using NpcConfigConf = fat.rawdata.NpcConfigConf;
using rawNpcConfig = fat.rawdata.NpcConfig;
using rawNpcConfigMapABValue = fat.rawdata.NpcConfigMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class NpcConfigVisitor
    {
		...
		public const string FileName = "NpcConfigConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过NpcConfigVisitor.SetABDefault单独设定NpcConfig的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过NpcConfigVisitor.SetABFallback单独设定NpcConfig的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawNpcConfig>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawNpcConfig>>(()=> new PBC::MapField<int, rawNpcConfig>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawNpcConfig>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().NpcConfigMap;
			var abMap = v.NpcConfigMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.NpcConfigMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].NpcConfigMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawNpcConfig> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().NpcConfigMap;
			if (v.NpcConfigMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.NpcConfigMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawNpcConfigMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(NpcConfigConf cc) {
			foreach(KeyValuePair <string, rawNpcConfigMapABValue> tagEntry in cc.NpcConfigMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.NpcConfigMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawNpcConfigMapABValue();
					cc.NpcConfigMapAB[tag] = ps;
				}
                var tagTmp = ps.NpcConfigMap;
				// 基础数据
				foreach(KeyValuePair <int, rawNpcConfig> entry in cc.NpcConfigMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawNpcConfig> entry in tagEntry.Value.NpcConfigMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.NpcConfig> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(NpcConfigConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.NpcConfigMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.NpcConfigMapAB != null)
			{
				foreach (var entry in cc.NpcConfigMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.NpcConfigMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.NpcConfigMap.MergeFrom(cis);
			Patch(updating.NpcConfigMap);
			updating.NpcConfigSortedKeys = genSortedKeys(updating.NpcConfigMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new NpcConfigConf();
            tmp.MergeFrom(cis);
			updating.NpcConfigMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().NpcConfigSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawNpcConfig> filter,string tag=""){
		...
		public static rawNpcConfig MustGet(int key,string tag="") {
		...
		public static rawNpcConfig? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawNpcConfig> GetByFilter(Func<rawNpcConfig, bool> filterFunc,string tag="") {
			List<rawNpcConfig> ret = new List<rawNpcConfig>();
			foreach(KeyValuePair <int, rawNpcConfig> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawNpcConfig? GetOneByFilter(Func<rawNpcConfig, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawNpcConfig> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawNpcConfig data) 
		{
			return data.Id;			
		}
    }
