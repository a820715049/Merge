...
using EventMineCartRewardConf = fat.rawdata.EventMineCartRewardConf;
using rawEventMineCartReward = fat.rawdata.EventMineCartReward;
using rawEventMineCartRewardMapABValue = fat.rawdata.EventMineCartRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartRewardVisitor
    {
		...
		public const string FileName = "EventMineCartRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartRewardVisitor.SetABDefault单独设定EventMineCartReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartRewardVisitor.SetABFallback单独设定EventMineCartReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartReward>>(()=> new PBC::MapField<int, rawEventMineCartReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartRewardMap;
			var abMap = v.EventMineCartRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartRewardMap;
			if (v.EventMineCartRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartRewardConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartRewardMapABValue> tagEntry in cc.EventMineCartRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartRewardMapABValue();
					cc.EventMineCartRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartReward> entry in cc.EventMineCartRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartReward> entry in tagEntry.Value.EventMineCartRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartRewardMapAB != null)
			{
				foreach (var entry in cc.EventMineCartRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartRewardMap.MergeFrom(cis);
			Patch(updating.EventMineCartRewardMap);
			updating.EventMineCartRewardSortedKeys = genSortedKeys(updating.EventMineCartRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartRewardConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartReward> filter,string tag=""){
		...
		public static rawEventMineCartReward MustGet(int key,string tag="") {
		...
		public static rawEventMineCartReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartReward> GetByFilter(Func<rawEventMineCartReward, bool> filterFunc,string tag="") {
			List<rawEventMineCartReward> ret = new List<rawEventMineCartReward>();
			foreach(KeyValuePair <int, rawEventMineCartReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartReward? GetOneByFilter(Func<rawEventMineCartReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartReward data) 
		{
			return data.Id;			
		}
    }
