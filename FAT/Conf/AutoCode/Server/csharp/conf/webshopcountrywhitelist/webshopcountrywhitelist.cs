...
using WebshopCountryWhitelistConf = fat.rawdata.WebshopCountryWhitelistConf;
using rawWebshopCountryWhitelist = fat.rawdata.WebshopCountryWhitelist;
using rawWebshopCountryWhitelistMapABValue = fat.rawdata.WebshopCountryWhitelistMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class WebshopCountryWhitelistVisitor
    {
		...
		public const string FileName = "WebshopCountryWhitelistConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过WebshopCountryWhitelistVisitor.SetABDefault单独设定WebshopCountryWhitelist的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过WebshopCountryWhitelistVisitor.SetABFallback单独设定WebshopCountryWhitelist的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<string, rawWebshopCountryWhitelist>> emptyAllReadonly = new Lazy<PBC::MapField<string, rawWebshopCountryWhitelist>>(()=> new PBC::MapField<string, rawWebshopCountryWhitelist>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<string, rawWebshopCountryWhitelist>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().WebshopCountryWhitelistMap;
			var abMap = v.WebshopCountryWhitelistMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.WebshopCountryWhitelistMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].WebshopCountryWhitelistMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<string, rawWebshopCountryWhitelist> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().WebshopCountryWhitelistMap;
			if (v.WebshopCountryWhitelistMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.WebshopCountryWhitelistMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawWebshopCountryWhitelistMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(WebshopCountryWhitelistConf cc) {
			foreach(KeyValuePair <string, rawWebshopCountryWhitelistMapABValue> tagEntry in cc.WebshopCountryWhitelistMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.WebshopCountryWhitelistMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawWebshopCountryWhitelistMapABValue();
					cc.WebshopCountryWhitelistMapAB[tag] = ps;
				}
                var tagTmp = ps.WebshopCountryWhitelistMap;
				// 基础数据
				foreach(KeyValuePair <string, rawWebshopCountryWhitelist> entry in cc.WebshopCountryWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <string, rawWebshopCountryWhitelist> entry in tagEntry.Value.WebshopCountryWhitelistMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<string,fat.rawdata.WebshopCountryWhitelist> tableData)
		{
		}
		...
		private static Dictionary<string, List<string>> genSortedKeys(WebshopCountryWhitelistConf cc) 
		{
			var ret = new Dictionary<string, List<string>>();
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				listFoundation = new List<string>();
				listFoundation.AddRange(cc.WebshopCountryWhitelistMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.WebshopCountryWhitelistMapAB != null)
			{
				foreach (var entry in cc.WebshopCountryWhitelistMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<string>();
						list.AddRange(entry.Value.WebshopCountryWhitelistMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.WebshopCountryWhitelistMap.MergeFrom(cis);
			Patch(updating.WebshopCountryWhitelistMap);
			updating.WebshopCountryWhitelistSortedKeys = genSortedKeys(updating.WebshopCountryWhitelistMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new WebshopCountryWhitelistConf();
            tmp.MergeFrom(cis);
			updating.WebshopCountryWhitelistMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().WebshopCountryWhitelistSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<string, rawWebshopCountryWhitelist> filter,string tag=""){
		...
		public static rawWebshopCountryWhitelist MustGet(string key,string tag="") {
		...
		public static rawWebshopCountryWhitelist? Get(string key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawWebshopCountryWhitelist> GetByFilter(Func<rawWebshopCountryWhitelist, bool> filterFunc,string tag="") {
			List<rawWebshopCountryWhitelist> ret = new List<rawWebshopCountryWhitelist>();
			foreach(KeyValuePair <string, rawWebshopCountryWhitelist> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawWebshopCountryWhitelist? GetOneByFilter(Func<rawWebshopCountryWhitelist, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <string, rawWebshopCountryWhitelist> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public string MapKey(rawWebshopCountryWhitelist data) 
		{
			return data.Id;			
		}
    }
