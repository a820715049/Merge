...
using StarExchangeConf = fat.rawdata.StarExchangeConf;
using rawStarExchange = fat.rawdata.StarExchange;
using rawStarExchangeMapABValue = fat.rawdata.StarExchangeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class StarExchangeVisitor
    {
		...
		public const string FileName = "StarExchangeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过StarExchangeVisitor.SetABDefault单独设定StarExchange的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过StarExchangeVisitor.SetABFallback单独设定StarExchange的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawStarExchange>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawStarExchange>>(()=> new PBC::MapField<int, rawStarExchange>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawStarExchange>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().StarExchangeMap;
			var abMap = v.StarExchangeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.StarExchangeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].StarExchangeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawStarExchange> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().StarExchangeMap;
			if (v.StarExchangeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.StarExchangeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawStarExchangeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(StarExchangeConf cc) {
			foreach(KeyValuePair <string, rawStarExchangeMapABValue> tagEntry in cc.StarExchangeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.StarExchangeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawStarExchangeMapABValue();
					cc.StarExchangeMapAB[tag] = ps;
				}
                var tagTmp = ps.StarExchangeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawStarExchange> entry in cc.StarExchangeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawStarExchange> entry in tagEntry.Value.StarExchangeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.StarExchange> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(StarExchangeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.StarExchangeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.StarExchangeMapAB != null)
			{
				foreach (var entry in cc.StarExchangeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.StarExchangeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.StarExchangeMap.MergeFrom(cis);
			Patch(updating.StarExchangeMap);
			updating.StarExchangeSortedKeys = genSortedKeys(updating.StarExchangeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new StarExchangeConf();
            tmp.MergeFrom(cis);
			updating.StarExchangeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().StarExchangeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawStarExchange> filter,string tag=""){
		...
		public static rawStarExchange MustGet(int key,string tag="") {
		...
		public static rawStarExchange? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawStarExchange> GetByFilter(Func<rawStarExchange, bool> filterFunc,string tag="") {
			List<rawStarExchange> ret = new List<rawStarExchange>();
			foreach(KeyValuePair <int, rawStarExchange> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawStarExchange? GetOneByFilter(Func<rawStarExchange, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawStarExchange> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawStarExchange data) 
		{
			return data.Id;			
		}
    }
