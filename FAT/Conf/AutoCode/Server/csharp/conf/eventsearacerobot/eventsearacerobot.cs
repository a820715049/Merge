...
using EventSeaRaceRobotConf = fat.rawdata.EventSeaRaceRobotConf;
using rawEventSeaRaceRobot = fat.rawdata.EventSeaRaceRobot;
using rawEventSeaRaceRobotMapABValue = fat.rawdata.EventSeaRaceRobotMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaRaceRobotVisitor
    {
		...
		public const string FileName = "EventSeaRaceRobotConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaRaceRobotVisitor.SetABDefault单独设定EventSeaRaceRobot的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaRaceRobotVisitor.SetABFallback单独设定EventSeaRaceRobot的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaRaceRobot>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaRaceRobot>>(()=> new PBC::MapField<int, rawEventSeaRaceRobot>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaRaceRobot>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaRaceRobotMap;
			var abMap = v.EventSeaRaceRobotMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaRaceRobotMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaRaceRobotMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaRaceRobot> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaRaceRobotMap;
			if (v.EventSeaRaceRobotMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaRaceRobotMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaRaceRobotMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaRaceRobotConf cc) {
			foreach(KeyValuePair <string, rawEventSeaRaceRobotMapABValue> tagEntry in cc.EventSeaRaceRobotMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaRaceRobotMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaRaceRobotMapABValue();
					cc.EventSeaRaceRobotMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaRaceRobotMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaRaceRobot> entry in cc.EventSeaRaceRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaRaceRobot> entry in tagEntry.Value.EventSeaRaceRobotMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaRaceRobot> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaRaceRobotConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaRaceRobotMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaRaceRobotMapAB != null)
			{
				foreach (var entry in cc.EventSeaRaceRobotMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaRaceRobotMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaRaceRobotMap.MergeFrom(cis);
			Patch(updating.EventSeaRaceRobotMap);
			updating.EventSeaRaceRobotSortedKeys = genSortedKeys(updating.EventSeaRaceRobotMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaRaceRobotConf();
            tmp.MergeFrom(cis);
			updating.EventSeaRaceRobotMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaRaceRobotSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaRaceRobot> filter,string tag=""){
		...
		public static rawEventSeaRaceRobot MustGet(int key,string tag="") {
		...
		public static rawEventSeaRaceRobot? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaRaceRobot> GetByFilter(Func<rawEventSeaRaceRobot, bool> filterFunc,string tag="") {
			List<rawEventSeaRaceRobot> ret = new List<rawEventSeaRaceRobot>();
			foreach(KeyValuePair <int, rawEventSeaRaceRobot> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaRaceRobot? GetOneByFilter(Func<rawEventSeaRaceRobot, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaRaceRobot> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaRaceRobot data) 
		{
			return data.Id;			
		}
    }
