...
using CustCmpltVIPConf = fat.rawdata.CustCmpltVIPConf;
using rawCustCmpltVIP = fat.rawdata.CustCmpltVIP;
using rawCustCmpltVIPMapABValue = fat.rawdata.CustCmpltVIPMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CustCmpltVIPVisitor
    {
		...
		public const string FileName = "CustCmpltVIPConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CustCmpltVIPVisitor.SetABDefault单独设定CustCmpltVIP的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CustCmpltVIPVisitor.SetABFallback单独设定CustCmpltVIP的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCustCmpltVIP>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCustCmpltVIP>>(()=> new PBC::MapField<int, rawCustCmpltVIP>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCustCmpltVIP>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CustCmpltVIPMap;
			var abMap = v.CustCmpltVIPMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CustCmpltVIPMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CustCmpltVIPMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCustCmpltVIP> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CustCmpltVIPMap;
			if (v.CustCmpltVIPMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CustCmpltVIPMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCustCmpltVIPMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CustCmpltVIPConf cc) {
			foreach(KeyValuePair <string, rawCustCmpltVIPMapABValue> tagEntry in cc.CustCmpltVIPMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CustCmpltVIPMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCustCmpltVIPMapABValue();
					cc.CustCmpltVIPMapAB[tag] = ps;
				}
                var tagTmp = ps.CustCmpltVIPMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCustCmpltVIP> entry in cc.CustCmpltVIPMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCustCmpltVIP> entry in tagEntry.Value.CustCmpltVIPMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CustCmpltVIP> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CustCmpltVIPConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CustCmpltVIPMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CustCmpltVIPMapAB != null)
			{
				foreach (var entry in cc.CustCmpltVIPMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CustCmpltVIPMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CustCmpltVIPMap.MergeFrom(cis);
			Patch(updating.CustCmpltVIPMap);
			updating.CustCmpltVIPSortedKeys = genSortedKeys(updating.CustCmpltVIPMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CustCmpltVIPConf();
            tmp.MergeFrom(cis);
			updating.CustCmpltVIPMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CustCmpltVIPSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCustCmpltVIP> filter,string tag=""){
		...
		public static rawCustCmpltVIP MustGet(int key,string tag="") {
		...
		public static rawCustCmpltVIP? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCustCmpltVIP> GetByFilter(Func<rawCustCmpltVIP, bool> filterFunc,string tag="") {
			List<rawCustCmpltVIP> ret = new List<rawCustCmpltVIP>();
			foreach(KeyValuePair <int, rawCustCmpltVIP> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCustCmpltVIP? GetOneByFilter(Func<rawCustCmpltVIP, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCustCmpltVIP> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCustCmpltVIP data) 
		{
			return data.Id;			
		}
    }
