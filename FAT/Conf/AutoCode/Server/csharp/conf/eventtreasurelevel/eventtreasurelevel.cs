...
using EventTreasureLevelConf = fat.rawdata.EventTreasureLevelConf;
using rawEventTreasureLevel = fat.rawdata.EventTreasureLevel;
using rawEventTreasureLevelMapABValue = fat.rawdata.EventTreasureLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventTreasureLevelVisitor
    {
		...
		public const string FileName = "EventTreasureLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventTreasureLevelVisitor.SetABDefault单独设定EventTreasureLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventTreasureLevelVisitor.SetABFallback单独设定EventTreasureLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventTreasureLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventTreasureLevel>>(()=> new PBC::MapField<int, rawEventTreasureLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventTreasureLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventTreasureLevelMap;
			var abMap = v.EventTreasureLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventTreasureLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventTreasureLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventTreasureLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventTreasureLevelMap;
			if (v.EventTreasureLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventTreasureLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventTreasureLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventTreasureLevelConf cc) {
			foreach(KeyValuePair <string, rawEventTreasureLevelMapABValue> tagEntry in cc.EventTreasureLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventTreasureLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventTreasureLevelMapABValue();
					cc.EventTreasureLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventTreasureLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventTreasureLevel> entry in cc.EventTreasureLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventTreasureLevel> entry in tagEntry.Value.EventTreasureLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventTreasureLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventTreasureLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventTreasureLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventTreasureLevelMapAB != null)
			{
				foreach (var entry in cc.EventTreasureLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventTreasureLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventTreasureLevelMap.MergeFrom(cis);
			Patch(updating.EventTreasureLevelMap);
			updating.EventTreasureLevelSortedKeys = genSortedKeys(updating.EventTreasureLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventTreasureLevelConf();
            tmp.MergeFrom(cis);
			updating.EventTreasureLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventTreasureLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventTreasureLevel> filter,string tag=""){
		...
		public static rawEventTreasureLevel MustGet(int key,string tag="") {
		...
		public static rawEventTreasureLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventTreasureLevel> GetByFilter(Func<rawEventTreasureLevel, bool> filterFunc,string tag="") {
			List<rawEventTreasureLevel> ret = new List<rawEventTreasureLevel>();
			foreach(KeyValuePair <int, rawEventTreasureLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventTreasureLevel? GetOneByFilter(Func<rawEventTreasureLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventTreasureLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventTreasureLevel data) 
		{
			return data.Id;			
		}
    }
