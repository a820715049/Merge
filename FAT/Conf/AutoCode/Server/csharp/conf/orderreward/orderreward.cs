...
using OrderRewardConf = fat.rawdata.OrderRewardConf;
using rawOrderReward = fat.rawdata.OrderReward;
using rawOrderRewardMapABValue = fat.rawdata.OrderRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderRewardVisitor
    {
		...
		public const string FileName = "OrderRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderRewardVisitor.SetABDefault单独设定OrderReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderRewardVisitor.SetABFallback单独设定OrderReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderReward>>(()=> new PBC::MapField<int, rawOrderReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderRewardMap;
			var abMap = v.OrderRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderRewardMap;
			if (v.OrderRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderRewardConf cc) {
			foreach(KeyValuePair <string, rawOrderRewardMapABValue> tagEntry in cc.OrderRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderRewardMapABValue();
					cc.OrderRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderReward> entry in cc.OrderRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderReward> entry in tagEntry.Value.OrderRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderRewardMapAB != null)
			{
				foreach (var entry in cc.OrderRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderRewardMap.MergeFrom(cis);
			Patch(updating.OrderRewardMap);
			updating.OrderRewardSortedKeys = genSortedKeys(updating.OrderRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderRewardConf();
            tmp.MergeFrom(cis);
			updating.OrderRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderReward> filter,string tag=""){
		...
		public static rawOrderReward MustGet(int key,string tag="") {
		...
		public static rawOrderReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderReward> GetByFilter(Func<rawOrderReward, bool> filterFunc,string tag="") {
			List<rawOrderReward> ret = new List<rawOrderReward>();
			foreach(KeyValuePair <int, rawOrderReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderReward? GetOneByFilter(Func<rawOrderReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderReward data) 
		{
			return data.Id;			
		}
    }
