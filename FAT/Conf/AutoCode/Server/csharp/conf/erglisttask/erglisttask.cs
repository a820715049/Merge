...
using ErgListTaskConf = fat.rawdata.ErgListTaskConf;
using rawErgListTask = fat.rawdata.ErgListTask;
using rawErgListTaskMapABValue = fat.rawdata.ErgListTaskMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ErgListTaskVisitor
    {
		...
		public const string FileName = "ErgListTaskConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ErgListTaskVisitor.SetABDefault单独设定ErgListTask的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ErgListTaskVisitor.SetABFallback单独设定ErgListTask的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawErgListTask>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawErgListTask>>(()=> new PBC::MapField<int, rawErgListTask>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawErgListTask>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ErgListTaskMap;
			var abMap = v.ErgListTaskMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ErgListTaskMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ErgListTaskMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawErgListTask> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ErgListTaskMap;
			if (v.ErgListTaskMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ErgListTaskMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawErgListTaskMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ErgListTaskConf cc) {
			foreach(KeyValuePair <string, rawErgListTaskMapABValue> tagEntry in cc.ErgListTaskMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ErgListTaskMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawErgListTaskMapABValue();
					cc.ErgListTaskMapAB[tag] = ps;
				}
                var tagTmp = ps.ErgListTaskMap;
				// 基础数据
				foreach(KeyValuePair <int, rawErgListTask> entry in cc.ErgListTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawErgListTask> entry in tagEntry.Value.ErgListTaskMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ErgListTask> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ErgListTaskConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ErgListTaskMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ErgListTaskMapAB != null)
			{
				foreach (var entry in cc.ErgListTaskMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ErgListTaskMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ErgListTaskMap.MergeFrom(cis);
			Patch(updating.ErgListTaskMap);
			updating.ErgListTaskSortedKeys = genSortedKeys(updating.ErgListTaskMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ErgListTaskConf();
            tmp.MergeFrom(cis);
			updating.ErgListTaskMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ErgListTaskSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawErgListTask> filter,string tag=""){
		...
		public static rawErgListTask MustGet(int key,string tag="") {
		...
		public static rawErgListTask? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawErgListTask> GetByFilter(Func<rawErgListTask, bool> filterFunc,string tag="") {
			List<rawErgListTask> ret = new List<rawErgListTask>();
			foreach(KeyValuePair <int, rawErgListTask> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawErgListTask? GetOneByFilter(Func<rawErgListTask, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawErgListTask> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawErgListTask data) 
		{
			return data.Id;			
		}
    }
