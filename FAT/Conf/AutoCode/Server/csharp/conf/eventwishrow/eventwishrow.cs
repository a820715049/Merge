...
using EventWishRowConf = fat.rawdata.EventWishRowConf;
using rawEventWishRow = fat.rawdata.EventWishRow;
using rawEventWishRowMapABValue = fat.rawdata.EventWishRowMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishRowVisitor
    {
		...
		public const string FileName = "EventWishRowConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishRowVisitor.SetABDefault单独设定EventWishRow的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishRowVisitor.SetABFallback单独设定EventWishRow的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishRow>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishRow>>(()=> new PBC::MapField<int, rawEventWishRow>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishRow>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishRowMap;
			var abMap = v.EventWishRowMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishRowMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishRowMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishRow> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishRowMap;
			if (v.EventWishRowMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishRowMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishRowMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishRowConf cc) {
			foreach(KeyValuePair <string, rawEventWishRowMapABValue> tagEntry in cc.EventWishRowMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishRowMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishRowMapABValue();
					cc.EventWishRowMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishRowMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishRow> entry in cc.EventWishRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishRow> entry in tagEntry.Value.EventWishRowMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishRow> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishRowConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishRowMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishRowMapAB != null)
			{
				foreach (var entry in cc.EventWishRowMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishRowMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishRowMap.MergeFrom(cis);
			Patch(updating.EventWishRowMap);
			updating.EventWishRowSortedKeys = genSortedKeys(updating.EventWishRowMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishRowConf();
            tmp.MergeFrom(cis);
			updating.EventWishRowMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishRowSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishRow> filter,string tag=""){
		...
		public static rawEventWishRow MustGet(int key,string tag="") {
		...
		public static rawEventWishRow? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishRow> GetByFilter(Func<rawEventWishRow, bool> filterFunc,string tag="") {
			List<rawEventWishRow> ret = new List<rawEventWishRow>();
			foreach(KeyValuePair <int, rawEventWishRow> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishRow? GetOneByFilter(Func<rawEventWishRow, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishRow> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishRow data) 
		{
			return data.Id;			
		}
    }
