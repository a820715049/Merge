...
using BuildingCostConf = fat.rawdata.BuildingCostConf;
using rawBuildingCost = fat.rawdata.BuildingCost;
using rawBuildingCostMapABValue = fat.rawdata.BuildingCostMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BuildingCostVisitor
    {
		...
		public const string FileName = "BuildingCostConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BuildingCostVisitor.SetABDefault单独设定BuildingCost的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BuildingCostVisitor.SetABFallback单独设定BuildingCost的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBuildingCost>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBuildingCost>>(()=> new PBC::MapField<int, rawBuildingCost>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBuildingCost>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BuildingCostMap;
			var abMap = v.BuildingCostMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BuildingCostMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BuildingCostMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBuildingCost> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BuildingCostMap;
			if (v.BuildingCostMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BuildingCostMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBuildingCostMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BuildingCostConf cc) {
			foreach(KeyValuePair <string, rawBuildingCostMapABValue> tagEntry in cc.BuildingCostMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BuildingCostMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBuildingCostMapABValue();
					cc.BuildingCostMapAB[tag] = ps;
				}
                var tagTmp = ps.BuildingCostMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBuildingCost> entry in cc.BuildingCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBuildingCost> entry in tagEntry.Value.BuildingCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BuildingCost> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BuildingCostConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BuildingCostMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BuildingCostMapAB != null)
			{
				foreach (var entry in cc.BuildingCostMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BuildingCostMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BuildingCostMap.MergeFrom(cis);
			Patch(updating.BuildingCostMap);
			updating.BuildingCostSortedKeys = genSortedKeys(updating.BuildingCostMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BuildingCostConf();
            tmp.MergeFrom(cis);
			updating.BuildingCostMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BuildingCostSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBuildingCost> filter,string tag=""){
		...
		public static rawBuildingCost MustGet(int key,string tag="") {
		...
		public static rawBuildingCost? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBuildingCost> GetByFilter(Func<rawBuildingCost, bool> filterFunc,string tag="") {
			List<rawBuildingCost> ret = new List<rawBuildingCost>();
			foreach(KeyValuePair <int, rawBuildingCost> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBuildingCost? GetOneByFilter(Func<rawBuildingCost, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBuildingCost> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBuildingCost data) 
		{
			return data.Id;			
		}
    }
