...
using LangDeHotfixConf = fat.rawdata.LangDeHotfixConf;
using rawLangDeHotfix = fat.rawdata.LangDeHotfix;
using rawLangDeHotfixSliceABValue = fat.rawdata.LangDeHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangDeHotfixVisitor
    {
		...
		public const string FileName = "LangDeHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangDeHotfixVisitor.SetABDefault单独设定LangDeHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangDeHotfixVisitor.SetABFallback单独设定LangDeHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangDeHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangDeHotfix>>(()=>new PBC::RepeatedField<rawLangDeHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangDeHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangDeHotfixSlice;
			var abMap = v.LangDeHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangDeHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangDeHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangDeHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangDeHotfixSlice;
			if (v.LangDeHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangDeHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangDeHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangDeHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangDeHotfixSliceABValue> tagEntry in cc.LangDeHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangDeHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangDeHotfixSliceABValue();
					cc.LangDeHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangDeHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangDeHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangDeHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangDeHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangDeHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangDeHotfixSlice.MergeFrom(cis);
			Patch(updating.LangDeHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangDeHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangDeHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangDeHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangDeHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangDeHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangDeHotfix> GetByFilter(Func<rawLangDeHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangDeHotfix>();
		...
		static public rawLangDeHotfix? GetOneByFilter(Func<rawLangDeHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
