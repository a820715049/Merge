...
using EventWeeklyRaffleGrpConf = fat.rawdata.EventWeeklyRaffleGrpConf;
using rawEventWeeklyRaffleGrp = fat.rawdata.EventWeeklyRaffleGrp;
using rawEventWeeklyRaffleGrpMapABValue = fat.rawdata.EventWeeklyRaffleGrpMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWeeklyRaffleGrpVisitor
    {
		...
		public const string FileName = "EventWeeklyRaffleGrpConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWeeklyRaffleGrpVisitor.SetABDefault单独设定EventWeeklyRaffleGrp的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWeeklyRaffleGrpVisitor.SetABFallback单独设定EventWeeklyRaffleGrp的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWeeklyRaffleGrp>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWeeklyRaffleGrp>>(()=> new PBC::MapField<int, rawEventWeeklyRaffleGrp>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWeeklyRaffleGrp>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWeeklyRaffleGrpMap;
			var abMap = v.EventWeeklyRaffleGrpMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWeeklyRaffleGrpMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWeeklyRaffleGrpMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWeeklyRaffleGrp> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWeeklyRaffleGrpMap;
			if (v.EventWeeklyRaffleGrpMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWeeklyRaffleGrpMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWeeklyRaffleGrpMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWeeklyRaffleGrpConf cc) {
			foreach(KeyValuePair <string, rawEventWeeklyRaffleGrpMapABValue> tagEntry in cc.EventWeeklyRaffleGrpMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWeeklyRaffleGrpMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWeeklyRaffleGrpMapABValue();
					cc.EventWeeklyRaffleGrpMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWeeklyRaffleGrpMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffleGrp> entry in cc.EventWeeklyRaffleGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWeeklyRaffleGrp> entry in tagEntry.Value.EventWeeklyRaffleGrpMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWeeklyRaffleGrp> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWeeklyRaffleGrpConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWeeklyRaffleGrpMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWeeklyRaffleGrpMapAB != null)
			{
				foreach (var entry in cc.EventWeeklyRaffleGrpMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWeeklyRaffleGrpMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWeeklyRaffleGrpMap.MergeFrom(cis);
			Patch(updating.EventWeeklyRaffleGrpMap);
			updating.EventWeeklyRaffleGrpSortedKeys = genSortedKeys(updating.EventWeeklyRaffleGrpMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWeeklyRaffleGrpConf();
            tmp.MergeFrom(cis);
			updating.EventWeeklyRaffleGrpMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWeeklyRaffleGrpSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWeeklyRaffleGrp> filter,string tag=""){
		...
		public static rawEventWeeklyRaffleGrp MustGet(int key,string tag="") {
		...
		public static rawEventWeeklyRaffleGrp? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWeeklyRaffleGrp> GetByFilter(Func<rawEventWeeklyRaffleGrp, bool> filterFunc,string tag="") {
			List<rawEventWeeklyRaffleGrp> ret = new List<rawEventWeeklyRaffleGrp>();
			foreach(KeyValuePair <int, rawEventWeeklyRaffleGrp> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWeeklyRaffleGrp? GetOneByFilter(Func<rawEventWeeklyRaffleGrp, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWeeklyRaffleGrp> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWeeklyRaffleGrp data) 
		{
			return data.Id;			
		}
    }
