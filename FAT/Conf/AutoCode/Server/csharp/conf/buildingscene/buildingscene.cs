...
using BuildingSceneConf = fat.rawdata.BuildingSceneConf;
using rawBuildingScene = fat.rawdata.BuildingScene;
using rawBuildingSceneMapABValue = fat.rawdata.BuildingSceneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BuildingSceneVisitor
    {
		...
		public const string FileName = "BuildingSceneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BuildingSceneVisitor.SetABDefault单独设定BuildingScene的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BuildingSceneVisitor.SetABFallback单独设定BuildingScene的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawBuildingScene>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawBuildingScene>>(()=> new PBC::MapField<int, rawBuildingScene>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawBuildingScene>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BuildingSceneMap;
			var abMap = v.BuildingSceneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BuildingSceneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BuildingSceneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawBuildingScene> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BuildingSceneMap;
			if (v.BuildingSceneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BuildingSceneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBuildingSceneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BuildingSceneConf cc) {
			foreach(KeyValuePair <string, rawBuildingSceneMapABValue> tagEntry in cc.BuildingSceneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BuildingSceneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBuildingSceneMapABValue();
					cc.BuildingSceneMapAB[tag] = ps;
				}
                var tagTmp = ps.BuildingSceneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawBuildingScene> entry in cc.BuildingSceneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawBuildingScene> entry in tagEntry.Value.BuildingSceneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.BuildingScene> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(BuildingSceneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.BuildingSceneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.BuildingSceneMapAB != null)
			{
				foreach (var entry in cc.BuildingSceneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.BuildingSceneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BuildingSceneMap.MergeFrom(cis);
			Patch(updating.BuildingSceneMap);
			updating.BuildingSceneSortedKeys = genSortedKeys(updating.BuildingSceneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BuildingSceneConf();
            tmp.MergeFrom(cis);
			updating.BuildingSceneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().BuildingSceneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawBuildingScene> filter,string tag=""){
		...
		public static rawBuildingScene MustGet(int key,string tag="") {
		...
		public static rawBuildingScene? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawBuildingScene> GetByFilter(Func<rawBuildingScene, bool> filterFunc,string tag="") {
			List<rawBuildingScene> ret = new List<rawBuildingScene>();
			foreach(KeyValuePair <int, rawBuildingScene> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawBuildingScene? GetOneByFilter(Func<rawBuildingScene, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawBuildingScene> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawBuildingScene data) 
		{
			return data.Id;			
		}
    }
