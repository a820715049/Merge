...
using EventMiniBoardDetailConf = fat.rawdata.EventMiniBoardDetailConf;
using rawEventMiniBoardDetail = fat.rawdata.EventMiniBoardDetail;
using rawEventMiniBoardDetailMapABValue = fat.rawdata.EventMiniBoardDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMiniBoardDetailVisitor
    {
		...
		public const string FileName = "EventMiniBoardDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMiniBoardDetailVisitor.SetABDefault单独设定EventMiniBoardDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMiniBoardDetailVisitor.SetABFallback单独设定EventMiniBoardDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMiniBoardDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMiniBoardDetail>>(()=> new PBC::MapField<int, rawEventMiniBoardDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMiniBoardDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMiniBoardDetailMap;
			var abMap = v.EventMiniBoardDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMiniBoardDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMiniBoardDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMiniBoardDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMiniBoardDetailMap;
			if (v.EventMiniBoardDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMiniBoardDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMiniBoardDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMiniBoardDetailConf cc) {
			foreach(KeyValuePair <string, rawEventMiniBoardDetailMapABValue> tagEntry in cc.EventMiniBoardDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMiniBoardDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMiniBoardDetailMapABValue();
					cc.EventMiniBoardDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMiniBoardDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMiniBoardDetail> entry in cc.EventMiniBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMiniBoardDetail> entry in tagEntry.Value.EventMiniBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMiniBoardDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMiniBoardDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMiniBoardDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMiniBoardDetailMapAB != null)
			{
				foreach (var entry in cc.EventMiniBoardDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMiniBoardDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMiniBoardDetailMap.MergeFrom(cis);
			Patch(updating.EventMiniBoardDetailMap);
			updating.EventMiniBoardDetailSortedKeys = genSortedKeys(updating.EventMiniBoardDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMiniBoardDetailConf();
            tmp.MergeFrom(cis);
			updating.EventMiniBoardDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMiniBoardDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMiniBoardDetail> filter,string tag=""){
		...
		public static rawEventMiniBoardDetail MustGet(int key,string tag="") {
		...
		public static rawEventMiniBoardDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMiniBoardDetail> GetByFilter(Func<rawEventMiniBoardDetail, bool> filterFunc,string tag="") {
			List<rawEventMiniBoardDetail> ret = new List<rawEventMiniBoardDetail>();
			foreach(KeyValuePair <int, rawEventMiniBoardDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMiniBoardDetail? GetOneByFilter(Func<rawEventMiniBoardDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMiniBoardDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMiniBoardDetail data) 
		{
			return data.Id;			
		}
    }
