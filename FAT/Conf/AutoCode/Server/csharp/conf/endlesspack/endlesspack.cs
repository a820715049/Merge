...
using EndlessPackConf = fat.rawdata.EndlessPackConf;
using rawEndlessPack = fat.rawdata.EndlessPack;
using rawEndlessPackMapABValue = fat.rawdata.EndlessPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessPackVisitor
    {
		...
		public const string FileName = "EndlessPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessPackVisitor.SetABDefault单独设定EndlessPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessPackVisitor.SetABFallback单独设定EndlessPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessPack>>(()=> new PBC::MapField<int, rawEndlessPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessPackMap;
			var abMap = v.EndlessPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessPackMap;
			if (v.EndlessPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessPackConf cc) {
			foreach(KeyValuePair <string, rawEndlessPackMapABValue> tagEntry in cc.EndlessPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessPackMapABValue();
					cc.EndlessPackMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessPack> entry in cc.EndlessPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessPack> entry in tagEntry.Value.EndlessPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessPackMapAB != null)
			{
				foreach (var entry in cc.EndlessPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessPackMap.MergeFrom(cis);
			Patch(updating.EndlessPackMap);
			updating.EndlessPackSortedKeys = genSortedKeys(updating.EndlessPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessPackConf();
            tmp.MergeFrom(cis);
			updating.EndlessPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessPack> filter,string tag=""){
		...
		public static rawEndlessPack MustGet(int key,string tag="") {
		...
		public static rawEndlessPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessPack> GetByFilter(Func<rawEndlessPack, bool> filterFunc,string tag="") {
			List<rawEndlessPack> ret = new List<rawEndlessPack>();
			foreach(KeyValuePair <int, rawEndlessPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessPack? GetOneByFilter(Func<rawEndlessPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessPack data) 
		{
			return data.Id;			
		}
    }
