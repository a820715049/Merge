...
using TrainChallengeConf = fat.rawdata.TrainChallengeConf;
using rawTrainChallenge = fat.rawdata.TrainChallenge;
using rawTrainChallengeMapABValue = fat.rawdata.TrainChallengeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class TrainChallengeVisitor
    {
		...
		public const string FileName = "TrainChallengeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过TrainChallengeVisitor.SetABDefault单独设定TrainChallenge的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过TrainChallengeVisitor.SetABFallback单独设定TrainChallenge的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawTrainChallenge>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawTrainChallenge>>(()=> new PBC::MapField<int, rawTrainChallenge>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawTrainChallenge>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().TrainChallengeMap;
			var abMap = v.TrainChallengeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.TrainChallengeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].TrainChallengeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawTrainChallenge> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().TrainChallengeMap;
			if (v.TrainChallengeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.TrainChallengeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawTrainChallengeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(TrainChallengeConf cc) {
			foreach(KeyValuePair <string, rawTrainChallengeMapABValue> tagEntry in cc.TrainChallengeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.TrainChallengeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawTrainChallengeMapABValue();
					cc.TrainChallengeMapAB[tag] = ps;
				}
                var tagTmp = ps.TrainChallengeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawTrainChallenge> entry in cc.TrainChallengeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawTrainChallenge> entry in tagEntry.Value.TrainChallengeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.TrainChallenge> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(TrainChallengeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.TrainChallengeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.TrainChallengeMapAB != null)
			{
				foreach (var entry in cc.TrainChallengeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.TrainChallengeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.TrainChallengeMap.MergeFrom(cis);
			Patch(updating.TrainChallengeMap);
			updating.TrainChallengeSortedKeys = genSortedKeys(updating.TrainChallengeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new TrainChallengeConf();
            tmp.MergeFrom(cis);
			updating.TrainChallengeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().TrainChallengeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawTrainChallenge> filter,string tag=""){
		...
		public static rawTrainChallenge MustGet(int key,string tag="") {
		...
		public static rawTrainChallenge? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawTrainChallenge> GetByFilter(Func<rawTrainChallenge, bool> filterFunc,string tag="") {
			List<rawTrainChallenge> ret = new List<rawTrainChallenge>();
			foreach(KeyValuePair <int, rawTrainChallenge> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawTrainChallenge? GetOneByFilter(Func<rawTrainChallenge, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawTrainChallenge> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawTrainChallenge data) 
		{
			return data.Id;			
		}
    }
