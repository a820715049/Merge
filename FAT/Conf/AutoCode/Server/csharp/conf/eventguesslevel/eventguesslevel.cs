...
using EventGuessLevelConf = fat.rawdata.EventGuessLevelConf;
using rawEventGuessLevel = fat.rawdata.EventGuessLevel;
using rawEventGuessLevelMapABValue = fat.rawdata.EventGuessLevelMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventGuessLevelVisitor
    {
		...
		public const string FileName = "EventGuessLevelConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventGuessLevelVisitor.SetABDefault单独设定EventGuessLevel的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventGuessLevelVisitor.SetABFallback单独设定EventGuessLevel的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventGuessLevel>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventGuessLevel>>(()=> new PBC::MapField<int, rawEventGuessLevel>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventGuessLevel>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventGuessLevelMap;
			var abMap = v.EventGuessLevelMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventGuessLevelMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventGuessLevelMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventGuessLevel> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventGuessLevelMap;
			if (v.EventGuessLevelMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventGuessLevelMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventGuessLevelMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventGuessLevelConf cc) {
			foreach(KeyValuePair <string, rawEventGuessLevelMapABValue> tagEntry in cc.EventGuessLevelMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventGuessLevelMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventGuessLevelMapABValue();
					cc.EventGuessLevelMapAB[tag] = ps;
				}
                var tagTmp = ps.EventGuessLevelMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventGuessLevel> entry in cc.EventGuessLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventGuessLevel> entry in tagEntry.Value.EventGuessLevelMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventGuessLevel> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventGuessLevelConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventGuessLevelMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventGuessLevelMapAB != null)
			{
				foreach (var entry in cc.EventGuessLevelMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventGuessLevelMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventGuessLevelMap.MergeFrom(cis);
			Patch(updating.EventGuessLevelMap);
			updating.EventGuessLevelSortedKeys = genSortedKeys(updating.EventGuessLevelMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventGuessLevelConf();
            tmp.MergeFrom(cis);
			updating.EventGuessLevelMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventGuessLevelSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventGuessLevel> filter,string tag=""){
		...
		public static rawEventGuessLevel MustGet(int key,string tag="") {
		...
		public static rawEventGuessLevel? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventGuessLevel> GetByFilter(Func<rawEventGuessLevel, bool> filterFunc,string tag="") {
			List<rawEventGuessLevel> ret = new List<rawEventGuessLevel>();
			foreach(KeyValuePair <int, rawEventGuessLevel> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventGuessLevel? GetOneByFilter(Func<rawEventGuessLevel, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventGuessLevel> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventGuessLevel data) 
		{
			return data.Id;			
		}
    }
