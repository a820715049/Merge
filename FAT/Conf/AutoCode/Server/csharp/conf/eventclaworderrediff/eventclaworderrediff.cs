...
using EventClawOrderReDiffConf = fat.rawdata.EventClawOrderReDiffConf;
using rawEventClawOrderReDiff = fat.rawdata.EventClawOrderReDiff;
using rawEventClawOrderReDiffMapABValue = fat.rawdata.EventClawOrderReDiffMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventClawOrderReDiffVisitor
    {
		...
		public const string FileName = "EventClawOrderReDiffConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventClawOrderReDiffVisitor.SetABDefault单独设定EventClawOrderReDiff的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventClawOrderReDiffVisitor.SetABFallback单独设定EventClawOrderReDiff的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventClawOrderReDiff>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventClawOrderReDiff>>(()=> new PBC::MapField<int, rawEventClawOrderReDiff>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventClawOrderReDiff>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventClawOrderReDiffMap;
			var abMap = v.EventClawOrderReDiffMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventClawOrderReDiffMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventClawOrderReDiffMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventClawOrderReDiff> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventClawOrderReDiffMap;
			if (v.EventClawOrderReDiffMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventClawOrderReDiffMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventClawOrderReDiffMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventClawOrderReDiffConf cc) {
			foreach(KeyValuePair <string, rawEventClawOrderReDiffMapABValue> tagEntry in cc.EventClawOrderReDiffMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventClawOrderReDiffMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventClawOrderReDiffMapABValue();
					cc.EventClawOrderReDiffMapAB[tag] = ps;
				}
                var tagTmp = ps.EventClawOrderReDiffMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventClawOrderReDiff> entry in cc.EventClawOrderReDiffMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventClawOrderReDiff> entry in tagEntry.Value.EventClawOrderReDiffMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventClawOrderReDiff> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventClawOrderReDiffConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventClawOrderReDiffMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventClawOrderReDiffMapAB != null)
			{
				foreach (var entry in cc.EventClawOrderReDiffMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventClawOrderReDiffMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventClawOrderReDiffMap.MergeFrom(cis);
			Patch(updating.EventClawOrderReDiffMap);
			updating.EventClawOrderReDiffSortedKeys = genSortedKeys(updating.EventClawOrderReDiffMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventClawOrderReDiffConf();
            tmp.MergeFrom(cis);
			updating.EventClawOrderReDiffMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventClawOrderReDiffSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventClawOrderReDiff> filter,string tag=""){
		...
		public static rawEventClawOrderReDiff MustGet(int key,string tag="") {
		...
		public static rawEventClawOrderReDiff? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventClawOrderReDiff> GetByFilter(Func<rawEventClawOrderReDiff, bool> filterFunc,string tag="") {
			List<rawEventClawOrderReDiff> ret = new List<rawEventClawOrderReDiff>();
			foreach(KeyValuePair <int, rawEventClawOrderReDiff> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventClawOrderReDiff? GetOneByFilter(Func<rawEventClawOrderReDiff, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventClawOrderReDiff> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventClawOrderReDiff data) 
		{
			return data.Id;			
		}
    }
