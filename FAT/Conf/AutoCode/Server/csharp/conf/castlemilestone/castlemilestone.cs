...
using CastleMilestoneConf = fat.rawdata.CastleMilestoneConf;
using rawCastleMilestone = fat.rawdata.CastleMilestone;
using rawCastleMilestoneMapABValue = fat.rawdata.CastleMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CastleMilestoneVisitor
    {
		...
		public const string FileName = "CastleMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CastleMilestoneVisitor.SetABDefault单独设定CastleMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CastleMilestoneVisitor.SetABFallback单独设定CastleMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCastleMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCastleMilestone>>(()=> new PBC::MapField<int, rawCastleMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCastleMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CastleMilestoneMap;
			var abMap = v.CastleMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CastleMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CastleMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCastleMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CastleMilestoneMap;
			if (v.CastleMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CastleMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCastleMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CastleMilestoneConf cc) {
			foreach(KeyValuePair <string, rawCastleMilestoneMapABValue> tagEntry in cc.CastleMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CastleMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCastleMilestoneMapABValue();
					cc.CastleMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.CastleMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCastleMilestone> entry in cc.CastleMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCastleMilestone> entry in tagEntry.Value.CastleMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CastleMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CastleMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CastleMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CastleMilestoneMapAB != null)
			{
				foreach (var entry in cc.CastleMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CastleMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CastleMilestoneMap.MergeFrom(cis);
			Patch(updating.CastleMilestoneMap);
			updating.CastleMilestoneSortedKeys = genSortedKeys(updating.CastleMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CastleMilestoneConf();
            tmp.MergeFrom(cis);
			updating.CastleMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CastleMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCastleMilestone> filter,string tag=""){
		...
		public static rawCastleMilestone MustGet(int key,string tag="") {
		...
		public static rawCastleMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCastleMilestone> GetByFilter(Func<rawCastleMilestone, bool> filterFunc,string tag="") {
			List<rawCastleMilestone> ret = new List<rawCastleMilestone>();
			foreach(KeyValuePair <int, rawCastleMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCastleMilestone? GetOneByFilter(Func<rawCastleMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCastleMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCastleMilestone data) 
		{
			return data.Id;			
		}
    }
