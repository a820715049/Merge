...
using EventThreeSignConf = fat.rawdata.EventThreeSignConf;
using rawEventThreeSign = fat.rawdata.EventThreeSign;
using rawEventThreeSignMapABValue = fat.rawdata.EventThreeSignMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventThreeSignVisitor
    {
		...
		public const string FileName = "EventThreeSignConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventThreeSignVisitor.SetABDefault单独设定EventThreeSign的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventThreeSignVisitor.SetABFallback单独设定EventThreeSign的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventThreeSign>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventThreeSign>>(()=> new PBC::MapField<int, rawEventThreeSign>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventThreeSign>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventThreeSignMap;
			var abMap = v.EventThreeSignMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventThreeSignMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventThreeSignMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventThreeSign> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventThreeSignMap;
			if (v.EventThreeSignMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventThreeSignMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventThreeSignMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventThreeSignConf cc) {
			foreach(KeyValuePair <string, rawEventThreeSignMapABValue> tagEntry in cc.EventThreeSignMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventThreeSignMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventThreeSignMapABValue();
					cc.EventThreeSignMapAB[tag] = ps;
				}
                var tagTmp = ps.EventThreeSignMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventThreeSign> entry in cc.EventThreeSignMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventThreeSign> entry in tagEntry.Value.EventThreeSignMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventThreeSign> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventThreeSignConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventThreeSignMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventThreeSignMapAB != null)
			{
				foreach (var entry in cc.EventThreeSignMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventThreeSignMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventThreeSignMap.MergeFrom(cis);
			Patch(updating.EventThreeSignMap);
			updating.EventThreeSignSortedKeys = genSortedKeys(updating.EventThreeSignMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventThreeSignConf();
            tmp.MergeFrom(cis);
			updating.EventThreeSignMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventThreeSignSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventThreeSign> filter,string tag=""){
		...
		public static rawEventThreeSign MustGet(int key,string tag="") {
		...
		public static rawEventThreeSign? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventThreeSign> GetByFilter(Func<rawEventThreeSign, bool> filterFunc,string tag="") {
			List<rawEventThreeSign> ret = new List<rawEventThreeSign>();
			foreach(KeyValuePair <int, rawEventThreeSign> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventThreeSign? GetOneByFilter(Func<rawEventThreeSign, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventThreeSign> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventThreeSign data) 
		{
			return data.Id;			
		}
    }
