...
using MergeFixedItemConf = fat.rawdata.MergeFixedItemConf;
using rawMergeFixedItem = fat.rawdata.MergeFixedItem;
using rawMergeFixedItemSliceABValue = fat.rawdata.MergeFixedItemSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeFixedItemVisitor
    {
		...
		public const string FileName = "MergeFixedItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeFixedItemVisitor.SetABDefault单独设定MergeFixedItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeFixedItemVisitor.SetABFallback单独设定MergeFixedItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawMergeFixedItem>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawMergeFixedItem>>(()=>new PBC::RepeatedField<rawMergeFixedItem>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawMergeFixedItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeFixedItemSlice;
			var abMap = v.MergeFixedItemSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeFixedItemSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeFixedItemSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawMergeFixedItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeFixedItemSlice;
			if (v.MergeFixedItemSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeFixedItemSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeFixedItemSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeFixedItemConf cc) {
			foreach(KeyValuePair <string, rawMergeFixedItemSliceABValue> tagEntry in cc.MergeFixedItemSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeFixedItemSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeFixedItemSliceABValue();
					cc.MergeFixedItemSliceAB[tag] = ps;
				}
                var tagTmp = ps.MergeFixedItemSlice;
				// 基础数据
				for(int i=0; i<cc.MergeFixedItemSlice.Count; ++i) 
				{
					tagTmp.Add(cc.MergeFixedItemSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.MergeFixedItemSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.MergeFixedItem> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeFixedItemSlice.MergeFrom(cis);
			Patch(updating.MergeFixedItemSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeFixedItemConf();
            tmp.MergeFrom(cis);
			updating.MergeFixedItemSlice = tmp;
		...
		public static void Range(Action<int, rawMergeFixedItem> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawMergeFixedItem MustGetByIndex(int index,string tag="") {
		...
		public static rawMergeFixedItem? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawMergeFixedItem> GetByFilter(Func<rawMergeFixedItem,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawMergeFixedItem>();
		...
		static public rawMergeFixedItem? GetOneByFilter(Func<rawMergeFixedItem,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
