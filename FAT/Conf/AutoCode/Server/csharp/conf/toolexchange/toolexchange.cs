...
using ToolExchangeConf = fat.rawdata.ToolExchangeConf;
using rawToolExchange = fat.rawdata.ToolExchange;
using rawToolExchangeMapABValue = fat.rawdata.ToolExchangeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ToolExchangeVisitor
    {
		...
		public const string FileName = "ToolExchangeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ToolExchangeVisitor.SetABDefault单独设定ToolExchange的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ToolExchangeVisitor.SetABFallback单独设定ToolExchange的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawToolExchange>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawToolExchange>>(()=> new PBC::MapField<int, rawToolExchange>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawToolExchange>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ToolExchangeMap;
			var abMap = v.ToolExchangeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ToolExchangeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ToolExchangeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawToolExchange> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ToolExchangeMap;
			if (v.ToolExchangeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ToolExchangeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawToolExchangeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ToolExchangeConf cc) {
			foreach(KeyValuePair <string, rawToolExchangeMapABValue> tagEntry in cc.ToolExchangeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ToolExchangeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawToolExchangeMapABValue();
					cc.ToolExchangeMapAB[tag] = ps;
				}
                var tagTmp = ps.ToolExchangeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawToolExchange> entry in cc.ToolExchangeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawToolExchange> entry in tagEntry.Value.ToolExchangeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ToolExchange> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ToolExchangeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ToolExchangeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ToolExchangeMapAB != null)
			{
				foreach (var entry in cc.ToolExchangeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ToolExchangeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ToolExchangeMap.MergeFrom(cis);
			Patch(updating.ToolExchangeMap);
			updating.ToolExchangeSortedKeys = genSortedKeys(updating.ToolExchangeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ToolExchangeConf();
            tmp.MergeFrom(cis);
			updating.ToolExchangeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ToolExchangeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawToolExchange> filter,string tag=""){
		...
		public static rawToolExchange MustGet(int key,string tag="") {
		...
		public static rawToolExchange? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawToolExchange> GetByFilter(Func<rawToolExchange, bool> filterFunc,string tag="") {
			List<rawToolExchange> ret = new List<rawToolExchange>();
			foreach(KeyValuePair <int, rawToolExchange> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawToolExchange? GetOneByFilter(Func<rawToolExchange, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawToolExchange> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawToolExchange data) 
		{
			return data.Id;			
		}
    }
