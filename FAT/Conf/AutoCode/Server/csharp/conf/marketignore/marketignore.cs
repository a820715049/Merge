...
using MarketIgnoreConf = fat.rawdata.MarketIgnoreConf;
using rawMarketIgnore = fat.rawdata.MarketIgnore;
using rawMarketIgnoreSliceABValue = fat.rawdata.MarketIgnoreSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketIgnoreVisitor
    {
		...
		public const string FileName = "MarketIgnoreConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketIgnoreVisitor.SetABDefault单独设定MarketIgnore的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketIgnoreVisitor.SetABFallback单独设定MarketIgnore的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawMarketIgnore>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawMarketIgnore>>(()=>new PBC::RepeatedField<rawMarketIgnore>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawMarketIgnore>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketIgnoreSlice;
			var abMap = v.MarketIgnoreSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketIgnoreSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketIgnoreSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawMarketIgnore> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketIgnoreSlice;
			if (v.MarketIgnoreSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketIgnoreSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketIgnoreSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketIgnoreConf cc) {
			foreach(KeyValuePair <string, rawMarketIgnoreSliceABValue> tagEntry in cc.MarketIgnoreSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketIgnoreSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketIgnoreSliceABValue();
					cc.MarketIgnoreSliceAB[tag] = ps;
				}
                var tagTmp = ps.MarketIgnoreSlice;
				// 基础数据
				for(int i=0; i<cc.MarketIgnoreSlice.Count; ++i) 
				{
					tagTmp.Add(cc.MarketIgnoreSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.MarketIgnoreSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.MarketIgnore> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketIgnoreSlice.MergeFrom(cis);
			Patch(updating.MarketIgnoreSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketIgnoreConf();
            tmp.MergeFrom(cis);
			updating.MarketIgnoreSlice = tmp;
		...
		public static void Range(Action<int, rawMarketIgnore> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawMarketIgnore MustGetByIndex(int index,string tag="") {
		...
		public static rawMarketIgnore? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawMarketIgnore> GetByFilter(Func<rawMarketIgnore,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawMarketIgnore>();
		...
		static public rawMarketIgnore? GetOneByFilter(Func<rawMarketIgnore,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
