...
using EventDecorateInfoConf = fat.rawdata.EventDecorateInfoConf;
using rawEventDecorateInfo = fat.rawdata.EventDecorateInfo;
using rawEventDecorateInfoMapABValue = fat.rawdata.EventDecorateInfoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDecorateInfoVisitor
    {
		...
		public const string FileName = "EventDecorateInfoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDecorateInfoVisitor.SetABDefault单独设定EventDecorateInfo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDecorateInfoVisitor.SetABFallback单独设定EventDecorateInfo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDecorateInfo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDecorateInfo>>(()=> new PBC::MapField<int, rawEventDecorateInfo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDecorateInfo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDecorateInfoMap;
			var abMap = v.EventDecorateInfoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDecorateInfoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDecorateInfoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDecorateInfo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDecorateInfoMap;
			if (v.EventDecorateInfoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDecorateInfoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDecorateInfoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDecorateInfoConf cc) {
			foreach(KeyValuePair <string, rawEventDecorateInfoMapABValue> tagEntry in cc.EventDecorateInfoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDecorateInfoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDecorateInfoMapABValue();
					cc.EventDecorateInfoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDecorateInfoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDecorateInfo> entry in cc.EventDecorateInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDecorateInfo> entry in tagEntry.Value.EventDecorateInfoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDecorateInfo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDecorateInfoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDecorateInfoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDecorateInfoMapAB != null)
			{
				foreach (var entry in cc.EventDecorateInfoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDecorateInfoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDecorateInfoMap.MergeFrom(cis);
			Patch(updating.EventDecorateInfoMap);
			updating.EventDecorateInfoSortedKeys = genSortedKeys(updating.EventDecorateInfoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDecorateInfoConf();
            tmp.MergeFrom(cis);
			updating.EventDecorateInfoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDecorateInfoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDecorateInfo> filter,string tag=""){
		...
		public static rawEventDecorateInfo MustGet(int key,string tag="") {
		...
		public static rawEventDecorateInfo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDecorateInfo> GetByFilter(Func<rawEventDecorateInfo, bool> filterFunc,string tag="") {
			List<rawEventDecorateInfo> ret = new List<rawEventDecorateInfo>();
			foreach(KeyValuePair <int, rawEventDecorateInfo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDecorateInfo? GetOneByFilter(Func<rawEventDecorateInfo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDecorateInfo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDecorateInfo data) 
		{
			return data.Id;			
		}
    }
