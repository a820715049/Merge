...
using EventAreaMilestoneConf = fat.rawdata.EventAreaMilestoneConf;
using rawEventAreaMilestone = fat.rawdata.EventAreaMilestone;
using rawEventAreaMilestoneMapABValue = fat.rawdata.EventAreaMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventAreaMilestoneVisitor
    {
		...
		public const string FileName = "EventAreaMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventAreaMilestoneVisitor.SetABDefault单独设定EventAreaMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventAreaMilestoneVisitor.SetABFallback单独设定EventAreaMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventAreaMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventAreaMilestone>>(()=> new PBC::MapField<int, rawEventAreaMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventAreaMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventAreaMilestoneMap;
			var abMap = v.EventAreaMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventAreaMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventAreaMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventAreaMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventAreaMilestoneMap;
			if (v.EventAreaMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventAreaMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventAreaMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventAreaMilestoneConf cc) {
			foreach(KeyValuePair <string, rawEventAreaMilestoneMapABValue> tagEntry in cc.EventAreaMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventAreaMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventAreaMilestoneMapABValue();
					cc.EventAreaMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.EventAreaMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventAreaMilestone> entry in cc.EventAreaMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventAreaMilestone> entry in tagEntry.Value.EventAreaMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventAreaMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventAreaMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventAreaMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventAreaMilestoneMapAB != null)
			{
				foreach (var entry in cc.EventAreaMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventAreaMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventAreaMilestoneMap.MergeFrom(cis);
			Patch(updating.EventAreaMilestoneMap);
			updating.EventAreaMilestoneSortedKeys = genSortedKeys(updating.EventAreaMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventAreaMilestoneConf();
            tmp.MergeFrom(cis);
			updating.EventAreaMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventAreaMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventAreaMilestone> filter,string tag=""){
		...
		public static rawEventAreaMilestone MustGet(int key,string tag="") {
		...
		public static rawEventAreaMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventAreaMilestone> GetByFilter(Func<rawEventAreaMilestone, bool> filterFunc,string tag="") {
			List<rawEventAreaMilestone> ret = new List<rawEventAreaMilestone>();
			foreach(KeyValuePair <int, rawEventAreaMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventAreaMilestone? GetOneByFilter(Func<rawEventAreaMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventAreaMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventAreaMilestone data) 
		{
			return data.Id;			
		}
    }
