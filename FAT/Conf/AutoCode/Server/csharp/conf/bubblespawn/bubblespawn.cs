...
using BubbleSpawnConf = fat.rawdata.BubbleSpawnConf;
using rawBubbleSpawn = fat.rawdata.BubbleSpawn;
using rawBubbleSpawnSliceABValue = fat.rawdata.BubbleSpawnSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BubbleSpawnVisitor
    {
		...
		public const string FileName = "BubbleSpawnConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BubbleSpawnVisitor.SetABDefault单独设定BubbleSpawn的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BubbleSpawnVisitor.SetABFallback单独设定BubbleSpawn的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawBubbleSpawn>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawBubbleSpawn>>(()=>new PBC::RepeatedField<rawBubbleSpawn>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawBubbleSpawn>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BubbleSpawnSlice;
			var abMap = v.BubbleSpawnSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BubbleSpawnSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BubbleSpawnSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawBubbleSpawn> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BubbleSpawnSlice;
			if (v.BubbleSpawnSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BubbleSpawnSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBubbleSpawnSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BubbleSpawnConf cc) {
			foreach(KeyValuePair <string, rawBubbleSpawnSliceABValue> tagEntry in cc.BubbleSpawnSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BubbleSpawnSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBubbleSpawnSliceABValue();
					cc.BubbleSpawnSliceAB[tag] = ps;
				}
                var tagTmp = ps.BubbleSpawnSlice;
				// 基础数据
				for(int i=0; i<cc.BubbleSpawnSlice.Count; ++i) 
				{
					tagTmp.Add(cc.BubbleSpawnSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.BubbleSpawnSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.BubbleSpawn> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BubbleSpawnSlice.MergeFrom(cis);
			Patch(updating.BubbleSpawnSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new BubbleSpawnConf();
            tmp.MergeFrom(cis);
			updating.BubbleSpawnSlice = tmp;
		...
		public static void Range(Action<int, rawBubbleSpawn> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawBubbleSpawn MustGetByIndex(int index,string tag="") {
		...
		public static rawBubbleSpawn? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawBubbleSpawn> GetByFilter(Func<rawBubbleSpawn,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawBubbleSpawn>();
		...
		static public rawBubbleSpawn? GetOneByFilter(Func<rawBubbleSpawn,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
