...
using PlotDialogConf = fat.rawdata.PlotDialogConf;
using rawPlotDialog = fat.rawdata.PlotDialog;
using rawPlotDialogMapABValue = fat.rawdata.PlotDialogMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PlotDialogVisitor
    {
		...
		public const string FileName = "PlotDialogConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PlotDialogVisitor.SetABDefault单独设定PlotDialog的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PlotDialogVisitor.SetABFallback单独设定PlotDialog的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPlotDialog>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPlotDialog>>(()=> new PBC::MapField<int, rawPlotDialog>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPlotDialog>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PlotDialogMap;
			var abMap = v.PlotDialogMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PlotDialogMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PlotDialogMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPlotDialog> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PlotDialogMap;
			if (v.PlotDialogMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PlotDialogMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPlotDialogMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PlotDialogConf cc) {
			foreach(KeyValuePair <string, rawPlotDialogMapABValue> tagEntry in cc.PlotDialogMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PlotDialogMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPlotDialogMapABValue();
					cc.PlotDialogMapAB[tag] = ps;
				}
                var tagTmp = ps.PlotDialogMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPlotDialog> entry in cc.PlotDialogMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPlotDialog> entry in tagEntry.Value.PlotDialogMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.PlotDialog> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PlotDialogConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PlotDialogMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PlotDialogMapAB != null)
			{
				foreach (var entry in cc.PlotDialogMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PlotDialogMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PlotDialogMap.MergeFrom(cis);
			Patch(updating.PlotDialogMap);
			updating.PlotDialogSortedKeys = genSortedKeys(updating.PlotDialogMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PlotDialogConf();
            tmp.MergeFrom(cis);
			updating.PlotDialogMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PlotDialogSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPlotDialog> filter,string tag=""){
		...
		public static rawPlotDialog MustGet(int key,string tag="") {
		...
		public static rawPlotDialog? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPlotDialog> GetByFilter(Func<rawPlotDialog, bool> filterFunc,string tag="") {
			List<rawPlotDialog> ret = new List<rawPlotDialog>();
			foreach(KeyValuePair <int, rawPlotDialog> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPlotDialog? GetOneByFilter(Func<rawPlotDialog, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPlotDialog> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPlotDialog data) 
		{
			return data.Id;			
		}
    }
