...
using ReplaceBundleConf = fat.rawdata.ReplaceBundleConf;
using rawReplaceBundle = fat.rawdata.ReplaceBundle;
using rawReplaceBundleMapABValue = fat.rawdata.ReplaceBundleMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ReplaceBundleVisitor
    {
		...
		public const string FileName = "ReplaceBundleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ReplaceBundleVisitor.SetABDefault单独设定ReplaceBundle的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ReplaceBundleVisitor.SetABFallback单独设定ReplaceBundle的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawReplaceBundle>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawReplaceBundle>>(()=> new PBC::MapField<int, rawReplaceBundle>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawReplaceBundle>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ReplaceBundleMap;
			var abMap = v.ReplaceBundleMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ReplaceBundleMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ReplaceBundleMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawReplaceBundle> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ReplaceBundleMap;
			if (v.ReplaceBundleMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ReplaceBundleMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawReplaceBundleMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ReplaceBundleConf cc) {
			foreach(KeyValuePair <string, rawReplaceBundleMapABValue> tagEntry in cc.ReplaceBundleMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ReplaceBundleMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawReplaceBundleMapABValue();
					cc.ReplaceBundleMapAB[tag] = ps;
				}
                var tagTmp = ps.ReplaceBundleMap;
				// 基础数据
				foreach(KeyValuePair <int, rawReplaceBundle> entry in cc.ReplaceBundleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawReplaceBundle> entry in tagEntry.Value.ReplaceBundleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ReplaceBundle> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ReplaceBundleConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ReplaceBundleMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ReplaceBundleMapAB != null)
			{
				foreach (var entry in cc.ReplaceBundleMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ReplaceBundleMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ReplaceBundleMap.MergeFrom(cis);
			Patch(updating.ReplaceBundleMap);
			updating.ReplaceBundleSortedKeys = genSortedKeys(updating.ReplaceBundleMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ReplaceBundleConf();
            tmp.MergeFrom(cis);
			updating.ReplaceBundleMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ReplaceBundleSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawReplaceBundle> filter,string tag=""){
		...
		public static rawReplaceBundle MustGet(int key,string tag="") {
		...
		public static rawReplaceBundle? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawReplaceBundle> GetByFilter(Func<rawReplaceBundle, bool> filterFunc,string tag="") {
			List<rawReplaceBundle> ret = new List<rawReplaceBundle>();
			foreach(KeyValuePair <int, rawReplaceBundle> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawReplaceBundle? GetOneByFilter(Func<rawReplaceBundle, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawReplaceBundle> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawReplaceBundle data) 
		{
			return data.Id;			
		}
    }
