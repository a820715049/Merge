...
using MiniGameBeadsStageConf = fat.rawdata.MiniGameBeadsStageConf;
using rawMiniGameBeadsStage = fat.rawdata.MiniGameBeadsStage;
using rawMiniGameBeadsStageMapABValue = fat.rawdata.MiniGameBeadsStageMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameBeadsStageVisitor
    {
		...
		public const string FileName = "MiniGameBeadsStageConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameBeadsStageVisitor.SetABDefault单独设定MiniGameBeadsStage的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameBeadsStageVisitor.SetABFallback单独设定MiniGameBeadsStage的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameBeadsStage>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameBeadsStage>>(()=> new PBC::MapField<int, rawMiniGameBeadsStage>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameBeadsStage>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameBeadsStageMap;
			var abMap = v.MiniGameBeadsStageMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameBeadsStageMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameBeadsStageMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameBeadsStage> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameBeadsStageMap;
			if (v.MiniGameBeadsStageMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameBeadsStageMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameBeadsStageMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameBeadsStageConf cc) {
			foreach(KeyValuePair <string, rawMiniGameBeadsStageMapABValue> tagEntry in cc.MiniGameBeadsStageMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameBeadsStageMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameBeadsStageMapABValue();
					cc.MiniGameBeadsStageMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameBeadsStageMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameBeadsStage> entry in cc.MiniGameBeadsStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameBeadsStage> entry in tagEntry.Value.MiniGameBeadsStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameBeadsStage> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameBeadsStageConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameBeadsStageMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameBeadsStageMapAB != null)
			{
				foreach (var entry in cc.MiniGameBeadsStageMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameBeadsStageMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameBeadsStageMap.MergeFrom(cis);
			Patch(updating.MiniGameBeadsStageMap);
			updating.MiniGameBeadsStageSortedKeys = genSortedKeys(updating.MiniGameBeadsStageMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameBeadsStageConf();
            tmp.MergeFrom(cis);
			updating.MiniGameBeadsStageMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameBeadsStageSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameBeadsStage> filter,string tag=""){
		...
		public static rawMiniGameBeadsStage MustGet(int key,string tag="") {
		...
		public static rawMiniGameBeadsStage? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameBeadsStage> GetByFilter(Func<rawMiniGameBeadsStage, bool> filterFunc,string tag="") {
			List<rawMiniGameBeadsStage> ret = new List<rawMiniGameBeadsStage>();
			foreach(KeyValuePair <int, rawMiniGameBeadsStage> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameBeadsStage? GetOneByFilter(Func<rawMiniGameBeadsStage, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameBeadsStage> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameBeadsStage data) 
		{
			return data.Id;			
		}
    }
