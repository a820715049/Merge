...
using DailyEventMilestoneConf = fat.rawdata.DailyEventMilestoneConf;
using rawDailyEventMilestone = fat.rawdata.DailyEventMilestone;
using rawDailyEventMilestoneMapABValue = fat.rawdata.DailyEventMilestoneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DailyEventMilestoneVisitor
    {
		...
		public const string FileName = "DailyEventMilestoneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DailyEventMilestoneVisitor.SetABDefault单独设定DailyEventMilestone的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DailyEventMilestoneVisitor.SetABFallback单独设定DailyEventMilestone的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDailyEventMilestone>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDailyEventMilestone>>(()=> new PBC::MapField<int, rawDailyEventMilestone>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDailyEventMilestone>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DailyEventMilestoneMap;
			var abMap = v.DailyEventMilestoneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DailyEventMilestoneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DailyEventMilestoneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDailyEventMilestone> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DailyEventMilestoneMap;
			if (v.DailyEventMilestoneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DailyEventMilestoneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDailyEventMilestoneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DailyEventMilestoneConf cc) {
			foreach(KeyValuePair <string, rawDailyEventMilestoneMapABValue> tagEntry in cc.DailyEventMilestoneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DailyEventMilestoneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDailyEventMilestoneMapABValue();
					cc.DailyEventMilestoneMapAB[tag] = ps;
				}
                var tagTmp = ps.DailyEventMilestoneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDailyEventMilestone> entry in cc.DailyEventMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDailyEventMilestone> entry in tagEntry.Value.DailyEventMilestoneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DailyEventMilestone> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DailyEventMilestoneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DailyEventMilestoneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DailyEventMilestoneMapAB != null)
			{
				foreach (var entry in cc.DailyEventMilestoneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DailyEventMilestoneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DailyEventMilestoneMap.MergeFrom(cis);
			Patch(updating.DailyEventMilestoneMap);
			updating.DailyEventMilestoneSortedKeys = genSortedKeys(updating.DailyEventMilestoneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DailyEventMilestoneConf();
            tmp.MergeFrom(cis);
			updating.DailyEventMilestoneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DailyEventMilestoneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDailyEventMilestone> filter,string tag=""){
		...
		public static rawDailyEventMilestone MustGet(int key,string tag="") {
		...
		public static rawDailyEventMilestone? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDailyEventMilestone> GetByFilter(Func<rawDailyEventMilestone, bool> filterFunc,string tag="") {
			List<rawDailyEventMilestone> ret = new List<rawDailyEventMilestone>();
			foreach(KeyValuePair <int, rawDailyEventMilestone> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDailyEventMilestone? GetOneByFilter(Func<rawDailyEventMilestone, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDailyEventMilestone> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDailyEventMilestone data) 
		{
			return data.Id;			
		}
    }
