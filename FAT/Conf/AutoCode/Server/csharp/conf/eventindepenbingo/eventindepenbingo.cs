...
using EventIndepenBingoConf = fat.rawdata.EventIndepenBingoConf;
using rawEventIndepenBingo = fat.rawdata.EventIndepenBingo;
using rawEventIndepenBingoMapABValue = fat.rawdata.EventIndepenBingoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventIndepenBingoVisitor
    {
		...
		public const string FileName = "EventIndepenBingoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventIndepenBingoVisitor.SetABDefault单独设定EventIndepenBingo的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventIndepenBingoVisitor.SetABFallback单独设定EventIndepenBingo的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventIndepenBingo>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventIndepenBingo>>(()=> new PBC::MapField<int, rawEventIndepenBingo>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventIndepenBingo>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventIndepenBingoMap;
			var abMap = v.EventIndepenBingoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventIndepenBingoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventIndepenBingoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventIndepenBingo> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventIndepenBingoMap;
			if (v.EventIndepenBingoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventIndepenBingoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventIndepenBingoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventIndepenBingoConf cc) {
			foreach(KeyValuePair <string, rawEventIndepenBingoMapABValue> tagEntry in cc.EventIndepenBingoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventIndepenBingoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventIndepenBingoMapABValue();
					cc.EventIndepenBingoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventIndepenBingoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventIndepenBingo> entry in cc.EventIndepenBingoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventIndepenBingo> entry in tagEntry.Value.EventIndepenBingoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventIndepenBingo> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventIndepenBingoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventIndepenBingoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventIndepenBingoMapAB != null)
			{
				foreach (var entry in cc.EventIndepenBingoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventIndepenBingoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventIndepenBingoMap.MergeFrom(cis);
			Patch(updating.EventIndepenBingoMap);
			updating.EventIndepenBingoSortedKeys = genSortedKeys(updating.EventIndepenBingoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventIndepenBingoConf();
            tmp.MergeFrom(cis);
			updating.EventIndepenBingoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventIndepenBingoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventIndepenBingo> filter,string tag=""){
		...
		public static rawEventIndepenBingo MustGet(int key,string tag="") {
		...
		public static rawEventIndepenBingo? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventIndepenBingo> GetByFilter(Func<rawEventIndepenBingo, bool> filterFunc,string tag="") {
			List<rawEventIndepenBingo> ret = new List<rawEventIndepenBingo>();
			foreach(KeyValuePair <int, rawEventIndepenBingo> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventIndepenBingo? GetOneByFilter(Func<rawEventIndepenBingo, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventIndepenBingo> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventIndepenBingo data) 
		{
			return data.Id;			
		}
    }
