...
using GameDiffConf = fat.rawdata.GameDiffConf;
using rawGameDiff = fat.rawdata.GameDiff;
using rawGameDiffSliceABValue = fat.rawdata.GameDiffSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GameDiffVisitor
    {
		...
		public const string FileName = "GameDiffConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GameDiffVisitor.SetABDefault单独设定GameDiff的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GameDiffVisitor.SetABFallback单独设定GameDiff的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawGameDiff>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawGameDiff>>(()=>new PBC::RepeatedField<rawGameDiff>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawGameDiff>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GameDiffSlice;
			var abMap = v.GameDiffSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GameDiffSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GameDiffSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawGameDiff> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GameDiffSlice;
			if (v.GameDiffSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GameDiffSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGameDiffSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GameDiffConf cc) {
			foreach(KeyValuePair <string, rawGameDiffSliceABValue> tagEntry in cc.GameDiffSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GameDiffSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGameDiffSliceABValue();
					cc.GameDiffSliceAB[tag] = ps;
				}
                var tagTmp = ps.GameDiffSlice;
				// 基础数据
				for(int i=0; i<cc.GameDiffSlice.Count; ++i) 
				{
					tagTmp.Add(cc.GameDiffSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.GameDiffSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.GameDiff> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GameDiffSlice.MergeFrom(cis);
			Patch(updating.GameDiffSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GameDiffConf();
            tmp.MergeFrom(cis);
			updating.GameDiffSlice = tmp;
		...
		public static void Range(Action<int, rawGameDiff> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawGameDiff MustGetByIndex(int index,string tag="") {
		...
		public static rawGameDiff? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawGameDiff> GetByFilter(Func<rawGameDiff,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawGameDiff>();
		...
		static public rawGameDiff? GetOneByFilter(Func<rawGameDiff,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
