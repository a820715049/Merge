...
using ComTrigAutoDetailConf = fat.rawdata.ComTrigAutoDetailConf;
using rawComTrigAutoDetail = fat.rawdata.ComTrigAutoDetail;
using rawComTrigAutoDetailMapABValue = fat.rawdata.ComTrigAutoDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComTrigAutoDetailVisitor
    {
		...
		public const string FileName = "ComTrigAutoDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComTrigAutoDetailVisitor.SetABDefault单独设定ComTrigAutoDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComTrigAutoDetailVisitor.SetABFallback单独设定ComTrigAutoDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComTrigAutoDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComTrigAutoDetail>>(()=> new PBC::MapField<int, rawComTrigAutoDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComTrigAutoDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComTrigAutoDetailMap;
			var abMap = v.ComTrigAutoDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComTrigAutoDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComTrigAutoDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComTrigAutoDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComTrigAutoDetailMap;
			if (v.ComTrigAutoDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComTrigAutoDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComTrigAutoDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComTrigAutoDetailConf cc) {
			foreach(KeyValuePair <string, rawComTrigAutoDetailMapABValue> tagEntry in cc.ComTrigAutoDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComTrigAutoDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComTrigAutoDetailMapABValue();
					cc.ComTrigAutoDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.ComTrigAutoDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComTrigAutoDetail> entry in cc.ComTrigAutoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComTrigAutoDetail> entry in tagEntry.Value.ComTrigAutoDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComTrigAutoDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComTrigAutoDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComTrigAutoDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComTrigAutoDetailMapAB != null)
			{
				foreach (var entry in cc.ComTrigAutoDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComTrigAutoDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComTrigAutoDetailMap.MergeFrom(cis);
			Patch(updating.ComTrigAutoDetailMap);
			updating.ComTrigAutoDetailSortedKeys = genSortedKeys(updating.ComTrigAutoDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComTrigAutoDetailConf();
            tmp.MergeFrom(cis);
			updating.ComTrigAutoDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComTrigAutoDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComTrigAutoDetail> filter,string tag=""){
		...
		public static rawComTrigAutoDetail MustGet(int key,string tag="") {
		...
		public static rawComTrigAutoDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComTrigAutoDetail> GetByFilter(Func<rawComTrigAutoDetail, bool> filterFunc,string tag="") {
			List<rawComTrigAutoDetail> ret = new List<rawComTrigAutoDetail>();
			foreach(KeyValuePair <int, rawComTrigAutoDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComTrigAutoDetail? GetOneByFilter(Func<rawComTrigAutoDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComTrigAutoDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComTrigAutoDetail data) 
		{
			return data.Id;			
		}
    }
