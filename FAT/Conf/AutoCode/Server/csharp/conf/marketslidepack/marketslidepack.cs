...
using MarketSlidePackConf = fat.rawdata.MarketSlidePackConf;
using rawMarketSlidePack = fat.rawdata.MarketSlidePack;
using rawMarketSlidePackMapABValue = fat.rawdata.MarketSlidePackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketSlidePackVisitor
    {
		...
		public const string FileName = "MarketSlidePackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketSlidePackVisitor.SetABDefault单独设定MarketSlidePack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketSlidePackVisitor.SetABFallback单独设定MarketSlidePack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketSlidePack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketSlidePack>>(()=> new PBC::MapField<int, rawMarketSlidePack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketSlidePack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketSlidePackMap;
			var abMap = v.MarketSlidePackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketSlidePackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketSlidePackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketSlidePack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketSlidePackMap;
			if (v.MarketSlidePackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketSlidePackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketSlidePackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketSlidePackConf cc) {
			foreach(KeyValuePair <string, rawMarketSlidePackMapABValue> tagEntry in cc.MarketSlidePackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketSlidePackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketSlidePackMapABValue();
					cc.MarketSlidePackMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketSlidePackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketSlidePack> entry in cc.MarketSlidePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketSlidePack> entry in tagEntry.Value.MarketSlidePackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketSlidePack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketSlidePackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketSlidePackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketSlidePackMapAB != null)
			{
				foreach (var entry in cc.MarketSlidePackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketSlidePackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketSlidePackMap.MergeFrom(cis);
			Patch(updating.MarketSlidePackMap);
			updating.MarketSlidePackSortedKeys = genSortedKeys(updating.MarketSlidePackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketSlidePackConf();
            tmp.MergeFrom(cis);
			updating.MarketSlidePackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketSlidePackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketSlidePack> filter,string tag=""){
		...
		public static rawMarketSlidePack MustGet(int key,string tag="") {
		...
		public static rawMarketSlidePack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketSlidePack> GetByFilter(Func<rawMarketSlidePack, bool> filterFunc,string tag="") {
			List<rawMarketSlidePack> ret = new List<rawMarketSlidePack>();
			foreach(KeyValuePair <int, rawMarketSlidePack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketSlidePack? GetOneByFilter(Func<rawMarketSlidePack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketSlidePack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketSlidePack data) 
		{
			return data.Id;			
		}
    }
