...
using LandmarkDetailConf = fat.rawdata.LandmarkDetailConf;
using rawLandmarkDetail = fat.rawdata.LandmarkDetail;
using rawLandmarkDetailMapABValue = fat.rawdata.LandmarkDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LandmarkDetailVisitor
    {
		...
		public const string FileName = "LandmarkDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LandmarkDetailVisitor.SetABDefault单独设定LandmarkDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LandmarkDetailVisitor.SetABFallback单独设定LandmarkDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawLandmarkDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawLandmarkDetail>>(()=> new PBC::MapField<int, rawLandmarkDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawLandmarkDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LandmarkDetailMap;
			var abMap = v.LandmarkDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LandmarkDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LandmarkDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawLandmarkDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LandmarkDetailMap;
			if (v.LandmarkDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LandmarkDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLandmarkDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LandmarkDetailConf cc) {
			foreach(KeyValuePair <string, rawLandmarkDetailMapABValue> tagEntry in cc.LandmarkDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LandmarkDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLandmarkDetailMapABValue();
					cc.LandmarkDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.LandmarkDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawLandmarkDetail> entry in cc.LandmarkDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawLandmarkDetail> entry in tagEntry.Value.LandmarkDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.LandmarkDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(LandmarkDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.LandmarkDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.LandmarkDetailMapAB != null)
			{
				foreach (var entry in cc.LandmarkDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.LandmarkDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LandmarkDetailMap.MergeFrom(cis);
			Patch(updating.LandmarkDetailMap);
			updating.LandmarkDetailSortedKeys = genSortedKeys(updating.LandmarkDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LandmarkDetailConf();
            tmp.MergeFrom(cis);
			updating.LandmarkDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().LandmarkDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawLandmarkDetail> filter,string tag=""){
		...
		public static rawLandmarkDetail MustGet(int key,string tag="") {
		...
		public static rawLandmarkDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawLandmarkDetail> GetByFilter(Func<rawLandmarkDetail, bool> filterFunc,string tag="") {
			List<rawLandmarkDetail> ret = new List<rawLandmarkDetail>();
			foreach(KeyValuePair <int, rawLandmarkDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawLandmarkDetail? GetOneByFilter(Func<rawLandmarkDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawLandmarkDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawLandmarkDetail data) 
		{
			return data.Id;			
		}
    }
