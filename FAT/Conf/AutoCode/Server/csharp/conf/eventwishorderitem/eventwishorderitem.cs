...
using EventWishOrderItemConf = fat.rawdata.EventWishOrderItemConf;
using rawEventWishOrderItem = fat.rawdata.EventWishOrderItem;
using rawEventWishOrderItemMapABValue = fat.rawdata.EventWishOrderItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishOrderItemVisitor
    {
		...
		public const string FileName = "EventWishOrderItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishOrderItemVisitor.SetABDefault单独设定EventWishOrderItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishOrderItemVisitor.SetABFallback单独设定EventWishOrderItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishOrderItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishOrderItem>>(()=> new PBC::MapField<int, rawEventWishOrderItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishOrderItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishOrderItemMap;
			var abMap = v.EventWishOrderItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishOrderItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishOrderItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishOrderItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishOrderItemMap;
			if (v.EventWishOrderItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishOrderItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishOrderItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishOrderItemConf cc) {
			foreach(KeyValuePair <string, rawEventWishOrderItemMapABValue> tagEntry in cc.EventWishOrderItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishOrderItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishOrderItemMapABValue();
					cc.EventWishOrderItemMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishOrderItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishOrderItem> entry in cc.EventWishOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishOrderItem> entry in tagEntry.Value.EventWishOrderItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishOrderItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishOrderItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishOrderItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishOrderItemMapAB != null)
			{
				foreach (var entry in cc.EventWishOrderItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishOrderItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishOrderItemMap.MergeFrom(cis);
			Patch(updating.EventWishOrderItemMap);
			updating.EventWishOrderItemSortedKeys = genSortedKeys(updating.EventWishOrderItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishOrderItemConf();
            tmp.MergeFrom(cis);
			updating.EventWishOrderItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishOrderItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishOrderItem> filter,string tag=""){
		...
		public static rawEventWishOrderItem MustGet(int key,string tag="") {
		...
		public static rawEventWishOrderItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishOrderItem> GetByFilter(Func<rawEventWishOrderItem, bool> filterFunc,string tag="") {
			List<rawEventWishOrderItem> ret = new List<rawEventWishOrderItem>();
			foreach(KeyValuePair <int, rawEventWishOrderItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishOrderItem? GetOneByFilter(Func<rawEventWishOrderItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishOrderItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishOrderItem data) 
		{
			return data.Id;			
		}
    }
