...
using EventMineCartRoundConf = fat.rawdata.EventMineCartRoundConf;
using rawEventMineCartRound = fat.rawdata.EventMineCartRound;
using rawEventMineCartRoundMapABValue = fat.rawdata.EventMineCartRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartRoundVisitor
    {
		...
		public const string FileName = "EventMineCartRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartRoundVisitor.SetABDefault单独设定EventMineCartRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartRoundVisitor.SetABFallback单独设定EventMineCartRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartRound>>(()=> new PBC::MapField<int, rawEventMineCartRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartRoundMap;
			var abMap = v.EventMineCartRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartRoundMap;
			if (v.EventMineCartRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartRoundConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartRoundMapABValue> tagEntry in cc.EventMineCartRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartRoundMapABValue();
					cc.EventMineCartRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartRound> entry in cc.EventMineCartRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartRound> entry in tagEntry.Value.EventMineCartRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartRoundMapAB != null)
			{
				foreach (var entry in cc.EventMineCartRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartRoundMap.MergeFrom(cis);
			Patch(updating.EventMineCartRoundMap);
			updating.EventMineCartRoundSortedKeys = genSortedKeys(updating.EventMineCartRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartRoundConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartRound> filter,string tag=""){
		...
		public static rawEventMineCartRound MustGet(int key,string tag="") {
		...
		public static rawEventMineCartRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartRound> GetByFilter(Func<rawEventMineCartRound, bool> filterFunc,string tag="") {
			List<rawEventMineCartRound> ret = new List<rawEventMineCartRound>();
			foreach(KeyValuePair <int, rawEventMineCartRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartRound? GetOneByFilter(Func<rawEventMineCartRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartRound data) 
		{
			return data.Id;			
		}
    }
