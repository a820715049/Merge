...
using EventWishBoardDetailConf = fat.rawdata.EventWishBoardDetailConf;
using rawEventWishBoardDetail = fat.rawdata.EventWishBoardDetail;
using rawEventWishBoardDetailMapABValue = fat.rawdata.EventWishBoardDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishBoardDetailVisitor
    {
		...
		public const string FileName = "EventWishBoardDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishBoardDetailVisitor.SetABDefault单独设定EventWishBoardDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishBoardDetailVisitor.SetABFallback单独设定EventWishBoardDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishBoardDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishBoardDetail>>(()=> new PBC::MapField<int, rawEventWishBoardDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishBoardDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishBoardDetailMap;
			var abMap = v.EventWishBoardDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishBoardDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishBoardDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishBoardDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishBoardDetailMap;
			if (v.EventWishBoardDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishBoardDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishBoardDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishBoardDetailConf cc) {
			foreach(KeyValuePair <string, rawEventWishBoardDetailMapABValue> tagEntry in cc.EventWishBoardDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishBoardDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishBoardDetailMapABValue();
					cc.EventWishBoardDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishBoardDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishBoardDetail> entry in cc.EventWishBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishBoardDetail> entry in tagEntry.Value.EventWishBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishBoardDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishBoardDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishBoardDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishBoardDetailMapAB != null)
			{
				foreach (var entry in cc.EventWishBoardDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishBoardDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishBoardDetailMap.MergeFrom(cis);
			Patch(updating.EventWishBoardDetailMap);
			updating.EventWishBoardDetailSortedKeys = genSortedKeys(updating.EventWishBoardDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishBoardDetailConf();
            tmp.MergeFrom(cis);
			updating.EventWishBoardDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishBoardDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishBoardDetail> filter,string tag=""){
		...
		public static rawEventWishBoardDetail MustGet(int key,string tag="") {
		...
		public static rawEventWishBoardDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishBoardDetail> GetByFilter(Func<rawEventWishBoardDetail, bool> filterFunc,string tag="") {
			List<rawEventWishBoardDetail> ret = new List<rawEventWishBoardDetail>();
			foreach(KeyValuePair <int, rawEventWishBoardDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishBoardDetail? GetOneByFilter(Func<rawEventWishBoardDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishBoardDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishBoardDetail data) 
		{
			return data.Id;			
		}
    }
