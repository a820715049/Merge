...
using EventMineBoardDetailConf = fat.rawdata.EventMineBoardDetailConf;
using rawEventMineBoardDetail = fat.rawdata.EventMineBoardDetail;
using rawEventMineBoardDetailMapABValue = fat.rawdata.EventMineBoardDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineBoardDetailVisitor
    {
		...
		public const string FileName = "EventMineBoardDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineBoardDetailVisitor.SetABDefault单独设定EventMineBoardDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineBoardDetailVisitor.SetABFallback单独设定EventMineBoardDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineBoardDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineBoardDetail>>(()=> new PBC::MapField<int, rawEventMineBoardDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineBoardDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineBoardDetailMap;
			var abMap = v.EventMineBoardDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineBoardDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineBoardDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineBoardDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineBoardDetailMap;
			if (v.EventMineBoardDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineBoardDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineBoardDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineBoardDetailConf cc) {
			foreach(KeyValuePair <string, rawEventMineBoardDetailMapABValue> tagEntry in cc.EventMineBoardDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineBoardDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineBoardDetailMapABValue();
					cc.EventMineBoardDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineBoardDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineBoardDetail> entry in cc.EventMineBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineBoardDetail> entry in tagEntry.Value.EventMineBoardDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineBoardDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineBoardDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineBoardDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineBoardDetailMapAB != null)
			{
				foreach (var entry in cc.EventMineBoardDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineBoardDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineBoardDetailMap.MergeFrom(cis);
			Patch(updating.EventMineBoardDetailMap);
			updating.EventMineBoardDetailSortedKeys = genSortedKeys(updating.EventMineBoardDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineBoardDetailConf();
            tmp.MergeFrom(cis);
			updating.EventMineBoardDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineBoardDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineBoardDetail> filter,string tag=""){
		...
		public static rawEventMineBoardDetail MustGet(int key,string tag="") {
		...
		public static rawEventMineBoardDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineBoardDetail> GetByFilter(Func<rawEventMineBoardDetail, bool> filterFunc,string tag="") {
			List<rawEventMineBoardDetail> ret = new List<rawEventMineBoardDetail>();
			foreach(KeyValuePair <int, rawEventMineBoardDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineBoardDetail? GetOneByFilter(Func<rawEventMineBoardDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineBoardDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineBoardDetail data) 
		{
			return data.Id;			
		}
    }
