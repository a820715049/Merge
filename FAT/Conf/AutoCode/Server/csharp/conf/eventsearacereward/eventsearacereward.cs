...
using EventSeaRaceRewardConf = fat.rawdata.EventSeaRaceRewardConf;
using rawEventSeaRaceReward = fat.rawdata.EventSeaRaceReward;
using rawEventSeaRaceRewardMapABValue = fat.rawdata.EventSeaRaceRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventSeaRaceRewardVisitor
    {
		...
		public const string FileName = "EventSeaRaceRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventSeaRaceRewardVisitor.SetABDefault单独设定EventSeaRaceReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventSeaRaceRewardVisitor.SetABFallback单独设定EventSeaRaceReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventSeaRaceReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventSeaRaceReward>>(()=> new PBC::MapField<int, rawEventSeaRaceReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventSeaRaceReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventSeaRaceRewardMap;
			var abMap = v.EventSeaRaceRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventSeaRaceRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventSeaRaceRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventSeaRaceReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventSeaRaceRewardMap;
			if (v.EventSeaRaceRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventSeaRaceRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventSeaRaceRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventSeaRaceRewardConf cc) {
			foreach(KeyValuePair <string, rawEventSeaRaceRewardMapABValue> tagEntry in cc.EventSeaRaceRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventSeaRaceRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventSeaRaceRewardMapABValue();
					cc.EventSeaRaceRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventSeaRaceRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventSeaRaceReward> entry in cc.EventSeaRaceRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventSeaRaceReward> entry in tagEntry.Value.EventSeaRaceRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventSeaRaceReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventSeaRaceRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventSeaRaceRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventSeaRaceRewardMapAB != null)
			{
				foreach (var entry in cc.EventSeaRaceRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventSeaRaceRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventSeaRaceRewardMap.MergeFrom(cis);
			Patch(updating.EventSeaRaceRewardMap);
			updating.EventSeaRaceRewardSortedKeys = genSortedKeys(updating.EventSeaRaceRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventSeaRaceRewardConf();
            tmp.MergeFrom(cis);
			updating.EventSeaRaceRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventSeaRaceRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventSeaRaceReward> filter,string tag=""){
		...
		public static rawEventSeaRaceReward MustGet(int key,string tag="") {
		...
		public static rawEventSeaRaceReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventSeaRaceReward> GetByFilter(Func<rawEventSeaRaceReward, bool> filterFunc,string tag="") {
			List<rawEventSeaRaceReward> ret = new List<rawEventSeaRaceReward>();
			foreach(KeyValuePair <int, rawEventSeaRaceReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventSeaRaceReward? GetOneByFilter(Func<rawEventSeaRaceReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventSeaRaceReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventSeaRaceReward data) 
		{
			return data.Id;			
		}
    }
