...
using EndlessPackDetailConf = fat.rawdata.EndlessPackDetailConf;
using rawEndlessPackDetail = fat.rawdata.EndlessPackDetail;
using rawEndlessPackDetailSliceABValue = fat.rawdata.EndlessPackDetailSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessPackDetailVisitor
    {
		...
		public const string FileName = "EndlessPackDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessPackDetailVisitor.SetABDefault单独设定EndlessPackDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessPackDetailVisitor.SetABFallback单独设定EndlessPackDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawEndlessPackDetail>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawEndlessPackDetail>>(()=>new PBC::RepeatedField<rawEndlessPackDetail>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawEndlessPackDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessPackDetailSlice;
			var abMap = v.EndlessPackDetailSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessPackDetailSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessPackDetailSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawEndlessPackDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessPackDetailSlice;
			if (v.EndlessPackDetailSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessPackDetailSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessPackDetailSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessPackDetailConf cc) {
			foreach(KeyValuePair <string, rawEndlessPackDetailSliceABValue> tagEntry in cc.EndlessPackDetailSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessPackDetailSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessPackDetailSliceABValue();
					cc.EndlessPackDetailSliceAB[tag] = ps;
				}
                var tagTmp = ps.EndlessPackDetailSlice;
				// 基础数据
				for(int i=0; i<cc.EndlessPackDetailSlice.Count; ++i) 
				{
					tagTmp.Add(cc.EndlessPackDetailSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.EndlessPackDetailSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.EndlessPackDetail> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessPackDetailSlice.MergeFrom(cis);
			Patch(updating.EndlessPackDetailSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessPackDetailConf();
            tmp.MergeFrom(cis);
			updating.EndlessPackDetailSlice = tmp;
		...
		public static void Range(Action<int, rawEndlessPackDetail> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawEndlessPackDetail MustGetByIndex(int index,string tag="") {
		...
		public static rawEndlessPackDetail? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawEndlessPackDetail> GetByFilter(Func<rawEndlessPackDetail,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawEndlessPackDetail>();
		...
		static public rawEndlessPackDetail? GetOneByFilter(Func<rawEndlessPackDetail,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
