...
using MergeMixCostConf = fat.rawdata.MergeMixCostConf;
using rawMergeMixCost = fat.rawdata.MergeMixCost;
using rawMergeMixCostMapABValue = fat.rawdata.MergeMixCostMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeMixCostVisitor
    {
		...
		public const string FileName = "MergeMixCostConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeMixCostVisitor.SetABDefault单独设定MergeMixCost的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeMixCostVisitor.SetABFallback单独设定MergeMixCost的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeMixCost>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeMixCost>>(()=> new PBC::MapField<int, rawMergeMixCost>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeMixCost>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeMixCostMap;
			var abMap = v.MergeMixCostMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeMixCostMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeMixCostMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeMixCost> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeMixCostMap;
			if (v.MergeMixCostMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeMixCostMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeMixCostMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeMixCostConf cc) {
			foreach(KeyValuePair <string, rawMergeMixCostMapABValue> tagEntry in cc.MergeMixCostMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeMixCostMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeMixCostMapABValue();
					cc.MergeMixCostMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeMixCostMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeMixCost> entry in cc.MergeMixCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeMixCost> entry in tagEntry.Value.MergeMixCostMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeMixCost> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeMixCostConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeMixCostMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeMixCostMapAB != null)
			{
				foreach (var entry in cc.MergeMixCostMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeMixCostMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeMixCostMap.MergeFrom(cis);
			Patch(updating.MergeMixCostMap);
			updating.MergeMixCostSortedKeys = genSortedKeys(updating.MergeMixCostMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeMixCostConf();
            tmp.MergeFrom(cis);
			updating.MergeMixCostMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeMixCostSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeMixCost> filter,string tag=""){
		...
		public static rawMergeMixCost MustGet(int key,string tag="") {
		...
		public static rawMergeMixCost? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeMixCost> GetByFilter(Func<rawMergeMixCost, bool> filterFunc,string tag="") {
			List<rawMergeMixCost> ret = new List<rawMergeMixCost>();
			foreach(KeyValuePair <int, rawMergeMixCost> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeMixCost? GetOneByFilter(Func<rawMergeMixCost, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeMixCost> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeMixCost data) 
		{
			return data.Id;			
		}
    }
