...
using EventMineGroupConf = fat.rawdata.EventMineGroupConf;
using rawEventMineGroup = fat.rawdata.EventMineGroup;
using rawEventMineGroupMapABValue = fat.rawdata.EventMineGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineGroupVisitor
    {
		...
		public const string FileName = "EventMineGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineGroupVisitor.SetABDefault单独设定EventMineGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineGroupVisitor.SetABFallback单独设定EventMineGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineGroup>>(()=> new PBC::MapField<int, rawEventMineGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineGroupMap;
			var abMap = v.EventMineGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineGroupMap;
			if (v.EventMineGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineGroupConf cc) {
			foreach(KeyValuePair <string, rawEventMineGroupMapABValue> tagEntry in cc.EventMineGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineGroupMapABValue();
					cc.EventMineGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineGroup> entry in cc.EventMineGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineGroup> entry in tagEntry.Value.EventMineGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineGroupMapAB != null)
			{
				foreach (var entry in cc.EventMineGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineGroupMap.MergeFrom(cis);
			Patch(updating.EventMineGroupMap);
			updating.EventMineGroupSortedKeys = genSortedKeys(updating.EventMineGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineGroupConf();
            tmp.MergeFrom(cis);
			updating.EventMineGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineGroup> filter,string tag=""){
		...
		public static rawEventMineGroup MustGet(int key,string tag="") {
		...
		public static rawEventMineGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineGroup> GetByFilter(Func<rawEventMineGroup, bool> filterFunc,string tag="") {
			List<rawEventMineGroup> ret = new List<rawEventMineGroup>();
			foreach(KeyValuePair <int, rawEventMineGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineGroup? GetOneByFilter(Func<rawEventMineGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineGroup data) 
		{
			return data.Id;			
		}
    }
