...
using SevenDayTaskGroupConf = fat.rawdata.SevenDayTaskGroupConf;
using rawSevenDayTaskGroup = fat.rawdata.SevenDayTaskGroup;
using rawSevenDayTaskGroupMapABValue = fat.rawdata.SevenDayTaskGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SevenDayTaskGroupVisitor
    {
		...
		public const string FileName = "SevenDayTaskGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SevenDayTaskGroupVisitor.SetABDefault单独设定SevenDayTaskGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SevenDayTaskGroupVisitor.SetABFallback单独设定SevenDayTaskGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSevenDayTaskGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSevenDayTaskGroup>>(()=> new PBC::MapField<int, rawSevenDayTaskGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSevenDayTaskGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SevenDayTaskGroupMap;
			var abMap = v.SevenDayTaskGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SevenDayTaskGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SevenDayTaskGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSevenDayTaskGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SevenDayTaskGroupMap;
			if (v.SevenDayTaskGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SevenDayTaskGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSevenDayTaskGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SevenDayTaskGroupConf cc) {
			foreach(KeyValuePair <string, rawSevenDayTaskGroupMapABValue> tagEntry in cc.SevenDayTaskGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SevenDayTaskGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSevenDayTaskGroupMapABValue();
					cc.SevenDayTaskGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.SevenDayTaskGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSevenDayTaskGroup> entry in cc.SevenDayTaskGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSevenDayTaskGroup> entry in tagEntry.Value.SevenDayTaskGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SevenDayTaskGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SevenDayTaskGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SevenDayTaskGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SevenDayTaskGroupMapAB != null)
			{
				foreach (var entry in cc.SevenDayTaskGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SevenDayTaskGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SevenDayTaskGroupMap.MergeFrom(cis);
			Patch(updating.SevenDayTaskGroupMap);
			updating.SevenDayTaskGroupSortedKeys = genSortedKeys(updating.SevenDayTaskGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SevenDayTaskGroupConf();
            tmp.MergeFrom(cis);
			updating.SevenDayTaskGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SevenDayTaskGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSevenDayTaskGroup> filter,string tag=""){
		...
		public static rawSevenDayTaskGroup MustGet(int key,string tag="") {
		...
		public static rawSevenDayTaskGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSevenDayTaskGroup> GetByFilter(Func<rawSevenDayTaskGroup, bool> filterFunc,string tag="") {
			List<rawSevenDayTaskGroup> ret = new List<rawSevenDayTaskGroup>();
			foreach(KeyValuePair <int, rawSevenDayTaskGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSevenDayTaskGroup? GetOneByFilter(Func<rawSevenDayTaskGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSevenDayTaskGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSevenDayTaskGroup data) 
		{
			return data.Id;			
		}
    }
