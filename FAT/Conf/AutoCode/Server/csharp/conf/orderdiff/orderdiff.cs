...
using OrderDiffConf = fat.rawdata.OrderDiffConf;
using rawOrderDiff = fat.rawdata.OrderDiff;
using rawOrderDiffMapABValue = fat.rawdata.OrderDiffMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class OrderDiffVisitor
    {
		...
		public const string FileName = "OrderDiffConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过OrderDiffVisitor.SetABDefault单独设定OrderDiff的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过OrderDiffVisitor.SetABFallback单独设定OrderDiff的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawOrderDiff>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawOrderDiff>>(()=> new PBC::MapField<int, rawOrderDiff>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawOrderDiff>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().OrderDiffMap;
			var abMap = v.OrderDiffMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.OrderDiffMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].OrderDiffMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawOrderDiff> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().OrderDiffMap;
			if (v.OrderDiffMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.OrderDiffMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawOrderDiffMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(OrderDiffConf cc) {
			foreach(KeyValuePair <string, rawOrderDiffMapABValue> tagEntry in cc.OrderDiffMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.OrderDiffMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawOrderDiffMapABValue();
					cc.OrderDiffMapAB[tag] = ps;
				}
                var tagTmp = ps.OrderDiffMap;
				// 基础数据
				foreach(KeyValuePair <int, rawOrderDiff> entry in cc.OrderDiffMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawOrderDiff> entry in tagEntry.Value.OrderDiffMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.OrderDiff> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(OrderDiffConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.OrderDiffMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.OrderDiffMapAB != null)
			{
				foreach (var entry in cc.OrderDiffMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.OrderDiffMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.OrderDiffMap.MergeFrom(cis);
			Patch(updating.OrderDiffMap);
			updating.OrderDiffSortedKeys = genSortedKeys(updating.OrderDiffMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new OrderDiffConf();
            tmp.MergeFrom(cis);
			updating.OrderDiffMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().OrderDiffSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawOrderDiff> filter,string tag=""){
		...
		public static rawOrderDiff MustGet(int key,string tag="") {
		...
		public static rawOrderDiff? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawOrderDiff> GetByFilter(Func<rawOrderDiff, bool> filterFunc,string tag="") {
			List<rawOrderDiff> ret = new List<rawOrderDiff>();
			foreach(KeyValuePair <int, rawOrderDiff> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawOrderDiff? GetOneByFilter(Func<rawOrderDiff, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawOrderDiff> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawOrderDiff data) 
		{
			return data.Id;			
		}
    }
