...
using LangEsConf = fat.rawdata.LangEsConf;
using rawLangEs = fat.rawdata.LangEs;
using rawLangEsSliceABValue = fat.rawdata.LangEsSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangEsVisitor
    {
		...
		public const string FileName = "LangEsConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangEsVisitor.SetABDefault单独设定LangEs的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangEsVisitor.SetABFallback单独设定LangEs的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangEs>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangEs>>(()=>new PBC::RepeatedField<rawLangEs>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangEs>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangEsSlice;
			var abMap = v.LangEsSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangEsSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangEsSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangEs> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangEsSlice;
			if (v.LangEsSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangEsSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangEsSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangEsConf cc) {
			foreach(KeyValuePair <string, rawLangEsSliceABValue> tagEntry in cc.LangEsSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangEsSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangEsSliceABValue();
					cc.LangEsSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangEsSlice;
				// 基础数据
				for(int i=0; i<cc.LangEsSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangEsSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangEsSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangEs> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangEsSlice.MergeFrom(cis);
			Patch(updating.LangEsSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangEsConf();
            tmp.MergeFrom(cis);
			updating.LangEsSlice = tmp;
		...
		public static void Range(Action<int, rawLangEs> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangEs MustGetByIndex(int index,string tag="") {
		...
		public static rawLangEs? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangEs> GetByFilter(Func<rawLangEs,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangEs>();
		...
		static public rawLangEs? GetOneByFilter(Func<rawLangEs,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
