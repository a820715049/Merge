...
using EventWishBarRewardConf = fat.rawdata.EventWishBarRewardConf;
using rawEventWishBarReward = fat.rawdata.EventWishBarReward;
using rawEventWishBarRewardMapABValue = fat.rawdata.EventWishBarRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventWishBarRewardVisitor
    {
		...
		public const string FileName = "EventWishBarRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventWishBarRewardVisitor.SetABDefault单独设定EventWishBarReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventWishBarRewardVisitor.SetABFallback单独设定EventWishBarReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventWishBarReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventWishBarReward>>(()=> new PBC::MapField<int, rawEventWishBarReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventWishBarReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventWishBarRewardMap;
			var abMap = v.EventWishBarRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventWishBarRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventWishBarRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventWishBarReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventWishBarRewardMap;
			if (v.EventWishBarRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventWishBarRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventWishBarRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventWishBarRewardConf cc) {
			foreach(KeyValuePair <string, rawEventWishBarRewardMapABValue> tagEntry in cc.EventWishBarRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventWishBarRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventWishBarRewardMapABValue();
					cc.EventWishBarRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventWishBarRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventWishBarReward> entry in cc.EventWishBarRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventWishBarReward> entry in tagEntry.Value.EventWishBarRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventWishBarReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventWishBarRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventWishBarRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventWishBarRewardMapAB != null)
			{
				foreach (var entry in cc.EventWishBarRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventWishBarRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventWishBarRewardMap.MergeFrom(cis);
			Patch(updating.EventWishBarRewardMap);
			updating.EventWishBarRewardSortedKeys = genSortedKeys(updating.EventWishBarRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventWishBarRewardConf();
            tmp.MergeFrom(cis);
			updating.EventWishBarRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventWishBarRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventWishBarReward> filter,string tag=""){
		...
		public static rawEventWishBarReward MustGet(int key,string tag="") {
		...
		public static rawEventWishBarReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventWishBarReward> GetByFilter(Func<rawEventWishBarReward, bool> filterFunc,string tag="") {
			List<rawEventWishBarReward> ret = new List<rawEventWishBarReward>();
			foreach(KeyValuePair <int, rawEventWishBarReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventWishBarReward? GetOneByFilter(Func<rawEventWishBarReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventWishBarReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventWishBarReward data) 
		{
			return data.Id;			
		}
    }
