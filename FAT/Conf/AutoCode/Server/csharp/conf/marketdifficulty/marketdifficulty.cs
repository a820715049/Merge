...
using MarketDifficultyConf = fat.rawdata.MarketDifficultyConf;
using rawMarketDifficulty = fat.rawdata.MarketDifficulty;
using rawMarketDifficultyMapABValue = fat.rawdata.MarketDifficultyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketDifficultyVisitor
    {
		...
		public const string FileName = "MarketDifficultyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketDifficultyVisitor.SetABDefault单独设定MarketDifficulty的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketDifficultyVisitor.SetABFallback单独设定MarketDifficulty的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketDifficulty>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketDifficulty>>(()=> new PBC::MapField<int, rawMarketDifficulty>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketDifficulty>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketDifficultyMap;
			var abMap = v.MarketDifficultyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketDifficultyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketDifficultyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketDifficulty> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketDifficultyMap;
			if (v.MarketDifficultyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketDifficultyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketDifficultyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketDifficultyConf cc) {
			foreach(KeyValuePair <string, rawMarketDifficultyMapABValue> tagEntry in cc.MarketDifficultyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketDifficultyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketDifficultyMapABValue();
					cc.MarketDifficultyMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketDifficultyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketDifficulty> entry in cc.MarketDifficultyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketDifficulty> entry in tagEntry.Value.MarketDifficultyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketDifficulty> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketDifficultyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketDifficultyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketDifficultyMapAB != null)
			{
				foreach (var entry in cc.MarketDifficultyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketDifficultyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketDifficultyMap.MergeFrom(cis);
			Patch(updating.MarketDifficultyMap);
			updating.MarketDifficultySortedKeys = genSortedKeys(updating.MarketDifficultyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketDifficultyConf();
            tmp.MergeFrom(cis);
			updating.MarketDifficultyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketDifficultySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketDifficulty> filter,string tag=""){
		...
		public static rawMarketDifficulty MustGet(int key,string tag="") {
		...
		public static rawMarketDifficulty? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketDifficulty> GetByFilter(Func<rawMarketDifficulty, bool> filterFunc,string tag="") {
			List<rawMarketDifficulty> ret = new List<rawMarketDifficulty>();
			foreach(KeyValuePair <int, rawMarketDifficulty> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketDifficulty? GetOneByFilter(Func<rawMarketDifficulty, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketDifficulty> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketDifficulty data) 
		{
			return data.Id;			
		}
    }
