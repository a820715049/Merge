...
using EventRedeemRewardConf = fat.rawdata.EventRedeemRewardConf;
using rawEventRedeemReward = fat.rawdata.EventRedeemReward;
using rawEventRedeemRewardMapABValue = fat.rawdata.EventRedeemRewardMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRedeemRewardVisitor
    {
		...
		public const string FileName = "EventRedeemRewardConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRedeemRewardVisitor.SetABDefault单独设定EventRedeemReward的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRedeemRewardVisitor.SetABFallback单独设定EventRedeemReward的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRedeemReward>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRedeemReward>>(()=> new PBC::MapField<int, rawEventRedeemReward>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRedeemReward>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRedeemRewardMap;
			var abMap = v.EventRedeemRewardMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRedeemRewardMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRedeemRewardMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRedeemReward> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRedeemRewardMap;
			if (v.EventRedeemRewardMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRedeemRewardMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRedeemRewardMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRedeemRewardConf cc) {
			foreach(KeyValuePair <string, rawEventRedeemRewardMapABValue> tagEntry in cc.EventRedeemRewardMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRedeemRewardMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRedeemRewardMapABValue();
					cc.EventRedeemRewardMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRedeemRewardMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRedeemReward> entry in cc.EventRedeemRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRedeemReward> entry in tagEntry.Value.EventRedeemRewardMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRedeemReward> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRedeemRewardConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRedeemRewardMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRedeemRewardMapAB != null)
			{
				foreach (var entry in cc.EventRedeemRewardMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRedeemRewardMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRedeemRewardMap.MergeFrom(cis);
			Patch(updating.EventRedeemRewardMap);
			updating.EventRedeemRewardSortedKeys = genSortedKeys(updating.EventRedeemRewardMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRedeemRewardConf();
            tmp.MergeFrom(cis);
			updating.EventRedeemRewardMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRedeemRewardSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRedeemReward> filter,string tag=""){
		...
		public static rawEventRedeemReward MustGet(int key,string tag="") {
		...
		public static rawEventRedeemReward? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRedeemReward> GetByFilter(Func<rawEventRedeemReward, bool> filterFunc,string tag="") {
			List<rawEventRedeemReward> ret = new List<rawEventRedeemReward>();
			foreach(KeyValuePair <int, rawEventRedeemReward> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRedeemReward? GetOneByFilter(Func<rawEventRedeemReward, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRedeemReward> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRedeemReward data) 
		{
			return data.Id;			
		}
    }
