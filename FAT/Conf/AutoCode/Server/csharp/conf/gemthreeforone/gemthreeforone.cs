...
using GemThreeForOneConf = fat.rawdata.GemThreeForOneConf;
using rawGemThreeForOne = fat.rawdata.GemThreeForOne;
using rawGemThreeForOneMapABValue = fat.rawdata.GemThreeForOneMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GemThreeForOneVisitor
    {
		...
		public const string FileName = "GemThreeForOneConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GemThreeForOneVisitor.SetABDefault单独设定GemThreeForOne的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GemThreeForOneVisitor.SetABFallback单独设定GemThreeForOne的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGemThreeForOne>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGemThreeForOne>>(()=> new PBC::MapField<int, rawGemThreeForOne>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGemThreeForOne>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GemThreeForOneMap;
			var abMap = v.GemThreeForOneMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GemThreeForOneMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GemThreeForOneMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGemThreeForOne> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GemThreeForOneMap;
			if (v.GemThreeForOneMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GemThreeForOneMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGemThreeForOneMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GemThreeForOneConf cc) {
			foreach(KeyValuePair <string, rawGemThreeForOneMapABValue> tagEntry in cc.GemThreeForOneMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GemThreeForOneMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGemThreeForOneMapABValue();
					cc.GemThreeForOneMapAB[tag] = ps;
				}
                var tagTmp = ps.GemThreeForOneMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGemThreeForOne> entry in cc.GemThreeForOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGemThreeForOne> entry in tagEntry.Value.GemThreeForOneMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GemThreeForOne> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GemThreeForOneConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GemThreeForOneMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GemThreeForOneMapAB != null)
			{
				foreach (var entry in cc.GemThreeForOneMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GemThreeForOneMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GemThreeForOneMap.MergeFrom(cis);
			Patch(updating.GemThreeForOneMap);
			updating.GemThreeForOneSortedKeys = genSortedKeys(updating.GemThreeForOneMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GemThreeForOneConf();
            tmp.MergeFrom(cis);
			updating.GemThreeForOneMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GemThreeForOneSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGemThreeForOne> filter,string tag=""){
		...
		public static rawGemThreeForOne MustGet(int key,string tag="") {
		...
		public static rawGemThreeForOne? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGemThreeForOne> GetByFilter(Func<rawGemThreeForOne, bool> filterFunc,string tag="") {
			List<rawGemThreeForOne> ret = new List<rawGemThreeForOne>();
			foreach(KeyValuePair <int, rawGemThreeForOne> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGemThreeForOne? GetOneByFilter(Func<rawGemThreeForOne, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGemThreeForOne> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGemThreeForOne data) 
		{
			return data.Id;			
		}
    }
