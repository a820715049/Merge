...
using EventOrderRateDetailConf = fat.rawdata.EventOrderRateDetailConf;
using rawEventOrderRateDetail = fat.rawdata.EventOrderRateDetail;
using rawEventOrderRateDetailMapABValue = fat.rawdata.EventOrderRateDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderRateDetailVisitor
    {
		...
		public const string FileName = "EventOrderRateDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderRateDetailVisitor.SetABDefault单独设定EventOrderRateDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderRateDetailVisitor.SetABFallback单独设定EventOrderRateDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderRateDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderRateDetail>>(()=> new PBC::MapField<int, rawEventOrderRateDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderRateDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderRateDetailMap;
			var abMap = v.EventOrderRateDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderRateDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderRateDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderRateDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderRateDetailMap;
			if (v.EventOrderRateDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderRateDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderRateDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderRateDetailConf cc) {
			foreach(KeyValuePair <string, rawEventOrderRateDetailMapABValue> tagEntry in cc.EventOrderRateDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderRateDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderRateDetailMapABValue();
					cc.EventOrderRateDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderRateDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderRateDetail> entry in cc.EventOrderRateDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderRateDetail> entry in tagEntry.Value.EventOrderRateDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderRateDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderRateDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderRateDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderRateDetailMapAB != null)
			{
				foreach (var entry in cc.EventOrderRateDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderRateDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderRateDetailMap.MergeFrom(cis);
			Patch(updating.EventOrderRateDetailMap);
			updating.EventOrderRateDetailSortedKeys = genSortedKeys(updating.EventOrderRateDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderRateDetailConf();
            tmp.MergeFrom(cis);
			updating.EventOrderRateDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderRateDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderRateDetail> filter,string tag=""){
		...
		public static rawEventOrderRateDetail MustGet(int key,string tag="") {
		...
		public static rawEventOrderRateDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderRateDetail> GetByFilter(Func<rawEventOrderRateDetail, bool> filterFunc,string tag="") {
			List<rawEventOrderRateDetail> ret = new List<rawEventOrderRateDetail>();
			foreach(KeyValuePair <int, rawEventOrderRateDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderRateDetail? GetOneByFilter(Func<rawEventOrderRateDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderRateDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderRateDetail data) 
		{
			return data.Id;			
		}
    }
