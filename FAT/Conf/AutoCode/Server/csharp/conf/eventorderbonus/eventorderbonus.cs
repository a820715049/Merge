...
using EventOrderBonusConf = fat.rawdata.EventOrderBonusConf;
using rawEventOrderBonus = fat.rawdata.EventOrderBonus;
using rawEventOrderBonusMapABValue = fat.rawdata.EventOrderBonusMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventOrderBonusVisitor
    {
		...
		public const string FileName = "EventOrderBonusConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventOrderBonusVisitor.SetABDefault单独设定EventOrderBonus的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventOrderBonusVisitor.SetABFallback单独设定EventOrderBonus的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventOrderBonus>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventOrderBonus>>(()=> new PBC::MapField<int, rawEventOrderBonus>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventOrderBonus>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventOrderBonusMap;
			var abMap = v.EventOrderBonusMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventOrderBonusMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventOrderBonusMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventOrderBonus> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventOrderBonusMap;
			if (v.EventOrderBonusMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventOrderBonusMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventOrderBonusMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventOrderBonusConf cc) {
			foreach(KeyValuePair <string, rawEventOrderBonusMapABValue> tagEntry in cc.EventOrderBonusMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventOrderBonusMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventOrderBonusMapABValue();
					cc.EventOrderBonusMapAB[tag] = ps;
				}
                var tagTmp = ps.EventOrderBonusMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventOrderBonus> entry in cc.EventOrderBonusMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventOrderBonus> entry in tagEntry.Value.EventOrderBonusMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventOrderBonus> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventOrderBonusConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventOrderBonusMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventOrderBonusMapAB != null)
			{
				foreach (var entry in cc.EventOrderBonusMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventOrderBonusMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventOrderBonusMap.MergeFrom(cis);
			Patch(updating.EventOrderBonusMap);
			updating.EventOrderBonusSortedKeys = genSortedKeys(updating.EventOrderBonusMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventOrderBonusConf();
            tmp.MergeFrom(cis);
			updating.EventOrderBonusMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventOrderBonusSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventOrderBonus> filter,string tag=""){
		...
		public static rawEventOrderBonus MustGet(int key,string tag="") {
		...
		public static rawEventOrderBonus? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventOrderBonus> GetByFilter(Func<rawEventOrderBonus, bool> filterFunc,string tag="") {
			List<rawEventOrderBonus> ret = new List<rawEventOrderBonus>();
			foreach(KeyValuePair <int, rawEventOrderBonus> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventOrderBonus? GetOneByFilter(Func<rawEventOrderBonus, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventOrderBonus> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventOrderBonus data) 
		{
			return data.Id;			
		}
    }
