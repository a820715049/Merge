...
using SpinPackConf = fat.rawdata.SpinPackConf;
using rawSpinPack = fat.rawdata.SpinPack;
using rawSpinPackMapABValue = fat.rawdata.SpinPackMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class SpinPackVisitor
    {
		...
		public const string FileName = "SpinPackConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过SpinPackVisitor.SetABDefault单独设定SpinPack的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过SpinPackVisitor.SetABFallback单独设定SpinPack的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawSpinPack>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawSpinPack>>(()=> new PBC::MapField<int, rawSpinPack>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawSpinPack>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().SpinPackMap;
			var abMap = v.SpinPackMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.SpinPackMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].SpinPackMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawSpinPack> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().SpinPackMap;
			if (v.SpinPackMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.SpinPackMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawSpinPackMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(SpinPackConf cc) {
			foreach(KeyValuePair <string, rawSpinPackMapABValue> tagEntry in cc.SpinPackMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.SpinPackMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawSpinPackMapABValue();
					cc.SpinPackMapAB[tag] = ps;
				}
                var tagTmp = ps.SpinPackMap;
				// 基础数据
				foreach(KeyValuePair <int, rawSpinPack> entry in cc.SpinPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawSpinPack> entry in tagEntry.Value.SpinPackMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.SpinPack> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(SpinPackConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.SpinPackMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.SpinPackMapAB != null)
			{
				foreach (var entry in cc.SpinPackMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.SpinPackMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.SpinPackMap.MergeFrom(cis);
			Patch(updating.SpinPackMap);
			updating.SpinPackSortedKeys = genSortedKeys(updating.SpinPackMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new SpinPackConf();
            tmp.MergeFrom(cis);
			updating.SpinPackMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().SpinPackSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawSpinPack> filter,string tag=""){
		...
		public static rawSpinPack MustGet(int key,string tag="") {
		...
		public static rawSpinPack? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawSpinPack> GetByFilter(Func<rawSpinPack, bool> filterFunc,string tag="") {
			List<rawSpinPack> ret = new List<rawSpinPack>();
			foreach(KeyValuePair <int, rawSpinPack> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawSpinPack? GetOneByFilter(Func<rawSpinPack, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawSpinPack> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawSpinPack data) 
		{
			return data.Id;			
		}
    }
