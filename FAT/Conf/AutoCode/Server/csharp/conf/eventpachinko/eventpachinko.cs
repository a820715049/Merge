...
using EventPachinkoConf = fat.rawdata.EventPachinkoConf;
using rawEventPachinko = fat.rawdata.EventPachinko;
using rawEventPachinkoMapABValue = fat.rawdata.EventPachinkoMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventPachinkoVisitor
    {
		...
		public const string FileName = "EventPachinkoConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventPachinkoVisitor.SetABDefault单独设定EventPachinko的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventPachinkoVisitor.SetABFallback单独设定EventPachinko的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventPachinko>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventPachinko>>(()=> new PBC::MapField<int, rawEventPachinko>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventPachinko>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventPachinkoMap;
			var abMap = v.EventPachinkoMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventPachinkoMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventPachinkoMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventPachinko> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventPachinkoMap;
			if (v.EventPachinkoMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventPachinkoMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventPachinkoMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventPachinkoConf cc) {
			foreach(KeyValuePair <string, rawEventPachinkoMapABValue> tagEntry in cc.EventPachinkoMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventPachinkoMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventPachinkoMapABValue();
					cc.EventPachinkoMapAB[tag] = ps;
				}
                var tagTmp = ps.EventPachinkoMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventPachinko> entry in cc.EventPachinkoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventPachinko> entry in tagEntry.Value.EventPachinkoMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventPachinko> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventPachinkoConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventPachinkoMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventPachinkoMapAB != null)
			{
				foreach (var entry in cc.EventPachinkoMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventPachinkoMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventPachinkoMap.MergeFrom(cis);
			Patch(updating.EventPachinkoMap);
			updating.EventPachinkoSortedKeys = genSortedKeys(updating.EventPachinkoMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventPachinkoConf();
            tmp.MergeFrom(cis);
			updating.EventPachinkoMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventPachinkoSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventPachinko> filter,string tag=""){
		...
		public static rawEventPachinko MustGet(int key,string tag="") {
		...
		public static rawEventPachinko? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventPachinko> GetByFilter(Func<rawEventPachinko, bool> filterFunc,string tag="") {
			List<rawEventPachinko> ret = new List<rawEventPachinko>();
			foreach(KeyValuePair <int, rawEventPachinko> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventPachinko? GetOneByFilter(Func<rawEventPachinko, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventPachinko> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventPachinko data) 
		{
			return data.Id;			
		}
    }
