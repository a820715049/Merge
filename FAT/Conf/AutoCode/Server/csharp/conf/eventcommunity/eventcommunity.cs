...
using EventCommunityConf = fat.rawdata.EventCommunityConf;
using rawEventCommunity = fat.rawdata.EventCommunity;
using rawEventCommunityMapABValue = fat.rawdata.EventCommunityMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventCommunityVisitor
    {
		...
		public const string FileName = "EventCommunityConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventCommunityVisitor.SetABDefault单独设定EventCommunity的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventCommunityVisitor.SetABFallback单独设定EventCommunity的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventCommunity>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventCommunity>>(()=> new PBC::MapField<int, rawEventCommunity>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventCommunity>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventCommunityMap;
			var abMap = v.EventCommunityMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventCommunityMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventCommunityMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventCommunity> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventCommunityMap;
			if (v.EventCommunityMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventCommunityMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventCommunityMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventCommunityConf cc) {
			foreach(KeyValuePair <string, rawEventCommunityMapABValue> tagEntry in cc.EventCommunityMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventCommunityMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventCommunityMapABValue();
					cc.EventCommunityMapAB[tag] = ps;
				}
                var tagTmp = ps.EventCommunityMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventCommunity> entry in cc.EventCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventCommunity> entry in tagEntry.Value.EventCommunityMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventCommunity> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventCommunityConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventCommunityMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventCommunityMapAB != null)
			{
				foreach (var entry in cc.EventCommunityMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventCommunityMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventCommunityMap.MergeFrom(cis);
			Patch(updating.EventCommunityMap);
			updating.EventCommunitySortedKeys = genSortedKeys(updating.EventCommunityMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventCommunityConf();
            tmp.MergeFrom(cis);
			updating.EventCommunityMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventCommunitySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventCommunity> filter,string tag=""){
		...
		public static rawEventCommunity MustGet(int key,string tag="") {
		...
		public static rawEventCommunity? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventCommunity> GetByFilter(Func<rawEventCommunity, bool> filterFunc,string tag="") {
			List<rawEventCommunity> ret = new List<rawEventCommunity>();
			foreach(KeyValuePair <int, rawEventCommunity> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventCommunity? GetOneByFilter(Func<rawEventCommunity, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventCommunity> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventCommunity data) 
		{
			return data.Id;			
		}
    }
