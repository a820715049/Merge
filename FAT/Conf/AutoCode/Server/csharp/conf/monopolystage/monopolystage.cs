...
using MonopolyStageConf = fat.rawdata.MonopolyStageConf;
using rawMonopolyStage = fat.rawdata.MonopolyStage;
using rawMonopolyStageMapABValue = fat.rawdata.MonopolyStageMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MonopolyStageVisitor
    {
		...
		public const string FileName = "MonopolyStageConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MonopolyStageVisitor.SetABDefault单独设定MonopolyStage的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MonopolyStageVisitor.SetABFallback单独设定MonopolyStage的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMonopolyStage>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMonopolyStage>>(()=> new PBC::MapField<int, rawMonopolyStage>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMonopolyStage>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MonopolyStageMap;
			var abMap = v.MonopolyStageMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MonopolyStageMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MonopolyStageMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMonopolyStage> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MonopolyStageMap;
			if (v.MonopolyStageMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MonopolyStageMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMonopolyStageMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MonopolyStageConf cc) {
			foreach(KeyValuePair <string, rawMonopolyStageMapABValue> tagEntry in cc.MonopolyStageMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MonopolyStageMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMonopolyStageMapABValue();
					cc.MonopolyStageMapAB[tag] = ps;
				}
                var tagTmp = ps.MonopolyStageMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMonopolyStage> entry in cc.MonopolyStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMonopolyStage> entry in tagEntry.Value.MonopolyStageMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MonopolyStage> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MonopolyStageConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MonopolyStageMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MonopolyStageMapAB != null)
			{
				foreach (var entry in cc.MonopolyStageMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MonopolyStageMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MonopolyStageMap.MergeFrom(cis);
			Patch(updating.MonopolyStageMap);
			updating.MonopolyStageSortedKeys = genSortedKeys(updating.MonopolyStageMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MonopolyStageConf();
            tmp.MergeFrom(cis);
			updating.MonopolyStageMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MonopolyStageSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMonopolyStage> filter,string tag=""){
		...
		public static rawMonopolyStage MustGet(int key,string tag="") {
		...
		public static rawMonopolyStage? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMonopolyStage> GetByFilter(Func<rawMonopolyStage, bool> filterFunc,string tag="") {
			List<rawMonopolyStage> ret = new List<rawMonopolyStage>();
			foreach(KeyValuePair <int, rawMonopolyStage> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMonopolyStage? GetOneByFilter(Func<rawMonopolyStage, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMonopolyStage> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMonopolyStage data) 
		{
			return data.Id;			
		}
    }
