...
using AreaBoardPackGroupConf = fat.rawdata.AreaBoardPackGroupConf;
using rawAreaBoardPackGroup = fat.rawdata.AreaBoardPackGroup;
using rawAreaBoardPackGroupMapABValue = fat.rawdata.AreaBoardPackGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class AreaBoardPackGroupVisitor
    {
		...
		public const string FileName = "AreaBoardPackGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过AreaBoardPackGroupVisitor.SetABDefault单独设定AreaBoardPackGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过AreaBoardPackGroupVisitor.SetABFallback单独设定AreaBoardPackGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawAreaBoardPackGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawAreaBoardPackGroup>>(()=> new PBC::MapField<int, rawAreaBoardPackGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawAreaBoardPackGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().AreaBoardPackGroupMap;
			var abMap = v.AreaBoardPackGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.AreaBoardPackGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].AreaBoardPackGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawAreaBoardPackGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().AreaBoardPackGroupMap;
			if (v.AreaBoardPackGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.AreaBoardPackGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawAreaBoardPackGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(AreaBoardPackGroupConf cc) {
			foreach(KeyValuePair <string, rawAreaBoardPackGroupMapABValue> tagEntry in cc.AreaBoardPackGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.AreaBoardPackGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawAreaBoardPackGroupMapABValue();
					cc.AreaBoardPackGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.AreaBoardPackGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawAreaBoardPackGroup> entry in cc.AreaBoardPackGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawAreaBoardPackGroup> entry in tagEntry.Value.AreaBoardPackGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.AreaBoardPackGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(AreaBoardPackGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.AreaBoardPackGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.AreaBoardPackGroupMapAB != null)
			{
				foreach (var entry in cc.AreaBoardPackGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.AreaBoardPackGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.AreaBoardPackGroupMap.MergeFrom(cis);
			Patch(updating.AreaBoardPackGroupMap);
			updating.AreaBoardPackGroupSortedKeys = genSortedKeys(updating.AreaBoardPackGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new AreaBoardPackGroupConf();
            tmp.MergeFrom(cis);
			updating.AreaBoardPackGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().AreaBoardPackGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawAreaBoardPackGroup> filter,string tag=""){
		...
		public static rawAreaBoardPackGroup MustGet(int key,string tag="") {
		...
		public static rawAreaBoardPackGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawAreaBoardPackGroup> GetByFilter(Func<rawAreaBoardPackGroup, bool> filterFunc,string tag="") {
			List<rawAreaBoardPackGroup> ret = new List<rawAreaBoardPackGroup>();
			foreach(KeyValuePair <int, rawAreaBoardPackGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawAreaBoardPackGroup? GetOneByFilter(Func<rawAreaBoardPackGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawAreaBoardPackGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawAreaBoardPackGroup data) 
		{
			return data.Id;			
		}
    }
