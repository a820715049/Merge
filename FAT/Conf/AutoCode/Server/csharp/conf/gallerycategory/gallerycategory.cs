...
using GalleryCategoryConf = fat.rawdata.GalleryCategoryConf;
using rawGalleryCategory = fat.rawdata.GalleryCategory;
using rawGalleryCategoryMapABValue = fat.rawdata.GalleryCategoryMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class GalleryCategoryVisitor
    {
		...
		public const string FileName = "GalleryCategoryConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过GalleryCategoryVisitor.SetABDefault单独设定GalleryCategory的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过GalleryCategoryVisitor.SetABFallback单独设定GalleryCategory的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawGalleryCategory>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawGalleryCategory>>(()=> new PBC::MapField<int, rawGalleryCategory>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawGalleryCategory>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().GalleryCategoryMap;
			var abMap = v.GalleryCategoryMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.GalleryCategoryMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].GalleryCategoryMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawGalleryCategory> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().GalleryCategoryMap;
			if (v.GalleryCategoryMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.GalleryCategoryMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawGalleryCategoryMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(GalleryCategoryConf cc) {
			foreach(KeyValuePair <string, rawGalleryCategoryMapABValue> tagEntry in cc.GalleryCategoryMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.GalleryCategoryMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawGalleryCategoryMapABValue();
					cc.GalleryCategoryMapAB[tag] = ps;
				}
                var tagTmp = ps.GalleryCategoryMap;
				// 基础数据
				foreach(KeyValuePair <int, rawGalleryCategory> entry in cc.GalleryCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawGalleryCategory> entry in tagEntry.Value.GalleryCategoryMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.GalleryCategory> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(GalleryCategoryConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.GalleryCategoryMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.GalleryCategoryMapAB != null)
			{
				foreach (var entry in cc.GalleryCategoryMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.GalleryCategoryMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.GalleryCategoryMap.MergeFrom(cis);
			Patch(updating.GalleryCategoryMap);
			updating.GalleryCategorySortedKeys = genSortedKeys(updating.GalleryCategoryMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new GalleryCategoryConf();
            tmp.MergeFrom(cis);
			updating.GalleryCategoryMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().GalleryCategorySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawGalleryCategory> filter,string tag=""){
		...
		public static rawGalleryCategory MustGet(int key,string tag="") {
		...
		public static rawGalleryCategory? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawGalleryCategory> GetByFilter(Func<rawGalleryCategory, bool> filterFunc,string tag="") {
			List<rawGalleryCategory> ret = new List<rawGalleryCategory>();
			foreach(KeyValuePair <int, rawGalleryCategory> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawGalleryCategory? GetOneByFilter(Func<rawGalleryCategory, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawGalleryCategory> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawGalleryCategory data) 
		{
			return data.Id;			
		}
    }
