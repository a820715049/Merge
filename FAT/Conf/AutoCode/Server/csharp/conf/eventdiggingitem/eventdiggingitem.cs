...
using EventDiggingItemConf = fat.rawdata.EventDiggingItemConf;
using rawEventDiggingItem = fat.rawdata.EventDiggingItem;
using rawEventDiggingItemMapABValue = fat.rawdata.EventDiggingItemMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventDiggingItemVisitor
    {
		...
		public const string FileName = "EventDiggingItemConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventDiggingItemVisitor.SetABDefault单独设定EventDiggingItem的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventDiggingItemVisitor.SetABFallback单独设定EventDiggingItem的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventDiggingItem>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventDiggingItem>>(()=> new PBC::MapField<int, rawEventDiggingItem>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventDiggingItem>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventDiggingItemMap;
			var abMap = v.EventDiggingItemMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventDiggingItemMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventDiggingItemMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventDiggingItem> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventDiggingItemMap;
			if (v.EventDiggingItemMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventDiggingItemMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventDiggingItemMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventDiggingItemConf cc) {
			foreach(KeyValuePair <string, rawEventDiggingItemMapABValue> tagEntry in cc.EventDiggingItemMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventDiggingItemMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventDiggingItemMapABValue();
					cc.EventDiggingItemMapAB[tag] = ps;
				}
                var tagTmp = ps.EventDiggingItemMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventDiggingItem> entry in cc.EventDiggingItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventDiggingItem> entry in tagEntry.Value.EventDiggingItemMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventDiggingItem> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventDiggingItemConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventDiggingItemMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventDiggingItemMapAB != null)
			{
				foreach (var entry in cc.EventDiggingItemMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventDiggingItemMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventDiggingItemMap.MergeFrom(cis);
			Patch(updating.EventDiggingItemMap);
			updating.EventDiggingItemSortedKeys = genSortedKeys(updating.EventDiggingItemMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventDiggingItemConf();
            tmp.MergeFrom(cis);
			updating.EventDiggingItemMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventDiggingItemSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventDiggingItem> filter,string tag=""){
		...
		public static rawEventDiggingItem MustGet(int key,string tag="") {
		...
		public static rawEventDiggingItem? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventDiggingItem> GetByFilter(Func<rawEventDiggingItem, bool> filterFunc,string tag="") {
			List<rawEventDiggingItem> ret = new List<rawEventDiggingItem>();
			foreach(KeyValuePair <int, rawEventDiggingItem> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventDiggingItem? GetOneByFilter(Func<rawEventDiggingItem, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventDiggingItem> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventDiggingItem data) 
		{
			return data.Id;			
		}
    }
