...
using DailyEventGroupConf = fat.rawdata.DailyEventGroupConf;
using rawDailyEventGroup = fat.rawdata.DailyEventGroup;
using rawDailyEventGroupMapABValue = fat.rawdata.DailyEventGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class DailyEventGroupVisitor
    {
		...
		public const string FileName = "DailyEventGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过DailyEventGroupVisitor.SetABDefault单独设定DailyEventGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过DailyEventGroupVisitor.SetABFallback单独设定DailyEventGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawDailyEventGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawDailyEventGroup>>(()=> new PBC::MapField<int, rawDailyEventGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawDailyEventGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().DailyEventGroupMap;
			var abMap = v.DailyEventGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.DailyEventGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].DailyEventGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawDailyEventGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().DailyEventGroupMap;
			if (v.DailyEventGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.DailyEventGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawDailyEventGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(DailyEventGroupConf cc) {
			foreach(KeyValuePair <string, rawDailyEventGroupMapABValue> tagEntry in cc.DailyEventGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.DailyEventGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawDailyEventGroupMapABValue();
					cc.DailyEventGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.DailyEventGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawDailyEventGroup> entry in cc.DailyEventGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawDailyEventGroup> entry in tagEntry.Value.DailyEventGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.DailyEventGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(DailyEventGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.DailyEventGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.DailyEventGroupMapAB != null)
			{
				foreach (var entry in cc.DailyEventGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.DailyEventGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.DailyEventGroupMap.MergeFrom(cis);
			Patch(updating.DailyEventGroupMap);
			updating.DailyEventGroupSortedKeys = genSortedKeys(updating.DailyEventGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new DailyEventGroupConf();
            tmp.MergeFrom(cis);
			updating.DailyEventGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().DailyEventGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawDailyEventGroup> filter,string tag=""){
		...
		public static rawDailyEventGroup MustGet(int key,string tag="") {
		...
		public static rawDailyEventGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawDailyEventGroup> GetByFilter(Func<rawDailyEventGroup, bool> filterFunc,string tag="") {
			List<rawDailyEventGroup> ret = new List<rawDailyEventGroup>();
			foreach(KeyValuePair <int, rawDailyEventGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawDailyEventGroup? GetOneByFilter(Func<rawDailyEventGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawDailyEventGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawDailyEventGroup data) 
		{
			return data.Id;			
		}
    }
