...
using MergeCategorySpecialConf = fat.rawdata.MergeCategorySpecialConf;
using rawMergeCategorySpecial = fat.rawdata.MergeCategorySpecial;
using rawMergeCategorySpecialMapABValue = fat.rawdata.MergeCategorySpecialMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeCategorySpecialVisitor
    {
		...
		public const string FileName = "MergeCategorySpecialConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeCategorySpecialVisitor.SetABDefault单独设定MergeCategorySpecial的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeCategorySpecialVisitor.SetABFallback单独设定MergeCategorySpecial的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeCategorySpecial>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeCategorySpecial>>(()=> new PBC::MapField<int, rawMergeCategorySpecial>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeCategorySpecial>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeCategorySpecialMap;
			var abMap = v.MergeCategorySpecialMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeCategorySpecialMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeCategorySpecialMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeCategorySpecial> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeCategorySpecialMap;
			if (v.MergeCategorySpecialMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeCategorySpecialMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeCategorySpecialMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeCategorySpecialConf cc) {
			foreach(KeyValuePair <string, rawMergeCategorySpecialMapABValue> tagEntry in cc.MergeCategorySpecialMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeCategorySpecialMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeCategorySpecialMapABValue();
					cc.MergeCategorySpecialMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeCategorySpecialMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeCategorySpecial> entry in cc.MergeCategorySpecialMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeCategorySpecial> entry in tagEntry.Value.MergeCategorySpecialMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeCategorySpecial> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeCategorySpecialConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeCategorySpecialMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeCategorySpecialMapAB != null)
			{
				foreach (var entry in cc.MergeCategorySpecialMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeCategorySpecialMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeCategorySpecialMap.MergeFrom(cis);
			Patch(updating.MergeCategorySpecialMap);
			updating.MergeCategorySpecialSortedKeys = genSortedKeys(updating.MergeCategorySpecialMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeCategorySpecialConf();
            tmp.MergeFrom(cis);
			updating.MergeCategorySpecialMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeCategorySpecialSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeCategorySpecial> filter,string tag=""){
		...
		public static rawMergeCategorySpecial MustGet(int key,string tag="") {
		...
		public static rawMergeCategorySpecial? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeCategorySpecial> GetByFilter(Func<rawMergeCategorySpecial, bool> filterFunc,string tag="") {
			List<rawMergeCategorySpecial> ret = new List<rawMergeCategorySpecial>();
			foreach(KeyValuePair <int, rawMergeCategorySpecial> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeCategorySpecial? GetOneByFilter(Func<rawMergeCategorySpecial, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeCategorySpecial> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeCategorySpecial data) 
		{
			return data.Id;			
		}
    }
