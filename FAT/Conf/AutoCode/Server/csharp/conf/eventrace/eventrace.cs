...
using EventRaceConf = fat.rawdata.EventRaceConf;
using rawEventRace = fat.rawdata.EventRace;
using rawEventRaceMapABValue = fat.rawdata.EventRaceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventRaceVisitor
    {
		...
		public const string FileName = "EventRaceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventRaceVisitor.SetABDefault单独设定EventRace的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventRaceVisitor.SetABFallback单独设定EventRace的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventRace>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventRace>>(()=> new PBC::MapField<int, rawEventRace>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventRace>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventRaceMap;
			var abMap = v.EventRaceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventRaceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventRaceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventRace> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventRaceMap;
			if (v.EventRaceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventRaceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventRaceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventRaceConf cc) {
			foreach(KeyValuePair <string, rawEventRaceMapABValue> tagEntry in cc.EventRaceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventRaceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventRaceMapABValue();
					cc.EventRaceMapAB[tag] = ps;
				}
                var tagTmp = ps.EventRaceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventRace> entry in cc.EventRaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventRace> entry in tagEntry.Value.EventRaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventRace> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventRaceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventRaceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventRaceMapAB != null)
			{
				foreach (var entry in cc.EventRaceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventRaceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventRaceMap.MergeFrom(cis);
			Patch(updating.EventRaceMap);
			updating.EventRaceSortedKeys = genSortedKeys(updating.EventRaceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventRaceConf();
            tmp.MergeFrom(cis);
			updating.EventRaceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventRaceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventRace> filter,string tag=""){
		...
		public static rawEventRace MustGet(int key,string tag="") {
		...
		public static rawEventRace? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventRace> GetByFilter(Func<rawEventRace, bool> filterFunc,string tag="") {
			List<rawEventRace> ret = new List<rawEventRace>();
			foreach(KeyValuePair <int, rawEventRace> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventRace? GetOneByFilter(Func<rawEventRace, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventRace> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventRace data) 
		{
			return data.Id;			
		}
    }
