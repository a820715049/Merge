...
using InventoryToolConf = fat.rawdata.InventoryToolConf;
using rawInventoryTool = fat.rawdata.InventoryTool;
using rawInventoryToolMapABValue = fat.rawdata.InventoryToolMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class InventoryToolVisitor
    {
		...
		public const string FileName = "InventoryToolConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过InventoryToolVisitor.SetABDefault单独设定InventoryTool的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过InventoryToolVisitor.SetABFallback单独设定InventoryTool的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawInventoryTool>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawInventoryTool>>(()=> new PBC::MapField<int, rawInventoryTool>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawInventoryTool>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().InventoryToolMap;
			var abMap = v.InventoryToolMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.InventoryToolMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].InventoryToolMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawInventoryTool> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().InventoryToolMap;
			if (v.InventoryToolMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.InventoryToolMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawInventoryToolMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(InventoryToolConf cc) {
			foreach(KeyValuePair <string, rawInventoryToolMapABValue> tagEntry in cc.InventoryToolMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.InventoryToolMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawInventoryToolMapABValue();
					cc.InventoryToolMapAB[tag] = ps;
				}
                var tagTmp = ps.InventoryToolMap;
				// 基础数据
				foreach(KeyValuePair <int, rawInventoryTool> entry in cc.InventoryToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawInventoryTool> entry in tagEntry.Value.InventoryToolMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.InventoryTool> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(InventoryToolConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.InventoryToolMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.InventoryToolMapAB != null)
			{
				foreach (var entry in cc.InventoryToolMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.InventoryToolMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.InventoryToolMap.MergeFrom(cis);
			Patch(updating.InventoryToolMap);
			updating.InventoryToolSortedKeys = genSortedKeys(updating.InventoryToolMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new InventoryToolConf();
            tmp.MergeFrom(cis);
			updating.InventoryToolMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().InventoryToolSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawInventoryTool> filter,string tag=""){
		...
		public static rawInventoryTool MustGet(int key,string tag="") {
		...
		public static rawInventoryTool? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawInventoryTool> GetByFilter(Func<rawInventoryTool, bool> filterFunc,string tag="") {
			List<rawInventoryTool> ret = new List<rawInventoryTool>();
			foreach(KeyValuePair <int, rawInventoryTool> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawInventoryTool? GetOneByFilter(Func<rawInventoryTool, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawInventoryTool> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawInventoryTool data) 
		{
			return data.Id;			
		}
    }
