...
using EventMineCartDetailConf = fat.rawdata.EventMineCartDetailConf;
using rawEventMineCartDetail = fat.rawdata.EventMineCartDetail;
using rawEventMineCartDetailMapABValue = fat.rawdata.EventMineCartDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventMineCartDetailVisitor
    {
		...
		public const string FileName = "EventMineCartDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventMineCartDetailVisitor.SetABDefault单独设定EventMineCartDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventMineCartDetailVisitor.SetABFallback单独设定EventMineCartDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventMineCartDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventMineCartDetail>>(()=> new PBC::MapField<int, rawEventMineCartDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventMineCartDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventMineCartDetailMap;
			var abMap = v.EventMineCartDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventMineCartDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventMineCartDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventMineCartDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventMineCartDetailMap;
			if (v.EventMineCartDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventMineCartDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventMineCartDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventMineCartDetailConf cc) {
			foreach(KeyValuePair <string, rawEventMineCartDetailMapABValue> tagEntry in cc.EventMineCartDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventMineCartDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventMineCartDetailMapABValue();
					cc.EventMineCartDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.EventMineCartDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventMineCartDetail> entry in cc.EventMineCartDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventMineCartDetail> entry in tagEntry.Value.EventMineCartDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventMineCartDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventMineCartDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventMineCartDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventMineCartDetailMapAB != null)
			{
				foreach (var entry in cc.EventMineCartDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventMineCartDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventMineCartDetailMap.MergeFrom(cis);
			Patch(updating.EventMineCartDetailMap);
			updating.EventMineCartDetailSortedKeys = genSortedKeys(updating.EventMineCartDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventMineCartDetailConf();
            tmp.MergeFrom(cis);
			updating.EventMineCartDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventMineCartDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventMineCartDetail> filter,string tag=""){
		...
		public static rawEventMineCartDetail MustGet(int key,string tag="") {
		...
		public static rawEventMineCartDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventMineCartDetail> GetByFilter(Func<rawEventMineCartDetail, bool> filterFunc,string tag="") {
			List<rawEventMineCartDetail> ret = new List<rawEventMineCartDetail>();
			foreach(KeyValuePair <int, rawEventMineCartDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventMineCartDetail? GetOneByFilter(Func<rawEventMineCartDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventMineCartDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventMineCartDetail data) 
		{
			return data.Id;			
		}
    }
