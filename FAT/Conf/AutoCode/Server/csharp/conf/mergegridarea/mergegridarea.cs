...
using MergeGridAreaConf = fat.rawdata.MergeGridAreaConf;
using rawMergeGridArea = fat.rawdata.MergeGridArea;
using rawMergeGridAreaSliceABValue = fat.rawdata.MergeGridAreaSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeGridAreaVisitor
    {
		...
		public const string FileName = "MergeGridAreaConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeGridAreaVisitor.SetABDefault单独设定MergeGridArea的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeGridAreaVisitor.SetABFallback单独设定MergeGridArea的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawMergeGridArea>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawMergeGridArea>>(()=>new PBC::RepeatedField<rawMergeGridArea>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawMergeGridArea>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeGridAreaSlice;
			var abMap = v.MergeGridAreaSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeGridAreaSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeGridAreaSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawMergeGridArea> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeGridAreaSlice;
			if (v.MergeGridAreaSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeGridAreaSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeGridAreaSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeGridAreaConf cc) {
			foreach(KeyValuePair <string, rawMergeGridAreaSliceABValue> tagEntry in cc.MergeGridAreaSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeGridAreaSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeGridAreaSliceABValue();
					cc.MergeGridAreaSliceAB[tag] = ps;
				}
                var tagTmp = ps.MergeGridAreaSlice;
				// 基础数据
				for(int i=0; i<cc.MergeGridAreaSlice.Count; ++i) 
				{
					tagTmp.Add(cc.MergeGridAreaSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.MergeGridAreaSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.MergeGridArea> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeGridAreaSlice.MergeFrom(cis);
			Patch(updating.MergeGridAreaSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeGridAreaConf();
            tmp.MergeFrom(cis);
			updating.MergeGridAreaSlice = tmp;
		...
		public static void Range(Action<int, rawMergeGridArea> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawMergeGridArea MustGetByIndex(int index,string tag="") {
		...
		public static rawMergeGridArea? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawMergeGridArea> GetByFilter(Func<rawMergeGridArea,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawMergeGridArea>();
		...
		static public rawMergeGridArea? GetOneByFilter(Func<rawMergeGridArea,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
