...
using RaceExtendGroupConf = fat.rawdata.RaceExtendGroupConf;
using rawRaceExtendGroup = fat.rawdata.RaceExtendGroup;
using rawRaceExtendGroupMapABValue = fat.rawdata.RaceExtendGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RaceExtendGroupVisitor
    {
		...
		public const string FileName = "RaceExtendGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RaceExtendGroupVisitor.SetABDefault单独设定RaceExtendGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RaceExtendGroupVisitor.SetABFallback单独设定RaceExtendGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawRaceExtendGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawRaceExtendGroup>>(()=> new PBC::MapField<int, rawRaceExtendGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawRaceExtendGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RaceExtendGroupMap;
			var abMap = v.RaceExtendGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RaceExtendGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RaceExtendGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawRaceExtendGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RaceExtendGroupMap;
			if (v.RaceExtendGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RaceExtendGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRaceExtendGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RaceExtendGroupConf cc) {
			foreach(KeyValuePair <string, rawRaceExtendGroupMapABValue> tagEntry in cc.RaceExtendGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RaceExtendGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRaceExtendGroupMapABValue();
					cc.RaceExtendGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.RaceExtendGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawRaceExtendGroup> entry in cc.RaceExtendGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawRaceExtendGroup> entry in tagEntry.Value.RaceExtendGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.RaceExtendGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(RaceExtendGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.RaceExtendGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.RaceExtendGroupMapAB != null)
			{
				foreach (var entry in cc.RaceExtendGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.RaceExtendGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RaceExtendGroupMap.MergeFrom(cis);
			Patch(updating.RaceExtendGroupMap);
			updating.RaceExtendGroupSortedKeys = genSortedKeys(updating.RaceExtendGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RaceExtendGroupConf();
            tmp.MergeFrom(cis);
			updating.RaceExtendGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().RaceExtendGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawRaceExtendGroup> filter,string tag=""){
		...
		public static rawRaceExtendGroup MustGet(int key,string tag="") {
		...
		public static rawRaceExtendGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawRaceExtendGroup> GetByFilter(Func<rawRaceExtendGroup, bool> filterFunc,string tag="") {
			List<rawRaceExtendGroup> ret = new List<rawRaceExtendGroup>();
			foreach(KeyValuePair <int, rawRaceExtendGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawRaceExtendGroup? GetOneByFilter(Func<rawRaceExtendGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawRaceExtendGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawRaceExtendGroup data) 
		{
			return data.Id;			
		}
    }
