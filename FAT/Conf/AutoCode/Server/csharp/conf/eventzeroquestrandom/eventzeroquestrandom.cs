...
using EventZeroQuestRandomConf = fat.rawdata.EventZeroQuestRandomConf;
using rawEventZeroQuestRandom = fat.rawdata.EventZeroQuestRandom;
using rawEventZeroQuestRandomMapABValue = fat.rawdata.EventZeroQuestRandomMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventZeroQuestRandomVisitor
    {
		...
		public const string FileName = "EventZeroQuestRandomConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventZeroQuestRandomVisitor.SetABDefault单独设定EventZeroQuestRandom的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventZeroQuestRandomVisitor.SetABFallback单独设定EventZeroQuestRandom的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventZeroQuestRandom>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventZeroQuestRandom>>(()=> new PBC::MapField<int, rawEventZeroQuestRandom>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventZeroQuestRandom>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventZeroQuestRandomMap;
			var abMap = v.EventZeroQuestRandomMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventZeroQuestRandomMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventZeroQuestRandomMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventZeroQuestRandom> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventZeroQuestRandomMap;
			if (v.EventZeroQuestRandomMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventZeroQuestRandomMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventZeroQuestRandomMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventZeroQuestRandomConf cc) {
			foreach(KeyValuePair <string, rawEventZeroQuestRandomMapABValue> tagEntry in cc.EventZeroQuestRandomMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventZeroQuestRandomMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventZeroQuestRandomMapABValue();
					cc.EventZeroQuestRandomMapAB[tag] = ps;
				}
                var tagTmp = ps.EventZeroQuestRandomMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventZeroQuestRandom> entry in cc.EventZeroQuestRandomMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventZeroQuestRandom> entry in tagEntry.Value.EventZeroQuestRandomMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventZeroQuestRandom> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventZeroQuestRandomConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventZeroQuestRandomMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventZeroQuestRandomMapAB != null)
			{
				foreach (var entry in cc.EventZeroQuestRandomMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventZeroQuestRandomMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventZeroQuestRandomMap.MergeFrom(cis);
			Patch(updating.EventZeroQuestRandomMap);
			updating.EventZeroQuestRandomSortedKeys = genSortedKeys(updating.EventZeroQuestRandomMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventZeroQuestRandomConf();
            tmp.MergeFrom(cis);
			updating.EventZeroQuestRandomMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventZeroQuestRandomSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventZeroQuestRandom> filter,string tag=""){
		...
		public static rawEventZeroQuestRandom MustGet(int key,string tag="") {
		...
		public static rawEventZeroQuestRandom? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventZeroQuestRandom> GetByFilter(Func<rawEventZeroQuestRandom, bool> filterFunc,string tag="") {
			List<rawEventZeroQuestRandom> ret = new List<rawEventZeroQuestRandom>();
			foreach(KeyValuePair <int, rawEventZeroQuestRandom> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventZeroQuestRandom? GetOneByFilter(Func<rawEventZeroQuestRandom, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventZeroQuestRandom> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventZeroQuestRandom data) 
		{
			return data.Id;			
		}
    }
