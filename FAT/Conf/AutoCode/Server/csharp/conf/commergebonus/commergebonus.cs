...
using ComMergeBonusConf = fat.rawdata.ComMergeBonusConf;
using rawComMergeBonus = fat.rawdata.ComMergeBonus;
using rawComMergeBonusMapABValue = fat.rawdata.ComMergeBonusMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ComMergeBonusVisitor
    {
		...
		public const string FileName = "ComMergeBonusConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ComMergeBonusVisitor.SetABDefault单独设定ComMergeBonus的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ComMergeBonusVisitor.SetABFallback单独设定ComMergeBonus的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawComMergeBonus>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawComMergeBonus>>(()=> new PBC::MapField<int, rawComMergeBonus>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawComMergeBonus>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ComMergeBonusMap;
			var abMap = v.ComMergeBonusMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ComMergeBonusMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ComMergeBonusMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawComMergeBonus> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ComMergeBonusMap;
			if (v.ComMergeBonusMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ComMergeBonusMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawComMergeBonusMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ComMergeBonusConf cc) {
			foreach(KeyValuePair <string, rawComMergeBonusMapABValue> tagEntry in cc.ComMergeBonusMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ComMergeBonusMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawComMergeBonusMapABValue();
					cc.ComMergeBonusMapAB[tag] = ps;
				}
                var tagTmp = ps.ComMergeBonusMap;
				// 基础数据
				foreach(KeyValuePair <int, rawComMergeBonus> entry in cc.ComMergeBonusMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawComMergeBonus> entry in tagEntry.Value.ComMergeBonusMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ComMergeBonus> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ComMergeBonusConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ComMergeBonusMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ComMergeBonusMapAB != null)
			{
				foreach (var entry in cc.ComMergeBonusMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ComMergeBonusMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ComMergeBonusMap.MergeFrom(cis);
			Patch(updating.ComMergeBonusMap);
			updating.ComMergeBonusSortedKeys = genSortedKeys(updating.ComMergeBonusMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ComMergeBonusConf();
            tmp.MergeFrom(cis);
			updating.ComMergeBonusMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ComMergeBonusSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawComMergeBonus> filter,string tag=""){
		...
		public static rawComMergeBonus MustGet(int key,string tag="") {
		...
		public static rawComMergeBonus? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawComMergeBonus> GetByFilter(Func<rawComMergeBonus, bool> filterFunc,string tag="") {
			List<rawComMergeBonus> ret = new List<rawComMergeBonus>();
			foreach(KeyValuePair <int, rawComMergeBonus> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawComMergeBonus? GetOneByFilter(Func<rawComMergeBonus, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawComMergeBonus> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawComMergeBonus data) 
		{
			return data.Id;			
		}
    }
