...
using EventIndepenBingoRoundConf = fat.rawdata.EventIndepenBingoRoundConf;
using rawEventIndepenBingoRound = fat.rawdata.EventIndepenBingoRound;
using rawEventIndepenBingoRoundMapABValue = fat.rawdata.EventIndepenBingoRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventIndepenBingoRoundVisitor
    {
		...
		public const string FileName = "EventIndepenBingoRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventIndepenBingoRoundVisitor.SetABDefault单独设定EventIndepenBingoRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventIndepenBingoRoundVisitor.SetABFallback单独设定EventIndepenBingoRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventIndepenBingoRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventIndepenBingoRound>>(()=> new PBC::MapField<int, rawEventIndepenBingoRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventIndepenBingoRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventIndepenBingoRoundMap;
			var abMap = v.EventIndepenBingoRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventIndepenBingoRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventIndepenBingoRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventIndepenBingoRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventIndepenBingoRoundMap;
			if (v.EventIndepenBingoRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventIndepenBingoRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventIndepenBingoRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventIndepenBingoRoundConf cc) {
			foreach(KeyValuePair <string, rawEventIndepenBingoRoundMapABValue> tagEntry in cc.EventIndepenBingoRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventIndepenBingoRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventIndepenBingoRoundMapABValue();
					cc.EventIndepenBingoRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventIndepenBingoRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventIndepenBingoRound> entry in cc.EventIndepenBingoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventIndepenBingoRound> entry in tagEntry.Value.EventIndepenBingoRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventIndepenBingoRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventIndepenBingoRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventIndepenBingoRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventIndepenBingoRoundMapAB != null)
			{
				foreach (var entry in cc.EventIndepenBingoRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventIndepenBingoRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventIndepenBingoRoundMap.MergeFrom(cis);
			Patch(updating.EventIndepenBingoRoundMap);
			updating.EventIndepenBingoRoundSortedKeys = genSortedKeys(updating.EventIndepenBingoRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventIndepenBingoRoundConf();
            tmp.MergeFrom(cis);
			updating.EventIndepenBingoRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventIndepenBingoRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventIndepenBingoRound> filter,string tag=""){
		...
		public static rawEventIndepenBingoRound MustGet(int key,string tag="") {
		...
		public static rawEventIndepenBingoRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventIndepenBingoRound> GetByFilter(Func<rawEventIndepenBingoRound, bool> filterFunc,string tag="") {
			List<rawEventIndepenBingoRound> ret = new List<rawEventIndepenBingoRound>();
			foreach(KeyValuePair <int, rawEventIndepenBingoRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventIndepenBingoRound? GetOneByFilter(Func<rawEventIndepenBingoRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventIndepenBingoRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventIndepenBingoRound data) 
		{
			return data.Id;			
		}
    }
