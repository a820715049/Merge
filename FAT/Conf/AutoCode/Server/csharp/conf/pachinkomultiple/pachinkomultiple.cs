...
using PachinkoMultipleConf = fat.rawdata.PachinkoMultipleConf;
using rawPachinkoMultiple = fat.rawdata.PachinkoMultiple;
using rawPachinkoMultipleMapABValue = fat.rawdata.PachinkoMultipleMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class PachinkoMultipleVisitor
    {
		...
		public const string FileName = "PachinkoMultipleConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过PachinkoMultipleVisitor.SetABDefault单独设定PachinkoMultiple的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过PachinkoMultipleVisitor.SetABFallback单独设定PachinkoMultiple的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawPachinkoMultiple>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawPachinkoMultiple>>(()=> new PBC::MapField<int, rawPachinkoMultiple>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawPachinkoMultiple>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().PachinkoMultipleMap;
			var abMap = v.PachinkoMultipleMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.PachinkoMultipleMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].PachinkoMultipleMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawPachinkoMultiple> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().PachinkoMultipleMap;
			if (v.PachinkoMultipleMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.PachinkoMultipleMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawPachinkoMultipleMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(PachinkoMultipleConf cc) {
			foreach(KeyValuePair <string, rawPachinkoMultipleMapABValue> tagEntry in cc.PachinkoMultipleMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.PachinkoMultipleMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawPachinkoMultipleMapABValue();
					cc.PachinkoMultipleMapAB[tag] = ps;
				}
                var tagTmp = ps.PachinkoMultipleMap;
				// 基础数据
				foreach(KeyValuePair <int, rawPachinkoMultiple> entry in cc.PachinkoMultipleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawPachinkoMultiple> entry in tagEntry.Value.PachinkoMultipleMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.PachinkoMultiple> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(PachinkoMultipleConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.PachinkoMultipleMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.PachinkoMultipleMapAB != null)
			{
				foreach (var entry in cc.PachinkoMultipleMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.PachinkoMultipleMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.PachinkoMultipleMap.MergeFrom(cis);
			Patch(updating.PachinkoMultipleMap);
			updating.PachinkoMultipleSortedKeys = genSortedKeys(updating.PachinkoMultipleMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new PachinkoMultipleConf();
            tmp.MergeFrom(cis);
			updating.PachinkoMultipleMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().PachinkoMultipleSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawPachinkoMultiple> filter,string tag=""){
		...
		public static rawPachinkoMultiple MustGet(int key,string tag="") {
		...
		public static rawPachinkoMultiple? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawPachinkoMultiple> GetByFilter(Func<rawPachinkoMultiple, bool> filterFunc,string tag="") {
			List<rawPachinkoMultiple> ret = new List<rawPachinkoMultiple>();
			foreach(KeyValuePair <int, rawPachinkoMultiple> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawPachinkoMultiple? GetOneByFilter(Func<rawPachinkoMultiple, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawPachinkoMultiple> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawPachinkoMultiple data) 
		{
			return data.Id;			
		}
    }
