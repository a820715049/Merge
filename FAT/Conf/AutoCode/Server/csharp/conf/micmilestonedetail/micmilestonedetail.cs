...
using MicMilestoneDetailConf = fat.rawdata.MicMilestoneDetailConf;
using rawMicMilestoneDetail = fat.rawdata.MicMilestoneDetail;
using rawMicMilestoneDetailMapABValue = fat.rawdata.MicMilestoneDetailMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MicMilestoneDetailVisitor
    {
		...
		public const string FileName = "MicMilestoneDetailConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MicMilestoneDetailVisitor.SetABDefault单独设定MicMilestoneDetail的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MicMilestoneDetailVisitor.SetABFallback单独设定MicMilestoneDetail的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMicMilestoneDetail>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMicMilestoneDetail>>(()=> new PBC::MapField<int, rawMicMilestoneDetail>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMicMilestoneDetail>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MicMilestoneDetailMap;
			var abMap = v.MicMilestoneDetailMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MicMilestoneDetailMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MicMilestoneDetailMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMicMilestoneDetail> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MicMilestoneDetailMap;
			if (v.MicMilestoneDetailMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MicMilestoneDetailMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMicMilestoneDetailMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MicMilestoneDetailConf cc) {
			foreach(KeyValuePair <string, rawMicMilestoneDetailMapABValue> tagEntry in cc.MicMilestoneDetailMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MicMilestoneDetailMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMicMilestoneDetailMapABValue();
					cc.MicMilestoneDetailMapAB[tag] = ps;
				}
                var tagTmp = ps.MicMilestoneDetailMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMicMilestoneDetail> entry in cc.MicMilestoneDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMicMilestoneDetail> entry in tagEntry.Value.MicMilestoneDetailMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MicMilestoneDetail> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MicMilestoneDetailConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MicMilestoneDetailMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MicMilestoneDetailMapAB != null)
			{
				foreach (var entry in cc.MicMilestoneDetailMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MicMilestoneDetailMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MicMilestoneDetailMap.MergeFrom(cis);
			Patch(updating.MicMilestoneDetailMap);
			updating.MicMilestoneDetailSortedKeys = genSortedKeys(updating.MicMilestoneDetailMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MicMilestoneDetailConf();
            tmp.MergeFrom(cis);
			updating.MicMilestoneDetailMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MicMilestoneDetailSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMicMilestoneDetail> filter,string tag=""){
		...
		public static rawMicMilestoneDetail MustGet(int key,string tag="") {
		...
		public static rawMicMilestoneDetail? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMicMilestoneDetail> GetByFilter(Func<rawMicMilestoneDetail, bool> filterFunc,string tag="") {
			List<rawMicMilestoneDetail> ret = new List<rawMicMilestoneDetail>();
			foreach(KeyValuePair <int, rawMicMilestoneDetail> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMicMilestoneDetail? GetOneByFilter(Func<rawMicMilestoneDetail, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMicMilestoneDetail> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMicMilestoneDetail data) 
		{
			return data.Id;			
		}
    }
