...
using LangPtHotfixConf = fat.rawdata.LangPtHotfixConf;
using rawLangPtHotfix = fat.rawdata.LangPtHotfix;
using rawLangPtHotfixSliceABValue = fat.rawdata.LangPtHotfixSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangPtHotfixVisitor
    {
		...
		public const string FileName = "LangPtHotfixConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangPtHotfixVisitor.SetABDefault单独设定LangPtHotfix的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangPtHotfixVisitor.SetABFallback单独设定LangPtHotfix的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangPtHotfix>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangPtHotfix>>(()=>new PBC::RepeatedField<rawLangPtHotfix>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangPtHotfix>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangPtHotfixSlice;
			var abMap = v.LangPtHotfixSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangPtHotfixSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangPtHotfixSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangPtHotfix> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangPtHotfixSlice;
			if (v.LangPtHotfixSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangPtHotfixSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangPtHotfixSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangPtHotfixConf cc) {
			foreach(KeyValuePair <string, rawLangPtHotfixSliceABValue> tagEntry in cc.LangPtHotfixSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangPtHotfixSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangPtHotfixSliceABValue();
					cc.LangPtHotfixSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangPtHotfixSlice;
				// 基础数据
				for(int i=0; i<cc.LangPtHotfixSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangPtHotfixSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangPtHotfixSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangPtHotfix> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangPtHotfixSlice.MergeFrom(cis);
			Patch(updating.LangPtHotfixSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangPtHotfixConf();
            tmp.MergeFrom(cis);
			updating.LangPtHotfixSlice = tmp;
		...
		public static void Range(Action<int, rawLangPtHotfix> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangPtHotfix MustGetByIndex(int index,string tag="") {
		...
		public static rawLangPtHotfix? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangPtHotfix> GetByFilter(Func<rawLangPtHotfix,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangPtHotfix>();
		...
		static public rawLangPtHotfix? GetOneByFilter(Func<rawLangPtHotfix,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
