...
using RoundCoinConf = fat.rawdata.RoundCoinConf;
using rawRoundCoin = fat.rawdata.RoundCoin;
using rawRoundCoinSliceABValue = fat.rawdata.RoundCoinSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class RoundCoinVisitor
    {
		...
		public const string FileName = "RoundCoinConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过RoundCoinVisitor.SetABDefault单独设定RoundCoin的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过RoundCoinVisitor.SetABFallback单独设定RoundCoin的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawRoundCoin>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawRoundCoin>>(()=>new PBC::RepeatedField<rawRoundCoin>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawRoundCoin>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().RoundCoinSlice;
			var abMap = v.RoundCoinSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.RoundCoinSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].RoundCoinSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawRoundCoin> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().RoundCoinSlice;
			if (v.RoundCoinSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.RoundCoinSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawRoundCoinSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(RoundCoinConf cc) {
			foreach(KeyValuePair <string, rawRoundCoinSliceABValue> tagEntry in cc.RoundCoinSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.RoundCoinSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawRoundCoinSliceABValue();
					cc.RoundCoinSliceAB[tag] = ps;
				}
                var tagTmp = ps.RoundCoinSlice;
				// 基础数据
				for(int i=0; i<cc.RoundCoinSlice.Count; ++i) 
				{
					tagTmp.Add(cc.RoundCoinSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.RoundCoinSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.RoundCoin> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.RoundCoinSlice.MergeFrom(cis);
			Patch(updating.RoundCoinSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new RoundCoinConf();
            tmp.MergeFrom(cis);
			updating.RoundCoinSlice = tmp;
		...
		public static void Range(Action<int, rawRoundCoin> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawRoundCoin MustGetByIndex(int index,string tag="") {
		...
		public static rawRoundCoin? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawRoundCoin> GetByFilter(Func<rawRoundCoin,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawRoundCoin>();
		...
		static public rawRoundCoin? GetOneByFilter(Func<rawRoundCoin,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
