...
using ItemReplaceConf = fat.rawdata.ItemReplaceConf;
using rawItemReplace = fat.rawdata.ItemReplace;
using rawItemReplaceMapABValue = fat.rawdata.ItemReplaceMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ItemReplaceVisitor
    {
		...
		public const string FileName = "ItemReplaceConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ItemReplaceVisitor.SetABDefault单独设定ItemReplace的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ItemReplaceVisitor.SetABFallback单独设定ItemReplace的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawItemReplace>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawItemReplace>>(()=> new PBC::MapField<int, rawItemReplace>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawItemReplace>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ItemReplaceMap;
			var abMap = v.ItemReplaceMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ItemReplaceMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ItemReplaceMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawItemReplace> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ItemReplaceMap;
			if (v.ItemReplaceMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ItemReplaceMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawItemReplaceMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ItemReplaceConf cc) {
			foreach(KeyValuePair <string, rawItemReplaceMapABValue> tagEntry in cc.ItemReplaceMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ItemReplaceMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawItemReplaceMapABValue();
					cc.ItemReplaceMapAB[tag] = ps;
				}
                var tagTmp = ps.ItemReplaceMap;
				// 基础数据
				foreach(KeyValuePair <int, rawItemReplace> entry in cc.ItemReplaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawItemReplace> entry in tagEntry.Value.ItemReplaceMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ItemReplace> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ItemReplaceConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ItemReplaceMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ItemReplaceMapAB != null)
			{
				foreach (var entry in cc.ItemReplaceMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ItemReplaceMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ItemReplaceMap.MergeFrom(cis);
			Patch(updating.ItemReplaceMap);
			updating.ItemReplaceSortedKeys = genSortedKeys(updating.ItemReplaceMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ItemReplaceConf();
            tmp.MergeFrom(cis);
			updating.ItemReplaceMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ItemReplaceSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawItemReplace> filter,string tag=""){
		...
		public static rawItemReplace MustGet(int key,string tag="") {
		...
		public static rawItemReplace? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawItemReplace> GetByFilter(Func<rawItemReplace, bool> filterFunc,string tag="") {
			List<rawItemReplace> ret = new List<rawItemReplace>();
			foreach(KeyValuePair <int, rawItemReplace> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawItemReplace? GetOneByFilter(Func<rawItemReplace, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawItemReplace> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawItemReplace data) 
		{
			return data.Id;			
		}
    }
