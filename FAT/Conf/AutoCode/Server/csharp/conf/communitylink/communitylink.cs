...
using CommunityLinkConf = fat.rawdata.CommunityLinkConf;
using rawCommunityLink = fat.rawdata.CommunityLink;
using rawCommunityLinkMapABValue = fat.rawdata.CommunityLinkMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CommunityLinkVisitor
    {
		...
		public const string FileName = "CommunityLinkConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CommunityLinkVisitor.SetABDefault单独设定CommunityLink的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CommunityLinkVisitor.SetABFallback单独设定CommunityLink的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCommunityLink>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCommunityLink>>(()=> new PBC::MapField<int, rawCommunityLink>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCommunityLink>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CommunityLinkMap;
			var abMap = v.CommunityLinkMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CommunityLinkMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CommunityLinkMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCommunityLink> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CommunityLinkMap;
			if (v.CommunityLinkMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CommunityLinkMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCommunityLinkMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CommunityLinkConf cc) {
			foreach(KeyValuePair <string, rawCommunityLinkMapABValue> tagEntry in cc.CommunityLinkMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CommunityLinkMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCommunityLinkMapABValue();
					cc.CommunityLinkMapAB[tag] = ps;
				}
                var tagTmp = ps.CommunityLinkMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCommunityLink> entry in cc.CommunityLinkMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCommunityLink> entry in tagEntry.Value.CommunityLinkMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CommunityLink> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CommunityLinkConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CommunityLinkMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CommunityLinkMapAB != null)
			{
				foreach (var entry in cc.CommunityLinkMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CommunityLinkMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CommunityLinkMap.MergeFrom(cis);
			Patch(updating.CommunityLinkMap);
			updating.CommunityLinkSortedKeys = genSortedKeys(updating.CommunityLinkMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CommunityLinkConf();
            tmp.MergeFrom(cis);
			updating.CommunityLinkMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CommunityLinkSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCommunityLink> filter,string tag=""){
		...
		public static rawCommunityLink MustGet(int key,string tag="") {
		...
		public static rawCommunityLink? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCommunityLink> GetByFilter(Func<rawCommunityLink, bool> filterFunc,string tag="") {
			List<rawCommunityLink> ret = new List<rawCommunityLink>();
			foreach(KeyValuePair <int, rawCommunityLink> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCommunityLink? GetOneByFilter(Func<rawCommunityLink, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCommunityLink> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCommunityLink data) 
		{
			return data.Id;			
		}
    }
