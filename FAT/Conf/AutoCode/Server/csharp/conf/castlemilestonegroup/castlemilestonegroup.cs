...
using CastleMilestoneGroupConf = fat.rawdata.CastleMilestoneGroupConf;
using rawCastleMilestoneGroup = fat.rawdata.CastleMilestoneGroup;
using rawCastleMilestoneGroupMapABValue = fat.rawdata.CastleMilestoneGroupMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class CastleMilestoneGroupVisitor
    {
		...
		public const string FileName = "CastleMilestoneGroupConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过CastleMilestoneGroupVisitor.SetABDefault单独设定CastleMilestoneGroup的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过CastleMilestoneGroupVisitor.SetABFallback单独设定CastleMilestoneGroup的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawCastleMilestoneGroup>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawCastleMilestoneGroup>>(()=> new PBC::MapField<int, rawCastleMilestoneGroup>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawCastleMilestoneGroup>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().CastleMilestoneGroupMap;
			var abMap = v.CastleMilestoneGroupMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.CastleMilestoneGroupMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].CastleMilestoneGroupMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawCastleMilestoneGroup> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().CastleMilestoneGroupMap;
			if (v.CastleMilestoneGroupMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.CastleMilestoneGroupMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawCastleMilestoneGroupMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(CastleMilestoneGroupConf cc) {
			foreach(KeyValuePair <string, rawCastleMilestoneGroupMapABValue> tagEntry in cc.CastleMilestoneGroupMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.CastleMilestoneGroupMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawCastleMilestoneGroupMapABValue();
					cc.CastleMilestoneGroupMapAB[tag] = ps;
				}
                var tagTmp = ps.CastleMilestoneGroupMap;
				// 基础数据
				foreach(KeyValuePair <int, rawCastleMilestoneGroup> entry in cc.CastleMilestoneGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawCastleMilestoneGroup> entry in tagEntry.Value.CastleMilestoneGroupMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.CastleMilestoneGroup> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(CastleMilestoneGroupConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.CastleMilestoneGroupMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.CastleMilestoneGroupMapAB != null)
			{
				foreach (var entry in cc.CastleMilestoneGroupMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.CastleMilestoneGroupMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.CastleMilestoneGroupMap.MergeFrom(cis);
			Patch(updating.CastleMilestoneGroupMap);
			updating.CastleMilestoneGroupSortedKeys = genSortedKeys(updating.CastleMilestoneGroupMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new CastleMilestoneGroupConf();
            tmp.MergeFrom(cis);
			updating.CastleMilestoneGroupMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().CastleMilestoneGroupSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawCastleMilestoneGroup> filter,string tag=""){
		...
		public static rawCastleMilestoneGroup MustGet(int key,string tag="") {
		...
		public static rawCastleMilestoneGroup? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawCastleMilestoneGroup> GetByFilter(Func<rawCastleMilestoneGroup, bool> filterFunc,string tag="") {
			List<rawCastleMilestoneGroup> ret = new List<rawCastleMilestoneGroup>();
			foreach(KeyValuePair <int, rawCastleMilestoneGroup> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawCastleMilestoneGroup? GetOneByFilter(Func<rawCastleMilestoneGroup, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawCastleMilestoneGroup> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawCastleMilestoneGroup data) 
		{
			return data.Id;			
		}
    }
