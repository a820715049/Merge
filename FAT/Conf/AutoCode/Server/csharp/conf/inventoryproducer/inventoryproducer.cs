...
using InventoryProducerConf = fat.rawdata.InventoryProducerConf;
using rawInventoryProducer = fat.rawdata.InventoryProducer;
using rawInventoryProducerMapABValue = fat.rawdata.InventoryProducerMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class InventoryProducerVisitor
    {
		...
		public const string FileName = "InventoryProducerConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过InventoryProducerVisitor.SetABDefault单独设定InventoryProducer的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过InventoryProducerVisitor.SetABFallback单独设定InventoryProducer的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawInventoryProducer>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawInventoryProducer>>(()=> new PBC::MapField<int, rawInventoryProducer>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawInventoryProducer>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().InventoryProducerMap;
			var abMap = v.InventoryProducerMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.InventoryProducerMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].InventoryProducerMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawInventoryProducer> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().InventoryProducerMap;
			if (v.InventoryProducerMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.InventoryProducerMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawInventoryProducerMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(InventoryProducerConf cc) {
			foreach(KeyValuePair <string, rawInventoryProducerMapABValue> tagEntry in cc.InventoryProducerMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.InventoryProducerMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawInventoryProducerMapABValue();
					cc.InventoryProducerMapAB[tag] = ps;
				}
                var tagTmp = ps.InventoryProducerMap;
				// 基础数据
				foreach(KeyValuePair <int, rawInventoryProducer> entry in cc.InventoryProducerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawInventoryProducer> entry in tagEntry.Value.InventoryProducerMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.InventoryProducer> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(InventoryProducerConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.InventoryProducerMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.InventoryProducerMapAB != null)
			{
				foreach (var entry in cc.InventoryProducerMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.InventoryProducerMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.InventoryProducerMap.MergeFrom(cis);
			Patch(updating.InventoryProducerMap);
			updating.InventoryProducerSortedKeys = genSortedKeys(updating.InventoryProducerMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new InventoryProducerConf();
            tmp.MergeFrom(cis);
			updating.InventoryProducerMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().InventoryProducerSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawInventoryProducer> filter,string tag=""){
		...
		public static rawInventoryProducer MustGet(int key,string tag="") {
		...
		public static rawInventoryProducer? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawInventoryProducer> GetByFilter(Func<rawInventoryProducer, bool> filterFunc,string tag="") {
			List<rawInventoryProducer> ret = new List<rawInventoryProducer>();
			foreach(KeyValuePair <int, rawInventoryProducer> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawInventoryProducer? GetOneByFilter(Func<rawInventoryProducer, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawInventoryProducer> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawInventoryProducer data) 
		{
			return data.Id;			
		}
    }
