...
using EventVineLeapOutConf = fat.rawdata.EventVineLeapOutConf;
using rawEventVineLeapOut = fat.rawdata.EventVineLeapOut;
using rawEventVineLeapOutMapABValue = fat.rawdata.EventVineLeapOutMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventVineLeapOutVisitor
    {
		...
		public const string FileName = "EventVineLeapOutConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventVineLeapOutVisitor.SetABDefault单独设定EventVineLeapOut的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventVineLeapOutVisitor.SetABFallback单独设定EventVineLeapOut的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventVineLeapOut>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventVineLeapOut>>(()=> new PBC::MapField<int, rawEventVineLeapOut>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventVineLeapOut>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventVineLeapOutMap;
			var abMap = v.EventVineLeapOutMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventVineLeapOutMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventVineLeapOutMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventVineLeapOut> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventVineLeapOutMap;
			if (v.EventVineLeapOutMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventVineLeapOutMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventVineLeapOutMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventVineLeapOutConf cc) {
			foreach(KeyValuePair <string, rawEventVineLeapOutMapABValue> tagEntry in cc.EventVineLeapOutMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventVineLeapOutMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventVineLeapOutMapABValue();
					cc.EventVineLeapOutMapAB[tag] = ps;
				}
                var tagTmp = ps.EventVineLeapOutMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventVineLeapOut> entry in cc.EventVineLeapOutMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventVineLeapOut> entry in tagEntry.Value.EventVineLeapOutMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventVineLeapOut> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventVineLeapOutConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventVineLeapOutMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventVineLeapOutMapAB != null)
			{
				foreach (var entry in cc.EventVineLeapOutMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventVineLeapOutMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventVineLeapOutMap.MergeFrom(cis);
			Patch(updating.EventVineLeapOutMap);
			updating.EventVineLeapOutSortedKeys = genSortedKeys(updating.EventVineLeapOutMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventVineLeapOutConf();
            tmp.MergeFrom(cis);
			updating.EventVineLeapOutMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventVineLeapOutSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventVineLeapOut> filter,string tag=""){
		...
		public static rawEventVineLeapOut MustGet(int key,string tag="") {
		...
		public static rawEventVineLeapOut? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventVineLeapOut> GetByFilter(Func<rawEventVineLeapOut, bool> filterFunc,string tag="") {
			List<rawEventVineLeapOut> ret = new List<rawEventVineLeapOut>();
			foreach(KeyValuePair <int, rawEventVineLeapOut> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventVineLeapOut? GetOneByFilter(Func<rawEventVineLeapOut, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventVineLeapOut> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventVineLeapOut data) 
		{
			return data.Id;			
		}
    }
