...
using BGMConf = fat.rawdata.BGMConf;
using rawBGM = fat.rawdata.BGM;
using rawBGMSliceABValue = fat.rawdata.BGMSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class BGMVisitor
    {
		...
		public const string FileName = "BGMConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过BGMVisitor.SetABDefault单独设定BGM的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过BGMVisitor.SetABFallback单独设定BGM的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawBGM>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawBGM>>(()=>new PBC::RepeatedField<rawBGM>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawBGM>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().BGMSlice;
			var abMap = v.BGMSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.BGMSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].BGMSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawBGM> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().BGMSlice;
			if (v.BGMSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.BGMSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawBGMSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(BGMConf cc) {
			foreach(KeyValuePair <string, rawBGMSliceABValue> tagEntry in cc.BGMSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.BGMSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawBGMSliceABValue();
					cc.BGMSliceAB[tag] = ps;
				}
                var tagTmp = ps.BGMSlice;
				// 基础数据
				for(int i=0; i<cc.BGMSlice.Count; ++i) 
				{
					tagTmp.Add(cc.BGMSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.BGMSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.BGM> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.BGMSlice.MergeFrom(cis);
			Patch(updating.BGMSlice);
		...
		public static void Range(Action<int, rawBGM> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawBGM MustGetByIndex(int index,string tag="") {
		...
		public static rawBGM? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawBGM> GetByFilter(Func<rawBGM,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawBGM>();
		...
		static public rawBGM? GetOneByFilter(Func<rawBGM,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
