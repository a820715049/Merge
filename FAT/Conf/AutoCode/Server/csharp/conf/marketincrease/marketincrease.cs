...
using MarketIncreaseConf = fat.rawdata.MarketIncreaseConf;
using rawMarketIncrease = fat.rawdata.MarketIncrease;
using rawMarketIncreaseMapABValue = fat.rawdata.MarketIncreaseMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketIncreaseVisitor
    {
		...
		public const string FileName = "MarketIncreaseConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketIncreaseVisitor.SetABDefault单独设定MarketIncrease的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketIncreaseVisitor.SetABFallback单独设定MarketIncrease的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketIncrease>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketIncrease>>(()=> new PBC::MapField<int, rawMarketIncrease>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketIncrease>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketIncreaseMap;
			var abMap = v.MarketIncreaseMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketIncreaseMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketIncreaseMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketIncrease> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketIncreaseMap;
			if (v.MarketIncreaseMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketIncreaseMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketIncreaseMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketIncreaseConf cc) {
			foreach(KeyValuePair <string, rawMarketIncreaseMapABValue> tagEntry in cc.MarketIncreaseMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketIncreaseMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketIncreaseMapABValue();
					cc.MarketIncreaseMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketIncreaseMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketIncrease> entry in cc.MarketIncreaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketIncrease> entry in tagEntry.Value.MarketIncreaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketIncrease> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketIncreaseConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketIncreaseMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketIncreaseMapAB != null)
			{
				foreach (var entry in cc.MarketIncreaseMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketIncreaseMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketIncreaseMap.MergeFrom(cis);
			Patch(updating.MarketIncreaseMap);
			updating.MarketIncreaseSortedKeys = genSortedKeys(updating.MarketIncreaseMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketIncreaseConf();
            tmp.MergeFrom(cis);
			updating.MarketIncreaseMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketIncreaseSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketIncrease> filter,string tag=""){
		...
		public static rawMarketIncrease MustGet(int key,string tag="") {
		...
		public static rawMarketIncrease? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketIncrease> GetByFilter(Func<rawMarketIncrease, bool> filterFunc,string tag="") {
			List<rawMarketIncrease> ret = new List<rawMarketIncrease>();
			foreach(KeyValuePair <int, rawMarketIncrease> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketIncrease? GetOneByFilter(Func<rawMarketIncrease, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketIncrease> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketIncrease data) 
		{
			return data.Id;			
		}
    }
