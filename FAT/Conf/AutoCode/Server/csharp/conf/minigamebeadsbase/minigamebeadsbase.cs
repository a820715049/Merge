...
using MiniGameBeadsBaseConf = fat.rawdata.MiniGameBeadsBaseConf;
using rawMiniGameBeadsBase = fat.rawdata.MiniGameBeadsBase;
using rawMiniGameBeadsBaseMapABValue = fat.rawdata.MiniGameBeadsBaseMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MiniGameBeadsBaseVisitor
    {
		...
		public const string FileName = "MiniGameBeadsBaseConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MiniGameBeadsBaseVisitor.SetABDefault单独设定MiniGameBeadsBase的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MiniGameBeadsBaseVisitor.SetABFallback单独设定MiniGameBeadsBase的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMiniGameBeadsBase>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMiniGameBeadsBase>>(()=> new PBC::MapField<int, rawMiniGameBeadsBase>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMiniGameBeadsBase>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MiniGameBeadsBaseMap;
			var abMap = v.MiniGameBeadsBaseMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MiniGameBeadsBaseMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MiniGameBeadsBaseMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMiniGameBeadsBase> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MiniGameBeadsBaseMap;
			if (v.MiniGameBeadsBaseMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MiniGameBeadsBaseMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMiniGameBeadsBaseMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MiniGameBeadsBaseConf cc) {
			foreach(KeyValuePair <string, rawMiniGameBeadsBaseMapABValue> tagEntry in cc.MiniGameBeadsBaseMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MiniGameBeadsBaseMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMiniGameBeadsBaseMapABValue();
					cc.MiniGameBeadsBaseMapAB[tag] = ps;
				}
                var tagTmp = ps.MiniGameBeadsBaseMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMiniGameBeadsBase> entry in cc.MiniGameBeadsBaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMiniGameBeadsBase> entry in tagEntry.Value.MiniGameBeadsBaseMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MiniGameBeadsBase> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MiniGameBeadsBaseConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MiniGameBeadsBaseMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MiniGameBeadsBaseMapAB != null)
			{
				foreach (var entry in cc.MiniGameBeadsBaseMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MiniGameBeadsBaseMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MiniGameBeadsBaseMap.MergeFrom(cis);
			Patch(updating.MiniGameBeadsBaseMap);
			updating.MiniGameBeadsBaseSortedKeys = genSortedKeys(updating.MiniGameBeadsBaseMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MiniGameBeadsBaseConf();
            tmp.MergeFrom(cis);
			updating.MiniGameBeadsBaseMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MiniGameBeadsBaseSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMiniGameBeadsBase> filter,string tag=""){
		...
		public static rawMiniGameBeadsBase MustGet(int key,string tag="") {
		...
		public static rawMiniGameBeadsBase? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMiniGameBeadsBase> GetByFilter(Func<rawMiniGameBeadsBase, bool> filterFunc,string tag="") {
			List<rawMiniGameBeadsBase> ret = new List<rawMiniGameBeadsBase>();
			foreach(KeyValuePair <int, rawMiniGameBeadsBase> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMiniGameBeadsBase? GetOneByFilter(Func<rawMiniGameBeadsBase, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMiniGameBeadsBase> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMiniGameBeadsBase data) 
		{
			return data.Id;			
		}
    }
