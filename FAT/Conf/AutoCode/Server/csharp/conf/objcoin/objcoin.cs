...
using ObjCoinConf = fat.rawdata.ObjCoinConf;
using rawObjCoin = fat.rawdata.ObjCoin;
using rawObjCoinMapABValue = fat.rawdata.ObjCoinMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ObjCoinVisitor
    {
		...
		public const string FileName = "ObjCoinConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ObjCoinVisitor.SetABDefault单独设定ObjCoin的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ObjCoinVisitor.SetABFallback单独设定ObjCoin的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawObjCoin>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawObjCoin>>(()=> new PBC::MapField<int, rawObjCoin>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawObjCoin>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ObjCoinMap;
			var abMap = v.ObjCoinMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ObjCoinMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ObjCoinMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawObjCoin> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ObjCoinMap;
			if (v.ObjCoinMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ObjCoinMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawObjCoinMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ObjCoinConf cc) {
			foreach(KeyValuePair <string, rawObjCoinMapABValue> tagEntry in cc.ObjCoinMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ObjCoinMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawObjCoinMapABValue();
					cc.ObjCoinMapAB[tag] = ps;
				}
                var tagTmp = ps.ObjCoinMap;
				// 基础数据
				foreach(KeyValuePair <int, rawObjCoin> entry in cc.ObjCoinMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawObjCoin> entry in tagEntry.Value.ObjCoinMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.ObjCoin> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ObjCoinConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ObjCoinMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ObjCoinMapAB != null)
			{
				foreach (var entry in cc.ObjCoinMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ObjCoinMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ObjCoinMap.MergeFrom(cis);
			Patch(updating.ObjCoinMap);
			updating.ObjCoinSortedKeys = genSortedKeys(updating.ObjCoinMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ObjCoinConf();
            tmp.MergeFrom(cis);
			updating.ObjCoinMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ObjCoinSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawObjCoin> filter,string tag=""){
		...
		public static rawObjCoin MustGet(int key,string tag="") {
		...
		public static rawObjCoin? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawObjCoin> GetByFilter(Func<rawObjCoin, bool> filterFunc,string tag="") {
			List<rawObjCoin> ret = new List<rawObjCoin>();
			foreach(KeyValuePair <int, rawObjCoin> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawObjCoin? GetOneByFilter(Func<rawObjCoin, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawObjCoin> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawObjCoin data) 
		{
			return data.Id;			
		}
    }
