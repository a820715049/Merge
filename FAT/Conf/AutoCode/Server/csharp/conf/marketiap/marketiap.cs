...
using MarketIAPConf = fat.rawdata.MarketIAPConf;
using rawMarketIAP = fat.rawdata.MarketIAP;
using rawMarketIAPMapABValue = fat.rawdata.MarketIAPMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MarketIAPVisitor
    {
		...
		public const string FileName = "MarketIAPConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MarketIAPVisitor.SetABDefault单独设定MarketIAP的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MarketIAPVisitor.SetABFallback单独设定MarketIAP的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMarketIAP>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMarketIAP>>(()=> new PBC::MapField<int, rawMarketIAP>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMarketIAP>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MarketIAPMap;
			var abMap = v.MarketIAPMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MarketIAPMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MarketIAPMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMarketIAP> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MarketIAPMap;
			if (v.MarketIAPMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MarketIAPMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMarketIAPMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MarketIAPConf cc) {
			foreach(KeyValuePair <string, rawMarketIAPMapABValue> tagEntry in cc.MarketIAPMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MarketIAPMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMarketIAPMapABValue();
					cc.MarketIAPMapAB[tag] = ps;
				}
                var tagTmp = ps.MarketIAPMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMarketIAP> entry in cc.MarketIAPMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMarketIAP> entry in tagEntry.Value.MarketIAPMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MarketIAP> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MarketIAPConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MarketIAPMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MarketIAPMapAB != null)
			{
				foreach (var entry in cc.MarketIAPMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MarketIAPMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MarketIAPMap.MergeFrom(cis);
			Patch(updating.MarketIAPMap);
			updating.MarketIAPSortedKeys = genSortedKeys(updating.MarketIAPMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MarketIAPConf();
            tmp.MergeFrom(cis);
			updating.MarketIAPMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MarketIAPSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMarketIAP> filter,string tag=""){
		...
		public static rawMarketIAP MustGet(int key,string tag="") {
		...
		public static rawMarketIAP? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMarketIAP> GetByFilter(Func<rawMarketIAP, bool> filterFunc,string tag="") {
			List<rawMarketIAP> ret = new List<rawMarketIAP>();
			foreach(KeyValuePair <int, rawMarketIAP> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMarketIAP? GetOneByFilter(Func<rawMarketIAP, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMarketIAP> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMarketIAP data) 
		{
			return data.Id;			
		}
    }
