...
using EventLimitMergeOrderConf = fat.rawdata.EventLimitMergeOrderConf;
using rawEventLimitMergeOrder = fat.rawdata.EventLimitMergeOrder;
using rawEventLimitMergeOrderMapABValue = fat.rawdata.EventLimitMergeOrderMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventLimitMergeOrderVisitor
    {
		...
		public const string FileName = "EventLimitMergeOrderConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventLimitMergeOrderVisitor.SetABDefault单独设定EventLimitMergeOrder的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventLimitMergeOrderVisitor.SetABFallback单独设定EventLimitMergeOrder的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventLimitMergeOrder>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventLimitMergeOrder>>(()=> new PBC::MapField<int, rawEventLimitMergeOrder>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventLimitMergeOrder>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventLimitMergeOrderMap;
			var abMap = v.EventLimitMergeOrderMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventLimitMergeOrderMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventLimitMergeOrderMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventLimitMergeOrder> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventLimitMergeOrderMap;
			if (v.EventLimitMergeOrderMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventLimitMergeOrderMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventLimitMergeOrderMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventLimitMergeOrderConf cc) {
			foreach(KeyValuePair <string, rawEventLimitMergeOrderMapABValue> tagEntry in cc.EventLimitMergeOrderMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventLimitMergeOrderMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventLimitMergeOrderMapABValue();
					cc.EventLimitMergeOrderMapAB[tag] = ps;
				}
                var tagTmp = ps.EventLimitMergeOrderMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventLimitMergeOrder> entry in cc.EventLimitMergeOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventLimitMergeOrder> entry in tagEntry.Value.EventLimitMergeOrderMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventLimitMergeOrder> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventLimitMergeOrderConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventLimitMergeOrderMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventLimitMergeOrderMapAB != null)
			{
				foreach (var entry in cc.EventLimitMergeOrderMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventLimitMergeOrderMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventLimitMergeOrderMap.MergeFrom(cis);
			Patch(updating.EventLimitMergeOrderMap);
			updating.EventLimitMergeOrderSortedKeys = genSortedKeys(updating.EventLimitMergeOrderMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventLimitMergeOrderConf();
            tmp.MergeFrom(cis);
			updating.EventLimitMergeOrderMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventLimitMergeOrderSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventLimitMergeOrder> filter,string tag=""){
		...
		public static rawEventLimitMergeOrder MustGet(int key,string tag="") {
		...
		public static rawEventLimitMergeOrder? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventLimitMergeOrder> GetByFilter(Func<rawEventLimitMergeOrder, bool> filterFunc,string tag="") {
			List<rawEventLimitMergeOrder> ret = new List<rawEventLimitMergeOrder>();
			foreach(KeyValuePair <int, rawEventLimitMergeOrder> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventLimitMergeOrder? GetOneByFilter(Func<rawEventLimitMergeOrder, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventLimitMergeOrder> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventLimitMergeOrder data) 
		{
			return data.Id;			
		}
    }
