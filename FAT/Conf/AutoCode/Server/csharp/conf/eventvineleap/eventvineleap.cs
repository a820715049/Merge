...
using EventVineLeapConf = fat.rawdata.EventVineLeapConf;
using rawEventVineLeap = fat.rawdata.EventVineLeap;
using rawEventVineLeapMapABValue = fat.rawdata.EventVineLeapMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventVineLeapVisitor
    {
		...
		public const string FileName = "EventVineLeapConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventVineLeapVisitor.SetABDefault单独设定EventVineLeap的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventVineLeapVisitor.SetABFallback单独设定EventVineLeap的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventVineLeap>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventVineLeap>>(()=> new PBC::MapField<int, rawEventVineLeap>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventVineLeap>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventVineLeapMap;
			var abMap = v.EventVineLeapMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventVineLeapMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventVineLeapMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventVineLeap> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventVineLeapMap;
			if (v.EventVineLeapMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventVineLeapMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventVineLeapMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventVineLeapConf cc) {
			foreach(KeyValuePair <string, rawEventVineLeapMapABValue> tagEntry in cc.EventVineLeapMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventVineLeapMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventVineLeapMapABValue();
					cc.EventVineLeapMapAB[tag] = ps;
				}
                var tagTmp = ps.EventVineLeapMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventVineLeap> entry in cc.EventVineLeapMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventVineLeap> entry in tagEntry.Value.EventVineLeapMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventVineLeap> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventVineLeapConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventVineLeapMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventVineLeapMapAB != null)
			{
				foreach (var entry in cc.EventVineLeapMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventVineLeapMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventVineLeapMap.MergeFrom(cis);
			Patch(updating.EventVineLeapMap);
			updating.EventVineLeapSortedKeys = genSortedKeys(updating.EventVineLeapMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventVineLeapConf();
            tmp.MergeFrom(cis);
			updating.EventVineLeapMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventVineLeapSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventVineLeap> filter,string tag=""){
		...
		public static rawEventVineLeap MustGet(int key,string tag="") {
		...
		public static rawEventVineLeap? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventVineLeap> GetByFilter(Func<rawEventVineLeap, bool> filterFunc,string tag="") {
			List<rawEventVineLeap> ret = new List<rawEventVineLeap>();
			foreach(KeyValuePair <int, rawEventVineLeap> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventVineLeap? GetOneByFilter(Func<rawEventVineLeap, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventVineLeap> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventVineLeap data) 
		{
			return data.Id;			
		}
    }
