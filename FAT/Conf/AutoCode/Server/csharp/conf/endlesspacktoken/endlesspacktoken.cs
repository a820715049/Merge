...
using EndlessPackTokenConf = fat.rawdata.EndlessPackTokenConf;
using rawEndlessPackToken = fat.rawdata.EndlessPackToken;
using rawEndlessPackTokenMapABValue = fat.rawdata.EndlessPackTokenMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EndlessPackTokenVisitor
    {
		...
		public const string FileName = "EndlessPackTokenConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EndlessPackTokenVisitor.SetABDefault单独设定EndlessPackToken的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EndlessPackTokenVisitor.SetABFallback单独设定EndlessPackToken的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEndlessPackToken>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEndlessPackToken>>(()=> new PBC::MapField<int, rawEndlessPackToken>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEndlessPackToken>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EndlessPackTokenMap;
			var abMap = v.EndlessPackTokenMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EndlessPackTokenMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EndlessPackTokenMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEndlessPackToken> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EndlessPackTokenMap;
			if (v.EndlessPackTokenMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EndlessPackTokenMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEndlessPackTokenMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EndlessPackTokenConf cc) {
			foreach(KeyValuePair <string, rawEndlessPackTokenMapABValue> tagEntry in cc.EndlessPackTokenMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EndlessPackTokenMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEndlessPackTokenMapABValue();
					cc.EndlessPackTokenMapAB[tag] = ps;
				}
                var tagTmp = ps.EndlessPackTokenMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEndlessPackToken> entry in cc.EndlessPackTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEndlessPackToken> entry in tagEntry.Value.EndlessPackTokenMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EndlessPackToken> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EndlessPackTokenConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EndlessPackTokenMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EndlessPackTokenMapAB != null)
			{
				foreach (var entry in cc.EndlessPackTokenMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EndlessPackTokenMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EndlessPackTokenMap.MergeFrom(cis);
			Patch(updating.EndlessPackTokenMap);
			updating.EndlessPackTokenSortedKeys = genSortedKeys(updating.EndlessPackTokenMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EndlessPackTokenConf();
            tmp.MergeFrom(cis);
			updating.EndlessPackTokenMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EndlessPackTokenSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEndlessPackToken> filter,string tag=""){
		...
		public static rawEndlessPackToken MustGet(int key,string tag="") {
		...
		public static rawEndlessPackToken? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEndlessPackToken> GetByFilter(Func<rawEndlessPackToken, bool> filterFunc,string tag="") {
			List<rawEndlessPackToken> ret = new List<rawEndlessPackToken>();
			foreach(KeyValuePair <int, rawEndlessPackToken> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEndlessPackToken? GetOneByFilter(Func<rawEndlessPackToken, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEndlessPackToken> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEndlessPackToken data) 
		{
			return data.Id;			
		}
    }
