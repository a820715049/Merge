...
using MergeDifficultyConf = fat.rawdata.MergeDifficultyConf;
using rawMergeDifficulty = fat.rawdata.MergeDifficulty;
using rawMergeDifficultyMapABValue = fat.rawdata.MergeDifficultyMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class MergeDifficultyVisitor
    {
		...
		public const string FileName = "MergeDifficultyConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过MergeDifficultyVisitor.SetABDefault单独设定MergeDifficulty的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过MergeDifficultyVisitor.SetABFallback单独设定MergeDifficulty的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawMergeDifficulty>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawMergeDifficulty>>(()=> new PBC::MapField<int, rawMergeDifficulty>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawMergeDifficulty>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().MergeDifficultyMap;
			var abMap = v.MergeDifficultyMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.MergeDifficultyMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].MergeDifficultyMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawMergeDifficulty> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().MergeDifficultyMap;
			if (v.MergeDifficultyMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.MergeDifficultyMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawMergeDifficultyMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(MergeDifficultyConf cc) {
			foreach(KeyValuePair <string, rawMergeDifficultyMapABValue> tagEntry in cc.MergeDifficultyMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.MergeDifficultyMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawMergeDifficultyMapABValue();
					cc.MergeDifficultyMapAB[tag] = ps;
				}
                var tagTmp = ps.MergeDifficultyMap;
				// 基础数据
				foreach(KeyValuePair <int, rawMergeDifficulty> entry in cc.MergeDifficultyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawMergeDifficulty> entry in tagEntry.Value.MergeDifficultyMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.MergeDifficulty> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(MergeDifficultyConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.MergeDifficultyMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.MergeDifficultyMapAB != null)
			{
				foreach (var entry in cc.MergeDifficultyMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.MergeDifficultyMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.MergeDifficultyMap.MergeFrom(cis);
			Patch(updating.MergeDifficultyMap);
			updating.MergeDifficultySortedKeys = genSortedKeys(updating.MergeDifficultyMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new MergeDifficultyConf();
            tmp.MergeFrom(cis);
			updating.MergeDifficultyMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().MergeDifficultySortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawMergeDifficulty> filter,string tag=""){
		...
		public static rawMergeDifficulty MustGet(int key,string tag="") {
		...
		public static rawMergeDifficulty? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawMergeDifficulty> GetByFilter(Func<rawMergeDifficulty, bool> filterFunc,string tag="") {
			List<rawMergeDifficulty> ret = new List<rawMergeDifficulty>();
			foreach(KeyValuePair <int, rawMergeDifficulty> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawMergeDifficulty? GetOneByFilter(Func<rawMergeDifficulty, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawMergeDifficulty> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawMergeDifficulty data) 
		{
			return data.Id;			
		}
    }
