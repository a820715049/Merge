...
using FeatureUnlockConf = fat.rawdata.FeatureUnlockConf;
using rawFeatureUnlock = fat.rawdata.FeatureUnlock;
using rawFeatureUnlockSliceABValue = fat.rawdata.FeatureUnlockSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class FeatureUnlockVisitor
    {
		...
		public const string FileName = "FeatureUnlockConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过FeatureUnlockVisitor.SetABDefault单独设定FeatureUnlock的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过FeatureUnlockVisitor.SetABFallback单独设定FeatureUnlock的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawFeatureUnlock>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawFeatureUnlock>>(()=>new PBC::RepeatedField<rawFeatureUnlock>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawFeatureUnlock>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().FeatureUnlockSlice;
			var abMap = v.FeatureUnlockSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.FeatureUnlockSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].FeatureUnlockSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawFeatureUnlock> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().FeatureUnlockSlice;
			if (v.FeatureUnlockSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.FeatureUnlockSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawFeatureUnlockSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(FeatureUnlockConf cc) {
			foreach(KeyValuePair <string, rawFeatureUnlockSliceABValue> tagEntry in cc.FeatureUnlockSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.FeatureUnlockSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawFeatureUnlockSliceABValue();
					cc.FeatureUnlockSliceAB[tag] = ps;
				}
                var tagTmp = ps.FeatureUnlockSlice;
				// 基础数据
				for(int i=0; i<cc.FeatureUnlockSlice.Count; ++i) 
				{
					tagTmp.Add(cc.FeatureUnlockSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.FeatureUnlockSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.FeatureUnlock> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.FeatureUnlockSlice.MergeFrom(cis);
			Patch(updating.FeatureUnlockSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new FeatureUnlockConf();
            tmp.MergeFrom(cis);
			updating.FeatureUnlockSlice = tmp;
		...
		public static void Range(Action<int, rawFeatureUnlock> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawFeatureUnlock MustGetByIndex(int index,string tag="") {
		...
		public static rawFeatureUnlock? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawFeatureUnlock> GetByFilter(Func<rawFeatureUnlock,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawFeatureUnlock>();
		...
		static public rawFeatureUnlock? GetOneByFilter(Func<rawFeatureUnlock,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
