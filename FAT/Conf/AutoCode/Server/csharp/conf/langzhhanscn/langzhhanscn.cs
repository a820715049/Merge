...
using LangZhHansCnConf = fat.rawdata.LangZhHansCnConf;
using rawLangZhHansCn = fat.rawdata.LangZhHansCn;
using rawLangZhHansCnSliceABValue = fat.rawdata.LangZhHansCnSliceABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class LangZhHansCnVisitor
    {
		...
		public const string FileName = "LangZhHansCnConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过LangZhHansCnVisitor.SetABDefault单独设定LangZhHansCn的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过LangZhHansCnVisitor.SetABFallback单独设定LangZhHansCn的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;
		private static Lazy<PBC::RepeatedField<rawLangZhHansCn>> emptyAllReadonly = new Lazy<PBC::RepeatedField<rawLangZhHansCn>>(()=>new PBC::RepeatedField<rawLangZhHansCn>());
		// 加载标记
		...
		// All 返回全量数据列表
		public static (PBC::RepeatedField<rawLangZhHansCn>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().LangZhHansCnSlice;
			var abMap = v.LangZhHansCnSliceAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.LangZhHansCnSlice,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].LangZhHansCnSlice,abUsing);
		}
		// All 返回全量数据列表
		public static PBC::RepeatedField<rawLangZhHansCn> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().LangZhHansCnSlice;
			if (v.LangZhHansCnSliceAB == null) {
				return new List<string>();
			}
			return new List<string>(v.LangZhHansCnSliceAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawLangZhHansCnSliceABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(LangZhHansCnConf cc) {
			foreach(KeyValuePair <string, rawLangZhHansCnSliceABValue> tagEntry in cc.LangZhHansCnSliceABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.LangZhHansCnSliceAB.TryGetValue(tag, out var ps))
				{
					ps = new rawLangZhHansCnSliceABValue();
					cc.LangZhHansCnSliceAB[tag] = ps;
				}
                var tagTmp = ps.LangZhHansCnSlice;
				// 基础数据
				for(int i=0; i<cc.LangZhHansCnSlice.Count; ++i) 
				{
					tagTmp.Add(cc.LangZhHansCnSlice[i]);
				}
				// patch 数据
				var patchData = tagEntry.Value.LangZhHansCnSlice;
		...
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::RepeatedField<fat.rawdata.LangZhHansCn> tableData)
		{
		}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.LangZhHansCnSlice.MergeFrom(cis);
			Patch(updating.LangZhHansCnSlice);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new LangZhHansCnConf();
            tmp.MergeFrom(cis);
			updating.LangZhHansCnSlice = tmp;
		...
		public static void Range(Action<int, rawLangZhHansCn> filter,string tag="") {
			var pl = All(tag);
		...
		public static rawLangZhHansCn MustGetByIndex(int index,string tag="") {
		...
		public static rawLangZhHansCn? GetByIndex(int index,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (index < 0 || index >= pl.Count)
			{
		...
		static public List<rawLangZhHansCn> GetByFilter(Func<rawLangZhHansCn,bool> filterFunc,string tag="") {
			var pl = All(tag);
			var ret = new List<rawLangZhHansCn>();
		...
		static public rawLangZhHansCn? GetOneByFilter(Func<rawLangZhHansCn,bool> filterFunc,string tag="")  {
			var (pl,tagActual) = _allWithTag(tag);
			foreach (var val in pl)
			{
				if (filterFunc(val))
