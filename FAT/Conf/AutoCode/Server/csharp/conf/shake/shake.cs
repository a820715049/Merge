...
using ShakeConf = fat.rawdata.ShakeConf;
using rawShake = fat.rawdata.Shake;
using rawShakeMapABValue = fat.rawdata.ShakeMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class ShakeVisitor
    {
		...
		public const string FileName = "ShakeConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过ShakeVisitor.SetABDefault单独设定Shake的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过ShakeVisitor.SetABFallback单独设定Shake的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawShake>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawShake>>(()=> new PBC::MapField<int, rawShake>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawShake>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().ShakeMap;
			var abMap = v.ShakeMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.ShakeMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].ShakeMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawShake> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().ShakeMap;
			if (v.ShakeMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.ShakeMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawShakeMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(ShakeConf cc) {
			foreach(KeyValuePair <string, rawShakeMapABValue> tagEntry in cc.ShakeMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.ShakeMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawShakeMapABValue();
					cc.ShakeMapAB[tag] = ps;
				}
                var tagTmp = ps.ShakeMap;
				// 基础数据
				foreach(KeyValuePair <int, rawShake> entry in cc.ShakeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawShake> entry in tagEntry.Value.ShakeMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.Shake> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(ShakeConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.ShakeMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.ShakeMapAB != null)
			{
				foreach (var entry in cc.ShakeMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.ShakeMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.ShakeMap.MergeFrom(cis);
			Patch(updating.ShakeMap);
			updating.ShakeSortedKeys = genSortedKeys(updating.ShakeMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new ShakeConf();
            tmp.MergeFrom(cis);
			updating.ShakeMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().ShakeSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawShake> filter,string tag=""){
		...
		public static rawShake MustGet(int key,string tag="") {
		...
		public static rawShake? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawShake> GetByFilter(Func<rawShake, bool> filterFunc,string tag="") {
			List<rawShake> ret = new List<rawShake>();
			foreach(KeyValuePair <int, rawShake> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawShake? GetOneByFilter(Func<rawShake, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawShake> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawShake data) 
		{
			return data.Id;			
		}
    }
