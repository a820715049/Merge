...
using EventStampRoundConf = fat.rawdata.EventStampRoundConf;
using rawEventStampRound = fat.rawdata.EventStampRound;
using rawEventStampRoundMapABValue = fat.rawdata.EventStampRoundMapABValue;
using PBC = global::Google.Protobuf.Collections;
using ConfManager = fat.conf.conf_loader.ConfManager;

namespace fat.conf
{
    public partial class EventStampRoundVisitor
    {
		...
		public const string FileName = "EventStampRoundConf";
		// 默认ABTag，可以通过ConfManager.SetABDefault设定全局默认的AB Tag，也可以通过EventStampRoundVisitor.SetABDefault单独设定EventStampRound的默认tag
		private static string DefaultABTag = ConfManager.TagFoundation;
		// 默认AB fallback策略，可以通过ConfManager.SetABFallback设定全局默认的AB fallback策略，也可以通过EventStampRoundVisitor.SetABFallback单独设定EventStampRound的fallback策略
		private static Dictionary<string, List<string>>? abFallback = null;	
		private static Lazy<PBC::MapField<int, rawEventStampRound>> emptyAllReadonly = new Lazy<PBC::MapField<int, rawEventStampRound>>(()=> new PBC::MapField<int, rawEventStampRound>());
		// 加载标记
		...
		// All 返回全量数据Map
		public static (PBC::MapField<int, rawEventStampRound>,string) _allWithTag(string abTag="")
		{
			if (!tryLazyLoad())
            {
				ConfManager.ExceptionOnLazyLoadFailed(FileName,"lazy load failed");
            }
			var v = ConfManager.GetCurrent().EventStampRoundMap;
			var abMap = v.EventStampRoundMapAB;
			var abUsing = abTagTryFallback(abTag==""?DefaultABTag:abTag,abMap);
			if(abUsing == ConfManager.TagFoundation){
				return (v.EventStampRoundMap,abUsing);
			}else if(abUsing == ""){
				return (emptyAllReadonly.Value,abUsing);
			}
			return (abMap[abUsing].EventStampRoundMap,abUsing);
		}
		// All 返回全量数据Map
		public static PBC::MapField<int, rawEventStampRound> All(string abTag="")
		{
		...
		public static List<string> ABList() {
			...
			var v = ConfManager.GetCurrent().EventStampRoundMap;
			if (v.EventStampRoundMapAB == null) {
				return new List<string>();
			}
			return new List<string>(v.EventStampRoundMapAB.Keys);
		}
		// 基础标签或abMap含数据则返回; 否则进入fallback策略，如未设定则返回空.
		public static string abTagTryFallback(string ab, PBC::MapField<string, rawEventStampRoundMapABValue> abMap)
		{
		...
		// 检测ab patch
		public static void Patch(EventStampRoundConf cc) {
			foreach(KeyValuePair <string, rawEventStampRoundMapABValue> tagEntry in cc.EventStampRoundMapABPatch) 
			{
				var tag = tagEntry.Key;
				if (!cc.EventStampRoundMapAB.TryGetValue(tag, out var ps))
				{
					ps = new rawEventStampRoundMapABValue();
					cc.EventStampRoundMapAB[tag] = ps;
				}
                var tagTmp = ps.EventStampRoundMap;
				// 基础数据
				foreach(KeyValuePair <int, rawEventStampRound> entry in cc.EventStampRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
				// patch数据
				foreach(KeyValuePair <int, rawEventStampRound> entry in tagEntry.Value.EventStampRoundMap) 
				{
					tagTmp[entry.Key] = entry.Value;
				}
			}
		}
		private static void __UpdateReferenceWithTableData(Dictionary<string, object> ctx,ConfManager current,PBC::MapField<int,fat.rawdata.EventStampRound> tableData)
		{
		}
		...
		private static Dictionary<string, List<int>> genSortedKeys(EventStampRoundConf cc) 
		{
			...
			if (!ret.TryGetValue(ConfManager.TagFoundation, out var listFoundation))
			{
				...
				listFoundation.AddRange(cc.EventStampRoundMap.Keys);
				ret[ConfManager.TagFoundation] = listFoundation;
			}

			if (cc.EventStampRoundMapAB != null)
			{
				foreach (var entry in cc.EventStampRoundMapAB)
				{
					if (!ret.TryGetValue(entry.Key, out var list))
					{
						list = new List<int>();
						list.AddRange(entry.Value.EventStampRoundMap.Keys);
						ret[entry.Key] = list;
					}
				}
			}
		...
		public static void  LoadFromStream(Google.Protobuf.CodedInputStream cis) {
			...
			updating.EventStampRoundMap.MergeFrom(cis);
			Patch(updating.EventStampRoundMap);
			updating.EventStampRoundSortedKeys = genSortedKeys(updating.EventStampRoundMap);
		...
		public static void Load(Dictionary<string, object> ctx, string key, Google.Protobuf.CodedInputStream cis)
        {
           ...
           if (updating == null) {
            ...
            var tmp = new EventStampRoundConf();
            tmp.MergeFrom(cis);
			updating.EventStampRoundMap = tmp;
		...
		{
			var v = ConfManager.GetCurrent().EventStampRoundSortedKeys;
			var abUsing = (tag=="")?DefaultABTag:tag;
			return v[abUsing];
		}
		public static void Range(Action<int, rawEventStampRound> filter,string tag=""){
		...
		public static rawEventStampRound MustGet(int key,string tag="") {
		...
		public static rawEventStampRound? Get(int key,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			if (pl.TryGetValue(key, out var value))
			{
				return value;
		...
		public static List<rawEventStampRound> GetByFilter(Func<rawEventStampRound, bool> filterFunc,string tag="") {
			List<rawEventStampRound> ret = new List<rawEventStampRound>();
			foreach(KeyValuePair <int, rawEventStampRound> entry in All(tag)) {
				if(filterFunc(entry.Value))
					ret.Add(entry.Value);
			}
			return ret;
		}
		static public rawEventStampRound? GetOneByFilter(Func<rawEventStampRound, bool> filterFunc,string tag="") {
			var (pl,tagActual) = _allWithTag(tag);
			foreach(KeyValuePair <int, rawEventStampRound> entry in pl) {
				if(filterFunc(entry.Value))
					return entry.Value;
			}
		...
		static public int MapKey(rawEventStampRound data) 
		{
			return data.Id;			
		}
    }
