...
namespace FAT
{
    public class PackEndlessWishBoard : GiftPack
    {
        //礼包数据结构
        public class EndlessPkgData
        {
            public int BelongPkgId;     //所属礼包id EventWishEndlessInfo.Id
        ...
        //配置数据
        private EventWishEndless endlessPackConf;
        ...
        {
            Lite = lite_;
            endlessPackConf = GetEventWishEndless(lite_.Param);
        ...
        private void _InitInfoWithUserGrade()
        {
            ...
            {
                var pkgConf = GetOneEventWishEndlessInfoByFilter(x => x.Id == pkgId);
                if (pkgConf != null)
                {
        ...
        public void TryGetReward(int targetIndex)
        {
            ...
            if (iapId == 0)
            {
                using (ObjectPool<List<RewardCommitData>>.GlobalPool.AllocStub(out var rewards))
                {
                    ...
                    //打点
                    DataTracker.wish_endless_reward.Track(this, _curRecIndex, true);
                }
            }
            //付费奖励
            else if (iapId > 0)
            {
                _purchaseSuccCb = (rewards) =>
                {
                    ...
                    MessageCenter.Get<MSG.GAME_ENDLESS_PGK_REC_SUCC>().Dispatch(targetIndex, rewards, null);
                    //打点
                    DataTracker.wish_endless_reward.Track(this, _curRecIndex, false);
                };
            ...
            for (int i = 0; i < pkgDataCount; i++)
            {
                if (i > payIndex && i < nextPayIndex)
                {
                    ...
                    if (pkgData.IapId == 0)
                    {
                        ...
                        //打点
                        DataTracker.wish_endless_reward.Track(this, i + 1, true);
                    }
                }
            }
        }
                ...
                for (int i = 0; i < _curPkgDataList.Count; i++)
                {
                    if (i >= _curRecIndex && i < curNextPayIndex)
                    {
                        ...
                        if (pkgData.IapId == 0)
                        {
                            ...
                            index++;
                            //打点
                            DataTracker.wish_endless_reward.Track(this, index, true);
                        }
                    }
                }
            }
