    ...
    public class FightBoardItemSpawnBonusHandler : ISpawnBonusHandler
    {
        ...
        private void SimulateSpawn(SpawnBonusContext context)
        {
            ...
            if (output.itemId > 0 && output.num > 0)
            {
                ...
                //往钓鱼棋盘上发奖励
                var reward = Game.Manager.rewardMan.BeginReward(output.itemId, finalNum, ReasonString.fight_getitem);
                var pos = BoardUtility.GetWorldPosByCoord(from.coord);
                UIFlyUtility.FlyRewardSetType(reward, pos, FlyType.FightBoardEntry);
                DataTracker.event_fight_get_item.Track(curActivity, Game.Manager.mergeBoardMan.activeWorld.activeBoard.boardId, output.itemId, ItemUtility.GetItemLevel(output.itemId), finalNum);
            }
        }
    }
}
