...
namespace FAT
{
    public class MBMiniBoardReward : MonoBehaviour
    {
        ...
        public void SetUp()
        {
            if (Game.Manager.miniBoardMan.World.rewardCount > 0)
            {
        ...
        {
            var item = Game.Manager.miniBoardMan.World.nextRewardItem;
            if (item == null)
            {
                _root.SetActive(false);
                if (now)
            ...
            _num.text = Game.Manager.miniBoardMan.World.rewardCount.ToString();
        }
