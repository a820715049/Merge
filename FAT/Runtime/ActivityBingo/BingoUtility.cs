...
namespace FAT
{
    ...
    /// </summary>
    public static class BingoUtility
    {
        #region 配置获取方法
        ...
        public static void FillHighestLeveItemByCategory(List<int> categoryIds, List<int> results)
        {
            results.Clear();
        ...
        public static BingoState GetBingoState(List<BingoItem> bingoItems, BingoItem bingo)
        {
            var state = BingoState.None;
            state.SetFlag(BingoState.ItemCompleted);
            if (bingoItems.Where(item => item.CoordX == bingo.CoordX).All(item => item.IsClaimed))
                state.SetFlag(BingoState.ColumnCompleted);
            if (bingoItems.Where(item => item.CoordY == bingo.CoordY).All(item => item.IsClaimed))
                state.SetFlag(BingoState.RowCompleted);
            if (bingo.IsDiagonal1 && bingoItems.Where(item => item.IsDiagonal1).All(item => item.IsClaimed))
                state.SetFlag(BingoState.Diagonal1Completed);
            if (bingo.IsDiagonal2 && bingoItems.Where(item => item.IsDiagonal2).All(item => item.IsClaimed))
                state.SetFlag(BingoState.Diagonal2Completed);
            if (bingoItems.All(item => item.IsClaimed))
                state.SetFlag(BingoState.FullHouse);
            return state;
        }

        public static void UpdateBingoItemState(List<BingoItem> bingoItems, BingoState state, BingoItem bingo)
        {
            bingo.IsClaimed = state.HasFlag(BingoState.ItemCompleted);
            ...
            if (state.HasFlag(BingoState.Diagonal1Completed))
                bingoItems.Where(item => item.IsDiagonal1).ToList().ForEach(item => item.HasBingo = true);
            if (state.HasFlag(BingoState.Diagonal2Completed))
