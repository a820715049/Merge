 ...
 *@Desc:
 *@Created Time:2024.06.04 星期二 02:09:31
 */
...
namespace FAT
{
    public class UIDecorateRes : UIBase
    {
        ...
        //代币
        [SerializeField] private TMP_Text tokenNumTmp;
        ...
        {
            var token = fat.conf.Data.GetObjBasic(Game.Manager.decorateMan.Activity.confD.RequireScoreId);
            tokenImg.SetImage(token.Icon);
            game = transform.Find("Content/Target/game").gameObject;
        ...
        protected override void OnPreOpen()
        {
            ...
            otherNode.gameObject.SetActive(false);
            _RefreshAllStatus();
        }
        ...
        protected override void OnAddListener()
        {
            ...
            MessageCenter.Get<MSG.DECORATE_SCORE_FLY>().AddListener(_RefreshToken);
            MessageCenter.Get<MSG.DECORATE_SCORE_UPDATE>().AddListener(_RefreshToken);
            MessageCenter.Get<MSG.DECORATE_RES_UI_STATE_CHANGE>().AddListener(_OnShowStateChange);
        }
        ...
        protected override void OnRemoveListener()
        {
            ...
            MessageCenter.Get<MSG.DECORATE_SCORE_FLY>().RemoveListener(_RefreshToken);
            MessageCenter.Get<MSG.DECORATE_SCORE_UPDATE>().RemoveListener(_RefreshToken);
            MessageCenter.Get<MSG.DECORATE_RES_UI_STATE_CHANGE>().RemoveListener(_OnShowStateChange);
        }

        public void ShowGameNode()
        {
            var seq = DOTween.Sequence();
            seq.Append(game.GetComponent<Image>().DOFade(1, 0.7f));
            seq.Append(game.GetComponent<Image>().DOFade(0, 0.5f));
        }
        ...
        {
            tokenNumTmp.text = Game.Manager.decorateMan.Score.ToString();
        }

        private void _RefreshToken(ActivityLike act, int num)
        {
            if(num<0)
                tokenNumTmp.text = Game.Manager.decorateMan.Score.ToString();
        }
