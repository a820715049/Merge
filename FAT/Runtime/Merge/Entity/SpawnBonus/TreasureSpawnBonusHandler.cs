 ...
 * @LastEditTime: 2024/9/27 16:49:54
 * Description: 消耗体力后 生成对应积分
 */

namespace FAT.Merge
{
    public class TreasureSpawnBonusHandler : ISpawnBonusHandler
    {
        public int priority;
        private ActivityTreasure activityTreasure;

        public TreasureSpawnBonusHandler(ActivityTreasure activity)
        {
            activityTreasure = activity;
        }
        ...
        void ISpawnBonusHandler.Process(SpawnBonusContext context)
        {
            ...
            if (context.from != null && context.energyCost > 0)
            {
                ...
                {
                    if (activityTreasure.ConfD.Cost == comp.firstCost.Cost)
