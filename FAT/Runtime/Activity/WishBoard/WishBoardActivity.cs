...
namespace FAT
{
    public class WishBoardActivity : ActivityLike, IBoardEntry, IBoardArchive, IActivityUpdate, IActivityOrderHandler, ISpawnEffectWithTrail, IExternalOutput
    {
        ...
        bool IActivityOrderHandler.OnPreUpdate(OrderData order, IOrderHelper helper, MergeWorldTracer tracer)
        {
            ...
            if (state == null || state.Value != Id)
            {
                ...
                changed = true;
                RecordStateHelper.UpdateRecord((int)OrderParamType.ScoreEventIdBR, Id, order.Record.Extra);
                RecordStateHelper.UpdateRecord((int)OrderParamType.ScoreBR, scoreTotal, order.Record.Extra);
                RecordStateHelper.UpdateRecord((int)OrderParamType.ScoreRewardBR, scoreId, order.Record.Extra);
            }
            return changed;
        }
        ...
        //检查是否是棋盘专属棋子
        public bool CheckIsWishBoardItem(int itemId)
        {
        ...
        public void OnNewItemShow(Merge.Item itemData)
        {
            ...
            if (CheckIsWishBoardItem(itemData.config.Id))
            {
        ...
        private void _MoveDownBoard(Board board, int downRowCount, int detailParam)
        {
            ...
            {
                if (BoardActivityUtility.FillWishBoardRowConfStr(detailParam, rowItems, _curDepthIndex, downRowCount))
                {
