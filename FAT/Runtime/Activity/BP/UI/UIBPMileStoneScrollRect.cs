...
namespace FAT
{
    ...
    public class UIBPMileStoneScrollRect : UICommonScrollRect<BPMileStoneCellViewData, UICommonScrollRectDefaultContext>
    {
        ...
        {
            var count = cellContainer.childCount;
            var cellList = new List<UIBPMileStoneCell>(count);

            // 收集所有 cell
            for (var i = 0; i < count; i++)
            {
                var cell = cellContainer.GetChild(i).GetComponent<UIBPMileStoneCell>();
                if (cell != null)
                {
                    cellList.Add(cell);
                }
            ...
            // 根据 Index 进行排序，Index 大的（等级高的）排在前面
            cellList.Sort((a, b) => b.Index.CompareTo(a.Index));

            // 重新设置 cell 的层级顺序
            for (var i = 0; i < cellList.Count; i++)
            {
                cellList[i].transform.SetSiblingIndex(i);
            }
        }
            ...
            Action onComplete = null)
        {
            ...
            MessageCenter.Get<UI_BP_MILESTONECELL_PLAY_PROGRESS>().Dispatch(duration, Context.SelectedIndex, cellIndex);
