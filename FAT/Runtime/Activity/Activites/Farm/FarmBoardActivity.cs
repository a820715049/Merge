...
namespace FAT
{
    public class FarmBoardActivity : ActivityLike, IBoardEntry, IBoardArchive, IActivityUpdate, IActivityOrderHandler, IBoardActivityHandbook, IBoardActivityRowConf
    {
        ...
        {
            var detailConf = Game.Manager.configMan.GetEventFarmBoardDetailConfig(detailId);
            return detailConf?.BoardRowId;
        }

        string IBoardActivityRowConf.GetRowConfStr(int rowId)
        {
            var rowConf = Game.Manager.configMan.GetEventFarmRowConfig(rowId);
            return rowConf?.UpMiniRow ?? "";
        }
