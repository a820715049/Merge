...
namespace FAT
{
    public class MBFarmBoardCloudHolder : MonoBehaviour
    {
        ...
        private string pool_key_item()
        {
            return $"farm_board_cloud_{_activity.Id}";
        }
        ...
        public void InitOnPreOpen(FarmBoardActivity act)
        {
            ...
            {
                GameObjectPoolManager.Instance.PreparePool(pool_key_item(), cloudViewPrefab);
            }
        ...
        private void _PrepareGrid()
        {
            ...
            for (int j = 0; j < height; j++)
            {
                for (int i = 0; i < width; i++)
                {
                    ...
                    {
                        go = GameObjectPoolManager.Instance.CreateObject(pool_key_item(), mRoot);
                        go.SetActive(true);
                        ...
                        // 云层放大放在prefab里做 换皮有的不需要放大
                        // trans.sizeDelta = new Vector2(cellSize, cellSize) * 1.1f; // 云层要比格子大一点
                        trans.sizeDelta = new Vector2(cellSize, cellSize);
            ...
            for (int j = 0; j < height; j++)
            {
                for (int i = 0; i < width; i++)
                {
                    ...
                    else
                    {
                        if (TryGetBelongCloud(clouds, i, j, out var cloud))
                        {
                            var go = GameObjectPoolManager.Instance.CreateObject(pool_key_item(), mRoot);
                            go.SetActive(true);
                            var cloudView = go.GetComponent<MBCloudView>();
                            cloudView.Init(cloud, i, j);
        ...
        private void _ReleaseGrid()
        {
            ...
            {
                GameObjectPoolManager.Instance.ReleaseObject(pool_key_item(), mCellDict[coord].gameObject);
            }
