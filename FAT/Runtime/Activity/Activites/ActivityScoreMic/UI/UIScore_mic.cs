...
namespace FAT
{
    public class UIScore_mic : UIBase, INavBack
    {
        ...
        public void HelpHide() {
            var r = helpRoot.transform;
            r.localScale = Vector3.one;
            hide = r.DOScale(Vector3.one * 0.1f, infoFadeDuration).SetEase(Ease.OutFlash)
                .OnComplete(() => {
                    helpRoot.gameObject.SetActive(false);
                    block.gameObject.SetActive(false);
                });
        ...
        private void RefreshList(int currentMilestoneIndex)
        {
            ...
            scroll.verticalNormalizedPosition = last ? 1 : 0;
        }
        ...
        private void PreCoPlayProgressAnimation()
        {
            ...
            {
                var last = 1 + offset + milestoneIndex - startMilestoneIndex;
                if (last > cellList.Count) continue;
                var uiItem = cellList[^(last)].GetComponent<UIMicItem>();
        ...
        private IEnumerator PlayCellAnimation(int index, bool isStart, bool isEnd)
        {
            ...
            {
                scroll.DOVerticalNormalizedPos(1, ToThisTime);
            }
