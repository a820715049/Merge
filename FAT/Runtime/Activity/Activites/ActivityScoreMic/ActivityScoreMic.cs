...
namespace FAT
{
    public class ActivityScoreMic : ActivityLike, IBoardEntry, IActivityOrderHandler
    {
        ...
        private void _RefreshScoreEntity()
        {
            ...
            if (conf != null)
            {
                ...
                _scoreEntity.Setup(TotalScore, this, Conf.Token, Conf.ExtraScore,
                    ReasonString.score_mic, "", Conf.BoardId, false);
            }
        }
        ...
        private void _UpdateMilestoneInfo(int scoreNum)
        {
            ...
            while(true);
            //检查是否有升级 如果有则弹窗，同时传入升级获得的奖励，默认每次升级最多只会有一个RewardCommitData
            if (hasLevelUp)
            {
                Game.Manager.screenPopup.TryQueue(MainPopup.popup, (PopupType)(-1), listT);
            }
            //检查里程碑是否满级 若满级则活动直接结束
            if (IsComplete())
            {
